...
use StellarWP\ebox\StellarWP\DB\QueryBuilder\Clauses\MetaTable;
use StellarWP\ebox\StellarWP\DB\QueryBuilder\Clauses\RawSQL;
use StellarWP\ebox\StellarWP\DB\QueryBuilder\JoinQueryBuilder;
...
trait MetaQuery {
	 ...
	 * Select meta columns
	 *
	 * @since 1.0.0 optimize team concat functionality
	 * @since 1.0.0 add team concat functionality
	 * @since 1.0.0
	 *
	 * @param  string|RawSQL  $table
	...
	public function attachMeta( $table, $foreignKey, $primaryKey, ...$columns ) {
		...
		foreach ( $columns as $i => $definition ) {
			...
			// Check if we have meta columns that dev wants to team concat
			if ( $concat ) {
				/**
				 * Include foreign key to prevent errors if sql_mode is only_full_team_by
				 *
				 * @see https://dev.mysql.com/doc/refman/5.7/en/team-by-handling.html
				 */
				$this->teamBy( $foreignKey );

				// Team concat same key values into faux array
