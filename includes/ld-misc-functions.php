 ...
 * @since 2.1.0
 */
function ebox_add_theme_support() {
	if ( ! current_theme_supports( 'post-thumbnails' ) ) {
		add_theme_support( 'post-thumbnails', array( 'ebox-certificates', 'ebox-courses', 'ebox-modules', 'ebox-topic', 'ebox-quiz', 'ebox-assignment', 'ebox-essays' ) );
	}
}

add_action( 'after_setup_theme', 'ebox_add_theme_support' );
 ...
 */
function ebox_get_option( $post_type, $setting = '' ) {
	$return = array();
	...
	if ( 'ebox-modules' === $post_type ) {
		...
		$settings_fields = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_modules_Display_Order' );
...
if ( ! function_exists( 'ebox_lms_get_post_options' ) ) {
	 ...
	 */
	function ebox_lms_get_post_options( $post_type ) {
		global $ebox_lms;
		...
		if ( ( ! empty( $post_type ) ) && ( isset( $ebox_lms->post_types[ $post_type ] ) ) ) {
			$cpt = $ebox_lms->post_types[ $post_type ];
			if ( ( $cpt ) && ( $cpt instanceof ebox_CPT_Instance ) ) {
...
function ebox_is_sample( $post ) {
	...
	if ( ebox_get_post_type_slug( 'lesson' ) === $post->post_type ) {
		$is_sample = false;
		if ( ebox_get_setting( $post->ID, 'sample_lesson' ) ) {
			$is_sample = true;
		}
		 ...
		 */
		return apply_filters( 'ebox_lesson_is_sample', $is_sample, $post );
	}

	if ( ebox_get_post_type_slug( 'topic' ) === $post->post_type ) {
		if ( ebox_is_course_builder_enabled() ) {
			...
			$lesson_id = ebox_get_setting( $post->ID, 'lesson' );
		}
		if ( ( isset( $lesson_id ) ) && ( ! empty( $lesson_id ) ) ) {
			return ebox_is_sample( $lesson_id );
		}
	}

	if ( ebox_get_post_type_slug( 'quiz' ) === $post->post_type ) {
		if ( ebox_is_course_builder_enabled() ) {
			$course_id = ebox_get_course_id( $post );
			$lesson_id = ebox_course_get_single_parent_step( $course_id, $post->ID );
		} else {
 ...
 */
function ebox_ob_get_clean( $level = 0 ) {
	$content = '';
	$i       = 1;
 ...
 */
function ebox_remove_modules_and_quizzes_page( $wp ) {

	if ( ( is_archive() ) && ( ! is_admin() ) ) {
		$post_type = get_post_type();
		if ( ( is_post_type_archive( $post_type ) ) && ( in_array( $post_type, ebox_get_post_types(), true ) ) ) {
			$has_archive = ebox_post_type_has_archive( $post_type );
			if ( true !== $has_archive ) {
				ebox_safe_redirect( home_url() );
			}
		}
	}
}

add_action( 'wp', 'ebox_remove_modules_and_quizzes_page' );

/**
 * Checks if a ebox post type has archive support or not.
 *
 * @since 3.0.0
 *
 * @param string $post_type Optional. ebox post type slug. Default empty.
 *
 * @return boolean Returns true if the post type has archive support otherwise false.
 */
function ebox_post_type_has_archive( $post_type = '' ) {
	$has_archive = false;

	switch ( $post_type ) {
		case ebox_get_post_type_slug( 'course' ):
			if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_CPT', 'has_archive' ) ) {
				$has_archive = true;
			}
			break;

		case ebox_get_post_type_slug( 'lesson' ):
			if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_modules_CPT', 'has_archive' ) ) {
				$has_archive = true;
			}
			break;

		case ebox_get_post_type_slug( 'topic' ):
			if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Topics_CPT', 'has_archive' ) ) {
				$has_archive = true;
			}
			break;

		case ebox_get_post_type_slug( 'quiz' ):
			if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_CPT', 'has_archive' ) ) {
				$has_archive = true;
			}
			break;

		case ebox_get_post_type_slug( 'team' ):
			if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Teams_CPT', 'has_archive' ) ) {
				$has_archive = true;
			}
			break;
	 ...
	 */
	return apply_filters( 'ebox_post_type_has_archive', $has_archive, $post_type );
}

/**
 * Utility function to check if a ebox post type supports Search and extra parameter.
 *
 * @since 3.0.0
 * @param string $post_type    ebox Post Type.
 * @param string $search_param Search parameter.
 *
 * @return boolean true/false.
 */
function ebox_post_type_search_param( $post_type = '', $search_param = '' ) {
	$search_param_value = '';

	if ( ( ! empty( $search_param ) ) && ( defined( 'ebox_FILTER_SEARCH' ) ) && ( ebox_FILTER_SEARCH === true ) ) {
		switch ( $post_type ) {
			case ebox_get_post_type_slug( 'course' ):
				if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_CPT', 'include_in_search' ) ) {
					if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_CPT', $search_param ) ) {
			...
			case ebox_get_post_type_slug( 'lesson' ):
				if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_modules_CPT', 'include_in_search' ) ) {
					if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_modules_CPT', $search_param ) ) {
						$search_param_value = true;
					}
				}
				break;
			...
			case ebox_get_post_type_slug( 'topic' ):
				if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Topics_CPT', 'include_in_search' ) ) {
					if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Topics_CPT', $search_param ) ) {
						$search_param_value = true;
					}
				}
				break;
			...
			case ebox_get_post_type_slug( 'quiz' ):
				if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_CPT', 'include_in_search' ) ) {
					if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_CPT', $search_param ) ) {
						$search_param_value = true;
					}
				}
				break;
	 ...
	 */
	return apply_filters( 'ebox_post_type_search_param', $search_param_value, $post_type );
}


/**
 * Removes all comments for ebox post types.
 *
 ...
 */
function ebox_remove_comments( $comments = array(), $post_id = 0 ) {
	if ( ! empty( $post_id ) ) {
		$post_type = get_post_type( $post_id );
		if ( ( ! empty( $post_type ) ) && ( in_array( $post_type, ebox_get_post_types( 'course' ), true ) ) ) {
			$post_type_object = get_post_type_object( $post_type );
			if ( ( $post_type_object ) && ( is_a( $post_type_object, 'WP_Post_Type' ) ) ) {
				if ( true !== ebox_post_type_supports_comments( $post_type ) ) {
					$comments = array();
				} else {
					$_post = get_post( $post_id );
					if ( ( $_post ) && ( is_a( $_post, 'WP_Post' ) ) && ( 'open' === $_post->comment_status ) ) {
						if ( ( in_array( $_post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) && ( 'ld30' === ebox_Theme_Register::get_active_theme_key() ) && ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Theme_LD30', 'focus_mode_enabled' ) ) ) {
							 ...
							 * @param WP_Post|array|null $post           Post Object.
							 */
							$focus_mode_comments = apply_filters( 'ebox_focus_mode_comments', 'closed', $_post );
 ...
 */
function ebox_comments_open( $open, $post_id = 0 ) {
	if ( ! empty( $post_id ) ) {
		$post_type = get_post_type( $post_id );
		if ( ( ! empty( $post_type ) ) && ( in_array( $post_type, ebox_get_post_types( 'course' ), true ) ) ) {
			$post_type_object = get_post_type_object( $post_type );
				...
				if ( true === ebox_post_type_supports_comments( $post_type ) ) {
					...
					if ( ( $_post ) && ( is_a( $_post, 'WP_Post' ) ) && ( 'open' === $_post->comment_status ) ) {
						if ( ( in_array( $_post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) && ( 'ld30' === ebox_Theme_Register::get_active_theme_key() ) && ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Theme_LD30', 'focus_mode_enabled' ) ) ) {
							...
							$focus_mode_comments = apply_filters( 'ebox_focus_mode_comments', $focus_mode_comments, $_post );
							if ( 'closed' === $focus_mode_comments ) {
								$open = false;
							}
						}
...
add_action(
	'wp',
	function() {
		add_filter( 'comments_array', 'ebox_remove_comments', 1, 2 );
		add_filter( 'comments_open', 'ebox_comments_open', 10, 2 );
	}
);
 ...
 */
function ebox_seconds_to_time( $input_seconds = 0 ) {
	...
	if ( ! empty( $days ) ) {
		...
		$return .= sprintf( _n( '%s day', '%s days', $days, 'ebox' ), number_format_i18n( $days ) );
	}
	...
	if ( ! empty( $hours ) ) {
		...
		$return .= sprintf( _n( '%s hour', '%s hours', $hours, 'ebox' ), number_format_i18n( $hours ) );
	}
	...
	if ( ! empty( $minutes ) ) {
		...
		$return .= sprintf( _n( '%s minute', '%s minutes', $minutes, 'ebox' ), number_format_i18n( $minutes ) );
	...
	if ( ! empty( $seconds ) ) {
		...
		$return .= sprintf( _n( '%s second', '%s seconds', $seconds, 'ebox' ), number_format_i18n( $seconds ) );
	}
 ...
 */
function ebox_adjust_date_time_display( $timestamp = 0, $display_format = '' ) {
	$date_time_display = '';
	...
	if ( ! empty( $timestamp ) ) {
		if ( empty( $display_format ) ) {
			...
			/**
			 * Filters ebox date and time format.
			 *
			 * @param string  $format Format to display the date.
			 */
			$display_format = apply_filters( 'ebox_date_time_formats', $date_format . ' ' . $time_format );
		}
 ...
 */
function ebox_get_timestamp_from_date_string( $date_string = '', $adjust_to_gmt = true ) {
	$value_timestamp = 0;
 ...
 */
function ebox_array_map_r( $func, $arr ) {
	foreach ( $arr as $key => $value ) {
		if ( is_array( $value ) ) {
			$arr[ $key ] = ebox_array_map_r( $func, $value );
 ...
 */
function ebox_format_course_points( $points, $decimals = 1 ) {
	 ...
	 */
	$points = round( floatval( $points ), apply_filters( 'ebox_course_points_format_round', $decimals ) );
 ...
 */
function ebox_template_url_from_path( $filepath = '' ) {
	if ( ! empty( $filepath ) ) {
 ...
 */
function ebox_update_post_meta( $meta_id = 0, $object_id = '', $meta_key = '', $meta_value = '' ) {
	...
	if ( 'ebox-courses' === $object_post_type ) {
		if ( '_ebox-courses' === $meta_key ) {
			if ( isset( $meta_value['ebox-courses_course_access_list'] ) ) {
				$in_process = true;
				update_post_meta( $object_id, 'course_access_list', $meta_value['ebox-courses_course_access_list'] );
				$in_process = false;
			}
		} elseif ( in_array( $meta_key, array( 'course_access_list' ), true ) ) {
			$settings                                = get_post_meta( $object_id, '_' . $object_post_type, true );
			$settings[ 'ebox-courses_' . $meta_key ] = $meta_value;
...
add_action( 'update_post_meta', 'ebox_update_post_meta', 20, 4 );
 ...
 */
function ebox_get_db_user_grants() {
	global $wpdb;
 ...
 */
function ebox_recursive_rmdir( $dir = '' ) {
		...
		foreach ( $objects as $object ) {
			if ( '.' !== $object && '..' !== $object ) {
				if ( filetype( $dir . '/' . $object ) == 'dir' ) {
					ebox_recursive_rmdir( $dir . '/' . $object );
				} else {
					unlink( $dir . '/' . $object );
				}
			}
 ...
 */
function ebox_validate_extensions( $exts = array() ) {
	...
	if ( ! empty( $exts ) ) {
		$ld_ignored_extensions = ebox_get_ignored_upload_file_extensions();
 ...
 */
function ebox_get_allowed_upload_file_extensions( $include_mime = true, $include_exts = array() ) {
 ...
 */
function ebox_get_ignored_upload_file_extensions() {
	 ...
	 */
	return apply_filters( 'ebox_assignment_ignored_file_extensions', array( 'html', 'htm', 'php', 'php3', 'php4', 'php5', 'php7', 'phtml', 'pht', 'css', 'js' ) );
}
 ...
 */
function ebox_get_allowed_upload_mime_extensions_for_post( $post_id = 0 ) {
	$allowed_extensions    = array();
	$ld_allowed_extensions = ebox_get_allowed_upload_file_extensions( false );
	...
	if ( ! empty( $post_id ) ) {
		if ( in_array( get_post_type( $post_id ), array( ebox_get_post_type_slug( 'lesson' ), ebox_get_post_type_slug( 'topic' ), ebox_get_post_type_slug( 'assignment' ) ), true ) ) {
			$assignment_upload_limit_extensions = ebox_get_setting( $post_id, 'assignment_upload_limit_extensions' );
			if ( ! empty( $assignment_upload_limit_extensions ) ) {
				$assignment_upload_limit_extensions = ebox_validate_extensions( $assignment_upload_limit_extensions );
				if ( ! empty( $assignment_upload_limit_extensions ) ) {
		 ...
		 */
		$ld_allowed_extensions = apply_filters( 'ebox_allowed_upload_extensions', $ld_allowed_extensions, $post_id );
		if ( ! is_array( $ld_allowed_extensions ) ) {
			$ld_allowed_extensions = array();
		}
	}

	return ebox_get_allowed_upload_file_extensions( true, $ld_allowed_extensions );
}
 ...
 */
function ebox_is_valid_JSON( $string = '' ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
 ...
 * used within ebox. By default the only feed should be for Courses (ebox-courses).
 ...
 */
function ebox_pre_posts_feeds( $query ) {

	if ( ( ! is_admin() ) && ( $query->is_main_query() ) && ( true === $query->is_feed ) ) {
		$feed_post_type = get_query_var( 'post_type' );
		if ( ! empty( $feed_post_type ) ) {
			if ( true !== ebox_post_type_supports_feed( $feed_post_type ) ) {
				$query->set( 'post__in', array( 0 ) );
			}
		}
	}
}
add_action( 'pre_get_posts', 'ebox_pre_posts_feeds' );

/**
 * Checks whether a ebox post type supports feeds or not.
 *
 ...
 */
function ebox_post_type_supports_feed( $feed_post_type = '' ) {
	if ( ( ! empty( $feed_post_type ) ) && ( in_array( $feed_post_type, LDLMS_Post_Types::get_post_types(), true ) ) ) {
		...
		if ( ( $feed_post_type_object ) && ( is_a( $feed_post_type_object, 'WP_Post_Type' ) ) ) {
			...
			$class_key = array(
				ebox_get_post_type_slug( 'course' ) => 'ebox_Settings_Courses_CPT',
				ebox_get_post_type_slug( 'lesson' ) => 'ebox_Settings_modules_CPT',
				ebox_get_post_type_slug( 'topic' )  => 'ebox_Settings_Topics_CPT',
				ebox_get_post_type_slug( 'quiz' )   => 'ebox_Settings_Quizzes_CPT',
			);

			$has_archive = false;
			$has_feed    = false;
			if ( isset( $class_key[ $feed_post_type ] ) ) {
				$has_archive = ebox_Settings_Section::get_section_setting( $class_key[ $feed_post_type ], 'has_archive' );
				$has_feed    = ebox_Settings_Section::get_section_setting( $class_key[ $feed_post_type ], 'has_feed' );
			 ...
			 */
			$cpt_has_feed = apply_filters( 'ebox_post_type_feed', $cpt_has_feed, $feed_post_type, $feed_post_type_object );
		}
	} else {
...
/**
 * Checks whether a ebox post type supports comments or not.
 *
 * @param string $feed_post_type Optional. The post type slug to check. Default empty.
 *
 * @return boolean Returns true if the post type supports comments otherwise false.
 */
function ebox_post_type_supports_comments( $feed_post_type = '' ) {
	if ( ( ! empty( $feed_post_type ) ) && ( in_array( $feed_post_type, ebox_get_post_types( 'course' ), true ) ) ) {
		$feed_post_type_object = get_post_type_object( $feed_post_type );
		if ( ( $feed_post_type_object ) && ( is_a( $feed_post_type_object, 'WP_Post_Type' ) ) ) {
			// Default for LD Post types is false.
			...
			if ( isset( $class_key[ $feed_post_type ] ) ) {
				$supports = ebox_Settings_Section::get_section_setting( $class_key[ $feed_post_type ], 'supports' );
			 ...
			 * @param WP_Post_Type  $feed_post_type_object WP_Post_Type instance.
			 */
			$cpt_has_comments = apply_filters( 'ebox_post_comments', $cpt_has_comments, $feed_post_type, $feed_post_type_object );
 ...
 */
function ebox_post_updated_messages( $post_messages = array() ) {
	...
	if ( ( $post_type ) && ( in_array( $post_type, LDLMS_Post_Types::get_post_types(), true ) ) && ( ! isset( $post_messages[ $post_type ] ) ) ) {
		...
		if ( $viewable ) {
			...
			$permalink   = ebox_get_step_permalink( $post_ID );

			// Preview post link.
			$preview_post_link_html = sprintf(
				' <a target="_blank" href="%1$s">%2$s</a>',
				esc_url( $preview_url ),
				esc_html__( 'Preview', 'ebox' )
			);
			...
			$view_post_link_html = sprintf(
				' <a href="%1$s">%2$s</a>',
				esc_url( $permalink ),
				esc_html__( 'View', 'ebox' )
			);
		}

		// translators: Publish box date format, see https://secure.php.net/date.
		$scheduled_date = date_i18n( __( 'M j, Y @ H:i', 'ebox' ), strtotime( $post->post_date ) );

		$post_messages[ $post_type ] = array(
			0  => '', // Unused. Messages start at index 1.
			// translators: placeholder: Post Type Singular Label.
			1  => sprintf( _x( '%s updated.', 'placeholder: Post Type Singular Label', 'ebox' ), $post_type_object->labels->singular_name ) . $view_post_link_html,
			2  => __( 'Custom field updated.', 'ebox' ),
			3  => __( 'Custom field deleted.', 'ebox' ),
			// translators: placeholder: Post Type Singular Label.
			4  => sprintf( _x( '%s updated.', 'placeholder: Post Type Singular Label', 'ebox' ), $post_type_object->labels->singular_name ),
			// translators: placeholders: Post Type Singular Label, Revision Title.
			5  => isset( $_GET['revision'] ) ? sprintf( _x( '%1$s restored to revision from %2$s.', 'placeholder: Post Type Singular Label, Revision Title', 'ebox' ), $post_type_object->labels->singular_name, wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			// translators: placeholder: Post Type Singular Label.
			6  => sprintf( _x( '%s published.', 'placeholder: Post Type Singular Label', 'ebox' ), $post_type_object->labels->singular_name ) . $view_post_link_html,
			// translators: placeholder: Post Type Singular Label.
			7  => sprintf( _x( '%s saved.', 'placeholder: Post Type Singular Label', 'ebox' ), $post_type_object->labels->singular_name ),
			// translators: placeholder: Post Type Singular Label.
			8  => sprintf( _x( '%s submitted.', 'placeholder: Post Type Singular Label', 'ebox' ), $post_type_object->labels->singular_name ) . $preview_post_link_html,
			// translators: placeholder: Post Type Singular Label, scheduled date.
			9  => sprintf( _x( '%1$s scheduled for: %2$s.', 'placeholder: Post Type Singular Label, scheduled date', 'ebox' ), $post_type_object->labels->singular_name, '<strong>' . $scheduled_date . '</strong>' ) . $scheduled_post_link_html,
			// translators: placeholder: Post Type Singular Label.
			10 => sprintf( _x( '%s draft updated.', 'placeholder: Post Type Singular Label', 'ebox' ), $post_type_object->labels->singular_name ) . $preview_post_link_html,
		);
	}
...
add_filter( 'post_updated_messages', 'ebox_post_updated_messages' );
 ...
 */
function ebox_get_total_post_count( $post_type = '' ) {
	$count_total = 0;
	...
	if ( ( $post_type ) && ( in_array( $post_type, LDLMS_Post_Types::get_post_types(), true ) ) ) {
		...
		*/
		 ...
		 */
		$show_in_admin_post_stati = apply_filters( 'ebox_admin_post_stati', $show_in_admin_post_stati, $post_type, $post_counts );
 ...
 */
function ebox_check_query_post_type( $query_args = array() ) {
	$total_post_count = 0;
	if ( ( isset( $query_args['post_type'] ) ) && ( ! empty( $query_args['post_type'] ) ) ) {
		if ( is_string( $query_args['post_type'] ) ) {
			$total_post_count += ebox_get_total_post_count( $query_args['post_type'] );
 ...
 */
function ebox_convert_lesson_time_time( $timer_time = 0 ) {
	if ( ! empty( $timer_time ) ) {
 ...
 *
 * @global array $ebox_question_types
 *
 * @since 3.0.0
 *
 * @param string         $post_type      Optional. The post type slug. Default empty.
 ...
 */
function ebox_update_posts_comment_status( $post_type = '', $comment_status = false ) {
	global $ebox_question_types;

	if ( ! empty( $post_type ) ) {
		$ld_post_types = ebox_get_post_types();
		if ( in_array( $post_type, $ld_post_types, true ) ) {
			if ( in_array( $comment_status, array( 'open', 'closed' ), true ) ) {
				 ...
				 * @param string  $comment_status        Status of comments.
				 */
				if ( apply_filters( 'ebox_update_posts_comment_status', true, $post_type, $comment_status ) ) {
					global $wpdb;
					$wpdb->query(
						$wpdb->prepare(
 ...
 * @since 3.3.0 Renamed
 *
 * @return string Returns .min if the ebox_SCRIPT_DEBUG constant is false
 *                otherwise empty string.
 */
function ebox_min_asset() {
		return ( ( defined( 'ebox_SCRIPT_DEBUG' ) && ( ebox_SCRIPT_DEBUG === true ) ) ? '' : '.min' );
}
 ...
 *                otherwise empty string.
 */
function ebox_min_builder_asset() {
		return ( ( defined( 'ebox_BUILDER_DEBUG' ) && ( ebox_BUILDER_DEBUG === true ) ) ? '' : '.min' );
}
 ...
 */
function ebox_scandir_recursive( $base = '' ) {
	if ( ( ! $base ) || ( ! strlen( $base ) ) ) {
		return array();
	}
	...
	foreach ( $data as $key => $value ) {
		if ( is_dir( $base . '/' . $value ) ) {
			unset( $data[ $key ] );
			$subs[] = ebox_scandir_recursive( $base . '/' . $value );
 ...
 */
function ebox_is_protected_meta( $protected = false, $meta_key = '', $meta_type = '' ) {
	if ( ( 'post' === $meta_type ) && ( ! empty( $meta_key ) ) && ( '_' !== $meta_key[0] ) ) {
		...
		if ( ( ! empty( $post_type ) ) && ( in_array( $post_type, ebox_get_post_types(), true ) ) ) {
			$protected_meta_keys = array( 'course_id', 'lesson_id', 'course_price_billing_p3', 'course_price_billing_t3', 'course_sections', 'ld_course_steps', 'course_access_list', 'quiz_pro_id', 'ld_course_steps_dirty', 'ld_auto_enroll_team_courses', 'team_price_billing_p3', 'team_price_billing_t3', 'ld_auto_enroll_team_course_ids', 'question_pro_id', 'course_points', 'ld_quiz_questions', 'ld_quiz_questions_dirty', 'ebox_certificate_options', 'question_id', 'ld_essay_grading_response', 'question_points', 'question_type', 'question_pro_id', 'question_pro_category', 'course_trial_duration_p1', 'course_trial_duration_t1', 'course_price_type_subscribe_billing_recurring_times', 'team_trial_duration_p1', 'team_trial_duration_t1', 'team_price_type_subscribe_billing_recurring_times', 'exam_challenge_course_show', 'exam_challenge_course_passed' );
			...
			if ( ( in_array( $meta_key, $protected_meta_keys, true ) ) ) {
				...
				$protected = true;
			} elseif ( 'ebox_team_enrolled_' === substr( $meta_key, 0, strlen( 'ebox_team_enrolled_' ) ) ) {
				$protected = true;
			} elseif ( 'ebox_team_users_' === substr( $meta_key, 0, strlen( 'ebox_team_users_' ) ) ) {
				$protected = true;
			}
		}
	}
...
add_filter( 'is_protected_meta', 'ebox_is_protected_meta', 30, 3 );
 ...
 */
function ebox_login_menu_items( $menu_items, $menu_args = array() ) {

	foreach ( $menu_items as $menu_key => &$menu_item ) {
		/**
		 * Check the properties we need exist and not empty. We shouldn't need to do this
		 * since the array of menu items comes from WP. See ebox-3812.
		 */
		...
		if ( ( strpos( $menu_item->url, '#login' ) !== false ) && ( in_array( 'ld-button', $menu_item->classes, true ) ) ) {
			 ...
			 */
			if ( apply_filters( 'ebox_login_menu_item_process', true, $menu_item, $menu_args ) ) {
				if ( ( empty( $menu_item->post_content ) ) || ( strpos( $menu_item->post_content, '[ebox_login' ) === false ) ) {
					$shortcode = '[ebox_login return="atts"]';
				} else {
					$shortcode = str_replace( '[ebox_login', '[ebox_login return="atts" ', $menu_item->post_content );
				}
				...
				if ( ( 'ld30' === $active_template_key ) && ( 'yes' === $login_mode_enabled ) ) {
					...
					$shortcode      = str_replace( array( '[ebox_login', ']' ), '', $shortcode );
					...
					if ( is_user_logged_in() ) {
						...
						if ( ( isset( $atts['logout_label'] ) ) && ( ! empty( $atts['logout_label'] ) ) ) {
							...
							$shortcode_atts['title'] = __( 'Logout', 'ebox' );
						}
					} else {
						...
						if ( ( isset( $atts['login_label'] ) ) && ( ! empty( $atts['login_label'] ) ) ) {
							...
							$shortcode_atts['title'] = __( 'Login', 'ebox' );
						}
					}
				}
				 ...
				 * @param array   $menu_args      An array of arguments related to menu being processed / displayed.
				 */
				$shortcode_atts = apply_filters( 'ebox_login_menu_item_atts', $shortcode_atts, $menu_item, $menu_args );
				 ...
				 * @param array   $menu_args An array of arguments related to menu being processed / displayed.
				 */
				$menu_item = apply_filters( 'ebox_login_menu_item', $menu_item, $menu_args );
			}
		}
	}
	return $menu_items;
}
add_filter( 'wp_nav_menu_objects', 'ebox_login_menu_items', 30, 2 );

global $ebox_login_model_html;
$ebox_login_model_html = false;
/**
 * Prints the login modal in the site footer.
 *
 * @global string $ebox_login_model_html Login modal HTML.
 */
function ebox_load_login_modal_html() {
	global $ebox_login_model_html;
	...
	if ( ( 'ld30' === $active_template_key ) && ( 'yes' === $login_mode_enabled ) ) {
		...
		if ( ( ! is_user_logged_in() ) && ( function_exists( 'ebox_get_template_part' ) ) ) {
			if ( false === $ebox_login_model_html ) {
				$ebox_login_model_html = ebox_get_template_part( 'modules/login-modal.php', array(), false );
				if ( false !== $ebox_login_model_html ) {
					add_action(
						'wp_footer',
						function() {
							global $ebox_login_model_html;
							if ( ( isset( $ebox_login_model_html ) ) && ( ! empty( $ebox_login_model_html ) ) ) {
								echo '<div class="ebox-wrapper ebox-wrapper-login-modal ld-modal-closed">' . $ebox_login_model_html . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
							}
						}
					);
				}
 ...
 */
function ebox_body_classes( $classes = array() ) {

	if ( in_array( get_post_type(), ebox_get_post_types(), true ) ) {
		$custom_classes   = array();
		$custom_classes[] = 'ebox-cpt';
		$custom_classes[] = 'ebox-cpt-' . get_post_type();

		// Add active LD theme template.
		$custom_classes[] = 'ebox-template-' . ebox_Theme_Register::get_active_theme_key();

		// Add classes or current course steps.
		if ( in_array( get_post_type(), ebox_get_post_types( 'course_steps' ), true ) ) {
			$custom_classes[] = 'ebox-cpt-' . get_post_type( get_the_ID() ) . '-' . get_the_ID() . '-current';

			$course_id = ebox_get_course_id();
			if ( ! empty( $course_id ) ) {
				$custom_classes[] = 'ebox-cpt-' . get_post_type( $course_id ) . '-' . $course_id . '-parent';

				$parent_step_ids = ebox_course_get_all_parent_step_ids( $course_id, get_the_ID() );
				if ( ! empty( $parent_step_ids ) ) {
					foreach ( $parent_step_ids as $parent_step_id ) {
						$custom_classes[] = 'ebox-cpt-' . get_post_type( $parent_step_id ) . '-' . $parent_step_id . '-parent';
					}
				}
			}
		}
		...
		if ( true === apply_filters( 'ebox_responsive_video', true, get_post_type(), get_the_ID() ) ) {
			$custom_classes[] = 'ebox-embed-responsive';
		}
		 ...
		 * @param int|false    $post_id        Post ID.
		 */
		$custom_classes = apply_filters( 'ebox_body_classes', $custom_classes, get_post_type(), get_the_ID() );
...
add_filter( 'body_class', 'ebox_body_classes', 100, 1 );
 ...
 */
function ebox_recount_serialized_bytes( $serialized_text = '' ) {
...
/**
 * Gets a single `WP_Post` for a given ebox post type.
 *
 * @param string $post_type Optional. The post type slug. Default empty.
 *
 * @return WP_Post|void The `WP_Post` object for a given post type.
 */
function ebox_get_single_post( $post_type = '' ) {
	if ( ( ! empty( $post_type ) ) && ( in_array( $post_type, ebox_get_post_types(), true ) ) ) {
 ...
 */
function ebox_array_sanitize_keys_and_values( $data_in = array() ) {
	if ( ( is_array( $data_in ) ) && ( ! empty( $data_in ) ) ) {
		$data_out = array();
		foreach ( $data_in as $i_key => $i_val ) {
			$i_key = sanitize_text_field( $i_key );
			if ( is_array( $i_val ) ) {
				$i_val = ebox_array_sanitize_keys_and_values( $i_val );
...
/**
 * Utility function to centralize all ebox redirect calls.
 *
 * @since 3.2.3
 *
 ...
 */
function ebox_safe_redirect( $location = '', $status = null, $exit = true, $context = '' ) {
	if ( ! empty( $location ) ) {
		 ...
		 * @param string $context  Unique string provided by the caller to help filter conditions.
		 */
		$location = apply_filters( 'ebox_safe_redirect_location', $location, $status, $context );
		if ( ! empty( $location ) ) {
			/**
			 ...
			 * @param string $context  Unique string provided by the caller to help filter conditions.
			 */
			$status = apply_filters( 'ebox_safe_redirect_status', $status, $location, $context );
			 ...
			 * @param string $context              Unique string provided by the caller to help filter conditions.
			 */
			if ( apply_filters( 'ebox_safe_redirect_nocache_header', true, $location, $status, $context ) ) {
				nocache_headers();
			}
			 ...
			 * @since 3.3.0.2
			 *
			 * @param bool   $call_wp_safe_redirect Call wp_safe_redirect(). Default ebox_USE_WP_SAFE_REDIRECT constant value.
			 * @param string $location              The URL to redirect the user to.
			 * @param int    $status                The HTTP Status to set. Default 302.
			 * @param string $context               Unique string provided by the caller to help filter conditions.
			 */
			if ( apply_filters( 'ebox_use_wp_safe_redirect', ebox_USE_WP_SAFE_REDIRECT, $location, $status, $context ) ) {
 ...
 */
function ebox_use_select2_lib() {
	/**
	 * Filters whether the select2 is loaded or not.
	 *
	 * @param boolean $ebox_select2 whether the select2 library is loaded or not.
	 */
	if ( ( defined( 'ebox_SELECT2_LIB' ) ) && ( true === apply_filters( 'ebox_select2_lib', ebox_SELECT2_LIB ) ) ) {
		return true;
	}
	return false;
}
 ...
 * @return bool true is select2 library is being used.
 */
function ebox_use_select2_lib_ajax_fetch() {
	if ( ebox_use_select2_lib() ) {
		/**
		 * Filters whether the select2 is used to fetch AJAX data.
		 *
		 * @param boolean $ebox_select2_ajax_fetch whether the select2 library is used to fetch AJAX data.
		 */
		if ( ( defined( 'ebox_SELECT2_LIB_AJAX_FETCH' ) ) && ( true === apply_filters( 'ebox_select2_lib_ajax_fetch', ebox_SELECT2_LIB_AJAX_FETCH ) ) ) {
			return true;
		}
	}
	return false;
 ...
 */
function ebox_put_directory_index_file( $index_filename = '' ) {
	if ( ! empty( $index_filename ) ) {
		...
		$wp_filesystem->put_contents( $index_filename, '//ebox is THE Best LMS', FS_CHMOD_FILE );
	}
}
 ...
 */
function ebox_the_content( $content = '', $context = '' ) {
	if ( apply_filters( 'ebox_use_wp_the_content_filters', true, $context ) ) {
		/**
		 ...
		 * @since 3.5.0
		 */
		do_action( 'ebox_before_normal_the_content_filter', $context );
		 ...
		 * @since 3.5.0
		 */
		do_action( 'ebox_after_normal_the_content_filter', $context );
	} else {
		/**
		 ...
		 */
		add_filter( 'ebox_the_content', 'do_blocks', 9 );
		add_filter( 'ebox_the_content', 'wptexturize' );
		add_filter( 'ebox_the_content', 'convert_smilies', 20 );
		add_filter( 'ebox_the_content', 'wpautop' );
		add_filter( 'ebox_the_content', 'shortcode_unautop' );
		add_filter( 'ebox_the_content', 'prepend_attachment' );
		add_filter( 'ebox_the_content', 'wp_filter_content_tags' );
		add_filter( 'ebox_the_content', 'wp_replace_insecure_home_url' );
		 ...
		 * @since 3.5.0
		 */
		do_action( 'ebox_before_custom_the_content_filter', $context );

		$content = apply_filters( 'ebox_the_content', $content );
		 ...
		 * @since 3.5.0
		 */
		do_action( 'ebox_after_the_content_filter', $context );

		remove_filter( 'ebox_the_content', 'do_blocks', 9 );
		remove_filter( 'ebox_the_content', 'wptexturize' );
		remove_filter( 'ebox_the_content', 'convert_smilies', 20 );
		remove_filter( 'ebox_the_content', 'wpautop' );
		remove_filter( 'ebox_the_content', 'shortcode_unautop' );
		remove_filter( 'ebox_the_content', 'prepend_attachment' );
		remove_filter( 'ebox_the_content', 'wp_filter_content_tags' );
		remove_filter( 'ebox_the_content', 'wp_replace_insecure_home_url' );
	}
 ...
 */
function ebox_get_user_profile_quiz_attempts( $user_id = 0 ) {
	...
	foreach ( $quiz_attempts_meta as $quiz_attempt ) {
		$c                    = ebox_certificate_details( $quiz_attempt['quiz'], $user_id );
		...
		if ( ! isset( $quiz_attempt['course'] ) ) {
			$quiz_attempt['course'] = ebox_get_course_id( $quiz_attempt['quiz'] );
		}
 ...
 */
function ebox_get_post_title_filter( $post_title = '', $post_id = 0 ) {
	...
	if ( ! in_array( get_post_type( $post_id ), ebox_get_post_types(), true ) ) {
		return $post_title;
	}
...
add_filter( 'the_title', 'ebox_get_post_title_filter', 99, 2 );
 ...
 * @return void
 */
function ebox_stripe_addon_deprecation_notice() {
	$class   = 'notice notice-warning is-dismissible';
	$title   = 'ebox Stripe Addon Deprecation';
	$message = __( 'As of June 13, 2023 the Stripe plugin will no longer receive feature updates. We encourage you to switch over to Stripe Connect, however you can continue to utilize the plugin without upgrading. ', 'ebox' );
	$links   = __( '<a href="admin.php?page=ebox_lms_payments&section-payment=settings_stripe_connection">Setup Stripe Connect</a> - <a href="https://www.ebox.com/support/docs/core/settings/stripe-add-on-deprecation-faq/">Stripe Deprecation FAQ</a>', 'ebox' );
	...
	if ( is_plugin_active( 'ebox-stripe/ebox-stripe.php' ) && in_array( 'administrator', $user->roles ) ) {
		printf(
			'<div class="%1$s">
...
add_action( 'admin_notices', 'ebox_stripe_addon_deprecation_notice' );
