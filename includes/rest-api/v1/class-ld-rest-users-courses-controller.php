<?php
/**
 * ebox REST API V1 User Courses Controller.
 *
 * @since 2.5.8
...
if ( ( ! class_exists( 'LD_REST_Users_Courses_Controller_V1' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V1' ) ) ) {
	/**
	 * Class ebox REST API V1 Courses Controller.
	 *
	 * @since 2.5.8
	 */
	class LD_REST_Users_Courses_Controller_V1 extends LD_REST_Posts_Controller_V1 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function register_routes() {
			...
			register_rest_route(
				...
				array(
					'args'   => array(
						'id' => array(
							'description' => esc_html__( 'User ID to enroll user into.', 'ebox' ),
							'required'    => true,
							'type'        => 'integer',
						),
					),
					...
					array(
						...
						'args'                => array(
							'course_ids' => array(
								// translators: courses.
								'description' => sprintf( esc_html_x( '%s IDs to add to User.', 'placeholder: courses', 'ebox' ), ebox_get_custom_label( 'courses' ) ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
					...
					array(
						...
						'args'                => array(
							'course_ids' => array(
								// translators: courses.
								'description' => sprintf( esc_html_x( '%s IDs to remove from User.', 'placeholder: courses', 'ebox' ), ebox_get_custom_label( 'courses' ) ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
		...
		public function get_user_courses( $request ) {
			...
			$course_ids       = ebox_user_get_enrolled_courses( $user_id, array(), true );
			if ( ! empty( $course_ids ) ) {
			 ...
			 */
			$args       = apply_filters( 'ebox_rest_user_courses_query', $args, $request );
