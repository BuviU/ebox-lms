<?php
/**
 * ebox REST API V1 Quizzes Post Controller.
 *
 * @since 2.5.8
...
if ( ( ! class_exists( 'LD_REST_Quizzes_Controller_V1' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V1' ) ) ) {
	...
	/**
	 * Class ebox REST API V1 Quizzes Post Controller.
	 *
	 * @since 2.5.8
	 */
				...
				array(
					'args'   => array(
						'id' => array(
							'description' => sprintf(
								// translators: quiz.
								esc_html_x( 'Unique identifier for the %s object.', 'placeholder: quiz', 'ebox' ),
		...
		public function get_items_permissions_check( $request ) {
			...
			if ( ( true === $return ) && ( 'view' === $request['context'] ) ) {
				...
				if ( ! empty( $topic_id ) ) {
					...
					if ( ( ! $this->topic_post ) || ( ! is_a( $this->topic_post, 'WP_Post' ) ) || ( 'ebox-topic' !== $this->topic_post->post_type ) ) {
						return new WP_Error(
							'rest_post_invalid_id',
							sprintf(
				...
				if ( ! ebox_is_admin_user() ) {
					...
					if ( ( $this->course_post ) && ( ! ebox_lms_has_access( $this->course_post->ID ) ) ) {
						return new WP_Error( 'ld_rest_cannot_view', esc_html__( 'Sorry, you are not allowed to view this item.', 'ebox' ), array( 'status' => rest_authorization_required_code() ) );
					}
				}
			}
		...
		public function rest_query_filter( $args, $request ) {
			...
			if ( ( $this->course_post ) && ( is_a( $this->course_post, 'WP_Post' ) ) && ( 'ebox-courses' === $this->course_post->post_type ) ) {
				...
				if ( $this->topic_post ) {
					...
					if ( 0 === $lesson_id ) {
						$step_ids = ebox_course_get_children_of_step( $this->course_post->ID, $this->course_post->ID, $this->post_type );
					} else {
