<?php
/**
 * ebox REST API V1 Team Users Controller.
 *
 * @since 2.5.8
...
if ( ( ! class_exists( 'LD_REST_Teams_Users_Controller_V1' ) ) && ( class_exists( 'LD_REST_Users_Controller_V1' ) ) ) {

	/**
	 * Class ebox REST API V1 Team Users Controller.
	 *
	 * @since 2.5.8
	 */
	class LD_REST_Teams_Users_Controller_V1 extends LD_REST_Users_Controller_V1 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function register_routes() {
			...
			register_rest_route(
				...
				array(
					'args'   => array(
						'id' => array(
							// translators: team, team leader.
							'description' => sprintf( esc_html_x( '%1$s ID to enroll %2$s into.', 'placeholder: team, team leader', 'ebox' ), ebox_get_custom_label_lower( 'team' ), ebox_get_custom_label_lower( 'team_leader' ) ),
							'required'    => true,
							'type'        => 'integer',
						),
					),
					...
					array(
						...
						'args'                => array(
							'user_ids' => array(
								'description' => esc_html__( 'User IDs to enroll into Team.', 'ebox' ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
					...
					array(
						...
						'args'                => array(
							'user_ids' => array(
								'description' => esc_html__( 'User IDs to remove from Team.', 'ebox' ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
		...
		/**
		 * Check Team Users Read Permissions.
		 *
		 * @since 2.5.8
		 *
		...
		public function get_teams_users_permissions_check( $request ) {
			...
			if ( ebox_is_team_leader_user() ) {
				...
				if ( ( ! empty( $leader_teams ) ) && ( in_array( $team_id, $leader_teams, true ) ) ) {
					return true;
				}
			}
		}
		...
		public function get_teams_users( $request ) {
			...
			/**
			 * Filters WP_User_Query arguments when querying teams users via the REST API.
			 *
			 ...
			 */
			$prepared_args = apply_filters( 'ebox_rest_teams_users_query', $prepared_args, $request );
		...
		public function update_teams_users( $request ) {
			...
			if ( ( ! is_array( $user_ids ) ) || ( empty( $user_ids ) ) ) {
				return new WP_Error( 'rest_post_invalid_id', esc_html__( 'Missing User IDs.', 'ebox' ) . ' ' . __CLASS__, array( 'status' => 404 ) );
			} else {
