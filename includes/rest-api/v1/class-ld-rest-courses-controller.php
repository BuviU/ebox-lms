<?php
/**
 * ebox REST API V1 Courses Post Controller.
 *
 * @since 2.5.8
...
if ( ( ! class_exists( 'LD_REST_Courses_Controller_V1' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V1' ) ) ) {
	...
	/**
	 * Class ebox REST API V1 Courses Post Controller.
	 *
	 * @since 2.5.8
	 */
	class LD_REST_Courses_Controller_V1 extends LD_REST_Posts_Controller_V1 /* //phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function register_routes() {
			...
			$this->sub_controllers['class-ld-rest-courses-teams-controller'] = new LD_REST_Courses_Teams_Controller_V1();
			$this->sub_controllers['class-ld-rest-courses-teams-controller']->register_routes();
		}
		...
		public function rest_prepare_response_filter( WP_REST_Response $response, WP_Post $post, WP_REST_Request $request ) {
			if ( $this->post_type === $post->post_type ) {
				...
				if ( ( ! empty( $request_route ) ) && ( strpos( $request_route, $base ) !== false ) ) {
					...
					if ( ! isset( $response->links['teams'] ) ) {
						$links['teams'] = array(
							'href'       => rest_url( trailingslashit( $base ) . $post->ID ) . '/teams',
							'embeddable' => true,
						);
					}
					if ( ! empty( $links ) ) {
