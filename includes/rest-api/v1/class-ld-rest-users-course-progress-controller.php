<?php
/**
 * ebox REST API V1 Course Progress Controller.
 *
 * @since 2.5.8
...
if ( ( ! class_exists( 'LD_REST_Users_Course_Progress_Controller_V1' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V1' ) ) ) {
	...
	/**
	 * Class ebox REST API V1 Course Progress Controller.
	 *
	 * @since 2.5.8
	 */
	class LD_REST_Users_Course_Progress_Controller_V1 extends LD_REST_Posts_Controller_V1 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
			...
			register_rest_route(
				...
				array(
					'args' => array(
						'id' => array(
							// translators: course.
							'description' => sprintf( esc_html_x( 'User ID to show %s progress', 'placeholder: course', 'ebox' ), ebox_get_custom_label_lower( 'course' ) ),
							'required'    => true,
							'type'        => 'integer',
						),
					),
		...
		public function get_users_progress( $request ) {
			...
			if ( ( ! empty( $user_course_ids ) ) && ( ebox_is_team_leader_user() ) ) {
				$gl_teams_corses = ebox_get_team_leader_teams_courses( get_current_user_id() );

				if ( ! empty( $gl_teams_corses ) ) {
					$user_course_ids = array_intersect( $gl_teams_corses, $user_course_ids );
				}
			}
		...
		public function user_meta_progress_normalized( $progress = array() ) {
			...
			if ( ( isset( $progress['modules'] ) ) && ( ! empty( $progress['modules'] ) ) ) {
				foreach ( $progress['modules']  as $lesson_id => $lesson_complete ) {
					$converted[ 'ebox-modules:' . $lesson_id ] = $lesson_complete;
					if ( ( isset( $progress['topics'][ $lesson_id ] ) ) && ( ! empty( $progress['topics'][ $lesson_id ] ) ) ) {
						foreach ( $progress['topics'][ $lesson_id ]  as $topic_id => $topic_complete ) {
							$converted[ 'ebox-topic:' . $topic_id ] = $topic_complete;
						}
					}
				}
			}
