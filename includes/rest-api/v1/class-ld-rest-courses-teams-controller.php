<?php
/**
 * ebox REST API V1 Courses Teams Post Controller.
 *
 * @since 2.5.8
...
if ( ( ! class_exists( 'LD_REST_Courses_Teams_Controller_V1' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V1' ) ) ) {

	/**
	 * Class ebox REST API V1 Courses Teams Post Controller.
	 *
	 * @since 2.5.8
	 */
	class LD_REST_Courses_Teams_Controller_V1 extends LD_REST_Posts_Controller_V1 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
			...
			register_rest_route(
				...
				array(
					'args'   => array(
						'id' => array(
							// translators: course.
							'description' => sprintf( esc_html_x( '%s ID to enroll into.', 'placeholder: course', 'ebox' ), ebox_get_custom_label( 'course' ) ),
							'required'    => true,
							'type'        => 'integer',
						),
					),
					...
					array(
						...
						'args'                => array(
							'team_ids' => array(
								...
								'description' => sprintf( esc_html_x( '%1$s IDs to enroll into %2$s.', 'placeholder: team, course', 'ebox' ), ebox_get_custom_label( 'team' ), ebox_get_custom_label( 'course' ) ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
					...
					array(
						...
						'args'                => array(
							'team_ids' => array(
								// translators: team, course.
								'description' => sprintf( esc_html_x( '%1$s IDs to remove from %2$s.', 'placeholder: team, course', 'ebox' ), ebox_get_custom_label( 'team' ), ebox_get_custom_label( 'course' ) ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
		...
		/**
		 * Check Courses Teams Read Permissions.
		 *
		 * @since 2.5.8
		 *
		...
		/**
		 * Check Courses Teams Update Permissions.
		 *
		 * @since 2.5.8
		 *
		 * @param WP_REST_Request $request WP_REST_Request instance.
		...
		/**
		 * Check Courses Teams Delete Permissions.
		 *
		 * @since 2.5.8
		 *
		 * @param WP_REST_Request $request WP_REST_Request instance.
		...
		public function get_courses_teams( $request ) {
			...
			$team_ids        = ebox_get_course_teams( $course_id, true );
			if ( ! empty( $team_ids ) ) {
			...
			/**
			 * Filters the query arguments for courses team REST request.
			 *
			 ...
			 *
			 * @param array           $args    An array of query arguments for getting courses teams.
			 * @param WP_REST_Request $request The REST request object.
			 */
			$args       = apply_filters( 'ebox_rest_courses_teams_query', $args, $request );
