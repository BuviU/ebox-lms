<?php
/**
 * ebox REST API V1 Courses Steps Controller.
 *
 * @since 2.5.8
...
if ( ( ! class_exists( 'LD_REST_Courses_Steps_Controller_V1' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V1' ) ) ) {
	/**
	 * Class ebox REST API V1 Courses Steps Controller.
	 *
	 * @since 2.5.8
	 */
	class LD_REST_Courses_Steps_Controller_V1 extends LD_REST_Posts_Controller_V1 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
			...
			register_rest_route(
				...
				array(
					'args'   => array(
						'id' => array(
							// translators: course.
							'description' => sprintf( esc_html_x( '%s ID to enroll user into.', 'placeholder: course', 'ebox' ), ebox_get_custom_label( 'course' ) ),
							'required'    => true,
							'type'        => 'integer',
						),
					),
			...
			$schema = array(
				...
				'properties' => array(
					...
					'type' => array(
						...
						'description' => sprintf( esc_html_x( 'The %s step type.', 'placeholder: course', 'ebox' ), ebox_get_custom_label_lower( 'course' ) ),
						'type'        => 'string',
						'enum'        => array(
							'all',
							'h',
		...
		public function get_course_steps_permissions_check( $request ) {
			if ( is_user_logged_in() ) {
				...
				$this->enrolled_courses = ebox_user_get_enrolled_courses( get_current_user_id() );
