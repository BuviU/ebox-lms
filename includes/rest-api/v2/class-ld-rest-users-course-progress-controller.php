<?php
/**
 * ebox V2 REST API User Course Progress Controller.
 *
...
if ( ( ! class_exists( 'LD_REST_Users_Course_Progress_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox V2 REST API User Course Progress Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Users_Course_Progress_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function get_public_item_header_schema() {
			...
			$schema = array(
				...
				'properties' => array(
					...
					'last_step'       => array(
						'description' => esc_html__( 'Last completed step', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'embed', 'view' ),
						'readonly'    => true,
					),
					'steps_completed' => array(
						'description' => esc_html__( 'Total completed steps', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					...
					'date_started'    => array(
						'description' => esc_html__( 'Date started', 'ebox' ),
						'type'        => array(
							'string',
							'null',
						),
					...
					'date_completed'  => array(
						'description' => esc_html__( 'Date completed', 'ebox' ),
						'type'        => array(
							'string',
							'null',
						),
		...
		public function get_public_item_step_schema() {
			...
			$schema = array(
				...
				'properties' => array(
					'step'            => array(
						'description' => esc_html__( 'Step ID', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'embed', 'view' ),
						'readonly'    => true,
					),
					'post_type'       => array(
						'description' => esc_html__( 'Post type for step', 'ebox' ),
						'type'        => 'string',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
		...
		protected function check_request_params( $request ) {
			...
			if ( ! is_a( $user, 'WP_User' ) ) {
				return new WP_Error( 'rest_user_invalid_id', esc_html__( 'Invalid or unknown User.', 'ebox' ), array( 'status' => 404 ) );
			}
			...
			if ( empty( $this->current_user_id ) ) {
				return new WP_Error( 'rest_user_invalid_id', esc_html__( 'Invalid current user.', 'ebox' ), array( 'status' => 404 ) );
			}

			if ( ( $this->current_user_id !== $this->user_id ) && ( ! ebox_is_admin_user( $this->current_user_id ) ) ) {

				if ( ebox_is_team_leader_user( $this->current_user_id ) ) {
					if ( ! ebox_is_team_leader_of_user( $this->current_user_id, $this->user_id ) ) {
						return new WP_Error( 'rest_user_invalid_id', esc_html__( 'Not allowed to view other user content.', 'ebox' ), array( 'status' => 401 ) );
					}
				} else {
					$this->user_id = $this->current_user_id;
				}

				$user_team_ids = ebox_get_users_team_ids( $this->user_id );
		...
		protected function get_request_courses( $request ) {
			...
			if ( ( $this->current_user_id !== $this->user_id ) && ( ! ebox_is_admin_user( $this->current_user_id ) ) ) {
				...
				if ( ebox_is_team_leader_user( $this->current_user_id ) ) {
					...
					$team_leader_team_ids = ebox_get_administrators_team_ids( $this->current_user_id );
					...
					$team_ids = array_intersect( $team_leader_team_ids, $user_team_ids );

					if ( ! empty( $team_ids ) ) {
						foreach ( $team_ids as $team_id ) {
							$course_ids = ebox_team_enrolled_courses( $team_id );
							if ( ! empty( $course_ids ) ) {
				...
				$user_course_progress = get_user_meta( $this->user_id, '_ebox-course_progress', true );
		...
		protected function get_query_args( $request, $user_course_ids = array() ) {
			...
			$args = apply_filters( 'ebox_rest_users_course_progress_query', $args, $request );
		...
		protected function get_user_course_progress_header( $user_id = 0, $course_id = 0 ) {
			...
			$user_course_progress_meta = get_user_meta( $user_id, '_ebox-course_progress', true );
			if ( isset( $user_course_progress_meta[ $course_id ] ) ) {
				$user_course_progress_header = $user_course_progress_meta[ $course_id ];
				if ( isset( $user_course_progress_header['modules'] ) ) {
					unset( $user_course_progress_header['modules'] );
				}
			...
			if ( ( isset( $this->user_course_activity[ $course_id ] ) ) && ( 'ebox-courses' === $this->user_course_activity[ $course_id ]['post_type'] ) ) {
			...
			$user_course_progress_header['steps_total'] = ebox_get_course_steps_count( $course_id );
			...
			$user_course_progress_header['status']          = ebox_course_status( $course_id, $user_id, true );
		...
		protected function convert_user_progress_meta_normalized( $progress = array() ) {
			...
			if ( ( isset( $progress['modules'] ) ) && ( ! empty( $progress['modules'] ) ) ) {
				foreach ( $progress['modules']  as $lesson_id => $lesson_complete ) {
