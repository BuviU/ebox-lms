<?php
/**
 * ebox V2 REST API Topics Post Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the ebox
 * custom post type Topics (ebox-topic).
 *
...
if ( ( ! class_exists( 'LD_REST_Topics_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox V2 REST API Topics Post Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Topics_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		protected function register_fields_metabox() {
			...
			$this->metaboxes['ebox_Settings_Metabox_Topic_Display_Content'] = ebox_Settings_Metabox_Topic_Display_Content::add_metabox_instance();
			...
			$this->metaboxes['ebox_Settings_Metabox_Topic_Access_Settings'] = ebox_Settings_Metabox_Topic_Access_Settings::add_metabox_instance();
		...
		public function get_item_permissions_check( $request ) {
			...
			if ( ( true === $return ) && ( ! ebox_is_admin_user() ) ) {
				...
				if ( ( $topic_id ) && ( ebox_lms_has_access( $topic_id ) ) ) {
					return true;
				}
