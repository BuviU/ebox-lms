<?php
/**
 * ebox REST API V2 Quiz Questions Post Controller.
 *
 * This Controller class is used for the ebox Questions (ebox-question)
 * custom post type.
 *
...
if ( ( ! class_exists( 'LD_REST_Questions_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Quiz Questions Post Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Questions_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
			...
			$this->fields = array(
				...
				'question_type'          => array(
					'schema'          => array(
						...
						'type'        => 'enum',
						'enum'        => array_keys( $ebox_question_types ),
						'required'    => false,
						'default'     => 'single',
						'context'     => array( 'view', 'edit' ),
					),
				...
				'points_total'           => array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Total Points amount', 'ebox' ),
						'type'        => 'integer',
						'required'    => false,
						'default'     => '',
						'context'     => array( 'view', 'edit' ),
				...
				'points_per_answer'      => array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Different points for each answer', 'ebox' ),
						'type'        => 'boolean',
						'required'    => false,
						'default'     => '',
						'context'     => array( 'view', 'edit' ),
				...
				'points_show_in_message' => array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Show reached points in the correct/incorrect message?', 'ebox' ),
						'type'        => 'boolean',
						'required'    => false,
						'default'     => '',
						'context'     => array( 'view', 'edit' ),
				...
				'points_diff_modus'      => array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Different points - modus 2 activate', 'ebox' ),
						'type'        => 'boolean',
						'required'    => false,
						'default'     => '',
						'context'     => array( 'view', 'edit' ),
				...
				'disable_correct'        => array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Disable answer correct setting.', 'ebox' ),
						'type'        => 'boolean',
						'required'    => false,
						'default'     => '',
						'context'     => array( 'view', 'edit' ),
				...
				'hints_message'          => array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Hint message.', 'ebox' ),
						'type'        => 'object',
						'required'    => false,
						'properties'  => array(
							'raw'      => array(
		...
		public function get_items_permissions_check( $request ) {
			...
			if ( ( true === $return ) && ( 'view' === $request['context'] ) ) {
				...
				if ( ! $this->rest_post_type_has_archive( $this->post_type ) ) {
					...
					if ( ! ebox_lms_has_access( $this->quiz_post->ID ) ) {
		...
		public function rest_query_filter( $query_args, $request ) {
			...
			if ( ! is_null( $this->quiz_post ) ) {
				$questions_ids = array_keys( ebox_get_quiz_questions( $this->quiz_post->ID ) );
		...
		public function update_item( $request ) {
			...
			return new WP_Error(
				...
				sprintf(
				// translators: placeholder: Question.
					esc_html_x( 'Could not update the %s.', 'placeholder: Question', 'ebox' ),
					\ebox_Custom_Label::get_label( 'question' )
				),
				array( 'status' => 500 )
			);
