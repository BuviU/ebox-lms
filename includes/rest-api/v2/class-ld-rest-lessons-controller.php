<?php
/**
 * ebox REST API V2 modules Post Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the ebox
 * custom post type modules (ebox-modules).
 *
...
if ( ( ! class_exists( 'LD_REST_modules_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {

	/**
	 * Class ebox REST API V2 modules Post Controller.
	 *
	 * @since 3.3.0
	 * @uses LD_REST_Posts_Controller_V2
	 */
	class LD_REST_modules_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function __construct( $post_type = '' ) {
			 ...
			 */
			$this->rest_base = $this->get_rest_base( 'modules' );
		}
		...
		protected function register_fields_metabox() {
			...
			$this->metaboxes['ebox_Settings_Metabox_Lesson_Display_Content'] = ebox_Settings_Metabox_Lesson_Display_Content::add_metabox_instance();

			require_once ebox_LMS_PLUGIN_DIR . '/includes/settings/settings-metaboxes/class-ld-settings-metabox-lesson-access-settings.php';
			$this->metaboxes['ebox_Settings_Metabox_Lesson_Access_Settings'] = ebox_Settings_Metabox_Lesson_Access_Settings::add_metabox_instance();
		...
		public function get_item_permissions_check( $request ) {
			...
			if ( ( true === $return ) && ( ! ebox_is_admin_user() ) ) {
				...
				if ( ( $lesson_id ) && ( ebox_lms_has_access( $lesson_id ) ) ) {
					return true;
				}
				...
				if ( ! $this->rest_post_type_has_archive( $this->post_type ) ) {
					if ( is_null( $this->course_post ) ) {
						return new WP_Error(
							...
							sprintf(
								// translators: placeholder: Course.
								esc_html_x( 'Invalid %s ID', 'placeholder: Course', 'ebox' ),
				...
				if ( ! empty( $step_ids ) ) {
					...
					if ( ! empty( $query_args['post__in'] ) ) {
						...
						if ( is_null( $request['posts_per_page'] ) ) {
							$query_args['posts_per_page'] = absint( ebox_get_course_modules_per_page( $this->course_post->ID ) ); // phpcs:ignore WordPress.WP.PostsPerPage.posts_per_page_posts_per_page
						}
					} else {
