<?php
/**
 * ebox V2 REST API User Controller.
 *
 * This Controller is used as the parent Controller for all ebox
 * User related REST requests. For example Team Users, Course Users, etc.
 *
...
if ( ( ! class_exists( 'LD_REST_Users_Controller_V2' ) ) && ( class_exists( 'WP_REST_Users_Controller' ) ) ) {
	...
	/**
	 * Class ebox V2 REST API User Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Users_Controller_V2 extends WP_REST_Users_Controller /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function rest_prepare_response_filter( WP_REST_Response $response, WP_User $user, WP_REST_Request $request ) {
			...
			$namespace = trailingslashit( ebox_REST_API_NAMESPACE ) . $this->version;
			...
			if ( ! isset( $current_links['teams'] ) ) {
				$links['teams'] = array(
					'href'       => rest_url( trailingslashit( $base ) . $user->ID ) . '/' . $this->get_rest_base( 'users-teams' ),
					'embeddable' => true,
				);
			}
