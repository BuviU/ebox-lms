<?php
/**
 * ebox REST API V2 Courses Steps Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the association
 * between the ebox Courses (ebox-courses) and the Course Steps.
 * Course Steps are modules (ebox-modules), Topics (ebox-topic), and
 * quizzes (ebox-quiz) post types.
 *
...
if ( ( ! class_exists( 'LD_REST_Courses_Steps_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Courses Steps Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Courses_Steps_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function register_routes() {
			...
			$courses_namespace = trailingslashit( ebox_REST_API_NAMESPACE ) . $this->version;
			...
			$schema = array(
				...
				'properties' => array(
					...
					'type' => array(
						'description' => sprintf(
							// translators: placeholder: course.
							esc_html_x( 'The %s step type.', 'placeholder: course', 'ebox' ),
		...
		public function get_collection_params() {
			...
			$query_params['type']    = array(
				'description' => __( 'Filter returned results by step type.', 'ebox' ),
				'type'        => 'string',
				'default'     => 'all',
				'enum'        => array(
					'all',
		...
		public function get_course_steps_permissions_check( $request ) {
			if ( ! is_user_logged_in() ) {
				 ...
				 */
				if ( apply_filters( 'ebox_rest_course_steps_allow_anonymous_read', false ) ) {
					return true;
				}
			...
			$enrolled_courses = ebox_user_get_enrolled_courses( get_current_user_id() );
				...
				if ( isset( $_GET['_embed'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					...
					if ( ( isset( $data_embed ) ) && ( ! empty( $data_embed ) ) ) {
						...
						foreach ( $data_embed as $post_type => $posts_ids ) {
							if ( ( ! empty( $posts_ids ) ) && ( ( in_array( $post_type, ebox_get_post_types( 'course_steps' ), true ) ) ) ) {

								$base_slug = $post_type;
								if ( 'ebox-courses' === $post_type ) {
									$base_slug = 'modules';
								} elseif ( 'ebox-modules' === $post_type ) {
									$base_slug = 'modules';
								} elseif ( 'ebox-topic' === $post_type ) {
									$base_slug = 'topics';
								} elseif ( 'ebox-quiz' === $post_type ) {
									$base_slug = 'quizzes';
								}
		...
		public function update_course_steps( $request ) {
			...
			if ( empty( $current_user_id ) ) {
				return new WP_Error( 'rest_not_logged_in', esc_html__( 'You are not currently logged in.', 'ebox' ), array( 'status' => 401 ) );
			}
			...
			if ( ! empty( $body ) ) {
				...
				if ( ( $body ) && ( json_last_error() == JSON_ERROR_NONE ) ) {
					...
					$steps['ebox-modules'] = array();
					$steps['ebox-quiz']    = array();

					if ( ( isset( $body['ebox-modules'] ) ) && ( ! empty( $body['ebox-modules'] ) ) ) {
						foreach ( $body['ebox-modules'] as $lesson_id => $lesson_set ) {
							$steps['ebox-modules'][ $lesson_id ]               = array();
							$steps['ebox-modules'][ $lesson_id ]['ebox-topic'] = array();
							$steps['ebox-modules'][ $lesson_id ]['ebox-quiz']  = array();

							if ( ( isset( $lesson_set['ebox-topic'] ) ) && ( ! empty( $lesson_set['ebox-topic'] ) ) ) {

								foreach ( $lesson_set['ebox-topic'] as $topic_id => $topic_set ) {
									$steps['ebox-modules'][ $lesson_id ]['ebox-topic'][ $topic_id ]              = array();
									$steps['ebox-modules'][ $lesson_id ]['ebox-topic'][ $topic_id ]['ebox-quiz'] = array();

									if ( ( isset( $topic_set['ebox-quiz'] ) ) && ( ! empty( $topic_set['ebox-quiz'] ) ) ) {
										foreach ( $topic_set['ebox-quiz'] as $quiz_id => $quiz_set ) {
											$steps['ebox-modules'][ $lesson_id ]['ebox-topic'][ $topic_id ]['ebox-quiz'][ $quiz_id ] = array();
										}
									}
								}
							}

							if ( ( isset( $lesson_set['ebox-quiz'] ) ) && ( ! empty( $lesson_set['ebox-quiz'] ) ) ) {
								foreach ( $lesson_set['ebox-quiz'] as $quiz_id => $quiz_set ) {
									$steps['ebox-modules'][ $lesson_id ]['ebox-quiz'][ $quiz_id ] = array();
								}
							}
						}
					}

					if ( ( isset( $body['ebox-quiz'] ) ) && ( ! empty( $body['ebox-quiz'] ) ) ) {
						$steps['ebox-quiz'] = $body['ebox-quiz'];
					}
