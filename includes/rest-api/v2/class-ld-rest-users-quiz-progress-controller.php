<?php
/**
 * ebox V2 REST API Users Quiz Progress Controller.
 *
 * @since 3.3.0
...
if ( ( ! class_exists( 'LD_REST_Users_Quiz_Progress_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox V2 REST API Users Quiz Progress Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Users_Quiz_Progress_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function get_public_item_schema() {
			$schema = array(
				...
				'properties' => array(
					...
					'percentage'    => array(
						'description' => esc_html__( 'Percentage passed', 'ebox' ),
						'type'        => 'number',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					'timespent'     => array(
						'description' => esc_html__( 'Timespent', 'ebox' ),
						'type'        => 'number',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					'has_graded'    => array(
						'description' => esc_html__( 'Has Graded', 'ebox' ),
						'type'        => 'boolean',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					'started'       => array(
						'description' => esc_html__( 'Started timestamp', 'ebox' ),
						'type'        => array(
							'string',
							'null',
						),
					...
					'completed'     => array(
						'description' => esc_html__( 'Completed timestamp', 'ebox' ),
						'type'        => array(
							'string',
							'null',
						),
					...
					'statistic'     => array(
						'description' => esc_html__( 'Statistic ID', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
		...
		public function get_items_permissions_check( $request ) {
			...
			if ( empty( $current_user_id ) ) {
				if ( ! current_user_can( 'edit_user', $user_id ) ) {
					return new WP_Error( 'rest_user_invalid_id', __( 'Invalid current user.', 'ebox' ), array( 'status' => 404 ) );
				}
			}

			if ( ( $user_id != $current_user_id ) && ( ! ebox_is_admin_user( $current_user_id ) ) ) {
				if ( ! current_user_can( 'edit_user', $user_id ) ) {
					return new WP_Error( 'rest_cannot_edit', __( 'Sorry, you are not allowed to edit this user.', 'ebox' ), array( 'status' => rest_authorization_required_code() ) );
				}
			}
		...
		public function get_items( $request ) {
			...
			if ( empty( $current_user_id ) ) {
				return new WP_Error( 'rest_user_invalid_id', esc_html__( 'Invalid current user ID.', 'ebox' ), array( 'status' => 404 ) );
			}

			$user_course_ids = array();
			if ( ( $current_user_id !== $user_id ) && ( ! ebox_is_admin_user( $current_user_id ) ) ) {
				if ( ebox_is_team_leader_user( $current_user_id ) ) {
					if ( ! ebox_is_team_leader_of_user( $current_user_id, $user_id ) ) {
						return new WP_Error( 'rest_user_invalid_id', esc_html__( 'Not allowed to view other user content.', 'ebox' ), array( 'status' => 401 ) );
					}

					$team_leader_team_ids = ebox_get_administrators_team_ids( $current_user_id );
			...
			// If the course_id is set then we can also search for modules and topics.
			...
			if ( ( isset( $course_info['quizzes'] ) ) && ( ! empty( $course_info['quizzes'] ) ) ) {
				...
				foreach ( $course_info['quizzes'] as $quiz ) {
					...
					if ( ! empty( $quiz['lesson'] ) ) {
						$links['lesson'] = array(
							array(
								'href'       => rest_url( $this->namespace . '/' . $this->get_rest_base( 'modules' ) . '/' . absint( $quiz['lesson'] ) ),
								'embeddable' => true,
							),
						);
					}
