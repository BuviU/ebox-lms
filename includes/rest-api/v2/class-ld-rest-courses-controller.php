<?php
/**
 * ebox REST API V2 Courses Post Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the ebox
 * custom post type Courses (ebox-courses).
 *
...
if ( ( ! class_exists( 'LD_REST_Courses_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Courses Post Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Courses_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function register_routes() {
			...
			include ebox_REST_API_DIR . '/' . $this->version . '/class-ld-rest-courses-steps-controller.php';
			...
			include ebox_REST_API_DIR . '/' . $this->version . '/class-ld-rest-courses-prerequisites-controller.php';
			...
			include ebox_REST_API_DIR . '/' . $this->version . '/class-ld-rest-courses-users-controller.php';
			...
			// Added support for nested wp-json/courses/<course ID>/teams URL.
			include ebox_REST_API_DIR . '/' . $this->version . '/class-ld-rest-courses-teams-controller.php';
			$this->sub_controllers['LD_REST_Courses_Teams_Controller_V2'] = new LD_REST_Courses_Teams_Controller_V2();
			$this->sub_controllers['LD_REST_Courses_Teams_Controller_V2']->register_routes();
		}
		...
		protected function register_fields_metabox() {
			...
			$this->metaboxes['ebox_Settings_Metabox_Course_Display_Content'] = ebox_Settings_Metabox_Course_Display_Content::add_metabox_instance();
			...
			$this->metaboxes['ebox_Settings_Metabox_Course_Access_Settings'] = ebox_Settings_Metabox_Course_Access_Settings::add_metabox_instance();
			...
			$this->metaboxes['ebox_Settings_Metabox_Course_Navigation_Settings'] = ebox_Settings_Metabox_Course_Navigation_Settings::add_metabox_instance();
			...
			if ( $this->post_type === $post->post_type ) {
				...
				if ( ( ! empty( $request_route ) ) && ( strpos( $request_route, $base ) !== false ) ) {
					...
					if ( ! isset( $current_links['price-type'] ) ) {
						$course_price_type = ebox_get_course_meta_setting( $post->ID, 'course_price_type' );
					...
					if ( ! isset( $current_links['teams'] ) ) {
						$links['teams'] = array(
							'href'       => rest_url( trailingslashit( $base ) . $post->ID ) . '/' . $this->get_rest_base( 'courses-teams' ),
							'embeddable' => true,
						);
					}
