<?php
/**
 * ebox REST API V2 Assignment (ebox-assignment) Post Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the ebox
 * custom post type Assignments (ebox-assignment).
 *
...
if ( ( ! class_exists( 'LD_REST_Assignments_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Assignment (ebox-assignment) Post Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Assignments_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
			...
			register_rest_field(
				...
				array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Assignment Approved Status', 'ebox' ),
						'type'        => 'string',
						'default'     => '',
						'required'    => false,
						'enum'        => array(
			...
			register_rest_field(
				...
				array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Assignment Points Enabled', 'ebox' ),
						'type'        => 'boolean',
						'default'     => 0,
						'required'    => false,
						'context'     => array( 'view' ),
			...
			register_rest_field(
				...
				array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Assignment Points Maximum', 'ebox' ),
						'type'        => 'integer',
						'default'     => 0,
						'required'    => false,
						'context'     => array( 'view' ),
			...
			register_rest_field(
				...
				array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Assignment Points Awarded', 'ebox' ),
						'type'        => 'integer',
						'default'     => 0,
						'required'    => false,
						'context'     => array( 'view', 'edit' ),
		...
		/**
		 * Check user permission to get/access modules.
		 *
		 * @since 3.3.0
		 *
		 * @param object $request  WP_REST_Request instance.
			...
			if ( ( isset( $postdata['id'] ) ) && ( ! empty( $postdata['id'] ) ) && ( $post_type == $this->post_type ) ) {
				...
				switch ( $field_name ) {
					...
					case 'approved_status':
						$field_value = ebox_is_assignment_approved_by_meta( $postdata['id'] );
						if ( '1' === $field_value ) {
					...
					case 'points_enabled':
						$field_value = ebox_assignment_is_points_enabled( $postdata['id'] );
						break;

					case 'points_max':
						if ( ebox_assignment_is_points_enabled( $postdata['id'] ) ) {
							$field_value = (int) ebox_get_setting( $parent_id, 'lesson_assignment_points_amount' );
						} else {
							$field_value = 0;
						}
						break;
			...
			if ( ( is_a( $post, 'WP_Post' ) ) && ( $post->post_type == $this->post_type ) ) {
				...
				switch ( $field_name ) {
					case 'approved_status':
						...
						if ( $post_value ) {
							ebox_assignment_mark_approved( $post->ID );
						}
						break;

					case 'points_awarded':
						if ( ebox_assignment_is_points_enabled( $post->ID ) ) {
							$max_points     = absint( ebox_get_setting( $lesson_id, 'lesson_assignment_points_amount' ) );
