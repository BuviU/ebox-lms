<?php
/**
 * ebox REST API V2 Courses Users Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the association
 * between a Course (ebox-courses) and Users enrolled.
 *
...
if ( ( ! class_exists( 'LD_REST_Courses_Users_Controller_V2' ) ) && ( class_exists( 'LD_REST_Users_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Courses Users Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Courses_Users_Controller_V2 extends LD_REST_Users_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
			...
			register_rest_route(
				...
				array(
					...
					array(
						...
						'args'                => array(
							'user_ids' => array(
								'description' => esc_html__( 'User IDs to update in Course. Limit 50 per request.', 'ebox' ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
					...
					array(
						...
						'args'                => array(
							'user_ids' => array(
								'description' => esc_html__( 'User IDs to remove from Course. Limit 50 per request.', 'ebox' ),
								'required'    => true,
								'type'        => 'array',
								'items'       => array(
									'type' => 'integer',
		...
		public function update_courses_users( $request ) {
			...
			if ( 'open' === $course_price_type ) {
				return new WP_Error(
					'ebox_rest_rejected_course_open',
					sprintf(
						// translators: placeholder: Course.
						esc_html_x(
			...
			foreach ( $user_ids as $user_id ) {
				...
				if ( ( true === $ignore_admin_users ) && ( ebox_is_admin_user( $user_id ) ) ) {
					$data_item->user_id = $user_id;
					$data_item->status  = 'failed';
					$data_item->code    = 'ebox_rest_admin_auto_enroll';
					$data_item->message = esc_html__( 'Admin users are auto-enrolled.', 'ebox' );
					$data[]             = $data_item;
