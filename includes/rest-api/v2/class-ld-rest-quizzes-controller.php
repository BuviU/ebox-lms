<?php
/**
 * ebox REST API V2 Quizzes Post Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the ebox
 * custom post type Quizzes (ebox-quiz).
 *
...
if ( ( ! class_exists( 'LD_REST_Quizzes_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Quizzes Post Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Quizzes_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		protected function register_fields_metabox() {
			...
			$this->metaboxes['ebox_Settings_Metabox_Quiz_Access_Settings'] = ebox_Settings_Metabox_Quiz_Access_Settings::add_metabox_instance();
			...
			$this->metaboxes['ebox_Settings_Metabox_Quiz_Progress_Settings'] = ebox_Settings_Metabox_Quiz_Progress_Settings::add_metabox_instance();

			require_once ebox_LMS_PLUGIN_DIR . '/includes/settings/settings-metaboxes/class-ld-settings-metabox-quiz-display-content.php';
			$this->metaboxes['ebox_Settings_Metabox_Quiz_Display_Content'] = ebox_Settings_Metabox_Quiz_Display_Content::add_metabox_instance();
			...
			$this->metaboxes['ebox_Settings_Metabox_Quiz_Results_Options'] = ebox_Settings_Metabox_Quiz_Results_Options::add_metabox_instance();
			...
			$this->metaboxes['ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings'] = ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings::add_metabox_instance();
		...
		/**
		 * For ebox post type we override the default order/orderby
		...
		public function get_item_permissions_check( $request ) {
			...
			if ( ( true === $return ) && ( ! ebox_is_admin_user() ) ) {
				...
				if ( ( $quiz_id ) && ( ebox_lms_has_access( $quiz_id ) ) ) {
					return true;
				}
		...
		public function get_items_permissions_check( $request ) {
			...
			if ( ( true === $return ) && ( 'view' === $request['context'] ) && ( ! ebox_is_admin_user() ) ) {
		...
		public function rest_query_filter( $query_args, $request ) {
			...
			if ( ( $this->course_post ) && ( is_a( $this->course_post, 'WP_Post' ) ) && ( 'ebox-courses' === $this->course_post->post_type ) ) {
				...
				if ( $this->topic_post ) {
					$step_ids = ebox_course_get_children_of_step( $this->course_post->ID, $this->topic_post->ID, $this->post_type );
				...
				if ( ! empty( $step_ids ) ) {
					...
					if ( ! isset( $_GET['orderby'] ) ) {
						if ( isset( $course_modules_args['orderby'] ) ) {
							$query_args['orderby'] = $course_modules_args['orderby'];
						} else {
					...
					if ( ! isset( $_GET['order'] ) ) {
						if ( isset( $course_modules_args['order'] ) ) {
							$query_args['order'] = $course_modules_args['order'];
						} else {
				...
				if ( get_current_user_id() ) {
					 ...
					 */
					$step_ids = ebox_get_non_course_qizzes(); // cspell:disable-line.
				} else {
					/**
					 ...
					 */
					$step_ids = ebox_get_open_quizzes( true );
				}
		...
		public function get_rest_settings_field_value( array $postdata, $field_name, WP_REST_Request $request, $post_type ) {
			...
			if ( 'lesson' === $field_name ) {
				$field_value = ebox_course_get_single_parent_step( $postdata['course'], $postdata['id'], ebox_get_post_type_slug( 'lesson' ) );
			} elseif ( 'topic' === $field_name ) {
				$field_value = ebox_course_get_single_parent_step( $postdata['course'], $postdata['id'], ebox_get_post_type_slug( 'topic' ) );
			}
				...
				if ( ( ! empty( $request_route ) ) && ( strpos( $request_route, $base ) !== false ) ) {
					...
					if ( ! isset( $response->links['statistics'] ) ) {
						...
						if ( ! empty( $quiz_pro_id ) ) {
							$quiz_pro_statistics_on = ebox_get_setting( $post, 'statisticsOn', true );
		...
		public function rest_after_insert_action( $post, $request, $creating ) {
			if ( ( $post ) && ( is_a( $post, 'WP_Post' ) ) && ( $post->post_type === $this->post_type ) ) {
				 ...
				 * includes/admin/classes-posts-edits/class-ebox-admin-quiz-edit.php
				 */
				...
				$quiz_id  = absint( ebox_get_setting( $post->ID, 'quiz_pro', true ) );
