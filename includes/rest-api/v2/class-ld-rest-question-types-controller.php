<?php
/**
 * ebox REST API V2 Question Types Controller.
 *
 * @since 3.3.0
...
if ( ( ! class_exists( 'LD_REST_Question_Types_Controller_V2' ) ) && ( class_exists( 'WP_REST_Controller' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Question Types Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Question_Types_Controller_V2 extends WP_REST_Controller /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
					...
					'args'   => array(
						'slug' => array(
							'description' => sprintf(
								...
								esc_html_x( 'An alphanumeric identifier for the %s type', 'placeholder: question', 'ebox' ),
		...
		protected function init_types_set() {
			$this->types = array(
				'single'             => array(
					'slug'        => 'single',
					'name'        => esc_html__( 'Single choice', 'ebox' ),
					'description' => '',
				),
				'multiple'           => array(
					'slug'        => 'multiple',
					'name'        => esc_html__( 'Multiple choice', 'ebox' ),
					'description' => '',
				),
				'free_answer'        => array(
					'slug'        => 'free_answer',
					'name'        => esc_html__( '"Free" choice', 'ebox' ),
					'description' => '',
				),

				'sort_answer'        => array(
					'slug'        => 'sort_answer',
					'name'        => esc_html__( '"Sorting" choice', 'ebox' ),
					'description' => '',
				),
				'matrix_sort_answer' => array(
					'slug'        => 'matrix_sort_answer',
					'name'        => esc_html__( '"Matrix Sorting" choice', 'ebox' ),
					'description' => '',
				),
				'cloze_answer'       => array(
					'slug'        => 'cloze_answer',
					'name'        => esc_html__( 'Closed', 'ebox' ),
					'description' => '',
				),
				'assessment_answer'  => array(
					'slug'        => 'assessment_answer',
					'name'        => esc_html__( 'Assessment', 'ebox' ),
					'description' => '',
				),
				'essay'              => array(
					'slug'        => 'essay',
					'name'        => esc_html__( 'Essay / Open Answer', 'ebox' ),
					'description' => '',
				),
			);
		}
		...
		public function get_item( $request ) {
			...
			if ( ( empty( $type_slug ) ) || ( ! isset( $this->types[ $type_slug ] ) ) ) {
				return new WP_Error(
					...
					sprintf(
						// translators: placeholder: Question.
						_x( 'Invalid %s Type.', 'placeholder: Question', 'ebox' ),
				...
				'properties' => array(
					'name'        => array(
						'description' => sprintf(
							...
							ebox_Custom_Label::label_to_lower( 'question' )
						),
						'type'        => 'string',
