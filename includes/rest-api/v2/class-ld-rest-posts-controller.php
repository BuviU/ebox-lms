<?php
/**
 * ebox REST API V2 Post Controller.
 *
 * This Controller is used as the parent Controller for all ebox
 * custom post types like Courses (ebox-courses), modules (ebox-modules), Topics (ebox-topic),
 * Quizzes (ebox-quiz) and, Questions (ebox-question).
 *
...
if ( ( ! class_exists( 'LD_REST_Posts_Controller_V2' ) ) && ( class_exists( 'WP_REST_Posts_Controller' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Post Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Posts_Controller_V2 extends WP_REST_Posts_Controller /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		 ...
		 * /ldlms/v2/users/X/teams where '/teams'
		 * is the $sub_base.
		 *
		 * @var array $sub_controllers.
		 */
		...
		public function __construct( $post_type = '' ) {
			 ...
			 */
			$this->namespace = trailingslashit( ebox_REST_API_NAMESPACE ) . $this->version;
		...
		protected function swap_rest_registered_fields( $object_type = null ) {
			...
			if ( ( $object_type ) && ( $object_type === $this->post_type ) ) {
				...
				if ( isset( $wp_rest_additional_fields[ $object_type ] ) ) {
					...
					global $ebox_lms;
					$post_args_fields = $ebox_lms->get_post_args_section( $object_type, 'fields' );
				...
				if ( ( is_a( $ld_post, 'WP_Post' ) ) && ( $ld_post->post_type == $this->post_type ) ) {
					...
					if ( isset( $this->metaboxes_fields[ $field_name ] ) ) {
						...
						if ( ( isset( $field_set['settings_field']['name'] ) ) && ( ! empty( $field_set['settings_field']['name'] ) ) ) {
							if ( ( isset( $field_set['metabox'] ) ) && ( ! empty( $field_set['metabox'] ) ) && ( 'ebox_Settings_Metabox' === get_parent_class( $field_set['metabox'] ) ) ) {
								...
								$field_value = ebox_get_setting( $ld_post, $field_set['settings_field']['name'] );
							}
						}
						if ( ( isset( $field_set['settings_field']['args']['validate_callback'] ) ) && ( ! empty( $field_set['settings_field']['args']['validate_callback'] ) ) && ( is_callable( $field_set['settings_field']['args']['validate_callback'] ) ) ) {
							...
							$field_instance = ebox_Settings_Fields::get_field_instance( $field_set['settings_field']['args']['type'] );
				...
				if ( isset( $this->metaboxes_fields[ $field_name ] ) ) {
					...
					if ( ( $field_instance ) && ( 'ebox_Settings_Fields' === get_parent_class( $field_instance ) ) ) {
						...
						if ( ( isset( $field_set['metabox'] ) ) && ( ! empty( $field_set['metabox'] ) ) && ( 'ebox_Settings_Metabox' === get_parent_class( $field_set['metabox'] ) ) ) {
							...
							$field_value = ebox_get_setting( $post, $setting_field_name );
							if ( $field_value !== $post_value ) {
								return ebox_update_setting( $post->ID, $setting_field_name, $post_value );
							}
						}
					}
				}
		...
		public function rest_collection_params_filter( array $query_params, WP_Post_Type $post_type ) {
			if ( ( in_array( $this->post_type, ebox_get_post_types( 'course_steps' ), true ) ) ) {
		...
		public function rest_prepare_response_filter( WP_REST_Response $response, WP_Post $post, WP_REST_Request $request ) {
			if ( ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) == 'yes' ) && ( in_array( $post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) ) {
				...
				if ( ( isset( $request_params_json['course_id'] ) ) && ( ! empty( $request_params_json['course_id'] ) ) ) {
					...
					if ( ! empty( $course_id ) ) {
						$link                                 = ebox_get_step_permalink( $post->ID, $course_id );
		...
		protected function rest_post_type_has_archive( $post_type_slug = '' ) {
			if ( ebox_is_admin_user() ) {
				$ebox_rest_archive_bypass = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Admin_User', 'bypass_course_limits_admin_users' );
				if ( 'yes' === $ebox_rest_archive_bypass ) {
					$ebox_rest_archive_bypass = true;
				} else {
					$ebox_rest_archive_bypass = ebox_post_type_has_archive( $post_type_slug );
				}
			} else {
				$ebox_rest_archive_bypass = ebox_post_type_has_archive( $post_type_slug );
			}
			...
			return apply_filters( 'ebox_rest_archive_bypass', $ebox_rest_archive_bypass, $post_type_slug );
		}
		...
		protected function rest_init_request_posts( WP_REST_Request $request ) {
			...
			if ( ! empty( $lesson_id ) ) {
				...
				if ( ( $lesson_post ) && ( is_a( $lesson_post, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'lesson' ) === $lesson_post->post_type ) ) {
			...
			if ( ! empty( $topic_id ) ) {
				...
				if ( ( $topic_post ) && ( is_a( $topic_post, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'topic' ) === $topic_post->post_type ) ) {
