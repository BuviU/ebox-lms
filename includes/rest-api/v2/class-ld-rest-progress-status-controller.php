<?php
/**
 * ebox REST API V2 Progress Status Controller.
 *
 * @since 3.4.0
...
if ( ( ! class_exists( 'LD_REST_Progress_Status_Controller_V2' ) ) && ( class_exists( 'WP_REST_Controller' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Progress Status Controller.
	 *
	 * @since 3.4.0
	...
	class LD_REST_Progress_Status_Controller_V2 extends WP_REST_Controller /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function register_routes() {
			...
			register_rest_route(
				...
				array(
					'args'   => array(
						'slug' => array(
							'description' => __( 'An alphanumeric identifier for the progress status.', 'ebox' ),
							'type'        => 'string',
						),
					),
					array(
		...
		protected function init_sets() {
			...
			$this->types = array();
			foreach ( $ebox_course_statuses as $status_slug => $status_label ) {
		...
		public function get_item( $request ) {
			...
			if ( ( empty( $progress_status_slug ) ) || ( ! isset( $this->types[ $progress_status_slug ] ) ) ) {
				return new WP_Error(
					...
					__( 'Invalid Progress Status.', 'ebox' ),
					array( 'status' => 404 )
				);
			}
		...
		public function get_item_schema() {
			...
			$schema = array(
				...
				'properties' => array(
					'name'        => array(
						'description' => __( 'The title for the progress status.', 'ebox' ),
						'type'        => 'string',
						'context'     => array( 'embed', 'view' ),
						'readonly'    => true,
					),
					'description' => array(
						'description' => __( 'The description for the progress status.', 'ebox' ),
						'type'        => 'string',
						'context'     => array( 'embed', 'view' ),
						'readonly'    => true,
					),
