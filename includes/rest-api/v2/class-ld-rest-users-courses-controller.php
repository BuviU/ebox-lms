 ...
 * between a User and Courses (ebox-courses).
 *
...
if ( ( ! class_exists( 'LD_REST_Users_Courses_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	class LD_REST_Users_Courses_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function update_user_courses( $request ) {
			...
			if ( ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Admin_User', 'courses_autoenroll_admin_users' ) ) && ( ebox_is_admin_user( $user_id ) ) ) {
				return new WP_Error(
					'ebox_rest_admin_auto_enroll',
					esc_html__( 'Admin users are auto-enrolled.', 'ebox' ),
					array(
						'status' => 404,
					)
				);
			...
			foreach ( $course_ids as $course_id ) {
				...
				if ( 'open' === $course_price_type ) {
					$data_item->course_id = $course_id;
					$data_item->status    = 'failed';
					$data_item->code      = 'ebox_rest_rejected_course_open';
					$data_item->message   = sprintf(
						// translators: placeholder: Course.
						esc_html_x(
		...
		public function delete_user_courses( $request ) {
			$user_id = $request['id'];
			if ( empty( $user_id ) ) {
				return new WP_Error( 'rest_post_invalid_id', esc_html__( 'Invalid User ID.', 'ebox' ), array( 'status' => 404 ) );
			}
