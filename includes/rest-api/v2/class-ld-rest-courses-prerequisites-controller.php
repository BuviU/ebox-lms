<?php
/**
 * ebox REST API V2 Courses Prerequisites Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the association
 * between the ebox Courses (ebox-courses) and Course
 * Prerequisites.
 *
...
if ( ( ! class_exists( 'LD_REST_Courses_Prerequisites_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Courses Prerequisites Controller.
	 *
	 * @since 3.3.0
			...
			if ( ( ! empty( $route_url ) ) && ( $ld_route_url === $route_url ) ) {
				...
				if ( ! empty( $course_id ) ) {
					...
					if ( ( $this->course_post ) && ( is_a( $this->course_post, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'course' ) === $this->course_post->post_type ) ) {
						$course_pre = ebox_get_course_prerequisites( $this->course_post->ID, false );
						if ( ! empty( $course_pre ) ) {
