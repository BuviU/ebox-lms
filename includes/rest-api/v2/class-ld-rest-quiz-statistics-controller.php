<?php
/**
 * ebox REST API Quiz Statistics Controller.
 *
 * @since 3.3.0
...
if ( ( ! class_exists( 'LD_REST_Quiz_Statistics_Controller_V2' ) ) && class_exists( 'WP_REST_Controller' ) ) {
	...
	/**
	 * Class ebox REST API Quiz Statistics Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Quiz_Statistics_Controller_V2 extends WP_REST_Controller implements Iterator /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function register_routes() {
			...
			add_filter( 'ebox_rest_statistic_response', array( $this, 'statistic_response_embed_links' ), 10, 1 );
			add_filter( 'ebox_rest_statistic_question_response', array( $this, 'statistic_question_response_embed_links' ), 10, 1 );
		...
		public function pre_callback_ops( $response, $handler, $request ) {
			if ( $this->is_qs_endpoint( $request ) ) {
				 ...
				 */
				include untrailingslashit( ebox_LMS_PLUGIN_DIR ) . '/includes/interfaces/interface-ldlms-answer.php';
				include untrailingslashit( ebox_LMS_PLUGIN_DIR ) . '/includes/classes/answer-types/class-ldlms-base-answer-type.php';
				include untrailingslashit( ebox_LMS_PLUGIN_DIR ) . '/includes/classes/answer-types/class-ldlms-sort-answer-type.php';
				include untrailingslashit( ebox_LMS_PLUGIN_DIR ) . '/includes/classes/answer-types/class-ldlms-assessment-answer-type.php';
				include untrailingslashit( ebox_LMS_PLUGIN_DIR ) . '/includes/classes/answer-types/class-ldlms-cloze-answer-type.php';
				include untrailingslashit( ebox_LMS_PLUGIN_DIR ) . '/includes/classes/answer-types/class-ldlms-free-answer-type.php';
				include untrailingslashit( ebox_LMS_PLUGIN_DIR ) . '/includes/classes/answer-types/class-ldlms-essay-answer-type.php';

				/**
				 * Add necessary filters for statistics query.
				 */
				add_filter( 'ebox_statrefs_joins', array( $this, 'join_quiz_master_and_postmeta' ) );
				add_filter( 'ebox_statrefs_where', array( $this, 'statistics_ref_where' ) );
				add_filter( 'ebox_statrefs_where', array( $this, 'statistics_ref_users_where' ), 20 );
				add_filter( 'ebox_statrefs_where', array( $this, 'statistics_ref_quiz_where' ), 25 );
				add_filter( 'ebox_statrefs_where', array( $this, 'statistics_ref_duration_where' ), 25 );
		...
		public function get_items( $request ) {
			if ( ! $this->stat_refs ) {
				return new WP_Error( 404, __( 'No records found for this request.', 'ebox' ) );
			}
		...
		public function get_item( $request ) {
			if ( ! $this->valid() ) {
				return new WP_Error( 404, __( 'Invalid entry', 'ebox' ) );
			}

			$stat_ref_id = $this->current()->getStatisticRefId();
			$stat_object = $this->stat_response_object();

			$stat_object = apply_filters( 'ebox_rest_statistic_response', (array) $stat_object );
		...
		public function get_questions( $request ) {
			...
			try {
				...
				foreach ( $questions as $question ) {
					...
					$question_response->question      = ebox_get_question_post_by_pro_id( $question->getId() ) ? ebox_get_question_post_by_pro_id( $question->getId() ) : null;
					...
					$question_response = apply_filters(
						'ebox_rest_statistic_question_response',
						$question_response,
						$question,
						$stat_model->getAnswerData(),
						$this->current()
		...
		public function can_access_stats( $request ) {
			...
			$quiz_pro_statistics_on = ebox_get_setting( $quiz_id, 'statisticsOn', true );
		...
		public function statistic_response_embed_links( array $response ) {
			...
			if ( ( isset( $response['quiz'] ) ) && ( ! empty( $response['quiz'] ) ) ) {
				...
				if ( ebox_get_post_type_slug( 'quiz' ) === get_post_type( absint( $response['quiz'] ) ) ) {
		...
		public function statistic_question_response_embed_links( array $response ) {
			...
			if ( ( isset( $response['quiz'] ) ) && ( ! empty( $response['quiz'] ) ) && ( ebox_get_post_type_slug( 'quiz' ) === get_post_type( absint( $response['quiz'] ) ) ) ) {
		 ...
		 * 2. Team leaders will have access to statistics of only those
		 *    users which belong to their teams.
		...
		private function users_for_stats() {
			...
			if ( ebox_is_admin_user() ) {
				...
				if ( ebox_get_team_leader_manage_courses() ) {
					/**
					 * If the Team Leader can manage_courses they have will access
					 ...
					 * Course within Team managed by the Team Leader.
					 */
					$quiz_users     = array();
					$leader_courses = ebox_get_teams_administrators_courses( get_current_user_id() );
					if ( ! empty( $leader_courses ) ) {
						$quiz_courses = array();
						if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) ) {
							$quiz_courses = ebox_get_courses_for_step( $quiz_id, true );
						} else {
							$quiz_course = ebox_get_setting( $quiz_id, 'course' );
						...
						if ( ! empty( $quiz_courses ) ) {
							...
							if ( ! empty( $common_courses ) ) {
								/**
								 * The following will get a list of all users within the Teams
								 * managed by the Team Leader. This list of users will be passed
								 ...
								 * are associated with multiple teams. And the user is in both
								 * teams. So potentially we will pull in statistics records for
								 * the other course quizzes.
								 */
								$quiz_users = ebox_get_teams_administrators_users( get_current_user_id() );
							}
						}
					}
				...
				// If here then non-admin and non-team leader user.
				$quiz_id = $this->request->get_param( 'quiz' );
				if ( ! empty( $quiz_id ) ) {
		...
		private function get_user_quiz_details() {
			...
			$user_quizzes       = (array) get_user_meta( $this->current()->getUserId(), '_ebox-quizzes', true );
		...
		private function make_answer_obj( $type, $args = array() ) {
			...
			switch ( $type ) {
				...
				default:
					...
					throw new Exception( sprintf( esc_html_x( 'Invalid %s type supplied', 'placeholder: question', 'ebox' ), ebox_get_custom_label( 'question' ) ), 404 );
			}
		}
		...
		public function get_collection_params_statistics() {
			...
			$query_params['order'] = array(
				'description' => __( 'Order sort attribute ascending or descending.', 'ebox' ),
				'type'        => 'string',
				'default'     => 'desc',
				'enum'        => array( 'asc', 'desc' ),
			);
			...
			$query_params['orderby'] = array(
				'description' => __( 'Sort collection by object attribute.', 'ebox' ),
				'type'        => 'string',
				'default'     => 'date',
				'enum'        => array(
					'date',
			...
			if ( ! isset( $query_params['user'] ) ) {
				$query_params['user'] = array(
					'description' => esc_html__( 'Filter by User ID', 'ebox' ),
					'type'        => 'integer',
					'required'    => false,
					'context'     => array( 'view', 'edit' ),
				);
			...
			$query_params['after'] = array(
				'description' => __( 'Limit response items after a given ISO8601 compliant date.', 'ebox' ),
				'type'        => 'string',
				'format'      => 'date-time',
			);

			$query_params['before'] = array(
				'description' => __( 'Limit response to items before a given ISO8601 compliant date.', 'ebox' ),
				'type'        => 'string',
				'format'      => 'date-time',
			);
		...
		protected function get_rest_base( $rest_slug = '', $default_value = '' ) {
			...
			if ( ! empty( $rest_slug ) ) {
				...
				$rest_base_value = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_REST_API', $rest_slug, $default_value );
			}
		...
		public function get_stats_questions_schema() {
			$schema = array(
				...
				'properties' => array(
					'id'            => array(
						'description' => __( 'Unique ID for Statistics Question.', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					'statistic'     => array(
						'description' => __( 'Statistics Ref ID.', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					...
					'points_scored' => array(
						'description' => esc_html__( 'Points scored.', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					'points_total'  => array(
						'description' => esc_html__( 'Points total.', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
					...
					'student'       => array(
						'description' => __( 'The collection of student submitted answers.', 'ebox' ),
						'type'        => 'object',
						'context'     => array( 'view' ),
						'readonly'    => true,
					),
		...
		public function get_stats_schema() {
			$schema = array(
				...
				'properties' => array(
					...
					'user'              => array(
						'description' => __( 'User ID.', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'embed', 'view' ),
						'readonly'    => true,
					),
					'date'              => array(
						'description' => __( 'Date.', 'ebox' ),
					...
					'answers_correct'   => array(
						'description' => __( 'Answer correct.', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'embed', 'view' ),
						'readonly'    => true,
					),
					'answers_incorrect' => array(
						'description' => __( 'Answer incorrect.', 'ebox' ),
						'type'        => 'integer',
						'context'     => array( 'embed', 'view' ),
						'readonly'    => true,
					),
