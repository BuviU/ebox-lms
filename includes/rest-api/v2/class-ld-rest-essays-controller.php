<?php
/**
 * ebox REST API V2 Essays Post Controller.
 *
 * This Controller class is used to GET/UPDATE/DELETE the ebox
 * custom post type essays (ebox-essay).
 *
...
if ( ( ! class_exists( 'LD_REST_Essays_Controller_V2' ) ) && ( class_exists( 'LD_REST_Posts_Controller_V2' ) ) ) {
	...
	/**
	 * Class ebox REST API V2 Essays Post Controller.
	 *
	 * @since 3.3.0
	...
	class LD_REST_Essays_Controller_V2 extends LD_REST_Posts_Controller_V2 /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function __construct( $post_type = '' ) {
			if ( empty( $post_type ) ) {
				$post_type = ebox_get_post_type_slug( 'essay' );
			}
			...
			register_rest_field(
				...
				array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Essay Points Maximum', 'ebox' ),
						'type'        => 'integer',
						'default'     => 0,
						'required'    => false,
						'context'     => array( 'view' ),
			...
			register_rest_field(
				...
				array(
					'schema'          => array(
						...
						'description' => esc_html__( 'Essay Points Awarded', 'ebox' ),
						'type'        => 'integer',
						'default'     => 0,
						'required'    => false,
		...
		public function rest_query_filter( $query_args, $request ) {
			...
			if ( is_user_logged_in() ) {
				...
				if ( ! empty( $filters['course_id'] ) ) {
					...
					if ( ( ! empty( $filters['topic_id'] ) ) && ( ebox_get_post_type_slug( 'topic' ) === get_post_type( $filters['topic_id'] ) ) ) {
				...
				if ( ! current_user_can( 'edit_others_essays' ) ) {
					if ( ebox_is_team_leader_user() ) {
						...
						if ( ! empty( $gl_team_ids ) ) {
							foreach ( $gl_team_ids as $team_id ) {
						...
						if ( ! empty( $filters['course_id'] ) ) {
							$user_team_ids = ebox_get_users_team_ids( get_current_user_id(), true );
							$user_team_ids = array_map( 'absint', $user_team_ids );
							if ( ( empty( $user_team_ids ) ) || ( ! in_array( $filters['course_id'], $user_team_ids, true ) ) ) {
								$query_args['post__in'] = array( 0 );
							}
						}
					}
			...
			if ( $this->post_type === $post->post_type ) {
				...
				if ( ( ! empty( $request_route ) ) && ( strpos( $request_route, $base ) !== false ) ) {
					...
					if ( ( ! isset( $response->links['lesson'] ) ) && ( ! empty( $lesson_id ) ) ) {
						$lesson_url = rest_url( trailingslashit( $this->namespace ) . $this->get_rest_base( 'modules' ) . '/' . $lesson_id );
			...
			if ( $post_type->name === $this->post_type ) {
				...
				if ( ! isset( $query_params['lesson'] ) ) {
					$query_params['lesson'] = array(
						'description' => sprintf(
							// translators: placeholder: Lesson.
							esc_html_x( 'Filter by %s ID', 'placeholder: Lesson', 'ebox' ),
							ebox_Custom_Label::get_label( 'lesson' )
						),
						'type'        => 'integer',
						'default'     => 0,
				...
				if ( ! isset( $query_params['topic'] ) ) {
					$query_params['topic'] = array(
						'description' => sprintf(
							// translators: placeholder: Topic.
							esc_html_x( 'Filter by %s ID', 'placeholder: Topic', 'ebox' ),
							ebox_Custom_Label::get_label( 'topic' )
						),
						'type'        => 'integer',
						'default'     => 0,
			...
			if ( ! empty( $post_id ) ) {
				if ( ! isset( $this->essay_post_data[ $post_id ] ) ) {
					...
					if ( ebox_get_post_type_slug( 'topic' ) === get_post_type( $lesson_id ) ) {
						...
						$lesson_id = (int) ebox_course_get_single_parent_step( $course_id, $topic_id );
					}
					...
					case 'points_awarded':
						...
						if ( ! empty( $quiz_id ) ) {
							...
							if ( $question instanceof WpProQuiz_Model_Question ) {
								$submitted_essay_data = ebox_get_submitted_essay_data( $quiz_id, $question_id, $post );
								...
								if ( $awarded_points !== $original_points_awarded ) {
									...
									ebox_update_submitted_essay_data( $quiz_id, $question_id, $post, $submitted_essay_data );
									...
									if ( ! is_null( $original_points_awarded ) && ! is_null( $submitted_essay_data['points_awarded'] ) ) {
										...
										ebox_update_quiz_data( $quiz_id, $question_id, $updated_scoring_data, $post );

										/** This action is documented in includes/quiz/ld-quiz-essays.php */
										do_action( 'ebox_essay_all_quiz_data_updated', $quiz_id, $question_id, $updated_scoring_data, $post );
									}
								}
							}
						}
