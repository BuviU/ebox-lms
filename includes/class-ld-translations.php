<?php
/**
 * ebox Admin Translations handler.
 * This class connects to a remote GlotPress server to retrieve needed translations for ebox core and related add-ons.
 *
 * @package ebox\Admin
 */
...
if ( ! class_exists( 'ebox_Translations' ) ) {
	/**
	 * Class for ebox Translations.
	 */
	class ebox_Translations {
				...
				if ( self::is_language_directory_writable( $project ) ) {
					...
					if ( ( isset( $translation_set['links'] ) ) && ( ! empty( $translation_set['links'] ) ) ) {
						foreach ( $translation_set['links'] as $link_key => $link_url ) {
							 ...
							 */
							$url_args = apply_filters( 'ebox_translations_url_args', array( 'timeout' => ebox_HTTP_REMOTE_GET_TIMEOUT ) );
								...
								if ( ! empty( $response_body ) ) {
									...
									if ( false !== $fp ) {
										...
										$reply_data['message'] = '<p>' . sprintf(
											...
											esc_html_x( 'Translation installed: %1$s (%2$s)', 'placeholders: Language Name, Language code', 'ebox' ),
							...
							if ( ( is_array( $response ) ) && ( wp_remote_retrieve_response_code( $response ) == '200' ) ) {
								...
								if ( ! empty( $response_body ) ) {
									...
									if ( false !== $fp ) {
										...
										$reply_data['message'] = '<p>' . sprintf(
											// translators: placeholders: Language Name, Language code.
											esc_html_x( 'Translation updated: %1$s (%2$s)', 'placeholders: Language Name, Language code', 'ebox' ),
											$translation_set['english_name'],
											$translation_set['wp_locale']
										) . '</p>';
									}
			...
			if ( ( ! empty( $project ) ) && ( ! empty( $locale ) ) ) {
				...
				if ( self::is_language_directory_writable( $project ) ) {
					...
					if ( ( isset( $translation_set['links'] ) ) && ( ! empty( $translation_set['links'] ) ) ) {
						foreach ( $translation_set['links'] as $link_key => $link_url ) {
							/** This filter is documented in includes/class-ld-translations.php */
							$url_args = apply_filters( 'ebox_translations_url_args', array() );
							...
							if ( file_exists( $dest_filename ) ) {
								...
								$reply_data['message'] = '<p>' . sprintf(
									// translators: placeholders: Language Name, Language code.
									esc_html_x( 'Translation removed: %1$s (%2$s)', 'placeholders: Language Name, Language code', 'ebox' ),
									$translation_set['english_name'],
									$translation_set['wp_locale']
								) . '</p>';
							}
			...
			if ( ! empty( $this->project_slug ) ) {
				...
				<div id="wrap-ld-translations-<?php echo esc_attr( $this->project_slug ); ?>" class="wrap wrap-ld-translations">
					...
					if ( ( ! empty( $this->available_translations ) ) || ( ! empty( $this->installed_translations ) ) ) {
						...
						?>
						<p><?php esc_html_e( 'No translations available for this plugin.', 'ebox' ); ?></p>
						<?php
					}
					?>
				</div>
		...
		public static function get_available_translations( $project = '', $force = false ) {
			if ( ! empty( self::$project_slugs ) ) {
				...
				if ( ! isset( $ld_translations['last_check'] ) ) {
					$ld_translations['last_check'] = time() - ( ebox_TRANSLATIONS_URL_CACHE + 1 );
				...
				if ( ( true === $force ) || ( $time_diff > ebox_TRANSLATIONS_URL_CACHE ) ) {
					...
					$url      = add_query_arg(
						...
						trailingslashit( ebox_TRANSLATIONS_URL_BASE )
					);
					$url_args = array( 'timeout' => 10 );
					/** This filter is documented in includes/class-ld-translations.php */
					$url_args = apply_filters( 'ebox_translations_url_args', $url_args );
		...
		public function show_installed_translations() {
			...
			if ( file_exists( $pot_file ) ) {
				?>
				<p style="float:right"><?php esc_html_e( 'Download the original strings (POT) File.', 'ebox' ); ?>

				<a id="ebox-translations-pot-file-<?php echo esc_attr( $this->project_slug ); ?>" class="button button-secondary ebox-translations-pot-file" href="<?php echo esc_url( self::get_action_url( 'pot_download', esc_attr( $this->project_slug ) ) ); ?>" title="<?php esc_html_e( 'Download POT File from your server.', 'ebox' ); ?>"><span class="dashicons dashicons-download"></span><?php esc_html_e( 'POT', 'ebox' ); ?></a>
			...
			<h4><?php esc_html_e( 'Installed Translations', 'ebox' ); ?></h4>
			<table class="ld-installed-translations wp-list-table widefat fixed striped posts">
			<thead>
				<tr>
					<th class="column-locale"><?php esc_html_e( 'Locale', 'ebox' ); ?></th>
					<th class="column-title"><?php esc_html_e( 'Name / Native', 'ebox' ); ?></th>
					<th class="column-actions-local"><?php esc_html_e( 'Download', 'ebox' ); ?></th>
					<th class="column-action-remote"><?php esc_html_e( 'Actions', 'ebox' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				if ( ( is_array( $this->available_translations ) ) && ( ! empty( $this->available_translations ) ) && ( is_array( $this->installed_translations ) ) && ( ! empty( $this->installed_translations ) ) ) {
					...
					?>
					<tr>
						<td colspan="4"><?php echo esc_html__( 'No Translations installed', 'ebox' ); ?></td>
						</tr>
						<?php
				}
				?>
			...
			echo sprintf(
				...
				esc_html_x( 'All translations are stored into the directory: %s', 'placeholder: Language directory', 'ebox' ),
			...
			if ( ! self::is_language_directory_writable( $this->project_slug ) ) {
				?>
					<br /><span class="error"><?php esc_html_e( 'The language directory is not writable', 'ebox' ); ?></span>
					<?php
			}
			?>
			</p>
				...
				<tr>
					...
					<td class="column-actions-local">
						...
						if ( isset( $installed_set['po'] ) ) {
							?>
							<a id="ebox-translations-po-file-<?php echo esc_attr( $locale ); ?>" class="button button-secondary ebox-translations-po-file" href="<?php echo esc_url( self::get_action_url( 'po_download', $this->project_slug, $locale ) ); ?>" title="<?php esc_html_e( 'Download PO File from your server.', 'ebox' ); ?>"><span class="dashicons dashicons-download"></span><?php esc_html_e( 'PO', 'ebox' ); ?></a>
							<?php
						}
						?>
					</td>
					<td class="column-actions-remote">
						<a id="ebox-translations-<?php echo esc_attr( $this->project_slug ); ?>-<?php echo esc_attr( $locale ); ?>-remove" class="button button-secondary ebox-translations-remove" href="<?php echo esc_url( self::get_action_url( 'remove', $this->project_slug, $locale ) ); ?>" title="<?php esc_html_e( 'Remove translation from server', 'ebox' ); ?>"><span class="dashicons dashicons-trash"></span></a>
						<?php

						if ( ! is_null( $translation_set ) ) {
							?>
							<a href="<?php echo esc_url( self::get_action_url( 'update', $this->project_slug, $locale ) ); ?>" class="button button-primary ebox-translations-update" title="<?php esc_html_e( 'Update translation from ebox', 'ebox' ); ?>"><?php esc_html_e( 'Update', 'ebox' ); ?></a>
							<?php
						}
						?>
					</td>
		...
		public function show_available_translations() {
			...
			if ( ( is_array( $this->available_translations ) ) && ( ! empty( $this->available_translations ) ) ) {
				...
				if ( ( ! empty( $available_translations['recommended'] ) ) || ( ! empty( $available_translations['available'] ) ) ) {
					?>
					<div id="ebox-translations-available">
						<h4><?php esc_html_e( 'Available Translations', 'ebox' ); ?></h4>
						<select id="ld-translation-install-locale-<?php echo esc_attr( $this->project_slug ); ?>" class="ld-translation-install-locale" data-project="<?php echo esc_attr( $this->project_slug ); ?>">
							<option value=""><?php esc_html_e( '-- Install Translation --', 'ebox' ); ?></option>
							<?php
							$show_opt_team = false;
							if ( ( ! empty( $available_translations['recommended'] ) ) && ( ! empty( $available_translations['available'] ) ) ) {
								$show_opt_team = true;
							...
							if ( ! empty( $available_translations['recommended'] ) ) {
								if ( $show_opt_team ) {
									?>
									<optteam label="<?php esc_html_e( 'Recommended', 'ebox' ); ?>">
									<?php
								}
							...
							if ( ! empty( $available_translations['available'] ) ) {
								if ( $show_opt_team ) {
									?>
									<optteam label="<?php esc_html_e( 'Available', 'ebox' ); ?>">
									<?php
								}
						...
						<a id="ebox-translation-install-<?php echo esc_attr( $this->project_slug ); ?>" class="button button-primary ebox-translations-install" href="#"><?php esc_html_e( 'Install', 'ebox' ); ?></a>
					</div>
					<?php
				}
			}
