 ...
 */
function ebox_delete_user_data( $user_id ) {
	global $wpdb;
	...
	if ( ! empty( $user_id ) ) {
		...
		$wpdb->delete(
			$wpdb->usermeta,
			array(
				'meta_key' => '_ebox-quizzes', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
				'user_id'  => $user->ID,
			)
		);
		...
		$wpdb->delete(
			$wpdb->usermeta,
			array(
				'meta_key' => '_ebox-course_progress', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
				'user_id'  => $user->ID,
			)
		);
		 ...
		 */
		do_action( 'ebox_delete_user_data', $user_id );
	}
}

add_action( 'delete_user', 'ebox_delete_user_data' );
 ...
 */
function ebox_user_get_enrolled_courses( $user_id = 0, $course_query_args = array(), $bypass_transient = false ) {
	...
	if ( false === $course_ids_transient ) {
		if ( ebox_can_user_autoenroll_courses( $user_id ) ) {
			$defaults = array(
			...
			$course_ids_open = ebox_get_posts_by_price_type( ebox_get_post_type_slug( 'course' ), 'open' );
			if ( ! empty( $course_ids_open ) ) {
				$course_ids = array_merge( $course_ids, $course_ids_open );
			}

			if ( true === ebox_use_legacy_course_access_list() ) {
				$course_ids_access = ebox_get_user_course_access_list( $user_id );
				if ( ! empty( $course_ids_access ) ) {
					$course_ids = array_merge( $course_ids, $course_ids_access );
				}
			}

			$course_ids_meta = ebox_get_user_courses_from_meta( $user_id );
			if ( ! empty( $course_ids_meta ) ) {
				$course_ids = array_merge( $course_ids, $course_ids_meta );
			}

			$course_ids_teams = ebox_get_user_teams_courses_ids( $user_id );
			if ( ! empty( $course_ids_teams ) ) {
				$course_ids = array_merge( $course_ids, $course_ids_teams );
			}
 ...
 */
function ebox_user_set_enrolled_courses( $user_id = 0, $user_courses_new = array() ) {

	if ( ! empty( $user_id ) ) {

		$user_courses_old = ebox_user_get_enrolled_courses( $user_id, true );
 ...
 */
function ebox_get_user_courses_from_meta( $user_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_show_user_course_complete( $user_id = 0 ) {
	...
	// See example snippet of this filter https://developers.ebox.com/hook/ebox_show_user_course_complete_options/.
	/**
	 ...
	 */
	return apply_filters( 'ebox_show_user_course_complete_options', $show_options, $user_id );
}
 ...
 * @param int $user_id Optional. User ID. Default 0.
 */
function ebox_save_user_course_complete( $user_id = 0 ) {
	...
	if ( ( isset( $_POST['ebox_delete_user_data'] ) ) && ( ! empty( $_POST['ebox_delete_user_data'] ) ) ) {
		return;
	}
	...
	if ( ( isset( $_POST['user_progress'] ) ) && ( isset( $_POST['user_progress'][ $user_id ] ) ) && ( ! empty( $_POST['user_progress'][ $user_id ] ) ) ) {
		if ( ( isset( $_POST[ 'user_progress-' . $user_id . '-nonce' ] ) ) && ( ! empty( $_POST[ 'user_progress-' . $user_id . '-nonce' ] ) ) ) {
			if ( wp_verify_nonce( $_POST[ 'user_progress-' . $user_id . '-nonce' ], 'user_progress-' . $user_id ) ) {
				...
				ebox_process_user_course_progress_update( $user_id, $user_progress );
			}
		}
	}
}
 ...
 * )
 */
function ebox_process_user_course_progress_update( $user_id = 0, $user_progress = array() ) {
	...
	if ( ( isset( $user_progress['course'] ) ) && ( ! empty( $user_progress['course'] ) ) ) {
		...
		foreach ( $user_progress['course'] as $course_id => $course_data_new ) {
			...
			$course_data_new = ebox_course_item_to_activity_sync( $user_id, $course_id, $course_data_new, $course_data_old );
		...
		if ( true === $course_changed ) {
			update_user_meta( $user_id, '_ebox-course_progress', $course_progress );
		}
	}
	...
	if ( ( isset( $user_progress['quiz'] ) ) && ( ! empty( $user_progress['quiz'] ) ) ) {
		...
		foreach ( $user_progress['quiz'] as $course_id => $course_quiz_set ) {
			foreach ( $course_quiz_set as  $quiz_id => $quiz_new_status ) {
				$quiz_meta = get_post_meta( $quiz_id, '_ebox-quiz', true );

				if ( ! empty( $quiz_meta ) ) {
					$quiz_old_status = ! ebox_is_quiz_notcomplete( $user_id, array( $quiz_id => 1 ), false, $course_id );
					...
					if ( (bool) true === (bool) $quiz_new_status ) {
						if ( (bool) true !== (bool) $quiz_old_status ) {
							...
							if ( isset( $quiz_meta['ebox-quiz_lesson'] ) ) {
								$lesson_id = absint( $quiz_meta['ebox-quiz_lesson'] );
							} else {
								$lesson_id = 0;
							}

							if ( isset( $quiz_meta['ebox-quiz_topic'] ) ) {
								$topic_id = absint( $quiz_meta['ebox-quiz_topic'] );
							} else {
								$topic_id = 0;
							}
							...
							$quizdata = array(
								...
								'rank'                => '-',
								'time'                => time(),
								'pro_quizid'          => absint( $quiz_meta['ebox-quiz_quiz_pro'] ),
								'course'              => $course_id,
								'lesson'              => $lesson_id,
								'topic'               => $topic_id,
								'points'              => 0,
							 ...
							 */
							do_action( 'ebox_quiz_completed', $quizdata, get_user_by( 'ID', $user_id ) );
							...
							if ( ! empty( $quiz_progress ) ) {
								foreach ( $quiz_progress as $quiz_idx => $quiz_item ) {
									...
									if ( ( $quiz_item['quiz'] == $quiz_id ) && ( true === $quiz_item['pass'] ) ) {
										...
										$quiz_activity       = ebox_reports_get_activity( $activity_query_args );
										if ( ( isset( $quiz_activity['results'] ) ) && ( ! empty( $quiz_activity['results'] ) ) ) {
											foreach ( $quiz_activity['results'] as $result ) {
												if ( ( isset( $result->activity_meta['pass'] ) ) && ( true === $result->activity_meta['pass'] ) ) {
													...
													ebox_update_user_activity_meta( $result->activity_id, 'pass', false );
									...
									if ( ( isset( $quiz_item['pro_quizid'] ) ) && ( ! empty( $quiz_item['pro_quizid'] ) ) ) {
										ebox_remove_user_quiz_locks( $user_id, $quiz_item['quiz'] );
									}
								}
							}
						}
		...
		if ( true === $quiz_changed ) {
			update_user_meta( $user_id, '_ebox-quizzes', $quiz_progress );
		}
	}

	if ( ! empty( $processed_course_ids ) ) {
		foreach ( array_unique( $processed_course_ids ) as $course_id ) {
			ebox_process_mark_complete( $user_id, $course_id );
			ebox_update_team_course_user_progress( $course_id, $user_id, true );
		}
	}
 ...
 * @return void|array
 */
function ebox_course_item_to_activity_sync( $user_id = 0, $course_id = 0, $course_data_new = array(), $course_data_old = array() ) {
	...
	// First we loop over the new Course data modules. We add any items not in the old array to the activity table.
	if ( ( isset( $course_data_new['modules'] ) ) && ( ! empty( $course_data_new['modules'] ) ) ) {
		foreach ( $course_data_new['modules'] as $lesson_id => $lesson_complete ) {
			$lesson_complete = (bool) $lesson_complete;
			if ( ( ! isset( $course_data_old['modules'][ $lesson_id ] ) ) || ( $lesson_complete !== (bool) $course_data_old['modules'][ $lesson_id ] ) ) {
				$lesson_args = array(
					'course_id'     => $course_id,
					'user_id'       => $user_id,
					'post_id'       => $lesson_id,
			...
			if ( ! empty( $lesson_topics ) ) {
				foreach ( $lesson_topics as $topic_id => $topic_complete ) {
					...
					if ( ( ! isset( $course_data_old['topics'][ $lesson_id ][ $topic_id ] ) ) || ( $topic_complete !== (bool) $course_data_old['topics'][ $lesson_id ][ $topic_id ] ) ) {
						...
						ebox_update_user_activity( $topic_args );
					}
				}
			}
		}
	...
	// Then we loop over the old course modules. Here if the lesson is NOT in the new course modules we need to change the 'activity_status' to false.
	if ( ( isset( $course_data_old['modules'] ) ) && ( ! empty( $course_data_old['modules'] ) ) ) {
		foreach ( $course_data_old['modules'] as $lesson_id => $lesson_complete ) {
			if ( ! isset( $course_data_new['modules'][ $lesson_id ] ) ) {
				ebox_update_user_activity(
					array(
						'course_id'          => $course_id,
						'user_id'            => $user_id,
	...
	$completed_steps = (int) ebox_course_get_completed_steps( $user_id, $course_id, $course_data_new );
	if ( ( ! isset( $course_data_new['completed'] ) ) || ( $completed_steps != $course_data_new['completed'] ) ) {
		...
		if ( empty( $completed_steps ) ) {
			...
			$course_activity = (array) ebox_get_user_activity( $course_args );
			if ( ! isset( $course_activity['activity_id'] ) ) {
				$course_args['activity_status']    = false;
				$course_args['activity_started']   = ebox_activity_course_get_earliest_started( $user_id, $course_id, time() );
				$course_args['activity_completed'] = 0;
			} else {
 ...
 */
function ebox_user_course_complete_all_steps( $user_id = 0, $course_id = 0 ) {
	...
	$course_access = ebox_lms_has_access( $course_id, $user_id );
	if ( true !== $course_access ) {
		return;
	}

	// If the course is already complete.
	if ( ebox_course_completed( $user_id, $course_id ) ) {
		return;
	}
	...
	if ( isset( $course_progress_steps_legacy['modules'] ) ) {
		foreach ( $course_progress_steps_legacy['modules'] as $lesson_id => &$lesson_completed ) {
			$lesson_completed = 1;
		}
	}
	...
	foreach ( $course_progress_steps_co as $key => $value ) {
		...
		if ( ebox_get_post_type_slug( 'quiz' ) === $step_type ) {
	...
	$changed_courses_count = ebox_process_user_course_progress_update( $user_id, $user_progress );
 ...
 */
function ebox_get_user_course_access_list( $user_id = 0 ) {
	global $wpdb;
	$user_course_ids = array();
	...
	if ( ! empty( $user_id ) ) {
		if ( true === ebox_use_legacy_course_access_list() ) {
			$data_settings_courses = ebox_data_upgrades_setting( 'course-access-lists' );
			if ( ( isset( $data_settings_courses['version'] ) ) && ( version_compare( $data_settings_courses['version'], ebox_SETTINGS_TRIGGER_UPGRADE_VERSION, '>=' ) ) ) {
				...
				$sql_str = 'SELECT post_id FROM ' . $wpdb->prefix . 'postmeta as postmeta INNER JOIN ' . $wpdb->prefix . "posts as posts ON posts.ID = postmeta.post_id WHERE posts.post_status='publish' AND posts.post_type='ebox-courses' AND postmeta.meta_key='course_access_list' AND (" . $is_like . ')';
			} else {
				// OR the access list is not empty.
				$not_like = " postmeta.meta_value NOT REGEXP '\"ebox-courses_course_access_list\";s:0:\"\";' ";
				...
				// 2. The user ID is at the front of the list as in "ebox-courses_course_access_list";*:"X,*";
				// 3. The user ID is in middle "ebox-courses_course_access_list";*:"*,X,*";
				// 4. The user ID is at the end "ebox-courses_course_access_list";*:"*,X";.
				$is_like = "
					postmeta.meta_value REGEXP 's:31:\"ebox-courses_course_access_list\";i:" . $user_id . ";s:34:\"ebox-courses_course_lesson_orderby\"'
					OR postmeta.meta_value REGEXP 's:31:\"ebox-courses_course_access_list\";i:" . $user_id . ";s:40:\"ebox-courses_course_prerequisite_compare\"'
					OR postmeta.meta_value REGEXP 's:31:\"ebox-courses_course_access_list\";i:" . $user_id . ";s:35:\"ebox-courses_course_lesson_per_page\"'

					OR postmeta.meta_value REGEXP 's:31:\"ebox-courses_course_access_list\";s:(.*):\"" . $user_id . "\";s:34:\"ebox-courses_course_lesson_orderby\"'
					OR postmeta.meta_value REGEXP 's:31:\"ebox-courses_course_access_list\";s:(.*):\"" . $user_id . "\";s:40:\"ebox-courses_course_prerequisite_compare\"'
					OR postmeta.meta_value REGEXP 's:31:\"ebox-courses_course_access_list\";s:(.*):\"" . $user_id . "\";s:35:\"ebox-courses_course_lesson_per_page\"'
				...
				$sql_str = 'SELECT post_id FROM ' . $wpdb->postmeta . ' as postmeta INNER JOIN ' . $wpdb->posts . " as posts ON posts.ID = postmeta.post_id WHERE posts.post_status='publish' AND posts.post_type='ebox-courses' AND postmeta.meta_key='_ebox-courses' AND ( " . $not_like . ' AND (' . $is_like . '))';
			}
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			$user_course_ids = $wpdb->get_col( $sql_str ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		} else {
			$user_course_ids = ebox_user_get_enrolled_courses( $user_id );
		}
	}
	return $user_course_ids;
}
...
/**
 * Gets all courses accessible by the user's teams.
 *
 * @since 2.3.0
 *
 * @param int $user_id Optional. User ID. Default 0.
 ...
 * @return array An array of course IDs.
 */
function ebox_get_user_teams_courses_ids( $user_id = 0 ) {
	$user_team_course_ids = array();

	if ( empty( $user_id ) ) {
		return $user_team_course_ids;
	}

	// Next we grab all the teams the user is a member of.
	$users_team_ids = ebox_get_users_team_ids( $user_id );

	if ( ! empty( $users_team_ids ) ) {
		foreach ( $users_team_ids as $team_id ) {
			$team_course_ids = ebox_team_enrolled_courses( $team_id );
			if ( ! empty( $team_course_ids ) ) {
				$user_team_course_ids = array_merge( $user_team_course_ids, $team_course_ids );
			}
		}
	}

	/**
	 * Filters the list of user team courses.
	 *
	 * @param array $user_team_course_ids An array of found user team courses.
	 * @param int   $user_id               User ID.
	 */
	return apply_filters( 'ebox_get_user_teams_courses_ids', $user_team_course_ids, $user_id );
}
 ...
 */
function ebox_wp_login( $user_login = '', $user = '' ) {
	if ( ! empty( $user_login ) ) {
		...
		if ( $user instanceof WP_User ) {
			update_user_meta( $user->ID, 'ebox-last-login', time() );
		}
	}
}
add_action( 'wp_login', 'ebox_wp_login', 99, 1 );
 ...
 */
function ebox_remove_user_quiz_locks( $user_id = 0, $quiz_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_get_user_course_points( $user_id = 0 ) {
	global $wpdb;
	...
	if ( ! empty( $user_id ) ) {
		...
		if ( ! empty( $course_points_results ) ) {
			foreach ( $course_points_results as $course_points ) {
				$course_points_sum += ebox_format_course_points( $course_points->points );
			}
		}

		$user_course_points = get_user_meta( $user_id, 'course_points', true );
		$user_course_points = ebox_format_course_points( $user_course_points );

		return ebox_format_course_points( (string) ( $course_points_sum + $user_course_points ) );
	}
}
 ...
 */
function ebox_get_quiz_statistics_ref_for_quiz_attempt( $user_id = 0, $quiz_attempt = array() ) {
	global $wpdb;
 ...
 */
function ebox_get_usermeta_shortcode_available_fields( $attr = array() ) {
	...
	return apply_filters(
		'ebox_usermeta_shortcode_available_fields',
		array(
			'user_login'      => esc_html__( 'User Login', 'ebox' ),
			'first_name'      => esc_html__( 'User First Name', 'ebox' ),
			'last_name'       => esc_html__( 'User Last Name', 'ebox' ),
			'first_last_name' => esc_html__( 'User First and Last Name', 'ebox' ),
			'display_name'    => esc_html__( 'User Display Name', 'ebox' ),
			'user_nicename'   => esc_html__( 'User Nicename', 'ebox' ),
			'nickname'        => esc_html__( 'User Nickname', 'ebox' ),
			'user_email'      => esc_html__( 'User Email', 'ebox' ),
			'user_url'        => esc_html__( 'User URL', 'ebox' ),
			'description'     => esc_html__( 'User Description', 'ebox' ),
		),
		$attr
	);
}
 ...
 */
function ebox_get_admin_courses_capabilities(): array {
	return array(
 ...
 * @return void
 */
function ebox_init_admin_courses_capabilities(): void {
	...
	foreach ( ebox_get_admin_courses_capabilities() as $capability ) {
...
/**
 * Utility function to return the admin user Teams capabilities.
 *
 * @since 3.2.3.5
 *
 * @return array of role capabilities.
 */
function ebox_get_admin_teams_capabilities(): array {
	return array(
		'read_post'              => 'read_team',
...
/**
 * Initialize the admin user Teams capabilities.
 *
 * @since 3.2.3.5
 *
 * @return void
 */
function ebox_init_admin_teams_capabilities(): void {
	$admin_role = get_role( 'administrator' );
	...
	foreach ( ebox_get_admin_teams_capabilities() as $capability ) {
		if ( ! $admin_role->has_cap( $capability ) ) {
			$admin_role->add_cap( $capability );
		}
	}
 ...
 * @return array of role capabilities.
 */
function ebox_get_admin_coupons_capabilities(): array {
	return array(
 ...
 * @return void
 */
function ebox_init_admin_coupons_capabilities(): void {
	$admin_role = get_role( 'administrator' );
	...
	foreach ( ebox_get_admin_coupons_capabilities() as $capability ) {
		if ( ! $admin_role->has_cap( $capability ) ) {
			$admin_role->add_cap( $capability );
		}
	}
...
add_action( 'admin_init', 'ebox_init_admin_coupons_capabilities' );

/**
 * Gets all expired courses for the user via the user meta 'ebox_course_expired_XXX'.
 *
 * @since 3.4.2
 *
 * @global wpdb $wpdb WordPress database abstraction object.
 ...
 */
function ebox_get_expired_user_courses_from_meta( $user_id = 0 ) {
	global $wpdb;
	...
	if ( ! empty( $user_id ) ) {
		...
		$expired_user_course_ids = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT REPLACE(meta_key, 'ebox_course_expired_', '') FROM " . $wpdb->usermeta . ' as usermeta WHERE user_id=%d AND meta_key LIKE %s ',
				$user_id,
				'ebox_course_expired_%'
			)
		);
 ...
 * @since 3.6.0
 */
function ebox_users_can_register() {
	if ( is_multisite() ) {
	 ...
	 */
	return (bool) apply_filters( 'ebox_users_can_register', $users_can_register );
}
