 ...
 * @since 2.2.0
 */
function ebox_register_essay_post_type() {

	$labels = array(
		'name'                     => esc_html_x( 'Submitted Essays', 'Post Type General Name', 'ebox' ),
		'singular_name'            => esc_html_x( 'Submitted Essay', 'Post Type Singular Name', 'ebox' ),
		'menu_name'                => esc_html__( 'Submitted Essays', 'ebox' ),
		'name_admin_bar'           => esc_html__( 'Submitted Essays', 'ebox' ),
		'parent_item_colon'        => esc_html__( 'Parent Submitted Essay:', 'ebox' ),
		'all_items'                => esc_html__( 'All Submitted Essays', 'ebox' ),
		'add_new_item'             => esc_html__( 'Add New Submitted Essay', 'ebox' ),
		'add_new'                  => esc_html__( 'Add New', 'ebox' ),
		'new_item'                 => esc_html__( 'New Submitted Essay', 'ebox' ),
		'edit_item'                => esc_html__( 'Edit Submitted Essay', 'ebox' ),
		'update_item'              => esc_html__( 'Update Submitted Essay', 'ebox' ),
		'view_item'                => esc_html__( 'View Submitted Essay', 'ebox' ),
		'view_items'               => esc_html__( 'View Submitted Essays', 'ebox' ),
		'search_items'             => esc_html__( 'Search Submitted Essays', 'ebox' ),
		'not_found'                => esc_html__( 'Submitted Essay Not found', 'ebox' ),
		'not_found_in_trash'       => esc_html__( 'Submitted Essay Not found in Trash', 'ebox' ),
		'item_published'           => esc_html__( 'Submitted Essay Published', 'ebox' ),
		'item_published_privately' => esc_html__( 'Submitted Essay Published Privately', 'ebox' ),
		'item_reverted_to_draft'   => esc_html__( 'Submitted Essay Reverted to Draft', 'ebox' ),
		'item_scheduled'           => esc_html__( 'Submitted Essay Scheduled', 'ebox' ),
		'item_updated'             => esc_html__( 'Submitted Essay Updated', 'ebox' ),
	...
	$show_in_rest = ebox_REST_API::enabled( ebox_get_post_type_slug( 'essay' ) ) || ebox_REST_API::gutenberg_enabled( ebox_get_post_type_slug( 'essay' ) );

	$args = array(
		'label'               => esc_html__( 'ebox-essays', 'ebox' ),
		// translators: quiz, question.
		'description'         => sprintf( esc_html_x( 'Submitted essays via a %1$s %2$s.', 'placeholder: quiz, question', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ), ebox_get_custom_label_lower( 'question' ) ),
		'labels'              => $labels,
	...
	$args = apply_filters( 'ebox-cpt-options', $args, 'ebox-essays' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores -- Better to keep it this way for now.

	register_post_type( 'ebox-essays', $args );
}

add_action( 'init', 'ebox_register_essay_post_type' );
 ...
 *
 * Add essay capabilities to administrators and team leaders.
 * Fires on `admin_init` hook.
 *
 * @since 2.2.0
 */
function ebox_add_essay_caps() {
	...
	if ( ( $team_leader_role ) && ( $team_leader_role instanceof WP_Role ) ) {
		$team_leader_role->add_cap( 'edit_essays' );
		$team_leader_role->add_cap( 'edit_others_essays' );
		$team_leader_role->add_cap( 'publish_essays' );
		$team_leader_role->add_cap( 'read_essays' );
		$team_leader_role->add_cap( 'read_private_essays' );
		$team_leader_role->add_cap( 'delete_essays' );
		$team_leader_role->add_cap( 'edit_published_essays' );
		$team_leader_role->add_cap( 'delete_others_essays' );
		$team_leader_role->add_cap( 'delete_published_essays' );
	}
}

add_action( 'admin_init', 'ebox_add_essay_caps' );
 ...
 */
function ebox_map_metacap_essays( $caps, $cap, $user_id, $args = array() ) {
	if ( ! is_string( $cap ) ) {
		return $caps;
	}
...
add_filter( 'map_meta_cap', 'ebox_map_metacap_essays', 10, 4 );
 ...
 * @since 2.2.0
 */
function ebox_register_essay_post_status() {
	register_post_status(
		'graded',
		array(
			'label'                     => esc_html_x( 'Graded', 'Custom Essay post type status: Graded', 'ebox' ),
			'public'                    => true,
			'exclude_from_search'       => true,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			// translators: placeholder: Graded Essay count.
			'label_count'               => _n_noop( 'Graded <span class="count">(%s)</span>', 'Graded <span class="count">(%s)</span>', 'ebox' ),
		)
	);
	...
	register_post_status(
		'not_graded',
		array(
			'label'                     => esc_html_x( 'Not Graded', 'Custom Essay post type status: Not Graded', 'ebox' ),
			'public'                    => true,
			'exclude_from_search'       => true,
			'show_in_admin_all_list'    => true,
			'show_in_admin_status_list' => true,
			// translators: placeholder: Not Graded Essay count.
			'label_count'               => _n_noop( 'Not Graded <span class="count">(%s)</span>', 'Not Graded <span class="count">(%s)</span>', 'ebox' ),
		)
	);
}

add_action( 'init', 'ebox_register_essay_post_status' );


/**
 * Manages the permissions for the essay post type.
 *
 * Only allow admins, team leaders, and essay owners to see the assignment.
 * Fires on `wp` hook.
 *
 ...
 * @since 2.2.1
 */
function ebox_essay_permissions() {

	if ( is_singular( ebox_get_post_type_slug( 'essay' ) ) ) {
		$can_view_file = false;

		$post = get_post();
		if ( ( $post ) && ( is_a( $post, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'essay' ) === $post->post_type ) ) {
			$user_id = get_current_user_id();

			if ( ! empty( $user_id ) ) {
				if ( ( ebox_is_admin_user( $user_id ) ) || ( $post->post_author == $user_id ) ) {
					$can_view_file = true;
		...
		if ( true === $can_view_file ) {
			...
			if ( ( ! empty( $uploaded_file ) ) && ( ! strstr( $post->post_content, $uploaded_file ) ) ) {
				$quiz_essay_upload_link = '<p><a target="_blank" href="' . esc_url( $uploaded_file ) . '">' . esc_html__( 'View uploaded file', 'ebox' ) . '</a></p>';
				...
				$quiz_essay_upload_link = apply_filters_deprecated(
					'ebox-quiz-essay-upload-link',
					array( $quiz_essay_upload_link ),
					'4.5.0',
					'ebox_quiz_essay_upload_link'
				);
				 ...
				 * @param string $upload_link Essay upload link HTML output.
				 */
				$quiz_essay_upload_link = apply_filters( 'ebox_quiz_essay_upload_link', $quiz_essay_upload_link );
			 ...
			 */
			$redirect_to_url = apply_filters( 'ebox_essay_permissions_redirect_url', $redirect_to_url );
		 ...
		 * with http://www.site.com?post_type=ebox-essays to access an archive.
		 *
		 * It would be nice if this is controllable via WP register_post_type() settings.
		 *
		 *  See ebox-6389 for more details.
		 */
		if ( get_query_var( 'post_type', '' ) === ebox_get_post_type_slug( 'essay' ) ) {
...
add_action( 'wp', 'ebox_essay_permissions' );
 ...
 */
function ebox_add_new_essay_response( $response, $this_question, $quiz, $post_data = null ) {
	...
	switch ( $essay_data->getGradedType() ) {
		case 'text':
			$essay_args['post_content'] = wp_kses(
				 ...
				 */
				apply_filters( 'ebox_essay_new_allowed_html', wp_kses_allowed_html( 'post' ) )
			);
			break;
		case 'upload':
			$essay_args['post_content'] = esc_html__( 'See upload below.', 'ebox' );
	}
	 ...
	 */
	$essay_args = apply_filters( 'ebox_new_essay_submission_args', $essay_args );
	...
	if ( ! empty( $essay_id ) ) {
		if ( ( isset( $post_data['quiz_id'] ) ) && ( ! empty( $post_data['quiz_id'] ) ) ) {
			...
			$quiz_id = ebox_get_quiz_id_by_pro_quiz_id( $this_question->getQuizId() );
		}

		if ( isset( $post_data['course_id'] ) ) {
			$course_id = intval( $post_data['course_id'] );
			if ( ! empty( $course_id ) ) {
				$lesson_id = ebox_course_get_single_parent_step( $course_id, $quiz_id );
			} else {
				$lesson_id = 0;
			}
		} else {
			$course_id = ebox_get_course_id( $quiz_id );
			$lesson_id = ebox_get_lesson_id( $quiz_id );
		}
	 ...
	 */
	do_action( 'ebox_new_essay_submitted', $essay_id, $essay_args );
 ...
 */
function ebox_get_submitted_essay_data( $quiz_id, $question_id, $essay ) {
	$users_quiz_data = get_user_meta( $essay->post_author, '_ebox-quizzes', true );
 ...
 */
function ebox_update_submitted_essay_data( $quiz_id, $question_id, $essay, $submitted_essay ) {
	$users_quiz_data = get_user_meta( $essay->post_author, '_ebox-quizzes', true );
	...
	update_user_meta( $essay->post_author, '_ebox-quizzes', $users_quiz_data );
	 ...
	 */
	do_action( 'ebox_essay_response_data_updated', $quiz_id, $question_id, $essay, $submitted_essay );
}
 ...
 * @param WP_Post $essay           The `WP_Post` essay object.
 */
function ebox_update_quiz_data( $quiz_id, $question_id, $updated_scoring, $essay ) {
	...
	foreach ( $users_quiz_data as $quiz_key => $quiz_data ) {
		...
		$quizmeta                             = get_post_meta( $quiz_data['quiz'], '_ebox-quiz', true );
		$passingpercentage                    = intVal( $quizmeta['ebox-quiz_passingpercentage'] );
		$users_quiz_data[ $quiz_key ]['pass'] = ( $users_quiz_data[ $quiz_key ]['percentage'] >= $passingpercentage ) ? 1 : 0;

		ebox_update_quiz_statistics( $quiz_id, $question_id, $updated_scoring, $essay, $users_quiz_data[ $quiz_key ] );
		ebox_update_quiz_activity( $essay->post_author, $users_quiz_data[ $quiz_key ] );
	}
			...
			if ( isset( $users_quiz_data[ $quiz_key ] ) ) {
				...
				if ( true === $send_quiz_completed ) {
					if ( isset( $users_quiz_data[ $quiz_key ]['course'] ) ) {
						...
						$course_id = ebox_get_course_id( $essay->ID );
					}

					ebox_process_mark_complete( $essay->post_author, $users_quiz_data[ $quiz_key ]['quiz'], false, $course_id );

					/** This action is documented in includes/ld-users.php */
					do_action( 'ebox_quiz_completed', $users_quiz_data[ $quiz_key ], get_user_by( 'ID', $essay->post_author ) );
				}
			}
		}
	}
	 ...
	 * @param WP_Post $essay           WP_Post object for essay.
	 */
	do_action( 'ebox_essay_quiz_data_updated', $quiz_id, $question_id, $updated_scoring, $essay );
}
 ...
 */
function ebox_update_quiz_activity( $user_id = 0, $quiz_data = array() ) {
 ...
 */
function ebox_update_quiz_statistics( $quiz_id, $question_id, $updated_quiz_data, $essay, $users_quiz_data ) {
	global $wpdb;
...
function ebox_upload_essay() {
	 ...
	 */
	if ( ! wp_verify_nonce( $nonce, 'ebox-upload-essay-' . $question_id ) ) {
		wp_send_json_error();
		die( 'Security check' );
	} else {
		...
		if ( ! is_user_logged_in() ) {
			 ...
			 */
			if ( ! apply_filters( 'ebox_essay_upload_user_check', false, $question_id ) ) {
				wp_send_json_error();
				die();
			}
		}

		$file_desc = ebox_essay_fileupload_process( $_FILES['essayUpload'], $question_id );
		...
		if ( ! empty( $file_desc ) ) {
			...
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Unknown error.', 'ebox' ),
				)
			);
		}
		die();
...
add_action( 'wp_ajax_ebox_upload_essay', 'ebox_upload_essay' );
add_action( 'wp_ajax_nopriv_ebox_upload_essay', 'ebox_upload_essay' );
 ...
 */
function ebox_essay_fileupload_process( $uploadfiles, $question_id ) {
	if ( is_array( $uploadfiles ) ) {
		...
		if ( 0 == $uploadfiles['error'] ) {
			...
			// clean filename.
			$filename = ebox_clean_filename( $uploadfiles['name'] );
			...
			$limit_file_exts = ebox_get_allowed_upload_mime_extensions_for_post( $question_id );
			...
			if ( ( empty( $filetype ) ) || ( empty( $filetype['ext'] ) ) || ( empty( $filetype['type'] ) ) || ( ! $limit_file_exts[ strtolower( $filetype['ext'] ) ] ) ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Invalid essay uploaded file type.', 'ebox' ),
					)
				);
				die();
			}
			...
			$filename        = apply_filters( 'ebox_essay_upload_filename', $filename, $question_id, $file_title, $filetype['ext'] );
			$upload_dir      = wp_upload_dir();
			$upload_dir_base = str_replace( '\\', '/', $upload_dir['basedir'] );
			$upload_url_base = $upload_dir['baseurl'];
			/** This filter is documented in includes/import/class-ld-import-quiz-statistics.php */
			$upload_dir_path = $upload_dir_base . apply_filters( 'ebox_essay_upload_dirbase', '/essays', $filename, $upload_dir );
			/** This filter is documented in includes/import/class-ld-import-quiz-statistics.php */
			$upload_url_path = $upload_url_base . apply_filters( 'ebox_essay_upload_urlbase', '/essays/', $filename, $upload_dir );
			...
			if ( ! file_exists( $upload_dir_path ) ) {
				if ( is_writable( dirname( $upload_dir_path ) ) ) {
					...
					wp_send_json_error(
						array(
							'message' => esc_html__( 'Unable to write to UPLOADS directory. Is this directory writable by the server?', 'ebox' ),
						)
					);
					die();
				}
			...
			if ( ! is_writeable( $upload_dir_path ) ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Unable to write to directory. Is this directory writable by the server?', 'ebox' ),
					)
				);
				die();
			}
			...
			if ( ! @move_uploaded_file( $file_tmp, $file_dest ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged -- Better not to touch it for now.
				wp_send_json_error(
					array(
						'message' => esc_html__( 'The uploaded file could not be move to the destination directory.', 'ebox' ),
					)
				);
				die();
			}
			...
			$file_desc['message']  = esc_html__( 'Essay upload success.', 'ebox' );
 ...
 */
function ebox_before_delete_essay( $post_id ) {

	if ( ( ! empty( $post_id ) ) && ( 'ebox-essays' == get_post_type( $post_id ) ) ) {
...
add_action( 'before_delete_post', 'ebox_before_delete_essay' );


/**
 * Updates the essays post meta with a reference to the quiz attempt user meta.
 *
 * Fires on `ebox_quiz_submitted` hook.
 *
 * @since 3.1.0
 *
 ...
 */
function ebox_quiz_submitted_update_essay( $quizdata, $user ) {
...
add_action( 'ebox_quiz_submitted', 'ebox_quiz_submitted_update_essay', 1, 2 );
 ...
 * @param int $user_id       User ID.
 */
function ebox_get_user_quiz_entry_for_essay( $essay_post_id = 0, $user_id = 0 ) {
	$essay_post_id = absint( $essay_post_id );
	if ( ! empty( $essay_post_id ) ) {
		$essay_post = get_post( $essay_post_id );
		if ( ( $essay_post ) && ( is_a( $essay_post, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'essay' ) === $essay_post->post_type ) ) {
			if ( empty( $user_id ) ) {
