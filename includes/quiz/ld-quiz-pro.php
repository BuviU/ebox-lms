<?php
/**
 * Extends WP Pro Quiz functionality to meet needs of ebox
 *
 * @since 2.1.0
 *
 * @package ebox\Quiz
 ...
 */
require_once ebox_LMS_LIBRARY_DIR . '/wp-pro-quiz/wp-pro-quiz.php';
...
class LD_QuizPro {
	...
	public function __construct() {
		...
		add_filter( 'ebox_quiz_content', array( $this, 'ebox_quiz_content' ), 1 );
		...
		add_action( 'ebox_quiz_submitted', array( $this, 'set_quiz_status_meta' ), 1, 2 );
	}
	...
	public function checkAnswers( $data ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.MethodNameInvalid -- Better to keep it this way.
		...
		ebox_quiz_debug_log_init( $quiz_post_id );
		ebox_quiz_debug_log_message( 'Browser version: ' . $_SERVER['HTTP_USER_AGENT'] );
		ebox_quiz_debug_log_message( '---------------------------------' );
		ebox_quiz_debug_log_message( 'in ' . __FUNCTION__ );
		ebox_quiz_debug_log_message( '_POST<pre>' . print_r( $_POST, true ) . '</pre>' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r -- It's okay, the second argument is true.

		ebox_quiz_debug_log_message( 'user_id ' . $user_id );
		ebox_quiz_debug_log_message( 'quiz id ' . $id );
		ebox_quiz_debug_log_message( 'quiz_post_id ' . $quiz_post_id );

		if ( defined( 'ebox_QUIZ_DEBUG' ) && ebox_QUIZ_DEBUG ) {
			/**
			 ...
			 */
			$data = apply_filters( 'ebox_quiz_check_answers_data', $data, $user_id, $quiz_post_id );

			ebox_quiz_debug_log_message( 'after filter: ebox_quiz_check_answers_data: data: ' . print_r( $data, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r -- It's okay, the second argument is true.
		}
		...
		foreach ( $data['responses'] as $question_id => $info ) {
			...
			foreach ( $questionModels as $questionModel ) {
				if ( $questionModel->getId() == intval( $question_id ) ) {
					...
					*/
					$question_legacy_sanitize_scheme = apply_filters( 'ebox_quiz_question_legacy_sanitize_scheme', false, $userResponse, $questionData );
					...
					switch ( $questionData['type'] ) {
						case 'free_answer':
							...
							if ( ( ! empty( $questionData['correct'] ) ) && ( '' !== $userResponse_filtered ) ) {
								 ...
								 */
								$format_correct = apply_filters( 'ebox_quiz_format_correct_answer', $format_correct, $questionData, $questionModel );
								...
								foreach ( $questionData['correct'] as $questionData_idx => $questionData_correct ) {
									...
									*/
									if ( apply_filters( 'ebox_quiz_question_free_answers_to_lowercase', true, $questionModel ) ) {
							 ...
							 */
							$points = apply_filters( 'ebox_ques_free_answer_pts', $points, $questionData, $userResponse );
							 ...
							 * @param string  $user_response User response data.
							 */
							$correct = apply_filters( 'ebox_ques_free_answer_correct', $correct, $questionData, $userResponse );
							...
							if ( ! empty( $questionData['correct'] ) ) {
								foreach ( $questionData['correct'] as $answerIndex => $correctAnswer ) {
									if ( $answer_pointed_activated ) {
										...
										if ( has_filter( 'ebox_ques_multiple_answer_pts_each' ) ) {
											/**
											 ...
											 */
											$points = apply_filters( 'ebox_ques_multiple_answer_pts_each', $points, $questionData, $answerIndex, $correctAnswer, $userResponse );
										} else {
											/**
										 ...
										 * @param array      $user_response  An array of user response data.
										 */
										$correct_this_item = apply_filters( 'ebox_ques_multiple_answer_correct_each', $correct_this_item, $questionData, $answerIndex, $correctAnswer, $userResponse );
										...
										// See https://developers.ebox.com/hook/ebox_ques_multiple_answer_pts_whole/ for examples of this filter.
										/**
										 * Filters points awarded for a multiple answer type question.
										 *
										 * ebox multiple answer question, allow points to be allocated for not marking an incorrect answer.
										 * ebox Core loops over all the answers in a multiple answer question. If the user does not mark an incorrect answer,
										 ...
										 * @param array      $user_response  An array of user response data.
										 */
										$points = apply_filters( 'ebox_ques_multiple_answer_pts_whole', $points, $questionData, $answerIndex, $correctAnswer, $userResponse );
										 ...
										 * @param array      $user_response  An array of user response data.
										 */
										$correct = apply_filters( 'ebox_ques_multiple_answer_correct_whole', $correct, $questionData, $answerIndex, $correctAnswer, $userResponse );
							...
							if ( ! empty( $questionData['correct'] ) ) {
								foreach ( $questionData['correct'] as $answerIndex => $correctAnswer ) {
									if ( $userResponse[ $answerIndex ] === true ) {
										...
										// See https://developers.ebox.com/hook/ebox_ques_single_answer_pts/ for examples of this filter.
										/**
										 * Filters points awarded for a single answer type question.
										 *
										 * ebox single answer question, allow points to be allocated for not marking an incorrect answer.
										 ...
										 * @param array      $user_response  An array of user response data.
										 */
										$points = apply_filters( 'ebox_ques_single_answer_pts', $points, $questionData, $answerIndex, $correctAnswer, $userResponse );
										/**
										 ...
										 * @param array      $user_response  An array of user response data.
										 */
										$correct = apply_filters( 'ebox_ques_single_answer_correct', $correct, $questionData, $answerIndex, $correctAnswer, $userResponse );
									}
								}
							}
						...
						case 'cloze_answer':
							...
							if ( ! empty( $questionData['correct'] ) ) {
								foreach ( $questionData['correct'] as $answerIndex => $correctArray ) {
									...
									if ( apply_filters( 'ebox_quiz_question_cloze_answers_to_lowercase', true ) ) {
										if ( function_exists( 'mb_strtolower' ) ) {
						...
						case 'essay':
							if ( ! empty( $userResponse ) ) {
								...
								$essay_id           = ebox_add_new_essay_response( $userResponse, $questionModel, $quiz, $data );
						...
						foreach ( $questionModels as $key => $value ) {
							if ( $value->getId() == $question_id ) {
								if ( $correct || $value->isCorrectSameText() ) {
									$extra['AnswerMessage'] = do_shortcode( ebox_the_content( $value->getCorrectMsg(), __FUNCTION__ ) );
								} else {
									$extra['AnswerMessage'] = do_shortcode( ebox_the_content( $value->getIncorrectMsg(), __FUNCTION__ ) );
								}
		...
		ebox_quiz_debug_log_message( __FUNCTION__ . ': results: ' . print_r( $results, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r -- It's okay, the second argument is true.
	...
	public function quiz_edit_redirect() {
		...
		if ( ! empty( $_GET['page'] ) && $_GET['page'] == 'ldAdvQuiz' && empty( $_GET['module'] ) && ! empty( $_GET['action'] ) && $_GET['action'] == 'addEdit' ) {
			...
			if ( ! empty( $_GET['post_id'] ) ) {
				...
				$post_id = ebox_get_quiz_id_by_pro_quiz_id( absint( $_GET['quizId'] ) );

				if ( ! empty( $post_id ) ) {
					header( 'Location: ' . admin_url( 'post.php?action=edit&post=' . absint( $post_id ) ) );
				} else {
					header( 'Location: ' . admin_url( 'edit.php?post_type=ebox-quiz' ) );
				}

				exit;
			}

			header( 'Location: ' . admin_url( 'post-new.php?post_type=ebox-quiz' ) );
			exit;
		}
	}
	...
	public static function showQuizContent( $pro_quiz_id ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.MethodNameInvalid -- Better to keep it this way.
		...
		if ( empty( $post ) || $post->post_type == 'ebox-quiz' ) {
			return '';
		}
	...
	public static function get_description( $pro_quiz_id ) {
		$post_id = ebox_get_quiz_id_by_pro_quiz_id( $pro_quiz_id );
	...
	public function set_quiz_status_meta( $quizdata, $user ) {
		...
		if ( ( isset( $quizdata['course'] ) ) && ( ! empty( $quizdata['course'] ) ) ) {
			...
			$course_start_time = ebox_activity_course_get_earliest_started( $user->ID, $quizdata['course'], $quizdata['started'] );
			$course_activity   = ebox_activity_start_course( $user->ID, $quizdata['course'], $course_start_time );
			if ( $course_activity ) {
				ebox_activity_update_meta_set(
					$course_activity->activity_id,
					array(
						'steps_completed' => ebox_course_get_completed_steps( $user->ID, $quizdata['course'] ),
			...
			if ( ( isset( $quizdata['lesson'] ) ) && ( ! empty( $quizdata['lesson'] ) ) ) {
				ebox_activity_start_lesson( $user->ID, $quizdata['course'], $quizdata['lesson'], $quizdata['started'] );
			}
			if ( ( isset( $quizdata['topic'] ) ) && ( ! empty( $quizdata['topic'] ) ) ) {
				ebox_activity_start_topic( $user->ID, $quizdata['course'], $quizdata['topic'], $quizdata['started'] );
			}
		}
	...
	public function wp_pro_quiz_completed( $statistic_ref_id = 0 ) {
		ebox_quiz_debug_log_message( 'in ' . __FUNCTION__ );
		ebox_quiz_debug_log_message( 'statistic_ref_id ' . $statistic_ref_id );
		...
		$course_id = ( ( isset( $_POST['course_id'] ) ) && ( intval( $_POST['course_id'] ) > 0 ) ) ? intval( $_POST['course_id'] ) : ebox_get_course_id( $quiz_post_id );
		...
		if ( isset( $_POST['quiz_nonce'] ) && isset( $_POST['quizId'] ) ) {
			if ( ! wp_verify_nonce( $_POST['quiz_nonce'], 'ebox-quiz-nonce-' . absint( $_POST['quiz'] ) . '-' . absint( $_POST['quizId'] ) . '-' . $user_id ) ) {
				return;
			}
		...
		if ( ! empty( $user_id ) ) {
			$user_quiz_meta = get_user_meta( $user_id, '_ebox-quizzes', true );
		...
		$quiz_post_settings = ebox_get_setting( $quiz_post_id );
		...
		$quizdata['ld_version'] = ebox_VERSION;
		...
		if ( ! empty( $user_id ) ) {
			...
			ebox_quiz_debug_log_message( 'calling update_user_meta()' );
			ebox_quiz_debug_log_message( 'quizdata<pre>' . print_r( $quizdata, true ) . '</pre>' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r -- It's okay, the second argument is true.

			update_user_meta( $user_id, '_ebox-quizzes', $user_quiz_meta );
		}
		...
		if ( ! empty( $user_id ) ) {
			 ...
			 */
			do_action( 'ebox_quiz_submitted', $quizdata, get_user_by( 'id', $user_id ) );

			/**
			 * Changed in 2.6.0. If the quiz has essay type questions that are not
			 * auto-graded we don't send out the 'ebox_quiz_completed' action.
			 */
			...
			if ( true === $send_quiz_completed ) {
				if ( ! empty( $course_id ) ) {
					...
					if ( ! empty( $quiz_parent_post_id ) ) {
						 ...
						 */
						if ( apply_filters( 'ebox_complete_all_parent_steps', true, $quiz_post_id, $user_id, $course_id ) ) {
							if ( ! empty( $topic_id ) ) {
								if ( ebox_can_complete_step( $user_id, $topic_id, $course_id ) ) {
									ebox_process_mark_complete( $user_id, $topic_id, false, $course_id );
								}
							}
							if ( ! empty( $lesson_id ) ) {
								if ( ebox_can_complete_step( $user_id, $lesson_id, $course_id ) ) {
									ebox_process_mark_complete( $user_id, $lesson_id, false, $course_id );
								}
							}
						} else {
							if ( ebox_can_complete_step( $user_id, $quiz_parent_post_id, $course_id ) ) {
								ebox_process_mark_complete( $user_id, $quiz_parent_post_id, false, $course_id );
							}
						}
					} else {
						...
						if ( ! empty( $quizzes ) ) {
							foreach ( $quizzes as $quiz ) {
								if ( ebox_is_quiz_notcomplete( $user_id, array( $quiz->ID => 1 ), false, $course_id ) ) {
						...
						if ( true === $all_quizzes_complete ) {
							ebox_process_mark_complete( $user_id, $course_id, false, $course_id );
						}
					}
				}
			...
			} elseif ( defined( 'ebox_QUIZ_ESSAY_SUBMIT_COMPLETED' ) && ebox_QUIZ_ESSAY_SUBMIT_COMPLETED === true ) {
				/** This action is documented in includes/ld-users.php */
				do_action( 'ebox_quiz_completed', $quizdata, get_user_by( 'id', $user_id ) );
			}
		}
		 ...
		 */
		$results[ $quiz_pro_id ]['quiz_result_settings'] = apply_filters( 'ebox_quiz_completed_result_settings', $results[ $quiz_pro_id ]['quiz_result_settings'], $quizdata );
	...
	public static function certificate_details( $pro_quiz_id = null ) {
		...
		if ( is_null( $pro_quiz_id ) ) {
			global $post;
			if ( ( $post instanceof WP_Post ) && ( $post->post_type == 'ebox-quiz' ) ) {
				$pro_quiz_id = $post->ID;
			}
		} else {
			if ( is_a( $pro_quiz_id, 'WpProQuiz_Model_Quiz' ) ) {
				...
				$quiz_post_id = ebox_get_quiz_id_by_pro_quiz_id( $pro_quiz_id );
			}

			if ( ! empty( $quiz_post_id ) ) {
				$quiz_post = get_post( $quiz_post_id );
				if ( ( $quiz_post instanceof WP_Post ) && ( $quiz_post->post_type == 'ebox-quiz' ) ) {
		...
		if ( ! empty( $quiz_post_id ) ) {
			echo '<script>';
			echo 'var certificate_details = ' . wp_json_encode( ebox_certificate_details( $quiz_post_id ) ) . ';';
			echo '</script>';
				...
				ebox_LMS::get_template(
					...
					array(
						...
						'message'      => sprintf(
							...
							esc_html_x( 'Certificate Pending - %s still need to be graded, please check your profile for the status', 'placeholder: questions', 'ebox' ),
			...
			echo '<script>';
				echo "var continue_details ='" . ebox_quiz_continue_link( $quiz_post_id ) . "';"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			echo '</script>';
		}
	}
	...
	public static function certificate_link( $content, $pro_quiz = null ) {
		...
		if ( ! empty( $quiz_post_id ) ) {
			$cd = ebox_certificate_details( $quiz_post_id );
				...
				$ret      = "<a class='btn-blue' href='" . $cd['certificateLink'] . "' target='_blank'>" . apply_filters(
					...
					ebox_LMS::get_template(
						...
						array(
							...
							'message'      => esc_html__( 'PRINT YOUR CERTIFICATE', 'ebox' ),
						)
					),
					$user_id,
					$quiz_post_id
	...
	public static function edithtml() {
		...
		if ( $pagenow == 'post-new.php' && isset( $_GET['post_type'] ) && $_GET['post_type'] == 'ebox-quiz' || $pagenow == 'post.php' && ! empty( $_GET['post'] ) && get_post( $_GET['post'] )->post_type == 'ebox-quiz' ) {
			...
			if ( ! empty( $_GET['post'] ) ) {
				...
				$quizId  = intval( ebox_get_setting( $post_id, 'quiz_pro', true ) );

				/** This filter is documented in includes/admin/classes-posts-edits/class-ebox-admin-quiz-edit.php */
				if ( apply_filters( 'ebox_disable_advance_quiz', false, $post_id ) ) {
					return '';
				}
			} else {
				global $post;
				if ( ( is_a( $post, 'WP_Post' ) ) && ( $post->post_type == 'ebox-quiz' ) ) {
					$post_id = $post->ID;
				}
			}
	...
	public static function edit_process( $post_id ) {
		...
		/** This filter is documented in includes/admin/classes-posts-edits/class-ebox-admin-quiz-edit.php */
		if ( 'ebox-quiz' != $post->post_type || empty( $_POST['form'] ) || ! empty( $_POST['disable_advance_quiz_save'] ) || apply_filters( 'ebox_disable_advance_quiz', false, $post ) ) {
			return;
		}
	...
	public static function showModalWindow() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.MethodNameInvalid -- Better to keep it this way.
		...
		/**
		 * Added for ebox-2754 to prevent loading the inline CSS when inside
	 ...
	 */
	public function ebox_quiz_content( $quiz_content ) {
		return $quiz_content;
	}
 ...
 */
function ebox_get_non_course_qizzes( $bypass_transient = false ) {
	global $wpdb;

	$global_quiz_ids = array();

	$transient_key = 'ebox_global_quiz_ids';
	if ( ! $bypass_transient ) {
	...
	if ( false === $global_quiz_ids_transient ) {
		...
		$global_quiz_ids_query_str = "SELECT posts.ID FROM {$wpdb->posts} as posts
			...
			WHERE posts.post_type = 'ebox-quiz'
 ...
 * @return array An array of Quiz IDs.
 */
function ebox_get_open_quizzes( $bypass_transient = false ) {
	global $wpdb;
	...
	if ( false === $open_quiz_ids_transient ) {
		...
		$global_quiz_ids = ebox_get_non_course_qizzes(); // cspell:disable-line.
 ...
 */
function ebox_quiz_debug_log_init( $quiz_id = 0 ) {
	global $quiz_debug_error_log_file;

	if ( defined( 'ebox_QUIZ_DEBUG' ) && ebox_QUIZ_DEBUG ) {
		$user_id      = get_current_user_id();
		$ld_debug_dir = dirname( ebox_TEMPLATES_DIR ) . '/debug';
		...
		ebox_put_directory_index_file( trailingslashit( $ld_debug_dir ) . 'index.php' );

		ebox_Admin_File_Download_Handler::register_file_path(
			'ebox-debug',
			$ld_debug_dir
		);

		ebox_Admin_File_Download_Handler::try_to_protect_file_path(
			$ld_debug_dir
		);

		$date_time                 = ebox_adjust_date_time_display( time(), 'Ymd' );
 ...
 */
function ebox_quiz_debug_log_message( $message = '' ) {
	global $quiz_debug_error_log_file;
	...
	if ( defined( 'ebox_QUIZ_DEBUG' ) && ebox_QUIZ_DEBUG ) {
		if ( ( ! empty( $message ) ) && ( ! empty( $quiz_debug_error_log_file ) ) ) {
			...
			$user_id   = get_current_user_id();
			$date_time = ebox_adjust_date_time_display( time(), 'Y-m-d H:i:s' );
