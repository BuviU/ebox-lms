<?php
/**
 * ebox Quiz and Question related functions.
 *
 * @since 2.6.0
 * @package ebox\Quiz
 */
 ...
 * @global array $ebox_shortcode_atts ebox global shortcode attributes.
 *
 * @since 2.6.0
 *
 ...
 */
function ebox_get_quiz_id_by_pro_quiz_id( $quiz_pro_id = 0 ) {
	global $wpdb;
	...
	if ( ( isset( $quiz_post_ids[ $quiz_pro_id ] ) ) && ( ! empty( $quiz_post_ids[ $quiz_pro_id ] ) ) ) {
		...
		global $ebox_shortcode_atts;
		if ( ! empty( $ebox_shortcode_atts ) ) {
			foreach ( array_reverse( $ebox_shortcode_atts ) as $shortcode_tag => $shortcode_atts ) {
		...
		if ( ( is_a( $queried_object, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'quiz' ) === $queried_object->post_type ) ) {
		...
		$like_i = 'ebox-quiz_quiz_pro";i:' . absint( $quiz_pro_id ) . ';';
		$like_s = '"ebox-quiz_quiz_pro";s:' . $quiz_pro_id_len . ':"' . $quiz_pro_id_str . '"';

		// Using REGEX because it is slightly faster then OR on text fields pattern search.
		$sql_str      = $wpdb->prepare( 'SELECT post_id FROM ' . $wpdb->postmeta . ' as postmeta INNER JOIN ' . $wpdb->posts . " as posts ON posts.ID=postmeta.post_id WHERE posts.post_type = %s AND posts.post_status = %s AND postmeta.meta_key=%s AND postmeta.meta_value REGEXP '" . $like_i . '|' . $like_s . "'", 'ebox-quiz', 'publish', '_ebox-quiz' );
		$quiz_post_id = $wpdb->get_var( $sql_str );
		if ( ! empty( $quiz_post_id ) ) {
 ...
 */
function ebox_get_question_post_by_pro_id( $question_pro_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_transition_quiz_question_post_status( $new_status, $old_status, $post ) {
	global $wpdb;

	if ( $new_status !== $old_status ) {
		if ( ( ! empty( $post ) ) && ( is_a( $post, 'WP_Post' ) ) && ( in_array( $post->post_type, array( 'ebox-question' ), true ) ) === true ) {
			...
			if ( ! empty( $quiz_ids ) ) {
				...
				foreach ( $quiz_ids as $quiz_id ) {
					ebox_set_quiz_questions_dirty( $quiz_id );
				}
			}
		}
	}
}
add_action( 'transition_post_status', 'ebox_transition_quiz_question_post_status', 10, 3 );
 ...
 */
function ebox_set_quiz_questions_dirty( $quiz_id = 0 ) {
	if ( ! empty( $quiz_id ) ) {
 ...
 */
function ebox_set_question_quizzes_dirty( $question_post_id = 0 ) {
	$question_post_id = absint( $question_post_id );
	if ( ! empty( $question_post_id ) ) {
		$question_quiz_ids = ebox_get_quizzes_for_question( $question_post_id, true );
		if ( ! empty( $question_quiz_ids ) ) {
			foreach ( $question_quiz_ids as $question_quiz_id => $quiz_title ) {
				ebox_set_quiz_questions_dirty( $question_quiz_id );
			}
		}
	}
}
...
/**
 * Adds a WPProQuiz question to mirror a Question post (ebox-question).
 *
 * @since 2.6.0
 *
 * @param int   $question_pro_id Optional. Post ID of Question (ebox-question). Default 0.
 ...
 */
function ebox_update_pro_question( $question_pro_id = 0, $post_data = array() ) {
	...
	if ( ( isset( $question_model ) ) && ( is_a( $question_model, 'WpProQuiz_Model_Question' ) ) ) {
		if ( ( isset( $post_data['post_type'] ) ) && ( ebox_get_post_type_slug( 'question' ) === $post_data['post_type'] ) ) {
			...
			if ( ( isset( $post_data['post_ID'] ) ) && ( ! empty( $post_data['post_ID'] ) ) ) {
				$quiz_post_id = ebox_get_setting( $post_data['post_ID'], 'quiz' );
				if ( ! empty( $quiz_post_id ) ) {
					$quiz_post_id = absint( $quiz_post_id );
					$quiz_pro_id  = ebox_get_setting( $quiz_post_id, 'quiz_pro' );
					if ( ! empty( $quiz_pro_id ) ) {
		...
		ebox_update_question_template( $question, $post_data );
 ...
 */
function ebox_update_question_template( $question = null, $post_data = array() ) {
 ...
 */
function ebox_get_quizzes_for_question( $question_post_id = 0, $return_flat_array = false ) {
	global $wpdb;
 ...
 */
function ebox_get_quiz_id( $id = null ) {
	global $post;
		...
		if ( ! defined( 'REST_REQUEST' ) || ! REST_REQUEST ) {
			if ( is_admin() ) {
				...
				if (
					...
					|| ( ! in_array( $post_type, array( 'ebox-question' ), true ) )
				) {
					return false;
				}
	...
	if ( ebox_get_post_type_slug( 'quiz' ) === $p->post_type ) {
		return $p->ID;
	}
	...
	if ( ( isset( $_GET['quiz_id'] ) ) && ( ! empty( $_GET['quiz_id'] ) ) ) {
		...
		if ( ebox_get_post_type_slug( 'quiz' ) === get_post_type( intval( $_GET['post'] ) ) ) {
 ...
 * @since 2.6.0
 */
function ebox_quiz_navigation_admin_box_content() {
	global $typenow;
	...
	if ( ( isset( $_GET['post'] ) ) && ( ! empty( $_GET['post'] ) ) ) {
		$quiz_id      = ebox_get_quiz_id( absint( $_GET['post'] ) );
	...
	if ( ! empty( $quiz_id ) ) {
		...
		if ( ( is_a( $current_post, 'WP_Post' ) ) && ( in_array( $current_post->post_type, array( 'ebox-quiz', 'ebox-question' ), true ) ) ) {
			if ( in_array( $current_post->post_type, array( 'ebox-question' ), true ) ) {
				$instance['current_question_id'] = $current_post->ID;
			}
		}

		ebox_quiz_navigation_admin( $quiz_id, $instance, $question_query_args );

		if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'shared_questions' ) == 'yes' ) {
			ebox_quiz_switcher_admin( $quiz_id );
		}
	} else {
		echo sprintf(
			// translators: placeholders: Questions.
			esc_html_x( 'No associated %s', 'placeholder: Questions', 'ebox' ),
 ...
 *
 * @return array An array of quiz question (ebox-question) post ids.
 */
function ebox_get_quiz_questions( $quiz_id = 0 ) {
	if ( ! empty( $quiz_id ) ) {
 ...
 */
function ebox_quiz_navigation_admin( $quiz_id = 0, $instance = array(), $question_query_args = array() ) {
	if ( empty( $quiz_id ) ) {
		return;
	}

	$quiz = get_post( absint( $quiz_id ) );
	if ( ( ! $quiz ) || ( ! is_a( $quiz, 'WP_Post' ) ) || ( ebox_get_post_type_slug( 'quiz' ) !== $quiz->post_type ) ) {
		return;
	}
	...
	$question_list = ebox_get_quiz_questions( $quiz_id );
	if ( ! empty( $question_list ) ) {
		$quiz_questions_per_page = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'question_num' );
 ...
 */
function ebox_quiz_switcher_admin( $quiz_id ) {
 ...
 * @since 2.6.0
 */
function ebox_wp_ajax_ld_quiz_navigation_admin_pager() {
	$reply_data = array();
	...
	if ( ( isset( $_POST['nonce'] ) ) && ( ! empty( $_POST['nonce'] ) ) && ( wp_verify_nonce( $_POST['nonce'], 'ebox-pager' ) ) ) {
		...
		if ( ( ! empty( $quiz_id ) ) && ( ! empty( $widget_data ) ) ) {
			if ( ( isset( $_POST['widget_data']['nonce'] ) ) && ( ! empty( $_POST['widget_data']['nonce'] ) ) && ( wp_verify_nonce( $_POST['widget_data']['nonce'], 'ld_quiz_navigation_admin_pager_nonce_' . $quiz_id . '_' . get_current_user_id() ) ) ) {
				...
				$level                              = ob_get_level();
				ob_start();
				ebox_quiz_navigation_admin( $quiz_id, $widget_data, $questions_query_args );
...
add_action( 'wp_ajax_ld_quiz_navigation_admin_pager', 'ebox_wp_ajax_ld_quiz_navigation_admin_pager' );
 ...
 */
function ebox_proquiz_sync_question_fields( $question_post_id = 0, $question_pro_id = 0 ) {
...
/**
 * Syncs the `WPProQuiz` question category with the ebox question taxonomy.
 *
 * @since 2.6.0
 *
 * @param int                          $question_post_id Optional. The `WP_Post` Question ID. Default 0.
 ...
 */
function ebox_proquiz_sync_question_category( $question_post_id = 0, $question_pro_id = 0 ) {
 ...
 *
 * This is similar to the function ebox_get_quiz_id_by_pro_quiz_id() but returns
 ...
 * @return array An array of quiz post IDs.
 */
function ebox_get_quiz_post_ids( $quiz_pro_id = 0 ) {
	static $quiz_post_ids = array();
	$quiz_pro_id          = absint( $quiz_pro_id );
	if ( ! empty( $quiz_pro_id ) ) {
 ...
 */
function ebox_get_question_pro_fields( $question_pro_id = 0, $fields = null ) {
	$values = array();
 ...
 */
function ebox_get_quiz_pro_fields( $quiz_pro_id = 0, $fields = null ) {
	$values = array();
 ...
 */
function ebox_get_quiz_primary_shared( $quiz_pro_id = 0, $set_first = true ) {
	...
	if ( ! empty( $quiz_pro_id ) ) {
		if ( ( ! isset( $quiz_primary_post_ids[ $quiz_pro_id ] ) ) || ( empty( $quiz_primary_post_ids[ $quiz_pro_id ] ) ) ) {
			...
			$quiz_post_ids                         = ebox_get_quiz_post_ids( $quiz_pro_id );
 ...
 */
function ebox_quiz_result_message_sort( $messages = array() ) {
	$sorted = array();
	if ( ( isset( $messages ) ) && ( ! empty( $messages ) ) ) {
		...
		for ( $i = 0; $i < ebox_QUIZ_RESULT_MESSAGE_MAX; $i++ ) {
 ...
 */
function ebox_quiz_get_repeats( $quiz_post_id = 0 ) {
	$repeats = '';

	$quiz_post_id = absint( $quiz_post_id );
	if ( ! empty( $quiz_post_id ) ) {
		$quiz_settings = ebox_get_setting( $quiz_post_id );
 ...
 */
function ebox_quiz_convert_lock_cookie( $cookie_quiz = null ) {
 ...
 * @since 3.5.0
 */
function ebox_quiz_statistics_users_select2() {
	$reply_data = array(
		'items'       => array(),
		'total_items' => 0,
		'total_pages' => 1,
	...
	if ( ebox_use_select2_lib_ajax_fetch() ) {
		if ( ( is_user_logged_in() ) && ( ( ebox_is_admin_user() ) || ( ebox_is_team_leader_user() ) ) ) {
			if ( ( isset( $_POST['nonce'] ) ) && ( ! empty( $_POST['nonce'] ) ) && ( wp_verify_nonce( $_POST['nonce'], 'ebox-quiz-statistics-history' . get_current_user_id() ) ) ) {
				...
				if ( ( ebox_is_team_leader_user() ) && ( 'advanced' !== ebox_get_team_leader_manage_users() ) ) {
					$included_user_ids = array();
					if ( ( isset( $_POST['quiz_post_id'] ) ) && ( ! empty( $_POST['quiz_post_id'] ) ) ) {
						$included_user_ids = ebox_get_teams_leaders_users_for_course_step( absint( $_POST['quiz_post_id'] ) );
					}
				 ...
				 */
				$user_query_args = apply_filters( 'ebox_quiz_statistics_users_select2_query_args', $user_query_args );
					...
					if ( ( $user_query ) && ( is_a( $user_query, 'WP_User_Query' ) ) ) {
						if ( ebox_is_admin_user() ) {
							if ( ( 1 === $user_query_args['paged'] ) && ( ( ! isset( $user_query_args['search'] ) ) ) || ( empty( $user_query_args['search'] ) ) ) {
								$reply_data['items'] = array(
									array(
										'id'       => 'filters_team',
										'text'     => esc_html__( 'Special Filters', 'ebox' ),
										'disabled' => 1,
									),
									array(
										'id'   => '-1',
										'text' => esc_html__( 'all users', 'ebox' ),
									),
									array(
										'id'   => '-2',
										'text' => esc_html__( 'only registered users', 'ebox' ),
									),
									array(
										'id'   => '-3',
										'text' => esc_html__( 'only anonymous users', 'ebox' ),
									),
									array(
										'id'       => 'users_team',
										'text'     => esc_html__( 'Users', 'ebox' ),
										'disabled' => 1,
									),
								);
							}
						...
						if ( property_exists( $user_query, 'results' ) ) {
							foreach ( $user_query->results as $user ) {
								 ...
								 */
								$user_display_name = apply_filters( 'ebox_quiz_statistics_users_select2_display_name', $user_display_name, $user );
				 ...
				 * @param array  $user_query_args An array of query arguments.
				 */
				$reply_data = apply_filters( 'ebox_quiz_statistics_users_select2_reply_data', $reply_data, $user_query_args );
			}
		}
	}
...
add_action( 'wp_ajax_ebox_quiz_statistics_users_select2', 'ebox_quiz_statistics_users_select2' );
 ...
 */
function ebox_prepare_quiz_resume_data_to_js( $quiz_resume_data = array() ) {
