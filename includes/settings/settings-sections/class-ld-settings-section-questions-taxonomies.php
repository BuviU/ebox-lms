<?php
/**
 * ebox Settings Section for Question Taxonomies Metabox.
 *
 * @since 2.6.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Questions_Taxonomies' ) ) ) {
	/**
	 * Class ebox Settings Section for Question Taxonomies Metabox.
	 *
	 * @since 2.6.0
	 */
	class ebox_Settings_Questions_Taxonomies extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_settings_questions_taxonomies';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_settings_questions_taxonomies';
			...
			$this->settings_section_label = sprintf(
				...
				esc_html_x( '%s Taxonomies', 'placeholder: Question', 'ebox' ),
			...
			$this->settings_section_description = sprintf(
				...
				esc_html_x( 'Control which taxonomies can be used with the ebox %1$s %2$s.', 'placeholder: Quiz, Questions', 'ebox' ),
			...
			$this->setting_option_values = wp_parse_args(
				...
				array(
					...
					'question_category'         => array(
						'' => esc_html__( 'Select a category', 'ebox' ),
					),
				)
			);
			...
			$this->setting_option_fields = array(
				'proquiz_question_category' => array(
					...
					'label'               => sprintf(
						...
						esc_html_x( '%s Categories', 'placeholder: Question', 'ebox' ),
						ebox_Custom_Label::get_label( 'question' )
					),
					'value'               => $this->setting_option_values['proquiz_question_category'],
					'options'             => array(
						'yes' => array(
							'label'       => esc_html__( 'Yes', 'ebox' ),
							'description' => '',
							'tooltip'     => sprintf(
								// translators: placeholder: Question.
								esc_html_x( '%s categories cannot be disabled.', 'placeholder: Question', 'ebox' ),
				...
				'_question_category'        => array(
					...
					'label'          => esc_html__( 'Category management', 'ebox' ),
					'help_text'      => esc_html__( 'Select a category to update or delete the title.', 'ebox' ),
					'value'          => '',
						...
						if ( 'update' === $_POST['field_action'] ) {
								...
								if ( ( $category ) && ( is_a( $category, 'WpProQuiz_Model_Category' ) ) ) {
									...
									if ( $category_current_name !== $category_new_name ) {
										...
										if ( $update_ret ) {
											...
											$reply_data['message'] = '<span style="color: green" >' . __( 'Category updated.', 'ebox' ) . '</span>';
										}
									}
								}
							}
							...
							if ( ( isset( $_POST['field_value'] ) ) && ( ! empty( $_POST['field_value'] ) ) ) {
								...
								if ( ( $category ) && ( is_a( $category, 'WpProQuiz_Model_Category' ) ) ) {
									...
									if ( $update_ret ) {
										$reply_data['status']  = true;
										$reply_data['message'] = '<span style="color: green" >' . __( 'Category deleted.', 'ebox' ) . '</span>';
									}
								}
							}
						}
...
add_action(
	...
	function() {
		ebox_Settings_Questions_Taxonomies::add_section_instance();
	}
);
