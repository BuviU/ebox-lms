<?php
/**
 * ebox Settings Section for modules Custom Post Type Metabox.
 *
 * @since 2.4.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_modules_CPT' ) ) ) {
	/**
	 * Class ebox Settings Section for modules Custom Post Type Metabox.
	 *
	 * @since 2.4.0
	 */
	class ebox_Settings_modules_CPT extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_settings_modules_cpt';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_settings_modules_cpt';
			...
			$this->settings_section_label = sprintf(
				...
				esc_html_x( '%s Custom Post Type Options', 'placeholder: Lesson', 'ebox' ),
			...
			$this->settings_section_description = sprintf(
				...
				esc_html_x( 'Control options specific to the %s post type', 'placeholder: modules', 'ebox' ),
		...
		public function load_settings_fields() {
			$cpt_archive_url = home_url( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'modules' ) );
			$cpt_rss_url     = add_query_arg( 'post_type', 'ebox-modules', get_post_type_archive_feed_link( 'post' ) );
			...
			$this->setting_option_fields = array(
				'include_in_search'    => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Lesson.
						esc_html_x( '%s Search', 'placeholder: Lesson', 'ebox' ),
						ebox_get_custom_label( 'lesson' )
					),
					'help_text'           => sprintf(
						// translators: placeholder: lesson.
						esc_html_x( 'Includes the %s post type in front end search results', 'placeholder: lesson', 'ebox' ),
				...
				'search_login_only'    => array(
					...
					'help_text'      => sprintf(
						// translators: placeholder: modules.
						esc_html_x( 'Include %s only if user is logged in.', 'placeholder: modules', 'ebox' ),
						ebox_get_custom_label( 'modules' )
					),
				...
				'search_enrolled_only' => array(
					...
					'help_text'      => sprintf(
						// translators: placeholder: modules.
						esc_html_x( 'Include %s only if user is enrolled.', 'placeholder: modules', 'ebox' ),
						ebox_get_custom_label( 'modules' )
					),
				...
				'has_archive'          => array(
					...
					'help_text'           => sprintf(
						// translators: placeholder: modules, link to WP Permalinks page.
						esc_html_x( 'Enables the front end archive page where all %1$s are listed. You must %2$s for the change to take effect.', 'placeholder: , link to WP Permalinks page', 'ebox' ),
				...
				'has_feed'             => array(
					...
					'help_text'      => sprintf(
						// translators: placeholder: lesson.
						esc_html_x( 'Enables an RSS feed for all %1$s posts.', 'placeholder: lesson', 'ebox' ),
						ebox_get_custom_label_lower( 'lesson' )
					),
...
add_action(
	...
	function() {
		ebox_Settings_modules_CPT::add_section_instance();
	}
);
