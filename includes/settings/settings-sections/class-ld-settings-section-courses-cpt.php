<?php
/**
 * ebox Settings Section for Courses Custom Post Type Metabox.
 *
 * @since 2.4.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Courses_CPT' ) ) ) {
	/**
	 * Class ebox Settings Section for Courses Custom Post Type Metabox.
	 *
	 * @since 2.4.0
	 */
	class ebox_Settings_Courses_CPT extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_settings_courses_cpt';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_settings_courses_cpt';
			...
			$this->settings_section_label = sprintf(
				...
				esc_html_x( '%s Custom Post Type Options', 'Course Custom Post Type Options', 'ebox' ),
			...
			$this->settings_section_description = sprintf(
				// translators: placeholder: Course.
				esc_html_x( 'Control the ebox %s Custom Post Type Options.', 'placeholder: Course', 'ebox' ),
				ebox_get_custom_label( 'course' )
			);
		...
		public function load_settings_fields() {
			$cpt_archive_url = home_url( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'courses' ) );
			...
			$this->setting_option_fields = array(
				'include_in_search' => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Search', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'help_text' => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Includes the %s post type in front end search results', 'placeholder: course', 'ebox' ),
				...
				'has_archive'       => array(
					...
					'help_text'           => sprintf(
						...
						esc_html_x( 'Enables the front end archive page where all %1$s are listed. You must %2$s for the change to take effect.', 'placeholders: courses, link to WP Permalinks page', 'ebox' ),
				...
				'has_feed'          => array(
					...
					'help_text'      => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Enables an RSS feed for all %1$s posts.', 'placeholder: course', 'ebox' ),
						ebox_get_custom_label_lower( 'course' )
					),
...
add_action(
	...
	function() {
		ebox_Settings_Courses_CPT::add_section_instance();
	}
);
