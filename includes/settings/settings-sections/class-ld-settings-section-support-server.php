<?php
/**
 * ebox Settings Section for Support Server Metabox.
 *
 * @since 3.1.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Section_Support_Server' ) ) ) {
	/**
	 * Class ebox Settings Section for Support Server Metabox.
	 *
	 * @since 3.1.0
	 */
	class ebox_Settings_Section_Support_Server extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->settings_section_label = esc_html__( 'Server Settings', 'ebox' );
		...
		public function ebox_support_sections_init( $support_sections = array() ) {
			...
			if ( ! isset( $support_sections[ $this->setting_option_key ] ) ) {
				...
				$this->settings_set['settings']['phpversion'] = array(
					'label'      => 'PHP Version',
					'label_html' => esc_html__( 'PHP Version', 'ebox' ),
					'value'      => $php_version,
				);

				$version_compare = version_compare( ebox_MIN_PHP_VERSION, $php_version, '>' );
				$color           = 'green';
				...
				if ( -1 == $version_compare ) {
					$this->settings_set['settings']['phpversion']['value_html'] .= ' - <a href="https://www.ebox.com/support/docs/getting-started/requirements/" target="_blank">' . esc_html__( 'ebox Minimum Requirements', 'ebox' ) . '</a>';
				...
				if ( defined( 'PHP_OS' ) ) {
					$this->settings_set['settings']['PHP_OS'] = array(
						'label'      => 'PHP OS',
						'label_html' => esc_html__( 'PHP OS', 'ebox' ),
						'value'      => PHP_OS,
					);
				}
				...
				if ( defined( 'PHP_OS_FAMILY' ) ) {
					$this->settings_set['settings']['PHP_OS_FAMILY'] = array(
						'label'      => 'PHP OS Family',
						'label_html' => esc_html__( 'PHP OS Family', 'ebox' ),
				...
				if ( true == $db_server_info['mysqldb_active'] ) {
					...
					$this->settings_set['settings']['db_version'] = array(
						'label'      => 'MySQL version',
						'label_html' => esc_html__( 'MySQL version', 'ebox' ),
						'value'      => $db_version,
					);

					$version_compare = version_compare( ebox_MIN_MYSQL_VERSION, $db_version, '>' );
					$color           = 'green';
					if ( -1 == $version_compare ) {
						$color = 'red';
					}
					...
					if ( -1 == $version_compare ) {
						$this->settings_set['settings']['db_version']['value_html'] .= ' - <a href="https://www.ebox.com/support/docs/getting-started/requirements/" target="_blank">' . esc_html__( 'ebox Minimum Requirements', 'ebox' ) . '</a>';
					...
					$this->settings_set['settings']['db_version'] = array(
						...
						'label_html' => esc_html__( 'MariaDB version', 'ebox' ),
						'value'      => $db_version,
					);

					$version_compare = version_compare( ebox_MIN_MARIA_VERSION, $db_version, '>' );
					$color           = 'green';
					if ( -1 == $version_compare ) {
						$color = 'red';
					}
				 ...
				 */
				$this->php_ini_settings = apply_filters( 'ebox_support_php_ini_settings', $this->php_ini_settings );
				if ( ! empty( $this->php_ini_settings ) ) {
					...
					if ( ! extension_loaded( 'curl' ) ) {
						...
						$this->settings_set['settings']['curl']['value_html'] = '<span style="color: red">' . esc_html__( 'No', 'ebox' ) . '</span>';

					} else {
						$this->settings_set['settings']['curl']['value']      = 'Yes<br />';
						$this->settings_set['settings']['curl']['value_html'] = '<span style="color: green">' . esc_html__( 'Yes', 'ebox' ) . '</span><br />';

						$version = curl_version();

						$this->settings_set['settings']['curl']['value']      .= 'Version: ' . $version['version'] . '<br />';
						$this->settings_set['settings']['curl']['value_html'] .= esc_html__( 'Version', 'ebox' ) . ': ' . $version['version'] . '<br />';

						$this->settings_set['settings']['curl']['value']      .= 'SSL Version: ' . $version['ssl_version'] . '<br />';
						$this->settings_set['settings']['curl']['value_html'] .= esc_html__( 'SSL Version', 'ebox' ) . ': ' . $version['ssl_version'] . '<br />';

						$this->settings_set['settings']['curl']['value']      .= 'Libz Version: ' . $version['libz_version'] . '<br />';
						$this->settings_set['settings']['curl']['value_html'] .= esc_html__( 'Libz Version', 'ebox' ) . ': ' . $version['libz_version'] . '<br />';

						$this->settings_set['settings']['curl']['value']      .= 'Protocols: ' . join( ', ', $version['protocols'] ) . '<br />';
						$this->settings_set['settings']['curl']['value_html'] .= esc_html__( 'Protocols', 'ebox' ) . ': ' . join( ', ', $version['protocols'] ) . '<br />';

						// phpcs:ignore WordPress.Security.NonceVerification.Recommended
						if ( isset( $_GET['ld_debug'] ) ) {
							$paypal_email         = get_option( 'ebox_settings_paypal' );
							...
							if ( ! $ca_certificates_path ) {
								if ( isset( $paypal_email['paypal_email'] ) && ! empty( $paypal_email['paypal_email'] ) ) {
									...
									$this->settings_set['settings']['curl']['value_html'] .= '<span style="color: red">' . esc_html__( 'Path to the CA certificates not set. Please add it to curl.cainfo in the php.ini file. Otherwise, PayPal may not work.', 'ebox' ) . '</span><br />';
								}

								if ( isset( $paypal_email['paypal_email'] ) && empty( $paypal_email['paypal_email'] ) ) {
									$this->settings_set['settings']['curl']['value']      .= 'Path to the CA certificates not set. (X)<br />';
									$this->settings_set['settings']['curl']['value_html'] .= esc_html__( 'Path to the CA certificates not set.', 'ebox' ) . '</span><br />';
								}
							} else {
								$this->settings_set['settings']['curl']['value']      .= 'Path to the CA certificates: ' . $ca_certificates_path . '<br />';
								$this->settings_set['settings']['curl']['value_html'] .= esc_html__( 'Path to the CA certificates', 'ebox' ) . ': ' . $ca_certificates_path . '</span><br />';
							}
						}
					}
				}
				 ...
				 */
				$this->php_extensions = apply_filters( 'ebox_support_php_extensions', $this->php_extensions );
				if ( ! empty( $this->php_extensions ) ) {
					...
					foreach ( $this->php_extensions as $ini_key ) {
						$this->settings_set['settings'][ $ini_key ] = array(
							...
							'value_html' => extension_loaded( $ini_key ) ? esc_html__( 'Yes', 'ebox' ) : '<span style="color: red">' . esc_html__( 'No', 'ebox' ) . '</span>',
						);
					}
				}
		...
		public function show_support_section( $settings_section_key = '', $settings_screen_id = '' ) {
			if ( $settings_section_key === $this->settings_section_key ) {
				...
				if ( is_a( $support_page_instance, 'ebox_Settings_Page_Support' ) ) {
...
add_action(
	...
	function() {
		ebox_Settings_Section_Support_Server::add_section_instance();
	}
);
