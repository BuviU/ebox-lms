<?php
/**
 * ebox Settings Section for Stripe Connect.
 *
 * @since   4.0.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Section_Stripe_Connect' ) ) ) {
	/**
	 * Class ebox Settings Section for Stripe Connect.
	 *
	 * @since 4.0.0
	 */
	class ebox_Settings_Section_Stripe_Connect extends ebox_Settings_Section {
		const CONNECT_SERVER_URL = 'https://connect.ebox.com/stripe/connect.php';
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_stripe_connection_settings';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_stripe_connection_settings';

			// Used within the Settings API to uniquely identify this section.
			$this->settings_section_key = 'settings_stripe_connection';

			// Section label/header.
			$this->settings_section_label = esc_html__( 'Stripe Connect Settings', 'ebox' );

			// Used to associate this section with the parent section.
			$this->settings_parent_section_key = 'settings_payments_list';

			$this->settings_section_listing_label = esc_html__( 'Stripe Connect', 'ebox' );
		...
		public static function get_default_stripe_webhook_url() {
			return add_query_arg(
				array( 'ebox-integration' => 'stripe_connect' ),
		...
		public function load_settings_fields() {
			$this->setting_option_fields = array(
				...
				'publishable_key_test' => array(
					...
					'label'     => __( 'Test Publishable Key', 'ebox' ),
					'help_text' => __( 'Test publishable key used in test mode.', 'ebox' ),
					'type'      => 'hidden',
					'value'     => $this->setting_option_values['publishable_key_test'] ?? '',
				),
				'secret_key_test'      => array(
					'name'      => 'secret_key_test',
					'label'     => __( 'Test Secret Key', 'ebox' ),
					'help_text' => __( 'Test secret key used in test mode.', 'ebox' ),
					'type'      => 'hidden',
					'value'     => $this->setting_option_values['secret_key_test'] ?? '',
				),
				'publishable_key_live' => array(
					'name'      => 'publishable_key_live',
					'label'     => __( 'Live Publishable Key', 'ebox' ),
					'help_text' => __( 'Live publishable key used in real transaction.', 'ebox' ),
					'type'      => 'hidden',
					'value'     => $this->setting_option_values['publishable_key_live'] ?? '',
				),
				'secret_key_live'      => array(
					'name'      => 'secret_key_live',
					'label'     => __( 'Live Secret Key', 'ebox' ),
					'help_text' => __( 'Live secret key used in real transaction.', 'ebox' ),
					'type'      => 'hidden',
					'value'     => $this->setting_option_values['secret_key_live'] ?? '',
				),
				'account_id'           => array(
					'name'  => 'account_id',
					'label' => __( 'Account Id', 'ebox' ),
					'type'  => 'hidden',
					'value' => $this->setting_option_values['account_id'] ?? '',
				),
				'payment_methods'      => array(
					'name'      => 'payment_methods',
					'label'     => __( 'Payment Methods', 'ebox' ),
					'help_text' => __( 'Stripe payment methods to be enabled on the site.', 'ebox' ),
					'value'     => $this->setting_option_values['payment_methods'],
					'type'      => 'checkbox',
					'options'   => array(
						'card'  => __( 'Credit Card', 'ebox' ),
						'ideal' => __( 'Ideal', 'ebox' ),
					),
				),
				'return_url'           => array(
				...
				'webhook_url'          => array(
					...
					'help_text' => esc_html__( 'Stripe webhook endpoint. You have to add this URL in the webhooks section of your Stripe Dashboard.', 'ebox' ),
		...
		public function show_notices() {
			...
			if ( ! empty( $_GET['ld_stripe_error'] ) && ! empty( $_GET['error_code'] ) && ! empty ( $_GET['error_message'] ) ) { // phpcs:ignore
				...
				<div class="notice notice-error is-dismissible">
					<p>
						<b>
							<?php esc_html_e( 'Stripe Error', 'ebox' ); ?>:
						</b>
			...
			if (
				...
				$webhook_second_detail = esc_html__(
					'Stripe webhooks are required so ebox can communicate properly with the payment gateway to confirm payment completion, renewals, and more.',
					'ebox'
				);
				?>
			...
			if ( isset( $_GET['ld_stripe_disconnected'] ) && self::STRIPE_RETURNED_AND_PROCESSED_SUCCESS === intval( $_GET['ld_stripe_disconnected'] ) ) { // phpcs:ignore
				...
				<div class="notice notice-success is-dismissible">
					<p>
						<?php esc_html_e( 'Stripe disconnected', 'ebox' ); ?>.
					</p>
				</div>
				<?php
			}
			...
			if ( $this->is_on_payments_setting_page() && ! $this->account_is_connected() ) {
				...
				<div class="notice connect-stripe" style="display: flex; justify-content: space-between; padding: 20px 25px;">
					<h1>
						<?php esc_html_e( 'Want to accept credit card payments directly on your website?', 'ebox' ); ?>
					</h1>
		...
		public function connection_button(): void {
			if ( $this->account_is_connected() ) :
				...
				<a href="<?php echo esc_url( $this->generate_disconnect_url() ); ?>"
					class="ebox-stripe-connect" title="<?php esc_html_e( 'Disconnect Stripe', 'ebox' ); ?>">
					...
					<span>
						<?php esc_html_e( 'Disconnect Stripe', 'ebox' ); ?>
					</span>
				</a>
				<?php
			else :
				...
				<a href="<?php echo esc_url( self::generate_connect_url() ); ?>"
					class="ebox-stripe-connect" title="<?php esc_html_e( 'Connect Stripe', 'ebox' ); ?>">
					<span class="stripe-logo">
						<svg width="15" height="21" viewBox="0 0 15 21" fill="none"
							xmlns="http://www.w3.org/2000/svg">
							<path d="M6.05469 6.55469C6.05469 5.69531 6.75781 5.34375 7.92969 5.34375C9.64844 5.34375 11.7969 5.85156 13.4766 6.78906V1.55469C11.6406 0.8125 9.80469 0.5 7.92969 0.5C3.4375 0.5 0.429688 2.88281 0.429688 6.82812C0.429688 13 8.86719 11.9844 8.86719 14.6406C8.86719 15.6953 7.96875 16.0078 6.75781 16.0078C4.88281 16.0078 2.5 15.2656 0.664062 14.25V19.25C2.5 20.0703 4.57031 20.5 6.75781 20.5391C11.3672 20.5391 14.5703 18.5469 14.5703 14.5234C14.5703 7.88281 6.05469 9.05469 6.05469 6.55469Z"
					...
					<span>
						<?php esc_html_e( 'Connect Stripe', 'ebox' ); ?>
					</span>
				</a>
				<?php
			endif;
		...
		private function is_on_payments_setting_page(): bool {
			return isset( $_GET['page'] ) && 'ebox_lms_payments' === $_GET['page']; // phpcs:ignore
		}
		...
		public static function is_stripe_connected(): bool {
			$options = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_Stripe_Connect' );
	...
	add_action(
		...
		array( ebox_Settings_Section_Stripe_Connect::class, 'add_section_instance' )
	);
}
