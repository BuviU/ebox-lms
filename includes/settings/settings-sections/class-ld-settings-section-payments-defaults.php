<?php
/**
 * ebox Settings Section for Payments Defaults Configurations Metabox.
 *
 * @since 4.1.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Section_Payments_Defaults' ) ) ) {

	/**
	 * Class ebox Settings Section for Payments Defaults Configurations Metabox.
	 *
	 * @since 4.1.0
	 */
	class ebox_Settings_Section_Payments_Defaults extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_settings_payments_defaults';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_settings_payments_defaults';

			// Used within the Settings API to uniquely identify this section.
			$this->settings_section_key = 'settings_payments_defaults';

			// Section label/header.
			$this->settings_section_label = esc_html__( 'Default Payments Configurations', 'ebox' );
		...
		public function load_settings_values() {
			...
			if ( ! isset( $this->setting_option_values['currency'] ) || empty( $this->setting_option_values['currency'] ) ) {
				...
				if ( is_plugin_active( 'ebox-stripe/ebox-stripe.php' ) && ! empty( $stripe_settings ) && ! empty( $stripe_settings['currency'] ) ) {
					...
					$stripe_currency = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Stripe_Connect', 'currency' );
					if ( 'on' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_PayPal', 'enabled' ) ) {
		...
		public function load_settings_fields() {
			...
			$this->setting_option_fields['currency'] = array(
				'name'             => 'currency',
				'type'             => 'select',
				'label'            => esc_html__( 'Currency', 'ebox' ),
				'help_text'        => sprintf(
					// translators: placeholder: Link to ISO 4217.
					esc_html_x( 'Enter the currency code for transactions. It should be one currency code from the %s list.', 'placeholder: URL to ISO 4217', 'ebox' ),
					'<a href="https://en.wikipedia.org/wiki/ISO_4217#Active_codes" target="_blank">' . esc_html__( 'ISO 4217', 'ebox' ) . '</a>'
				),
		...
		public function display_currency_selector( $field_args = array() ): void {
			...
			if ( ( isset( $field_args['type'] ) ) && ( ! empty( $field_args['type'] ) ) ) {
				$field_ref = ebox_Settings_Fields::get_field_instance( $field_args['type'] );
				if ( is_a( $field_ref, 'ebox_Settings_Fields' ) ) {
					...
					$currency_codes_list = ebox_currency_codes_list();
					...
					if ( empty( $currency_country ) ) {
						...
						$select_html .= sprintf(
							...
							$old_currency,
							! empty( $old_currency ) ? $old_currency : esc_html__( 'Select your country', 'ebox' )
						);
					}
...
add_action(
	...
	function() {
		ebox_Settings_Section_Payments_Defaults::add_section_instance();
	}
);
