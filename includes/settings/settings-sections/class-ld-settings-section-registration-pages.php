<?php
/**
 * ebox Settings Section for Registration Pages Metabox.
 *
 * @since 3.6.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Section_Registration_Pages' ) ) ) {

	/**
	 * Class ebox Settings Section for Registration Pages Metabox.
	 *
	 * @since 3.6.0
	 */
	class ebox_Settings_Section_Registration_Pages extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_settings_registration_pages';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_settings_registration_pages';

			// Used within the Settings API to uniquely identify this section.
			$this->settings_section_key = 'settings_registration_pages';

			// Section label/header.
			$this->settings_section_label = esc_html__( 'Registration/Login Pages', 'ebox' );
		...
		public function load_settings_fields() {
			...
			$this->setting_option_fields['registration'] = array(
				'name'             => 'registration',
				'type'             => 'select',
				'label'            => esc_html__( 'Registration/Login', 'ebox' ),
			...
			$this->setting_option_fields['registration_success'] = array(
				...
				'type'             => 'select',
				'label'            => esc_html__( 'Registration Success', 'ebox' ),
			...
			$this->setting_option_fields['reset_password'] = array(
				'name'             => 'reset_password',
				'type'             => 'select',
				'label'            => esc_html__( 'Reset Password', 'ebox' ),
		...
		public function display_pages_selector( $field_args = array() ) {
			...
			if ( ( isset( $field_args['type'] ) ) && ( ! empty( $field_args['type'] ) ) ) {
				...
				if ( is_a( $field_ref, 'ebox_Settings_Fields' ) ) {
					...
					$select_args = array(
						'echo'             => false,
						'show_option_none' => esc_html__( 'Select a Page...', 'ebox' ),
						'name'             => $field_name,
						'id'               => $field_id,
						'class'            => $field_class,
					);
			...
			if ( ! strpos( $content, 'ld-reset-password' ) ) {
				wp_update_post(
					array(
						...
						'post_content' => $content . '<!-- wp:ebox/ld-reset-password {"width":""} /-->',
						'post_type'    => 'page',
					)
				);
			}
...
add_action(
	...
	function() {
		ebox_Settings_Section_Registration_Pages::add_section_instance();
	}
);
