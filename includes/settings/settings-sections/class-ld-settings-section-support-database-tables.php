<?php
/**
 * ebox Settings Section for Support Database Tables Metabox.
 *
 * @since 3.1.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Section_Support_Database_Tables' ) ) ) {
	/**
	 * Class ebox Settings Section for Support Database Tables Metabox.
	 *
	 * @since 3.1.0
	 */
	class ebox_Settings_Section_Support_Database_Tables extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->settings_section_label = esc_html__( 'Database Tables', 'ebox' );
		...
		public function ebox_support_sections_init( $support_sections = array() ) {
			...
			if ( ! isset( $support_sections[ $this->setting_option_key ] ) ) {
				 ...
				 */
				$this->db_tables = apply_filters( 'ebox_support_db_tables', $this->db_tables );
				if ( ! empty( $this->db_tables ) ) {
					...
					foreach ( $this->db_tables as $db_key => $db_table ) {
						...
						if ( is_array( $table_status_info ) ) {
							...
							$this->settings_set['settings'][ $db_table ]['value_html'] = '<span style="color: green">' . esc_html__( 'Yes', 'ebox' ) . '</span>' . $rows_str;
							...
							if ( false === $valid_index ) {
								...
								$this->settings_set['settings'][ $db_table ]['value_html'] .= '<span style="color: red">' . esc_html__( 'AUTO_INCREMENT missing', 'ebox' ) . '</span>';
							}
						} else {
							$this->settings_set['settings'][ $db_table ]['value']      = 'No - (X)';
							$this->settings_set['settings'][ $db_table ]['value_html'] = '<span style="color: red">' . esc_html__( 'No', 'ebox' ) . '</span>';
						}
					}
				}
				/**
				 * Filters ebox admin support section settings.
				 *
				 ...
				 */
				$this->system_info['ld_database_tables'] = apply_filters( 'ebox_support_section', $this->settings_set, 'ld_database_tables' );
		...
		public function admin_notice_upgrade_notice() {
			...
			if ( ( isset( $this->admin_notice_tables ) ) && ( ! empty( $this->admin_notice_tables ) ) ) {
				...
				<?php
					echo wp_kses_post(
						wpautop(
							'IMPORTANT: The following database tables are missing AUTO_INCREMENT on the primary index. This means data cannot be written to these tables. Please try reactivating ebox ASAP.<br />' . implode( ', ', $this->admin_notice_tables )
						)
					);
				?>
				</div>
...
add_action(
	...
	function() {
		ebox_Settings_Section_Support_Database_Tables::add_section_instance();
	}
);
