<?php
/**
 * ebox Settings Section for Topics Custom Post Type Metabox.
 *
 * @since 2.4.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Topics_CPT' ) ) ) {
	/**
	 * Class ebox Settings Section for Topics Custom Post Type Metabox.
	 *
	 * @since 2.4.0
	 */
	class ebox_Settings_Topics_CPT extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_settings_topics_cpt';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_settings_topics_cpt';
			...
			$this->settings_section_description = sprintf(
				...
				esc_html_x( 'Control the ebox %s Custom Post Type Options.', 'placeholder: Topics', 'ebox' ),
		...
		public function load_settings_fields() {
			$cpt_archive_url = home_url( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'topics' ) );
			$cpt_rss_url     = add_query_arg( 'post_type', 'ebox-topic', get_post_type_archive_feed_link( 'post' ) );
			...
			$this->setting_option_fields = array(
				'include_in_search'    => array(
					...
					'label'               => sprintf(
						...
						esc_html_x( '%s Search', 'placeholder: Topic', 'ebox' ),
						ebox_get_custom_label( 'topic' )
					),
					'help_text'           => sprintf(
						// translators: placeholder: topic.
						esc_html_x( 'Includes the %s post type in front end search results', 'placeholder: topic', 'ebox' ),
				...
				'search_login_only'    => array(
					...
					'help_text'      => sprintf(
						// translators: placeholder: Topics.
						esc_html_x( 'Include %s only if user is logged in.', 'placeholder: Topics', 'ebox' ),
						ebox_get_custom_label( 'topics' )
					),
				...
				'search_enrolled_only' => array(
					...
					'help_text'      => sprintf(
						// translators: placeholder: Topics.
						esc_html_x( 'Include %s only if user is enrolled.', 'placeholder: Topics', 'ebox' ),
						ebox_get_custom_label( 'topics' )
					),
				...
				'has_archive'          => array(
					...
					'help_text'           => sprintf(
						...
						esc_html_x( 'Enables the front end archive page where all %1$s are listed. You must %2$s for the change to take effect.', 'placeholder: topics, link to WP Permalinks page', 'ebox' ),
				...
				'has_feed'             => array(
					...
					'help_text'      => sprintf(
						// translators: placeholder: topic.
						esc_html_x( 'Enables an RSS feed for all %1$s posts.', 'placeholder: topic', 'ebox' ),
						ebox_get_custom_label_lower( 'topic' )
					),
...
add_action(
	...
	function() {
		ebox_Settings_Topics_CPT::add_section_instance();
	}
);
