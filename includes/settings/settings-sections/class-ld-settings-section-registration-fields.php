<?php
/**
 * ebox Settings Section for Registration Fields Metabox.
 *
 * @since 3.6.0
...
if ( ( class_exists( 'ebox_Settings_Section' ) ) && ( ! class_exists( 'ebox_Settings_Section_Registration_Fields' ) ) ) {

	/**
	 * Class ebox Settings Section for Registration Fields Metabox.
	 *
	 * @since 3.6.0
	 */
	class ebox_Settings_Section_Registration_Fields extends ebox_Settings_Section {
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'ebox_settings_registration_fields';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'ebox_settings_registration_fields';

			// Used within the Settings API to uniquely identify this section.
			$this->settings_section_key = 'settings_registration_fields';

			// Section label/header.
			$this->settings_section_label = esc_html__( 'Registration Fields', 'ebox' );
		...
		public function load_settings_values() {
			...
			$this->setting_option_values['username_placeholder'] = esc_html__( 'Username', 'ebox' );
			...
			$this->setting_option_values['email_placeholder'] = esc_html__( 'Email', 'ebox' );
			...
			$this->setting_option_values['first_name_placeholder'] = esc_html__( 'First Name', 'ebox' );
			...
			$this->setting_option_values['last_name_placeholder'] = esc_html__( 'Last Name', 'ebox' );
			...
			$this->setting_option_values['password_placeholder'] = esc_html__( 'Password', 'ebox' );
		...
		public function show_settings_section( $section = null ) {
			...
			<div class="ebox ebox_options">
				<table class="ebox-settings-table ebox-settings-table-registrations-fields ebox-settings-table-sortable widefat striped" cellspacing="0">
				<thead>
				<tr>
					<th class="col-name-move"></th>
					<th class="col-name-enabled"><?php esc_html_e( 'Enabled', 'ebox' ); ?></th>
					<th class="col-name-required"><?php esc_html_e( 'Required', 'ebox' ); ?></th>
					<th class="col-name-label"><?php esc_html_e( 'Label', 'ebox' ); ?></th>
				<tr>
				</thead>
				<tbody>
				<?php
					...
					?>
						<tr>
							<td class="col-name-move col-valign-middle">
								...
								<input type="hidden" name="ebox_settings_registration_fields[fields_order][]" id="ebox_settings_registration_fields_fields_order_<?php echo esc_attr( $field_prefix ); ?>" class="ebox-section-field ebox-section-field-hidden" value="<?php echo esc_attr( $field_prefix ); ?>">
							</td>
							...
							if ( isset( $fields[ $field_prefix . '_enabled' ] ) && ( 'first_name' === $field_prefix || 'last_name' === $field_prefix ) ) {
								...
								echo '<input type="hidden" value="yes" name="ebox_settings_registration_fields[' . esc_attr( $field_prefix ) . '_enabled]" />';
								echo esc_html_e( 'Required', 'ebox' );
							}
							?>
								</div>
							</td>
							<td class="col-name-required col-valign-middle">
								...
								if ( isset( $fields[ $field_prefix . '_required' ] ) && ( 'first_name' === $field_prefix || 'last_name' === $field_prefix ) ) {
									...
									echo '<input type="hidden" value="yes" name="ebox_settings_registration_fields[' . esc_attr( $field_prefix ) . '_required]" / checked="checked">';
									echo esc_html_e( 'Required', 'ebox' );
								}
								?>
								</div>
...
add_action(
	...
	function() {
		ebox_Settings_Section_Registration_Fields::add_section_instance();
	}
);
