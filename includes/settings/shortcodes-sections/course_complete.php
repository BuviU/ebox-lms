<?php
/**
 * ebox Shortcode Section for Course Complete [course_complete].
 *
 * @since 2.4.0
...
if ( ( class_exists( 'ebox_Shortcodes_Section' ) ) && ( ! class_exists( 'ebox_Shortcodes_Section_course_complete' ) ) ) {
	/**
	 * Class ebox Shortcode Section for Course Complete [course_complete].
	 *
	 * @since 2.4.0
	 */
	class ebox_Shortcodes_Section_course_complete extends ebox_Shortcodes_Section /* phpcs:ignore PEAR.NamingConventions.ValidClassName.Invalid */ {
		...
		public function __construct( $fields_args = array() ) {
			...
			$this->shortcodes_section_title = sprintf(
				...
				esc_html_x( '%s Complete', 'placeholder: Course', 'ebox' ),
				ebox_Custom_Label::get_label( 'course' )
			);
			$this->shortcodes_section_type        = 2;
			$this->shortcodes_section_description = sprintf(
				// translators: placeholder: course.
				esc_html_x( 'This shortcode shows the content if the user has completed the %s. The shortcode can be used on any page or widget area.', 'placeholders: course', 'ebox' ),
