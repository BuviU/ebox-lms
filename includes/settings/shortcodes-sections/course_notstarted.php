<?php
/**
 * ebox Shortcode Section for Course Not Started [course_notstarted].
 *
 * @since 2.4.0
...
if ( ( class_exists( 'ebox_Shortcodes_Section' ) ) && ( ! class_exists( 'ebox_Shortcodes_Section_course_notstarted' ) ) ) {
	/**
	 * Class ebox Shortcode Section for Course Not Started [course_notstarted].
	 *
	 * @since 2.4.0
	 */
	class ebox_Shortcodes_Section_course_notstarted extends ebox_Shortcodes_Section /* phpcs:ignore PEAR.NamingConventions.ValidClassName.Invalid */ {
		...
		public function __construct( $fields_args = array() ) {
			...
			$this->shortcodes_section_title = sprintf( esc_html_x( '%s Not Started', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) );
			$this->shortcodes_section_type  = 2;

			// translators: placeholder: course.
			$this->shortcodes_section_description = sprintf( wp_kses_post( _x( 'This shortcode shows the content if the user has access to the %s but not yet started. The shortcode can be used on <strong>any</strong> page or widget area.', 'placeholders: course', 'ebox' ) ), ebox_get_custom_label_lower( 'course' ) );
