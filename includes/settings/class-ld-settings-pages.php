<?php
/**
 * ebox Settings Page Abstract Class.
 *
 * @since 2.4.0
...
if ( ! class_exists( 'ebox_Settings_Page' ) ) {
	/**
	 * Class for ebox Settings Pages.
	 *
	 * @since 2.4.0
	 */
	class ebox_Settings_Page {
		...
		/**
		 * Match the parent menu below ebox main menu. This will be the URL as in
		 * edit.php?post_type=ebox-courses, admin.php?page=ebox-lms-reports, admin.php?page=ebox_lms_settings
		 *
		 ...
		 */
		protected $menu_page_capability = ebox_ADMIN_CAPABILITY_CHECK;

		/**
		 * Match the WP Screen ID. DO NOT SET. This is set when from add_submenu_page().
		 * The value set via WP will be something like 'ebox-courses_page_courses-options'
		 * for the URL admin.php?page=courses-options because it will reside within the
		 * ebox-courses submenu.
		 *
		...
		/**
		 * Match the URL 'page=' parameter value. For example admin.php?page=ebox-lms-reports
		 * value will be 'ebox-lms-reports'
		 *
		...
		public function __construct() {
			...
			add_action( 'ebox_admin_tabs_set', array( $this, 'admin_tabs' ), 10 );
			...
			if ( ! empty( $this->settings_page_id ) ) {
				...
				if ( ! isset( $ebox_pages[ $this->settings_page_id ] ) ) {
					$ebox_pages[] = $this->settings_page_id;
				}
			}
		 ...
		 *
		 * @return ebox_Settings_Page|null Page instance.
		 */
			...
			if ( ( ! empty( $criteria_field ) ) && ( ! empty( $criteria_value ) ) ) {
				if ( ! empty( self::$_instances ) ) {
					foreach ( self::$_instances as $_instance_key => $_instance_object ) {
						if ( ( $_instance_object ) && ( is_a( $_instance_object, 'ebox_Settings_Page' ) ) ) {
		...
		final public static function get_global_settings_page_names(): array {
			return array_keys(
				array_filter(
					self::$_instances,
					function ( ebox_Settings_Page $instance ): bool {
						return 'admin.php?page=ebox_lms_settings' === $instance->parent_menu_page_url;
					}
				)
			);
		}
		...
		public function admin_init() {
			...
			/**
			 * Fires on ebox settings page init.
			 *
			 ...
			 */
			if ( true === apply_filters( 'ebox_settings_show_section_submit', $this->show_submit_meta, $this->settings_page_id ) ) {
				$submit_obj = new ebox_Settings_Section_Side_Submit(
					array(
			 ...
			 * @param string $settings_page_id Settings Page ID.
			 */
			if ( true === apply_filters( 'ebox_settings_show_section_quick_liinks', $this->show_quick_links_meta, $this->settings_page_id ) ) { // cspell:disable-line.
				$ql_obj = new ebox_Settings_Section_Side_Quick_Links(
					array(
						'settings_screen_id' => $this->settings_screen_id,
						'settings_page_id'   => $this->settings_page_id,
					)
		...
		public function load_settings_page() {
			...
			if ( defined( 'ebox_SETTINGS_SECTION_TYPE' ) && ( 'metabox' === ebox_SETTINGS_SECTION_TYPE ) ) {
				 ...
				 */
				do_action( 'ebox_add_meta_boxes', $this->settings_screen_id );
			...
			/**
			 * Fires on ebox settings page load.
			 *
			 * @deprecated 3.2.3 Use {@see 'ebox_settings_page_load'} instead.
			 *
			 * @param string $setting_screen_id Settings screen ID.
			 * @param string $setting_page_id   Settings page ID.
			 */
			do_action_deprecated(
				'ebox-settings-page-load',
			 ...
			 * @param string $setting_page_id   Settings page ID.
			 */
			do_action( 'ebox_settings_page_load', $this->settings_screen_id, $this->settings_page_id );
		}
		...
		public function load_footer_scripts() {
			...
			<script type="text/javascript">
				...
				jQuery( function($) {
					...
					$('.ebox-settings-page-wrap .submitdelete').on('click', function() {
			 ...
			 * @param string $setting_page_id   Settings page ID.
			 */
			do_action( 'ebox_settings_page_footer_scripts', $this->settings_screen_id, $this->settings_page_id );
		}
		...
		public function show_settings_page() {
			if ( defined( 'ebox_SETTINGS_SECTION_TYPE' ) && ( 'metabox' === ebox_SETTINGS_SECTION_TYPE ) ) {
				...
				<div class="wrap ebox-settings-page-wrap">
					 ...
					 * @param string $setting_page_id   Settings page ID.
					 */
					do_action( 'ebox_settings_page_before_title', $this->settings_screen_id, $this->settings_page_id );
					?>
					 ...
					 * @param string $setting_page_id   Settings page ID.
					 */
					do_action( 'ebox_settings_page_after_title', $this->settings_screen_id, $this->settings_page_id );
					?>
					 ...
					 * @param string $setting_page_id   Settings page ID.
					 */
					do_action( 'ebox_settings_page_before_form', $this->settings_screen_id, $this->settings_page_id );
					?>
					 ...
					 * @param string $setting_page_id   Settings page ID.
					 */
					do_action( 'ebox_settings_page_inside_form_top', $this->settings_screen_id, $this->settings_page_id );
					?>
						...
						<div id="poststuff">
							<div id="post-body" class="metabox-holder columns-<?php echo 1 == get_current_screen()->get_columns() ? '1' : '2'; ?>">
								...
								<div id="postbox-container-2" class="postbox-container">
									 ...
									 * @param string $setting_page_id   Settings page ID.
									 */
									do_action( 'ebox_settings_page_before_metaboxes', $this->settings_screen_id, $this->settings_page_id );
									?>
									 ...
									 * @param string $setting_page_id   Settings page ID.
									 */
									do_action( 'ebox_settings_page_after_metaboxes', $this->settings_screen_id, $this->settings_page_id );
									?>
								</div>
							</div>
							<br class="clear">
					 ...
					 * @param string $setting_page_id   Settings page ID.
					 */
					do_action( 'ebox_settings_page_inside_form_bottom', $this->settings_screen_id, $this->settings_page_id );
					?>
					 ...
					 * @param string $setting_page_id   Settings page ID.
					 */
					do_action( 'ebox_settings_page_after_form', $this->settings_screen_id, $this->settings_page_id );
					?>
				</div>
				<?php
				...
				<div class="wrap ebox-settings-page-wrap">
					...
					?>
					<?php submit_button( esc_html__( 'Save Changes', 'ebox' ) ); ?>
					<?php echo $this->get_admin_page_form( false ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML. ?>
				</div>
				<?php
			}
		...
		public function get_admin_page_title() {
			 ...
			 */
			if ( true === apply_filters( 'ebox_admin_page_title_should_display', false ) ) {
				/**
				 * Filters admin settings page title HTML output.
				 *
				 * @param string $title_output The admin settings title page.
				 */
				return apply_filters( 'ebox_admin_page_title', '<h1>' . esc_html( get_admin_page_title() ) . '</h1>' );
			} else {
				return '<h1 class="ebox-empty-page-title"></h1>';
			}
		}
		...
		public function get_admin_page_form( $start = true ) {
			if ( true === $this->settings_form_wrap ) {
				if ( true === $start ) {
					 ...
					 */
					return apply_filters( 'ebox_admin_page_form', '<form id="ebox-settings-page-form" method="post" action="options.php">', $start );
				} else {
					/** This filter is documented in includes/settings/class-ld-settings-pages.php  */
					return apply_filters( 'ebox_admin_page_form', '</form>', $start );
				}
			}
			return '';
		}
		...
		public function get_settings_sections() {
			if ( is_null( $this->page_sections ) ) {
				$this->page_sections = ebox_Settings_Section::get_all_sections_by( 'settings_page_id', $this->settings_page_id );
			}
		...
		public function show_settings_sections_sub_menu() {
			if ( true === $this->settings_metabox_as_sub ) {
				$page_sections = ebox_Settings_Section::get_all_sections_by( 'settings_page_id', $this->settings_page_id, null, 'settings_section_key' );
				...
				if ( ! empty( $q_links ) ) {
					echo '<div id="ebox-settings-sub-wrap"  class="wrap">';
					echo '<ul id="ebox-settings-sub-advanced" class="ebox-settings-sub subsubsub">';
					...
					if ( ! empty( $q_links ) ) {
						...
						foreach ( $q_links as $q_link_key => $q_link ) {
							...
							if ( $current_sub === $q_link_key ) {
								$q_link_class = ' class="ebox-settings-sub-current" ';
							}
		 ...
		 *
		 * Called from filter `ebox_show_metabox` to check if metabox should be shown
		...
		public function filter_option_page_capability( $capability ) {
			if ( defined( 'ebox_ADMIN_CAPABILITY_CHECK' ) && ebox_ADMIN_CAPABILITY_CHECK ) {
				$capability = ebox_ADMIN_CAPABILITY_CHECK;
			}
 ...
 * @since 3.0.0
 *
 * @global array $ebox_assets_loaded An array of loaded assets.
 */
function ebox_admin_settings_page_assets() {
	global $ebox_assets_loaded;
	...
	if ( ! isset( $ebox_assets_loaded['styles']['ebox-admin-settings-page'] ) ) {
		wp_enqueue_style(
			'ebox-admin-settings-page',
			ebox_LMS_PLUGIN_URL . 'assets/css/ebox-admin-settings-page' . ebox_min_asset() . '.css',
			array(),
			ebox_SCRIPT_VERSION_TOKEN
		);
		wp_style_add_data( 'ebox-admin-settings-page', 'rtl', 'replace' );
		$ebox_assets_loaded['styles']['ebox-admin-settings-page'] = __FUNCTION__;
	}

	if ( ! isset( $ebox_assets_loaded['scripts']['ebox-admin-settings-page'] ) ) {
		wp_enqueue_script(
			'ebox-admin-settings-page',
			ebox_LMS_PLUGIN_URL . 'assets/js/ebox-admin-settings-page' . ebox_min_asset() . '.js',
			array( 'jquery', 'wp-color-picker' ),
			ebox_SCRIPT_VERSION_TOKEN,
			true
		);
		$ebox_assets_loaded['scripts']['ebox-admin-settings-page'] = __FUNCTION__;
		...
		if ( ! isset( $script_data['admin_notice_settings_fields_errors'] ) ) {
			$script_data['admin_notice_settings_fields_errors_container'] = '<div id="ebox-settings-fields-notice-errors" class="ebox-settings-fields-notice-errors notice notice-error"><p class="errors-header">' . esc_html__( 'You have errors on the following settings', 'ebox' ) . '</p><ul class="errors-list"></ul></div>';
		}

		if ( ! isset( $script_data['selec2_language'] ) ) {
			$script_data['selec2_language'] = array(
				'loadingMore'    => esc_html__( 'Loading more results…', 'ebox' ),
				'noResults'      => esc_html__( 'No results found', 'ebox' ),
				'searching'      => esc_html__( 'Searching…', 'ebox' ),
				'removeAllItems' => esc_html__( 'Remove all items', 'ebox' ),
				'removeItem'     => esc_html__( 'Remove item', 'ebox' ),
			);
		}
		...
		if ( ! isset( $script_data['settings_fields_errors'] ) ) {
			$script_data['settings_fields_errors'] = array(
				'number' => array(
					'invalid'  => esc_html__( 'Entered value is invalid.', 'ebox' ),
					'empty'    => esc_html__( 'Value cannot be empty.', 'ebox' ),
					'negative' => esc_html__( 'Entered value cannot be negative.', 'ebox' ),
					'decimal'  => esc_html__( 'Entered value cannot contain decimal.', 'ebox' ),
					'minimum'  => esc_html__( 'Entered value less than minimum.', 'ebox' ),
					'maximum'  => esc_html__( 'Entered value greater than maximum.', 'ebox' ),
				),
			);
		}
		 ...
		 */
		$script_data = apply_filters( 'ebox_admin_settings_data', $script_data );
		if ( ( empty( $script_data ) ) || ( ! is_array( $script_data ) ) ) {
			$script_data = array();
		}

		$script_data = array( 'json' => wp_json_encode( $script_data ) );
		wp_localize_script( 'ebox-admin-settings-page', 'ebox_admin_settings_data', $script_data );
	}
}

add_action(
	'wp_ajax_ebox_settings_select2_query',
	function() {
		$result = array(
			'items'       => array(),
			'total_items' => 0,
		...
		if ( is_a( $query_results, 'WP_Query' ) ) {
			...
			if ( ! empty( $include_selected ) && 1 === $query_args['paged'] ) {
				$title = sprintf(
					...
					esc_html_x( '%s (current selection)', 'placeholder: post title', 'ebox' ),
			...
			if (
				...
				foreach ( $query_results->posts as $item ) {
					$result['items'][] = array(
						'id'   => $item->ID,
						'text' => html_entity_decode( ebox_format_step_post_title_with_status_label( $item ) ),
					);
				}
			}
