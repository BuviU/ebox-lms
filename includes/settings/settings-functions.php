...
/**
 * Gets the ebox setting for a post.
 *
 * @since 2.1.0
 *
 ...
 */
function ebox_get_setting( $post, $setting = null ) {
	...
	if ( is_a( $post, 'WP_Post' ) ) {
		...
		if ( 'lesson' === $setting ) {
			return ebox_get_lesson_id( $post->ID );
		}

		if ( 'course' === $setting ) {
			return ebox_get_course_id( $post->ID );
		}

		$post_type_prefix = ebox_get_post_type_key( $post->post_type );
		if ( in_array( $setting, array( $post_type_prefix . '_price_billing_p3', $post_type_prefix . '_price_billing_t3', $post_type_prefix . '_trial_duration_t1', $post_type_prefix . '_trial_duration_p1' ), true ) ) {
			...
			if ( ! empty( $price_billing_t3 ) ) {
				$price_billing_t3_new = ebox_billing_cycle_field_frequency_validate( $price_billing_t3 );
				...
				if ( ! empty( $price_billing_p3 ) ) {
					$price_billing_p3_new = ebox_billing_cycle_field_interval_validate( $price_billing_p3, $price_billing_t3 );
			...
			if ( ! empty( $trial_duration_t1 ) ) {
				$trial_duration_t1_new = ebox_billing_cycle_field_frequency_validate( $trial_duration_t1 );
				...
				if ( ! empty( $trial_duration_p1 ) ) {
					$trial_duration_p1_new = ebox_billing_cycle_field_interval_validate( $trial_duration_p1, $trial_duration_t1 );
...
/**
 * Updates the ebox setting for a post.
 *
 * @since 2.1.0
 *
 * @param int|WP_Post $post    The `WP_Post` object or Post ID.
 ...
 */
function ebox_update_setting( $post, $setting, $value ) {
	if ( empty( $setting ) ) {
		return false;
	}
	...
	if ( is_a( $post, 'WP_Post' ) ) {
		...
		if ( 'course' === $setting ) {
			...
			$value = ebox_convert_course_access_list( $value );
			update_post_meta( $post->ID, 'course_access_list', $value );

		} elseif ( 'course_points' === $setting ) {
			$course_points = ebox_format_course_points( $value );
		...
		} elseif ( 'team_price_type' === $setting ) {
			update_post_meta( $post->ID, '_ld_price_type', $value );
		} elseif ( 'team_price_billing_t3' === $setting ) {
			if ( ! empty( $value ) ) {
				update_post_meta( $post->ID, 'team_price_billing_t3', $value );
			} else {
				delete_post_meta( $post->ID, 'team_price_billing_t3' );
			}
		} elseif ( 'team_price_billing_p3' === $setting ) {
			if ( ! empty( $value ) ) {
				update_post_meta( $post->ID, 'team_price_billing_p3', $value );
			} else {
				delete_post_meta( $post->ID, 'team_price_billing_p3' );
			}
		...
		} elseif ( 'team_trial_duration_t1' === $setting ) {
			if ( ! empty( $value ) ) {
				update_post_meta( $post->ID, 'team_trial_duration_t1', $value );
			} else {
				delete_post_meta( $post->ID, 'team_trial_duration_t1' );
			}
		} elseif ( 'team_trial_duration_p1' === $setting ) {
			if ( ! empty( $value ) ) {
				update_post_meta( $post->ID, 'team_trial_duration_p1', $value );
			} else {
				delete_post_meta( $post->ID, 'team_trial_duration_p1' );
			}
			...
			$value = intval( $value );
			if ( ! empty( $value ) ) {
				ebox_update_course_exam_challenge( $post->ID, $value, false );
			} else {
				ebox_update_course_exam_challenge( $post->ID, $value, true );
			}
			...
			in_array(
				...
				array(
					...
					ebox_COUPON_META_KEY_TYPE,
					ebox_COUPON_META_KEY_REDEMPTIONS,
					ebox_COUPON_META_KEY_START_DATE,
					ebox_COUPON_META_KEY_END_DATE,
					ebox_COUPON_META_KEY_PREFIX_APPLY_TO_ALL . 'courses',
					ebox_COUPON_META_KEY_PREFIX_APPLY_TO_ALL . 'teams',
				),
				true
			)
		) {
