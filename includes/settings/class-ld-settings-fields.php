<?php
/**
 * ebox Settings Fields API.
 *
 * @since 3.0.0
 * @package ebox\Settings\Fields
 */
...
if ( ! class_exists( 'ebox_Settings_Fields' ) ) {
	...
	class ebox_Settings_Fields {
		...
		public static function show_section_field_row( $field ) {
				...
				if ( ! empty( $settings_errors ) ) {
					foreach ( $settings_errors as $settings_error ) {
						if ( ( $settings_error['setting'] == $field['args']['setting_option_key'] ) && ( $settings_error['code'] == $field['args']['name'] ) && ( 'error' == $settings_error['type'] ) ) {
							$field_error_class = 'ebox-settings-field-error';
						}
					}
				}
			}
			...
			if ( ( isset( $field['args']['type'] ) ) && ( ! empty( $field['args']['type'] ) ) ) {
				...
				$field_class = 'ebox_input_type_' . $field['args']['type'];
			}
			...
			if ( ( isset( $field['args']['type'] ) ) && ( 'hidden' !== $field['args']['type'] ) ) {
				 ...
				 */
				$output = apply_filters( 'ebox_settings_row_outside_before', '', $field['args'] );
				if ( ! empty( $output ) ) {
					echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
				}
				?>
				<div id="<?php echo esc_attr( $field['args']['id'] ); ?>_field" class="ebox_input <?php echo esc_attr( $field_class ); ?> <?php echo esc_attr( $field_error_class ); ?>">
					<?php
						/**
						 ...
						 * @param array  $field_arguments An array of setting field arguments.
						 */
						$output = apply_filters( 'ebox_settings_row_inside_before', '', $field['args'] );
					if ( ! empty( $output ) ) {
						echo $output;// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
					};
					?>
					...
					if ( ( isset( $field['args']['row_description_before'] ) ) && ( ! empty( $field['args']['row_description_before'] ) ) ) {
						echo '<span class="ebox_row_description ebox_row_description_before">' . esc_html( $field['args']['row_description_before'] ) . '</span>';
					}
					?>
					<?php if ( ( ! isset( $field['args']['label_none'] ) ) || ( true !== $field['args']['label_none'] ) ) { ?>
						 ...
						 * @param array  $field_arguments An array of setting field arguments.
						 */
						$output = apply_filters( 'ebox_settings_row_label_outside_before', '', $field['args'] );
						if ( ! empty( $output ) ) {
							echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
						}
						?>
						<span class="ebox_option_label
						<?php
						if ( ( isset( $field['args']['label_full'] ) ) && ( true === $field['args']['label_full'] ) ) {
							echo ' ebox_option_label_full';
						}
						?>
							">
							<?php
							 ...
							 * @param array  $field_arguments An array of setting field arguments.
							 */
								$output = apply_filters( 'ebox_settings_row_label_inside_before', '', $field['args'] );
							if ( ! empty( $output ) ) {
								echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
							};
							?>
							<a class="ebox_help_text_link"
								<?php if ( ( isset( $field['args']['help_text'] ) ) && ( ! empty( $field['args']['help_text'] ) ) ) { ?>
									style="cursor:pointer;" title="<?php esc_html_e( 'Click for Help!', 'ebox' ); ?>"
									onclick="toggleVisibility('<?php echo esc_attr( $field['args']['id'] ); ?>_tip');"
								<?php } ?>
								>
								<?php if ( ( isset( $field['args']['help_text'] ) ) && ( ! empty( $field['args']['help_text'] ) ) ) { ?>
									<img alt="" src="<?php echo esc_url( ebox_LMS_PLUGIN_URL ); ?>assets/images/question.png" />
								<?php } ?>

								<label for="<?php echo esc_attr( $field['args']['label_for'] ); ?>" class="ebox_label">
								<?php
								...
								if ( isset( $field['args']['required'] ) ) {
									?>
									<span class="ebox_required_field"><abbr title="<?php esc_html_e( 'Required', 'ebox' ); ?>">*</abbr></span>
									<?php
								}
								?>
								</label>
							...
							if ( ( isset( $field['args']['help_text'] ) ) && ( ! empty( $field['args']['help_text'] ) ) ) {
								...
								?>
								<div id="<?php echo esc_attr( $field['args']['id'] ); ?>_tip" class="ebox_help_text_div" <?php echo $help_style; ?>> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $help_style hardcoded above. ?>
									<label class="ebox_help_text"><?php echo wp_kses_post( $field['args']['help_text'] ); ?></label>
								</div>
								<?php
							}
							?>
							<?php
								 ...
								 * @param array  $field_arguments An array of setting field arguments.
								 */
								$output = apply_filters( 'ebox_settings_row_label_inside_after', '', $field['args'] );
							if ( ! empty( $output ) ) {
								echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
							};
							?>
							...
							<?php
								 ...
								 * @param array  $field_arguments An array of setting field arguments.
								 */
								$output = apply_filters( 'ebox_settings_row_label_outside_after', '', $field['args'] );
							if ( ! empty( $output ) ) {
								echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
							};
							?>
					...
					<?php
						 ...
						 * @param array  $field_arguments An array of setting field arguments.
						 */
						$output = apply_filters( 'ebox_settings_row_input_outside_before', '', $field['args'] );
					if ( ! empty( $output ) ) {
						echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
					};
					?>
					<span class="ebox_option_input
					<?php
					if ( ( isset( $field['args']['input_full'] ) ) && ( true === $field['args']['input_full'] ) ) {
						echo ' ebox_option_input_full';
					}
					?>
						">
						<?php
							 ...
							 * @param array  $field_arguments An array of setting field arguments.
							 */
							$output = apply_filters( 'ebox_settings_row_input_inside_before', '', $field['args'] );
						if ( ! empty( $output ) ) {
							echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
						}
						...
						if ( ( ! isset( $field['args']['input_show'] ) ) || ( true === $field['args']['input_show'] ) ) {
							...
							<div class="ebox_option_div">
								...
								if ( isset( $field['args']['input_note'] ) && ! empty( $field['args']['input_note'] ) ) {
									?>
									<span class="ebox_option_input_note"><?php echo wp_kses_post( $field['args']['input_note'] ); ?></span>
									<?php
								}
								?>
							</div>
						 ...
						 * @param array  $field_arguments An array of setting field arguments.
						 */
						$output = apply_filters( 'ebox_settings_row_input_inside_after', '', $field['args'] );
						if ( ! empty( $output ) ) {
							echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
						};
						?>
					...
					<?php
						 ...
						 * @param array  $field_arguments An array of setting field arguments.
						 */
						$output = apply_filters( 'ebox_settings_row_input_outside_after', '<p class="ld-clear"></p>', $field['args'] );
					if ( ! empty( $output ) ) {
						echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
					};
					?>
					...
					if ( ( isset( $field['args']['row_description_after'] ) ) && ( ! empty( $field['args']['row_description_after'] ) ) ) {
						echo '<span class="ebox_row_description ebox_row_description_after">' . esc_html( $field['args']['row_description_after'] ) . '</span>';
					}
					?>
					 ...
					 * @param array  $field_arguments An array of setting field arguments.
					 */
					$output = apply_filters( 'ebox_settings_row_inside_after', '', $field['args'] );
					if ( ! empty( $output ) ) {
						echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
					}
					?>
				 ...
				 * @param array  $field_arguments An array of setting field arguments.
				 */
				$output = apply_filters( 'ebox_settings_row_outside_after', '', $field['args'] );
				if ( ! empty( $output ) ) {
					echo $output; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML.
				}
			} else {
		...
		public function get_field_attribute_class( $field_args = array(), $wrap = true ) {
			...
			$field_attribute .= 'ebox-section-field ebox-section-field-' . $this->field_type;
		...
		public function get_field_error_message( $field_args = array() ) {
			...
			if ( ( isset( $field_args['input_error'] ) ) && ( ! empty( $field_args['input_error'] ) ) ) {
				$field_attribute .= '<div class="ebox-section-field-error" style="display:none;">' . $field_args['input_error'] . '</div>';
			}
