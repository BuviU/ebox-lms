...
if ( ( class_exists( 'ebox_Settings_Fields' ) ) && ( ! class_exists( 'ebox_Settings_Fields_Checkbox' ) ) ) {

	/**
	 * Class ebox Checkbox Settings Field.
	 *
	 * @since 3.0.0
	 * @uses ebox_Settings_Fields
	 */
	class ebox_Settings_Fields_Checkbox extends ebox_Settings_Fields {
		...
		public function create_section_field( $field_args = array() ) {
			...
			if ( ( isset( $field_args['options'] ) ) && ( ! empty( $field_args['options'] ) ) ) {
				...
				foreach ( $field_args['options'] as $option_key => $option_label ) {
					...
					$html .= '<p class="ebox-section-field-checkbox-p">';
...
add_action(
	...
	function() {
		ebox_Settings_Fields_Checkbox::add_field_instance( 'checkbox' );
	}
);
