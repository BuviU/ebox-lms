...
if ( ( class_exists( 'ebox_Settings_Fields' ) ) && ( ! class_exists( 'ebox_Settings_Fields_Select' ) ) ) {
	/**
	 * ebox Select Settings Field.
	 *
	 * @since 3.0.0
	 ...
	 */
	class ebox_Settings_Fields_Select extends ebox_Settings_Fields {
			...
			if ( ( isset( $field_args['options'] ) ) && ( ! empty( $field_args['options'] ) ) ) {
				foreach ( $field_args['options'] as $option_key => $option_label ) {
					...
					if ( is_array( $option_label ) ) {
						...
						// Support for <optteam> within the select.
						if ( ( isset( $option_label['optteam_options'] ) ) && ( ! empty( $option_label['optteam_options'] ) ) ) {

							$html .= '<optteam ';
							if ( ( isset( $option_label['optteam_label'] ) ) && ( ! empty( $option_label['optteam_label'] ) ) ) {
								$html .= ' label="' . esc_html( $option_label['optteam_label'] ) . '" ';
							}
							$html .= '>';

							foreach ( $option_label['optteam_options'] as $optteam_option_key => $optteam_option_label ) {
								$html .= '<option value="' . esc_attr( $optteam_option_key ) . '" ' . $selected_item . '>' . wp_kses_post( $optteam_option_label ) . '</option>';
							}
							$html .= '</optteam>';
						} else {
							/**
							...
							if ( ( isset( $option_label['inline_fields'] ) ) && ( ! empty( $option_label['inline_fields'] ) ) ) {
								foreach ( $option_label['inline_fields'] as $sub_field_key => $sub_fields ) {
									...
									$html_sub_fields .= ebox_ob_get_clean( $level );
									$html_sub_fields .= '</div>';
								}
							}
						}
...
add_action(
	...
	function() {
		ebox_Settings_Fields_Select::add_field_instance( 'select' );
	}
);
