<?php
/**
 * ebox Input Text Settings Field.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Fields' ) ) && ( ! class_exists( 'ebox_Settings_Fields_Text' ) ) ) {

	/**
	 * Class ebox Input Text Settings Field.
	 *
	 * @since 3.0.0
	 * @uses ebox_Settings_Fields
	 */
	class ebox_Settings_Fields_Text extends ebox_Settings_Fields {
		...
		public function create_section_field( $field_args = array() ) {
			...
			if ( ( isset( $field_args['value_prefix'] ) ) && ( ! empty( $field_args['value_prefix'] ) ) ) {
				$html .= '<span class="ebox_settings_field_text_prefix ' . $this->get_field_attribute_class( $field_args, false ) . '">' . $field_args['value_prefix'];
...
add_action(
	...
	function() {
		ebox_Settings_Fields_Text::add_field_instance( 'text' );
	}
);
