<?php
/**
 * ebox Timer Entry Settings Field.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Fields' ) ) && ( ! class_exists( 'ebox_Settings_Fields_Timer_Entry' ) ) ) {

	/**
	 * Class ebox Timer Entry Settings Field.
	 *
	 * @since 3.0.0
	 * @uses ebox_Settings_Fields
	 */
	class ebox_Settings_Fields_Timer_Entry extends ebox_Settings_Fields {
		...
		public function create_section_field( $field_args = array() ) {
			...
			if ( ( isset( $field_args['value'] ) ) && ( ! empty( $field_args['value'] ) ) ) {
				$field_args['value'] = ebox_convert_lesson_time_time( $field_args['value'] );
			...
			$minute_field = '<span class="screen-reader-text">' . esc_html__( 'Minute', 'ebox' ) . '</span><input type="number" min="0" max="59" placeholder="MM" class="ld_date_mn ' . $field_class . '" name="' . $field_name . '[mn]" value="' . $value_mn . '" size="2" maxlength="2" autocomplete="off" />';

			$second_field = '<span class="screen-reader-text">' . esc_html__( 'Seconds', 'ebox' ) . '</span><input type="number" min="0" max="59" placeholder="SS" class="ld_date_ss ' . $field_class . '" name="' . $field_name . '[ss]" value="' . $value_ss . '" size="2" maxlength="2" autocomplete="off" />';

			$html .= '<div class="ld_timer_selector">' . sprintf(
				// translators: placeholders: Hour, Minute, Second.
				esc_html__( '%1$s:%2$s:%3$s', 'ebox' ),
				$hour_field,
				$minute_field,
				$second_field
			) . '</div>';
		...
		public function rest_value_to_field_value( $val = '', $key = '', $field_args = array() ) {
			...
			if ( ! empty( $val ) ) {
				$val      = ebox_convert_lesson_time_time( $val );
...
add_action(
	...
	function() {
		ebox_Settings_Fields_Timer_Entry::add_field_instance( 'timer-entry' );
	}
);
