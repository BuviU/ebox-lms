<?php
/**
 * ebox Quiz Load Templates Settings Field.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Fields' ) ) && ( ! class_exists( 'ebox_Settings_Fields_Quiz_Templates_Load' ) ) ) {
	/**
	 * Class ebox Quiz Load Templates Settings Field.
	 *
	 * @since 3.0.0
	 * @uses ebox_Settings_Fields
	 */
	class ebox_Settings_Fields_Quiz_Templates_Load extends ebox_Settings_Fields {
		...
		public function create_section_field( $field_args = array() ) {
			...
			if ( isset( $field_args['template_type'] ) ) {
				...
				global $post_type;
				if ( ebox_get_post_type_slug( 'quiz' ) === $post_type ) {
					$template_type = WpProQuiz_Model_Template::TEMPLATE_TYPE_QUIZ;
				} elseif ( ebox_get_post_type_slug( 'question' ) === $post_type ) {
			...
			$html .= '<div class="ld-settings-info-banner ld-settings-info-banner-alert">' . wpautop(
				sprintf(
					...
					esc_html_x( 'Loading a template into this %s will replace ALL existing settings.', 'placeholders: Quiz.', 'ebox' ),
			...
			if ( ( isset( $_GET['post'] ) ) && ( ! empty( $_GET['post'] ) ) && ( isset( $_GET['templateLoadId'] ) ) && ( ! empty( $_GET['templateLoadId'] ) ) ) {
				...
				$template_url = add_query_arg( 'currentTab', ebox_get_post_type_slug( 'quiz' ) . '-settings', $template_url );
				...
				if ( ebox_use_select2_lib() ) {
					$html .= '<option value="-1">' . esc_html__( 'Search or select a template…', 'ebox' ) . '</option>';
				} else {
					$html .= '<option value="">' . esc_html__( 'Select a Template to load', 'ebox' ) . '</option>';
				}
			}
			...
			if ( 'yes' !== ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) ) {
				...
				$html .= '<p><label for="templateload-option-replace-course"><input type="checkbox" id="templateload-option-replace-course" name="templateLoadReplaceCourse" />' . sprintf(
					...
					esc_html_x( 'Replace associated steps (%1$s, %2$s, or %3$s) with values from template.', 'placeholders: course, modules, topics.', 'ebox' ),
			...
			$html .= '<input type="submit" name="templateLoad" value="' . esc_html__( 'load template', 'ebox' ) . '" class="button-primary"></p>';
...
add_action(
	...
	function() {
		ebox_Settings_Fields_Quiz_Templates_Load::add_field_instance( 'quiz-templates-load' );
	}
);
