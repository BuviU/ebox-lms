<?php
/**
 * ebox Media Upload Settings Field.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Fields' ) ) && ( ! class_exists( 'ebox_Settings_Fields_Media_Upload' ) ) ) {

	/**
	 * Class ebox Media Upload Settings Field.
	 *
	 * @since 3.0.0
	 * @uses ebox_Settings_Fields
	 */
	class ebox_Settings_Fields_Media_Upload extends ebox_Settings_Fields {
		...
		public function create_section_field( $field_args = array() ) {
			...
			$html .= '<div class="ebox-section-field-media-upload_wrapper" ';
			$html .= ' id="' . esc_attr( $this->get_field_attribute_id( $field_args, false ) ) . '_wrapper" ';
			$html .= '>';

			$default_img_url = ebox_LMS_PLUGIN_URL . 'assets/images/nologo.jpg'; // cspell:disable-line.
			...
			$html .= '</div>';
			$html .= '<input type="button" class="button image-remove-button" title="' . esc_html__( 'remove image', 'ebox' ) . '" value="' . esc_html_x( 'X', 'placeholder: clear image', 'ebox' ) . '" />';
			$html .= '<input type="button" class="button image-upload-button" title="' . esc_html__( 'Select/upload image', 'ebox' ) . '"  value="' . esc_html__( 'Select image', 'ebox' ) . '" />';
			$html .= '<input ';
			$html .= ' type="hidden" ';
...
add_action(
	...
	function() {
		ebox_Settings_Fields_Media_Upload::add_field_instance( 'media-upload' );
	}
);
