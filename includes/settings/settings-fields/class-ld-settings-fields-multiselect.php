<?php
/**
 * ebox Multiselect Settings Field.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Fields' ) ) && ( ! class_exists( 'ebox_Settings_Fields_Multiselect' ) ) ) {
	/**
	 * Class ebox Multiselect Settings Field.
	 *
	 * @since 3.0.0
	 * @uses ebox_Settings_Fields
	 */
	class ebox_Settings_Fields_Multiselect extends ebox_Settings_Fields {
		...
		public function create_section_field( $field_args = array() ) {
			...
			if ( ( isset( $field_args['options'] ) ) && ( ! empty( $field_args['options'] ) ) ) {
				foreach ( $field_args['options'] as $option_key => $option_label ) {
					if ( ( '' === $option_key ) && ( ebox_use_select2_lib() ) ) {
						continue;
					}
					$selected_item = '';
...
add_action(
	...
	function() {
		ebox_Settings_Fields_Multiselect::add_field_instance( 'multiselect' );
	}
);
