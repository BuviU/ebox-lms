<?php
/**
 * ebox Settings Metabox Abstract Class.
 *
...
if ( ! class_exists( 'ebox_Settings_Metabox' ) ) {
	/**
	 * Class for ebox Settings Sections.
	 */
	class ebox_Settings_Metabox {
		...
		public function load_settings_values() {
			...
			$setting_values = ebox_get_setting( $this->_post );
		...
		public function load_settings_field( $setting_option_field = array() ) {
			...
			$field_ref = ebox_Settings_Fields::get_field_instance( $setting_option_field['type'] );
			if ( ! $field_ref ) {
				return $setting_option_field;
			}
		...
		public function add_meta_boxes( $settings_screen_id = '' ) {
			...
			if ( $settings_screen_id === $this->settings_screen_id ) {
				 ...
				 * @param string  $settings_screen_id Settings screen ID.
				 */
				if ( apply_filters( 'ebox_show_metabox', true, $this->settings_metabox_key, $this->settings_screen_id ) ) {

					if ( ! isset( $ebox_metaboxes[ $this->settings_screen_id ] ) ) {
						$ebox_metaboxes[ $this->settings_screen_id ] = array();
					}
					$ebox_metaboxes[ $this->settings_screen_id ][ $this->settings_metabox_key ] = $this->settings_metabox_key;
		...
		public function save_post_meta_box( $post_id = 0, $saved_post = null, $update = null, $settings_field_updates = null ) {
			...
			if ( ( ! empty( $settings_field_updates ) ) && ( is_array( $settings_field_updates ) ) ) {
				foreach ( $settings_field_updates as $_key => $_val ) {
					ebox_update_setting( $saved_post, $_key, $_val );
				}
			}
		}
		...
		public function trigger_metabox_settings_post_filters( $settings_field_updates = array() ) {
			 ...
			 * @param string $settings_screen_id    Settings screen ID.
			 */
			$settings_field_updates = apply_filters( 'ebox_metabox_save_fields', $settings_field_updates, $this->settings_metabox_key, $this->settings_screen_id );
			/**
			 * Filters settings meta box save fields.
			 *
			 ...
			 * @param string $settings_screen_id    Settings screen ID.
			 */
			$settings_field_updates = apply_filters( 'ebox_metabox_save_fields_' . $this->settings_metabox_key, $settings_field_updates, $this->settings_metabox_key, $this->settings_screen_id );
		...
		public function show_settings_metabox( $metabox = null ) {
			if ( ( $metabox ) && ( is_object( $metabox ) ) && ( isset( self::$_instances[ get_class( $metabox ) ] ) ) ) {
				...
				if ( ( isset( $metabox->settings_fields_callback ) ) && ( ! empty( $metabox->settings_fields_callback ) ) && ( is_callable( $metabox->settings_fields_callback ) ) ) {
					 ...
					 * @param string $settings_key Settings key used as option name while saving settings.
					 */
					do_action( 'ebox_metabox_description_before', $metabox->settings_metabox_key );
					 ...
					 * @param string $settings_key Settings key used as option name while saving settings.
					 */
					do_action( 'ebox_metabox_description_after', $metabox->settings_metabox_key );

					if ( ebox_get_post_type_slug( 'quiz' ) === $this->settings_screen_id ) {
					 ...
					 * @param string $settings_key Settings key used as option name while saving settings.
					 */
					do_action( 'ebox_metabox_options_div_before', $metabox->settings_metabox_key );
					echo '<div class="ebox ebox_options ' . esc_attr( $metabox->settings_metabox_key ) . '">';
					 ...
					 * @param string $settings_key Settings key used as option name while saving settings.
					 */
					do_action( 'ebox_metabox_options_div_inside_top', $metabox->settings_metabox_key );
					$this->show_settings_metabox_fields( $metabox );

					/**
					 * Fires inside the meta box options div at the bottom.
					 */
					do_action( 'ebox_metabox_options_div_inside_bottom', $metabox->settings_metabox_key );
					echo '</div>';
					 ...
					 * @param string $settings_key Settings key used as option name while saving settings.
					 */
					do_action( 'ebox_metabox_options_div_after', $metabox->settings_metabox_key );
		...
		protected function show_settings_metabox_fields( $metabox = null ) {
			if ( $metabox ) {
				ebox_Settings_Fields::show_section_fields( $metabox->setting_option_fields );
			}
		}
		...
		public function init_quiz_edit( $post, $reload_pro_quiz = false ) {
			...
			if ( ( $post ) && ( is_a( $post, 'WP_Post' ) ) ) {
				...
				$pro_quiz_id = absint( ebox_get_setting( $post->ID, 'quiz_pro' ) );
					...
					if ( ( isset( $_GET['templateLoadId'] ) ) && ( ! empty( $_GET['templateLoadId'] ) ) ) {
						...
						if ( ! empty( $template_id ) ) {
							...
							if ( null !== $data ) {
								...
								if ( isset( $data[ '_' . ebox_get_post_type_slug( 'quiz' ) ] ) ) {
									$quiz_postmeta = $data[ '_' . ebox_get_post_type_slug( 'quiz' ) ];
									// phpcs:ignore WordPress.Security.NonceVerification.Recommended
									if ( ( ! isset( $_GET['templateLoadReplaceCourse'] ) ) || ( 'on' !== $_GET['templateLoadReplaceCourse'] ) ) {
										if ( isset( $quiz_postmeta['course'] ) ) {
											$quiz_postmeta['course'] = absint( ebox_get_setting( $post->ID, 'course' ) );
										}
										if ( isset( $quiz_postmeta['lesson'] ) ) {
											$quiz_postmeta['lesson'] = absint( ebox_get_setting( $post->ID, 'lesson' ) );
										}
									}
									...
									if ( ( ! isset( $_GET['templateLoadReplaceQuestions'] ) ) || ( 'on' !== $_GET['templateLoadReplaceQuestions'] ) ) {
										if ( isset( $quiz_postmeta['quiz_pro'] ) ) {
											$quiz_postmeta['quiz_pro'] = absint( ebox_get_setting( $post->ID, 'quiz_pro' ) );
										}
									}
									$data[ '_' . ebox_get_post_type_slug( 'quiz' ) ] = $quiz_postmeta;

									foreach ( $this->settings_fields_map as $_internal => $_external ) {
										if ( isset( $data[ '_' . ebox_get_post_type_slug( 'quiz' ) ][ $_external ] ) ) {
											$this->setting_option_values[ $_internal ] = $data[ '_' . ebox_get_post_type_slug( 'quiz' ) ][ $_external ];
										}
									}
								} else {
									$quiz_postmeta = array();
