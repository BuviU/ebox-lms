<?php
/**
 * ebox Deprecated Settings Pages Class.
 *
 * @since 3.6.0
...
if ( ! class_exists( 'ebox_Settings_Pages_Deprecated' ) ) {
	/**
	 * Class for ebox Settings Pages Deprecated.
	 *
	 * @since 3.6.0
	 */
	class ebox_Settings_Pages_Deprecated {
		 ...
		 * @param string $version          The version of ebox that deprecated the function.
		...
		public static function deprecated_settings_page( $page_slug = '', $version = '', $replacement_args = array() ) {
			...
			if ( ( ! empty( $page_slug ) ) && ( ! empty( $version ) ) ) {
				 ...
				 * @param string $version          The version of ebox that deprecated the function.
				 */
				do_action( 'ebox_deprecated_settings_page_run', $page_slug, $replacement_args, $version );
				 ...
				 */
				if ( WP_DEBUG && apply_filters( 'ebox_deprecated_settings_page_url_error', true ) ) {
					if ( function_exists( '__' ) ) {
							...
							trigger_error( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error
								wp_kses_post(
									sprintf(
										...
										__( 'ebox Settings Page %1$s is <strong>deprecated</strong> since version %2$s! Use %3$s instead.', 'ebox' ),
							...
							trigger_error( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error
								wp_kses_post(
									sprintf(
										...
										__( 'ebox Settings Page %1$s is <strong>deprecated</strong> since version %2$s with no alternative available.', 'ebox' ),
										esc_url( add_query_arg( 'page', $page_slug, admin_url( 'admin.php' ) ) ),
										$version
									)
								),
						...
						if ( $replacement_args ) {
							trigger_error( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error
								wp_kses_post(
									sprintf(
										'ebox Settings Page %1$s is <strong>deprecated</strong> since version %2$s! Use %3$s instead.',
										esc_url( add_query_arg( 'page', $page_slug, admin_url( 'admin.php' ) ) ),
										$version,
										esc_url( add_query_arg( $replacement_args, admin_url( 'admin.php' ) ) )
									)
							...
							trigger_error( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error
								wp_kses_post(
									sprintf(
										'ebox Settings Page %1$s is <strong>deprecated</strong> since version %2$s with no alternative available.',
										esc_url( add_query_arg( 'page', $page_slug, admin_url( 'admin.php' ) ) ),
										$version
									)
								),
				 ...
				 */
				$redirect_url = apply_filters( 'ebox_deprecated_settings_page_url_redirect', $redirect_url, $page_slug, $version, $replacement_args );
...
add_action(
	'ebox_admin_init',
	function() {
		ebox_Settings_Pages_Deprecated::init();
	}
);

ebox_Settings_Pages_Deprecated::register_deprecated_settings_page(
	'ebox_lms_settings_custom_labels',
	'3.6.0',
	array(
...
ebox_Settings_Pages_Deprecated::register_deprecated_settings_page(
	'ebox_lms_settings_paypal',
	'3.6.0',
	array(
		'page'             => 'ebox_lms_payments',
