<?php
/**
 * ebox Settings Metabox for Lesson Access Settings.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Lesson_Access_Settings' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Lesson Access Settings.
	 *
	 * @since 3.0.0
	 */
	class ebox_Settings_Metabox_Lesson_Access_Settings extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_metabox_key = 'ebox-lesson-access-settings';

			// Section label/header.
			$this->settings_section_label = sprintf(
				// translators: placeholder: Lesson.
				esc_html_x( '%s Access Settings', 'placeholder: Lesson', 'ebox' ),
				ebox_get_custom_label( 'lesson' )
			);

			$this->settings_section_description = sprintf(
				// translators: placeholder: modules.
				esc_html_x( 'Controls the timing and way %s can be accessed.', 'placeholder: modules', 'ebox' ),
		...
		public function load_settings_fields() {
			...
			$this->setting_option_fields = array(
				...
				'sample_lesson'   => array(
					...
					'label'   => sprintf(
						// translators: placeholder: Lesson.
						esc_html_x( 'Sample %s', 'placeholder: Lesson', 'ebox' ),
						ebox_get_custom_label( 'lesson' )
					),
					...
					'options' => array(
						'on' => sprintf(
							...
							esc_html_x( 'This %1$s is accessible to all visitors regardless of %2$s enrollment', 'placeholder: lesson, course', 'ebox' ),
				...
				'lesson_schedule' => array(
					...
					'label'   => sprintf(
						// Translators: placeholder: Lesson.
						esc_html_x( '%s Release Schedule', 'placeholder: Lesson', 'ebox' ),
						ebox_get_custom_label( 'lesson' )
					),
					'type'    => 'radio',
					...
					'options' => array(
						''                            => array(
							...
							'description' => sprintf(
								// translators: placeholder: lesson, course.
								esc_html_x( 'The %1$s is made available on %2$s enrollment.', 'placeholder: lesson, course', 'ebox' ),
								ebox_get_custom_label_lower( 'lesson' ),
								ebox_get_custom_label_lower( 'course' )
							),
						),
						'visible_after'               => array(
							...
							'description'         => sprintf(
								// translators: placeholder: lesson, course.
								esc_html_x( 'The %1$s will be available X days after %2$s enrollment.', 'placeholder: lesson, course.', 'ebox' ),
								ebox_get_custom_label_lower( 'lesson' ),
								ebox_get_custom_label_lower( 'course' )
							),
							'inline_fields'       => array(
						...
						'visible_after_specific_date' => array(
							...
							'description'         => sprintf(
								// translators: placeholder: lesson.
								esc_html_x( 'The %s will be available on a specific date.', 'placeholder: lesson', 'ebox' ),
								ebox_get_custom_label_lower( 'lesson' )
							),
							'inline_fields'       => array(
	...
	add_filter(
		...
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Lesson_Access_Settings'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Lesson_Access_Settings' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Lesson_Access_Settings'] = ebox_Settings_Metabox_Lesson_Access_Settings::add_metabox_instance();
			}
