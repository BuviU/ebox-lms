<?php
/**
 * ebox Settings Metabox for Quiz Results Page Display & Content Options.
 *
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Quiz_Results_Options' ) ) ) {
	/**
	 * Class to create the settings section.
	 */
	class ebox_Settings_Metabox_Quiz_Results_Options extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_metabox_key = 'ebox-quiz-results-options';

			// Section label/header.
			$this->settings_section_label = esc_html__( 'Results Page Display', 'ebox' );

			$this->settings_section_description = esc_html__( 'Controls how the results page will look', 'ebox' );
		...
		public function load_settings_fields() {
			...
			$this->setting_option_fields = array(
				'resultGradeEnabled'         => array(
					...
					'label'               => esc_html__( 'Result Message(s)', 'ebox' ),
					'value'               => $this->setting_option_values['resultGradeEnabled'],
					'default'             => '',
					'help_text'           => esc_html__( "When enabled, the first message will be displayed to ALL users. To customize the message based on earned score, add new Graduation Levels and set the 'From' field to the desired grade.", 'ebox' ),
					'options'             => array(
						''   => '',
						'on' => sprintf(
							// translators: placeholder: Quiz.
							esc_html_x( 'The message below is displayed on the %s results page.', 'placeholder: Quiz', 'ebox' ),
				...
				'btnRestartQuizHidden'       => array(
					...
					'label'   => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'Restart %s button', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'Quiz' )
					),
				...
				'custom_result_data_display' => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Custom Results Display', 'ebox' ),
					'value'               => $this->setting_option_values['custom_result_data_display'],
					'default'             => 'on',
					'options'             => array(
						''   => '',
						'on' => esc_html__( 'Enable the items you wish to display on the Result Page', 'ebox' ),
					),
				...
				'showAverageResult'          => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'Average Score', 'ebox' ),
					'help_text'      => sprintf(
						// translators: placeholder: quiz.
						esc_html_x( 'Display the average score of all users who took the %s', 'placeholder: quiz', 'ebox' ),
				...
				'showCategoryScore'          => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'Category Score', 'ebox' ),
					'help_text'      => sprintf(
						// translators: placeholder: Question.
						esc_html_x( 'Display the score achieved for each %s Category', 'placeholder: Question', 'ebox' ),
				...
				'hideResultPoints'           => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'Overall Score', 'ebox' ),
					'parent_setting' => 'custom_result_data_display',
					...
					'options'        => array(
						...
						''   => sprintf(
							// translators: placeholder: Quiz.
							esc_html_x( 'The achieved %s score is NOT be displayed on the Results page', 'placeholder: Quiz', 'ebox' ),
							ebox_get_custom_label( 'quiz' )
						),
					),
					'rest'           => array(
				...
				'hideResultCorrectQuestion'  => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'No. of Correct Answers', 'ebox' ),
					'parent_setting' => 'custom_result_data_display',
					...
					'options'        => array(
						...
						''   => sprintf(
							...
							esc_html_x( 'The number of correctly answered %s is NOT displayed on the Results page.', 'placeholder: Questions', 'ebox' ),
				...
				'hideResultQuizTime'         => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'Time Spent', 'ebox' ),
					'parent_setting' => 'custom_result_data_display',
				...
				'custom_answer_feedback'     => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Custom Answer Feedback', 'ebox' ),
					'help_text'           => sprintf(
						// translators: placeholder: questions.
						esc_html_x( 'Select which data users should be able to view when reviewing their submitted %s.', 'placeholder: questions', 'ebox' ),
				...
				'hideAnswerMessageBox'       => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'Correct / Incorrect Messages', 'ebox' ),
				...
				'disabledAnswerMark'         => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'Correct / Incorrect Answer Marks', 'ebox' ),
				...
				'btnViewQuestionHidden'      => array(
					...
					'label'          => sprintf(
						// translators: placeholder: Questions.
						esc_html_x( 'View %s Button', 'placeholder: Questions', 'ebox' ),
						ebox_get_custom_label( 'questions' )
					),
		...
		public function get_custom_result_messages() {
			...
			<div  id="ebox-quiz-resultList">
						...
						<div class="resultHeader">
							...
							<?php
								echo sprintf(
									...
									esc_html_x( 'From %s %% score, display this message:', 'placeholder: input form field', 'ebox' ),
							...
							<div class="expand-arrow expand-arrow-<?php echo esc_attr( $message_arrow_direction ); ?>">
								...
								<?php if ( true === $message_delete_enabled ) { ?>
									<input type="button" value="<?php esc_html_e( 'Delete graduation', 'ebox' ); ?>" class="deleteResult">
								<?php } ?>
				...
				<input type="button" class="addResult" name="addResult" id="addResult" value="<?php esc_html_e( 'Add graduation', 'ebox' ); ?>">
			</div>
			<?php
			$html .= ebox_ob_get_clean( $level );
	...
	add_filter(
		...
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Quiz_Results_Options'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Quiz_Results_Options' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Quiz_Results_Options'] = ebox_Settings_Metabox_Quiz_Results_Options::add_metabox_instance();
			}
