<?php
/**
 * ebox Settings Metabox for Quiz Progress Settings.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Quiz_Progress_Settings' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Quiz Progress Settings.
	 *
	 * @since 3.0.0
	 */
	class ebox_Settings_Metabox_Quiz_Progress_Settings extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_metabox_key = 'ebox-quiz-progress-settings';

			// Section label/header.
			$this->settings_section_label = esc_html__( 'Progression and Restriction Settings', 'ebox' );

			$this->settings_section_description = sprintf(
				// translators: placeholder: quiz.
				esc_html_x( 'Controls the requirement for accessing and completing the %s', 'placeholder: quiz', 'ebox' ),
		...
		public function load_settings_values() {
			...
			if ( true === $this->settings_values_loaded ) {
				...
				if ( ! isset( $_GET['templateLoadId'] ) ) {
					$this->setting_option_values['quiz_resume'] = ebox_get_setting( $this->_post->ID, 'quiz_resume' );
				}
				...
				if ( ( isset( $this->setting_option_values['quiz_resume_cookie_send_timer'] ) ) && ( '' !== $this->setting_option_values['quiz_resume_cookie_send_timer'] ) ) {
					...
					if ( ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_MIN > $this->setting_option_values['quiz_resume_cookie_send_timer'] ) {
						$this->setting_option_values['quiz_resume_cookie_send_timer'] = ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_MIN;
					}
				} else {
					$this->setting_option_values['quiz_resume_cookie_send_timer'] = ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_DEFAULT;
				}
		...
		public function load_settings_fields() {
			...
			$this->setting_option_fields = array(
				'passingpercentage'             => array(
					...
					'label'       => esc_html__( 'Passing Score', 'ebox' ),
					'type'        => 'number',
					...
					'rest'        => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => esc_html__( 'Passing Score Percentage', 'ebox' ),
								'type'        => 'float',
								'default'     => 0.0,
							),
						),
				...
				'certificate'                   => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( '%s Certificate', 'placeholder: Quiz', 'ebox' ),
				...
				'threshold'                     => array(
					'name'           => 'threshold',
					'label'          => esc_html__( 'Certificate Awarded for', 'ebox' ),
					'type'           => 'number',
					'default'        => '80',
					'placeholder'    => 'e.g. 80',
					'class'          => '-small',
					'help_text'      => esc_html__( 'Set the score needed to receive a certificate. This can be different from the "Passing Score".', 'ebox' ),
					'input_label'    => esc_html__( '% score', 'ebox' ),
					'attrs'          => array(
						'min' => '0',
						'max' => '100',
					),
				...
				'quiz_resume'                   => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'Enable %s Saving', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
					...
					'options'             => array(
						''   => '',
						'on' => esc_html__( 'Progress will be saved to the server', 'ebox' ),
					),
					'help_text'           => sprintf(
						// translators: placeholder: quiz, quiz.
						esc_html_x( '%1$s saving allows your users to save their current %2$s progress and return to it at a later date and preserve their progress.', 'placeholder: quiz, quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' ),
						ebox_get_custom_label( 'quiz' )
					),
				...
				'quiz_resume_cookie_send_timer' => array(
					...
					'label'          => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'Save %s data to the server every', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
					'type'           => 'number',
					'class'          => '-small',
					...
					'attrs'          => array(
						'step' => 1,
						'min'  => ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_MIN,
					),
					'rest'           => array(
						'show_in_rest' => ebox_REST_API::enabled(),
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => esc_html__( 'Save cookie data to the server every', 'ebox' ),
								'type'        => 'integer',
								'default'     => ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_DEFAULT,
							),
						),
					),
				...
				'retry_restrictions'            => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'Restrict %s Retakes', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
					'type'                => 'checkbox-switch',
					'options'             => array(
				...
				'repeats'                       => array(
					'name'           => 'repeats',
					'label'          => esc_html__( 'Number of Retries Allowed', 'ebox' ),
					'help_text'      => esc_html__( 'You must input a whole number value or leave blank to default to 0.', 'ebox' ),
					'type'           => 'number',
					'class'          => '-small',
					'default'        => '',
					...
					'rest'           => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => esc_html__( 'Number of repeats allowed. blank is unlimited, 0 is 1 repeats, 1 is 2 repeats, etc.', 'ebox' ),
								'type'        => 'string',
								'default'     => '',
							),
						),
				...
				'quizRunOnceType'               => array(
					...
					'label'          => esc_html__( 'Retries Applicable to', 'ebox' ),
					'type'           => 'select',
					'default'        => '1',
					'value'          => $this->setting_option_values['quizRunOnceType'],
					'options'        => array(
						'1' => esc_html__( 'All users', 'ebox' ),
						'2' => esc_html__( 'Registered users only', 'ebox' ),
						'3' => esc_html__( 'Anonymous user only', 'ebox' ),
					),
					'parent_setting' => 'retry_restrictions',
				),
				'quizRunOnceCookie'             => array(
					...
					'type'           => 'checkbox',
					'options'        => array(
						'on' => esc_html__( 'Use a cookie to restrict anonymous visitors', 'ebox' ),
					),
				...
				'quiz_reset_cookies'            => array(
					...
					'type'           => 'custom',
					'html'           => '<div><input class="button-secondary" type="button" name="resetQuizLock" data-nonce="' . wp_create_nonce( 'ebox-wpproquiz-reset-lock' ) . '" value="' . esc_html__( 'Reset the user identification', 'ebox' ) . '"><span id="resetLockMsg" style="display:none; background-color: rgb(255, 255, 173); border: 1px solid rgb(143, 143, 143); padding: 4px; margin-left: 5px; ">' . esc_html__( 'User identification has been reset.', 'ebox' ) . '</span><p class="description"></p></div>',
					'label'          => '',
					'help_text'      => esc_html__( 'Anonymous visitors only', 'ebox' ),
					'parent_setting' => 'retry_restrictions',
				),
				...
				'forcingQuestionSolve'          => array(
					...
					'label'   => sprintf(
						...
						esc_html_x( '%s Completion', 'placeholder: Question', 'ebox' ),
					...
					'options' => array(
						'on' => sprintf(
							...
							esc_html_x( 'All %s required to complete', 'placeholder: Questions', 'ebox' ),
				...
				'quiz_time_limit_enabled'       => array(
					...
					'label'               => esc_html__( 'Time Limit', 'ebox' ),
					'type'                => 'checkbox-switch',
					'options'             => array(
						'on' => '',
					),
					...
					'rest'                => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => esc_html__( 'Time Limit Enabled', 'ebox' ),
								'type'        => 'boolean',
								'default'     => false,
							),
						),
				...
				'timeLimit'                     => array(
					'name'           => 'timeLimit',
					'label'          => esc_html__( 'Automatically Submit After', 'ebox' ),
					'type'           => 'timer-entry',
					'class'          => 'small-text',
					'placeholder'    => esc_html__( 'e.g. 0', 'ebox' ),
					'default'        => '',
					...
					'rest'           => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => esc_html__( 'Automatically Submit After', 'ebox' ),
								'type'        => 'integer',
								'default'     => 0,
							),
						),
	...
	add_filter(
		...
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Quiz_Progress_Settings'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Quiz_Progress_Settings' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Quiz_Progress_Settings'] = ebox_Settings_Metabox_Quiz_Progress_Settings::add_metabox_instance();
			}
