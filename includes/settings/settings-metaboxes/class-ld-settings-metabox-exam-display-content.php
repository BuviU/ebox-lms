<?php
/**
 * ebox Settings Metabox for Exam Display and Content Options.
 *
 * @since 4.0.0
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Exam_Display_Content' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Exam Display and Content Options.
	 *
	 * @since 4.0.0
	 */
	class ebox_Settings_Metabox_Exam_Display_Content extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_screen_id = ebox_get_post_type_slug( 'exam' );

			// Used within the Settings API to uniquely identify this section.
			$this->settings_metabox_key = 'ebox-exam-display-content-settings';
			...
			$this->settings_section_description = sprintf(
				...
				esc_html_x( 'Controls the look and feel of the %s and optional content settings', 'placeholder: exam', 'ebox' ),
				ebox_get_custom_label_lower( 'exam' )
			);
		...
		public function load_settings_fields() {
			...
			if ( ebox_use_select2_lib() ) {
				...
				if ( ! empty( $this->setting_option_values['exam_challenge_course_show'] ) ) {
					...
					if ( ( $exam_post ) && ( is_a( $exam_post, 'WP_Post' ) ) ) {
						$select_exam_challenge_course_show_options[ $exam_post->ID ] = ebox_format_step_post_title_with_status_label( $exam_post );
					}
				}
				...
				if ( ! empty( $this->setting_option_values['exam_challenge_course_passed'] ) ) {
					...
					if ( ( $exam_post ) && ( is_a( $exam_post, 'WP_Post' ) ) ) {
						$select_exam_challenge_course_passed_options[ $exam_post->ID ] = ebox_format_step_post_title_with_status_label( $exam_post );
					}
				}
				...
				if ( ebox_use_select2_lib_ajax_fetch() ) {
					...
					$select_exam_challenge_course_show_options   = $ebox_lms->select_a_course();
					$select_exam_challenge_course_passed_options = $ebox_lms->select_a_course();
				}
			} else {
			...
			$this->setting_option_fields = array(
				'show_new_enroll'              => array(
					...
					'label'     => esc_html__( 'New enrollees only', 'ebox' ),
					'default'   => 'on',
					'help_text' => sprintf(
						// translators: placeholder: Exam, Course.
						esc_html_x( 'Show %1$s to users that have a status of "Not Started" for the %2$s.', 'placeholder: Exam, Course', 'ebox' ),
						ebox_get_custom_label_lower( 'exam' ),
				...
				'exam_challenge_course_show'   => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Exam, Course.
						esc_html_x( '%1$s %2$s Show', 'placeholder: Exam, Course', 'ebox' ),
						ebox_get_custom_label( 'exam' ),
						ebox_get_custom_label( 'course' )
					),
					'default'     => '',
					'help_text'   => sprintf(
						...
						esc_html_x( 'Select the %1$s you want to display this %2$s on.', 'placeholder: Course, Exam', 'ebox' ),
						ebox_get_custom_label_lower( 'course' ),
						ebox_get_custom_label_lower( 'exam' )
					),
				...
				'exam_challenge_course_passed' => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Exam, Course.
						esc_html_x( '%1$s Passed %2$s', 'placeholder: Exam, Course', 'ebox' ),
						ebox_get_custom_label( 'exam' ),
						ebox_get_custom_label( 'course' )
					),
					'default'     => '',
					'help_text'   => sprintf(
						...
						esc_html_x( 'Select the %1$s you want the user to complete when they successfully complete this %2$s. If different from the above setting, the user must already be enrolled in the chosen %3$s.', 'placeholder: Course, Exam, Course', 'ebox' ),
						ebox_get_custom_label_lower( 'course' ),
						ebox_get_custom_label_lower( 'exam' ),
						ebox_get_custom_label_lower( 'course' )
					),
				...
				'exam_passed_button_label'     => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( '%s Passed Button Label', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'default'   => '',
					'help_text' => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( 'Label for button when a user passes a %s', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label_lower( 'exam' )
					),
				...
				'exam_passed_redirect_url'     => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( '%s Passed Redirect URL', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'default'   => '',
					'help_text' => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( 'The URL to redirect the user to when clicking on the button after passing a %s.', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label_lower( 'exam' )
					),
				...
				'message_passed'               => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( '%s Passed Message', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'default'     => '',
					'help_text'   => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( 'Message shown when user passes the %s', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label_lower( 'exam' )
					),
					'editor_args' => array(
				...
				'exam_failed_button_label'     => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( '%s Failed Button Label', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'default'   => '',
					'help_text' => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( 'Label for button when a user fails a %s', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label_lower( 'exam' )
					),
				...
				'exam_failed_redirect_url'     => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Exam, Course.
						esc_html_x( '%s Failed Redirect URL', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'default'   => '',
					'help_text' => sprintf(
						// translators: placeholder: Course, Exam.
						esc_html_x( 'The URL to redirect the user to when clicking on the button after failing a %s.', 'placeholder: Course, Exam', 'ebox' ),
						ebox_get_custom_label_lower( 'exam' )
					),
				...
				'message_failed'               => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( '%s Failed Message', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'default'     => '',
					'help_text'   => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( 'Message shown when user fails the %s', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'editor_args' => array(
		...
		public function filter_saved_fields( $settings_values = array(), $settings_metabox_key = '', $settings_screen_id = '' ) {
			if ( ( $settings_screen_id === $this->settings_screen_id ) && ( $settings_metabox_key === $this->settings_metabox_key ) ) {
				...
				if ( (int) $settings_values['exam_challenge_course_show'] !== (int) $this->setting_option_values['exam_challenge_course_show'] ) {
					if ( ! empty( $this->setting_option_values['exam_challenge_course_show'] ) ) {
						ebox_update_setting( (int) $this->setting_option_values['exam_challenge_course_show'], 'exam_challenge', '' );
					}
					if ( ! empty( $settings_values['exam_challenge_course_show'] ) ) {
						ebox_update_setting( (int) $settings_values['exam_challenge_course_show'], 'exam_challenge', $this->_post->ID );
					}
				}
	...
	add_filter(
		'ebox_post_settings_metaboxes_init_' . ebox_get_post_type_slug( 'exam' ),
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Exam_Display_Content'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Exam_Display_Content' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Exam_Display_Content'] = ebox_Settings_Metabox_Exam_Display_Content::add_metabox_instance();
			}
