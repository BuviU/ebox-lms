<?php
/**
 * ebox Settings Metabox for Quiz Admin & Data Handling Settings.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Quiz Admin & Data Handling Settings.
	 *
	 * @since 3.0.0
	 */
	class ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_screen_id = 'ebox-quiz';

			// Used within the Settings API to uniquely identify this section.
			$this->settings_metabox_key = 'ebox-quiz-admin-data-handling-settings';

			// Section label/header.
			$this->settings_section_label = esc_html__( 'Administrative and Data Handling Settings', 'ebox' );

			$this->settings_section_description = esc_html__( 'Controls data handling options, notifications and templates.', 'ebox' );
		...
		public function load_settings_values() {
			...
			if ( true === $this->settings_values_loaded ) {
				...
				if ( ( isset( $this->quiz_edit['quiz'] ) ) && ( ! empty( $this->quiz_edit['quiz'] ) ) ) {
					...
					$this->setting_option_values['associated_settings'] = ebox_get_setting( get_the_ID(), 'quiz_pro' );
		...
		public function load_settings_fields() {
			...
			$pro_quiz_options_default = array(
				'-1'  => esc_html__( 'Select a ProQuiz association', 'ebox' ),
				'new' => esc_html__( 'New ProQuiz association', 'ebox' ),
			);
			...
			$this->setting_option_fields = array(
				'toplistDataAddBlock' => array(
					...
					'label'       => esc_html__( 'Re-apply after', 'ebox' ),
					'label_full'  => true,
					'input_full'  => true,
					'type'        => 'number',
					'value'       => $this->setting_option_values['toplistDataAddBlock'],
					'input_label' => esc_html__( 'minutes', 'ebox' ),
					'default'     => '1',
					'attrs'       => array(
						'step' => 1,
						'min'  => 0,
			...
			$this->setting_option_fields = array(
				'toplistDataShowIn' => array(
					...
					'default'    => '1',
					'options'    => array(
						'1' => esc_html__( 'Below the result text', 'ebox' ),
						'2' => esc_html__( 'In a button', 'ebox' ),
					),
					'rest'       => array(
						'show_in_rest' => ebox_REST_API::enabled(),
						'rest_args'    => array(
			...
			$this->setting_option_fields = array(
				'statisticsIpLock' => array(
					...
					'label_full'        => true,
					'label'             => esc_html__( 'IP-lock time limit', 'ebox' ),
					'label_description' => esc_html__( 'Protect the statistics from spam. Results will only be saved every X minutes.', 'ebox' ),
					'input_full'        => true,
					'type'              => 'number',
					'class'             => '-small',
			...
			$this->setting_option_fields = array(
				'emailNotification' => array(
					...
					'label'             => esc_html__( 'Email trigger', 'ebox' ),
					'label_full'        => true,
					'label_description' => sprintf(
						// translators: placeholder: quiz.
						esc_html_x( 'The admin will receive an email notification when the following users have taken the %s.', 'placeholder: quiz', 'ebox' ),
					...
					'options'           => array(
						WpProQuiz_Model_Quiz::QUIZ_EMAIL_NOTE_ALL => esc_html__( 'All users', 'ebox' ),
						WpProQuiz_Model_Quiz::QUIZ_EMAIL_NOTE_REG_USER => esc_html__( 'Registered users only', 'ebox' ),
					),
					'rest'              => array(
						'show_in_rest' => ebox_REST_API::enabled(),
						'rest_args'    => array(
			...
			$this->setting_option_fields = array(
				'timeLimitCookie' => array(
					...
					'label_full'        => true,
					'label'             => esc_html__( 'Cookie time limit', 'ebox' ),
					'label_description' => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'Save the user’s answers into a browser cookie until the %s is submitted', 'placeholder: Quiz', 'ebox' ),
			...
			$this->setting_option_fields = array(
				'associated_settings' => array(
					...
					'label'             => sprintf(
						// translators: Quiz.
						esc_html_x( 'Associated %s Database Table', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
					'label_description' => wp_kses_post( 'This will change the database association.<br /><strong>We do not recommend editing this</strong> unless for a specific purpose.', 'ebox' ),
					'input_full'        => true,
			...
			$this->setting_option_fields = array(
				...
				'formActivated'               => array(
					...
					'label'               => esc_html__( 'Custom Fields', 'ebox' ),
					'value'               => $this->setting_option_values['formActivated'],
					'default'             => '',
					'help_text'           => sprintf(
						// translators: placeholder: quiz, Quiz.
						esc_html_x( 'Enable this option to gather data from your users before or after the %1$s. All data is stored in the %2$s Statistics.', 'placeholder: quiz, Quiz', 'ebox' ),
						ebox_get_custom_label_lower( 'quiz' ),
						ebox_get_custom_label( 'quiz' )
					),
					'options'             => array(
				...
				'formShowPosition'            => array(
					...
					'type'           => 'radio',
					'label'          => esc_html__( 'Display Position', 'ebox' ),
					...
					'options'        => array(
						WpProQuiz_Model_Quiz::QUIZ_FORM_POSITION_START => sprintf(
							// translators: placeholder: quiz.
							esc_html_x( 'On the %s startpage', 'placeholder: quiz.', 'ebox' ),
							ebox_get_custom_label_lower( 'quiz' )
						),
						WpProQuiz_Model_Quiz::QUIZ_FORM_POSITION_END => sprintf(
							// translators: placeholder: quiz, quiz.
							esc_html_x( 'At the end of the %1$s (before the %2$s result)', 'placeholder: quiz, quiz', 'ebox' ),
							ebox_get_custom_label_lower( 'quiz' ),
							ebox_get_custom_label_lower( 'quiz' )
						),
					),
				...
				'toplistActivated'            => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Leaderboard', 'ebox' ),
				...
				'toplistDataAddPermissions'   => array(
					...
					'type'           => 'select',
					'label'          => esc_html__( 'Who can apply?', 'ebox' ),
					'value'          => $this->setting_option_values['toplistDataAddPermissions'],
					'options'        => array(
						'1' => esc_html__( 'All user', 'ebox' ),
						'2' => esc_html__( 'Registered users only', 'ebox' ),
						'3' => esc_html__( 'Anonymous users only', 'ebox' ),
					),
				...
				'toplistDataAddMultiple'      => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Multiple Applications per user', 'ebox' ),
					'value'               => $this->setting_option_values['toplistDataAddMultiple'],
					'parent_setting'      => 'toplistActivated',
					'default'             => '',
					'options'             => array(
						''   => '',
						'on' => esc_html__( 'Users can apply more than once to the leaderboard', 'ebox' ),
					),
					'inline_fields'       => array(
				...
				'toplistDataAddAutomatic'     => array(
					...
					'type'           => 'checkbox',
					'label'          => esc_html__( 'Automatic user entry', 'ebox' ),
				...
				'toplistDataShowLimit'        => array(
					...
					'label'          => esc_html__( 'Number of displayed entries', 'ebox' ),
					'type'           => 'number',
					'class'          => '-small',
					'parent_setting' => 'toplistActivated',
				...
				'toplistDataSort'             => array(
					...
					'type'           => 'select',
					'label'          => esc_html__( 'Sort list by?', 'ebox' ),
					'value'          => $this->setting_option_values['toplistDataSort'],
					'parent_setting' => 'toplistActivated',
					'default'        => '1',
					'options'        => array(
						'1' => esc_html__( 'Best user', 'ebox' ),
						'2' => esc_html__( 'Newest entry', 'ebox' ),
						'3' => esc_html__( 'Oldest entry', 'ebox' ),
					),
					'rest'           => array(
						'show_in_rest' => ebox_REST_API::enabled(),
						'rest_args'    => array(
				...
				'toplistDataShowIn_enabled'   => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'Display on %s results page', 'placeholder: Quiz.', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
				...
				'toplistDataCaptcha'          => array(
					...
					'type'           => 'checkbox',
					'label'          => esc_html__( 'Really Simple CAPTCHA', 'ebox' ),
					'help_text'      => sprintf(
						// translators: placeholder: links to Real Simple CAPTCHA.
						esc_html_x( 'This option requires additional plugin: %s', 'placeholder: links to Real Simple CAPTCHA', 'ebox' ),
				...
				'viewProfileStatistics'       => array(
					...
					'type'           => 'checkbox-switch',
					'label'          => esc_html__( 'Front-end Profile Display', 'ebox' ),
				...
				'statisticsIpLock_enabled'    => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Statistics IP-lock', 'ebox' ),
				...
				'email_enabled'               => array(
					'name'                => 'email_enabled',
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Email Notifications', 'ebox' ),
				...
				'templates_enabled'           => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( '%s Templates', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
				...
				'templateLoadId'              => array(
					...
					'label'          => esc_html__( 'Use Template', 'ebox' ),
				...
				'templateSaveList'            => array(
					...
					'label'          => esc_html__( 'Save as Template', 'ebox' ),
					'template_type'  => WpProQuiz_Model_Template::TEMPLATE_TYPE_QUIZ,
					'parent_setting' => 'templates_enabled',
				),
				...
				'advanced_settings'           => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Advanced Settings', 'ebox' ),
				...
				'timeLimitCookie_enabled'     => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Browser Cookie Answer Protection', 'ebox' ),
					'value'               => $this->setting_option_values['timeLimitCookie_enabled'],
					'help_text'           => sprintf(
						// translators: placeholder: quizzes.
						esc_html_x( 'Browser cookies have limited memory. This may not work with large %s.', 'placeholder: quizzes', 'ebox' ),
				...
				'associated_settings_enabled' => array(
					...
					'type'                => 'checkbox-switch',
					'label'               => esc_html__( 'Associated Settings', 'ebox' ),
					'value'               => '',
					'default'             => '',
					'options'             => array(
						'on' => '',
		...
		public function filter_saved_fields( $settings_values = array(), $settings_metabox_key = '', $settings_screen_id = '' ) {
			if ( ( $settings_screen_id === $this->settings_screen_id ) && ( $settings_metabox_key === $this->settings_metabox_key ) ) {
				...
				if ( empty( $settings_values['quiz_pro'] ) ) {
					$settings_values['quiz_pro'] = ebox_get_setting( get_the_ID(), 'quiz_pro' );
				}
	...
	add_filter(
		'ebox_post_settings_metaboxes_init_' . ebox_get_post_type_slug( 'quiz' ),
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings'] = ebox_Settings_Metabox_Quiz_Admin_Data_Handling_Settings::add_metabox_instance();
			}
