<?php
/**
 * ebox Settings Metabox for Quiz Access Settings.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Quiz_Access_Settings' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Quiz Access Settings.
	 *
	 * @since 3.0.0
	 */
	class ebox_Settings_Metabox_Quiz_Access_Settings extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_metabox_key = 'ebox-quiz-access-settings';

			// Section label/header.
			$this->settings_section_label = sprintf(
				// translators: placeholder: Quiz.
				esc_html_x( '%s Access Settings', 'placeholder: Quiz', 'ebox' ),
				ebox_get_custom_label( 'quiz' )
			);

			$this->settings_section_description = sprintf(
				// translators: placeholder: quiz.
				esc_html_x( 'Controls the requirements for accessing the %s', 'placeholder: quiz', 'ebox' ),
			...
			add_filter( 'ebox_settings_row_input_inside_before', array( $this, 'settings_row_input_inside_before' ), 30, 2 );
		...
		public function load_settings_fields() {
			...
			if ( ( isset( $this->setting_option_values['course'] ) ) && ( ! empty( $this->setting_option_values['course'] ) ) ) {
				$select_lesson_options = $ebox_lms->select_a_lesson_or_topic( absint( $this->setting_option_values['course'] ), true, false );
			}
			...
			if ( ebox_use_select2_lib() ) {
				$select_lesson_options_default = array(
					'-1' => sprintf(
						...
						esc_html_x( 'Search or select a %1$s or %2$s…', 'placeholder: Lesson, Topic', 'ebox' ),
				...
				$select_lesson_options_default = array(
					'' => sprintf(
						// translators: placeholder: Lesson, Topic.
						esc_html_x( 'Select a %1$s or %2$s', 'placeholder: Lesson, Topic', 'ebox' ),
						ebox_get_custom_label( 'lesson' ),
						ebox_get_custom_label( 'topic' )
					),
				);
			...
			if ( ebox_use_select2_lib() ) {
				$select_quiz_prerequisite_options_default = array(
					'-1' => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'No previous %s required', 'placeholder: Quiz.', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
				);
			} else {
			...
			$this->setting_option_fields = array(
				'visible_after' => array(
					...
					'label_none'  => true,
					'input_full'  => true,
					'input_label' => esc_html__( 'day(s)', 'ebox' ),
					'attrs'       => array(
						'step' => 1,
						'min'  => 0,
					),
					...
					'rest'        => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => esc_html__( 'Visible After X day(s)', 'ebox' ),
								'type'        => 'integer',
								'default'     => 0,
							),
						),
				...
				'visible_after_specific_date' => array(
					...
					'rest'       => array(
						...
						'rest_args'    => array(
							'schema' => array(
								// translators: placeholder: Lesson.
								'description' => esc_html__( 'Visible After Specific Date (YYYY-MM-DD)', 'ebox' ),
								'type'        => 'date',
								'default'     => '',
							),
						),
			...
			$this->setting_option_fields = array(
				'course'                  => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Course.
						esc_html_x( 'Associated %s', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'        => 'select',
					'default'     => '',
					...
					'attrs'       => array(
						'data-ld_selector_nonce'   => wp_create_nonce( 'ebox-courses' ),
				...
				'lesson'                  => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Lesson.
						esc_html_x( 'Associated %s', 'placeholder: Lesson', 'ebox' ),
						ebox_get_custom_label( 'lesson' )
					),
					'type'        => 'select',
					'default'     => '',
					...
					'attrs'       => array(
						'data-ld_selector_nonce'   => wp_create_nonce( 'ebox-modules' ),
						'data-ld_selector_default' => '1',
					),
					'rest'        => array(
						'show_in_rest' => ebox_REST_API::enabled(),
				...
				'lesson_schedule'         => array(
					...
					'label'   => sprintf(
						// Translators: placeholder: Quiz.
						esc_html_x( '%s Release Schedule', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
					'type'    => 'radio',
					...
					'options' => array(
						''                            => array(
							'label'       => esc_html__( 'Immediately', 'ebox' ),
							'description' => sprintf(
								// translators: placeholder: quiz, course.
								esc_html_x( 'The %1$s is made available on %2$s enrollment.', 'placeholder: quiz, course', 'ebox' ),
								ebox_get_custom_label_lower( 'quiz' ),
								ebox_get_custom_label_lower( 'course' )
							),
						),
						'visible_after'               => array(
							'label'               => esc_html__( 'Enrollment-based', 'ebox' ),
							'description'         => sprintf(
								// translators: placeholder: quiz, course.
								esc_html_x( 'The %1$s will be available X days after %2$s enrollment.', 'placeholder: quiz, course.', 'ebox' ),
								ebox_get_custom_label_lower( 'quiz' ),
								ebox_get_custom_label_lower( 'course' )
							),
							'inline_fields'       => array(
						...
						'visible_after_specific_date' => array(
							'label'               => esc_html__( 'Specific date', 'ebox' ),
							'description'         => sprintf(
								// translators: placeholder: quiz.
								esc_html_x( 'The %s will be available on a specific date.', 'placeholder: quiz', 'ebox' ),
								ebox_get_custom_label_lower( 'quiz' )
							),
							'inline_fields'       => array(
					...
					'rest'    => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => esc_html__( 'Available Release Schedule', 'ebox' ),
								'type'        => 'string',
								'default'     => '',
								'required'    => false,
								'enum'        => array(
				...
				'prerequisiteList'        => array(
					...
					'label'     => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( '%s Prerequisites', 'placeholder: Quiz', 'ebox' ),
						ebox_get_custom_label( 'quiz' )
					),
					'help_text' => sprintf(
						// translators: placeholders: quizzes, quiz.
						esc_html_x( 'Select one or more %1$s that must be completed prior to taking this %2$s.', 'placeholders: Quizzes Quiz', 'ebox' ),
				...
				'startOnlyRegisteredUser' => array(
					...
					'type'      => 'checkbox',
					'label'     => esc_html__( 'Allowed Users', 'ebox' ),
					'value'     => $this->setting_option_values['startOnlyRegisteredUser'],
					'help_text' => sprintf(
						// translators: placeholders: quizzes, course, courses, quiz.
						esc_html_x( 'This option is especially useful if administering %1$s via shortcodes on non-%2$s pages, or if your %3$s are open but you wish only authenticated users to take the %4$s.', 'placeholders: quizzes, courses, quiz.', 'ebox' ),
					...
					'options'   => array(
						'on' => sprintf(
							// translators: placeholder: quiz.
							esc_html_x( 'Only registered users can take this %s', 'placeholder: quiz', 'ebox' ),
							ebox_get_custom_label( 'quiz' )
						),
					),
					'rest'      => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								esc_html__( 'Only Logged-in Users', 'ebox' ),
								'type'      => 'boolean',
								'default'   => false,
							),
						),
		...
		/**
		 * Filter function for `ebox_settings_row_input_inside_before` to add a custom HTML before the input.
		 *
		 * @since 4.2.0
		 *
			...
			if ( 'lesson_schedule' === $field_args['name'] ) {
				if ( is_admin() ) {
					...
					if ( ( is_a( $screen, 'WP_Screen' ) ) && ( 'post' === $screen->base ) && ( 'edit' === $screen->parent_base ) ) {
						if ( ( is_a( $this, __CLASS__ ) ) && ( ! empty( $this->setting_option_values['lesson'] ) ) && ( ( ebox_get_setting( $this->setting_option_values['lesson'], 'visible_after' ) ) || ( ebox_get_setting( $this->setting_option_values['lesson'], 'visible_after_specific_date' ) ) ) ) {
							if ( ! ebox_is_course_shared_steps_enabled() ) {
								$post_link_label = ebox_get_custom_label_lower( ebox_get_post_type_key( get_post_type( $this->setting_option_values['lesson'] ) ) );

								$lesson_schedule_alert = sprintf(
									// translators: placeholder: parent post edit link.
									esc_html_x( 'The parent %s is setup for scheduled release. This step will be available after.', 'placeholder: parent post edit link.', 'ebox' ),
	...
	add_filter(
		...
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Quiz_Access_Settings'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Quiz_Access_Settings' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Quiz_Access_Settings'] = ebox_Settings_Metabox_Quiz_Access_Settings::add_metabox_instance();
			}
