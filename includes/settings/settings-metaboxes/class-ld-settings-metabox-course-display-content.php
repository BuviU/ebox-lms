<?php
/**
 * ebox Settings Metabox for Course Display and Content Options.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Course_Display_Content' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Course Display and Content Options.
	 *
	 * @since 3.0.0
	 */
	class ebox_Settings_Metabox_Course_Display_Content extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_metabox_key = 'ebox-course-display-content-settings';
			...
			$this->settings_section_description = sprintf(
				...
				esc_html_x( 'Controls the look and feel of the %s and optional content settings', 'placeholder: course', 'ebox' ),
		...
		public function load_settings_values() {
			...
			if ( true === $this->settings_values_loaded ) {
				...
				if ( 'CUSTOM' === $this->setting_option_values['course_lesson_per_page'] ) {
					...
					$this->setting_option_values['course_lesson_per_page_custom'] = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Management_Display', 'course_pagination_modules' );
					$this->setting_option_values['course_topic_per_page_custom']  = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Management_Display', 'course_pagination_topics' );
				}

				if ( ! isset( $this->setting_option_values['course_lesson_orderby'] ) ) {
					$this->setting_option_values['course_lesson_orderby'] = ebox_get_setting( get_the_ID(), 'course_lesson_orderby' );
				}
				if ( ! isset( $this->setting_option_values['course_lesson_order'] ) ) {
					$this->setting_option_values['course_lesson_order'] = ebox_get_setting( get_the_ID(), 'course_lesson_order' );
				}
				...
				$this->setting_option_values['exam_challenge'] = (int) ebox_get_course_exam_challenge( $this->_post->ID );
		...
		public function load_settings_fields() {
			...
			if ( ebox_use_select2_lib() ) {
				...
				if ( ! empty( $this->setting_option_values['certificate'] ) ) {
					...
					if ( ( $cert_post ) && ( is_a( $cert_post, 'WP_Post' ) ) ) {
						$select_cert_options[ $cert_post->ID ] = ebox_format_step_post_title_with_status_label( $cert_post );
					}
				}

				$select_exam_challenge_options_default = sprintf(
					// translators: placeholder: Exam.
					esc_html_x( 'Search or select a %s…', 'placeholder: Exam', 'ebox' ),
				...
				if ( ! empty( $this->setting_option_values['exam_challenge'] ) ) {
					...
					if ( ( $exam_post ) && ( is_a( $exam_post, 'WP_Post' ) ) ) {
						$select_exam_challenge_options[ $exam_post->ID ] = ebox_format_step_post_title_with_status_label( $exam_post );
					}
				}
				...
				$select_exam_challenge_options_default = array(
					'' => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( 'Select %s', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
				);
				$select_cert_options                   = $ebox_lms->select_a_certificate();
			...
			$this->setting_option_fields = array(
				'course_materials_enabled'      => array(
					...
					'label'               => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Materials', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'help_text'           => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'List and display support materials for the %s. This is visible to all users (including non-enrollees) by default.', 'placeholder: course', 'ebox' ),
						ebox_get_custom_label_lower( 'course' )
					),
					...
					'options'             => array(
						'on' => sprintf(
							// translators: placeholder: Course.
							esc_html_x( 'Any content added below is displayed on the main %s page', 'placeholder: Course', 'ebox' ),
							ebox_get_custom_label( 'course' )
						),
						''   => '',
				...
				'certificate'                   => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Certificate', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'default'     => '',
				...
				'exam_challenge'                => array(
					...
					'label'       => sprintf(
						// translators: placeholder: Exam.
						esc_html_x( '%s Show', 'placeholder: Exam', 'ebox' ),
						ebox_get_custom_label( 'exam' )
					),
					'default'     => '',
				...
				'course_disable_content_table'  => array(
					...
					'help_text' => sprintf(
						// translators: placeholder: Course.
						esc_html_x( 'Choose whether to display the %s content table to ALL users or only enrollees', 'placeholder: Course', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'options'   => array(
					...
					'rest'      => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => sprintf(
									// translators: placeholder: Course.
									esc_html_x( 'Disable %s Content Table', 'placeholder: Course', 'ebox' ),
									ebox_get_custom_label( 'course' )
								),
								'type'        => 'boolean',
								'default'     => false,
				...
				'course_lesson_per_page'        => array(
					...
					'help_text'           => sprintf(
						...
						esc_html_x( 'Customize the pagination options for this %1$s content table and %2$s navigation widget.', 'placeholders: course, course', 'ebox' ),
						ebox_get_custom_label_lower( 'course' ),
						ebox_get_custom_label_lower( 'course' )
					),
					'options'             => array(
						''       => esc_html__( 'Currently showing default pagination', 'ebox' ),
						'CUSTOM' => '',
					),
					...
					'rest'           => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								// translators: placeholder: modules per page.
								'description' => sprintf( esc_html_x( '%s per page', 'placeholder: modules per page', 'ebox' ), ebox_get_custom_label( 'modules' ) ),
								'type'        => 'integer',
					...
					'rest'           => array(
						...
						'rest_args'    => array(
							'schema' => array(
								...
								'description' => sprintf( esc_html_x( '%s per page', 'placeholder: Topics per page', 'ebox' ), ebox_get_custom_label( 'topics' ) ),
								'type'        => 'integer',
				...
				'course_lesson_order_enabled'   => array(
					...
					'label'               => sprintf(
						...
						esc_html_x( 'Custom %s Order', 'placeholder: Lesson', 'ebox' ),
						ebox_get_custom_label( 'lesson' )
					),
					'type'                => 'checkbox-switch',
					'help_text'           => sprintf(
						// translators: placeholders: modules, topics.
						esc_html_x( 'Customize the display order of %1$s and %2$s.', 'placeholders: modules, topics', 'ebox' ),
					...
					'options'             => array(
						''   => sprintf(
							...
							esc_html_x( 'Using default sorting by %1$s in %2$s order', 'placeholder: lesson order by, lesson order direction labels', 'ebox' ),
							'<em>' .
							ebox_Settings_Section::get_section_setting_select_option_label( 'ebox_Settings_Section_modules_Display_Order', 'orderby' )
							. '</em>',
							'<em>' .
							ebox_Settings_Section::get_section_setting_select_option_label( 'ebox_Settings_Section_modules_Display_Order', 'order' )
							. '</em>'
						),
						'on' => '',
					),
		...
		public function filter_saved_fields( $settings_values = array(), $settings_metabox_key = '', $settings_screen_id = '' ) {
			if ( ( $settings_screen_id === $this->settings_screen_id ) && ( $settings_metabox_key === $this->settings_metabox_key ) ) {
				...
				/**
				 * Check the modules Per Page set course_lesson_per_page/course_lesson_per_page_custom. If 'course_lesson_per_page' setting is
				...
				if ( (int) $settings_values['exam_challenge'] !== (int) $this->setting_option_values['exam_challenge'] ) {
					if ( ! empty( $this->setting_option_values['exam_challenge'] ) ) {
						ebox_update_setting( (int) $this->setting_option_values['exam_challenge'], 'exam_challenge_course_show', '' );
					}
					if ( ! empty( $settings_values['exam_challenge'] ) ) {
						ebox_update_setting( (int) $settings_values['exam_challenge'], 'exam_challenge_course_show', $this->_post->ID );
					}
				}
	...
	add_filter(
		...
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Course_Display_Content'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Course_Display_Content' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Course_Display_Content'] = ebox_Settings_Metabox_Course_Display_Content::add_metabox_instance();
			}
