<?php
/**
 * ebox Settings Metabox for Topic Access Settings.
 *
 * @since 3.0.0
...
if ( ( class_exists( 'ebox_Settings_Metabox' ) ) && ( ! class_exists( 'ebox_Settings_Metabox_Topic_Access_Settings' ) ) ) {
	/**
	 * Class ebox Settings Metabox for Topic Access Settings.
	 *
	 * @since 3.0.0
	 */
	class ebox_Settings_Metabox_Topic_Access_Settings extends ebox_Settings_Metabox {
		...
		public function __construct() {
			...
			$this->settings_metabox_key = 'ebox-topic-access-settings';

			// Section label/header.
			$this->settings_section_label = sprintf(
				// translators: placeholder: Topic.
				esc_html_x( '%s Access Settings', 'placeholder: Topic', 'ebox' ),
				ebox_get_custom_label( 'topic' )
			);

			$this->settings_section_description = sprintf(
				// translators: placeholder: topic.
				esc_html_x( 'Controls how, where, and when the %s can be accessed.', 'placeholder: topic', 'ebox' ),
				ebox_get_custom_label( 'topic' )
			);
		...
		public function load_settings_fields() {
			...
			if ( ( isset( $this->setting_option_values['course'] ) ) && ( ! empty( $this->setting_option_values['course'] ) ) ) {
				$select_lesson_options = $ebox_lms->select_a_lesson( absint( $this->setting_option_values['course'] ) );
			}
			...
			if ( ebox_use_select2_lib() ) {
				...
				$select_lesson_options_default = array(
					'' => sprintf(
						// translators: placeholder: Lesson.
						esc_html_x( 'Select %s', 'placeholder: Lesson', 'ebox' ),
						ebox_get_custom_label( 'lesson' )
					),
				);
			...
			$this->setting_option_fields = array(
				'course'          => array(
					...
					'label'       => sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Associated %s', 'Associated Course Label', 'ebox' ),
						ebox_get_custom_label( 'course' )
					),
					'type'        => 'select',
					'lazy_load'   => true,
					'help_text'   => sprintf(
						...
						esc_html_x( 'Associate this %1$s with a %2$s.', 'placeholder: Topic, Course.', 'ebox' ),
						ebox_get_custom_label( 'topic' ),
						ebox_get_custom_label( 'course' )
					),
					'default'     => '',
				...
				'lesson'          => array(
					...
					'help_text'   => sprintf(
						...
						esc_html_x( 'Associate this %1$s with a %2$s.', 'placeholders: Topic, Lesson', 'ebox' ),
						ebox_get_custom_label( 'topic' ),
						ebox_get_custom_label( 'lesson' )
					),
					'default'     => '',
				...
				'lesson_schedule' => array(
					...
					'label'   => sprintf(
						// Translators: placeholder: Topic.
						esc_html_x( '%s Release Schedule', 'placeholder: Topic', 'ebox' ),
						ebox_get_custom_label( 'topic' )
					),
					'type'    => 'radio',
					...
					'options' => array(
						''                            => array(
							...
							'description' => sprintf(
								...
								esc_html_x( 'The %1$s is made available on %2$s enrollment.', 'placeholder: topic, course', 'ebox' ),
						...
						'visible_after'               => array(
							...
							'description'         => sprintf(
								// translators: placeholder: topic, course.
								esc_html_x( 'The %1$s will be available X days after %2$s enrollment.', 'placeholder: topic, course.', 'ebox' ),
								ebox_get_custom_label_lower( 'topic' ),
								ebox_get_custom_label_lower( 'course' )
							),
							'inline_fields'       => array(
						...
						'visible_after_specific_date' => array(
							...
							'description'         => sprintf(
								// translators: placeholder: topic.
								esc_html_x( 'The %s will be available on a specific date.', 'placeholder: topic', 'ebox' ),
								ebox_get_custom_label_lower( 'topic' )
							),
							'inline_fields'       => array(
	...
	add_filter(
		...
		function( $metaboxes = array() ) {
			if ( ( ! isset( $metaboxes['ebox_Settings_Metabox_Topic_Access_Settings'] ) ) && ( class_exists( 'ebox_Settings_Metabox_Topic_Access_Settings' ) ) ) {
				$metaboxes['ebox_Settings_Metabox_Topic_Access_Settings'] = ebox_Settings_Metabox_Topic_Access_Settings::add_metabox_instance();
			}
