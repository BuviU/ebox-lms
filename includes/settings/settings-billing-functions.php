<?php
/**
 * ebox Settings billing functions
 *
 * @since 3.5.0
 * @package ebox
 */
 ...
 */
function ebox_billing_cycle_setting_field_html( int $post_id = 0, string $post_type = '' ): string {
	...
	if ( ! empty( $post_id ) ) {
		$price_billing_t3 = ebox_get_setting( $post_id, $settings_prefix . '_price_billing_t3' );
		$price_billing_p3 = ebox_get_setting( $post_id, $settings_prefix . '_price_billing_p3' );
	}
	...
	$html .= '<option value="">' . esc_html__( 'select interval', 'ebox' ) . '</option>';
	$html .= '<option value="D" ' . selected( $price_billing_t3, 'D', false ) . '>' . esc_html__( 'day(s)', 'ebox' ) . '</option>';
	$html .= '<option value="W" ' . selected( $price_billing_t3, 'W', false ) . '>' . esc_html__( 'week(s)', 'ebox' ) . '</option>';
	$html .= '<option value="M" ' . selected( $price_billing_t3, 'M', false ) . '>' . esc_html__( 'month(s)', 'ebox' ) . '</option>';
	$html .= '<option value="Y" ' . selected( $price_billing_t3, 'Y', false ) . '>' . esc_html__( 'year(s)', 'ebox' ) . '</option>';
	$html .= '</select>';
	 ...
	 * @param string $post_type Post type slug.
	 */
	return apply_filters( 'ebox_billing_cycle_settings_field_html', $html, $post_id, $post_type );
}
 ...
 */
function ebox_trial_duration_setting_field_html( int $post_id = 0, string $post_type = '' ): string {
	$post_id = absint( $post_id );
	if ( empty( $post_id ) ) {
		if ( isset( $_GET['post'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$post_id = absint( $_GET['post'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	if ( ! empty( $post_id ) ) {
		$trial_duration_t1 = ebox_get_setting( $post_id, $settings_prefix . '_trial_duration_t1' );
		$trial_duration_p1 = ebox_get_setting( $post_id, $settings_prefix . '_trial_duration_p1' );
	}
	...
	$html .= '<option value="">' . esc_html__( 'select interval', 'ebox' ) . '</option>';
	$html .= '<option value="D" ' . selected( $trial_duration_t1, 'D', false ) . '>' . esc_html__( 'day(s)', 'ebox' ) . '</option>';
	$html .= '<option value="W" ' . selected( $trial_duration_t1, 'W', false ) . '>' . esc_html__( 'week(s)', 'ebox' ) . '</option>';
	$html .= '<option value="M" ' . selected( $trial_duration_t1, 'M', false ) . '>' . esc_html__( 'month(s)', 'ebox' ) . '</option>';
	$html .= '<option value="Y" ' . selected( $trial_duration_t1, 'Y', false ) . '>' . esc_html__( 'year(s)', 'ebox' ) . '</option>';
	$html .= '</select>';
	 ...
	 * @param string $post_type Post type slug.
	 */
	return apply_filters( 'ebox_trial_duration_settings_field_html', $html, $post_id, $post_type );
}
 ...
 */
function ebox_billing_cycle_field_frequency_validate( string $price_billing_t3 ): string {
 ...
 */
function ebox_billing_cycle_field_interval_validate( int $price_billing_p3, string $price_billing_t3 ): int {
	$price_billing_t3     = ebox_billing_cycle_field_frequency_validate( $price_billing_t3 );
	$price_billing_p3_max = ebox_billing_cycle_field_frequency_max( $price_billing_t3 );
 ...
 * @return int Valid interval or zero.
 */
function ebox_billing_cycle_field_frequency_max( string $price_billing_t3 ): int {
	switch ( $price_billing_t3 ) {
		case 'D':
			$price_billing_p3 = 90;
			break;
