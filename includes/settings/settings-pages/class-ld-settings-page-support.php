...
if ( ( class_exists( 'ebox_Settings_Page' ) ) && ( ! class_exists( 'ebox_Settings_Page_Support' ) ) ) {
	/**
	 * Class ebox Settings Page Support.
	 *
	 * @since 3.1.0
	 */
	class ebox_Settings_Page_Support extends ebox_Settings_Page {
		...
		public function __construct() {
			...
			$this->settings_page_title   = esc_html__( 'Support', 'ebox' );
			...
			add_action( 'ebox_settings_page_load', array( $this, 'ebox_settings_page_load' ) );
		 ...
		 */
		public function ebox_settings_page_load( $settings_screen_id = '' ) {
			global $ebox_lms;
			...
			if ( $settings_screen_id === $this->settings_screen_id ) {
				...
				wp_enqueue_style(
					'ebox-admin-support-page',
					ebox_LMS_PLUGIN_URL . 'assets/css/ebox-admin-support-page' . ebox_min_asset() . '.css',
					array(),
					ebox_SCRIPT_VERSION_TOKEN
				);
				wp_style_add_data( 'ebox-admin-support-page', 'rtl', 'replace' );
				$ebox_assets_loaded['styles']['ebox-admin-support-page'] = __FUNCTION__;
			}
		}
		...
		public function gather_system_details() {
			 ...
			 */
			$this->system_info = apply_filters( 'ebox_support_sections_init', $this->system_info );
			 ...
			 * @param array $system_info An array of support sections.
			 */
			$this->system_info = apply_filters( 'ebox_support_sections', $this->system_info );
				...
				switch ( $output_type ) {
					...
					default:
						if ( ( isset( $_set['desc'] ) ) & ( ! empty( $_set['desc'] ) ) ) {
							?>
							<div class="ebox-support-settings-desc"><?php echo wp_kses_post( wptexturize( $_set['desc'] ) ); ?></div>
							<?php
						}

						if ( ( isset( $_set['columns'] ) ) && ( ! empty( $_set['columns'] ) ) && ( isset( $_set['settings'] ) ) && ( ! empty( $_set['settings'] ) ) ) {
							?>
							<table cellspacing="0" class="ebox-support-settings">
								<thead>
									<tr>
									<?php
									foreach ( $_set['columns'] as $column_key => $column_set ) {
										 ...
										 */
										$column_class = apply_filters( 'ebox_support_column_class', $column_class, $column_key, $_key );
										?>
										...
										if ( 'settings-sub-section-' === substr( $setting_key, 0, strlen( 'settings-sub-section-' ) ) ) {
											...
											foreach ( $_set['columns'] as $column_key => $column_set ) {
												...
												<?php
													...
													apply_filters( 'ebox_support_column_class', '', $column_key, $_key );
												?>
												">
												<?php
...
add_action(
	...
	function() {
		ebox_Settings_Page_Support::add_page_instance();
	}
);
