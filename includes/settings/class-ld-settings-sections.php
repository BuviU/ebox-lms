<?php
/**
 * ebox Settings Sections Abstract Class.
 *
 * @since 2.4.0
...
if ( ! class_exists( 'ebox_Settings_Section' ) ) {
	...
	/**
	 * Class for ebox Settings Sections.
	 *
	 * @since 2.4.0
	 */
	class ebox_Settings_Section {
		...
		protected function __construct() {
			...
			add_action( 'ebox_settings_page_init', array( $this, 'settings_page_init' ) );
			if ( defined( 'ebox_SETTINGS_SECTION_TYPE' ) && ( 'metabox' === ebox_SETTINGS_SECTION_TYPE ) ) {
				add_action( 'ebox_add_meta_boxes', array( $this, 'add_meta_boxes' ) );
			}
			...
			add_filter( 'ebox_settings_section_save_fields_' . $this->setting_option_key, array( $this, 'filter_section_save_fields' ), 30, 4 );
			...
			if ( ( ! empty( $criteria_field ) ) && ( ! empty( $criteria_value ) ) ) {
				if ( ! empty( self::$_instances ) ) {
					foreach ( self::$_instances as $_instance_key => $_instance_object ) {
						if ( ( $_instance_object ) && ( is_a( $_instance_object, 'ebox_Settings_Section' ) ) ) {
		...
		public function load_settings_values() {
			...
			if ( true === $this->load_options ) {
				...
				if ( ( false === $this->setting_option_values ) || ( '' === $this->setting_option_values ) ) {
					 ...
					 * @since 3.4.0.2
					 */
					$gl_user_activate = get_option( 'ebox_teams_team_leader_user_activate', '' );
		...
		public function load_settings_fields() {
			...
			foreach ( $this->setting_option_fields as &$setting_option_field ) {
				...
				$field_instance = ebox_Settings_Fields::get_field_instance( $setting_option_field['type'] );
				if ( ( $field_instance ) && ( 'ebox_Settings_Fields' === get_parent_class( $field_instance ) ) ) {
					...
					if ( ! isset( $setting_option_field['display_callback'] ) ) {
						$display_ref = ebox_Settings_Fields::get_field_instance( $setting_option_field['type'] )->get_creation_function_ref();
						if ( $display_ref ) {
					...
					if ( ! isset( $setting_option_field['validate_callback'] ) ) {
						$validate_ref = ebox_Settings_Fields::get_field_instance( $setting_option_field['type'] )->get_validation_function_ref();
						if ( $validate_ref ) {
					...
					if ( ! isset( $setting_option_field['value_callback'] ) ) {
						$value_ref = ebox_Settings_Fields::get_field_instance( $setting_option_field['type'] )->get_value_function_ref();
						if ( $value_ref ) {
		...
		public function section_pre_update_option( $value, $old_value, $setting_option_key = '' ) {
			 ...
			 */
			$value = apply_filters( 'ebox_settings_section_save_fields_' . $this->setting_option_key, $value, $old_value, $this->settings_section_key, $this->settings_screen_id );
		...
		public function section_update_option( $old_value = '', $value = '', $section_key = '' ) {
			if ( ( ! empty( $section_key ) ) && ( $section_key === $this->setting_option_key ) ) {
				...
				if ( ! defined( 'ebox_SETTINGS_UPDATING' ) ) {
					/**
					 * Define ebox LMS - Set during settings save processing
					 *
					 * @since 3.1.0
					 ...
					 */
					define( 'ebox_SETTINGS_UPDATING', true );
				}
		...
		public function show_settings_section_reset_confirm_link() {
			if ( ! empty( $this->reset_confirm_message ) ) {
				...
				?>
				<p class="delete-action ebox_input">
					<a href="<?php echo esc_url( $reset_url ); ?>" class="button-secondary submitdelete deletion" data-confirm="<?php echo esc_html( $this->reset_confirm_message ); ?>"><?php esc_html_e( 'Reset Settings', 'ebox' ); ?></a>
				</p>
				<?php
			}
		}
		...
		public function add_meta_boxes( $settings_screen_id = '' ) {
			...
			if ( $settings_screen_id === $this->settings_screen_id ) {
				...
				$show_section = apply_filters( 'ebox_show_metabox', $show_section, $this->metabox_key, $this->settings_screen_id );
				 ...
				 * @param string  $settings_screen_id   Settings screen ID.
				 */
				$show_section = apply_filters( 'ebox_show_section', $show_section, $this->settings_section_key, $this->settings_screen_id );

				if ( true === $show_section ) {
					if ( ! isset( $ebox_metaboxes[ $this->settings_screen_id ] ) ) {
						$ebox_metaboxes[ $this->settings_screen_id ] = array();
					}
					$ebox_metaboxes[ $this->settings_screen_id ][ $this->metabox_key ] = $this->metabox_key;
		...
		public function show_settings_section( $section = null ) {
			...
			if ( ( isset( $this->settings_fields_callback ) ) && ( ! empty( $this->settings_fields_callback ) ) && ( is_callable( $this->settings_fields_callback ) ) ) {
				 ...
				 */
				do_action( 'ebox_section_before', $this->settings_section_key, $this->settings_screen_id );

				echo '<div class="ebox ebox_options ' . esc_attr( $this->settings_section_key ) . '">';
				 ...
				 * @param string $setting_screen_id    Settings screen ID.
				 */
				do_action( 'ebox_section_fields_before', $this->settings_section_key, $this->settings_screen_id );
				 ...
				 * @param string $setting_screen_id    Settings screen ID.
				 */
				do_action( 'ebox_section_fields_after', $this->settings_section_key, $this->settings_screen_id );
				 ...
				 * @param string $setting_screen_id    Settings screen ID.
				 */
				do_action( 'ebox_section_reset_before', $this->settings_section_key, $this->settings_screen_id );
				 ...
				 * @param string $setting_screen_id    Settings screen ID.
				 */
				do_action( 'ebox_section_reset_after', $this->settings_section_key, $this->settings_screen_id );
				 ...
				 * @param string $setting_screen_id    Settings screen ID.
				 */
				do_action( 'ebox_section_after', $this->settings_section_key, $this->settings_screen_id );
			}
		}
		...
		public function show_settings_section_fields( $page, $section ) {
			...
			ebox_Settings_Fields::show_section_fields( $wp_settings_fields[ $page ][ $section ] );
		}
