...
if ( ! class_exists( 'LDLMS_Base_Answer_Type' ) ) {
	...
	class LDLMS_Base_Answer_Type implements LDLMS_Answer {
		...
		public function setup() {
			...
			add_filter( 'ebox_quiz_statistics_question_rest_response', array( $this, 'add_answer_text' ), 10, 2 );
			add_filter( 'ebox_rest_statistic_answer_node_data', array( $this, 'maybe_add_points' ), 10, 4 );
			add_filter( 'ebox_rest_statistic_answer_node_data', array( $this, 'maybe_add_answer_values' ), 20, 5 );
		}
		...
		public function get_answers() {
			...
			foreach ( $this->answer_data as $position => $data ) {
				...
				$answer_node_data = apply_filters(
					'ebox_rest_statistic_answer_node_data',
					$answer_node_data,
					'answer',
					$data,
					$this->question->getId()
