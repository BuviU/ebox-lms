...
if ( ( ! class_exists( 'LDLMS_Model_User_Quiz_Progress' ) ) && ( class_exists( 'LDLMS_Model_User' ) ) ) {
	...
	/**
	 * Class for ebox LDLMS_Model_User_Quiz_Progress.
	 *
	 * @since 3.4.0
	...
	class LDLMS_Model_User_Quiz_Progress extends LDLMS_Model_User {
		 ...
		 */
		private $progress_meta_key = '_ebox-quizzes';
			...
			if ( ! empty( $quiz_progress ) ) {
				foreach ( $quiz_progress as $quiz_idx => $quiz_item ) {
					...
					if ( isset( $quiz_item['pass'] ) ) {
						...
						$quiz_passingpercentage = (int) ebox_get_setting( $quiz_item['quiz'], 'passingpercentage' );
 ...
 */
function ebox_user_get_quiz_progress( $user_id = 0, $quiz_id = 0, $course_id = 0 ) {
	$user_id   = absint( $user_id );
	$quiz_id   = absint( $quiz_id );
	$course_id = absint( $course_id );
 ...
 * @param integer $course_id Course Post ID.
 */
function ebox_user_quiz_has_completed( $user_id = 0, $quiz_id = 0, $course_id = 0 ) {
	$user_id   = absint( $user_id );
	$quiz_id   = absint( $quiz_id );
	$course_id = absint( $course_id );
