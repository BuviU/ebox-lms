	...
	class LDLMS_Factory_Post extends LDLMS_Factory {
		...
		public static function course( $course = null, $reload = false ) {
			if ( ! empty( $course ) ) {
				...
				$course_id = 0;
				if ( ( is_a( $course, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'course' ) === $course->post_type ) ) {
		...
		public static function lesson( $lesson = null, $reload = false ) {
			if ( ! empty( $lesson ) ) {
				...
				$lesson_id = 0;
				if ( ( is_a( $lesson, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'lesson' ) === $lesson->post_type ) ) {
		 ...
		 */
		public static function get_course_modules( $course = null, $lesson = null ) {
			if ( ! empty( $course ) ) {
		...
		public static function quiz_questions( $quiz = null, $reload = false ) {
			if ( ! empty( $quiz ) ) {
				...
				if ( ( is_a( $quiz, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'quiz' ) === $quiz->post_type ) ) {
		...
		public static function exam( $exam = null, $atts = array(), $reload = false ) {
			if ( ! empty( $exam ) ) {
				...
				$exam_id = 0;
				if ( ( is_a( $exam, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'exam' ) === $exam->post_type ) ) {
