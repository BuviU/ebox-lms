<?php
/**
 * ebox User Progress Course Class.
 *
 * @since 3.4.0
 * @package ebox\User\Progression
 */
...
if ( ( ! class_exists( 'LDLMS_Model_User_Course_Progress' ) ) && ( class_exists( 'LDLMS_Model_User' ) ) ) {
	...
	/**
	 * Class for ebox ebox User Progress Course Class.
	 *
	 * @since 3.4.0
	...
	class LDLMS_Model_User_Course_Progress extends LDLMS_Model_User {
		 ...
		 */
		private $progress_meta_key = '_ebox-course_progress';
		...
		public function get_course_progress_legacy( $course_id = 0 ) {
			...
			if ( ( ! isset( $progress_legacy['modules'] ) ) || ( ! is_array( $progress_legacy['modules'] ) ) ) {
				$progress_legacy['modules'] = array();
			}
		 ...
		 *
		 * This is the legacy tree structure used by the '_ebox-course_progress'
		 * user_meta key.
		 *
		 * @since 3.4.0
		 *
		...
		protected function build_course_progress_legacy( $course_id = 0 ) {
			...
			if ( ! empty( $course_id ) ) {
				...
				$steps_legacy    = ebox_course_get_steps_by_type( $course_id, 'legacy' );
				 ...
				 */
				if ( ( isset( $steps_legacy['modules'] ) ) && ( ! empty( $steps_legacy['modules'] ) ) ) {

					foreach ( $steps_legacy['modules'] as $lesson_id => $lesson_id_status ) {
						if ( ( isset( $progress_legacy['modules'][ $lesson_id ] ) ) && ( $progress_legacy['modules'][ $lesson_id ] ) ) {
							$steps_legacy['modules'][ $lesson_id ] = $progress_legacy['modules'][ $lesson_id ];
						}
					}
					$progress_legacy['modules'] = $steps_legacy['modules'];
				}
		...
		protected function build_course_progress_completion_order( $course_id = 0 ) {
			...
			if ( ! empty( $course_id ) ) {
				...
				$course_steps_co = ebox_course_get_steps_by_type( $course_id, 'co' );
				if ( ! empty( $course_steps_co ) ) {
					$lesson_slug = ebox_get_post_type_slug( 'lesson' );
					$topic_slug  = ebox_get_post_type_slug( 'topic' );
					$quiz_slug   = ebox_get_post_type_slug( 'quiz' );
					...
					foreach ( $course_steps_co as $course_step ) {
						...
						if ( in_array( $s_post_type, array( $lesson_slug, $topic_slug ), true ) ) {
							...
							$has_completed_quiz = ebox_user_quiz_has_completed( $this->user_id, $s_post_id, $course_id );

							$progress_co[ $course_step ] = (int) $has_completed_quiz;
						}
					}

					if ( ( isset( $progress_legacy['modules'] ) ) && ( ! empty( $progress_legacy['modules'] ) ) ) {
						foreach ( $progress_legacy['modules'] as $lesson_id => $status ) {
		...
		protected function build_course_progress_linear_order( $course_id = 0 ) {
			...
			if ( ! empty( $course_id ) ) {
				...
				$course_steps_co = ebox_course_get_steps_by_type( $course_id, 'l' );
				if ( ! empty( $course_steps_co ) ) {
					$lesson_slug = ebox_get_post_type_slug( 'lesson' );
					$topic_slug  = ebox_get_post_type_slug( 'topic' );
					$quiz_slug   = ebox_get_post_type_slug( 'quiz' );
			...
			if ( ! empty( $course_id ) ) {
				if ( ( isset( $this->progress[ $course_id ]['co'] ) ) && ( ! empty( $this->progress[ $course_id ]['co'] ) ) ) {
					foreach ( $this->progress[ $course_id ]['co'] as $progress_step_key => $status ) {
						...
						if ( in_array( $progress_step_post_type, ebox_get_post_type_slug( array( 'lesson', 'topic' ) ), true ) ) {
							$course_completed_count += (int) $status;
						}
					}
				}

				if ( ( ebox_has_global_quizzes( $course_id ) ) && ( ebox_is_all_global_quizzes_complete( $this->user_id, $course_id ) ) ) {
		...
		protected function build_course_progress_summary( $course_id = 0 ) {
			...
			if ( ! empty( $course_id ) ) {
				...
				$progress_summary['total']     = (int) ebox_course_get_steps_count( $course_id );
				...
				if ( ! empty( $activity_items ) ) {
					foreach ( $activity_items as $activity_item ) {
						...
						if ( ( ! empty( $activity_item['post_id'] ) ) && ( ! empty( $activity_item['activity_type'] ) ) ) {
							$activity_item_post_type = ebox_get_post_type_slug( $activity_item['activity_type'] );
							if ( in_array( $activity_item_post_type, ebox_get_post_types( 'course' ), true ) ) {
 ...
 */
function ebox_user_get_course_progress( $user_id = 0, $course_id = 0, $type = 'legacy' ) {
	$user_id   = absint( $user_id );
	$course_id = absint( $course_id );
 ...
 *                array should match 'legacy' structure.
 */
function ebox_user_set_course_progress( $user_id = 0, $course_id = 0, $progress = array() ) {
	$user_id   = absint( $user_id );
	$course_id = absint( $course_id );
 ...
 */
function ebox_user_progress_get_previous_incomplete_step( $user_id = 0, $course_id = 0, $step_id = 0, $return_parent_id = true ) {
	$user_id = absint( $user_id );
	if ( empty( $user_id ) ) {
		return false;
	}
		...
		foreach ( $course_progress_steps as $progress_step_key => $progress_status ) {
			if ( $step_key === $progress_step_key ) {
				...
				if ( true === $return_parent_id ) {
					 ...
					 * if previous modules are completed. If we only returned the incomplete step id it
					 * was cause the alert banner to show on the lesson instead of the lesson steps table.
					 */
					$parent_steps = ebox_course_get_all_parent_step_ids( $course_id, $progress_step_post_id );
 ...
 */
function ebox_user_progress_get_next_incomplete_step( $user_id = 0, $course_id = 0, $step_id = 0 ) {
	$user_id = absint( $user_id );
	if ( empty( $user_id ) ) {
		return false;
	}
 ...
 */
function ebox_user_progress_get_all_incomplete_steps( $user_id = 0, $course_id = 0 ) {
	$user_id = absint( $user_id );
	if ( empty( $user_id ) ) {
		return false;
	}
 ...
 */
function ebox_user_progress_get_first_incomplete_step( $user_id = 0, $course_id = 0 ) {
	$step_id = 0;
	...
	$incomplete_steps = ebox_user_progress_get_all_incomplete_steps( $user_id, $course_id );
 ...
 */
function ebox_user_progression_get_incomplete_child_steps( $user_id = 0, $course_id = 0, $step_id = 0 ) {
	...
	$child_steps = ebox_course_get_children_of_step( $course_id, $step_id, '', 'ids', true );
	if ( ! empty( $child_steps ) ) {
		foreach ( $child_steps as $child_step_id ) {
			if ( true !== ebox_user_progress_is_step_complete( $user_id, $course_id, $child_step_id ) ) {
 ...
 */
function ebox_user_progression_get_complete_child_steps( $user_id = 0, $course_id = 0, $step_id = 0 ) {
 ...
 */
function ebox_user_progress_is_step_complete( $user_id = 0, $course_id = 0, $step_id = 0 ) {
	$user_id = absint( $user_id );
	if ( empty( $user_id ) ) {
		return false;
	}
 ...
 */
function ebox_user_progress_get_parent_incomplete_step( $user_id = 0, $course_id = 0, $step_id = 0 ) {
	$user_id = absint( $user_id );
	if ( empty( $user_id ) ) {
		return false;
	}
	 ...
	 */
	$step_parent_ids = ebox_course_get_all_parent_step_ids( $course_id, $step_id );
	if ( ( is_array( $step_parent_ids ) ) && ( ! empty( $step_parent_ids ) ) ) {
		foreach ( $step_parent_ids as $step_parent_id ) {
			if ( in_array( get_post_type( $step_parent_id ), ebox_get_post_type_slug( array( 'lesson', 'topic' ) ), true ) ) {
				if ( ! ebox_user_progress_is_step_complete( $user_id, $course_id, $step_parent_id ) ) {
					if ( 'on' === ebox_get_setting( $step_parent_id, 'lesson_video_enabled' ) ) {
						if ( ! empty( ebox_get_setting( $step_parent_id, 'lesson_video_url' ) ) ) {
							if ( 'BEFORE' === ebox_get_setting( $step_parent_id, 'lesson_video_shown' ) ) {
								if ( ! ebox_video_complete_for_step( $step_parent_id, $course_id, $user_id ) ) {
