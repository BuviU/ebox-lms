<?php
/**
 * ebox Course Steps Class.
 *
 * @since 2.5.0
 * @package ebox\Course\Steps
 */
	...
	class LDLMS_Course_Steps extends LDLMS_Model {
		...
		public function __construct( $course_id = 0 ) {
			if ( ! empty( $course_id ) ) {
				...
				$this->steps_post_types = ebox_get_post_types( 'course_steps' );
			}
		}
		...
		protected function load_steps_meta() {
			...
			if ( ( isset( $this->meta['course_builder_enabled'] ) ) && ( true === (bool) $this->meta['course_builder_enabled'] ) ) {
				if ( ! (bool) ebox_is_course_builder_enabled() ) {
					$this->meta['steps'] = array();
					$save_after_load     = true;
				}
			}
			...
			if ( ( isset( $this->meta['course_shared_steps_enabled'] ) ) && ( true === (bool) $this->meta['course_shared_steps_enabled'] ) ) {
				if ( ! (bool) ebox_is_course_shared_steps_enabled() ) {
					$this->meta['steps'] = array();
					$save_after_load     = true;
				}
			}
			...
			$this->meta['version']   = ebox_VERSION;
			...
			$this->meta['course_builder_enabled']      = (bool) ebox_is_course_builder_enabled();
			$this->meta['course_shared_steps_enabled'] = (bool) ebox_is_course_shared_steps_enabled();
		...
		protected function calculate_steps_count() {
			...
			if ( ( isset( $this->steps['h'] ) ) && ( ! empty( $this->steps['h'] ) ) ) {
				...
				if ( ! isset( $this->steps['t'] ) ) {
					$this->steps['t'] = $this->steps_teamed_by_type( $this->steps['h'] );
				}

				// All modules.
				if ( isset( $this->steps['t']['ebox-modules'] ) ) {
					$steps_count += count( $this->steps['t']['ebox-modules'] );
				}

				// All topics.
				if ( isset( $this->steps['t']['ebox-topic'] ) ) {
					$steps_count += count( $this->steps['t']['ebox-topic'] );
				}

				// Any Global Quizzes only count as 1.
				if ( ( isset( $this->steps['h']['ebox-quiz'] ) ) && ( ! empty( $this->steps['h']['ebox-quiz'] ) ) ) {
					$steps_count++;
				}
			}
		...
		protected function build_steps() {
			...
			if ( ! empty( $this->steps['h'] ) ) {
				...
				if ( empty( $this->steps['l'] ) ) {
					$this->steps['l'] = $this->steps_teamed_linear( $this->steps['h'] );
				}

				if ( empty( $this->steps['co'] ) ) {
					$this->steps['co'] = $this->steps_teamed_completion_order( $this->steps['h'] );
				}

				if ( empty( $this->steps['r'] ) ) {
					$this->steps['r'] = $this->steps_teamed_reverse_keys( $this->steps['h'] );
				}

				if ( empty( $this->steps['sections'] ) ) {
					$this->steps['sections'] = $this->steps_teamed_sections( $this->steps['h'] );
				}

				if ( empty( $this->steps['legacy'] ) ) {
					$this->steps['legacy'] = $this->steps_teamed_legacy( $this->steps['h'] );
					if ( ! isset( $this->steps['legacy']['modules'] ) ) {
						$this->steps['legacy']['modules'] = array();
					}
		...
		/**
		 * This converts the normal hierarchy steps into an array teams be the post type. This is easier for search.
		 *
		 * @since 2.5.0
		 *
		 ...
		 */
		protected function steps_teamed_by_type( $steps = array() ) {
			$steps_by_type = array();
					...
					if ( ( is_array( $steps_type_set ) ) && ( ! empty( $steps_type_set ) ) ) {
						foreach ( $steps_type_set as $step_id => $step_id_set ) {
							...
							if ( ( is_array( $step_id_set ) ) && ( ! empty( $step_id_set ) ) ) {
								$sub_steps = $this->steps_teamed_by_type( $step_id_set );
								if ( ! empty( $sub_steps ) ) {
		 ...
		 */
		protected function steps_teamed_linear( $steps = array() ) {
			$steps_linear = array();
				...
				foreach ( $steps as $steps_type => $steps_type_set ) {
					if ( ( is_array( $steps_type_set ) ) && ( ! empty( $steps_type_set ) ) ) {
						foreach ( $steps_type_set as $step_id => $step_id_set ) {
							...
							if ( ( is_array( $step_id_set ) ) && ( ! empty( $step_id_set ) ) ) {
								$sub_steps = $this->steps_teamed_linear( $step_id_set );
								if ( ! empty( $sub_steps ) ) {
		 ...
		 */
		protected function steps_teamed_legacy( $steps = array() ) {
			$steps_legacy = array();
			...
			if ( ! empty( $steps ) ) {
				foreach ( $steps as $steps_type => $steps_type_set ) {
					if ( ( is_array( $steps_type_set ) ) && ( ! empty( $steps_type_set ) ) ) {
						foreach ( $steps_type_set as $step_id => $step_id_set ) {
							if ( ebox_get_post_type_slug( 'lesson' ) === $steps_type ) {
								if ( ! isset( $steps_legacy['modules'] ) ) {
									$steps_legacy['modules'] = array();
								}
								$steps_legacy['modules'][ $step_id ] = 0;

								if ( ! isset( $steps_legacy['topics'] ) ) {
									$steps_legacy['topics'] = array();
								}
								$steps_legacy['topics'][ $step_id ] = $this->steps_teamed_legacy( $step_id_set );

								if ( ! isset( $steps_legacy['total'] ) ) {
									$steps_legacy['total'] = $this->calculate_steps_count();
								}
							} elseif ( ebox_get_post_type_slug( 'topic' ) === $steps_type ) {
		...
		/**
		 * Steps teamed by Completion Order.
		 *
		 * @since 3.4.0
		 *
		 * @param array $steps Array of Course step nodes and items.
		 ...
		 */
		protected function steps_teamed_completion_order( $steps = array() ) {
			$steps_co = array();
				...
				foreach ( $steps as $steps_type => $steps_type_set ) {
					if ( ( is_array( $steps_type_set ) ) && ( ! empty( $steps_type_set ) ) ) {
						foreach ( $steps_type_set as $step_id => $step_id_set ) {
							...
							if ( ( is_array( $step_id_set ) ) && ( ! empty( $step_id_set ) ) ) {
								$sub_steps = $this->steps_teamed_completion_order( $step_id_set );
								if ( ! empty( $sub_steps ) ) {
		...
		/**
		 * Team Steps by Section.
		 *
		 * @since 3.4.0
		 *
		 * @param array $steps Array of Course sections nodes and related modules.
		 *
		 * @return array Array of sections.
		 */
		protected function steps_teamed_sections( $steps = array() ) {
			$sections = array();
			...
			$modules = array_keys( $steps['ebox-modules'] );
			...
			foreach ( $sections_array as $section ) {
				...
				array_splice( $modules, (int) $section->order, 0, array( $section ) );
			}
		...
		protected function load_steps_objects() {
			 ...
			 */
			$steps_query_args = apply_filters( 'ebox_course_steps_objects_query_args', $steps_query_args, $this->course_id );
			...
			if ( ( count( $all_objects_ids ) !== count( $all_steps_ids ) ) ) {
						...
						if ( ! empty( $all_diff_ids ) ) {
							foreach ( $all_diff_ids as $all_diff_id ) {
								if ( ( isset( $this->objects[ $all_diff_id ] ) ) && ( is_a( $this->objects[ $all_diff_id ], 'WP_Post' ) ) ) {
									 ...
									 */
									if ( ebox_get_post_type_slug( 'topic' ) === $diff_post->post_type ) {
										$valid_post = true;
										...
										$course_id_setting   = (int) ebox_get_setting( $diff_post->ID, 'course' );
										...
										if ( ( empty( $course_id_post_meta ) ) || ( empty( $course_id_setting ) ) || ( $course_id_setting !== $course_id_post_meta ) ) {
											...
											$lesson_id_setting   = (int) ebox_get_setting( $diff_post->ID, 'lesson' );
										...
										if ( true !== $valid_post ) {
											ebox_update_setting( $diff_post->ID, 'course', 0 );
											ebox_update_setting( $diff_post->ID, 'lesson', 0 );
										}
									}
								}
							}
				...
				if ( ! empty( $all_add_ids ) ) {
					...
					$all_steps_chunks_ids = array_chunk( $all_add_ids, ebox_LMS_COURSE_STEPS_LOAD_BATCH_SIZE );
		...
		protected function get_all_steps_ids() {
			...
			if ( isset( $this->steps['t'] ) ) {
				...
				if ( isset( $this->steps['h'] ) ) {
					$steps_by_type = $this->steps_teamed_by_type( $this->steps['h'] );
				}
			}

			if ( ! empty( $steps_by_type ) ) {
				if ( isset( $steps_by_type['ebox-modules'] ) ) {
					$all_steps_ids = array_merge( $all_steps_ids, $steps_by_type['ebox-modules'] );
				}
				if ( isset( $steps_by_type['ebox-topic'] ) ) {
					$all_steps_ids = array_merge( $all_steps_ids, $steps_by_type['ebox-topic'] );
				}
				if ( isset( $steps_by_type['ebox-quiz'] ) ) {
					$all_steps_ids = array_merge( $all_steps_ids, $steps_by_type['ebox-quiz'] );
				}
			}
		 ...
		 */
		protected function steps_teamed_reverse_keys( $steps = array() ) {
		...
		public function set_step_to_course() {
			...
			if ( ! empty( $course_steps_add ) ) {
				$course_steps_add_chunks = array_chunk( $course_steps_add, ebox_LMS_DEFAULT_CB_INSERT_CHUNK_SIZE );
		...
		protected function set_step_to_course_legacy() {
			...
			ebox_update_setting( $this->course_id, 'course_lesson_orderby', 'menu_order' );
			ebox_update_setting( $this->course_id, 'course_lesson_order', 'ASC' );
			...
			if ( ! empty( $course_steps_add ) ) {
				foreach ( $course_steps_add as $post_id ) {
					ebox_update_setting( $post_id, 'course', $this->course_id );
				}
			}
			...
			if ( ! empty( $course_steps_remove ) ) {
				foreach ( $course_steps_remove as $post_id ) {
					ebox_update_setting( $post_id, 'course', 0 );
					ebox_update_setting( $post_id, 'lesson', 0 );
				}
			}
		...
		private function set_step_to_course_relationship( $steps = array(), $parent_lesson_id = 0 ) {
			...
			if ( ! empty( $steps ) ) {
				foreach ( $steps as $steps_type => $steps_type_set ) {
					...
					} elseif ( ( ebox_get_post_type_slug( 'quiz' ) === $steps_type ) && ( 0 === $parent_lesson_id ) ) {
					...
					if ( ! empty( $sql_str ) ) {
						...
						if ( ( ! empty( $step_type_ids_add ) ) && ( ! empty( $parent_lesson_id ) ) ) {
							foreach ( $step_type_ids_add as $post_id ) {
								ebox_update_setting( $post_id, 'lesson', $parent_lesson_id );
							}
						}
		...
		private function set_step_to_course_order( $steps = array() ) {
			...
			if ( ! empty( $steps ) ) {
				...
				foreach ( $steps as $step_order => $step_id ) {
					...
					if ( ( defined( 'ebox_BUILDER_STEPS_UPDATE_POST' ) ) && ( true === ebox_BUILDER_STEPS_UPDATE_POST ) ) {
						$edit_post = array(
							'ID'         => $step_id,
		...
		private function load_steps_legacy() {
			...
			if ( ! empty( $this->course_id ) ) {
				...
				if ( ( true === $this->meta['course_builder_enabled'] ) && ( true !== $this->meta['empty'] ) ) {
					...
					$course_lesson_order = ebox_get_course_modules_order( $this->course_id );
				}
				...
				if ( ( is_a( $lesson_steps_query, 'WP_Query' ) ) && ( property_exists( $lesson_steps_query, 'posts' ) ) && ( ! empty( $lesson_steps_query->posts ) ) ) {
					...
					foreach ( $lesson_steps_query->posts as $lesson ) {
						$steps[ ebox_get_post_type_slug( 'lesson' ) ][ $lesson->ID ] = array();

						$steps[ ebox_get_post_type_slug( 'lesson' ) ][ $lesson->ID ][ ebox_get_post_type_slug( 'topic' ) ] = array();
						$steps[ ebox_get_post_type_slug( 'lesson' ) ][ $lesson->ID ][ ebox_get_post_type_slug( 'quiz' ) ]  = array();
						...
						if ( ( is_a( $topic_steps_query, 'WP_Query' ) ) && ( property_exists( $topic_steps_query, 'posts' ) ) && ( ! empty( $topic_steps_query->posts ) ) ) {
							foreach ( $topic_steps_query->posts as $topic ) {
								$steps[ ebox_get_post_type_slug( 'lesson' ) ][ $lesson->ID ][ ebox_get_post_type_slug( 'topic' ) ][ $topic->ID ] = array();
								$steps[ ebox_get_post_type_slug( 'lesson' ) ][ $lesson->ID ][ ebox_get_post_type_slug( 'topic' ) ][ $topic->ID ][ ebox_get_post_type_slug( 'quiz' ) ] = array();
								...
								if ( ( is_a( $topic_quiz_steps_query, 'WP_Query' ) ) && ( property_exists( $topic_quiz_steps_query, 'posts' ) ) && ( ! empty( $topic_quiz_steps_query->posts ) ) ) {
									foreach ( $topic_quiz_steps_query->posts as $quiz ) {
										$steps[ ebox_get_post_type_slug( 'lesson' ) ][ $lesson->ID ][ ebox_get_post_type_slug( 'topic' ) ][ $topic->ID ][ ebox_get_post_type_slug( 'quiz' ) ][ $quiz->ID ] = array();
						...
						if ( ( is_a( $lesson_quiz_steps_query, 'WP_Query' ) ) && ( property_exists( $lesson_quiz_steps_query, 'posts' ) ) && ( ! empty( $lesson_quiz_steps_query->posts ) ) ) {
							foreach ( $lesson_quiz_steps_query->posts as $quiz ) {
								$steps[ ebox_get_post_type_slug( 'lesson' ) ][ $lesson->ID ][ ebox_get_post_type_slug( 'quiz' ) ][ $quiz->ID ] = array();
					...
					$steps[ ebox_get_post_type_slug( 'lesson' ) ] = array();
				}
				...
				if ( ! empty( $this->objects ) ) {
					foreach ( $this->objects as $step_object ) {
						if ( ebox_get_post_type_slug( 'quiz' ) === $step_object->post_type ) {
				...
				if ( ( is_a( $quiz_steps_query, 'WP_Query' ) ) && ( property_exists( $quiz_steps_query, 'posts' ) ) && ( ! empty( $quiz_steps_query->posts ) ) ) {
					foreach ( $quiz_steps_query->posts as $quiz ) {
						$steps[ ebox_get_post_type_slug( 'quiz' ) ][ $quiz->ID ] = array();

						$this->objects[ $quiz->ID ] = $quiz;
					}
				} else {
					$steps[ ebox_get_post_type_slug( 'quiz' ) ] = array();
				}
			}
		...
		public function get_children_steps( $parent_post_id = 0, $return_post_type = '', $return_type = 'ids', $nested = false ) {
			...
			if ( ! empty( $parent_post_id ) ) {
				...
				/**
				 * Here we need to drill through the top-level step (modules) to get ot our parent_post_type
				 */
		...
		public static function steps_split_keys( $steps, $parent_type = '' ) {
			if ( ebox_get_post_type_slug( 'lesson' ) === $parent_type ) {
				$course_steps_split_keys = array(
					'ebox-topic' => array(),
					'ebox-quiz'  => array(),
				);
			} elseif ( ebox_get_post_type_slug( 'topic' ) === $parent_type ) {
				$course_steps_split_keys = array(
					'ebox-quiz' => array(),
				);
			} elseif ( ebox_get_post_type_slug( 'quiz' ) === $parent_type ) {
				$course_steps_split_keys = array();
		...
		protected function get_step_post_statuses() {
			 ...
			 */
			return apply_filters( 'ebox_course_steps_post_status_keys', $post_status_keys );
		}
	}
}
