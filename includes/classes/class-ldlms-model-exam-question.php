...
if ( ( class_exists( 'LDLMS_Model_Post' ) ) && ( ! class_exists( 'LDLMS_Model_Exam_Question' ) ) ) {
	...
	class LDLMS_Model_Exam_Question extends LDLMS_Model_Post {
		...
		private function init() {
			...
			$this->question_block['attrs']['ebox_question_model'] = $this;
			...
			if ( ( isset( $this->question_block['innerBlocks'] ) ) && ( is_array( $this->question_block['innerBlocks'] ) ) ) {
				foreach ( $this->question_block['innerBlocks'] as &$question_block_inner ) {
					$question_block_inner['attrs']['ebox_exam_model']     = $this->question_block['attrs']['ebox_exam_model'];
					$question_block_inner['attrs']['ebox_question_model'] = $this;
		...
		public function __get( $property = '' ) {
			...
			switch ( $property ) {
				...
				case 'exam_model':
					if ( isset( $this->question_block['attrs']['ebox_exam_model'] ) ) {
						$return_val = $this->question_block['attrs']['ebox_exam_model'];
					}
					break;
		...
		private static function get_types() {
			return array(
				'single'   => array(
					'key'   => 'single',
					'label' => esc_html__( 'Single Choice', 'ebox' ),
					'model' => 'LDLMS_Model_Exam_Question_Single',
				),
				'multiple' => array(
					'key'   => 'multiple',
					'label' => esc_html__( 'Multiple Choice', 'ebox' ),
