...
if ( ( class_exists( 'LDLMS_Model_Post' ) ) && ( ! class_exists( 'LDLMS_Model_Exam' ) ) ) {
	...
	class LDLMS_Model_Exam extends LDLMS_Model_Post {
		...
		public function __construct( $post_id = 0, $atts = array() ) {
			$this->post_type = ebox_get_post_type_slug( 'exam' );
		...
		public function get_result_message( $format = true ) {
			...
			if ( true === $this->exam_is_graded ) {
				if ( true === $this->exam_grade ) {
					$result_message = ebox_get_setting( $this->post_id, 'message_passed' );
				} else {
					$result_message = ebox_get_setting( $this->post_id, 'message_failed' );
				}
		...
		public function get_result_button_params() {
			...
			if ( true === $this->exam_is_graded ) {
				if ( true === $this->exam_grade ) {
					...
					$exam_result_button['button_label'] = ebox_get_setting( $this->post->ID, 'exam_passed_button_label' );
					$exam_result_button['redirect_url'] = ebox_get_setting( $this->post->ID, 'exam_passed_redirect_url' );

					if ( empty( $exam_result_button['button_label'] ) ) {
						$exam_result_button['button_label'] = esc_html__( 'Proceed', 'ebox' );
					}

					if ( empty( $exam_result_button['redirect_url'] ) ) {
						$exam_challenge_course_passed = ebox_get_setting( $this->post_id, 'exam_challenge_course_passed' );
						if ( empty( $exam_challenge_course_passed ) ) {
							$exam_challenge_course_show = ebox_get_setting( $this->post_id, 'exam_challenge_course_show' );
					...
					$exam_result_button['button_label'] = ebox_get_setting( $this->post->ID, 'exam_failed_button_label' );
					$exam_result_button['redirect_url'] = ebox_get_setting( $this->post->ID, 'exam_failed_redirect_url' );
				...
				*/
				$exam_result_button = apply_filters(
					'ebox_exam_challenge_to_course_passed_redirect',
					$exam_result_button,
					$this->post->ID,
					$exam_status_slug
				);
		...
		public function get_front_content() {
			...
			if ( ! empty( $questions_output ) ) {
				$exam_output .= ebox_LMS::get_template(
					'exam/partials/exam_result_message.php',
					array(
						'ebox_exam_model' => $this,
					)
				);
		...
		public function load_question_models_from_post_content() {
			...
			if ( false === $questions_loaded ) {
				...
				foreach ( $content_blocks as  &$content_block ) {
					if ( 'ebox/ld-exam' === $content_block['blockName'] ) {
						if ( ( isset( $content_block['innerBlocks'] ) ) && ( is_array( $content_block['innerBlocks'] ) ) && ( ! empty( $content_block['innerBlocks'] ) ) ) {
							...
							foreach ( $content_block['innerBlocks'] as &$exam_inner_block ) {
								if ( 'ebox/ld-exam-question' === $exam_inner_block['blockName'] ) {
									...
									$exam_inner_block['attrs']['ebox_exam_model'] = $this;
		...
		protected function set_exam_activity() {
			...
			$exam_activity = ebox_get_user_activity( $activity_args, true );
				...
				foreach ( $this->question_models as $question_model ) {
					...
					if ( ( isset( $question_block['innerBlocks'] ) ) && ( is_array( $question_block['innerBlocks'] ) ) && ( ! empty( $question_block['innerBlocks'] ) ) ) {
						foreach ( $question_block['innerBlocks'] as &$question_block_inner ) {
							if ( 'ebox/ld-question-answers-block' === $question_block_inner['blockName'] ) {
		...
		protected function set_course_complete(): bool {
			if ( true === $this->exam_grade ) {
				$exam_challenge_passed = ebox_get_setting( $this->post_id, 'exam_challenge_course_passed' );
				if ( empty( $exam_challenge_passed ) ) {
					$exam_challenge_passed = $this->course_id;
				}

				if ( ! empty( $exam_challenge_passed ) ) {
					$course_access = ebox_lms_has_access( $exam_challenge_passed, $this->user_id );
					if ( true !== $course_access ) {
						ld_update_course_access( $this->user_id, $exam_challenge_passed );
					}

					if ( ! ebox_course_completed( $this->user_id, $exam_challenge_passed ) ) {
						return ebox_user_course_complete_all_steps( $this->user_id, $exam_challenge_passed );
					}
				}
			}
