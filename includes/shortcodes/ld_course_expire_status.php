<?php
/**
 * ebox `[ld_course_expire_status]` shortcode processing.
 *
 * @since 2.1.0
 ...
 */
function ebox_course_expire_status_shortcode( $atts = array(), $content = '', $shortcode_slug = 'ld_course_expire_status' ) {
	...
	$atts = shortcode_atts(
		array(
			...
			'label_before' => sprintf( esc_html_x( '%s access will expire on:', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) ),
			// translators: placeholder: Course.
			'label_after'  => sprintf( esc_html_x( '%s access expired on:', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) ),
	 ...
	 */
	$atts = apply_filters( 'ebox_ld_course_expire_status_shortcode_atts', $atts );
	...
	if ( ( ! empty( $atts['course_id'] ) ) && ( ! empty( $atts['user_id'] ) ) ) {
		if ( ebox_lms_has_access( $atts['course_id'], $atts['user_id'] ) ) {
			...
			if ( empty( $courses_access_from ) ) {
				$courses_access_from = ebox_user_team_enrolled_to_course_from( $atts['user_id'], $atts['course_id'] );
			}
			...
			if ( ! empty( $courses_access_from ) ) {
				...
				if ( ! empty( $atts['expires_on_timestamp'] ) ) {
					...
					$atts['expires_on_formatted'] = ebox_adjust_date_time_display( $atts['expires_on_timestamp'], $atts['format'] );

					$content_shortcode .= ' <span class="ebox-course-expire-status-date ebox-course-expire-status-date-expires-on">' . $atts['expires_on_formatted'] . '</span>';
				}
			}
		} else {

			$atts['expired_on_timestamp'] = get_user_meta( $atts['user_id'], 'ebox_course_expired_' . $atts['course_id'], true );

			if ( ! empty( $atts['expired_on_timestamp'] ) ) {

				$content_shortcode .= $atts['label_after'];

				$atts['expired_on_formatted'] = ebox_adjust_date_time_display( $atts['expired_on_timestamp'], $atts['format'] );

				$content_shortcode .= ' <span class="ebox-course-expire-status-date ebox-course-expire-status-date-expired-on">' . $atts['expired_on_formatted'] . '</span>';
		...
		if ( ! empty( $content_shortcode ) ) {
			...
			$atts['content'] = do_shortcode( $content_shortcode );
				return ebox_LMS::get_template(
					'ebox_course_expire_status_message',
					array(
						'shortcode_atts' => $atts,
					),
					false
...
add_shortcode( 'ld_course_expire_status', 'ebox_course_expire_status_shortcode', 10, 3 );
