<?php
/**
 * ebox `[ld_quiz_list]` shortcode processing.
 *
 * @since 2.1.0
 *
...
function ld_quiz_list( $attr = array(), $content = '', $shortcode_slug = 'ld_quiz_list' ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
	...
	$attr['post_type'] = ebox_get_post_type_slug( 'quiz' );
	...
	if ( ( isset( $attr['course_id'] ) ) && ( ! empty( $attr['course_id'] ) ) || isset( $attr['lesson_id'] ) && ( ! empty( $attr['lesson_id'] ) ) ) {
		...
		if ( isset( $attr['lesson_id'] ) ) {
			...
			if ( ! empty( $attr['lesson_id'] ) ) {
				$course_steps = ebox_get_lesson_quiz_list( $attr['lesson_id'], null, $attr['course_id'] );
				if ( ! empty( $course_steps ) ) {
					$course_steps = wp_list_pluck( $course_steps, 'post' );
					$course_steps = wp_list_pluck( $course_steps, 'ID' );
				}
			} else {
				$course_steps = ebox_get_global_quiz_list( $attr['course_id'] );
				if ( ! empty( $course_steps ) ) {
					$course_steps = wp_list_pluck( $course_steps, 'ID' );
				}
			}
			...
			if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
				$course_steps = ebox_course_get_steps_by_type( intval( $attr['course_id'] ), $attr['post_type'] );
			}
		}
