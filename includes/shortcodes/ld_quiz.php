<?php
/**
 * ebox `[ld_quiz]` shortcode processing.
 *
 * @since 2.1.0
 *
 ...
 * @since 4.0.0
 *
 * @param array  $atts           See `ebox_quiz_shortcode_function` function for parameter details.
 * @param string $content        See `ebox_quiz_shortcode_function` function for parameter details.
 * @param bool   $show_materials Whether to show quiz materials. Default false.
 */
function ebox_quiz_shortcode( $atts = array(), $content = '', $show_materials = false ) {
	$atts['show_materials'] = $show_materials;

	return ebox_quiz_shortcode_function( $atts, $content );
}
 ...
 * @global array   $ebox_shortcode_atts
 *
 * @param array  $atts {
 ...
 */
function ebox_quiz_shortcode_function( $atts = array(), $content = '', $shortcode_slug = 'ld_quiz' ) {
	...
	if ( empty( $course_id ) ) {
		if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) !== 'yes' ) {
			$course_id = ebox_get_setting( $quiz_post, 'course' );
	...
	$ebox_shortcode_atts['ld_quiz'] = $atts;
	// Clear out any previous 'LDAdvQuiz' data.
	if ( isset( $ebox_shortcode_atts['LDAdvQuiz'] ) ) {
		unset( $ebox_shortcode_atts['LDAdvQuiz'] );
	}
	$ebox_shortcode_used = true;

	$lesson_progression_enabled = false;
	if ( ! empty( $atts['course_id'] ) ) {
		$lesson_progression_enabled = ebox_lesson_progression_enabled( $atts['course_id'] );
	}
	...
	if ( $quiz_post instanceof WP_Post ) {
		$quiz_settings = ebox_get_setting( $atts['quiz_id'] );
		$meta          = ebox_CPT_Instance::$instances['ebox-quiz']->get_settings_values( 'ebox-quiz' );

		if ( ( ! empty( $course_id ) ) && ( ! empty( $user_id ) ) ) {
			$has_access = ebox_lms_has_access( $course_id, $user_id );
		}

		$show_content   = ! ( ! empty( $lesson_progression_enabled ) && ! ebox_is_quiz_accessable( $user_id, $quiz_post, false, $course_id ) ); // cspell:disable-line.
		$attempts_count = 0;
			...
			if ( ! empty( $quiz_settings['quiz_pro'] ) ) {
				...
				if ( ( $pro_quiz_edit ) && ( is_a( $pro_quiz_edit, 'WpProQuiz_Model_Quiz' ) ) ) {
					...
					if ( $pro_quiz_edit->isQuizRunOnce() ) {
						$repeats = 0;
						// Update for later.
						ebox_update_setting( $quiz_post, 'repeats', $repeats );
					}
				}
			}
		}
		...
		$bypass_course_limits_admin_users = ebox_can_user_bypass( $user_id, 'ebox_course_lesson_access_from', $course_id, $quiz_post );

		// For logged in users to allow an override filter.
		/** This filter is documented in includes/course/ld-course-progress.php */
		$bypass_course_limits_admin_users = apply_filters( 'ebox_prerequities_bypass', $bypass_course_limits_admin_users, $user_id, $course_id, $quiz_post ); // cspell:disable-line -- prerequities are prerequisites...
		 ...
		 *
		 * See example https://developers.ebox.com/hook/ebox_quiz_attempts/
		 *
		 * @since 3.1.0
		 *
		 ...
		 */
		$attempts_left = apply_filters( 'ebox_quiz_attempts', $attempts_left, absint( $attempts_count ), absint( $user_id ), absint( $quiz_post->ID ) );
		$attempts_left = absint( $attempts_left );

		if ( ! empty( $lesson_progression_enabled ) && ! ebox_is_quiz_accessable( $user_id, $quiz_post, false, $course_id ) ) { // cspell:disable-line.
		 ...
		 */
		$access_message = apply_filters( 'ebox_content_access', '', $quiz_post );
		if ( ! empty( $access_message ) ) {
			...
			if ( ! empty( $quiz_settings['quiz_pro'] ) ) {
				...
				if ( ( ! empty( $course_id ) ) && ( ! empty( $quiz_id ) ) ) {
					...
					$course_activity = ebox_activity_start_course( $user_id, $course_id, $activity_started_time );
					if ( $course_activity ) {
						ebox_activity_update_meta_set(
							$course_activity->activity_id,
							array(
								'steps_completed' => ebox_course_get_completed_steps( $user_id, $course_id ),
								'steps_last_id'   => $quiz_id,
							)
						);
					}
					$quiz_settings['lesson'] = ebox_course_get_single_parent_step( $course_id, $quiz_id, ebox_get_post_type_slug( 'lesson' ) );
					$quiz_settings['lesson'] = absint( $quiz_settings['lesson'] );
					if ( ! empty( $quiz_settings['lesson'] ) ) {
						ebox_activity_start_lesson( $user_id, $course_id, $quiz_settings['lesson'], $activity_started_time );
					}

					$quiz_settings['topic'] = ebox_course_get_single_parent_step( $course_id, $quiz_id, ebox_get_post_type_slug( 'topic' ) );
					$quiz_settings['topic'] = absint( $quiz_settings['topic'] );
					if ( ! empty( $quiz_settings['topic'] ) ) {
						ebox_activity_start_topic( $user_id, $course_id, $quiz_settings['topic'], $activity_started_time );
					}
				}
			 ...
			 * @param WP_Post $quiz_post    Quiz WP_Post object.
			 */
			$quiz_content = apply_filters( 'ebox_quiz_content', $quiz_content, $quiz_post );
		}

		$level = ob_get_level();
		ob_start();
		$template_file = ebox_LMS::get_template( 'quiz', null, null, true );
		...
		if ( ( defined( 'ebox_NEW_LINE_AND_CR_TO_SPACE' ) ) && ( ebox_NEW_LINE_AND_CR_TO_SPACE == true ) ) {
		...
		$content = '<div class="ebox ' . $user_has_access . '"  id="ebox_post_' . $quiz_post->ID . '">' . apply_filters( 'ebox_content', $content, $quiz_post ) . '</div>';
	}

	return $content;
}
add_shortcode( 'ld_quiz', 'ebox_quiz_shortcode_function', 10, 3 );
