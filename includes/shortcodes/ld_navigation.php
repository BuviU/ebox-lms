<?php
/**
 * ebox `[ld_navigation]` shortcode processing.
 *
 * @since 4.0.0
 ...
 */
function ebox_navigation_shortcode( $atts = array(), $content = '', $shortcode_slug = 'ld_navigation' ) {
	...
	// Only for modules or Topics. Use on Quiz or Course will not display anythings.
	if ( ! empty( $atts['post_id'] ) ) {
		$post_type = get_post_type( $atts['post_id'] );
		if ( ( empty( $post_type ) ) || ( ! in_array( $post_type, ebox_get_post_type_slug( array( 'lesson', 'topic' ) ), true ) ) ) {
			return $content;
		}
	}
	...
	if ( ( ! empty( $atts['course_id'] ) ) && ( ! empty( $atts['post_id'] ) ) ) {
		...
		if ( ! empty( $atts['user_id'] ) ) {
			if ( ( true !== ebox_lesson_progression_enabled( $atts['course_id'] ) ) || ( true === ebox_can_user_bypass( $atts['user_id'], 'ebox_course_progression' ) ) ) {
				$incomplete_child_steps = ebox_user_progression_get_incomplete_child_steps( $atts['user_id'], $atts['course_id'], $atts['post_id'] );
				...
				$can_complete = ebox_can_complete_step( $atts['user_id'], $atts['post_id'], $atts['course_id'], true );
			}
		}
		 ...
		 */
		$can_complete = apply_filters( 'ebox-lesson-can-complete', $can_complete, $atts['post_id'], $atts['course_id'], $atts['user_id'] ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores

		$context = ebox_get_post_type_key( get_post_type( $atts['post_id'] ) );
		...
		ebox_LMS::get_template(
			...
			array(
				...
				'course_settings'  => ebox_get_setting( $atts['course_id'] ),
		...
		if ( ! empty( $shortcode_out ) ) {
			$shown_content[ $shown_content_key ] .= '<div class="ebox-wrapper ebox-shortcode-wrap-' . esc_attr( $shortcode_slug ) . '-' . esc_attr( $shown_content_key ) . '">' . $shortcode_out . '</div>';
		}
	}
...
add_shortcode( 'ld_navigation', 'ebox_navigation_shortcode', 10, 3 );
