<?php
/**
 * ebox `[course_notstarted]` shortcode processing.
 *
 * @since 2.1.0
 ...
 */
function ebox_course_notstarted_shortcode( $atts = array(), $content = '', $shortcode_slug = 'course_notstarted' ) {
	...
	if ( ! empty( $content ) ) {
		 ...
		 */
		$atts = apply_filters( 'ebox_course_notstarted_shortcode_atts', $atts );

		$atts['content'] = ebox_course_status_content_shortcode( $atts, $atts['content'], esc_html__( 'Not Started', 'ebox' ) );
		return ebox_LMS::get_template(
			'ebox_course_not_started_message',
			array(
				'shortcode_atts' => $atts,
			),
			false
...
add_shortcode( 'course_notstarted', 'ebox_course_notstarted_shortcode', 10, 3 );
