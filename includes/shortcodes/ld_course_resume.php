<?php
/**
 * ebox `[ld_course_resume]` shortcode processing.
 *
 * @since 3.1.4
...
function ld_course_resume_shortcode( $atts = array(), $content = '', $shortcode_slug = 'ld_course_resume' ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
	...
	if ( empty( $atts['course_id'] ) ) {
		...
		if ( ( empty( $atts['course_id'] ) ) && ( ! empty( $atts['user_id'] ) ) ) {
			$atts['course_id'] = ebox_get_last_active_course( $atts['user_id'] );
		}
	}
	...
	if ( empty( $atts['label'] ) ) {
		if ( ! empty( $content ) ) {
			...
			$atts['label'] = sprintf( esc_html_x( 'Resume %s', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) );
		}
	}
	 ...
	 */
	$atts = apply_filters( 'ebox_shortcode_atts_ld_course_resume', $atts );
	...
	if ( ( ! empty( $atts['user_id'] ) ) && ( ! empty( $atts['course_id'] ) ) ) {
		if ( ebox_lms_has_access( $atts['course_id'], $atts['user_id'] ) ) {
			...
			$course_status = ebox_course_status( $atts['course_id'], $atts['user_id'], true );
			if ( 'completed' !== $course_status ) {
				$user_course_last_step_id = ebox_user_progress_get_first_incomplete_step( $atts['user_id'], $atts['course_id'] );
				if ( ! empty( $user_course_last_step_id ) ) {
					$user_course_last_step_id = ebox_user_progress_get_parent_incomplete_step( $atts['user_id'], $atts['course_id'], $user_course_last_step_id );
					$course_permalink         = ebox_get_step_permalink( $user_course_last_step_id, $atts['course_id'] );
					if ( ! empty( $course_permalink ) ) {
						...
						if ( true === $atts['button'] ) {
							$content .= '<div class="ebox-wrapper">';
						}
