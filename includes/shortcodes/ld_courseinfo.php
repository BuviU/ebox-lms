<?php
/**
 * ebox `[courseinfo]` shortcode processing.
 *
 * @since 2.1.0
 ...
 */
function ebox_courseinfo( $attr = array(), $content = '', $shortcode_slug = 'courseinfo' ) {
	...
	if ( '' === $shortcode_atts['course_id'] ) {
		if ( ( isset( $_GET['course_id'] ) ) && ( ! empty( $_GET['course_id'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$shortcode_atts['course_id'] = ebox_get_course_id();
		}
	}
	...
	if ( empty( $shortcode_atts['course_id'] ) || empty( $shortcode_atts['user_id'] ) ) {
		 ...
		 */
		return apply_filters( 'ebox_courseinfo', '', $shortcode_atts );
	}
	...
	switch ( $shortcode_atts['show'] ) {
		case 'course_title':
			...
			return apply_filters( 'ebox_courseinfo', $shortcode_atts[ $shortcode_atts['show'] ], $shortcode_atts );
		...
		case 'course_price_type':
			...
			$course = get_post( $shortcode_atts['course_id'] );
			if ( ( $course ) && ( is_a( $course, 'WP_Post' ) ) ) {
				$shortcode_atts[ $shortcode_atts['show'] ] = ebox_get_setting( $shortcode_atts['course_id'], 'course_price_type' );
			}
		...
		case 'course_price':
			...
			$course = get_post( $shortcode_atts['course_id'] );
			if ( ( $course ) && ( is_a( $course, 'WP_Post' ) ) ) {
				$shortcode_atts[ $shortcode_atts['show'] ] = ebox_get_setting( $shortcode_atts['course_id'], 'course_price' );
			}
		...
		case 'course_users_count':
			...
			$course = get_post( $shortcode_atts['course_id'] );
			if ( ( $course ) && ( is_a( $course, 'WP_Post' ) ) ) {
				$course_users_query = ebox_get_users_for_course( $shortcode_atts['course_id'] );
		...
		case 'user_course_time':
			...
			if ( 'time' === $shortcode_atts['seconds_format'] ) {
				...
				/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
				return apply_filters( 'ebox_courseinfo', ebox_seconds_to_time( $shortcode_atts[ $shortcode_atts['show'] ] ), $shortcode_atts );
			} else {
		...
		case 'cumulative_count':
			...
			$quizdata = get_user_meta( $shortcode_atts['user_id'], '_ebox-quizzes', true );
			$quizzes  = ebox_course_get_steps_by_type( intval( $shortcode_atts['course_id'] ), 'ebox-quiz' );
			if ( empty( $quizzes ) ) {
				/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
				return apply_filters( 'ebox_courseinfo', 0, $shortcode_atts );
			}
			...
			*/
			$quizzes = apply_filters( 'ebox_courseinfo_quizzes', $quizzes, $shortcode_atts );
		...
		case 'aggregate_count':
			...
			*/
			...
			if ( 'timespent' == $field ) {
				if ( 'time' === $shortcode_atts['seconds_format'] ) {
					/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
					return apply_filters( 'ebox_courseinfo', ebox_seconds_to_time( $shortcode_atts[ $shortcode_atts['show'] ] ), $shortcode_atts['show'] );
				} else {
					/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
					return apply_filters( 'ebox_courseinfo', $shortcode_atts[ $shortcode_atts['show'] ], $shortcode_atts['show'] );
				}
			} else {
				/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
				return apply_filters( 'ebox_courseinfo', $shortcode_atts[ $shortcode_atts['show'] ], $shortcode_atts );
		...
		case 'completed_on':
			...
			if ( empty( $completed_on ) ) {
				$completed_on = ebox_user_get_course_completed_date( $shortcode_atts['user_id'], $shortcode_atts['course_id'] );
				if ( empty( $completed_on ) ) {
					/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
					return apply_filters( 'ebox_courseinfo', '-', $shortcode_atts );
				}
			...
			/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
			return apply_filters( 'ebox_courseinfo', ebox_adjust_date_time_display( $completed_on, $shortcode_atts['format'] ), $shortcode_atts );

		case 'enrolled_on':
			$shortcode_atts[ $shortcode_atts['show'] ] = '';

			$enrolled_on = ebox_user_team_enrolled_to_course_from( $shortcode_atts['user_id'], $shortcode_atts['course_id'] );
			...
			/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
			return apply_filters( 'ebox_courseinfo', ebox_adjust_date_time_display( $enrolled_on, $shortcode_atts['format'] ), $shortcode_atts );

		case 'course_points':
			$shortcode_atts[ $shortcode_atts['show'] ] = '';

			$course_points                             = ebox_get_course_points( $shortcode_atts['course_id'], $shortcode_atts['decimals'] );
			$course_points                             = number_format( $course_points, $shortcode_atts['decimals'] );
			$shortcode_atts[ $shortcode_atts['show'] ] = $course_points;

			/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
			return apply_filters( 'ebox_courseinfo', $course_points, $shortcode_atts );

		case 'user_course_points':
			$shortcode_atts[ $shortcode_atts['show'] ] = '';

			$user_course_points                        = ebox_get_user_course_points( $shortcode_atts['user_id'] );
			...
			/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
			return apply_filters( 'ebox_courseinfo', $user_course_points, $shortcode_atts );
...
add_shortcode( 'courseinfo', 'ebox_courseinfo', 10, 3 );
