<?php
/**
 * ebox `[ld_course_list]` shortcode processing.
 *
 * @since 2.1.0
...
function ld_course_list( $attr = array(), $content = '', $shortcode_slug = 'ld_course_list' ) {
	...
	$post_type_class = 'ebox_Settings_Courses_Taxonomies';

	if ( ( isset( $attr['post_type'] ) ) && ( ! empty( $attr['post_type'] ) ) ) {

		if ( ebox_get_post_type_slug( 'lesson' ) == $attr['post_type'] ) {
			$post_type_slug  = 'lesson';
			$post_type_class = 'ebox_Settings_modules_Taxonomies';
		} elseif ( ebox_get_post_type_slug( 'topic' ) == $attr['post_type'] ) {
			$post_type_slug  = 'topic';
			$post_type_class = 'ebox_Settings_Topics_Taxonomies';
		} elseif ( ebox_get_post_type_slug( 'quiz' ) == $attr['post_type'] ) {
			$post_type_slug  = 'quiz';
			$post_type_class = 'ebox_Settings_Quizzes_Taxonomies';
		} elseif ( ebox_get_post_type_slug( 'team' ) == $attr['post_type'] ) {
			$post_type_slug  = 'team';
			$post_type_class = 'ebox_Settings_Teams_Taxonomies';
		}
	}
	...
	if ( in_array( $atts['post_type'], ebox_get_post_types( 'course' ), true ) ) {
	...
	} elseif ( ebox_get_post_type_slug( 'team' ) === $atts['post_type'] ) {
		if ( ( 'true' === $atts['myteams'] ) || ( 'enrolled' === $atts['myteams'] ) ) {
			if ( is_user_logged_in() ) {
				$atts['myteams'] = 'enrolled';
			} else {
				return '';
			}
		} elseif ( 'not-enrolled' === $atts['myteams'] ) {
			if ( is_user_logged_in() ) {
				$atts['myteams'] = 'not-enrolled';
			} else {
				return '';
			}
		} else {
		...
		$atts['team_status'] = array();
		if ( 'enrolled' === $atts['myteams'] ) {
			if ( ! empty( $atts['status'] ) ) {
				if ( ! is_array( $atts['status'] ) ) {
					$atts['status'] = explode( ',', $atts['status'] );
				}
				...
				foreach ( $atts['status'] as $team_status ) {
					if ( 'completed' == $team_status ) {
						$atts['team_status'][] = 'completed';
					} elseif ( 'in_progress' == $team_status ) {
						$atts['team_status'][] = 'in-progress';
					} elseif ( 'not_started' == $team_status ) {
						$atts['team_status'][] = 'not-started';
					}
				}
			} else {
	...
	if ( false === $atts['num'] ) {
		if ( ( isset( $atts['course_id'] ) ) && ( ! empty( $atts['course_id'] ) ) ) {
			$atts['num'] = ebox_get_course_modules_per_page( intval( $atts['course_id'] ) );
		} else {
			$atts['num'] = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'per_page' );
		}
	} elseif ( '-1' == $atts['num'] ) {
		$atts['num'] = 0;
	} else {
	...
	/* Bypasses team leader filtering, see ebox-5664 */
	$attr_org = $attr;
	...
	if ( is_user_logged_in() ) {
		...
		if ( ( isset( $atts['user_id'] ) ) && ( false === $atts['user_id'] ) ) {
			...
			if ( ebox_is_team_leader_user( get_current_user_id() ) ) {
				$teams = ebox_get_administrators_team_ids( get_current_user_id() );
				if ( ! empty( $teams ) ) {
					$user_courses = array();
					foreach ( $teams as $team_id ) {
						if ( ebox_is_user_in_team( $atts['user_id'], $team_id ) ) {
							$team_courses = ebox_team_enrolled_courses( $team_id );
							if ( ! empty( $team_courses ) ) {
								$user_courses = array_merge( $user_courses, $team_courses );
							}
						}
					}
	...
	if ( ( ! empty( $course_id ) ) && ( is_null( $post__in ) ) ) {
		if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
			$filter['post__in'] = ebox_course_get_steps_by_type( $course_id, $atts['post_type'] );
		} else {
			$meta_query = array(
				'key'     => 'course_id',
	...
	if ( ebox_Settings_Section::get_section_setting( $post_type_class, 'wp_post_category' ) == 'yes' ) {
	...
	if ( ebox_Settings_Section::get_section_setting( $post_type_class, 'wp_post_tag' ) == 'yes' ) {
	...
	if ( ebox_Settings_Section::get_section_setting( $post_type_class, 'ld_' . $post_type_slug . '_category' ) == 'yes' ) {
	...
	if ( ebox_Settings_Section::get_section_setting( $post_type_class, 'ld_' . $post_type_slug . '_tag' ) == 'yes' ) {
	...
	if ( isset( $atts['price_type'] ) && ! empty( $atts['price_type'] ) ) {
		...
		foreach ( $price_type as $list ) {
			$cpt_slug         = ( ebox_get_post_type_slug( 'course' ) === $atts['post_type'] ) ? 'course' : 'team';
			$price_type_posts = array_merge( $price_type_posts, ebox_get_posts_by_price_type( ebox_get_post_type_slug( $cpt_slug ), $list ) );
		}
	...
	if ( ( in_array( $atts['post_type'], ebox_get_post_types( 'course' ), true ) ) && ( is_null( $post__in ) ) ) {
		if ( 'enrolled' == $mycourses ) {
			$courses_enrolled   = array();
			$filter['post__in'] = ebox_user_get_enrolled_courses( $atts['user_id'] );
			...
			if ( ! empty( $course_status ) ) {
				...
				$user_courses_reports = ebox_reports_get_activity( $activity_query_args );
			...
			$atts['status']   = '';
			$courses_enrolled = ebox_user_get_enrolled_courses( $atts['user_id'] );
			if ( ! empty( $price_type_posts ) && ! empty( $courses_enrolled ) ) {
	...
	} elseif ( ( ebox_get_post_type_slug( 'team' ) === $atts['post_type'] ) && ( is_null( $post__in ) ) ) {
		if ( 'enrolled' == $myteams ) {
			$teams_enrolled = array();
			$user_team_ids  = ebox_get_users_team_ids( $atts['user_id'] );
			if ( empty( $user_team_ids ) ) {
				return;
			}

			if ( ! empty( $team_status ) ) {
				foreach ( $user_team_ids as $team_id ) {
					$team_status = ebox_get_user_team_status( $team_id, $atts['user_id'], true );
					if ( ( ! empty( $team_status ) ) && ( ! empty( $atts['team_status'] ) ) && ( in_array( $team_status, $atts['team_status'] ) ) ) {
						$filter['post__in'][] = $team_id;
					}
				}
				if ( empty( $filter['post__in'] ) ) {
					return;
				...
				$filter['post__in'] = $user_team_ids;
			}

			if ( ! empty( $user_team_ids ) ) {
				$teams_enrolled = array_map( 'absint', $user_team_ids );
			} else {
				return;
			}

			if ( ! empty( $price_type_posts ) && ! empty( $teams_enrolled ) ) {
				$filter['post__in'] = array_intersect( $price_type_posts, $teams_enrolled );
			} elseif ( ! empty( $price_type_posts ) ) {
				$filter['post__in'] = $price_type_posts;
			} elseif ( ! empty( $teams_enrolled ) ) {
				$filter['post__in'] = $teams_enrolled;
			}

			if ( empty( $filter['post__in'] ) ) {
				return;
			}
		} elseif ( 'not-enrolled' == $myteams ) {
			$atts['status']  = '';
			$teams_enrolled = ebox_get_users_team_ids( $atts['user_id'] );
			if ( ! empty( $price_type_posts ) && ! empty( $teams_enrolled ) ) {
				$filter_price_type_enrolled = array_diff( $price_type_posts, $teams_enrolled );
				if ( ! empty( $filter_price_type_enrolled ) ) {
					$filter['post__in'] = $filter_price_type_enrolled;
				} else {
					return;
				...
				$filter['post__not_in'] = $teams_enrolled;
			}
		} elseif ( ! empty( $price_type_posts ) ) {
			$filter['post__in'] = $price_type_posts;
		}
	 ...
	 * @param array $atts  Combined and filtered attribute list.
	 */
	$filter = apply_filters( 'ebox_ld_course_list_query_args', $filter, $atts );
	...
	if ( ( trim( $categoryselector ) == 'true' ) && ( ebox_Settings_Section::get_section_setting( $post_type_class, 'wp_post_category' ) == 'yes' ) ) {
		$cats = array();

		if ( ( isset( $_GET['catid'] ) ) && ( ! empty( $_GET['catid'] ) ) ) { // cspell:disable-line.
			$atts['cat'] = intval( $_GET['catid'] ); // cspell:disable-line.
			// Duplicated variable related to changes on ebox-5664 and ebox-5756.
		...
		if ( ! empty( $cat_posts ) ) {
			...
			if ( ! empty( $cats ) ) {
				...
				$get_categories_args = apply_filters(
					'ebox_course_list_category_args',
					array(
						'taxonomy' => 'category',
						'type'     => $post_type,
	...
	if ( ( trim( $atts[ $post_type_slug . '_categoryselector' ] ) == 'true' ) && ( empty( $categoryselector ) )
		&& ( ebox_Settings_Section::get_section_setting( $post_type_class, 'ld_' . $post_type_slug . '_category' ) == 'yes' ) ) {
		$ld_cats = array();
		...
		if ( ! empty( $cat_posts ) ) {
			...
			if ( ! empty( $ld_cats ) ) {
				...
				$get_ld_categories_args = apply_filters(
					'ebox_course_list_' . $post_type_slug . '_category_args',
					array(
						'taxonomy' => 'ld_' . $post_type_slug . '_category',
						'type'     => $post_type,
	...
	if ( 'true' == $include_outer_wrapper ) {
		if ( ! empty( $categories ) ) {
			...
			$categorydropdown .= '<form method="get">
					<label for="ld_categorydropdown_select">' . esc_html__( 'Categories', 'ebox' ) . '</label>
					<select id="ld_categorydropdown_select" name="catid" onChange="jQuery(\'#ld_categorydropdown form\').submit()">'; // cspell:disable-line.
			$categorydropdown .= '<option value="">' . esc_html__( 'Select category', 'ebox' ) . '</option>';
		...
		if ( ! empty( $ld_categories ) ) {
			...
			$ld_categorydropdown .= '<option value="">' . sprintf(
				...
				esc_html_x( 'Select %s', 'placeholder: LD Category label', 'ebox' ),
	 ...
	 */
	if ( apply_filters( 'ebox_shortcode_course_list_legacy_loop', $user_legacy_loop, $atts ) ) {
 ...
 */
function ebox_course_status_content_shortcode( $atts, $content, $status ) {

	$atts['user_id']   = empty( $atts['user_id'] ) ? get_current_user_id() : intval( $atts['user_id'] );
	$atts['course_id'] = empty( $atts['course_id'] ) ? ebox_get_course_id() : ebox_get_course_id( intval( $atts['course_id'] ) );
	...
	if ( ( ! empty( $atts['course_id'] ) ) && ( ! empty( $atts['user_id'] ) ) && ( get_current_user_id() == $atts['user_id'] ) ) {
		if ( ebox_lms_has_access( $atts['course_id'], $atts['user_id'] ) ) {
			if ( ebox_course_status( $atts['course_id'], $atts['user_id'] ) == $status ) {
