<?php
/**
 * ebox `[ld_course_certificate]` shortcode processing.
 *
 * @since 2.1.0
...
function ld_course_certificate_shortcode( $atts = array(), $content = '', $shortcode_slug = 'ld_course_certificate' ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
	...
	$link    = ebox_get_course_certificate_link( $course_id, $user_id );
	...
	return apply_filters( // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
		...
		"<div id='ebox_course_certificate'><a href='" . $link . "' class='btn-blue' target='_blank'>" . apply_filters( 'ld_certificate_link_label', esc_html__( 'PRINT YOUR CERTIFICATE', 'ebox' ), $user_id, $course_id ) . '</a></div>',
		$link,
		$course_id,
		$user_id
	);
