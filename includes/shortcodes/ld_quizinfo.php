<?php
/**
 * ebox `[quizinfo]` shortcode processing.
 *
 * @since 2.1.0
 *
 ...
 */
function ebox_quizinfo( $attr = array(), $content = '', $shortcode_slug = 'quizinfo' ) {
	...
	$quizinfo = get_user_meta( $user_id, '_ebox-quizzes', true );
	...
	switch ( $show ) {
		case 'timestamp':
			if ( ( isset( $selected_quizinfo['time'] ) ) && ( ! empty( $selected_quizinfo['time'] ) ) ) {
				$selected_quizinfo['timestamp'] = ebox_adjust_date_time_display( $selected_quizinfo['time'], $format );
			} else {
		...
		case 'pass':
			if ( ( isset( $selected_quizinfo['pass'] ) ) && ( ! empty( $selected_quizinfo['pass'] ) ) ) {
				$selected_quizinfo['pass'] = esc_html__( 'Yes', 'ebox' );
			} else {
				$selected_quizinfo['pass'] = esc_html__( 'No', 'ebox' );
			}
			break;

		case 'quiz_title':
			if ( ( ! empty( $quiz ) ) && ( get_post_type( $quiz ) == ebox_get_post_type_slug( 'quiz' ) ) ) {
		...
		case 'course_title':
			if ( ( isset( $selected_quizinfo['course'] ) ) && ( ! empty( $selected_quizinfo['course'] ) ) ) {
				...
				$course_id = ebox_get_setting( $quiz, 'course' );
			}
		...
		case 'timespent':
			if ( ( isset( $selected_quizinfo['timespent'] ) ) && ( ! empty( $selected_quizinfo['timespent'] ) ) ) {
				$selected_quizinfo['timespent'] = ebox_seconds_to_time( $selected_quizinfo['timespent'] );
			}
			break;
	...
	if ( isset( $selected_quizinfo[ $show ] ) ) {
		 ...
		 */
		return apply_filters( 'ebox_quizinfo', $selected_quizinfo[ $show ], $shortcode_atts, $selected_quizinfo );
	} else {
		/** This filter is documented in includes/quiz/ld-quiz-info-shortcode.php */
		return apply_filters( 'ebox_quizinfo', '', $shortcode_atts, $selected_quizinfo );
	}
}
add_shortcode( 'quizinfo', 'ebox_quizinfo', 10, 3 );
