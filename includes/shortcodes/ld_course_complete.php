<?php
/**
 * ebox `[course_complete]` shortcode processing.
 *
 * @since 2.1.0
 ...
 */
function ebox_course_complete_shortcode( $atts = array(), $content = '', $shortcode_slug = 'course_complete' ) {
	...
	if ( ! empty( $content ) ) {
		 ...
		 */
		$atts = apply_filters( 'ebox_course_complete_shortcode_atts', $atts );

		$atts['content'] = ebox_course_status_content_shortcode( $atts, $atts['content'], esc_html__( 'Completed', 'ebox' ) );
		return ebox_LMS::get_template(
			'ebox_course_complete_message',
			array(
				'shortcode_atts' => $atts,
			),
			false
...
add_shortcode( 'course_complete', 'ebox_course_complete_shortcode', 10, 3 );
