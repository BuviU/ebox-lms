<?php
/**
 * ebox `[ld_profile]` shortcode processing.
 *
 * @since 2.1.0
 *
 ...
 */
function ebox_profile( $atts = array(), $content = '', $shortcode_slug = 'ld_profile' ) {
	...
	/**
	 * ebox-6274: Patch to ensure the user_id is valid.
	 */
	if ( ( (int) get_current_user_id() !== (int) $atts['user_id'] ) && ( ! ebox_is_admin_user( get_current_user_id() ) ) ) {
		if ( ebox_is_team_leader_user( get_current_user_id() ) ) {
			// If team leader user we ensure the preview user_id is within their team(s).
			if ( ! ebox_is_team_leader_of_user( get_current_user_id(), $atts['user_id'] ) ) {
	...
	if ( false === $atts['per_page'] ) {
		$atts['per_page'] = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'per_page' );
	} else {
		$atts['per_page'] = intval( $atts['per_page'] );
	}

	if ( false === $atts['quiz_num'] ) {
		$atts['quiz_num'] = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'per_page' );
	} else {
	 ...
	 */
	$atts = apply_filters( 'ebox_profile_shortcode_atts', $atts );
	...
	$quiz_attempts = ebox_get_user_profile_quiz_attempts( $current_user->ID );
...
add_shortcode( 'ld_profile', 'ebox_profile', 10, 3 );
