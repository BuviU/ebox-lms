<?php
/**
 * ebox `[usermeta]` shortcode processing.
 *
 * @since 2.1.0
 *
 ...
 */
function ebox_usermeta_shortcode( $attr = array(), $content = '', $shortcode_slug = 'usermeta' ) {
	...
	if ( ( ! empty( $attr['user_id'] ) ) && ( ! empty( $attr['field'] ) ) ) {
		...
		if ( ( $userdata ) && ( is_a( $userdata, 'WP_User' ) ) ) {
			...
			if ( ( ebox_is_admin_user() ) || ( get_current_user_id() == $attr['user_id'] ) ) {
				$usermeta_available_fields = array( $attr['field'] => $attr['field'] );
			} else {
				$usermeta_available_fields = ebox_get_usermeta_shortcode_available_fields( $attr );
			}
			 ...
			 */
			$content = apply_filters( 'ebox_usermeta_shortcode_field_value_display', $value, $attr, $usermeta_available_fields );
		}
	}

	return $content;
}
add_shortcode( 'usermeta', 'ebox_usermeta_shortcode', 10, 3 );
