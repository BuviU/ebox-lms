<?php
/**
 * Utility class to contain all transient and cache related functions used within ebox.
 *
 * @since 3.1.0
 * @package ebox
 */
...
if ( ! class_exists( 'LDLMS_Transients' ) ) {
	...
	class LDLMS_Transients {
		 ...
		 * @since 3.4.1
		 *
		 * @var string $cache_team;
		 */
		private static $cache_team = 'ebox';
		...
		public static function get( $transient_key = '' ) {
			...
			if ( ! empty( $transient_key ) ) {
				 ...
				 */
				if ( ! apply_filters( 'ebox_bypass_cache', false, $transient_key ) ) {
					 ...
					 * @param string  $transient_key  Transient Key.
					 */
					if ( apply_filters( 'ebox_object_cache_enabled', ebox_OBJECT_CACHE_ENABLED, $transient_key ) ) {
						$found          = false;
						$transient_data = wp_cache_get( $transient_key, self::$cache_team, false, $found );
						if ( false === $found ) {
							$transient_data = false;
						}
					} elseif (
						 ...
						 * @param string  $transient_key       Transient Key.
						 */
						! apply_filters( 'ebox_transients_disabled', ebox_TRANSIENTS_DISABLED, $transient_key )
					) {
		...
		public static function set( $transient_key = '', $transient_data = '', $transient_expire = MINUTE_IN_SECONDS ) {
			...
			if ( ! empty( $transient_key ) ) {
				$transient_expire = apply_filters( 'ebox_cache_expire', $transient_expire, $transient_key );
				...
				if ( ! empty( $transient_expire ) ) {
					...
					if ( apply_filters( 'ebox_object_cache_enabled', ebox_OBJECT_CACHE_ENABLED, $transient_key ) ) {
						return wp_cache_set( $transient_key, $transient_data, self::$cache_team, $transient_expire );
					} elseif (
						/** This filter is documented in includes/class-ld-transients.php */
						! apply_filters( 'ebox_transients_disabled', ebox_TRANSIENTS_DISABLED, $transient_key )
					) {
		...
		public static function delete( $transient_key = '' ) {
			if ( ! empty( $transient_key ) ) {
				/** This filter is documented in includes/class-ld-transients.php */
				if ( apply_filters( 'ebox_object_cache_enabled', ebox_OBJECT_CACHE_ENABLED, $transient_key ) ) {
					return wp_cache_delete( $transient_key, self::$cache_team );
				} elseif (
					/** This filter is documented in includes/class-ld-transients.php */
					! apply_filters( 'ebox_transients_disabled', ebox_TRANSIENTS_DISABLED, $transient_key )
				) {
		...
		public static function purge_all() {
			/** This filter is documented in includes/class-ld-transients.php */
			if ( apply_filters( 'ebox_object_cache_enabled', ebox_OBJECT_CACHE_ENABLED, 'ebox_all_purge' ) ) {
				wp_cache_flush();
			} elseif (
				/** This filter is documented in includes/class-ld-transients.php */
				! apply_filters( 'ebox_transients_disabled', ebox_TRANSIENTS_DISABLED, 'ebox_all_purge' )
			) {
				global $wpdb;
