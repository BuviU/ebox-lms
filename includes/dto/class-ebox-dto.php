...
if ( ! class_exists( 'ebox_DTO' ) ) {
	/**
	 * DTO class.
	 *
	 * @since 4.5.0
	 */
	abstract class ebox_DTO {
		/**
		 ...
		 *
		 * @throws ebox_DTO_Validation_Exception If one or more properties are invalid.
		 *
		 * @return void
		 */
		...
		public function to_array(): array {
			if ( count( $this->only_keys ) ) {
				$array = ebox_Arr::only( $this->all(), $this->only_keys );
			} else {
				$array = ebox_Arr::except( $this->all(), $this->except_keys );
			}
		 ...
		 * @since 4.5.0
		 *
		 * @return array<string,ebox_DTO_Property_Validator[]>
		 */
		...
		private function validate(): void {
			...
			if ( count( $validation_errors ) ) {
				throw new ebox_DTO_Validation_Exception( $this, $validation_errors );
			}
		}
