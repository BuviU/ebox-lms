...
if ( ! class_exists( 'ebox_Pricing_DTO' ) && class_exists( 'ebox_DTO' ) ) {
	/**
	 * Pricing DTO class. Used for the product and transaction pricing.
	 *
	 * @since 4.5.0
	 */
	class ebox_Pricing_DTO extends ebox_DTO {
		...
		protected function get_validators(): array {
			...
			if ( ! empty( $this->currency ) ) {
				$validators['currency'] = array(
					ebox_DTO_Property_Validator_String_Case::uppercase(),
				);
			}

			if ( ! empty( $this->duration_length ) ) {
				$validators['duration_length'] = array(
					new ebox_DTO_Property_Validator_Possible_Values( self::VALID_DURATION_VALUES ),
				);
			}
