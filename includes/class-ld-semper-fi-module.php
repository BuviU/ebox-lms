...
if ( ! class_exists( 'Semper_Fi_Module' ) ) {
	...
	abstract class Semper_Fi_Module {
		 ...
		 * @var string
		 */
		protected $parent_option = 'ebox_cpt_options';
		...
		public function __call( $name, $arguments ) {
			...
			throw new InvalidArgumentException( sprintf( esc_html_x( "Method %s doesn't exist", 'placeholder: method name', 'ebox' ), $name ) );
		}
		...
		public function __construct() {
			...
			$this->plugin_path['basename']   = plugin_basename( $this->plugin_path['dir'] . '/ebox_lms.php' );
		...
		public function display_tabs( $location ) {
			...
			if ( ! empty( $tabs ) ) {
				?>
				<div class="ebox_tabs_div">
					<label class="ebox_head_nav">
						<?php
						foreach ( $tabs as $k => $v ) {
							?>
									<a class="ebox_head_nav_tab ebox_head_nav_
									<?php
						...
						if ( null !== $this->locations ) {
							...
							if ( is_array( $post_custom_fields ) ) {
								foreach ( $post_custom_fields as $field_name => $field ) {
									if ( ( substr( $field_name, 1, 7 ) == 'ebox' ) && ( $field[0] ) ) {
										$has_data = true;
		...
		public function output_error( $error ) {
			echo wp_kses_post( '<div class="ebox_module error" style="text-align:center;">' . $error . '</div>' );
			return false;
		}
		...
		public function is_admin() {
			return current_user_can( ebox_ADMIN_CAPABILITY_CHECK );
		}
		...
		public function enqueue_scripts() {
			...
			if ( is_admin() ) {
				...
				if ( ! empty( $this->pointers ) ) {
					wp_enqueue_script( 'wp-pointer', false, array( 'jquery' ), ebox_SCRIPT_VERSION_TOKEN, false );
					$this->script_data['pointers'] = $this->pointers;
				}
			}

			$this->script_data['ebox_categories_lang'] = esc_html__( 'ebox Categories', 'ebox' );
			$this->script_data['loading_lang']              = esc_html__( 'Loading...', 'ebox' );
			// translators: placeholder: Lesson.
			$this->script_data['select_a_lesson_lang'] = sprintf( esc_html_x( '-- Select a %s --', 'placeholder: Lesson', 'ebox' ), ebox_Custom_Label::get_label( 'lesson' ) );
			// translators: placeholders: Lesson, Topic.
			$this->script_data['select_a_lesson_or_topic_lang']      = sprintf( esc_html_x( '-- Select a %1$s or %2$s --', 'placeholders: Lesson, Topic', 'ebox' ), ebox_Custom_Label::get_label( 'lesson' ), ebox_Custom_Label::get_label( 'topic' ) );
			$this->script_data['advanced_quiz_preview_link']         = admin_url( 'admin.php?page=ldAdvQuiz&module=preview&id=' );
			$this->script_data['valid_recurring_paypal_day_range']   = esc_html__( 'Valid range is 1 to 90 when the Billing Cycle is set to days.', 'ebox' );
			$this->script_data['valid_recurring_paypal_week_range']  = esc_html__( 'Valid range is 1 to 52 when the Billing Cycle is set to weeks.', 'ebox' );
			$this->script_data['valid_recurring_paypal_month_range'] = esc_html__( 'Valid range is 1 to 24 when the Billing Cycle is set to months.', 'ebox' );
			$this->script_data['valid_recurring_paypal_year_range']  = esc_html__( 'Valid range is 1 to 5 when the Billing Cycle is set to years.', 'ebox' );

			$this->script_data['valid_recurring_paypal_day_max']   = ebox_billing_cycle_field_frequency_max( 'D' );
			$this->script_data['valid_recurring_paypal_week_max']  = ebox_billing_cycle_field_frequency_max( 'W' );
			$this->script_data['valid_recurring_paypal_month_max'] = ebox_billing_cycle_field_frequency_max( 'M' );
			$this->script_data['valid_recurring_paypal_year_max']  = ebox_billing_cycle_field_frequency_max( 'Y' );

			global $post;
			if ( ( $post instanceof WP_Post ) && ( 'ebox-quiz' === $post->post_type ) ) {
				$this->script_data['quiz_pro'] = intval( ebox_get_setting( $post->ID, 'quiz_pro' ) );
			}
		...
		public function add_menu( $parent_slug ) {
			...
			if ( null === $this->locations ) {
				$hookname = add_submenu_page( $parent_slug, $name, $name, ebox_ADMIN_CAPABILITY_CHECK, $default_options_page, array( $this, 'display_settings_page' ) );
					...
					if ( isset( $v['display'] ) && ! empty( $v['display'] ) ) {
						foreach ( $v['display'] as $posttype ) {
							...
							if ( $this->tabbed_metaboxes ) {
								...
								add_filter( 'ebox_add_post_metabox', array( $this, 'filter_return_metaboxes' ) );
							} else {
								 ...
								 */
								if ( ( 'ebox-quiz' === $posttype ) && ( apply_filters( 'ebox_settings_metaboxes_legacy_quiz', ebox_SETTINGS_METABOXES_LEGACY_QUIZ, $posttype ) ) ) {
									/**
								 ...
								 * @param string  $post_type   Post type slug.
								 */
								elseif ( apply_filters( 'ebox_settings_metaboxes_legacy', ebox_SETTINGS_METABOXES_LEGACY, $posttype ) ) {
					...
					if ( isset( $v['type'] ) && ( 'metabox' === $v['type'] ) ) {
						...
						if ( $update ) {
							if ( 'ebox-courses' === $k ) {
								...
								if ( ! isset( $options['ebox-courses_course_prerequisite_enabled'] ) ) {
									$options['ebox-courses_course_prerequisite_enabled'] = 'off';
								}
								if ( ! isset( $options['ebox-courses_course_prerequisite'] ) ) {
									$options['ebox-courses_course_prerequisite'] = array();
								}

								// IF prerequisites not enabled then clear out the courses array.
								if ( 'on' !== $options['ebox-courses_course_prerequisite_enabled'] ) {
									$options['ebox-courses_course_prerequisite'] = array();
								} else {

									if ( ( isset( $options['ebox-courses_course_prerequisite'] ) ) && ( ! empty( $options['ebox-courses_course_prerequisite'] ) ) ) {
										foreach ( $options['ebox-courses_course_prerequisite'] as $c_idx => $c_id ) {
											$c_id = intval( $c_id );
											if ( empty( $c_id ) ) {
												unset( $options['ebox-courses_course_prerequisite'][ $c_idx ] );
											}
										}
									}
								}
								...
								if ( empty( $options['ebox-courses_course_prerequisite'] ) ) {
									$options['ebox-courses_course_prerequisite_enabled'] = 'off';
								} else {
									$options['ebox-courses_course_prerequisite'] = array_values( $options['ebox-courses_course_prerequisite'] );
								}

								if ( ( isset( $options['ebox-courses_course_points_enabled'] ) ) && ( 'on' === $options['ebox-courses_course_points_enabled'] ) ) {
									if ( isset( $options['ebox-courses_course_points'] ) ) {
										$course_points = ebox_format_course_points( $options['ebox-courses_course_points'] );
									} else {
										$course_points = 0;
									}
								...
								if ( isset( $options['ebox-courses_course_access_list'] ) ) {
									$options['ebox-courses_course_access_list'] = ebox_convert_course_access_list( $options['ebox-courses_course_access_list'] );
									update_post_meta( $post_id, 'course_access_list', $options['ebox-courses_course_access_list'] );
								}
							} elseif ( ( 'ebox-modules' === $k ) && ( isset( $options[ $k . '_visible_after_specific_date' ] ) ) ) {
							...
							// ebox-261: We need to reset the reference to the post meta ‘quiz_pro_id’ when the quiz is saved
							// in case the ‘Associated Settings’ selector is updated.
							elseif ( ( 'ebox-quiz' === $k ) && ( isset( $options[ $k . '_quiz_pro' ] ) ) ) {

								$quiz_pro_id_new = absint( $options['ebox-quiz_quiz_pro'] );
								...
								if ( $quiz_pro_id_org !== $quiz_pro_id_new ) {
									 ...
									 */
									$quiz_id_primary_org = absint( ebox_get_quiz_primary_shared( $quiz_pro_id_org, false ) );
									if ( $quiz_id_primary_org === $post_id ) {
										delete_post_meta( $post_id, 'quiz_pro_primary_' . $quiz_pro_id_org );
										$quiz_post_ids = ebox_get_quiz_post_ids( $quiz_pro_id_org );
									...
									$quiz_id_primary_new = absint( ebox_get_quiz_primary_shared( $quiz_pro_id_new, false ) );
								...
								if ( isset( $options[ $k . '_quiz' ] ) ) {
									...
									if ( $quiz_id_old !== $quiz_id_new ) {
										...
										if ( ! empty( $quiz_id_old ) ) {
											ebox_set_quiz_questions_dirty( $quiz_id_old );
										}

										if ( ! empty( $quiz_id_new ) ) {
											ebox_set_quiz_questions_dirty( $quiz_id_new );
										}
									}
								}
							}
		...
		/**
		 * Outputs radio buttons, checkboxes, selects, multiselects, handles teams.
		 *
		 * @since 2.1.0
		 *
		...
		public function do_multi_input( $args ) {
			...
			if ( ( 'radio' === $type ) || ( 'checkbox' === $type ) ) {
				$strings = array(
					'block'     => "%s\n",
					'team'     => "\t<b>%s</b><br>\n%s\n",
					'item'      => "\t<label class='ebox_option_setting_label'><input type='$type' %s name='%s' value='%s' %s> %s</label>\n",
				...
				if ( ( isset( $args['options']['lazy_load'] ) ) && ( true === $args['options']['lazy_load'] ) ) {
					if ( ( isset( $args['options']['lazy_load_data'] ) ) && ( ! empty( $args['options']['lazy_load_data'] ) ) ) {
						...
						$lazy_load_data                             = ' ebox_lazy_load_data="' . htmlspecialchars( wp_json_encode( $args['options']['lazy_load_data'] ) ) . '" ';

						$lazy_load_spinner = '<br /><span style="display:none;" class="ebox_lazy_loading"><img class="ebox_lazy_load_spinner" alt="' . esc_html__( 'loading', 'ebox' ) . '" src="' . admin_url( '/images/wpspin_light.gif' ) . '" /> ' . esc_html__( 'loading', 'ebox' ) . '</span>';
					}
				}
				...
				if ( in_array( $args['name'], array( 'ebox-modules_course', 'ebox-topic_course', 'ebox-topic_lesson' ), true ) === true ) {
					if ( ( ! isset( $args['value'] ) ) || ( empty( $args['value'] ) ) ) {
						$show_required = true;
					}
				}
				if ( true === $show_required ) {
					$block .= '<span class="ld-error dashicons dashicons-warning" title="' . esc_html__( 'Required', 'ebox' ) . '"></span>';
				}
			...
			if ( isset( $options['initial_options'] ) && is_array( $options['initial_options'] ) ) {
				...
				foreach ( $options['initial_options'] as $l => $option ) {
					...
					if ( $is_team ) {
						$buf1 .= sprintf( $strings['team'], $l, $buf2 );
					} else {
						$buf1 .= $buf2;
					}
				}
		...
		public function get_option_html( $args ) {
			...
			switch ( $options['type'] ) {
				...
				case 'image':
					$buf .= "<input class='ebox_upload_image_button' type='button' value='" . esc_html__( 'Upload Image', 'ebox' ) . "' style='float:left;' />" .
					"<input class='ebox_upload_image_label' name='$name' type='text' readonly $attr value='$value' size=57 style='float:left;clear:left;' />\n";
					break;
				...
				case 'number':
					...
					if ( ( 'ebox-courses_course_points' === $name ) || ( 'ebox-courses_course_points_access' === $name ) ) {
						$value = ebox_format_course_points( $value );
					}
				...
				case 'wp_date_selector':
					if ( ! empty( $value ) ) {
						if ( ! is_numeric( $value ) ) {
							$value = ebox_get_timestamp_from_date_string( $value );
						} else {
					...
					$month_field = '<span class="screen-reader-text">' . esc_html__( 'Month', 'ebox' ) . '</span><select class="ld_date_mm" name="' . $name . '[mm]" ><option value=""></option>';
					for ( $i = 1; $i < 13; $i++ ) {
					...
					$day_field    = '<span class="screen-reader-text">' . esc_html__( 'Day', 'ebox' ) . '</span><input type="number" placeholder="DD" min="1" max="31" class="ld_date_jj" name="' . $name . '[jj]" value="' . $value_jj . '" size="2" maxlength="2" autocomplete="off" />';
					$year_field   = '<span class="screen-reader-text">' . esc_html__( 'Year', 'ebox' ) . '</span><input  type="number" placeholder="YYYY" min="0000" max="9999" class="ld_date_aa" name="' . $name . '[aa]" value="' . $value_aa . '" size="4" maxlength="4" autocomplete="off" />';
					$hour_field   = '<span class="screen-reader-text">' . esc_html__( 'Hour', 'ebox' ) . '</span><input type="number" min="0" max="23" placeholder="HH" class="ld_date_hh" name="' . $name . '[hh]" value="' . $value_hh . '" size="2" maxlength="2" autocomplete="off" />';
					$minute_field = '<span class="screen-reader-text">' . esc_html__( 'Minute', 'ebox' ) . '</span><input type="number" min="0" max="59" placeholder="MM" class="ld_date_mn" name="' . $name . '[mn]" value="' . $value_mn . '" size="2" maxlength="2" autocomplete="off" />';
			...
			if ( ! empty( $options['count'] ) ) {
				...
				. sprintf( esc_html__( ' characters. Most search engines use a maximum of %1$s chars for the %2$s.', 'ebox' ), $size, strtolower( $options['name'] ) );
			}
	 ...
	 *
		 * @const string
		 */
		const DISPLAY_HELP_START = '<a class="ebox_help_text_link" style="cursor:pointer;" title="%s" onclick="toggleVisibility(\'%s_tip\');"><img src="%s/question.png" /><label class="ebox_label textinput">%s</label></a>';
	 ...
	 *
		 * @const string
		 */
		const DISPLAY_HELP_END = '<div class="ebox_help_text_div" style="display:none" id="%s_tip"><label class="ebox_help_text">%s</label></div>';
	 ...
	 *
		 * @const string
		 */
		const DISPLAY_LABEL_FORMAT = '<span class="ebox_option_label" style="text-align:%s;vertical-align:top;">%s</span>';
	 ...
	 *
		 * @const string
		 */
		const DISPLAY_TOP_LABEL = "</div>\n<div class='ebox_input ebox_top_label'>\n";
	 ...
	 *
		 * @const string
		 */
		const DISPLAY_ROW_TEMPLATE = '<div class="ebox_input %s" id="%s">%s<span class="ebox_option_input"><div class="ebox_option_div" %s>%s</div>%s</span><p style="clear:left"></p></div>';
		...
		public function get_option_row( $name, $opts, $args ) {
			...
			if ( 'none' !== $opts['label'] ) {
				if ( isset( $opts['help_text'] ) ) {
					$help_text   = sprintf( self::DISPLAY_HELP_START, esc_html__( 'Click for Help!', 'ebox' ), $name, $this->plugin_path['images_url'], $opts['name'] );
		...
		public function display_options( $location = null, $meta_args = null ) {
			...
			$container = "<div class='ebox ebox_options {$this->prefix}settings'>";
		...
		public function handle_settings_updates( $location = null ) {
			...
			if ( ( isset( $_POST['action'] ) && 'sfp_update_module' === $_POST['action'] && ( isset( $_POST['Submit_Default'] ) || isset( $_POST['Submit_All_Default'] ) || ! empty( $_POST['Submit'] ) ) ) ) {
				...
				if ( isset( $_POST['Submit_Default'] ) || isset( $_POST['Submit_All_Default'] ) ) {
					$message = esc_html__( 'Options Reset.', 'ebox' );

					if ( isset( $_POST['Submit_All_Default'] ) ) {
						$this->reset_options( $location, true );

						/**
						 * Fires after resetting ebox options.
						 *
						 * @since 2.1.0
						 */
						do_action( 'ebox_options_reset' );
					} else {
				...
				if ( ! empty( $_POST['Submit'] ) ) {
					$message         = esc_html__( 'Options Updated.', 'ebox' );
					$default_options = $this->default_options( $location );
