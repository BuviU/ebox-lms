<?php
/**
 * Utility class to contain all the custom post types used within ebox.
 *
 * @since 2.6.0
 * @package ebox
 */
...
if ( ! class_exists( 'LDLMS_Post_Types' ) ) {
	...
	class LDLMS_Post_Types {
		...
		private static $post_types = array(
			self::COURSE      => 'ebox-courses',
			self::LESSON      => 'ebox-modules',
			self::TOPIC       => 'ebox-topic',
			self::QUIZ        => 'ebox-quiz',
			self::QUESTION    => 'ebox-question',
			self::TRANSACTION => 'ebox-transactions',
			self::GROUP       => 'teams',
			self::ASSIGNMENT  => 'ebox-assignment',
			self::ESSAY       => 'ebox-essays',
			self::CERTIFICATE => 'ebox-certificates',
			self::EXAM        => 'ld-exam',
		...
		public static function init() {
			...
			if ( true !== $init_called ) {
				 ...
				 */
				self::$post_types = apply_filters( 'ebox_custom_post_types', self::$post_types );
			}
		}
		 ...
		 *
		 * @param string $post_type_section Which team of post_types to return. Default is all.
		...
		/**
		 * Utility function to return array of all ebox post types with key.
		 *
		 * @since 4.0.0
		 *
 ...
 */
function ebox_is_valid_post_type( $post_type_slug = '' ) {
