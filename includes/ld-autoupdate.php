...
if ( ! class_exists( 'nss_plugin_updater_ebox_lms' ) ) {

	/**
	 * Class to update ebox
	 */
	class nss_plugin_updater_ebox_lms {
		...
		public function __construct( $update_path, $plugin_slug ) {
			...
			$this->current_version  = ebox_VERSION;
			...
			if ( ( defined( 'ebox_PLUGIN_LICENSE_INTERVAL' ) ) && ( ebox_PLUGIN_LICENSE_INTERVAL > 3600 ) ) {
				$this->plugin_license_cache_time_limit = ebox_PLUGIN_LICENSE_INTERVAL;
			}

			$this->plugin_info_cache_time_limit = 600; // 10 minutes
			if ( ( defined( 'ebox_PLUGIN_INFO_INTERVAL' ) ) && ( ebox_PLUGIN_INFO_INTERVAL > 600 ) ) {
				$this->plugin_info_cache_time_limit = ebox_PLUGIN_INFO_INTERVAL;
			}
		...
		public function nss_plugin_license_update() {
			...
			if ( ( isset( $_POST['ld_plugin_license_nonce'] ) ) && ( ! empty( $_POST['ld_plugin_license_nonce'] ) ) && ( wp_verify_nonce( $_POST['ld_plugin_license_nonce'], 'update_nss_plugin_license_' . $this->code ) ) ) {
				...
				update_option( 'nss_plugin_license_' . $this->code, trim( $license ), ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
				update_option( 'nss_plugin_license_email_' . $this->code, trim( $email ), ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
		...
		public function check_notice() {
			...
			if ( ( isset( $_REQUEST['page'] ) ) && ( 'nss_plugin_license-' . $this->code . '-settings' === $_REQUEST['page'] ) ||
				( isset( $_REQUEST['page'] ) ) && ( 'ebox-setup' === $_REQUEST['page'] ) ) {
		...
		public function admin_notice() {
			...
			if ( true !== $notice_shown ) {
				...
				if ( ! in_array( $current_screen->id, array( 'admin_page_nss_plugin_license-ebox_lms-settings', 'dashboard', 'admin_page_ebox-setup' ), true ) ) {
					$notice_shown = true;

					if ( ebox_get_license_show_notice() ) {
						?>
						<div class="<?php echo esc_attr( ebox_get_license_class( 'notice notice-error is-dismissible ebox-license-is-dismissible' ) ); ?>" <?php echo ebox_get_license_data_attrs(); ?>> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Hardcoded, escaped in function. ?>
							<p><?php echo wp_kses_post( ebox_get_license_message( 2 ) ); ?></p>
						</div>
						<?php
					}
				}
		...
		public function admin_notice_upgrade_notice() {
			...
			if ( true !== $notice_shown_upgrade_notice ) {
				...
				if ( apply_filters( 'ebox_upgrade_notice_admin_show', true ) ) {
		...
		/**
		 * Returns the metadata of the ebox plugin
		 *
		 * @since 2.1.0
		 *
		 * @return object Metadata of the ebox plugin
		 */
		...
		public function check_update( $transient ) {
			...
			if ( ( $info ) && ( property_exists( $info, 'new_version' ) ) ) {
				...
				update_option( 'nss_plugin_remote_version_' . $this->slug, $remote_version, ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
			}
			...
			if ( ( ! empty( $remote_version ) ) && ( version_compare( $this->current_version, $remote_version, '<' ) ) ) {
				...
				if ( ! property_exists( $obj, 'icons' ) ) {
					...
					$obj->icons = array(
						'default' => ebox_LMS_PLUGIN_URL . '/assets/images/ld-plugin-image.jpg',
					);
				}
			...
			if ( ! empty( $this->update_path ) ) {
				if ( class_exists( 'ebox_Addon_Updater' ) ) {
					if ( is_null( $this->ld_updater ) ) {
						$this->ld_updater = ebox_Addon_Updater::get_instance();
					}
					$this->ld_updater->get_addon_plugins( $override_cache );
					return $this->ld_updater->update_plugin_readme( 'ebox-core-readme', $override_cache );
				}
			}
		}
		...
		public function check_info( $false, $action, $arg ) {
			...
			if ( $arg->slug === $this->slug ) {
				...
				if ( 'plugin_information' == $action ) {
					...
					update_option( 'nss_plugin_info_' . $this->slug, $information, ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
					$false = $information;
				}
			}
		...
		public function getRemote_version() {
			if ( ! empty( $this->update_path ) ) {
				...
				if ( defined( 'ebox_UPDATE_HTTP_METHOD' ) ) {
					// error_log( $_SERVER['SERVER_NAME'] . ': ' . __FUNCTION__ . ': ebox_UPDATE_HTTP_METHOD['. ebox_UPDATE_HTTP_METHOD . ']' );

					if ( 'post' === ebox_UPDATE_HTTP_METHOD ) {
						$request = wp_remote_post(
							$this->update_path,
							array(
								'body'    => array( 'action' => 'version' ),
								'timeout' => ebox_HTTP_REMOTE_POST_TIMEOUT,
							)
						);
					} elseif ( 'get' === ebox_UPDATE_HTTP_METHOD ) {
		...
		public function getRemote_information() {
			...
			if ( ( ! empty( $this->update_path ) ) && ( $this->time_to_recheck_information() ) ) {
				...
				if ( ! is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) === 200 ) {
					...
					update_option( 'nss_plugin_info_' . $this->slug, $information, ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
					update_option( 'nss_plugin_info_check_' . $this->slug, time(), ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
			...
			if ( ( ! empty( $this->update_path ) ) && ( $this->time_to_recheck_license() ) ) {
				...
				if ( ! is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) === 200 ) {
					...
					if ( '1' !== $request_body ) {
						...
						update_option( 'nss_plugin_check_' . $this->slug, time(), ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );

						/**
						 * NOTE: The getRemote_license() does not update the option.
						 * So we need to do it. And it needs to be set as an array structure.
						 */
						update_option( 'nss_plugin_remote_license_' . $this->slug, array( 'value' => $license_status ), ebox_PLUGIN_LICENSE_OPTIONS_AUTOLOAD );
					}
		...
		public function nss_plugin_license_menupage() {
			...
			if ( ! ebox_is_admin_user() ) {
				wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'ebox' ) );
			}
			...
			<div class=wrap>
				<form method="post" action="<?php echo esc_attr( $_SERVER['REQUEST_URI'] ); ?>">
					...
					?>
					<h1><?php esc_html_e( 'License Settings', 'ebox' ); ?></h1>
					<br />
					<?php
					if ( '1' === $license_status ) {
						?>
						<div class="notice notice-success">
							<p><?php esc_html_e( 'Your license is valid.', 'ebox' ); ?></p>
							</div>
							<?php
					} else {
						if ( ebox_get_license_show_notice() ) {
							...
							<div class="<?php echo esc_attr( ebox_get_license_class( 'notice notice-error is-dismissible ebox-license-is-dismissible' ) ); ?>" <?php echo ebox_get_license_data_attrs(); ?>> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Hardcoded, escaped in function ?>
								<p>
								<?php
								echo ebox_get_license_message(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Hardcoded, escaped in function.
								?>
								</p>
							</div>
							<?php
					...
					?>
					<p><label for="nss_plugin_license_email_<?php echo esc_attr( $code ); ?>"><?php esc_html_e( 'Email:', 'ebox' ); ?></label><br />
					...
					esc_html_e( apply_filters( 'format_to_edit', $email ), 'ebox' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText, WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP Core Hook
					?>" /></p> <?php // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentAfterEnd ?>

					<p><label ><?php esc_html_e( 'License Key:', 'ebox' ); ?></label><br />
					...
					/** This filter is documented in https://developer.wordpress.org/reference/hooks/format_to_edit/ */
					esc_html_e( apply_filters( 'format_to_edit', $license ), 'ebox' ); // phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText, WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP Core Hook
					?>" /></p> <?php // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentAfterEnd ?>

					<div class="submit">
						<input type="submit" name="update_nss_plugin_license_<?php echo esc_attr( $code ); ?>" value="<?php esc_html_e( 'Update License', 'ebox' ); ?>" class="button button-primary"/>
					</div>
				</form>
...
/**
 * Gets the `nss_plugin_updater_ebox_lms` instance.
 *
 ...
 * @return void|nss_plugin_updater_ebox_lms The `nss_plugin_updater_ebox_lms` instance.
 */
function ebox_get_updater_instance( $force_new = false ) {
	static $updater_ebox_lms = null;

	if ( true === $force_new ) {
		if ( ! is_null( $updater_ebox_lms ) ) {
			$updater_ebox_lms = null;
		}
	}

	if ( ! is_a( $updater_ebox_lms, 'nss_plugin_updater_ebox_lms' ) ) {
		$nss_plugin_updater_plugin_remote_path = 'https://support.ebox.com/';
		// $nss_plugin_updater_plugin_remote_path = 'http://local-support.ebox.com/';
		$nss_plugin_updater_plugin_slug = basename( ebox_LMS_PLUGIN_DIR ) . '/ebox_lms.php';
		$updater_ebox_lms               = new nss_plugin_updater_ebox_lms( $nss_plugin_updater_plugin_remote_path, $nss_plugin_updater_plugin_slug );
	}

	if ( ( $updater_ebox_lms ) && ( is_a( $updater_ebox_lms, 'nss_plugin_updater_ebox_lms' ) ) ) {
		return $updater_ebox_lms;
	}
}

/**
 * Checks Whether the ebox license is valid or not.
 *
 * @return boolean
 */
function ebox_is_ebox_license_valid() {
	if ( ebox_is_ebox_hub_active() ) { // new license system.
		return ebox_is_license_hub_valid();
	}

	$updater_ebox_lms = ebox_get_updater_instance();
	if ( ( $updater_ebox_lms ) && ( is_a( $updater_ebox_lms, 'nss_plugin_updater_ebox_lms' ) ) ) {
		return $updater_ebox_lms->is_license_valid();
	}
 ...
 */
function ebox_get_last_license_check_time() {
	if ( ebox_is_ebox_hub_active() ) { // new license system.
		return ebox_get_last_license_hub_check_time();
	}

	return intval( get_option( 'nss_plugin_check_ebox_lms', 0 ) );
}
 ...
 * @since 3.1.8
 */
function ebox_updates_enabled() {
	$updates_enabled = true;

	if ( ( defined( 'ebox_UPDATES_ENABLED' ) ) && ( true !== ebox_UPDATES_ENABLED ) ) {
	 ...
	 */
	return (bool) apply_filters( 'ebox_updates_enabled', $updates_enabled );
}
 ...
 * @since 3.1.8
 */
function ebox_get_license_show_notice() {
	if ( ( defined( 'ebox_LICENSE_PANEL_SHOW' ) ) && ( false === ebox_LICENSE_PANEL_SHOW ) ) {
		return false;
	}

	if ( ! ebox_updates_enabled() ) {
		$current_screen = get_current_screen();
		if ( ! in_array( $current_screen->id, array( 'admin_page_nss_plugin_license-ebox_lms-settings', 'admin_page_ebox-setup' ), true ) ) {
			return false;
		}

		$user_id = get_current_user_id();
		if ( ! empty( $user_id ) ) {
			$notice_dismissed_timestamp = get_user_meta( $user_id, 'ebox_license_notice_dismissed', true );
 ...
 */
function ebox_get_license_message( $mode = 1 ) {
	if ( ebox_updates_enabled() ) {
		if ( 2 === $mode ) {
			$updater_ebox_lms = ebox_get_updater_instance();
			return sprintf(
				// translators: placeholders: Plugin name. Plugin update link.
				esc_html_x( 'License of your plugin %1$s is invalid or incomplete. Please click %2$s and update your license.', 'placeholders: Plugin name. Plugin update link.', 'ebox' ),
				'<strong>' . esc_html( $updater_ebox_lms->get_plugin_data()->Name ) . '</strong>',
				'<a href="' . get_admin_url( null, 'admin.php?page=nss_plugin_license-ebox_lms-settings' ) . '">' . esc_html__( 'here', 'ebox' ) . '</a>'
			);
		} elseif ( 1 === $mode ) {
			return sprintf(
				// translators: placeholder: Link to purchase ebox.
				esc_html_x( 'Please enter your email and a valid license or %s a license now.', 'placeholder: link to purchase ebox', 'ebox' ),
				"<a href='http://www.ebox.com/' target='_blank' rel='noreferrer noopener'>" . esc_html__( 'buy', 'ebox' ) . '</a>'
			);
		}
	} else {
		return sprintf(
			// translators: placeholders: Plugin name. Plugin update link.
			esc_html_x( 'ebox update and license calls are temporarily disabled. Click %s for more information.', 'placeholders: FAQ update link.', 'ebox' ),
			'<a target="_blank" rel="noopener noreferrer" aria-label="' . esc_html__( 'opens in a new tab', 'ebox' ) . '" href="https://www.ebox.com/support/docs/faqs/why-are-the-license-updates-and-license-checks-disabled-on-my-site/">' . esc_html__( 'here', 'ebox' ) . '</a>'
		);
	}
}
 ...
 */
function ebox_get_license_class( $class = '' ) {
	if ( ! ebox_updates_enabled() ) {
		$class = 'notice notice-info is-dismissible ebox-updates-disabled-dismissible';
	}
 ...
 * @since 3.1.8
 */
function ebox_get_license_data_attrs() {
	if ( ! ebox_updates_enabled() ) {
 ...
 * @since 3.1.8
 */
function ebox_license_notice_dismissed_ajax() {
	$user_id = get_current_user_id();
	if ( ! empty( $user_id ) ) {
		if ( ( isset( $_POST['action'] ) ) && ( 'ebox_license_notice_dismissed' === $_POST['action'] ) ) {
			if ( ( isset( $_POST['ebox_license_notice_dismissed_nonce'] ) ) && ( ! empty( $_POST['ebox_license_notice_dismissed_nonce'] ) ) && ( wp_verify_nonce( $_POST['ebox_license_notice_dismissed_nonce'], 'notice-dismiss-nonce-' . $user_id ) ) ) {
				update_user_meta( $user_id, 'ebox_license_notice_dismissed', time() );
			}
		}
	}
...
add_action( 'wp_ajax_ebox_license_notice_dismissed', 'ebox_license_notice_dismissed_ajax' );
 ...
 * @since 4.3.1
 */
function ebox_hub_upgrade_dismissed_ajax() {
	$user_id = get_current_user_id();
	if ( ! empty( $user_id ) ) {
		if ( ( isset( $_POST['action'] ) ) && ( 'ebox_hub_upgrade_dismissed' === $_POST['action'] ) ) {
			if ( ( isset( $_POST['ebox_hub_upgrade_dismissed_nonce'] ) ) && ( ! empty( $_POST['ebox_hub_upgrade_dismissed_nonce'] ) ) && ( wp_verify_nonce( $_POST['ebox_hub_upgrade_dismissed_nonce'], 'notice-dismiss-nonce-' . $user_id ) ) ) {
				delete_option( 'ebox_show_hub_upgrade_admin_notice' );
			}
		}
	}
...
add_action( 'wp_ajax_ebox_hub_upgrade_dismissed', 'ebox_hub_upgrade_dismissed_ajax' );
 ...
 * @since 3.1.8
 */
function ebox_license_hide_propanel_notice() {
	if ( ! ebox_updates_enabled() ) {
		?>
		<style>
...
add_filter( 'admin_footer', 'ebox_license_hide_propanel_notice', 99 );
