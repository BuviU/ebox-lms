<?php
/**
 * ebox class for displaying the setup wizard.
 *
 * @package    ebox
 * @since      4.0.0
 */
...
if ( ! class_exists( 'ebox_Setup_Wizard' ) ) {
	/**
	 * Setup wizard class.
	 */
	class ebox_Setup_Wizard {
		/**
		 * The opened status, can be completed, ongoing or closed.
		 */
		const STATUS_KEY = 'ebox_setup_wizard_status';

		const STATUS_COMPLETED = 'completed';
		const STATUS_ONGOING   = 'ongoing';
		const STATUS_CLOSED    = 'closed';

		const DATA_KEY = 'ebox_setup_wizard';

		const CERTIFICATE_BUILDER_SLUG   = 'ebox-certificate-builder/ebox-certificate-builder.php';
		const COURSE_GRID_SLUG           = 'ebox-course-grid/ebox_course_grid.php';
		const WOOCOMMERCE_SLUG           = 'woocommerce/woocommerce.php';
		const ebox_WOOCOMMERCE_SLUG = 'ebox-woocommerce/ebox_woocommerce.php';

		const HANDLE = 'ebox-setup-wizard';

		const LICENSE_KEY       = 'nss_plugin_license_ebox_lms';
		const LICENSE_EMAIL_KEY = 'nss_plugin_license_email_ebox_lms';

		const ADMIN_REDIRECT_PAGE       = 'admin.php?page=ebox-setup';
		const FINAL_ADMIN_REDIRECT_PAGE = 'admin.php?page=ebox-setup';
		...
		public function __construct() {
			...
			add_action( 'ebox_activated', array( $this, 'set_redirect_flag' ) );
			add_action( 'admin_init', array( $this, 'redirect_after_activation' ), 1 );
			add_action( 'admin_menu', array( $this, 'register_menu' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
			add_action( 'admin_init', array( $this, 'dismiss' ) );
			add_action( 'wp_ajax_ebox_setup_wizard_verify_license', array( $this, 'verify_license' ) );
			add_action( 'wp_ajax_ebox_setup_wizard_save_data', array( $this, 'save_data' ) );
			add_action( 'wp_ajax_ebox_finalize', array( $this, 'finalize_setup' ) );
		...
		public function enable_stripe_connect_and_redirect() {
			...
			if ( ebox_Settings_Section_Stripe_Connect::is_stripe_connected() ) {
			...
			ebox_safe_redirect( admin_url( 'admin.php?page=' . self::HANDLE ) );
		}
		...
		public function set_redirect_flag() {
			...
			update_option( 'ebox_setup_wizard_redirect', true );
		}
		...
		public function redirect_after_activation() {
			$should_redirect = get_option( 'ebox_setup_wizard_redirect' );
			if ( ! $should_redirect ) {
				return;
			}

			delete_option( 'ebox_setup_wizard_redirect' );
			ebox_safe_redirect( admin_url( 'admin.php?page=' . self::HANDLE ) );
		}
		...
		public function dismiss() {
			...
			ebox_safe_redirect( admin_url( self::ADMIN_REDIRECT_PAGE ) );
		}
		...
		private function get_completed_redirect_url(): string {
			/**
				 ...
				 */
				return apply_filters( 'ebox_setup_wizard_completed_redirect_url', admin_url( self::FINAL_ADMIN_REDIRECT_PAGE ) );
		}
		...
		public function finalize_setup() {
			...
			switch ( $data['step'] ) {
				...
				case 'process_certificate_builder':
					...
					break;
				case 'process_woo':
					// install woocommerce plugin and ebox add-on.
					if ( 'true' === $data['woocommerce'] ) {
						$ret = $this->maybe_install_a_plugin( self::WOOCOMMERCE_SLUG );
						if ( true === $ret ) {
							activate_plugin( self::WOOCOMMERCE_SLUG );
						}
						$ret = $this->maybe_install_a_plugin( self::ebox_WOOCOMMERCE_SLUG );
						if ( true === $ret ) {
							activate_plugin( self::ebox_WOOCOMMERCE_SLUG );
						}
					}
					break;
				case 'update_settings':
					...
					ebox_Settings_Section::set_section_setting( 'ebox_Settings_Section_Emails_Sender_Settings', 'from_email', $from_email );

					// team access and management.
					if ( is_array( $data['course_type'] ) && in_array( 'team_courses', $data['course_type'], true ) ) {
						// optional team settings.
						ebox_Settings_Section::set_section_setting( 'ebox_Settings_Teams_CPT', 'public', isset( $data['public_team'] ) && $data['public_team'] ? 'yes' : '' );
						ebox_Settings_Section::set_section_setting( 'ebox_Settings_Teams_CPT', 'has_archive', isset( $data['team_archive_page'] ) && $data['team_archive_page'] ? 'yes' : '' );
						ebox_Settings_Section::set_section_setting( 'ebox_Settings_Section_Teams_Team_Leader_User', 'manage_courses_enabled', isset( $data['manage_user'] ) && $data['manage_user'] ? 'yes' : '' );
						ebox_Settings_Section::set_section_setting( 'ebox_Settings_Section_Teams_Team_Leader_User', 'teams_autoenroll_managed', isset( $data['team_auto_enroll'] ) && $data['team_auto_enroll'] ? 'yes' : '' );
						ebox_Settings_Section::set_section_setting( 'ebox_Settings_Section_Teams_Team_Leader_User', 'courses_autoenroll', isset( $data['course_auto_enroll'] ) && $data['course_auto_enroll'] ? 'yes' : '' );
						ebox_Settings_Section::set_section_setting( 'ebox_Settings_Section_Teams_Team_Leader_User', 'bypass_course_limits', isset( $data['bypass_course_limit'] ) && $data['bypass_course_limit'] ? 'yes' : '' );
					}
			...
			if ( $done ) {
				 ...
				 * @since 4.1.2
				 */
				do_action( 'ebox_setup_wizard_completed' );
			}
		...
		protected function create_courses_listing_page() {
			wp_insert_post(
				array(
					'post_title'   => __( 'Courses', 'ebox' ),
					'post_content' => '<!-- wp:ebox/ld-course-list /-->',
					'post_status'  => 'publish',
					'post_type'    => 'page',
				)
			);
		...
		protected function create_profile_page(): void {
			wp_insert_post(
				array(
					'post_title'   => __( 'Profile', 'ebox' ),
					'post_content' => '<!-- wp:ebox/ld-profile /-->',
					'post_status'  => 'publish',
					'post_type'    => 'page',
				)
			);
		...
		protected function create_registration_pages() {
			...
			$page_data = array(
				array(
					...
					'title'      => 'Registration',
					'content'    => '<!-- wp:ebox/ld-registration /-->',
				),
				array(
					'identifier' => 'registration_success',
					'title'      => 'Registration Success',
					'content'    => '<!-- wp:paragraph --><p>' . __( 'Welcome', 'ebox' ) . '</p><!-- /wp:paragraph -->',
				),
				array(
					'identifier' => 'reset_password',
					'title'      => 'Reset Password',
					'content'    => '<!-- wp:ebox/ld-reset-password {"width":""} /-->',
				),
			);

			foreach ( $page_data as $page_values ) {
				$page_id = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Registration_Pages', $page_values['identifier'] );

				if ( ! empty( $page_id ) ) {
					continue;
				}

				ebox_Settings_Section::set_section_setting(
		...
		public function verify_license() {
			...
			if ( ebox_is_ebox_hub_active() ) {
				$license_status = ebox_validate_hub_license( $email, $license_key );
			} else {
				$updater_ebox_lms = ebox_get_updater_instance( true );
				if ( ( $updater_ebox_lms ) && ( is_a( $updater_ebox_lms, 'nss_plugin_updater_ebox_lms' ) ) ) {
					 ...
					 */
					delete_option( 'nss_plugin_check_ebox_lms' );
					$license_status = $updater_ebox_lms->getRemote_license();
				}
		 ...
		 * @param string $option_name Option name. Default: 'ebox_setup_wizard'.
		 */
		...
		private function get_scenes(): array {
			$available_scenes = array(
				'step-0' => esc_html__( 'Welcome', 'ebox' ),
				'step-1' => esc_html__( 'Your Info', 'ebox' ),
				'step-2' => esc_html__( 'Your Courses', 'ebox' ),
				'step-3' => esc_html__( 'Payment', 'ebox' ),
				'step-4' => esc_html__( 'Summary', 'ebox' ),
			);
			 ...
			 */
			$available_scenes = apply_filters( 'ebox_setup_wizard_available_scenes', $available_scenes );

			// make sure that we have at least one scene.
			if ( empty( $available_scenes ) ) {
				$available_scenes = array( 'step-0' => esc_html__( 'Welcome', 'ebox' ) );
			}
		...
		public function enqueue_scripts() {
			...
			if ( is_object( $screen ) && 'toplevel_page_ebox-setup-wizard' === $screen->id ) {
				wp_register_style(
					self::HANDLE,
					ebox_LMS_PLUGIN_URL . 'assets/js/setup-wizard/dist/css/style.css',
					array(),
					constant( 'SCRIPT_DEBUG' ) === true ? time() : ebox_VERSION
				);
				wp_enqueue_style( self::HANDLE );

				wp_register_script(
					self::HANDLE,
					ebox_LMS_PLUGIN_URL . 'assets/js/setup-wizard/dist/js/index.js',
					array( 'react', 'react-dom', 'wp-i18n' ),
					constant( 'SCRIPT_DEBUG' ) === true ? time() : ebox_VERSION,
					true
				);
				$data             = get_option( self::DATA_KEY );
				$currency_code    = ebox_get_currency_code();
				$currency_country = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Payments_Defaults', 'country' ) ?? '';
				...
				wp_localize_script(
					...
					array(
						'urls'           => array(
							'assets'         => ebox_LMS_PLUGIN_URL . 'assets/js/setup-wizard/dist/',
							'dismiss'        => admin_url(
								wp_sprintf(
							...
							'support'        => 'https://support.ebox.com',
							'stripe_connect' => ebox_Settings_Section_Stripe_Connect::generate_connect_url( admin_url( 'admin-post.php?action=stripe_connect_wizard_process' ) ),
						...
						'data'           => array(
							...
							'team_access'             => $data['team_access'] ?? 'no',
							'team_leader'             => $data['team_leader'] ?? 'no',
							...
							'currency_select2_default' => ! empty( $currency_country ) ? ucwords( mb_strtolower( $currency_country ) ) . ' (' . ebox_get_currency_symbol( $currency_code ) . ') ' : '',
							'stripe_connected'         => ebox_Settings_Section_Stripe_Connect::is_stripe_connected(),
							'stripe_webhook_notice'    => wp_kses_post( ebox_Settings_Section_Stripe_Connect::get_stripe_webhook_notice() ),
						),
						'plugins'        => array(
		...
		protected function should_display(): bool {
			 ...
			 */
			return apply_filters( 'ebox_setup_wizard_should_display', $should_display );
		}
