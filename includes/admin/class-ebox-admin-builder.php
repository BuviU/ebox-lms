<?php
/**
 * ebox Course Builder Metabox Base.
 *
 * @since 2.6.0
...
if ( ! class_exists( 'ebox_Admin_Builder' ) ) {

	/**
	 * Class ebox Course Builder Metabox Base.
	 *
	 * @since 2.6.0
	 */
	class ebox_Admin_Builder {
		 ...
		 * @var string
		 */
		protected $builder_prefix = 'ebox_builder';
		...
		public function builder_admin_footer() {
			...
			wp_enqueue_style(
				'ebox-new-builder-style',
				ebox_LMS_PLUGIN_URL . 'assets/js/builder/dist/builder' . ebox_min_builder_asset() . '.css',
				array( 'wp-editor' ),
				ebox_SCRIPT_VERSION_TOKEN
			);
			wp_style_add_data( 'ebox-new-builder-style', 'rtl', 'replace' );

			wp_enqueue_script(
				'ebox-new-builder-script',
				ebox_LMS_PLUGIN_URL . 'assets/js/builder/dist/builder' . ebox_min_builder_asset() . '.js',
				array( 'wp-i18n', 'ebox-new-header-script', 'wp-data' ),
				ebox_SCRIPT_VERSION_TOKEN,
				true
			);
			...
			wp_enqueue_script(
				'ebox-force-metaboxes',
				ebox_LMS_PLUGIN_URL . 'assets/js/builder/dist/metaboxes' . ebox_min_builder_asset() . '.js',
			...
			wp_localize_script( 'ebox-new-builder-script', 'ebox_builder_assets', $this->builder_assets );
			wp_localize_script( 'ebox-force-metaboxes', 'ebox_builder_metaboxes', $metaboxes );
		...
		public function show_builder_box( $post ) {
			...
			?>
			<div id="ebox_builder_box_wrap" class="ebox_builder_box_wrap" data-ld-course-id="<?php echo intval( $this->builder_post_id ); ?>" data-ld-typenow="<?php echo esc_attr( $post->post_type ); ?>">
		...
		public function show_builder_header_right() {
			...
			<div class="ebox-header-right">
				<span class="ld-show-all"><?php esc_html_e( 'Expand All', 'ebox' ); ?></span>
				<span class="ld-divide-all">|</span>
				<span class="ld-hide-all"><?php esc_html_e( 'Collapse All', 'ebox' ); ?></span>
			</div>
			<?php
		}
			...
			foreach ( $this->selector_post_types as $selector_post_type ) {
				...
				if ( is_a( $post_type_object, 'WP_Post_Type' ) ) {
					...
					$this->builder_assets[ $this->builder_post_type ]['messages'][ 'confirm_remove_' . $selector_post_type ] = sprintf(
						...
						ebox_Custom_Label::get_label( $this->get_label_for_post_type( $selector_post_type ) ),
						ebox_Custom_Label::get_label( $builder_post_type_label )
					);
					...
					if ( ! empty( $post_type_query_args ) ) {
						...
						?>
						<div class="ebox-selector-container ebox-selector-container-<?php echo esc_attr( $selector_post_type ); ?>" data-ld-type="<?php echo esc_attr( $selector_post_type ); ?>" data-ld-selected="<?php echo esc_attr( $selector_post_type_steps ); ?>">
							<h3 class="ebox-selector-header"><span class="ebox-selector-title"><?php echo ebox_get_custom_label( $this->get_label_for_post_type( $selector_post_type, false ) ); ?></span><span class="ld-course-builder-action ld-course-builder-action-show-hide ld-course-builder-action-show dashicons" title="<?php esc_html_e( 'Expand/Collapse Section', 'ebox' ); ?>"></span><span class="ld-course-builder-action ld-course-builder-action-add dashicons" title="<?php esc_html_e( 'New', 'ebox' ); ?>"><img src="<?php echo esc_url( admin_url( 'images/wpspin_light-2x.gif' ) ); ?>" alt="" /></span></h3> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output ?>
							<div class="ebox-selector-post-listing">
								<?php
									$row_single = $this->build_selector_row_single( null, $selector_post_type );
								if ( ! empty( $row_single ) ) {
									?>
										<ul class="ebox-row-placeholder" style="display:none"><?php echo $row_single; ?></ul> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML ?>
										<?php
								}
								?>
								<div class="ebox-selector-pager">
									<p class="pager-info">
								...
								<div class="ebox-selector-search"><input type="text" placeholder="<?php esc_html_e( 'Search...', 'ebox' ); ?>" /></div>
								<ul id="ebox-selector-post-listing-<?php echo esc_attr( $selector_post_type ); ?>" class="ebox-selector-post-listing dropfalse"> <?php // cspell:disable-line. ?>
									<?php
...
if ( ( defined( 'ebox_COURSE_BUILDER' ) ) && ( ebox_COURSE_BUILDER === true ) ) {
	require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-builders/class-ebox-admin-course-builder-metabox.php';
}
if ( ( defined( 'ebox_QUIZ_BUILDER' ) ) && ( ebox_QUIZ_BUILDER === true ) ) {
	require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-builders/class-ebox-admin-quiz-builder-metabox.php';
}

add_action(
	'wp_ajax_ebox_builder_selector_pager',
	function() {
		...
		$builder_data = ebox_verify_builder_data( $builder_data );
		...
		if ( is_a( $builder_instance, $builder_data['builder_class'] ) ) {
			...
			$builder_instance->ebox_builder_selector_pager( $builder_query_args );
		}
		echo wp_json_encode( array() );
		wp_die();
	}
...
add_action(
	'wp_ajax_ebox_builder_selector_search',
	function() {
		if ( ! current_user_can( 'read' ) ) {
			echo wp_json_encode( array() );
			wp_die();
		...
		if ( is_a( $builder_instance, $builder_data['builder_class'] ) ) {
			$builder_instance->builder_init( $builder_data['builder_post_id'] );
			$builder_instance->ebox_builder_selector_search( $builder_query_args );
		}
		echo wp_json_encode( array() );
		wp_die();
	}
...
add_action(
	'wp_ajax_ebox_builder_selector_step_trash',
	function() {
		...
		if ( is_a( $builder_instance, $builder_data['builder_class'] ) ) {
			$builder_instance->builder_init( $builder_data['builder_post_id'] );
			$builder_instance->ebox_builder_selector_step_trash( $builder_query_args );
		}
		echo wp_json_encode( array() );
		wp_die();
	}
...
add_action(
	'wp_ajax_ebox_builder_selector_step_new',
	function() {
		...
		if ( is_a( $builder_instance, $builder_data['builder_class'] ) ) {
			$builder_instance->builder_init( $builder_data['builder_post_id'] );
			$builder_instance->ebox_builder_selector_step_new( $builder_query_args );
		}
		echo wp_json_encode( array() );
		wp_die();
	}
...
add_action(
	'wp_ajax_ebox_builder_selector_step_title',
	function() {
		if ( ! current_user_can( 'edit_courses' ) ) {
			echo wp_json_encode( array() );
			wp_die();
		...
		if ( is_a( $builder_instance, $builder_data['builder_class'] ) ) {
			$builder_instance->builder_init( $builder_data['builder_post_id'] );
			$builder_instance->ebox_builder_selector_step_title( $builder_query_args );
		}
		echo wp_json_encode( array() );
 ...
 */
function ebox_verify_builder_data( $builder_data = array() ) {
	...
	$nonce_field_value = 'ebox_builder_' . $builder_data['builder_post_type'] . '_' . $builder_data['builder_post_id'] . '_nonce';
	if ( ( ! isset( $builder_data['builder_nonce'] ) ) || ( ! wp_verify_nonce( $builder_data['builder_nonce'], $nonce_field_value ) ) ) {
		echo wp_json_encode( array() );
		wp_die();
	}

	if ( ! is_subclass_of( $builder_data['builder_class'], 'ebox_Admin_Builder' ) ) {
		echo wp_json_encode( array() );
		wp_die();
	}
