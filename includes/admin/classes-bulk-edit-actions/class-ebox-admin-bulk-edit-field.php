...
if ( ! class_exists( 'ebox_Admin_Bulk_Edit_Field' ) ) {
	/**
	 * ebox admin field abstract class.
	 *
	 * @since 4.2.0
	 */
	class ebox_Admin_Bulk_Edit_Field {
			...
			if ( count( array_intersect_key( $attributes, array_flip( self::MINIMUM_REQUIRED_ATTRIBUTES ) ) ) !== count( self::MINIMUM_REQUIRED_ATTRIBUTES ) ) {
				wp_die(
					sprintf(
						...
						esc_html__( 'Missing required attributes for field: %s', 'ebox' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
