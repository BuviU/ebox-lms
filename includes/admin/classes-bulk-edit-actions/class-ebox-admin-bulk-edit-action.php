<?php
/**
 * ebox Bulk Edit Action abstract class.
 *
 * @since 4.2.0
 *
...
if ( ! class_exists( 'ebox_Admin_Bulk_Edit_Action' ) ) {
	/**
	 * ebox Bulk Edit Action abstract class.
	 *
	 * @since 4.2.0
	 */
	abstract class ebox_Admin_Bulk_Edit_Action {
		/**
		 * Filters.
		 *
		 * @since 4.2.0
		 ...
		 * @since 4.2.0
		 *
		 * @param ebox_Admin_Action_Scheduler $bulk_edit_scheduler The bulk edit scheduler instance.
		 *
		 * @return void
		 */
		public function init( ebox_Admin_Action_Scheduler $bulk_edit_scheduler ): void {
			$this->bulk_edit_scheduler = $bulk_edit_scheduler;

			$this->init_fields();
			$this->init_filters();

			if ( is_admin() && ebox_is_admin_user() ) {
				add_action(
		 ...
		 * @since 4.2.0
		 *
		 * @return ebox_Admin_Bulk_Edit_Field[]
		 */
		public function get_fields(): array {
			 ...
			 */
			$fields = apply_filters( 'ebox_bulk_edit_fields', $this->fields, $this );

			$mapped_fields = array();

			foreach ( $fields as $field ) {
				if ( ! $field instanceof ebox_Admin_Bulk_Edit_Field ) {
					continue;
				}
		...
		public function get_filters(): array {
			 ...
			 * @param object $class   Instance of the called class.
			 */
			$filters = apply_filters( 'ebox_bulk_edit_filters', $this->filters, $this );

			$mapped_filters = array();

			foreach ( $filters as $filter ) {
				if ( ! $filter instanceof ebox_Admin_Filter ) {
					continue;
				}
		...
		public function process_ajax_action_get_affected_posts_number(): void {
			...
			if ( null === $affected_posts_number ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Failed to get affected posts number.', 'ebox' ),
					)
				);
			}
		...
		public function process_ajax_action_update_posts() {
			...
			if ( empty( $enabled_fields ) ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'No fields selected.', 'ebox' ),
					)
				);
			}
			...
			if ( empty( $affected_posts_number ) ) {
				wp_send_json_error(
					array(
						...
						'message' => sprintf( __( 'No affected %s.', 'ebox' ), $this->get_tab_name() ),
					)
				);
			}
			...
			$this->bulk_edit_scheduler->enqueue_task(
				...
				sprintf(
					...
					__( 'The bulk editing of %1$d %2$s is in the processing queue. Please refresh this page to see the progress.', 'ebox' ),
					$affected_posts_number,
					'<b>' . $this->get_tab_name() . '</b>'
				),
				sprintf(
					// translators: placeholder: affected posts number, post type.
					__( 'The bulk editing of %1$d %2$s is running. Please refresh this page to see the progress.', 'ebox' ),
					$affected_posts_number,
					'<b>' . $this->get_tab_name() . '</b>'
				)
			);
			...
			wp_send_json_success(
				array(
					'message' => sprintf(
						// translators: placeholder: affected posts number, post type.
						__( 'The bulk editing of %1$d %2$s will be processed soon. You can see the progress refreshing this page.', 'ebox' ),
						$affected_posts_number,
						$this->get_tab_name()
					),
				)
		...
		public function update_posts_task( array $affected_posts_ids, array $fields ): void {
			...
			ebox_Admin_Action_Scheduler::add_admin_notice(
				sprintf(
					// translators: placeholder: affected posts number, post type.
					__( 'The bulk edit of %1$d %2$s finished successfully.', 'ebox' ),
		...
		private function get_bulk_edit_task_name(): string {
			return 'ebox_bulk_edit_task_' . $this->get_post_type();
		}
