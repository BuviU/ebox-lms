<?php
/**
 * ebox Quiz Essays (ebox-essays) Posts Listing.
 *
 * @since 3.2.3
 * @package ebox\Essay\Listing
 */
...
if ( ( class_exists( 'ebox_Admin_Posts_Listing' ) ) && ( ! class_exists( 'ebox_Admin_Essays_Listing' ) ) ) {

	/**
	 * Class ebox Quiz Essays (ebox-essays) Posts Listing.
	 *
	 * @since 3.2.3
	 * @uses ebox_Admin_Posts_Listing
	 */
	class ebox_Admin_Essays_Listing extends ebox_Admin_Posts_Listing {
		const VIEW_AJAX_ACTION = 'ebox_essay_load_modal_content';
		...
		public function __construct() {
			$this->post_type = ebox_get_post_type_slug( 'essay' );
		...
		public function listing_init() {
			...
			$this->selectors = array(
				'author'      => array(
					'type'                     => 'user',
					'show_all_value'           => '',
					'show_all_label'           => esc_html__( 'All Authors', 'ebox' ),
				...
				'topic_id'    => array(
					'type'                     => 'post_type',
					'post_type'                => ebox_get_post_type_slug( 'topic' ),
					'show_all_value'           => '',
					'show_all_label'           => sprintf(
						// translators: placeholder: Topics.
						esc_html_x( 'All %s', 'placeholder: Topics', 'ebox' ),
				...
				'quiz_id'     => array(
					...
					'show_all_label'           => sprintf(
						...
						esc_html_x( 'All %s', 'placeholder: Quizzes', 'ebox' ),
				...
				'question_id' => array(
					...
					'show_all_label'           => sprintf(
						...
						esc_html_x( 'All %s', 'placeholder: Questions', 'ebox' ),
			...
			$this->columns = array(
				'title'           => array(
					'label' => sprintf(
						...
						esc_html_x( 'Essay %s Title', 'placeholder: Essay Question Title', 'ebox' ),
						ebox_get_custom_label( 'question' )
					),
				),
				'author'          => array(
					'label' => esc_html__( 'Submitted By', 'ebox' ),
				),
				'approval_status' => array(
					'label'   => esc_html__( 'Status / Points', 'ebox' ),
					'after'   => 'author',
				...
				'quiz'            => array(
					'label'   => sprintf(
						...
						esc_html_x( 'Assigned %s', 'Assigned Quiz Label', 'ebox' ),
				...
				'question'        => array(
					'label'   => sprintf(
						...
						esc_html_x( 'Assigned %s', 'Assigned Question Label', 'ebox' ),
				...
				'course'          => array(
					'label'    => sprintf(
						...
						esc_html_x( 'Assigned %s', 'Assigned Course Label', 'ebox' ),
				...
				'lesson_topic'    => array(
					'label'   => sprintf(
						...
						esc_html_x( 'Assigned %1$s / %2$s', 'Placeholders: Lesson, Topic', 'ebox' ),
			...
			add_filter(
				'ebox_listing_selector_user_selector_query_args',
				array( $this, 'ebox_listing_selector_user_query_args_essays' ),
				30,
				2
			);
		}
		...
		public function on_load_listing() {
			if ( $this->post_type_check() ) {
				...
				add_filter( 'ebox_admin_settings_data', array( $this, 'add_ebox_admin_settings_data' ) );
				add_filter( 'post_row_actions', array( $this, 'add_inline_actions' ), 30, 2 );
				add_filter( 'ebox_listing_table_query_vars_filter', array( $this, 'listing_table_query_vars_filter_essays' ), 30, 3 );
		...
		public function listing_table_query_vars_filter_essays( $q_vars, $post_type, $query ) {
			if ( $post_type === $this->post_type ) {
				...
				if ( ( isset( $author_selector['selected'] ) ) && ( ! empty( $author_selector['selected'] ) ) ) {
					...
					} elseif ( ( ebox_is_team_leader_user( get_current_user_id() ) ) && ( 'advanced' !== ebox_get_team_leader_manage_users() ) ) {
						if ( ebox_is_team_leader_of_user( get_current_user_id(), $author_selector['selected'] ) ) {
							$q_vars['author__in'] = array( $author_selector['selected'] );
						} else {
				...
				} elseif ( ( isset( $team_selector['selected'] ) ) && ( ! empty( $team_selector['selected'] ) ) ) {
					$user_ids = ebox_get_teams_user_ids( absint( $team_selector['selected'] ) );
					if ( ! empty( $user_ids ) ) {
		 ...
		 * @param string $post_type Post Type being displayed.
		 */
		public function ebox_listing_selector_user_query_args_essays( $q_vars, $post_type ) {
			if ( $post_type === $this->post_type ) {
				$team_selector = $this->get_selector( 'team_id' );
				if ( ( isset( $team_selector['selected'] ) ) && ( ! empty( $team_selector['selected'] ) ) ) {
					$user_ids = ebox_get_teams_user_ids( absint( $team_selector['selected'] ) );
					if ( ! empty( $user_ids ) ) {
		...
		protected function listing_filter_by_lesson( $q_vars, $selector = array() ) {
			if ( ( isset( $selector['selected'] ) ) && ( ! empty( $selector['selected'] ) ) ) {
				if ( ( isset( $selector['show_empty_value'] ) ) && ( $selector['show_empty_value'] === $selector['selected'] ) ) {
					...
					if ( ! empty( $course_id ) ) {
						$topics = ebox_get_topic_list( $selector['selected'], $course_id );
						if ( ! empty( $topics ) ) {
		...
		protected function listing_filter_by_topic( $q_vars, $selector = array() ) {
			if ( ( isset( $selector['selected'] ) ) && ( ! empty( $selector['selected'] ) ) ) {
				if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) ) {
					$steps_ids = ebox_course_get_steps_by_type( $selector['selected'], $this->post_type );
					if ( ! empty( $steps_ids ) ) {
		...
		public function essay_bulk_actions() {
			...
			if ( ( ! empty( $post->post_type ) ) && ( ebox_get_post_type_slug( 'essay' ) === $post->post_type ) ) {
					...
					<script type="text/javascript">
						...
						jQuery( 'table.wp-list-table' ).on( 'click', 'button.editinline', function( e ) {
							...
							if ( typeof select_post_status !== 'undefined' ) {
								var select_parent_el = select_post_status.parents('.inline-edit-team');
		 ...
		 * @return array
		 */
		public function add_ebox_admin_settings_data( array $script_data ): array {
			$script_data['labels']['download'] = esc_html__( 'Download', 'ebox' );
		...
		public function add_inline_actions( array $row_actions, WP_Post $post ): array {
			...
			if ( empty( $file_url ) || $file_is_image ) {
				...
				$row_actions['quick_view'] = sprintf(
					'<a class="view-ebox-essay" href="%s" aria-label="%s">%s</a>',
					esc_url( $view_url ),
					esc_attr( $view_label ),
					esc_html( $view_label )
				);
		...
		protected function essay_bulk_actions_approve() {
			...
			if ( ( ! isset( $_REQUEST['post_type'] ) ) || ( ebox_get_post_type_slug( 'essay' ) !== $_REQUEST['post_type'] ) ) {
				return;
			}
			...
			if ( 'approve_essay' === $action ) {
				...
				if ( ( isset( $_REQUEST['post'] ) ) && ( ! empty( $_REQUEST['post'] ) ) ) { // @phpstan-ignore-line
					...
					foreach ( $essays as $essay_id ) {
						...
						if ( ( ! empty( $essay_post ) ) && ( $essay_post instanceof WP_Post ) && ( ebox_get_post_type_slug( 'essay' ) === $essay_post->post_type ) ) {
							...
							// Stole the following section ot code from ebox_save_essay_status_metabox_data().
							$submitted_essay_data = ebox_get_submitted_essay_data( $quiz_id, $question_id, $essay_post );
							 ...
							 */
							$submitted_essay_data = apply_filters( 'ebox_essay_status_data', $submitted_essay_data );
							ebox_update_submitted_essay_data( $quiz_id, $question_id, $essay_post, $submitted_essay_data );
							...
							if ( ! is_null( $submitted_essay_data['points_awarded'] ) ) {
								 ...
								 */
								$updated_scoring = apply_filters( 'ebox_updated_essay_scoring', $updated_scoring_data );
								ebox_update_quiz_data( $quiz_id, $question_id, $updated_scoring_data, $essay_post );
								 ...
								 */
								do_action( 'ebox_essay_all_quiz_data_updated', $quiz_id, $question_id, $updated_scoring_data, $essay_post );
							}
						}
					}
				}
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( ( ! empty( $post_status_object ) ) && ( is_object( $post_status_object ) ) && ( property_exists( $post_status_object, 'label' ) ) ) {
					echo '<div class="ld-approval-status">' . sprintf(
						...
						esc_html_x( 'Status: %s', 'placeholder: Status', 'ebox' ),
				...
				if ( ! empty( $quiz_id ) ) {
					...
					if ( $question instanceof WpProQuiz_Model_Question ) {
						...
						if ( 'not_graded' === $essay->post_status ) {
							$points_label = '<label class="ebox-listing-row-field-label" for="essay_points_' . absint( $post_id ) . '">' . esc_html__( 'Points', 'ebox' ) . '</label>';

							$points_input = '<input id="essay_points_' . absint( $post_id ) . '" class="small-text ebox-award-points" type="number" value="' . absint( $current_points ) . '" max="' . absint( $max_points ) . '" min="0" step="1" name="essay_points[' . absint( $post_id ) . ']" />';

							echo sprintf(
								// translators: placeholders: Points label, points input, maximum points.
								esc_html_x( '%1$s: %2$s / %3$d', 'placeholders: Points label, points input, maximum points', 'ebox' ),
							...
							$points_field = '<span class="ebox-listing-row-field-label">' . esc_html__( 'Points', 'ebox' ) . '</span>';
							echo sprintf(
								// translators: placeholders: Points label, current points, maximum points.
								esc_html_x( '%1$s: %2$d / %3$d', 'placeholders: Points label, points input, maximum points', 'ebox' ),
				...
				if ( 'not_graded' === $essay->post_status ) {
					...
					<button id="essay_approve_<?php echo absint( $post_id ); ?>" class="small essay_approve_single"><?php esc_html_e( 'approve', 'ebox' ); ?></button>
					</div>
					<?php
				}
			}
		...
		protected function show_column_essay_course( $post_id = 0 ) {
			if ( ! empty( $post_id ) ) {
				...
				if ( ! empty( $course_id ) ) {
					...
					$row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';

					if ( current_user_can( 'edit_post', $course_id ) ) {
						$row_actions['ld-post-edit'] = '<a href="' . esc_url( get_edit_post_link( $course_id ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
					}
					if ( is_post_type_viewable( get_post_type( $course_id ) ) ) {
						$row_actions['ld-post-view'] = '<a href="' . esc_url( get_permalink( $course_id ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
					}
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( ! empty( $lesson_id ) ) {
					...
					if ( current_user_can( 'edit_post', $lesson_id ) ) {
						...
						$row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
					}
					...
					if ( is_post_type_viewable( get_post_type( $lesson_id ) ) ) {
						...
						$row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
					}
					echo wp_kses_post( $this->list_table_row_actions( $row_actions ) );
				}
			}
		...
		protected function show_column_essay_quiz( $post_id = 0 ) {
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( empty( $quiz_post_id ) ) {
					$user_quiz = ebox_get_user_quiz_entry_for_essay( $post_id );
				...
				if ( ! empty( $quiz_post_id ) ) {
					...
					if ( ( $quiz_post ) && ( is_a( $quiz_post, 'WP_Post' ) ) ) {
						...
						$row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $quiz_post_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';
						...
						if ( current_user_can( 'edit_post', $quiz_post_id ) ) {
							...
							$row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $quiz_post_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
						}

						if ( is_post_type_viewable( get_post_type( $quiz_post_id ) ) ) {
							if ( ! empty( $course_id ) ) {
								$view_url = ebox_get_step_permalink( $quiz_post_id, $course_id );
							} else {
								$view_url = get_permalink( $quiz_post_id );
							}

							$row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $quiz_post_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
						}
						echo wp_kses_post( $this->list_table_row_actions( $row_actions ) );
					}
				}
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( empty( $question_post_id ) ) {
					...
					if ( ! empty( $question_pro_id ) ) {
						$question_post_id = ebox_get_question_post_by_pro_id( $question_pro_id );
						$question_post_id = absint( $question_post_id );
						if ( ! empty( $question_post_id ) ) {
				...
				if ( ! empty( $question_post_id ) ) {
					...
					if ( ( $question_post ) && ( is_a( $question_post, 'WP_Post' ) ) ) {
						$question_title = ebox_format_step_post_title_with_status_label( $question_post );
						...
						$row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $question_post_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';
						...
						if ( current_user_can( 'edit_post', $question_post_id ) ) {
							...
							$row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $question_post_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
						}

						if ( is_post_type_viewable( get_post_type( $question_post_id ) ) ) {
							if ( ! empty( $quiz_id ) ) {
								$view_url = ebox_get_step_permalink( $question_post_id, $quiz_id );

								$row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $question_post_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
							}
						}
						echo wp_kses_post( $this->list_table_row_actions( $row_actions ) );
					}
		...
		protected function selector_filter_for_essay_quiz( $q_vars = array(), $selector = array() ) {
			...
			if ( ( ebox_is_team_leader_user( get_current_user_id() ) ) && ( 'advanced' !== ebox_get_team_leader_manage_courses() ) ) {
				if ( empty( $course_id ) ) {
					...
					$quiz_items = $ebox_lms->select_a_quiz( $course_id, $lesson_id );
					if ( ! empty( $quiz_items ) ) {
		...
		protected function selector_filter_for_essay_question( $q_vars = array(), $selector = array() ) {
			...
			if ( empty( $quiz_id ) ) {
				...
				$questions_ids = ebox_get_quiz_questions( $quiz_id );
		...
		protected function show_essay_question_selector( $selector = array() ) {
			...
			$quiz_id          = (int) $this->get_selector( 'quiz_id', 'selected' );
			if ( ! empty( $quiz_id ) ) {
				$quiz_questions = ebox_get_quiz_questions( $quiz_id );
		 ...
		 *
		 * Fires on `views_edit-ebox-essays` and `views_edit-ebox-assignment` hook.
		 *
		 * @since 3.2.3
		 *
		...
		public function add_view_modal() {
			?>
			<div id='ebox-admin-table-modal' style="display: none"></div>
			<?php
		}
		...
		public static function load_modal_content(): void {
			...
			if (
				...
				ebox_get_post_type_slug( 'essay' ) !== $essay->post_type
			) {
				return;
			}
	...
	add_action( 'wp_ajax_' . ebox_Admin_Essays_Listing::VIEW_AJAX_ACTION, array( ebox_Admin_Essays_Listing::class, 'load_modal_content' ) );
}

new ebox_Admin_Essays_Listing();
