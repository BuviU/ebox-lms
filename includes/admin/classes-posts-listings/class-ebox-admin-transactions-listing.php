<?php
/**
 * ebox Transactions (ebox-transactions) Posts Listing.
 *
 * @since 3.2.0
 * @package ebox\Transactions\Listing
 */
...
if ( class_exists( 'ebox_Admin_Posts_Listing' ) && ! class_exists( 'ebox_Admin_Transactions_Listing' ) ) {
	/**
	 * Class ebox Transactions (ebox-transactions) Posts Listing.
	 *
	 * @since 3.2.0
	 */
	class ebox_Admin_Transactions_Listing extends ebox_Admin_Posts_Listing {
		...
		public function listing_init() {
			...
			$this->selectors = array(
				'payment_processors' => array(
					'type'                   => 'early',
					'show_all_value'         => '',
					'show_all_label'         => esc_html__( 'Show All Payment Processors', 'ebox' ),
					'options'                => ebox_Payment_Gateway::get_select_list(),
				...
				'transaction_type'   => array(
					'type'                   => 'early',
					'show_all_value'         => '',
					'show_all_label'         => esc_html__( 'Show All Transactions Types', 'ebox' ),
					'options'                => array(
						'return-success'                => esc_html__( 'PayPal Purchase Pending', 'ebox' ),
						'web_accept'                    => esc_html__( 'PayPal Purchase Complete', 'ebox' ),
						'subscr_cancel'                 => esc_html__( 'PayPal Subscription Canceled', 'ebox' ),
						'subscr_eot'                    => esc_html__( 'PayPal Subscription Expired', 'ebox' ),
						'subscr_failed'                 => esc_html__( 'PayPal Subscription Payment Failed', 'ebox' ),
						'subscr_payment'                => esc_html__( 'PayPal Subscription Payment Success', 'ebox' ),
						'subscr_signup'                 => esc_html__( 'PayPal Subscription Signup', 'ebox' ),
						'stripe_paynow'                 => esc_html__( 'Stripe Purchase', 'ebox' ),
						'stripe_subscribe'              => esc_html__( 'Stripe Subscription', 'ebox' ),
						'razorpay_paynow'               => esc_html__( 'Razorpay Purchase', 'ebox' ),
						'razorpay_subscribe'            => esc_html__( 'Razorpay Subscription (no trial)', 'ebox' ),
						'razorpay_subscribe_paid_trial' => esc_html__( 'Razorpay Subscription (paid trial)', 'ebox' ),
						'razorpay_subscribe_free_trial' => esc_html__( 'Razorpay Subscription (free trial)', 'ebox' ),
					),
			...
			$this->columns = array(
				...
				'access'  => array(
					'label'   => esc_html__( 'Access', 'ebox' ),
					'display' => array( $this, 'show_column_access' ),
				),
				'gateway' => array(
					'label'   => esc_html__( 'Gateway', 'ebox' ),
					'display' => array( $this, 'show_column_gateway' ),
				),
				'info'    => array(
					'label'   => esc_html__( 'Info', 'ebox' ),
		...
		protected function filter_by_payment_gateway( array $q_vars, array $selector = array() ): array {
			...
			if ( ebox_Paypal_IPN_Gateway::get_name() === $selector['selected'] ) {
				$q_vars['meta_query']['relation'] = 'OR';
				$q_vars['meta_query'][]           = array(
					'key'     => ebox_Transaction_Model::$meta_key_gateway_name,
					'compare' => '=',
			...
			} elseif ( ebox_Stripe_Gateway::get_name() === $selector['selected'] ) {
				$q_vars['meta_query']['relation'] = 'OR';
				$q_vars['meta_query'][]           = array(
					'key'     => ebox_Transaction_Model::$meta_key_gateway_name,
					'compare' => '=',
			...
			} elseif ( ebox_Razorpay_Gateway::get_name() === $selector['selected'] ) {
				$q_vars['meta_query'][] = array(
					'key'     => ebox_Transaction_Model::$meta_key_gateway_name,
					'compare' => '=',
					'value'   => $selector['selected'],
		...
		protected function filter_by_transaction_type( array $q_vars, array $selector = array() ): array {
			...
			if ( 'web_accept' === $selector['selected'] ) {
				...
				$q_vars['meta_query'][] = array(
					...
					'value' => ebox_PRICE_TYPE_PAYNOW,
				);
			} elseif ( 'stripe_subscribe' === $selector['selected'] ) {
				$q_vars['meta_query'][] = array(
					'key'   => 'stripe_price_type',
					'value' => ebox_PRICE_TYPE_SUBSCRIBE,
				);
			} elseif ( 'razorpay_paynow' === $selector['selected'] ) {
				$q_vars['meta_query'][] = array(
					array(
						'key'   => ebox_Transaction_Model::$meta_key_gateway_name,
						'value' => ebox_Razorpay_Gateway::get_name(),
					),
					array(
						'key'   => 'price_type',
						'value' => ebox_PRICE_TYPE_PAYNOW,
				...
				$q_vars['meta_query'][] = array(
					...
					array(
						'key'   => ebox_Transaction_Model::$meta_key_price_type,
						'value' => ebox_PRICE_TYPE_SUBSCRIBE,
					),
					array(
						'key'     => ebox_Transaction_Model::$meta_key_has_trial,
						'compare' => '!=',
						'value'   => 1,
					),
				);
				...
				$q_vars['meta_query'][] = array(
					...
					array(
						'key'   => ebox_Transaction_Model::$meta_key_has_free_trial,
						'value' => 0,
					),
				);
		...
		protected function filter_by_product( array $q_vars, array $selector = array() ): array {
			...
			$q_vars['meta_query']['relation'] = 'OR';
			foreach ( ebox_Transaction_Model::$product_id_meta_keys as $meta_key ) {
				$q_vars['meta_query'][] = array(
					'key'   => $meta_key,
		...
		protected function show_column_gateway( int $post_id ): void {
			$transaction = ebox_Transaction_Model::find( $post_id );

			if ( ! $transaction ) {
				return;
			}

			$gateway_select = ebox_Payment_Gateway::get_select_list();
		...
		protected function show_column_info( int $post_id ): void {
			...
			if ( $transaction->is_free() ) {
				esc_html_e( 'Enrolled by a coupon with no payment', 'ebox' );
				return;
			}
			...
			echo sprintf(
				...
				esc_html_x( 'Price: %s', 'placeholder: Transaction price', 'ebox' ),
				esc_html(
					ebox_get_price_formatted( $pricing->price, $pricing->currency )
				)
			);
			...
			if ( $pricing->discount > 0 ) {
				...
				echo sprintf(
					...
					esc_html_x( 'Discount: %s', 'placeholder: Transaction discount', 'ebox' ),
					esc_html(
						ebox_get_price_formatted( $pricing->discount * -1, $pricing->currency )
					)
				);
				echo '<br/>';
				...
				echo sprintf(
					...
					esc_html_x( 'Final price: %s', 'placeholder: Transaction final price', 'ebox' ),
					esc_html(
						ebox_get_price_formatted( $pricing->discounted_price, $pricing->currency )
					)
				);
			}
			...
			if ( $transaction->is_subscription() ) {
				...
				echo sprintf(
					...
					esc_html_x( 'Recurring times: %s', 'placeholder: Transaction recurring times', 'ebox' ),
					esc_html(
				...
				echo sprintf(
					...
					esc_html_x( 'Billing cycle: every %1$d %2$s', 'placeholder: Transaction billing cycle value, Transaction billing cycle length', 'ebox' ),
					esc_attr( (string) $pricing->duration_value ),
					esc_html(
						ebox_get_grammatical_number_label_for_interval( $pricing->duration_value, $pricing->duration_length )
					)
				);
				...
				if ( $pricing->trial_duration_value > 0 ) {
					...
					echo sprintf(
						...
						esc_html_x( 'Trial price: %s', 'placeholder: Transaction trial price', 'ebox' ),
						esc_html(
							ebox_get_price_formatted( $pricing->trial_price, $pricing->currency )
						)
					);
					echo '<br/>';
					...
					echo sprintf(
						...
						esc_html_x( 'Trial duration: %1$d %2$s', 'placeholder: Transaction trial duration value, Transaction trial duration length', 'ebox' ),
						esc_attr( (string) $pricing->trial_duration_value ),
						esc_html(
							ebox_get_grammatical_number_label_for_interval( $pricing->trial_duration_value, $pricing->trial_duration_length )
						)
					);
				}
			}
		...
		protected function show_column_coupon( int $post_id ): void {
			...
			if ( ! empty( $coupon_data->code ) ) {
				...
				$formatted_amount = $coupon_data->type === ebox_COUPON_TYPE_FLAT
					? ebox_get_price_formatted( $coupon_data->amount, $pricing->currency )
					: $coupon_data->amount . '%';

				echo sprintf(
					// Translators: placeholder: Coupon code.
					esc_html_x( 'Code: %s', 'placeholder: Coupon code', 'ebox' ),
				...
				echo sprintf(
					...
					esc_html_x( 'Type: %s', 'placeholder: Coupon type', 'ebox' ),
				...
				echo sprintf(
					...
					esc_html_x( 'Amount: %s', 'placeholder: Coupon amount', 'ebox' ),
				...
				echo esc_html__( 'Unknown', 'ebox' );
			} else {
				echo esc_html__( 'No', 'ebox' );
			}
		}

		/**
		 * Output a course/team.
		 *
		 * @since 3.2.3
		 *
		 * @param int $post_id Transaction Post ID.
		...
		protected function show_column_product( int $post_id ): void {
			...
			if ( $transaction->is_parent() ) {
				...
				echo esc_html(
					$children_count . ' ' . _n( 'product', 'products', $children_count, 'ebox' )
				);
			...
			// Show HTML.
			echo sprintf(
				// translators: placeholder: Post type label (Course/Team), Link to Course/Team.
				esc_html_x( '%1$s: %2$s', 'placeholder: Post type label (Course/Team), Link to Course/Team', 'ebox' ),
		...
		public function transactions_bulk_actions(): void {
			...
			$remove_access_action_label = esc_html__( 'Remove access', 'ebox' );
			$add_access_action_label    = esc_html__( 'Add access', 'ebox' );
			?>
		...
		protected function transactions_bulk_actions_update_access(): void {
			...
			$transactions = ebox_Transaction_Model::find_many(
			...
			foreach ( $transactions as $transaction ) {
				...
				if ( self::ACTION_REMOVE_ACCESS === $action && $user_has_access ) {
					...
					if ( ebox_Paypal_IPN_Gateway::get_name() === $transaction->get_gateway_name() ) {
						 ...
						 *
						 * @param string[] $removal_statuses Array of PayPal IPN subscription statuses.
						 * @param WP_Post  $product          Course or Team.
						 * @param WP_User  $user             User.
						 *
						 * @return string[] Array of PayPal IPN subscription statuses.
						 */
...
new ebox_Admin_Transactions_Listing();
