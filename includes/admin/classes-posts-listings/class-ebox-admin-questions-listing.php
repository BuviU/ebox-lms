<?php
/**
 * ebox Quiz Questions (ebox-question) Posts Listing.
 *
 * @since 2.6.0
 * @package ebox\Question\Listing
 */
...
if ( ( class_exists( 'ebox_Admin_Posts_Listing' ) ) && ( ! class_exists( 'ebox_Admin_Questions_Listing' ) ) ) {

	/**
	 * Class ebox Quiz Questions (ebox-question) Posts Listing.
	 *
	 * @since 2.6.0
	 * @uses ebox_Admin_Posts_Listing
	 */
	class ebox_Admin_Questions_Listing extends ebox_Admin_Posts_Listing {
		...
		public function listing_init() {
			...
			$this->selectors = array(
				'question_type'         => array(
					...
					'show_all_label'         => sprintf(
						...
						esc_html_x( 'All %s Types', 'placeholder: Question', 'ebox' ),
				...
				'question_pro_category' => array(
					...
					'show_all_label'         => sprintf(
						// translators: placeholder: Question.
						esc_html_x( 'All %s Categories', 'placeholder: Question', 'ebox' ),
						ebox_Custom_Label::get_label( 'question' )
					),
			...
			$this->columns = array(
				...
				'quiz'            => array(
					'label'    => sprintf(
						// translators: placeholder: Quiz.
						esc_html_x( 'Assigned %s', 'placeholder: Quiz', 'ebox' ),
						ebox_Custom_Label::get_label( 'quiz' )
					),
			...
			if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Questions_Taxonomies', 'proquiz_question_category' ) == 'yes' ) {
			...
			if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'shared_questions' ) ) {
			...
			if ( $this->post_type_check() ) {
				...
				if ( isset( $_GET['quiz_id'] ) ) {
					...
					if ( ! empty( $quiz_id ) ) {
						...
						$new_title     = '<a href="' . esc_url( $quizzes_url ) . '">' . ebox_Custom_Label::get_label( 'quizzes' ) . '</a> &gt; <a href="' . get_edit_post_link( $quiz_id ) . '">' . get_the_title( $quiz_id ) . '</a> - ' . esc_html( $post_type_object->labels->name );
		...
		protected function show_column_question_type( $post_id = 0, $selector = array() ) {
			...
			if ( ( ! empty( $question_type_slug ) ) && ( isset( $ebox_question_types[ $question_type_slug ] ) ) ) {
				$question_type_label = $ebox_question_types[ $question_type_slug ];
				...
				$link_aria_label = sprintf(
					...
					esc_html_x( 'Filter listing by %s type', 'placeholder: Question Type', 'ebox' ),
					ebox_Custom_Label::get_label( 'question' )
				);
			...
			if ( ! empty( $post_id ) ) {
				if ( ! isset( $field_values[ $post_id ] ) ) {
					...
					if ( ! empty( $question_pro_id ) ) {
						$field_values[ $post_id ] = ebox_get_question_pro_fields( $question_pro_id, array( 'points', 'answer_type', 'category_id', 'category_name' ) );
					} else {
...
new ebox_Admin_Questions_Listing();
