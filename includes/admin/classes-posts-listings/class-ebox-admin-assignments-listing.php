<?php
/**
 * ebox Assignments (ebox-assignment) Posts Listing.
 *
 * @since 3.2.3
 * @package ebox\Assignment\Listing
 */
...
if ( ( class_exists( 'ebox_Admin_Posts_Listing' ) ) && ( ! class_exists( 'ebox_Admin_Assignments_Listing' ) ) ) {

	/**
	 * Class ebox Assignments (ebox-assignment) Posts Listing.
	 *
	 * @since 3.2.3
	 * @uses ebox_Admin_Posts_Listing
	 */
	class ebox_Admin_Assignments_Listing extends ebox_Admin_Posts_Listing {
		...
		public function listing_init() {
			...
			$this->selectors = array(
				...
				'approval_status' => array(
					'type'                   => 'early',
					'show_all_value'         => '',
					'show_all_label'         => esc_html__( 'Approval Status', 'ebox' ),
					'options'                => array(
						'approved'     => esc_html__( 'Approved', 'ebox' ),
						'not_approved' => esc_html__( 'Not Approved', 'ebox' ),
					),
			...
			add_filter(
				...
				array( $this, 'ebox_listing_selector_user_query_args_assignments' ),
				30,
				2
			);
		}
		...
		public function on_load_listing() {
			if ( $this->post_type_check() ) {
				...
				add_filter( 'ebox_listing_table_query_vars_filter', array( $this, 'listing_table_query_vars_filter_assignments' ), 30, 3 );
		 ...
		 * @param string $post_type Post Type being displayed.
		 */
		public function ebox_listing_selector_user_query_args_assignments( $q_vars, $post_type ) {
			if ( $post_type === $this->post_type ) {
				$team_selector = $this->get_selector( 'team_id' );
			...
			if ( ( isset( $selector['selected'] ) ) && ( ! empty( $selector['selected'] ) ) ) {
				if ( ( isset( $selector['show_empty_value'] ) ) && ( $selector['show_empty_value'] === $selector['selected'] ) ) {
					...
					if ( ( $course_selector ) && ( isset( $course_selector['selected'] ) ) && ( ! empty( $course_selector['selected'] ) ) ) {
						$topics = ebox_get_topic_list( $selector['selected'], $course_selector['selected'] );
						if ( ! empty( $topics ) ) {
		...
		protected function show_column_assignment_approval_status( $post_id = 0, $column_meta = array() ) {
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( ! empty( $lesson_id ) ) {
					$approval_status_flag = ebox_is_assignment_approved_by_meta( $post_id );
					if ( 1 == $approval_status_flag ) {
						$approval_status_slug  = 'approved';
						$approval_status_label = _x( 'Approved', 'Assignment approval status', 'ebox' );
					} else {
						$approval_status_slug  = 'not_approved';
						$approval_status_label = _x( 'Not Approved', 'Assignment approval status', 'ebox' );
					}

					echo '<div class="ld-approval-status">' . sprintf(
						// translators: placeholder: Status label, Status value.
						esc_html_x( '%1$s: %2$s', 'placeholder: Status label, Status value', 'ebox' ),
						'<span class="ebox-listing-row-field-label">' . esc_html__( 'Status', 'ebox' ) . '</span>',
						esc_html( $approval_status_label )
					) . '</div>';

					echo '<div class="ld-approval-points">';
					if ( ebox_assignment_is_points_enabled( $post_id ) ) {
						$max_points = 0;
						$max_points = ebox_get_setting( $lesson_id, 'lesson_assignment_points_amount' );

						$current_points = get_post_meta( $post_id, 'points', true );
						if ( 1 != $approval_status_flag ) {
							$points_label = '<label class="ebox-listing-row-field-label" for="assignment_points_' . absint( $post_id ) . '">' . esc_html__( 'Points', 'ebox' ) . '</label>';
							$points_input = '<input id="assignment_points_' . absint( $post_id ) . '" class="small-text ebox-award-points" type="number" value="' . absint( $current_points ) . '" max="' . absint( $max_points ) . '" min="0" step="1" name="assignment_points[' . absint( $post_id ) . ']" />';
							echo sprintf(
						...
						echo sprintf(
							...
							esc_html_x( '%s: Not Enabled', 'placeholder: Points label', 'ebox' ),
							'<span class="ebox-listing-row-field-label">' . esc_html__( 'Points', 'ebox' ) . '</span>'
						);
					}
					echo '</div>';
					...
					if ( 1 != $approval_status_flag ) {
						...
						<div class="ld-approval-action">
							<button id="assignment_approve_<?php echo absint( $post_id ); ?>" class="small assignment_approve_single"><?php esc_html_e( 'approve', 'ebox' ); ?></button>
						</div>
						<?php
					}
				}
		...
		public function assignment_bulk_actions() {
			...
			if ( ( ! empty( $post->post_type ) ) && ( ebox_get_post_type_slug( 'assignment' ) === $post->post_type ) ) {
		...
		protected function assignment_bulk_actions_approve() {
			...
			if ( ( ! isset( $_REQUEST['post_type'] ) ) || ( ebox_get_post_type_slug( 'assignment' ) !== $_REQUEST['post_type'] ) ) {
				return;
			}
			...
			if ( 'approve_assignment' === $action ) {
				if ( ( isset( $_REQUEST['post'] ) ) && ( ! empty( $_REQUEST['post'] ) ) ) { // @phpstan-ignore-line
					...
					foreach ( $assignments as $assignment_id ) {
						...
						if ( ( ! empty( $assignment_post ) ) && ( is_a( $assignment_post, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'assignment' ) === $assignment_post->post_type ) ) {

							$user_id   = absint( $assignment_post->post_author );
							$lesson_id = get_post_meta( $assignment_post->ID, 'lesson_id', true );

							if ( ebox_assignment_is_points_enabled( $assignment_id ) === true ) {
							...
							ebox_approve_assignment( $user_id, $lesson_id, $assignment_id );
						}
					}
				}
			}
		...
		public function post_row_actions( $row_actions = array(), $post = null ): array {
			...
			if ( $file_is_image ) {
				...
				$row_actions['quick_view'] = sprintf(
					'<a class="view-ebox-assignment" href="%s" data-title="%s" aria-label="%s">%s</a>',
					esc_url( $file_url ),
...
new ebox_Admin_Assignments_Listing();
