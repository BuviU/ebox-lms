<?php
/**
 * ebox Exams (ld-exam) Posts Listing Class.
 *
 * @package ebox\Admin
 */
...
if ( ( class_exists( 'ebox_Admin_Posts_Listing' ) ) && ( ! class_exists( 'ebox_Admin_Exams_Listing' ) ) ) {
	/**
	 * Class for ebox Exams Listing Pages.
	 */
	class ebox_Admin_Exams_Listing extends ebox_Admin_Posts_Listing {
		...
		public function listing_init() {
			...
			$this->selectors = array(
				'exam_challenge_course_show'   => array(
					...
					'show_all_label'         => sprintf(
						...
						esc_html_x( 'All %s Show', 'placeholder: Courses', 'ebox' ),
				...
				'exam_challenge_course_passed' => array(
					...
					'show_all_label'         => sprintf(
						// translators: placeholder: Courses.
						esc_html_x( 'All %s Pass', 'placeholder: Courses', 'ebox' ),
						ebox_Custom_Label::get_label( 'courses' )
					),
					'show_empty_value'       => 'empty',
					'show_empty_label'       => sprintf(
			...
			$this->columns = array(
				'exam_challenge_course_show'   => array(
					'label'    => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Show', 'placeholder: Course', 'ebox' ),
						ebox_Custom_Label::get_label( 'course' )
					),
					'after'    => 'title',
				...
				'exam_challenge_course_passed' => array(
					'label'    => sprintf(
						// translators: placeholder: Course.
						esc_html_x( '%s Passed', 'placeholder: Course', 'ebox' ),
						ebox_Custom_Label::get_label( 'course' )
					),
			...
			add_action( 'admin_notices', array( $this, 'ebox_exam_legacy_theme_warning' ) );
		...
		protected function show_column_step_course_show( $post_id = 0, $column_meta = array() ) {
			if ( ! empty( $post_id ) ) {
				$course_id = (int) ebox_get_setting( $post_id, 'exam_challenge_course_show' );
				if ( ! empty( $course_id ) ) {
		...
		protected function show_column_step_course_passed( $post_id = 0, $column_meta = array() ) {
			if ( ! empty( $post_id ) ) {
				$course_id = (int) ebox_get_setting( $post_id, 'exam_challenge_course_passed' );
				if ( ! empty( $course_id ) ) {
					$course_post = get_post( $course_id );
					if ( ( $course_post ) && ( is_a( $course_post, 'WP_Post' ) ) ) {
						$course_title = ebox_format_step_post_title_with_status_label( $course_post );
		 ...
		 * @since 4.0.0
		 */
		public function ebox_exam_legacy_theme_warning() {
			if ( ebox_is_active_theme( 'legacy' ) ) {
				$message = ebox_exam_legacy_theme_warning_message();
				if ( ! empty( $message ) ) {
