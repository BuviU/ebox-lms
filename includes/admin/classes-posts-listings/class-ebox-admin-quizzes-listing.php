<?php
/**
 * ebox Quizzes (ebox-quiz) Posts Listing.
 *
 * @since 3.0.0
 * @package ebox\Quiz\Listing
 */
...
if ( ( class_exists( 'ebox_Admin_Posts_Listing' ) ) && ( ! class_exists( 'ebox_Admin_Quizzes_Listing' ) ) ) {

	/**
	 * Class ebox Quizzes (ebox-quiz) Posts Listing.
	 *
	 * @since 3.0.0
	 * @uses ebox_Admin_Posts_Listing
	 */
	class ebox_Admin_Quizzes_Listing extends ebox_Admin_Posts_Listing {
		...
		public function listing_init() {
			...
			$this->columns = array(
				'shortcode'    => array(
					'label'   => esc_html__( 'Shortcode', 'ebox' ),
					'after'   => 'title',
		...
		public function on_load_listing() {
			if ( $this->post_type_check() ) {
				...
				add_filter( 'ebox_listing_table_query_vars_filter', array( $this, 'listing_table_query_vars_filter_quizzes' ), 30, 3 );
				 ...
				 * @since 3.4.1
				 */
				$ld_data_upgrade_quiz_post_meta = ebox_Admin_Data_Upgrades::get_instance( 'ebox_Admin_Data_Upgrades_Quiz_Post_Meta' );
				if ( ( $ld_data_upgrade_quiz_post_meta ) && ( is_a( $ld_data_upgrade_quiz_post_meta, 'ebox_Admin_Data_Upgrades_Quiz_Post_Meta' ) ) ) {
		...
		protected function show_column_shortcode( $post_id = 0, $selector = array() ) {
			if ( ! empty( $post_id ) ) {
				$valid_quiz  = false;
				$quiz_pro_id = ebox_get_setting( $post_id, 'quiz_pro' );
		...
		public function post_row_actions( $row_actions = array(), $post = null ) {
			if ( $this->post_type_check() ) {
				...
				if ( ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'enabled' ) ) && ( current_user_can( 'edit_post', $post->ID ) ) && ( ! isset( $row_actions['ld-quiz-builder'] ) ) ) {
					/**
					 ...
					 */
					if ( apply_filters( 'ebox_show_quiz_builder_row_actions', true, $post ) === true ) {
						$label = sprintf(
							// translators: placeholder: Quiz.
							esc_html_x( 'Use %s Builder', 'placeholder: Quiz', 'ebox' ),
							ebox_Custom_Label::get_label( 'quiz' )
						);

						$link = add_query_arg(
							array(
								'currentTab' => 'ebox_quiz_builder',
							),
				...
				$pro_quiz_id = ebox_get_setting( $post, 'quiz_pro' );
				if ( ! empty( $pro_quiz_id ) ) {
					if ( ( ! isset( $row_actions['questions'] ) ) || ( empty( $row_actions['questions'] ) ) ) {
						if ( ( true === ebox_is_data_upgrade_quiz_questions_updated() ) && ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'enabled' ) ) ) {
							$link = add_query_arg(
								array(
						...
						$label = sprintf(
							...
							esc_html_x( 'Show %1$s %2$s', 'placeholder: Quiz, Questions', 'ebox' ),
					...
					if ( ( current_user_can( 'wpProQuiz_show_statistics' ) ) && ( ( ! isset( $row_actions['statistics'] ) ) || ( empty( $row_actions['statistics'] ) ) ) ) {
						if ( ebox_get_setting( $post->ID, 'statisticsOn' ) ) {
							...
							$label = sprintf(
								// translators: placeholder: Quiz.
								esc_html_x( 'Show %s Statistics', 'placeholder: Quiz', 'ebox' ),
								ebox_Custom_Label::get_label( 'quiz' )
							);
					...
					if ( ( current_user_can( 'wpProQuiz_toplist_edit' ) ) && ( ( ! isset( $row_actions['leaderboard'] ) ) || ( empty( $row_actions['leaderboard'] ) ) ) ) {
						if ( ebox_get_setting( $post->ID, 'toplistActivated' ) ) {
							...
							$label = sprintf(
								// translators: placeholder: Quiz.
								esc_html_x( 'Show %s Leaderboard', 'placeholder: Quiz', 'ebox' ),
								ebox_Custom_Label::get_label( 'quiz' )
							);
...
new ebox_Admin_Quizzes_Listing();
