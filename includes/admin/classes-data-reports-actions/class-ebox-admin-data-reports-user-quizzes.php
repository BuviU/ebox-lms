...
if ( ( ! class_exists( 'ebox_Admin_Data_Reports_Quizzes' ) ) && ( class_exists( 'ebox_Admin_Settings_Data_Reports' ) ) ) {

	/**
	 * Class ebox Quiz Reports.
	 *
	 * @since 2.3.0
	 * @uses ebox_Admin_Settings_Data_Reports
	 */
	class ebox_Admin_Data_Reports_Quizzes extends ebox_Admin_Settings_Data_Reports {
				...
				<td class="ebox-data-reports-button-container" style="width: 20%">
					...
					<?php
						printf(
							...
							esc_html_x( 'Export User %s Data', 'Export User Quiz Data Label', 'ebox' ),
					...
					if ( ( isset( $data['init'] ) ) && ( 1 == $data['init'] ) ) {
						...
						if ( ( isset( $data['filters'] ) ) && ( ! empty( $data['filters'] ) ) ) {
							...
							if ( ( isset( $data['team_id'] ) ) && ( ! empty( $data['team_id'] ) ) ) {
								...
								$this->transient_data['course_ids'] = ebox_team_enrolled_courses( intval( intval( $data['team_id'] ) ) );
					...
					if ( ! empty( $this->transient_data['users_ids'] ) ) {
						...
						if ( true !== $_doing_init ) {
							...
							if ( ! empty( $course_progress_data ) ) {
								...
								$this->csv_parse = apply_filters( 'ebox_csv_object', $this->csv_parse, 'quizzes' );
						...
						$data['progress_label']   = sprintf(
							...
							esc_html_x( '%1$d of %2$s Users', 'placeholders: result count, total count', 'ebox' ),
							$data['result_count'],
							$data['total_count']
						);
					}
		...
		public function set_report_headers() {
			...
			$this->data_headers['quiz_id']    = array(
				'label'   => esc_html__( 'quiz_id', 'ebox' ),
				'default' => '',
				'display' => array( $this, 'report_column' ),
			);
			$this->data_headers['quiz_title'] = array(
				'label'   => esc_html__( 'quiz_title', 'ebox' ),
				'default' => '',
				'display' => array( $this, 'report_column' ),
			);

			$this->data_headers['quiz_score'] = array(
				'label'   => esc_html__( 'score', 'ebox' ),
				'default' => '0',
				'display' => array( $this, 'report_column' ),
			);
			$this->data_headers['quiz_total'] = array(
				'label'   => esc_html__( 'total', 'ebox' ),
				'default' => '0',
				'display' => array( $this, 'report_column' ),
			);
			...
			$this->data_headers['quiz_points_total'] = array(
				'label'   => esc_html__( 'points_total', 'ebox' ),
				'default' => '0',
				'display' => array( $this, 'report_column' ),
			);

			$this->data_headers['quiz_percentage'] = array(
				'label'   => esc_html__( 'percentage', 'ebox' ),
				'default' => '0',
				'display' => array( $this, 'report_column' ),
			);

			$this->data_headers['quiz_time_spent'] = array(
				'label'   => esc_html__( 'time_spent', 'ebox' ),
				'default' => '0',
				'display' => array( $this, 'report_column' ),
			);

			$this->data_headers['quiz_passed'] = array(
				'label'   => esc_html__( 'passed', 'ebox' ),
				'default' => esc_html_x( 'NO', 'Quiz Passed Report label: NO', 'ebox' ),
				'display' => array( $this, 'report_column' ),
			);
		...
		public function report_column( $column_value, $column_key, $report_item, $report_user ) {
			...
			switch ( $column_key ) {
				...
				case 'quiz_date':
					if ( ( property_exists( $report_item, 'activity_completed' ) ) && ( ! empty( $report_item->activity_completed ) ) ) {
						$column_value = ebox_adjust_date_time_display( $report_item->activity_completed, 'Y-m-d' );
					}
					break;
				...
				case 'quiz_passed':
					if ( ( property_exists( $report_item, 'activity_meta' ) ) && ( ! empty( $report_item->activity_meta ) ) ) {
						if ( ( isset( $report_item->activity_meta['pass'] ) ) && ( 1 == $report_item->activity_meta['pass'] ) ) {
							$column_value = esc_html_x( 'YES', 'Quiz Passed Report label: YES', 'ebox' );
						}
					}
					break;
			...
			/** This filter is documented in includes/admin/classes-data-reports-actions/class-ebox-admin-data-reports-user-courses.php */
			return apply_filters( 'ebox_report_column_item', $column_value, $column_key, $report_item, $report_user );
		}
