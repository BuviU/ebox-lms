...
if ( ( ! class_exists( 'ebox_Admin_Data_Reports_Courses' ) ) && ( class_exists( 'ebox_Admin_Settings_Data_Reports' ) ) ) {

	/**
	 * Class ebox Course Reports.
	 *
	 * @since 2.3.0
	 * @uses ebox_Admin_Settings_Data_Reports
	 */
	class ebox_Admin_Data_Reports_Courses extends ebox_Admin_Settings_Data_Reports {
		...
		public function __construct() {
			...
			add_filter( 'ebox_admin_report_register_actions', array( $this, 'register_report_action' ) );
		}
		...
		public function show_report_action() {
			?>
			<tr id="ebox-data-reports-container-<?php echo esc_attr( $this->data_slug ); ?>" class="ebox-data-reports-container">
				<td class="ebox-data-reports-button-container" style="width: 20%">
					<button class="ebox-data-reports-button button button-primary" data-nonce="<?php echo esc_attr( wp_create_nonce( 'ebox-data-reports-' . $this->data_slug . '-' . get_current_user_id() ) ); ?>" data-slug="<?php echo esc_attr( $this->data_slug ); ?>">
					<?php
					printf(
					// translators: Export User Course Data Label.
						esc_html_x( 'Export User %s Data', 'Export User Course Data Label', 'ebox' ),
						ebox_Custom_Label::get_label( 'course' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
					);
					?>
					</button></td>
				<td class="ebox-data-reports-status-container" style="width: 80%">
					<div style="display:none;" class="meter ebox-data-reports-status">
		...
		public function process_report_action( $data = array() ) {
			...
			if ( ( isset( $data['nonce'] ) ) && ( ! empty( $data['nonce'] ) ) ) {
				if ( wp_verify_nonce( $data['nonce'], 'ebox-data-reports-' . $this->data_slug . '-' . get_current_user_id() ) ) {
					...
					if ( ( isset( $data['init'] ) ) && ( 1 == $data['init'] ) ) {
						...
						if ( ( isset( $data['filters'] ) ) && ( ! empty( $data['filters'] ) ) ) {
							...
							if ( ( isset( $data['team_id'] ) ) && ( ! empty( $data['team_id'] ) ) ) {
								$this->transient_data['users_ids'] = ebox_get_teams_user_ids( intval( $data['team_id'] ) );
								$this->transient_data['posts_ids'] = ebox_team_enrolled_courses( intval( $data['team_id'] ) );
							} else {
								$this->transient_data['posts_ids'] = '';
								$this->transient_data['users_ids'] = ebox_get_report_user_ids();
							}
					...
					if ( ! empty( $this->transient_data['users_ids'] ) ) {
						...
						if ( true !== (bool) $_doing_init ) {
							...
							foreach ( $this->transient_data['users_ids'] as $user_id_idx => $user_id ) {
								...
								if ( false !== $report_user ) {
									if ( ( isset( $this->transient_data['course_ids'] ) ) && ( ! empty( $this->transient_data['course_ids'] ) ) ) {
										...
										$post_ids = ebox_user_get_enrolled_courses( intval( $user_id ), $course_query_args, true );
									}
							...
							if ( ! empty( $course_progress_data ) ) {
								...
								$this->csv_parse = apply_filters( 'ebox_csv_object', $this->csv_parse, 'courses' );

								/** This filter is documented in includes/class-ld-lms.php */
								$this->csv_parse = apply_filters( 'ebox_csv_object', $this->csv_parse, $this->data_slug );
								 ...
								 */
								$course_progress_data = apply_filters( 'ebox_csv_data', $course_progress_data, $this->data_slug );
						...
						$data['progress_label'] = sprintf( esc_html_x( '%1$d of %2$s Users', 'placeholders: result count, total count', 'ebox' ), $data['result_count'], $data['total_count'] );
		...
		public function set_report_headers() {
			 ...
			 * @param string $data_slug    The slug of the data in the CSV.
			 */
			$this->data_headers = apply_filters( 'ebox_data_reports_headers', $this->data_headers, $this->data_slug );
		}
		...
		public function send_report_headers_to_csv() {
			if ( ! empty( $this->data_headers ) ) {
				...
				/** This filter is documented in includes/admin/classes-data-reports-actions/class-ebox-admin-data-reports-user-courses.php */
				$this->data_headers = apply_filters( 'ebox_csv_data', $this->data_headers, $this->data_slug );
		...
		public function set_report_filenames( $data ) {
			...
			$ld_file_part = '/ebox/reports/ebox_reports_' . str_replace( array( 'ld_data_reports_', '-' ), array( '', '_' ), $this->transient_key ) . '.csv';
			...
			if ( ! file_exists( dirname( $ld_wp_upload_filename ) ) ) {
				if ( wp_mkdir_p( dirname( $ld_wp_upload_filename ) ) === false ) {
					$data['error_message'] = esc_html__( 'ERROR: Cannot create working folder. Check that the parent folder is writable', 'ebox' ) . ' ' . $ld_wp_upload_filename;
					return $data;
				}
			}

			ebox_put_directory_index_file( trailingslashit( dirname( $ld_wp_upload_filename ) ) . 'index.php' );
			 ...
			 * @param string $data_slug       The slug of the data in the CSV.
			 */
			$this->transient_data['report_filename'] = apply_filters( 'ebox_report_filename', $ld_wp_upload_filename, $this->data_slug );
		...
		public function report_column( $column_value, $column_key, $report_item, $report_user ) {
			...
			switch ( $column_key ) {
				...
				case 'course_steps_total':
					...
					if ( ! empty( $course_id ) ) {
						if ( isset( $this->transient_data['course_step_totals'][ $course_id ] ) ) {
							...
							$column_value = ebox_get_course_steps_count( $course_id );
				...
				case 'course_steps_completed':
					...
					if ( ! empty( $course_id ) ) {
						...
						if ( true === $user_completed_course ) {
							...
							$column_value = ebox_course_get_completed_steps( $report_item->user_id, $course_id );
				...
				case 'course_completed':
					$column_value = esc_html_x( 'NO', 'Course Complete Report label: NO', 'ebox' );

					if ( ! empty( $course_id ) ) {
						$completed_on = get_user_meta( $report_item->user_id, 'course_completed_' . $course_id, true );
						if ( ! empty( $completed_on ) ) {
							$column_value = esc_html_x( 'YES', 'Course Complete Report label: YES', 'ebox' );
						} elseif ( property_exists( $report_item, 'activity_status' ) ) {
				...
				case 'course_completed_on':
					if ( ! empty( $course_id ) ) {
						$completed_on = get_user_meta( $report_item->user_id, 'course_completed_' . $course_id, true );
						if ( ! empty( $completed_on ) ) {
							return ebox_adjust_date_time_display( $completed_on, 'Y-m-d' );
						} elseif ( property_exists( $report_item, 'activity_status' ) ) {
							if ( true === (bool) $report_item->activity_status ) {
								if ( ( property_exists( $report_item, 'activity_completed' ) ) && ( ! empty( $report_item->activity_completed ) ) ) {
									return ebox_adjust_date_time_display( $report_item->activity_completed, 'Y-m-d' );
								}
							}
						}
					}
			 ...
			 * @param string     $data_slug    The slug of the data in the CSV.
			 */
			return apply_filters( 'ebox_report_column_item', $column_value, $column_key, $report_item, $report_user, $this->data_slug );
		}
