...
if ( ( class_exists( 'WP_Plugin_Install_List_Table' ) ) && ( ! class_exists( 'ebox_Admin_Addons_List_Table' ) ) ) {
	 ...
	 */
	class ebox_Admin_Addons_List_Table extends WP_Plugin_Install_List_Table {
		...
		public function __construct() {
			...
			$this->tabs = array(
				'ebox'   => array(
					'label' => esc_html__( 'ebox', 'ebox' ),
					'url'   => add_query_arg( 'tab', 'ebox' ),
				),
				'third-party' => array(
					'label' => esc_html__( 'Third Party', 'ebox' ),
		...
		public function prepare_items() {
			if ( 'ebox' === $this->current_tab ) {
				$this->prepare_items_ebox();
				 ...
				 */
				$this->items = apply_filters( 'ebox_addon_tab_items_' . $this->current_tab, array() );
			}
		}
		 ...
		 * @since 2.5.5
		 */
		public function prepare_items_ebox() {
			$this->addon_updater = ebox_Addon_Updater::get_instance();
		...
		public function prepare_items_third_party() {
			...
			$args['tag'] = sanitize_title_with_dashes( 'ebox' );
			...
			if ( ! empty( $this->items ) ) {
				foreach ( $this->items as $idx => $item ) {
					if ( 'wplms-ebox-migration' === $item['slug'] ) {
			...
			if ( isset( $api->info['teams'] ) ) {
				$this->teams = $api->info['teams'];
			}
		...
		public function display_rows() {
			if ( 'ebox' == $this->current_tab ) {
				 ...
				 * @since 2.5.5
				 */
				do_action( 'ebox_addon_display_rows_' . $this->current_tab );
			}
		}
		 ...
		 * @since 2.5.5
		 */
		public function display_rows_ebox() {
			...
			foreach ( (array) $this->items as $plugin ) {
						...
						switch ( $status['status'] ) {
							case 'install':
								if ( $status['url'] ) {
									...
									$action_links[] = '<a class="install-now button" data-slug="' . esc_attr( $plugin['slug'] ) . '" href="' . esc_url( $status['url'] ) . '" aria-label="' . esc_attr( sprintf( esc_html__( 'Install %s now', 'ebox' ), $name ) ) . '" data-name="' . esc_attr( $name ) . '">' . esc_html__( 'Install Now', 'ebox' ) . '</a>';
								}
								break;
							...
							case 'update_available':
								if ( $status['url'] ) {
									/* translators: 1: Plugin name and version */
									$action_links[] = '<a class="update-now button aria-button-if-js" data-plugin="' . esc_attr( $status['file'] ) . '" data-slug="' . esc_attr( $plugin['slug'] ) . '" href="' . esc_url( $status['url'] ) . '" aria-label="' . esc_attr( sprintf( esc_html__( 'Update %s now', 'ebox' ), $name ) ) . '" data-name="' . esc_attr( $name ) . '">' . esc_html__( 'Update Now', 'ebox' ) . '</a>';
								}
								break;
							...
							case 'newer_installed':
								if ( is_plugin_active( $status['file'] ) ) {
									$action_links[] = '<button type="button" class="button button-disabled" disabled="disabled">' . esc_html_x( 'Active', 'plugin', 'ebox' ) . '</button>';
								} elseif ( current_user_can( 'activate_plugin', $status['file'] ) ) {
									$button_text = esc_html__( 'Activate', 'ebox' );
									/* translators: %s: Plugin name */
									$button_label = esc_html_x( 'Activate %s', 'plugin', 'ebox' );
									...
									if ( is_network_admin() ) {
										$button_text = esc_html__( 'Network Activate', 'ebox' );
										/* translators: %s: Plugin name */
										$button_label = esc_html_x( 'Network Activate %s', 'plugin', 'ebox' );
									...
									$action_links[] = '<button type="button" class="button button-disabled" disabled="disabled">' . esc_html_x( 'Installed', 'plugin', 'ebox' ) . '</button>';
								}
								break;
						}
					}
				...
				/* translators: 1: Plugin name and version. */
				$action_links[] = '<a href="' . esc_url( $details_link ) . '" class="thickbox open-plugin-details-modal" aria-label="' . esc_attr( sprintf( esc_html__( 'More information about %s', 'ebox' ), $name ) ) . '" data-title="' . esc_attr( $name ) . '">' . esc_html__( 'More Details', 'ebox' ) . '</a>';
				...
				if ( ! empty( $plugin['icons']['svg'] ) ) {
					...
					$plugin_icon_url = $plugin['icons']['ebox'];
				}

				if ( ( ! empty( $plugin_icon_url ) ) && ( substr( $plugin_icon_url, 0, 2 ) != '//' ) ) {
					$plugin_icon_url = ebox_LMS_PLUGIN_URL . $plugin_icon_url;
				} else {
					$plugin_icon_url = ebox_LMS_PLUGIN_URL . 'assets/images-add-ons/' . basename( $plugin_icon_url );
				}
			...
			<div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
				...
				<div class="plugin-card-bottom">
					<div class="column-updated">
						<strong><?php esc_html_e( 'Last Updated:', 'ebox' ); ?></strong>
						<?php
						printf(
						// translators: placeholder: Human relative date time.
							esc_html_x( '%s ago', 'placeholder: human relative date time', 'ebox' ),
					...
					<div class="column-compatibility">
						...
						if ( ! empty( $plugin['tested'] ) && version_compare( substr( $wp_version, 0, strlen( $plugin['tested'] ) ), $plugin['tested'], '>' ) ) {
							echo '<span class="compatibility-untested">' . esc_html__( 'Untested with your version of WordPress', 'ebox' ) . '</span>';
						} elseif ( ! empty( $plugin['requires'] ) && version_compare( substr( $wp_version, 0, strlen( $plugin['requires'] ) ), $plugin['requires'], '<' ) ) {
							echo '<span class="compatibility-incompatible">' . wp_kses_post( __( '<strong>Incompatible</strong> with your version of WordPress', 'ebox' ) ) . '</span>';
						} else {
							echo '<span class="compatibility-compatible">' . wp_kses_post( __( '<strong>Compatible</strong> with your version of WordPress', 'ebox' ) ) . '</span>';
						}
						?>
					</div>
				</div>
		...
		protected function get_views() {
			 ...
			 */
			$this->tabs = apply_filters( 'ebox_addon_tabs', $this->tabs );
		...
		public function show_update_button() {
			...
			echo '<a href="' . esc_url( $page_url ) . '" id="ebox-updater" class="button button-primary" style=" float: right; margin: 13px 0;">' . esc_html__( 'Check Updates', 'ebox' ) . '</a>';
		}
	}
}
