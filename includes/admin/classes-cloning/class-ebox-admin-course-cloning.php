...
if ( class_exists( 'ebox_Admin_Cloning' ) && ! class_exists( 'ebox_Admin_Course_Cloning' ) ) {
	/**
	 * Class ebox Admin Course Cloning.
	 *
	 * @since 4.2.0
	 */
	class ebox_Admin_Course_Cloning extends ebox_Admin_Cloning {
		/**
		 ...
		 * @since 4.2.0
		 *
		 * @param WP_Post $ld_object The ebox WP_Post object.
		 *
		 * @return boolean
		 */
		protected function run_clone_immediately( WP_Post $ld_object ): bool {
			return ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) === 'yes';
		}
		 ...
		 * @since 4.2.0
		 *
		 * @param WP_Post $ld_object The ebox WP_Post course.
		...
		public function clone( WP_Post $ld_object, array $args = array() ) {
			...
			if ( ! $shared_steps ) {
				$lesson_cloning_class = new ebox_Admin_Lesson_Cloning();
				$topic_cloning_class  = new ebox_Admin_Topic_Cloning();
				$quiz_cloning_class   = new ebox_Admin_Quiz_Cloning();
			}
			...
			if ( ! $shared_steps ) {
				...
				ebox_course_set_steps_dirty( $new_course_id );
			}
		...
		private function course_meta_updates( WP_Post $course, int $new_course_id, bool $shared_steps ): void {
			...
			ebox_update_setting( $new_course_id, 'exam_challenge', '' );

			$course_meta = get_post_meta( $course->ID );
			if ( is_array( $course_meta ) ) {
				foreach ( $course_meta as $meta_key => $meta_value ) {
					// update team meta.
					$team_enrolled_prefix = 'ebox_team_enrolled';
					if ( substr( $meta_key, 0, strlen( $team_enrolled_prefix ) ) === $team_enrolled_prefix ) {
