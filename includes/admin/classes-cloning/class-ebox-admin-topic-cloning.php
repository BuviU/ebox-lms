...
if ( class_exists( 'ebox_Admin_Cloning' ) && ! class_exists( 'ebox_Admin_Topic_Cloning' ) ) {
	/**
	 * Class ebox Admin Topic Cloning.
	 *
	 * @since 4.2.0
	 */
	class ebox_Admin_Topic_Cloning extends ebox_Admin_Cloning {
		/**
		 ...
		 * @since 4.2.0
		 *
		 * @param WP_Post $ld_object The ebox WP_Post topic.
		...
		private function course_meta_updates( WP_Post $topic, int $new_topic_id, array $args ): void {
			...
			if ( is_array( $topic_meta ) ) {
				foreach ( $topic_meta as $meta_key => $meta_value ) {
					// topic settings meta.
					if ( '_ebox-topic' === $meta_key ) {
						$topic_settings = maybe_unserialize( $meta_value[0] );
						if ( isset( $topic_settings['ebox-topic_course'] ) ) {
							$topic_settings['ebox-topic_course'] = ! empty( $args['new_course_id'] ) ? $args['new_course_id'] : 0;
						}
						if ( isset( $topic_settings['ebox-topic_lesson'] ) ) {
							$topic_settings['ebox-topic_lesson'] = ! empty( $args['new_lesson_id'] ) ? $args['new_lesson_id'] : 0;
						}
