...
if ( class_exists( 'ebox_Admin_Cloning' ) && ! class_exists( 'ebox_Admin_Lesson_Cloning' ) ) {
	/**
	 * Class ebox Admin Lesson Cloning.
	 *
	 * @since 4.2.0
	 */
	class ebox_Admin_Lesson_Cloning extends ebox_Admin_Cloning {
		/**
		 ...
		 * @since 4.2.0
		 *
		 * @param WP_Post $ld_object The ebox WP_Post lesson.
		...
		private function course_meta_updates( WP_Post $lesson, int $new_lesson_id, array $args ): void {
			...
			if ( is_array( $lesson_meta ) ) {
				foreach ( $lesson_meta as $meta_key => $meta_value ) {
					...
					if ( '_ebox-modules' === $meta_key ) {
						$lesson_settings = maybe_unserialize( $meta_value[0] );
						if ( isset( $lesson_settings['ebox-modules_course'] ) ) {
							$lesson_settings['ebox-modules_course'] = ! empty( $args['new_course_id'] ) ? $args['new_course_id'] : 0;
						}
