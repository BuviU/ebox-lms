...
if ( class_exists( 'ebox_Admin_Cloning' ) && ! class_exists( 'ebox_Admin_Quiz_Cloning' ) ) {
	/**
	 * Class ebox Admin Quiz Cloning.
	 *
	 * @since 4.2.0
	 */
	class ebox_Admin_Quiz_Cloning extends ebox_Admin_Cloning {
		/**
		 ...
		 * @since 4.2.0
		 *
		 * @param WP_Post $ld_object The ebox WP_Post quiz.
		...
		public function clone( WP_Post $ld_object, array $args = array() ) {
			...
			if ( false === $result ) {
				return new WP_Error(
					'ebox_cloning_quiz_error',
					sprintf(
						// translators: placeholder: Quiz name, Quiz error.
						__( 'Error cloning quiz %1$s: %2$s', 'ebox' ),
			...
			$settings_option                     = get_post_meta( $new_quiz_id, '_ebox-quiz', true );
			$settings_option['ebox-quiz_course'] = ! empty( $args['new_course_id'] ) ? $args['new_course_id'] : 0;
			$settings_option['ebox-quiz_lesson'] = ! empty( $args['new_lesson_id'] ) ? $args['new_lesson_id'] : 0;
			update_post_meta( $new_quiz_id, '_ebox-quiz', $settings_option );
