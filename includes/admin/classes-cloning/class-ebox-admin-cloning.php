...
if ( ! class_exists( 'ebox_Admin_Cloning' ) ) {
	/**
	 * Class ebox Admin Cloning.
	 *
	 * @since 4.2.0
	 */
	abstract class ebox_Admin_Cloning {
		/**
		...
		/**
		 * Clone the ebox Object.
		 *
		 * @since 4.2.0
		 *
		 * @param WP_Post $ld_object The ebox WP_Post.
		 ...
		 * @since 4.2.0
		 *
		 * @param ebox_Admin_Action_Scheduler $cloning_scheduler The cloning scheduler instance.
		 *
		 * @return void
		 */
		public function init( ebox_Admin_Action_Scheduler $cloning_scheduler ): void {
		...
		/**
		 * Get the ebox Object name for cloning.
		 *
		 * @since 4.2.0
		 *
		 * @return string The ebox Object name for cloning.
		 */
		...
		private function get_cloning_action_name(): string {
			return 'ebox_cloning_action_' . $this->get_cloning_object();
		}
		...
		private function get_cloning_task_name(): string {
			return 'ebox_cloning_task_' . $this->get_cloning_object();
		}
		...
		public function add_cloning_option( array $actions, WP_Post $post ): array {
			if ( ebox_get_post_type_slug( $this->get_cloning_object() ) === $post->post_type ) {
		...
		public function action_cloning(): void {
			$redirect_url = admin_url( 'edit.php?post_type=' . ebox_get_post_type_slug( $this->get_cloning_object() ) );

			// request validation.
			if ( ! isset( $_GET['object_id'] ) || ! isset( $_GET['nonce'] ) ) {
				ebox_Admin_Action_Scheduler::add_admin_notice( __( 'Invalid request.', 'ebox' ), 'error', 0, $redirect_url );
			}
			...
			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['nonce'] ) ), $this->get_cloning_action_name() . $object_id ) ) {
				ebox_Admin_Action_Scheduler::add_admin_notice( __( 'Request expired. Please try it again.', 'ebox' ), 'error', $object_id, $redirect_url );
			}

			// object validation.
			$object = ! empty( $object_id ) ? get_post( $object_id ) : false;
			if ( empty( $object ) || ebox_get_post_type_slug( $this->get_cloning_object() ) !== $object->post_type ) {
				// translators: placeholder: object name.
				ebox_Admin_Action_Scheduler::add_admin_notice( sprintf( __( 'Invalid ebox %s.', 'ebox' ), $this->get_cloning_object() ), 'error', $object_id, $redirect_url );
			}
			...
			if ( $this->run_clone_immediately( $object ) ) {
				...
				ebox_Admin_Action_Scheduler::add_admin_notice(
					sprintf(
					// translators: placeholder: current object link, new object name.
						__( 'The cloning of %1$s into %2$s is scheduled. Please refresh this page to see the progress.', 'ebox' ),
				...
				$this->cloning_scheduler->enqueue_task(
					...
					sprintf(
					// translators: placeholder: current object link, new object name.
						__( 'The cloning of %1$s into %2$s is in the processing queue. Please refresh this page to see the progress.', 'ebox' ),
						'<a href="' . get_edit_post_link( $object ) . '">' . esc_html( $object->post_title ) . '</a>',
						'<b>' . esc_html( $this->get_default_copy_name( $object ) ) . '</b>'
					),
					sprintf(
					// translators: placeholder: current object link, new object name.
						__( 'The cloning of %1$s into %2$s is running. Please refresh this page to see the progress.', 'ebox' ),
						'<a href="' . get_edit_post_link( $object ) . '">' . esc_html( $object->post_title ) . '</a>',
						'<b>' . esc_html( $this->get_default_copy_name( $object ) ) . '</b>'
					)
				);
		 ...
		 * @since 4.2.0
		 *
		 * @param int $object_id The ebox object ID.
		...
		public function cloning_task( int $object_id, int $user_id = 0 ): void {
			...
			// validate post.
			if ( empty( $ld_post ) || ebox_get_post_type_slug( $this->get_cloning_object() ) !== $ld_post->post_type ) {
				ebox_Admin_Action_Scheduler::add_admin_notice(
					sprintf(
						// translators: placeholder: object name.
						__( 'The %1$s can not be cloned because it is not a valid ebox %2$s.', 'ebox' ),
			...
			ebox_Admin_Action_Scheduler::add_admin_notice(
				sprintf(
					// translators: placeholder: ebox object type, cloned object name, new object edit link.
					__( 'Your %1$s %2$s has been cloned successfully into %3$s.', 'ebox' ),
					$this->get_cloning_object(),
		...
		protected function get_default_copy_name( WP_Post $post ): string {
			 ...
			 * @param WP_Post $post      The post object.
			 *
			 * @return string The default name for the ebox cloned post.
			 */
			return apply_filters(
				'ebox_cloning_get_default_copy_name',
				sprintf(
				// translators: placeholder: ebox object name.
					__( 'Copy of %s', 'ebox' ),
					$post->post_title
				),
				$post
		...
		protected function clone_post_fully( WP_Post $post, array $clone_args ): int {
			 ...
			 */
			$new_post_data = apply_filters( 'ebox_cloning_get_cloned_post_data', $new_post_data, $post, $clone_args );
		...
		protected function get_cloning_excluded_meta_keys( WP_Post $post ): array {
			 ...
			 */
			return apply_filters( 'ebox_cloning_excluded_meta_keys', $excluded_keys, $post );
		}
		 ...
		 * @since 4.2.0
		 *
		 * @param ebox_Admin_Action_Scheduler $cloning_scheduler The cloning scheduler.
		 *
		 * @return void
		 */
		public static function init_classes( ebox_Admin_Action_Scheduler $cloning_scheduler ): void {
			$cloning_classes = array(
				array(
					'file_path'  => 'class-ebox-admin-course-cloning.php',
					'class_name' => ebox_Admin_Course_Cloning::class,
				),
				array(
					'file_path'  => 'class-ebox-admin-lesson-cloning.php',
					'class_name' => ebox_Admin_Lesson_Cloning::class,
				),
				array(
					'file_path'  => 'class-ebox-admin-topic-cloning.php',
					'class_name' => ebox_Admin_Topic_Cloning::class,
				),
				array(
					'file_path'  => 'class-ebox-admin-quiz-cloning.php',
					'class_name' => ebox_Admin_Quiz_Cloning::class,
				),
			);

			$folder_path = ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-cloning/';
			 ...
			 */
			self::$cloning_classes = apply_filters( 'ebox_cloning_classes', self::$cloning_classes );

			self::$cloning_classes = array_filter(
				self::$cloning_classes,
				function( $cloning_class ) {
					return $cloning_class instanceof ebox_Admin_Cloning;
				}
			);
