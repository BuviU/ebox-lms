<?php
/**
 * ebox Admin Essay Edit.
 *
 * @since 3.2.3
 * @package ebox\Essay\Edit
 */
...
if ( ( class_exists( 'ebox_Admin_Post_Edit' ) ) && ( ! class_exists( 'ebox_Admin_Essay_Edit' ) ) ) {

	/**
	 * Class ebox Admin Essay Edit.
	 *
	 * @since 3.2.3
	 * @uses ebox_Admin_Post_Edit
	 */
	class ebox_Admin_Essay_Edit extends ebox_Admin_Post_Edit {
		...
		public function on_load() {
			if ( $this->post_type_check() ) {
				...
				add_meta_box(
					'ebox_essay_upload_div',
					esc_html__( 'Essay Upload', 'ebox' ),
					array( $this, 'essay_upload_meta_box' ),
					ebox_get_post_type_slug( 'essay' ),
					'normal',
					'high'
				);

				add_meta_box(
					'ebox_essay_status_div',
					esc_html__( 'Essay Grading Status', 'ebox' ),
		...
		public function essay_grading_meta_box( $essay ) {
			...
			if ( $question && is_a( $question, 'WpProQuiz_Model_Question' ) ) {
				$submitted_essay_data = ebox_get_submitted_essay_data( $quiz_id, $question->getId(), $essay );
			}

			wp_nonce_field( 'ebox-essay-grading-nonce-' . $essay->ID, 'ebox-essay-grading-nonce' );
			...
			<div class="submitbox" id="submitpost">
				<div id="minor-publishing">
					<div id="misc-publishing-actions">
								...
								<div id="post-status-select">
									<select name='post_status' id='post_status'>
										<option <?php selected( $essay->post_status, 'not_graded' ); ?>
											value='not_graded'><?php esc_html_e( 'Not Graded', 'ebox' ); ?></option>
										<option <?php selected( $essay->post_status, 'graded' ); ?>
											value='graded'><?php esc_html_e( 'Graded', 'ebox' ); ?></option>
									</select>
								</div>
						...
						<div class="misc-pub-section">
							<?php if ( $question && is_a( $question, 'WpProQuiz_Model_Question' ) ) : ?>
								<p>
									...
									<strong><?php echo sprintf( esc_html_x( 'Essay %s', 'placeholder: question', 'ebox' ), esc_html( ebox_get_custom_label( 'question' ) ) ); ?>:</strong> <?php echo wp_kses_post( $question->getQuestion() ); ?>
									<?php
									...
									if ( ( true === ebox_is_data_upgrade_quiz_questions_updated() ) && ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'enabled' ) === 'yes' ) ) {
										$question_post_id = ebox_get_question_post_by_pro_id( $question->getId() );
									...
									?>
									<span>(<a href="<?php echo esc_url( $question_edit_url ); ?>"><?php esc_html_e( 'Edit', 'ebox' ); ?></a>)</span>
								</p>
								<p><strong><?php esc_html_e( 'Points available', 'ebox' ); ?>:</strong> <?php echo esc_html( $question->getPoints() ); ?></p>
								<p>
									<strong><?php esc_html_e( 'Points awarded', 'ebox' ); ?>:</strong>
							...
							<?php else : ?>
								<p> <?php printf( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
									...
									esc_html_x( 'We could not find the essay %s for this response', 'placeholder: question', 'ebox' ),
							...
							if ( empty( $essay_quiz_post_id ) ) {
								...
								if ( count( $essay_quiz_query->posts ) > 1 ) {
									?>
										...
										printf( esc_html_x( 'Essay %s', 'placeholder: Quiz.', 'ebox' ), ebox_Custom_Label::get_label( 'quiz' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
										?>
										:</strong>
										<select name="essay_quiz_post_id">
											<option value=""><?php printf( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
												// translators: placeholder: Quiz.
												esc_html_x( 'No %s', 'placeholder: Quiz', 'ebox' ),
							...
							if ( ! empty( $essay_quiz_post_id ) ) {
								...
								?>
									<p><strong><?php printf( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
										// translators: placeholder: Quiz.
										esc_html_x( 'Essay %s', 'placeholder: Quiz', 'ebox' ),
										ebox_Custom_Label::get_label( 'quiz' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
									...
									<?php if ( ! empty( $essay_quiz_edit_link ) ) { ?>
											<span>(<a href="<?php echo esc_url( $essay_quiz_edit_link ); ?>"><?php esc_html_e( 'edit', 'ebox' ); ?></a>)</span>
									<?php } ?>
									</p>
								<?php
								...
								if ( ! empty( $essay_quiz_course_id ) ) {
									...
									?>
										<p><strong><?php printf( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
											...
											esc_html_x( 'Essay %s', 'placeholder: Course', 'ebox' ),
										...
										<?php if ( ! empty( $course_quiz_edit_link ) ) { ?>
												<span>(<a href="<?php echo esc_url( $course_quiz_edit_link ); ?>"><?php esc_html_e( 'edit', 'ebox' ); ?></a>)</span>
										<?php } ?>
										</p>
									<?php
									...
									if ( ! empty( $essay_quiz_lesson_id ) ) {
										...
										?>
											<p><strong><?php printf( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
												...
												esc_html_x( 'Essay %s', 'placeholder: Lesson', 'ebox' ),
											...
											<?php if ( ! empty( $lesson_quiz_edit_link ) ) { ?>
													<span>(<a href="<?php echo esc_url( $lesson_quiz_edit_link ); ?>"><?php esc_html_e( 'edit', 'ebox' ); ?></a>)</span>
											<?php } ?>
											</p>
										<?php
									}
						...
						$date_format = esc_html__( 'M j, Y @ H:i', 'ebox' );
						if ( 0 != $essay->ID ) {
								...
								<span id="timestamp">
									...
									echo wp_kses_post(
										sprintf(
											...
											_x( 'Submitted on: <b>%s</b>', 'placeholder: Essay submit date', 'ebox' ),
											esc_html( $date )
										)
									);
									?>
				...
				<div id="major-publishing-actions">
					...
					<div id="delete-action">
						...
						if ( current_user_can( 'delete_post', $essay->ID ) ) :
							if ( ! EMPTY_TRASH_DAYS ) :
								$delete_text = esc_html__( 'Delete Permanently', 'ebox' );
							else :
								$delete_text = esc_html__( 'Move to Trash', 'ebox' );
							endif;
							?>
					...
					<div id="publishing-action">
						...
						<?php if ( $can_publish ) : ?>
							<input name="original_publish" type="hidden" id="original_publish" value="<?php esc_attr_e( 'Update', 'ebox' ); ?>"/>
							<?php submit_button( esc_html__( 'Update', 'ebox' ), 'primary button-large', 'submit', false ); ?>
						<?php endif; ?>
					</div>
					<div class="clear"></div>
				</div>
		...
		public function essay_upload_meta_box( $essay ) {
			...
			if ( ! empty( $upload ) ) {
				...
				printf(
					// translators: placeholder: question.
					esc_html_x( 'Upload was not provided for this %s', 'placeholder: question', 'ebox' ),
					esc_html( ebox_get_custom_label_lower( 'question' ) )
				);
			}
		}
		...
		protected function remove_subbmitdiv_metabox() {
			remove_meta_box( 'submitdiv', ebox_get_post_type_slug( 'essay' ), 'side' );
		}
		 ...
		 *
		 * Fires on `save_post_ebox-essays` hook.
		 *
		 * @since 3.2.3
		 *
		...
		public function save_essay_status_metabox_data( $essay_id, $essay, $update ) {
			...
			if ( ( ! isset( $_POST['ebox-essay-grading-nonce'] ) ) || ( empty( $_POST['ebox-essay-grading-nonce'] ) ) || ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ebox-essay-grading-nonce'] ) ), 'ebox-essay-grading-nonce-' . $essay_id ) ) ) {
				return;
			}
			...
			$submitted_essay = ebox_get_submitted_essay_data( $quiz_id, $question_id, $essay );
			 ...
			 */
			$submitted_essay = apply_filters( 'ebox_essay_status_data', $submitted_essay );
			ebox_update_submitted_essay_data( $quiz_id, $question_id, $essay, $submitted_essay );
			...
			if ( ! is_null( $original_points_awarded ) && ! is_null( $points_awarded ) ) {
				 ...
				 */
				$updated_scoring = apply_filters( 'ebox_updated_essay_scoring', $updated_scoring );
				ebox_update_quiz_data( $quiz_id, $question_id, $updated_scoring, $essay );
				 ...
				 */
				do_action( 'ebox_essay_all_quiz_data_updated', $quiz_id, $question_id, $updated_scoring, $essay );
			}
		}
...
function ebox_team_leader_can_edit_essay_filter( $allcaps, $cap, $args, $user ) {
	...
	if ( ebox_get_post_type_slug( 'essay' ) !== $typenow ) {
		return $allcaps;
	}
...
add_action(
	...
	function () {
		if ( ebox_is_team_leader_user() ) {
			add_filter( 'user_has_cap', 'ebox_team_leader_can_edit_essay_filter', 10, 4 );
		}
	},
	10
);
