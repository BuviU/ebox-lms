<?php
/**
 * ebox Admin Quiz Edit.
 *
 * @since 2.4.0
 * @package ebox\Quiz\Edit
 */
...
if ( ( class_exists( 'ebox_Admin_Post_Edit' ) ) && ( ! class_exists( 'ebox_Admin_Quiz_Edit' ) ) ) {

	/**
	 * Class ebox Admin Quiz Edit.
	 *
	 * @since 2.4.0
	 * @uses ebox_Admin_Post_Edit
	 */
	class ebox_Admin_Quiz_Edit extends ebox_Admin_Post_Edit {
		 ...
		 *
		 * @var ebox_Admin_Metabox_Quiz_Builder
		 */
		...
		public function __construct() {
			$this->post_type = ebox_get_post_type_slug( 'quiz' );
		...
		public function init_quiz_edit( $post ) {
			if ( ! is_a( $this->pro_quiz_edit, 'WpProQuiz_View_QuizEdit' ) ) {
				$quiz_pro_id = (int) ebox_get_setting( $post->ID, 'quiz_pro' );
		...
		public function on_load() {
			if ( $this->post_type_check() ) {
				...
				if ( ! apply_filters( 'ebox_settings_metaboxes_legacy_quiz', ebox_SETTINGS_METABOXES_LEGACY_QUIZ, $this->post_type ) ) {
					require_once ebox_LMS_PLUGIN_DIR . 'includes/settings/settings-metaboxes/class-ld-settings-metabox-quiz-access-settings.php';

					require_once ebox_LMS_PLUGIN_DIR . 'includes/settings/settings-metaboxes/class-ld-settings-metabox-quiz-progress-settings.php';

					require_once ebox_LMS_PLUGIN_DIR . 'includes/settings/settings-metaboxes/class-ld-settings-metabox-quiz-display-content.php';

					require_once ebox_LMS_PLUGIN_DIR . 'includes/settings/settings-metaboxes/class-ld-settings-metabox-quiz-results-display-content-options.php';

					require_once ebox_LMS_PLUGIN_DIR . 'includes/settings/settings-metaboxes/class-ld-settings-metabox-quiz-admin-data-handling-settings.php';
				}

				parent::on_load();

				if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'enabled' ) == 'yes' ) {
					 ...
					 */
					if ( apply_filters( 'ebox_show_quiz_builder', $this->use_quiz_builder ) === true ) {
						$this->quiz_builder = ebox_Admin_Metabox_Quiz_Builder::add_instance();
						$this->quiz_builder->builder_on_load();
					}
				}

				add_filter( 'ebox_header_data', 'ebox\Admin\QuizBuilderHelpers\get_quiz_data', 100 );
			}
		}
		...
		public function save_post( $post_id = 0, $post = null, $update = false ) {
			...
			$quiz_id  = absint( ebox_get_setting( $post_id, 'quiz_pro' ) );
			$pro_quiz = new WpProQuiz_Controller_Quiz();
			$pro_quiz->route(
				array(
		...
		public function add_metaboxes( $post_type = '', $post = null ) {
			...
			if ( $this->post_type_check( $post_type ) ) {
				 ...
				 */
				if ( apply_filters( 'ebox_disable_advance_quiz', false, $post->ID ) ) {
					return;
				}

				/** This filter is documented in includes/admin/classes-posts-edits/class-ebox-admin-quiz-edit.php */
				if ( true === apply_filters( 'ebox_show_quiz_builder', $this->use_quiz_builder ) ) {
					$data_settings = ebox_data_upgrades_setting( 'pro-quiz-questions' );

					$quiz_questions_data_upgrade_link = '';
					if ( ( isset( $data_settings['last_run'] ) ) && ( ! empty( $data_settings['last_run'] ) ) ) {
						$admin_url = admin_url( 'admin.php?page=ebox_data_upgrades' );
					...
					add_meta_box(
						...
						sprintf(
							...
							esc_html_x( 'ebox %s Builder', 'placeholder: Quiz', 'ebox' ),
				...
				if ( apply_filters( 'ebox_settings_metaboxes_legacy_quiz', ebox_SETTINGS_METABOXES_LEGACY_QUIZ, $this->post_type ) ) {
					add_meta_box(
						'ebox_quiz_advanced_aggregated',
						// translators: placeholder: Quiz.
						sprintf( esc_html_x( 'ebox %s Advanced Settings', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'quiz' ) ),
		...
		public function quiz_advanced_page_box_advanced_settings( $post ) {
			...
			$this->quiz_advanced_section_header(
				sprintf(
					// translators: placeholder: Quiz.
					esc_html_x( 'ebox %s Advanced Settings', 'placeholder: Quiz', 'ebox' ),
					ebox_Custom_Label::get_label( 'quiz' )
				)
			);
			...
			$this->quiz_advanced_section_header(
				sprintf(
					...
					esc_html_x( 'ebox %1$s %2$s Settings', 'placeholder: Quiz, Question', 'ebox' ),
			...
			$this->quiz_advanced_section_header(
				sprintf(
					// translators: placeholder: Quiz.
					esc_html_x( 'ebox %s Result Settings', 'placeholder: Quiz', 'ebox' ),
					ebox_Custom_Label::get_label( 'quiz' )
				)
			);
			...
			$this->quiz_advanced_section_header(
				sprintf(
					// translators: placeholder: Quiz.
					esc_html_x( 'ebox %s Mode Settings', 'placeholder: Quiz', 'ebox' ),
					ebox_Custom_Label::get_label( 'quiz' )
				)
			);
			...
			$this->quiz_advanced_section_header(
				sprintf(
					// translators: placeholder: Quiz.
					esc_html_x( 'ebox %s Result Text Settings', 'placeholder: Quiz', 'ebox' ),
					ebox_Custom_Label::get_label( 'quiz' )
				)
			);
			...
			$this->quiz_advanced_section_header(
				sprintf(
					// translators: placeholder: Quiz.
					esc_html_x( 'ebox %s Template Settings', 'placeholder: Quiz', 'ebox' ),
					ebox_Custom_Label::get_label( 'quiz' )
				)
			);
			...
			$this->quiz_advanced_section_header(
				sprintf(
					// translators: placeholder: Quiz.
					esc_html_x( 'ebox %s Leaderboard Settings', 'placeholder: Quiz', 'ebox' ),
					ebox_Custom_Label::get_label( 'quiz' )
				)
			);
			...
			$this->quiz_advanced_section_header(
				sprintf(
					// translators: placeholder: Quiz.
					esc_html_x( 'ebox %s Custom Fields Settings', 'placeholder: Quiz', 'ebox' ),
					ebox_Custom_Label::get_label( 'quiz' )
				)
			);
								...
								<select id="templateLoadId" name="templateLoadId">
									...
									if ( ( isset( $_GET['post'] ) ) && ( ! empty( $_GET['post'] ) ) && ( isset( $_GET['templateLoadId'] ) ) && ( ! empty( $_GET['templateLoadId'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
										...
										echo '<option value="' . esc_url( $template_url ) . '">' . sprintf(
											...
											esc_html_x( 'Revert: %s', 'placeholder: Quiz Title', 'ebox' ),
