...
if ( ! class_exists( 'ebox_Admin_Posts_Listing' ) ) {

	/**
	 * Class ebox Posts Listing Abstract.
	 *
	 * @since 2.6.0
	 */
	abstract class ebox_Admin_Posts_Listing {
		/**
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_ebox_listing_select2_query', array( $this, 'ajax_listing_select2_query' ), 10 );
		...
		public function listing_init() {
			 ...
			 */
			$this->selectors = apply_filters( 'ebox_listing_selectors', $this->selectors, $this->post_type );
			 ...
			 * @param string $post_type Post Type for listing table.
			 */
			$this->columns = apply_filters( 'ebox_listing_columns', $this->columns, $this->post_type );
		...
		public function listing_taxonomies_init() {
			...
			if ( has_filter( 'ebox-admin-taxonomy-filters-display' ) ) {
				/**
				 * Filters admin settings taxonomy filters list.
				 *
				 * @since 2.4.0
				 * @deprecated 3.2.3 Use {@see 'ebox_listing_taxonomies'} instead.
				 *
				...
				$object_taxonomies = apply_filters_deprecated(
					'ebox-admin-taxonomy-filters-display',
					array( $object_taxonomies, $this->post_type ),
					'3.2.3',
					'ebox_listing_taxonomies'
				);
			}
			 ...
			 */
			$object_taxonomies = apply_filters( 'ebox_listing_taxonomies', $object_taxonomies, $this->post_type );
		...
		protected function register_listing_set() {
			...
			if ( ! isset( self::$listing_sets[ $class_nonce ] ) ) {
				...
				foreach ( $this->selectors as $selector_id => &$selector ) {
					...
					if ( 'post_type' === $selector['type'] ) {
						...
						if ( ( ! isset( $selector['field_label'] ) ) || ( empty( $selector['field_label'] ) ) ) {
							$selector['field_label'] = sprintf(
								...
								esc_html_x( 'Filter by %s', 'placeholder: Post Type singular name', 'ebox' ),
						...
						if ( ( ! isset( $selector['field_label'] ) ) || ( empty( $selector['field_label'] ) ) ) {
							$selector['field_label'] = sprintf(
								...
								esc_html_x( 'Filter by %s', 'placeholder: Taxonomy singular name', 'ebox' ),
					...
					if ( ( ! $selector['select2'] ) || ( ! ebox_use_select2_lib_ajax_fetch() ) ) {
				...
				foreach ( $this->selectors as $key => &$selector ) {
					if ( ( isset( $selector['field_name'] ) ) && ( ! empty( $selector['field_name'] ) ) ) {
						...
						if ( ( isset( $_GET[ $selector['field_name'] ] ) ) && ( ! empty( $_GET[ $selector['field_name'] ] ) ) ) {
							 ...
							 */
							$selector['selected'] = apply_filters( 'ebox_listing_selector_value', $selector['selected'], $selector );
						}
					}
				}
			}
		...
		protected function selector_value_for_author( $user_id = 0, $selector = array() ) {
			...
			if ( ( ebox_is_team_leader_user( get_current_user_id() ) ) && ( 'advanced' !== ebox_get_team_leader_manage_users() ) ) {
				$gl_user_ids = ebox_get_teams_administrators_users( get_current_user_id() );
		 ...
		 * @since 3.2.3
		 *
		 * @param int   $team_id  Selected Team ID.
		 * @param array $selector  Array of attributes used to display the filter selector.
		 */
		protected function selector_value_for_team( $team_id = 0, $selector = array() ) {
			if ( ( isset( $selector['show_empty_value'] ) ) && ( $team_id !== $selector['show_empty_value'] ) ) {
				...
				if ( ( ebox_is_team_leader_user( get_current_user_id() ) ) && ( 'advanced' !== ebox_get_team_leader_manage_teams() ) ) {
					...
					$gl_team_ids = array_map( 'absint', $gl_team_ids );
					if ( ( empty( $gl_team_ids ) ) || ( ! in_array( $team_id, $gl_team_ids, true ) ) ) {
						$team_id = 0;
					}
				}
			}
		...
		public function parse_query_table_listing( $query ) {
			...
			if ( $this->post_type_check() ) {
				 ...
				 * @param object $query     WP_Query instance.
				 */
				$q_vars = apply_filters( 'ebox_listing_table_query_vars_filter', $q_vars, $this->post_type, $query );
			}
		}
		...
		public function restrict_manage_posts_selectors( $post_type = '', $location = '' ) {
			...
			if ( has_filter( 'ebox-admin-cpt-filters-display' ) ) {
				/**
				 * Filters list of CPT shown for a filter.
				 *
				 * @since 2.3.1
				 * @deprecated 3.2.3 Use {@see 'ebox_listing_selectors'} instead.
				 *
				 * @param array $cpt_filters_shown An array of custom post types shown filter.
				 */
				apply_filters_deprecated(
					'ebox-admin-cpt-filters-display',
					array( array() ),
					'3.2.3',
					'ebox_listing_selectors'
				);
			}
		...
		protected function list_table_row_actions( $actions, $always_visible = false ) {
			...
			$out .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . esc_html__( 'Show more details', 'ebox' ) . '</span></button>';
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( ! empty( $course_id ) ) {
					...
					if ( ( $course_post ) && ( is_a( $course_post, 'WP_Post' ) ) ) {
						$course_title = ebox_format_step_post_title_with_status_label( $course_post );
						...
						$row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $course_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';

						if ( current_user_can( 'edit_post', $course_id ) ) {
							$row_actions['ld-post-edit'] = '<a href="' . esc_url( get_edit_post_link( $course_id ) ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $course_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
						}

						if ( is_post_type_viewable( get_post_type( $course_id ) ) ) {
							$row_actions['ld-post-view'] = '<a href="' . esc_url( get_permalink( $course_id ) ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $course_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
						}
					...
					echo '<span class="ld-error dashicons dashicons-warning" title="' . sprintf(
						...
						esc_html_x( '%s Required', 'placeholder: Course', 'ebox' ),
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( ! empty( $lesson_id ) ) {
					...
					if ( ( $lesson_post ) && ( is_a( $lesson_post, 'WP_Post' ) ) ) {
						$lesson_title = ebox_format_step_post_title_with_status_label( $lesson_post );
						...
						$row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $lesson_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';

						$course_id = ebox_get_course_id( $lesson_id );
						if ( current_user_can( 'edit_post', $lesson_id ) ) {
							...
							$row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $lesson_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
						}

						if ( is_post_type_viewable( get_post_type( $lesson_id ) ) ) {
							if ( ! empty( $course_id ) ) {
								$view_url = ebox_get_step_permalink( $lesson_id, $course_id );
							} else {
								$view_url = get_permalink( $lesson_id );
							}

							$row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $lesson_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
						}
						echo $this->list_table_row_actions( $row_actions ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML
					}
				} elseif ( ( isset( $column_meta['required'] ) ) && ( true === $column_meta['required'] ) ) {
					echo '<span class="ld-error dashicons dashicons-warning" title="' . sprintf(
						...
						esc_html_x( '%s Required', 'placeholder: Lesson', 'ebox' ),
		...
		protected function show_column_step_topic( $post_id = 0, $column_meta = array() ) {
			if ( ! empty( $post_id ) ) {
				...
				if ( ! empty( $topic_id ) ) {
					if ( ebox_get_post_type_slug( 'topic' ) === get_post_type( $topic_id ) ) {
						$row_actions = array();
						...
						$row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $topic_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';
						...
						if ( current_user_can( 'edit_post', $topic_id ) ) {
							...
							$row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $topic_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
						}

						if ( is_post_type_viewable( get_post_type( $topic_id ) ) ) {
							if ( ! empty( $course_id ) ) {
								$view_url = ebox_get_step_permalink( $topic_id, $course_id );
							} else {
								$view_url = get_permalink( $topic_id );
							}
							$row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $topic_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
						}
						echo $this->list_table_row_actions( $row_actions ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML
					}
				}
				...
				echo '<span class="ld-error dashicons dashicons-warning" title="' . sprintf(
					...
					esc_html_x( '%s Required', 'placeholder: Topic', 'ebox' ),
		...
		protected function show_column_step_lesson_or_topic( $post_id = 0, $column_meta = array() ) {
			if ( ! empty( $post_id ) ) {
				...
				$lesson_id = get_post_meta( $post_id, 'lesson_id', true );
				if ( ! empty( $lesson_id ) ) {
					if ( ebox_get_post_type_slug( 'topic' ) === get_post_type( $lesson_id ) ) {
					...
					if ( ! empty( $lesson_id ) ) {
						...
						if ( ( $lesson_post ) && ( is_a( $lesson_post, 'WP_Post' ) ) ) {
							...
							echo sprintf(
								...
								esc_html_x( '%1$s: %2$s', 'Placeholders: Lesson label, Lesson Filter Anchor', 'ebox' ),
								ebox_Custom_Label::get_label( 'lesson' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
								'<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $lesson_id, 'filter' ) ) . '">' . wp_kses_post( $lesson_title ) . '</a>'
							);

							$lesson_row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $lesson_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';
							...
							if ( current_user_can( 'edit_post', $lesson_id ) ) {
								...
								$lesson_row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $lesson_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
							}
							...
							if ( is_post_type_viewable( get_post_type( $lesson_id ) ) ) {
								...
								$lesson_row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $lesson_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
							}
					...
					if ( ! empty( $topic_id ) ) {
						...
						if ( ( $topic_post ) && ( is_a( $topic_post, 'WP_Post' ) ) ) {
							$topic_title = ebox_format_step_post_title_with_status_label( $topic_post );
							...
							echo sprintf(
								...
								esc_html_x( '%1$s: %2$s', 'Placeholders: Topic label, Topic Filter Anchor', 'ebox' ),
								ebox_Custom_Label::get_label( 'topic' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
								'<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $topic_id, 'filter' ) ) . '">' . wp_kses_post( $topic_title ) . '</a>'
							);

							$topic_row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $topic_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';
							...
							if ( current_user_can( 'edit_post', $topic_id ) ) {
								...
								$topic_row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $topic_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
							}
							...
							if ( is_post_type_viewable( get_post_type( $topic_id ) ) ) {
								...
								$topic_row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $topic_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
							}
			...
			if ( ! empty( $post_id ) ) {
				...
				if ( ! empty( $quiz_id ) ) {
					...
					if ( ( $quiz_post ) && ( is_a( $quiz_post, 'WP_Post' ) ) ) {
						$quiz_title = ebox_format_step_post_title_with_status_label( $quiz_post );
						...
						$row_actions['ld-post-filter'] = '<a href="' . esc_url( $filter_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $quiz_id, 'filter' ) ) . '">' . esc_html__( 'filter', 'ebox' ) . '</a>';

						$course_id = ebox_get_course_id( $quiz_id );
						if ( current_user_can( 'edit_post', $quiz_id ) ) {
							...
							$row_actions['ld-post-edit'] = '<a href="' . esc_url( $edit_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $quiz_id, 'edit' ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
						}

						if ( is_post_type_viewable( get_post_type( $quiz_id ) ) ) {
							if ( ! empty( $course_id ) ) {
								$view_url = ebox_get_step_permalink( $quiz_id, $course_id );
							} else {
								$view_url = get_permalink( $quiz_id );
							}
							$row_actions['ld-post-view'] = '<a href="' . esc_url( $view_url ) . '" aria-label="' . esc_attr( $this->get_aria_label_for_post( $quiz_id, 'view' ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';
						}
						echo $this->list_table_row_actions( $row_actions ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML
					}
				} elseif ( ( isset( $column_meta['required'] ) ) && ( true === $column_meta['required'] ) ) {
					echo '<span class="ld-error dashicons dashicons-warning" title="' . sprintf(
						...
						esc_html_x( '%s Required', 'placeholder: Quiz', 'ebox' ),
		...
		protected function show_reset_button() {
			...
			echo '<a href="' . esc_url( $redirect_url ) . '" class="button button-secondary" id="ld_filter_reset" style="margin: 0 5px 0 0;">' . esc_html__( 'Reset', 'ebox' ) . '</a>';
		}
		...
		protected function show_taxonomy_selector( $selector = array() ) {
			 ...
			 * @param string $post_type     Listing table post type.
			 */
			$dropdown_args = apply_filters( 'ebox_listing_selector_taxonomy_args', $dropdown_args, $selector, $this->post_type );
		...
		protected function show_post_type_selector( $selector = array() ) {
			if ( ebox_get_total_post_count( $selector['post_type'] ) !== 0 ) {
				$selector = $this->build_selector_post_type_options( $selector );
			...
			if ( is_a( $selector['query_results'], 'WP_User_Query' ) ) {
				if ( ! empty( $selector['query_results']->get_results() ) ) {
					foreach ( $selector['query_results']->get_results() as $u ) {
						 ...
						 */
						$u = apply_filters( 'ebox_listing_selector_user_option_before', $u, $selector['query_args'], $selector['query_results'], $this->post_type );
						if ( is_a( $u, 'WP_User' ) ) {
							 ...
							 * @param string  $post_type       The post type of the screen shown.
							 */
							$options_after = apply_filters( 'ebox_listing_selector_user_option_after', array(), $u, $selector['query_args'], $selector['query_results'], $this->post_type );
			...
			if ( is_a( $selector['query_results'], 'WP_Query' ) ) {
				if ( ! empty( $selector['query_results']->posts ) ) {
					foreach ( $selector['query_results']->posts as $p ) {
						if ( has_filter( 'ebox_post_listing_before_option' ) ) {
							/**
							 * Filters the post listing before displaying it to user.
							 *
							 * @since 3.0.0
							 * @deprecated 3.2.3 Use {@see 'ebox_listing_selector_post_type_option_before'} instead.
							 *
							...
							$p = apply_filters_deprecated(
								'ebox_post_listing_before_option',
								array( $p, $selector['query_args'], $this->post_type ),
								'3.2.3',
								'ebox_listing_selector_post_type_option_before'
							);
						}
						 ...
						 * @param string  $post_type       The post type of the screen shown.
						 */
						$p = apply_filters( 'ebox_listing_selector_post_type_option_before', $p, $selector['query_args'], $selector['query_results'], $this->post_type );

						if ( is_a( $p, 'WP_Post' ) ) {
							$selector['options'][ absint( $p->ID ) ] = ebox_format_step_post_title_with_status_label( $p );

							if ( has_action( 'ebox_post_listing_after_option' ) ) {
								/**
								 * Fires after the admin post listing option.
								 *
								 * @since 3.0.0
								 * @deprecated 3.2.3 Use {@see 'ebox_listing_selector_post_type_option_after'} instead.
								 *
								 * @param WP_Post $post            WP_Post object.
								 * @param array   $query_arguments An array of admin post listing query arguments.
								 */
								do_action_deprecated(
									'ebox_post_listing_after_option',
									array( $p, $selector['query_args'], $this->post_type ),
									'3.2.3',
									'ebox_listing_selector_post_type_option_after'
								);
							}
							 ...
							 * @param string $post_type       The post type of the screen shown.
							 */
							$options_after = apply_filters( 'ebox_listing_selector_post_type_option_after', array(), $p, $selector['query_args'], $selector['query_results'], $this->post_type );
							if ( ! empty( $options_after ) ) {
								foreach ( $options_after as $after_key => $after_val ) {
									$selector['options'][ absint( $after_key ) ] = esc_attr( $after_val );
								}
		...
		protected function get_selector_post_type_query_results( $selector = array() ) {
			...
			if ( has_filter( 'ebox_course_post_options_filter' ) ) {
				/**
				 * Filters course filter query arguments.
				 *
				 * @since 2.2.1
				 * @deprecated 3.2.3 Use {@see 'ebox_listing_filter_query_args'} instead.
				 *
				...
				$selector['query_args'] = apply_filters_deprecated(
					'ebox_course_post_options_filter',
					array( $selector['query_args'], $this->post_type ),
					'3.2.3',
					'ebox_listing_filter_query_args'
				);
			}

			if ( has_filter( 'ebox_lesson_post_options_filter' ) ) {
				/**
				 * Filters course filter query arguments.
				 *
				 * @since 2.2.1
				 ...
				 */
				$selector['query_args'] = apply_filters_deprecated(
					'ebox_lesson_post_options_filter',
					array( $selector['query_args'], $this->post_type ),
					'3.2.3',
					'ebox_listing_filter_query_args'
				);
			...
			if ( has_filter( 'ebox_show_post_type_selector_filter' ) ) {
				...
				$selector_args['query_args'] = apply_filters_deprecated(
					'ebox_show_post_type_selector_filter',
					array( $selector['query_args'], $this->post_type ),
					'3.2.3',
					'ebox_listing_filter_query_args'
				);
			 ...
			 * @param string $post_type       The post type of the screen shown.
			 */
			$selector['query_args'] = apply_filters( 'ebox_listing_selector_post_type_query_args', $selector['query_args'], $selector, $this->post_type );

			$query_results = new WP_Query( $selector['query_args'] );

			if ( has_filter( 'ebox_post_listing_results_posts' ) ) {
				/**
				 * Filters post type listing filter posts results.
				 *
				 * @since 3.0.0
				 * @deprecated 3.2.3 Use {@see 'ebox_listing_selector_post_type_query_results'} instead.
				 *
				...
				$query_results->posts = apply_filters_deprecated(
					'ebox_post_listing_results_posts',
					array( $query_results->posts, $selector['query_args'], $this->post_type ),
					'3.2.3',
					'ebox_listing_selector_post_type_query_results'
				);
			}
			 ...
			 * @param string $post_type       The post type of the screen shown.
			 */
			$query_results->posts = apply_filters( 'ebox_listing_selector_post_type_query_results', $query_results->posts, $selector['query_args'], $this->post_type );
		...
		protected function get_user_selector_query_results( $selector = array() ) {
			 ...
			 * @param string $post_type       The post type of the screen shown.
			 */
			$selector['query_args'] = apply_filters( 'ebox_listing_selector_user_selector_query_args', $selector['query_args'], $selector, $this->post_type );
			 ...
			 * @param string $post_type       The post type of the screen shown.
			 */
			$query_results->results = apply_filters( 'ebox_listing_selector_user_query_results', $query_results->results, $selector['query_args'], $this->post_type ); // @phpstan-ignore-line
		 ...
		 * @return array $q_vars  Query Args array.
		 */
		protected function selector_filter_for_team( $q_vars = array(), $selector = array() ) {
			if ( ( ebox_is_team_leader_user( get_current_user_id() ) ) && ( 'advanced' !== ebox_get_team_leader_manage_teams() ) ) {
				$gl_team_ids = ebox_get_administrators_team_ids( get_current_user_id() );
		...
		protected function selector_filter_for_lesson( $q_vars = array(), $selector = array() ) {
			...
			if ( ! empty( $course_id ) ) {
				$modules_items = $ebox_lms->select_a_lesson_or_topic( absint( $course_id ), false, false );
				if ( ! empty( $modules_items ) ) {
					$q_vars['post__in'] = array_keys( $modules_items );
				} else {
					$q_vars['post__in'] = array( 0 );
				}
			} else {
		...
		protected function selector_filter_for_topic( $q_vars = array(), $selector = array() ) {
			...
			if ( ( ! empty( $course_id ) ) && ( ! empty( $lesson_id ) ) ) {
				$topics_array = ebox_get_topic_list( $lesson_id, $course_id );
		 ...
		 */
		protected function listing_filter_by_team( $q_vars, $selector = array() ) {
			if ( ( isset( $selector['selected'] ) ) && ( ! empty( $selector['selected'] ) ) ) {
				if ( ( isset( $selector['show_empty_value'] ) ) && ( $selector['show_empty_value'] === $selector['selected'] ) ) {
					$course_ids = ebox_get_all_courses_with_teams();
					if ( ! empty( $course_ids ) ) {
					...
					$course_ids = ebox_team_enrolled_courses( absint( $selector['selected'] ) );
					if ( ! empty( $course_ids ) ) {
						if ( ebox_get_post_type_slug( 'course' ) === $this->post_type ) {
		...
		protected function listing_filter_by_lesson( $q_vars, $selector = array() ) {
			if ( ( isset( $selector['selected'] ) ) && ( ! empty( $selector['selected'] ) ) ) {
				if ( ( isset( $selector['show_empty_value'] ) ) && ( $selector['show_empty_value'] === $selector['selected'] ) ) {
					...
					if ( ! empty( $course_id ) ) {
						if ( ebox_is_course_shared_steps_enabled() ) {
							$steps_ids = ebox_course_get_children_of_step( $course_id, $selector['selected'], $this->post_type, 'ids', true );
							if ( ! empty( $steps_ids ) ) {
							 ...
							 */
							if ( in_array( $this->post_type, ebox_get_post_type_slug( array( 'quiz', 'essay', 'assignment' ) ), true ) ) {
								// First we get the course modules and check if the lesson selector value is a valid lesson within the course.
								$course_modules_ids = array();
								$course_modules     = ebox_get_lesson_list( $course_selector['selected'] );
								if ( ! empty( $course_modules ) ) {
									$course_modules_ids = wp_list_pluck( $course_modules, 'ID' );
								}

								if ( ( ! empty( $course_modules_ids ) ) && ( in_array( $selector['selected'], $course_modules_ids, true ) ) ) {
									if ( ! isset( $q_vars['meta_query'] ) ) {
										$q_vars['meta_query'] = array(); // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query
									}
		...
		protected function listing_filter_by_topic( $q_vars, $selector = array() ) {
			...
			} elseif ( in_array( $this->post_type, ebox_get_post_type_slug( array( 'quiz', 'essay', 'assignment' ) ), true ) ) {
				if ( ! ebox_is_course_shared_steps_enabled() ) {
					$course_id = 0;
					$lesson_id = 0;
					...
					if ( ( ! empty( $course_id ) ) && ( ! empty( $lesson_id ) ) ) {
						$topics = ebox_course_get_topics( $course_id, $lesson_id );
						if ( ! empty( $topics ) ) {
		...
		public function post_row_actions( $row_actions = array(), $post = null ) {
			if ( $this->post_type_check() ) {
				...
				ebox_check_primary_course_for_step( $post->ID );
			}
		...
		protected function check_query_post_type_count( $post_type = '' ) {
			...
			if ( ! empty( $post_type ) ) {
				if ( is_string( $post_type ) ) {
					$total_post_count += ebox_get_total_post_count( $post_type );
				} elseif ( is_array( $post_type ) ) {
					foreach ( $post_type as $pt ) {
						$total_post_count += ebox_get_total_post_count( $pt );
					}
				}
			}
		...
		protected function hide_empty_taxonomy( $taxonomy_slug = '' ) {
			 ...
			 */
			if ( apply_filters( 'ebox_listing_taxonomies_hide_empty', true, $taxonomy_slug, $this->post_type ) ) {
				...
				switch ( $label_context ) {
					case 'filter':
						$aria_label = sprintf(
							...
							esc_html_x( 'Filter listing by "%s"', 'placeholder: Post title', 'ebox' ),
					...
					case 'edit':
						$aria_label = sprintf(
							// translators: placeholder: Post title.
							esc_html_x( 'Edit "%s"', 'placeholder: Post title', 'ebox' ),
							get_the_title( $post_id )
						);
						break;
					...
					case 'view':
						$aria_label = sprintf(
							// translators: placeholder: Post title.
							esc_html_x( 'View "%s"', 'placeholder: Post title', 'ebox' ),
							get_the_title( $post_id )
						);
...
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-courses-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-modules-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-topics-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-quizzes-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-questions-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-teams-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-certificates-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-transactions-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-assignments-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-essays-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-users-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-exams-listing.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-posts-listings/class-ebox-admin-coupons-listing.php';
