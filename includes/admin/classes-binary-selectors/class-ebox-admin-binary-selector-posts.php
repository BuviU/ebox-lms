...
if ( ( ! class_exists( 'ebox_Binary_Selector_Posts' ) ) && ( class_exists( 'ebox_Binary_Selector' ) ) ) {

	/**
	 * Class ebox Binary Selector Posts.
	 *
	 * @since 2.2.1
	 * @uses ebox_Binary_Selector
	 */
	class ebox_Binary_Selector_Posts extends ebox_Binary_Selector {
		...
		public function __construct( $args = array() ) {
			...
			$defaults = array(
				'paged'               => 1,
				'posts_per_page'      => (int) ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'per_page' ),
				'orderby'             => 'title',
				'order'               => 'ASC',
			...
			if ( $this->is_valid_position( $position ) ) {
				if ( ! empty( $this->element_items[ $position ] ) ) {
					foreach ( $this->element_items[ $position ] as $post ) {
						...
						$item_title = ebox_format_step_post_title_with_status_label( $post );

						/** This filter is documented in includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-users.php */
						$item_title = apply_filters( 'ebox_binary_selector_item', $item_title, $post, $position, $this->selector_class );

						if ( ( is_array( $this->args['selected_ids'] ) ) && ( ! empty( $this->args['selected_ids'] ) ) ) {
							if ( in_array( absint( $post->ID ), $this->args['selected_ids'], true ) ) {
								$disabled_class = 'ebox-binary-selector-item-disabled';
								if ( 'left' == $position ) {
						...
						$options_html .= '<option class="ebox-binary-selector-item ' . $disabled_class . '" ' . $disabled_state . ' value="' . $post->ID . '" data-value="' . $post->ID . '">' . wp_strip_all_tags( $item_title ) . '</option>';
					}
				}
			}
