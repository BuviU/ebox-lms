...
if ( ( ! class_exists( 'ebox_Binary_Selector_Users' ) ) && ( class_exists( 'ebox_Binary_Selector' ) ) ) {

	/**
	 * Class ebox Binary Selector Users.
	 *
	 * @since 2.2.1
	 * @uses ebox_Binary_Selector
	 */
	class ebox_Binary_Selector_Users extends ebox_Binary_Selector {
		...
		public function __construct( $args = array() ) {
			...
			$defaults = array(
				'paged'   => 1,
				'number'  => ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'per_page' ),
			...
			if ( $this->is_valid_position( $position ) ) {
				if ( ! empty( $this->element_items[ $position ] ) ) {
					foreach ( $this->element_items[ $position ] as $user ) {
						 ...
						 */
						$user_name = apply_filters( 'ebox_binary_selector_item', $user->display_name . ' (' . $user->user_login . ')', $user, $position, $this->selector_class );
						if ( ! empty( $user_name ) ) {
						...
						$options_html .= '<option class="ebox-binary-selector-item ' . $disabled_class . '" ' . $disabled_state . ' value="' . $user->ID . '" data-value="' . $user->ID . '">' . $user_name . '</option>';
					}
				}
			}
		...
		protected function query_selection_section_items( $position = '' ) {
			...
			if ( $this->is_valid_position( $position ) ) {
				 ...
				 */
				if ( apply_filters( 'ebox_exclude_user_no_role', true, $this->args, $position, $this->selector_class ) ) {
