<?php
/**
 * ebox Data Upgrades for Quiz Post Meta
 *
 * @since 3.4.1
...
if ( ( class_exists( 'ebox_Admin_Data_Upgrades' ) ) && ( ! class_exists( 'ebox_Admin_Data_Upgrades_Quiz_Post_Meta' ) ) ) {
	/**
	 * Class to create the Data Upgrade for Quiz Post Meta.
	 *
	 * @since 3.4.1
	 */
	class ebox_Admin_Data_Upgrades_Quiz_Post_Meta extends ebox_Admin_Data_Upgrades {
		...
		public function process_post_meta( $force = false ) {
			...
			if ( is_admin() ) {
				...
				/** This filter is documented in includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-course-post-meta.php */
			...
			if ( ! empty( $post_ids_diff ) ) {
				...
				foreach ( $post_ids_diff as $post_id ) {
					...
					if ( ! empty( $certificate ) ) {
						$threshold = ebox_get_setting( $post_id, 'threshold' );
						if ( ! empty( $threshold ) ) {
...
add_action(
	...
	function() {
		ebox_Admin_Data_Upgrades_Quiz_Post_Meta::add_instance();
	}
);
