<?php
/**
 * ebox Data Upgrades for User Quizzes.
 *
 * @since 2.6.0
...
if ( ( class_exists( 'ebox_Admin_Data_Upgrades' ) ) && ( ! class_exists( 'ebox_Admin_Data_Upgrades_User_Meta_Quizzes' ) ) ) {

	/**
	 * Class ebox Data Upgrades for User Quizzes.
	 *
	 * @since 2.6.0
	 * @uses ebox_Admin_Data_Upgrades
	 */
	class ebox_Admin_Data_Upgrades_User_Meta_Quizzes extends ebox_Admin_Data_Upgrades {
			...
			<tr id="ebox-data-upgrades-container-<?php echo esc_attr( $this->data_slug ); ?>" class="ebox-data-upgrades-container">
				...
				<td class="ebox-data-upgrades-status-container">
					...
					printf(
						...
						esc_html_x( 'Upgrade User %s Data', 'placeholder: Quiz', 'ebox' ),
					...
					printf(
						// translators: placeholder: quiz.
						esc_html_x( 'This upgrade will sync your existing user data for %s into a new database table for better reporting. (Required)', 'placeholder: quiz', 'ebox' ),
		...
		protected function convert_user_meta_quizzes_progress_to_activity( $user_id = 0 ) {
			...
			if ( empty( $this->transient_data['current_user']['item_idx'] ) ) {
				ebox_report_clear_user_activity_by_types( $user_id, array( 'quiz' ) );
			}

			$user_course_ids_used = array();

			$user_meta_quizzes_progress = get_user_meta( $user_id, '_ebox-quizzes', true );
			if ( ( ! empty( $user_meta_quizzes_progress ) ) && ( is_array( $user_meta_quizzes_progress ) ) ) {
				...
				foreach ( $user_meta_quizzes_progress as $idx => $quiz_data ) {
					...
					if ( ( ! $quiz_post ) || ( ! is_a( $quiz_post, 'WP_Post' ) ) || ( 'ebox-quiz' !== $quiz_post->post_type ) ) {
						continue;
					}

					if ( ! isset( $quiz_data['course'] ) ) {
						$quiz_data['course']                          = ebox_get_course_id( intval( $quiz_data['quiz'] ) );
				...
				if ( true === $user_meta_quizzes_progress_changed ) {
					update_user_meta( $user_id, '_ebox-quizzes', $user_meta_quizzes_progress );
				}
			}
...
add_action(
	...
	function() {
		ebox_Admin_Data_Upgrades_User_Meta_Quizzes::add_instance();
	}
);
