<?php
/**
 * ebox Data Upgrades for Quiz Questions.
 *
 * @since 2.6.0
...
if ( ( class_exists( 'ebox_Admin_Data_Upgrades' ) ) && ( ! class_exists( 'ebox_Admin_Data_Upgrades_Quiz_Questions' ) ) ) {

	/**
	 * Class ebox Data Upgrades for Quiz Questions.
	 *
	 * @since 2.6.0
	 * @uses ebox_Admin_Data_Upgrades
	 */
	class ebox_Admin_Data_Upgrades_Quiz_Questions extends ebox_Admin_Data_Upgrades {
			...
			<tr id="ebox-data-upgrades-container-<?php echo esc_attr( $this->data_slug ); ?>" class="ebox-data-upgrades-container">
				...
				<td class="ebox-data-upgrades-status-container">
					...
					printf(
						...
						esc_html_x( 'Upgrade %1$s %2$s', 'placeholders: Quiz, Questions', 'ebox' ),
					...
					echo wp_kses_post(
						sprintf(
							...
							_x( 'This upgrade will convert the %1$s %2$s to WordPress custom post type. <strong>This is required before enabling %3$s Builder.</strong> (Optional)', 'placeholders: Quiz questions, Quiz', 'ebox' ),
							esc_html( ebox_get_custom_label_lower( 'Quiz' ) ),
							esc_html( ebox_get_custom_label_lower( 'questions' ) ),
							esc_html( ebox_get_custom_label( 'Quiz' ) )
						)
					);
					?>
					</p>
					...
					if ( ( true === $show_progress ) && ( ! isset( $_GET['quiz_id'] ) ) ) {
						...
						if ( ( isset( $data_settings['last_run'] ) ) && ( ! empty( $data_settings['last_run'] ) ) ) {
							...
							if ( ( isset( $_GET['quiz_id'] ) ) && ( ! empty( $_GET['quiz_id'] ) ) ) {
								// phpcs:ignore WordPress.Security.NonceVerification.Recommended
								if ( ebox_get_post_type_slug( 'quiz' ) === get_post_type( absint( $_GET['quiz_id'] ) ) ) {
									// phpcs:ignore WordPress.Security.NonceVerification.Recommended
									...
									<?php
										printf(
											...
											esc_html_x( 'Invalid %s ID', 'placeholders: Quiz', 'ebox' ),
											ebox_Custom_Label::get_label( 'Quiz' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
										)
									?>
										</p>
							...
							if ( ! empty( $process_quiz_id ) ) {
								?>
								<p id="ebox-data-upgrades-quiz-<?php echo esc_attr( $this->data_slug ); ?>" class="ebox-data-upgrades-quiz">
								<?php
								printf(
									// translators: placeholders: Questions, Quiz, Quiz Title.
									esc_html_x( 'Reprocess %1$s for %2$s: "%3$s"', 'placeholders: Questions, Quiz, Quiz Title', 'ebox' ),
									ebox_Custom_Label::get_label( 'Questions' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
									ebox_Custom_Label::get_label( 'Quiz' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
								...
								?>
									...
									<input type="hidden" name="ebox-data-upgrades-quiz" value="<?php echo absint( $_GET['quiz_id'] ); ?>" />
									</p>
									<?php
							} else {
								?>
								<p id="ebox-data-upgrades-mismatched-<?php echo esc_attr( $this->data_slug ); ?>" class="ebox-data-upgrades-mismatched"><input type="checkbox" name="ebox-data-upgrades-mismatched" value="1" checked="checked" />
									<?php
									printf(
										// translators: placeholders: Questions.
										esc_html_x( 'Process Mismatched %s only?', 'placeholders: Questions', 'ebox' ),
										ebox_Custom_Label::get_label( 'Questions' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
									);
									?>
									</p>
		...
		public function process_upgrade_action( $data = array() ) {
				...
				if ( ( wp_verify_nonce( $data['nonce'], 'ebox-data-upgrades-' . $this->data_slug . '-' . get_current_user_id() ) ) && ( current_user_can( ebox_ADMIN_CAPABILITY_CHECK ) ) ) {
					...
					if ( ( isset( $data['init'] ) ) && ( '1' === $data['init'] ) ) {
						...
						if ( 'true' !== $data['continue'] ) {
							...
							$quiz_builder_option                           = get_option( 'ebox_settings_quizzes_builder' );
							$quiz_builder_option['force_quiz_builder']     = '';
							$quiz_builder_option['force_shared_questions'] = '';
							update_option( 'ebox_settings_quizzes_builder', $quiz_builder_option );
			...
			if ( ( isset( $data['progress_percent'] ) ) && ( 100 == $data['progress_percent'] ) ) {
				...
				$quiz_builder_option            = get_option( 'ebox_settings_quizzes_builder', array() );
		...
		protected function query_items( $increment_paged = true ) {
			...
			if ( ( isset( $this->transient_data['quiz_id'] ) ) && ( ! empty( $this->transient_data['quiz_id'] ) ) ) {
				...
				if ( empty( $pro_quiz_id ) ) {
					$pro_quiz_id = ebox_get_setting( $this->transient_data['quiz_id'], 'quiz_pro' );
					if ( ! empty( $pro_quiz_id ) ) {
				...
				if ( ( isset( $this->transient_data['total_count'] ) ) && ( ! empty( $this->transient_data['total_count'] ) ) ) {
					...
					$rows = $wpdb->get_col( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
						$wpdb->prepare(
							...
							$this->transient_data['paged'] * ebox_LMS_DEFAULT_DATA_UPGRADE_BATCH_SIZE
						)
					);
					if ( ! empty( $rows ) ) {
		...
		/**
		 * Determine if there are mismatched ProQuiz Questions not found as WP Posts (ebox-question).
		 *
		 * @since 2.6.4
		 */
			...
			while ( true ) {
				$pro_ids = $wpdb->get_col( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
					$wpdb->prepare(
						...
						'1',
						ebox_LMS_DEFAULT_DATA_UPGRADE_BATCH_SIZE,
						$pro_ids_paged * ebox_LMS_DEFAULT_DATA_UPGRADE_BATCH_SIZE
					)
				);
				if ( ! empty( $pro_ids ) ) {
				...
				while ( true ) {
					$post_ids = $wpdb->get_col( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
						$wpdb->prepare(
							...
							'question_pro_id',
							ebox_LMS_DEFAULT_DATA_UPGRADE_BATCH_SIZE,
							$post_ids_paged * ebox_LMS_DEFAULT_DATA_UPGRADE_BATCH_SIZE
						)
					);
					if ( ! empty( $post_ids ) ) {
		...
		protected function build_progress_output( $data = array() ) {
			...
			$data['progress_label'] = sprintf(
				...
				esc_html_x( '%1$s: %2$d of %3$d %4$s', 'placeholders: progress status, result count, total count, Questions label', 'ebox' ),
				esc_html( $progress_status ),
				esc_html( $data['result_count'] ),
				esc_html( $data['total_count'] ),
				( $data['total_count'] > 1 ) ? ebox_Custom_Label::get_label( 'questions' ) : ebox_Custom_Label::get_label( 'question' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			);

			if ( ( isset( $data['progress_percent'] ) ) && ( 100 == $data['progress_percent'] ) ) {
				if ( ! empty( $this->transient_data['skipped'] ) ) {
					$skipped_out  = '<div class="ebox-skipped">';
					$skipped_out .= sprintf(
						// translators: placeholder: Number of skipped Questions.
						esc_html_x( 'ProQuiz %1$s Skipped %2$d', 'placeholder: Number of skipped Questions', 'ebox' ),
		...
		protected function convert_proquiz_question( $question_pro_id = 0 ) {
			...
			$quiz_builder_option = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Quizzes_Builder' );
			...
			if ( ( empty( $question_pro_id ) ) || ( ! isset( $this->transient_data['current_user']['user_id'] ) ) || ( $question_pro_id !== $this->transient_data['current_user']['user_id'] ) ) {
				$this->transient_data['skipped'][ $question_pro_id ] = sprintf(
					...
					esc_html_x( '[%1$d] Empty %2$s ProID given.', 'placeholder: ProQuiz Question ID, question', 'ebox' ),
					$question_pro_id,
			...
			if ( ( ! $question_pro ) || ( ! is_a( $question_pro, 'WpProQuiz_Model_Question' ) ) ) {
				$this->transient_data['skipped'][ $question_pro_id ] = sprintf(
					...
					esc_html_x( 'ProQuestion ID [%d] Model not found.', 'placeholder: ProQuiz Question ID', 'ebox' ),
					$question_pro_id
				);
				return true;
			}
			...
			if ( ( isset( $quiz_builder_option['shared_questions'] ) ) && ( 'yes' !== $quiz_builder_option['shared_questions'] ) ) {
				if ( empty( $quiz_pro_id ) ) {
					$this->transient_data['skipped'][ $question_pro_id ] = sprintf(
						...
						esc_html_x( 'ProQuestion ID [%1$d] ProQuiz ID empty [%2$d].', 'placeholder: ProQuiz Question ID, ProQuiz Quiz ID', 'ebox' ),
						$question_pro_id,
						$quiz_pro_id
					);
					return true;
				...
				if ( ( ! $quiz_pro ) || ( ! is_a( $quiz_pro, 'WpProQuiz_Model_Quiz' ) ) ) {
					$this->transient_data['skipped'][ $question_pro_id ] = sprintf(
						// translators: placeholder: ProQuiz Question ID, ProQuiz Quiz ID.
						esc_html_x( 'ProQuestion ID [%1$d] ProQuiz ID Model not found [%2$d].', 'placeholder: ProQuiz Question ID, ProQuiz Quiz ID', 'ebox' ),
						$question_pro_id,
						$quiz_pro_id
					);
					return true;
			...
			$question_insert_post_id = ebox_get_question_post_by_pro_id( $question_pro_id );
			if ( empty( $question_insert_post_id ) ) {
				$question_insert_post                 = array();
				$question_insert_post['post_type']    = ebox_get_post_type_slug( 'question' );
			...
			if ( ! empty( $question_insert_post_id ) ) {
				ebox_proquiz_sync_question_fields( $question_insert_post_id, $question_pro );
				...
				if ( is_a( $question_pro, 'WpProQuiz_Model_Question' ) ) {
					...
					if ( ( ! empty( $quiz_pro_id ) ) && ( ! empty( $quiz_post_ids ) ) ) {
						foreach ( $quiz_post_ids as $idx => $quiz_post_id ) {
							ebox_set_quiz_questions_dirty( $quiz_post_id );

							if ( 0 === $idx ) {
								ebox_update_setting( $question_insert_post_id, 'quiz', absint( $quiz_post_id ) );
								$quiz_primary_post_id = ebox_get_quiz_primary_shared( $quiz_pro_id, false );
...
add_action(
	...
	function() {
		ebox_Admin_Data_Upgrades_Quiz_Questions::add_instance();
	}
);
