<?php
/**
 * ebox Data Upgrades for WPProQuiz DB Table rename.
 *
 * @since 2.6.0
...
if ( ( class_exists( 'ebox_Admin_Data_Upgrades' ) ) && ( ! class_exists( 'ebox_Admin_Data_Upgrades_Rename_WPProQuiz_Tables' ) ) ) {

	/**
	 *  Class ebox Data Upgrades for WPProQuiz DB Table rename.
	 *
	 * @since 2.6.0
	 * @uses ebox_Admin_Data_Upgrades
	 */
	class ebox_Admin_Data_Upgrades_Rename_WPProQuiz_Tables extends ebox_Admin_Data_Upgrades {
			...
			if ( ! isset( $this->settings['prefixes'] ) ) {
				...
				if ( ( empty( $ld_prior_version ) ) || ( '0.0.0.0' === $ld_prior_version ) || ( 'new' === $ld_prior_version ) ) {
					...
					$this->settings['prefixes'] = array(
						'current' => ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT,
						'alt'     => ebox_LMS_DATABASE_PREFIX_SUB,
					);
					...
					if ( ( isset( $legacy_proquiz_tables['quiz_master']['exists'] ) ) && ( true === $legacy_proquiz_tables['quiz_master']['exists'] ) ) {
						...
						$this->settings['prefixes']     = array(
							'current' => ebox_LMS_DATABASE_PREFIX_SUB,
							'alt'     => ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT,
						);
					}
		...
		public function show_upgrade_action() {
			 ...
			 */
			if ( true !== apply_filters( 'ebox_show_upgrade_section', $this->settings['show_upgrade'], $this->data_slug ) ) {
				return;
			}
			...
			<tr id="ebox-data-upgrades-container-<?php echo esc_attr( $this->data_slug ); ?>" class="ebox-data-upgrades-container">
				<td class="ebox-data-upgrades-button-container">
					...
					<button style="display:none" class="ebox-data-upgrades-button-<?php echo esc_attr( $this->data_slug ); ?>-reload button button-secondary">
					<?php
						esc_html_e( 'Reload', 'ebox' );
					?>
					</button>
				</td>
				<td class="ebox-data-upgrades-status-container">
					...
					<?php
						esc_html_e( 'Rename WPProQuiz DB Tables', 'ebox' );
					?>
					</span>
					<p>
					<?php
						echo wp_kses_post(
							__( 'This upgrade will rename the existing WPProQuiz database tables to a new name inline with ebox standards.<br /><strong>It is recommended you set your site to maintenance mode before performing this upgrade.</strong>', 'ebox' )
						);
					?>
					</p>
					<p id="ebox-data-upgrades-<?php echo esc_attr( $this->data_slug ); ?>-prefix" class="ebox-data-upgrades-prefix">
						<input type="radio" id="ebox-data-upgrades-prefix-wp" name="ebox-data-upgrades-prefix" data-current-prefix="<?php echo esc_attr( $this->settings['prefixes']['current'] ); ?>" value="<?php echo esc_attr( ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT ); ?>" <?php checked( $this->settings['prefixes']['current'], ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT ); ?> />
						<label
						<?php
						if ( ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT === $this->settings['prefixes']['current'] ) {
							echo ' style=" font-weight: bold; " ';
						}
						?>
						for="ebox-data-upgrades-prefix-wp">
						<?php
						// translators: placeholders: default prefix, example table name.
						echo sprintf( esc_html_x( '%1$s - Legacy ProQuiz table prefix. Ex. %2$s', 'placeholders: default prefix, example table name', 'ebox' ), 'wp_', '<code>' . esc_html( $wpdb->prefix . ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT ) . 'pro_quiz_question</code>' );
						?>
						</label><br />

						<input type="radio" id="ebox-data-upgrades-prefix-ebox" name="ebox-data-upgrades-prefix" data-current-prefix="<?php echo esc_attr( $this->settings['prefixes']['current'] ); ?>" value="<?php echo esc_attr( ebox_LMS_DATABASE_PREFIX_SUB ); ?>" <?php checked( $this->settings['prefixes']['current'], ebox_LMS_DATABASE_PREFIX_SUB ); ?> />
						<label
						<?php
						if ( ebox_LMS_DATABASE_PREFIX_SUB === $this->settings['prefixes']['current'] ) {
							echo ' style=" font-weight: bold; " ';
						}
						?>
						for="ebox-data-upgrades-prefix-ebox">
						<?php
						// translators: placeholders: ebox prefix, example table name.
						echo sprintf( esc_html_x( '%1$s - ebox ProQuiz table prefix. Ex. %2$s', 'placeholders: ebox prefix, example table name', 'ebox' ), 'ebox_', '<code>' . esc_html( $wpdb->prefix . ebox_LMS_DATABASE_PREFIX_SUB ) . 'pro_quiz_question</code>' );
						?>
						</label>
					</p>
					<p><a href="#" class="ebox-data-upgrades-show-tables"><?php esc_html_e( 'show tables', 'ebox' ); ?></a></p>

					<div id="ebox-data-upgrades-<?php echo esc_attr( $this->data_slug ); ?>-show-tables-list" style="display:none;">

						<table id="tables-list-<?php echo esc_attr( $this->settings['prefixes']['alt'] ); ?>">
						<thead>
							<tr>
								<th><?php esc_html_e( 'Source Tables', 'ebox' ); ?></th>
								<th></th>
								<th><?php esc_html_e( 'Destination Tables', 'ebox' ); ?></th>
							</tr>
						</thead>
						<tbody>
						<?php
		...
		public function process_upgrade_action( $data = array() ) {
			...
			if ( ( isset( $data['nonce'] ) ) && ( ! empty( $data['nonce'] ) ) ) {
				if ( wp_verify_nonce( $data['nonce'], 'ebox-data-upgrades-' . $this->data_slug . '-' . get_current_user_id() ) ) {
					$this->init_settings();
			...
			if ( ( isset( $data['progress_percent'] ) ) && ( 100 == $data['progress_percent'] ) ) {
				...
				if ( ebox_LMS_DATABASE_PREFIX_SUB === $this->transient_data['proquiz_prefix'] ) {
					$data['prefixes'] = array(
						'current' => ebox_LMS_DATABASE_PREFIX_SUB,
						'alt'     => ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT,
					);

				} elseif ( ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT === $this->transient_data['proquiz_prefix'] ) {
					$data['prefixes'] = array(
						'current' => ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT,
						'alt'     => ebox_LMS_DATABASE_PREFIX_SUB,
					);
		...
		protected function query_items() {
			...
			if ( ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT === $this->transient_data['proquiz_prefix'] ) {
				$dest_prefix   = ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT;
				$source_prefix = ebox_LMS_DATABASE_PREFIX_SUB;
			} elseif ( ebox_LMS_DATABASE_PREFIX_SUB === $this->transient_data['proquiz_prefix'] ) {
				$dest_prefix   = ebox_LMS_DATABASE_PREFIX_SUB;
				$source_prefix = ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT;
			}
		...
		protected function build_progress_output( $data = array() ) {
			...
			$data['progress_label'] = sprintf(
				...
				esc_html_x( '%1$s: %2$d of %3$d ProQuiz Table', 'placeholders: progress status, result count, total count', 'ebox' ),
				$progress_status,
				$data['result_count'],
				$data['total_count']
			);
...
add_action(
	...
	function() {
		ebox_Admin_Data_Upgrades_Rename_WPProQuiz_Tables::add_instance();
	}
);
