...
if ( ! class_exists( 'ebox_Admin_Settings_Data_Reports' ) ) {
	/**
	 * ebox Reports Base Class.
	 *
	 * @since 2.3.0
	 */
	class ebox_Admin_Settings_Data_Reports {
		...
		public function __construct() {
			...
			$this->parent_menu_page_url  = 'admin.php?page=ebox-lms-reports';
			$this->menu_page_capability  = ebox_ADMIN_CAPABILITY_CHECK;
			$this->settings_page_id      = 'ebox-lms-reports';
			$this->settings_page_title   = esc_html_x( 'Reports', 'ebox Report Menu Label', 'ebox' );
			...
			if ( ! defined( 'ebox_PROCESS_TIME_PERCENT' ) ) {
				...
				define( 'ebox_PROCESS_TIME_PERCENT', apply_filters( 'ebox_process_time_percent', 80 ) );
			}

			if ( ! defined( 'ebox_PROCESS_TIME_SECONDS' ) ) {
				/** This filter is documented in includes/admin/class-ebox-admin-data-upgrades.php */
				define( 'ebox_PROCESS_TIME_SECONDS', apply_filters( 'ebox_process_time_seconds', 10 ) );
			}
		...
		public function init_check_for_download_request() {
			if ( isset( $_GET['ld-report-download'] ) ) {
				...
				if ( ( isset( $_GET['data-nonce'] ) ) && ( ! empty( $_GET['data-nonce'] ) ) && ( isset( $_GET['data-slug'] ) ) && ( ! empty( $_GET['data-slug'] ) ) ) {
					...
					if ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['data-nonce'] ) ), 'ebox-data-reports-' . sanitize_text_field( wp_unslash( $_GET['data-slug'] ) ) . '-' . get_current_user_id() ) ) {
						...
						if ( ( isset( $transient_data['report_filename'] ) ) && ( ! empty( $transient_data['report_filename'] ) ) ) {
							...
							if ( ( file_exists( $report_filename ) ) && ( is_readable( $report_filename ) ) ) {
								 ...
								 */
								$http_headers = apply_filters( 'ebox_csv_download_headers', $http_headers, $transient_data, sanitize_text_field( wp_unslash( $_GET['data-slug'] ) ) );
								 ...
								 * @since 2.4.7
								 */
								do_action( 'ebox_csv_download_after_headers' );
		 ...
		 * @param object $ld_admin_tabs      ebox Admin Tabs instance.
		 */
		...
		public function on_load_panel() {
			...
			wp_enqueue_script(
				'ebox-admin-settings-data-reports-script',
				ebox_LMS_PLUGIN_URL . 'assets/js/ebox-admin-settings-data-reports' . ebox_min_asset() . '.js',
				array( 'jquery' ),
				ebox_SCRIPT_VERSION_TOKEN,
				true
			);
			$ebox_assets_loaded['scripts']['ebox-admin-settings-data-reports-script'] = __FUNCTION__;
		...
		public function init_report_actions() {
			 ...
			 */
			$this->report_actions = apply_filters( 'ebox_admin_report_register_actions', $this->report_actions );
		}
		...
		public function admin_page() {
			 ...
			 * @since 3.0.0
			 */
			do_action( 'ebox_settings_page_before_content' );
			?>
			<div id="ebox-settings" class="wrap ebox-settings-page-wrap">
				<h1 class="ebox-empty-page-title"></h1>
				<form method="post" action="options.php">
					<div id="poststuff">
						<div id="advanced-sortables" class="meta-box-sortables">
							<div id="ebox-courses_metabox" class="postbox ld_settings_postbox ebox-settings-postbox">
								<div class="postbox-header">
									<h2 class="hndle ui-sortable-handle"><?php esc_html_e( 'User Reports', 'ebox' ); ?></h2>
								</div>
								<div class="inside">
									<div class="ebox ebox_options ebox-courses_settings">
										<table id="ebox-data-reports" class="wc_status_table widefat" cellspacing="0">
										<?php
										wp_nonce_field( 'ebox-data-reports-nonce-' . get_current_user_id(), 'ebox-data-reports-nonce' );
		...
		public function get_transient( $transient_key = '' ) {
			...
			if ( ! empty( $transient_key ) ) {
				...
				$options_key   = 'ebox_reports_' . $transient_key;

				if ( ( defined( 'ebox_TRANSIENT_CACHE_STORAGE' ) ) && ( 'file' === ebox_TRANSIENT_CACHE_STORAGE ) ) { // @phpstan-ignore-line
					$wp_upload_dir = wp_upload_dir();

					$ld_file_part = '/ebox/cache/ebox_reports_data_' . $transient_key . '.txt';
...
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-reports-actions/class-ebox-admin-data-reports-user-courses.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-reports-actions/class-ebox-admin-data-reports-user-quizzes.php';

add_action(
	'plugins_loaded',
	function() {
		new ebox_Admin_Data_Reports_Courses();
		new ebox_Admin_Data_Reports_Quizzes();
	}
);
 ...
 * @since 2.3.0
 */
function ebox_data_reports_ajax() {
	$reply_data = array( 'status' => false );

	if ( current_user_can( 'read' ) ) {
		if ( ( isset( $_POST['nonce'] ) ) && ( ! empty( $_POST['nonce'] ) ) && ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ebox-data-reports-nonce-' . get_current_user_id() ) ) ) {
...
add_action( 'wp_ajax_ebox-data-reports', 'ebox_data_reports_ajax' );
