...
if ( ! class_exists( 'ebox_Admin_Data_Upgrades' ) ) {

	/**
	 * Class ebox Data Upgrades Base.
	 *
	 * @since 2.6.0
	 */
	class ebox_Admin_Data_Upgrades {
		...
		protected function __construct() {
			 ...
			 */
			$process_time_percent = apply_filters( 'ebox_process_time_percent', 80 );
			if ( ! defined( 'ebox_PROCESS_TIME_PERCENT' ) ) {
				/**
				 * Define ebox LMS - Set the processing time (percent) for Data
				 ...
				 */
				define( 'ebox_PROCESS_TIME_PERCENT', $process_time_percent );
			}
			 ...
			 */
			$process_time_seconds = apply_filters( 'ebox_process_time_seconds', 10 );
			if ( ! defined( 'ebox_PROCESS_TIME_SECONDS' ) ) {
				/**
				 * Define ebox LMS - Set the processing time (seconds) for Data
				 * Upgrade and Report Export processing.
				 *
				 * During the Data Upgrade or Reporting processing there is a series of AJAX
				 * calls made. This define controls how long the AJAX call can run before
				 ...
				 */
				define( 'ebox_PROCESS_TIME_SECONDS', $process_time_seconds );
			}
		}
		...
		/**
		 * Initialize the ebox Settings array
		 *
		 * @since 2.6.0
		 *
		...
		private function init_data_settings( $force_reload = false ) {
			...
			if ( ( true !== $this->data_settings_loaded ) || ( true === $force_reload ) ) {
				...
				$this->data_settings = get_option( 'ebox_data_settings', array() );
				...
				if ( empty( $this->data_settings['prior_version'] ) ) {
					if ( get_option( 'ebox_quiz_migration_completed' ) ) {
				...
				if ( version_compare( ebox_VERSION, $this->data_settings['current_version'], 'ne' ) ) {
					...
					$this->data_settings['current_version']           = ebox_VERSION;
					$this->data_settings['version_history'][ time() ] = ebox_VERSION;
					$data_settings_changed                            = true;
				}
				...
				if ( true === $data_settings_changed ) {
					...
					update_option( 'ebox_data_settings', $this->data_settings );
				}
			}
		}

		/**
		 * Get the ebox Settings array
		 *
		 * @since 2.6.0
		 *
		...
		public function set_data_settings( $key = '', $value = '' ) {
			...
			return update_option( 'ebox_data_settings', $this->data_settings );
		}
		...
		public function show_upgrade_admin_notice() {
			if ( true !== self::$admin_notice_shown ) {
				...
				$admin_notice_message = sprintf(
					// translators: placeholder: link to ebox Data Upgrade admin page.
					esc_html_x( 'ebox Notice: Please perform a %s. This is a required step to ensure accurate reporting.', 'placeholder: link to ebox Data Upgrade admin page', 'ebox' ),
					'<a href="' . esc_url( admin_url( 'admin.php?page=ebox_data_upgrades' ) ) . '">' . esc_html__( 'ebox Data Upgrade', 'ebox' ) . '</a>'
				);
				?>
		...
		public function check_upgrade_admin_notice() {
			...
			if ( ( isset( $this->data_settings['user-meta-courses']['version'] ) ) && ( $this->data_settings['user-meta-courses']['version'] < ebox_SETTINGS_TRIGGER_UPGRADE_VERSION ) ) {
				$show_admin_notice = true;
			}

			if ( ( isset( $this->data_settings['user-meta-quizzes']['version'] ) ) && ( $this->data_settings['user-meta-quizzes']['version'] < ebox_SETTINGS_TRIGGER_UPGRADE_VERSION ) ) {
				$show_admin_notice = true;
			}
		...
		public function admin_page() {
			$banner_message = esc_html__( 'The Data Upgrades should only be run if prompted or advised by ebox Support. There is no need to re-run the Data Upgrades every time you update ebox core or one of the add-ons. Re-running the data upgrades when not needed can result in data corruption.', 'ebox' );
			...
			?>
			<table id="ebox-data-upgrades" class="wc_status_table widefat" cellspacing="0">
			<?php
			wp_nonce_field( 'ebox-data-upgrades-nonce-' . get_current_user_id(), 'ebox-data-upgrades-nonce' );
		...
		public function set_last_run_info( $data = array() ) {
			$data_settings = array_merge(
				array(
					...
					'version'  => ebox_SETTINGS_TRIGGER_UPGRADE_VERSION,
				),
				$data
			);
		...
		public function get_last_run_info() {
			...
			$last_run_info = esc_html__( 'Last run: none', 'ebox' );
				...
				if ( isset( $data_settings['user_id'] ) ) {
					...
					if ( ( $user ) && ( is_a( $user, 'WP_User' ) ) ) {
						$last_run_info = sprintf(
							...
							_x( 'Last run: %1$s by %2$s', 'placeholders: date/time, user name', 'ebox' ),
							ebox_adjust_date_time_display( $data_settings['last_run'] ),
							$user->display_name
						);
					}
				}
		...
		protected function out_of_timer() {
			...
			if ( ( $this->process_times['percent'] >= ebox_PROCESS_TIME_PERCENT ) || ( $this->process_times['ticks'] > ebox_PROCESS_TIME_SECONDS ) ) {
				return true;
			}
		...
		protected function get_transient( $transient_key = '' ) {
			...
			if ( ! empty( $transient_key ) ) {
				...
				if ( ( defined( 'ebox_TRANSIENT_CACHE_STORAGE' ) ) && ( 'file' === ebox_TRANSIENT_CACHE_STORAGE ) ) { // @phpstan-ignore-line
					$wp_upload_dir = wp_upload_dir();

					$ld_file_part = '/ebox/cache/ebox_data_upgrade_' . $options_key . '.txt';
					...
					if ( ! file_exists( dirname( $ld_transient_filename ) ) ) {
						if ( wp_mkdir_p( dirname( $ld_transient_filename ) ) === false ) {
							$data['error_message'] = esc_html__( 'ERROR: Cannot create working folder. Check that the parent folder is writable', 'ebox' ) . ' ' . dirname( $ld_transient_filename );
							return;
						}
					}

					ebox_put_directory_index_file( trailingslashit( dirname( $ld_transient_filename ) ) . 'index.php' );
...
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-translations.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-team-leader-role.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-course-post-meta.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-team-post-meta.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-quiz-post-meta.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-user-activity-db-table.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-user-meta-courses.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-user-meta-quizzes.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-quiz-questions.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-course-access-list-convert.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-data-upgrades-actions/class-ebox-admin-data-upgrades-rename_wpproquiz-tables.php';
 ...
 * @since 2.6.0
 */
do_action( 'ebox_data_upgrades_init' );
 ...
 * @return void
 */
function ebox_data_upgrades_ajax() {

	$reply_data = array( 'status' => false );

	if ( ( is_user_logged_in() ) && ( ebox_is_admin_user() ) ) {
		if ( ( isset( $_POST['nonce'] ) ) && ( ! empty( $_POST['nonce'] ) ) && ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ebox-data-upgrades-nonce-' . get_current_user_id() ) ) ) {
...
add_action( 'wp_ajax_ebox-data-upgrades', 'ebox_data_upgrades_ajax' );
 ...
 *
 * @since 2.6.0
 * @since 3.3.0 Renamed to ebox_is_data_upgrade_quiz_questions_updated
 * @return boolean true if has been run.
 */
function ebox_is_data_upgrade_quiz_questions_updated() {
	$data_settings_quiz_questions = ebox_data_upgrades_setting( 'pro-quiz-questions' );
 ...
 */
function ebox_data_upgrades_setting( $settings_key = '' ) {
	$element = ebox_Admin_Data_Upgrades::get_instance();
	if ( ( $element ) && ( is_a( $element, 'ebox_Admin_Data_Upgrades' ) ) ) {
