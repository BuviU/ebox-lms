...
if ( ! class_exists( 'ebox_Admin_File_Download_Handler' ) ) {
	/**
	 * Class ebox Admin file download handler.
	 *
	 * @since 4.3.0.1
	 */
	class ebox_Admin_File_Download_Handler {
		const LD_FILE_DOWNLOAD_ACTION = 'ebox_file_download';
		...
		public static function get_download_url( string $file_path_id, string $file_name ): string {
			if ( ! isset( self::$file_paths[ $file_path_id ] ) ) {
				...
				throw new InvalidArgumentException( sprintf( __( 'File path "%s" is not registered', 'ebox' ), $file_path_id ) );
			}
		...
		public static function try_to_protect_file_path( string $file_path ): string {
			...
			return $protect_instructions . '<br/><br/>' . sprintf(
				...
				esc_html__( 'For further details, please read this help document: %s.', 'ebox' ),
				'<a target="_blank" href="https://ebox.com/support/developers/protecting-files/">' .
				esc_html__( 'Protecting files', 'ebox' ) .
				'</a>'
			);
		}
			...
			add_action(
				...
				function() {
					...
					if ( ! wp_verify_nonce( $nonce, self::LD_FILE_DOWNLOAD_ACTION . $file_path_id . $file_name ) ) {
						echo esc_html__( 'URL expired. Please refresh the page and try it again.', 'ebox' );
						exit;
					}

					if ( ! isset( self::$file_paths[ $file_path_id ] ) ) {
						echo esc_html__( 'Invalid URL.', 'ebox' );
						exit;
					}

					// checks permissions.
					if ( ! ebox_is_admin_user() ) {
						echo esc_html__( 'You do not have sufficient permissions to download this file.', 'ebox' );
						exit;
					}

					$file_path = self::$file_paths[ $file_path_id ] . DIRECTORY_SEPARATOR . $file_name;
					if ( ! file_exists( $file_path ) ) {
						echo esc_html__( 'File does not exist.', 'ebox' );
						exit;
					}
