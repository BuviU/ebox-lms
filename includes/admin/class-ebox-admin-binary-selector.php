...
if ( ! class_exists( 'ebox_Binary_Selector' ) ) {

	/**
	 * Class for ebox Binary Selector.
	 *
	 * @since 2.1.2
	 */
	class ebox_Binary_Selector {
		...
		protected static $allowed_classes = array(
			'ebox_Binary_Selector_Users',
			'ebox_Binary_Selector_Course_Users',
			'ebox_Binary_Selector_Team_Users',
			'ebox_Binary_Selector_Team_Leaders',
			'ebox_Binary_Selector_Posts',
			'ebox_Binary_Selector_Team_Courses',
			'ebox_Binary_Selector_Team_Courses_Enroll',
			'ebox_Binary_Selector_Course_Teams',
			'ebox_Binary_Selector_User_Courses',
			'ebox_Binary_Selector_User_Teams',
			'ebox_Binary_Selector_Leader_Teams',
			'ebox_Binary_Selector_Exam_Challenge_Courses',
		);
		...
		public function __construct( $args = array() ) {
			...
			$this->defaults = array(
				...
				'min_height'         => '250px',
				'lazy_load'          => false,
				'search_label_left'  => esc_html__( 'Search:', 'ebox' ),
				'search_label_right' => esc_html__( 'Search:', 'ebox' ),
				'is_search'          => false,
				'is_pager'           => false,
			);
			 ...
			 */
			$this->args = apply_filters( 'ebox_binary_selector_args', $this->args, $this->selector_class );
		...
		public function show() {
			...
			?>
			<div id="<?php echo esc_attr( $this->args['html_id'] ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'ebox-binary-selector-' . get_current_user_id() ) ); ?>" class="<?php echo esc_attr( $this->args['html_class'] ); ?> ebox-binary-selector" data="<?php echo htmlspecialchars( wp_json_encode( $element_data ) ); ?>"><?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				<input type="hidden" class="ebox-binary-selector-form-element" name="<?php echo esc_attr( $this->args['html_name'] ); ?>" value="<?php echo htmlspecialchars( wp_json_encode( $this->args['selected_ids'], JSON_FORCE_OBJECT ) ); ?>"/><?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				...
				<input type="hidden" name="<?php echo esc_attr( $this->args['html_id'] ); ?>-changed" class="ebox-binary-selector-form-changed" value="" />

				<?php $this->show_selections_title(); ?>
				<table class="ebox-binary-selector-table">
				<tr>
					<?php
				...
				if ( ( isset( $this->args['max_height'] ) ) && ( ! empty( $this->args['max_height'] ) ) ) {
					?>
					<style>
					.ebox-binary-selector .ebox-binary-selector-section .ebox-binary-selector-items {
		...
		protected function show_selections_section_controls() {
			?>
			<td class="ebox-binary-selector-section ebox-binary-selector-section-middle">
				<a href="#" class="ebox-binary-selector-button-add">
				<?php if ( is_rtl() ) { ?>
					<img src="<?php echo esc_url( ebox_LMS_PLUGIN_URL . 'assets/images/arrow_left.png' ); ?>" />
				<?php } else { ?>
					<img src="<?php echo esc_url( ebox_LMS_PLUGIN_URL . 'assets/images/arrow_right.png' ); ?>" />
				<?php } ?>
				</a><br>

				<a href="#" class="ebox-binary-selector-button-remove">
				<?php if ( is_rtl() ) { ?>
					<img src="<?php echo esc_url( ebox_LMS_PLUGIN_URL . 'assets/images/arrow_right.png' ); ?>" />
				<?php } else { ?>
					<img src="<?php echo esc_url( ebox_LMS_PLUGIN_URL . 'assets/images/arrow_left.png' ); ?>" />
		...
		protected function show_selections_section( $position = '' ) {
			...
			if ( ( 'left' === $position ) || ( 'right' === $position ) ) {
				?>
				<td class="ebox-binary-selector-section ebox-binary-selector-section-<?php echo esc_attr( $position ); ?>">
					<input placeholder="<?php echo esc_attr( $this->get_search_label( $position ) ); ?>" type="text" id="ebox-binary-selector-search-<?php echo esc_attr( $this->args['html_slug'] ); ?>-<?php echo esc_attr( $position ); ?>" class="ebox-binary-selector-search ebox-binary-selector-search-<?php echo esc_attr( $position ); ?>" />

					<select multiple="multiple" class="ebox-binary-selector-items ebox-binary-selector-items-<?php echo esc_attr( $position ); ?>">
						<?php $this->show_selections_section_items( $position ); ?>
					</select>

					<ul class="ebox-binary-selector-pager ebox-binary-selector-pager-<?php echo esc_attr( $position ); ?>">
		...
		protected function show_selections_section_pager( $position = '' ) {
			if ( $this->is_valid_position( $position ) ) {
				?>
				<li class="ebox-binary-selector-pager-prev"><a class="ebox-binary-selector-pager-prev" style="display:none;" href="#"><?php esc_html_e( '&lsaquo; prev', 'ebox' ); ?></a></li>
				<li class="ebox-binary-selector-pager-info" style="display:none;">
				<?php
				echo sprintf(
					// translators: placeholder: Page X of Y.
					esc_html_x( 'Page %1$s of %2$s', 'placeholder: Page X of Y', 'ebox' ),
					'<span class="current_page"></span>',
					'<span class="total_pages"></span>'
				);
				?>
				</li>
				<li class="ebox-binary-selector-pager-next"><a class="ebox-binary-selector-pager-next" style="display:none;" href="#"><?php esc_html_e( 'next  &rsaquo;', 'ebox' ); ?></a></li>
				<?php
			}
		}
		 ...
		 *
		 * Used by the ebox_binary_selector_pager_ajax() function.
		 *
		 * @since 3.2.0
		 *
...
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-users.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-course-users.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-team-users.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-team-leaders.php';

require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-posts.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-team-courses.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-team-courses-enroll.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-course-teams.php';

require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-user-courses.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-user-teams.php';
require_once ebox_LMS_PLUGIN_DIR . 'includes/admin/classes-binary-selectors/class-ebox-admin-binary-selector-leader-teams.php';
 ...
 * @since 2.1.2
 */
function ebox_binary_selector_pager_ajax() {

	$reply_data = array( 'status' => false );

	if ( ( is_user_logged_in() ) && ( isset( $_POST['nonce'] ) ) && ( ! empty( $_POST['nonce'] ) ) && ( wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'ebox-binary-selector-' . get_current_user_id() ) ) ) {
		if ( ( isset( $_POST['query_data'] ) ) && ( ! empty( $_POST['query_data'] ) ) ) {
			if ( ( isset( $_POST['query_data']['query_vars'] ) ) && ( ! empty( $_POST['query_data']['query_vars'] ) ) ) {
				...
				if ( isset( $_POST['query_data']['selector_class'] ) ) {
					...
					if ( ( ebox_Binary_Selector::check_class( $bs_class ) ) && ( is_subclass_of( $bs_class, 'ebox_Binary_Selector' ) ) ) {
...
add_action( 'wp_ajax_ebox_binary_selector_pager', 'ebox_binary_selector_pager_ajax' );
