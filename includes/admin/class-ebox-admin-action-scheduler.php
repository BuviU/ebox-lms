<?php
/**
 * ebox Admin Action Scheduler class.
 *
 * @since 4.2.0
 *
 * @package ebox\Scheduler
 */
...
if ( ! class_exists( 'ebox_Admin_Action_Scheduler' ) ) {
	/**
	 * ebox admin action scheduler class.
	 *
	 ...
	 * @since 4.2.0
	 */
	class ebox_Admin_Action_Scheduler {
		const SCHEDULER_NOTICES_OPTION_KEY     = 'ebox_scheduler_notices';
		const SCHEDULER_FATAL_TASKS_OPTION_KEY = 'ebox_scheduler_fatal_tasks';
		const LD_GROUP_NAME_PREFIX             = 'ebox';
		const SCHEDULER_TIMEOUT                = 3600; // 1 hour.


		/**
		 * Name of the action scheduler team.
		 *
		 * @var string
		 */
		private $team_name;
		...
		/**
		 * Registered callbacks for that team.
		 *
		 * @var array
		 */
		 ...
		 * Construct.
		 *
		 * @param string $team_name Name of the Scheduler team.
		 */
		public function __construct( string $team_name ) {
			$this->team_name = self::get_full_team_name( $team_name );
			$this->task_handler();
		}
		 ...
		 */
		private static function get_full_team_name( string $team_name ): string {
			return self::LD_GROUP_NAME_PREFIX . '/' . sanitize_key( $team_name );
		}
		...
		public function register_callback( string $task_name, callable $callback, $priority = 10, $accepted_args = 1 ) {
			if ( ! is_callable( $callback ) ) {
				throw new InvalidArgumentException( __( 'Callback is not callable', 'ebox' ) );
			}
		...
		public function enqueue_task( string $task_name, array $task_args, $related_object = 0, string $pending_message = '', string $progress_message = '' ) {
			if ( ! isset( $this->tasks_callbacks[ $task_name ] ) ) {
				...
				throw new InvalidArgumentException( sprintf( __( 'Task "%s" is not registered', 'ebox' ), $task_name ) );
			}

			$task = array(
				'task_team_name'  => $this->team_name,
				...
				function( $action_id ) {
					...
					add_filter(
						...
						function() use ( $action_id ) {
							ebox_Admin_Action_Scheduler::add_fatal_task( $action_id );
		...
		private static function mark_fatal_tasks_as_failed() {
			add_action(
				'init',
				function() {
					$fatal_tasks = get_option( ebox_Admin_Action_Scheduler::SCHEDULER_FATAL_TASKS_OPTION_KEY, array() );
					delete_option( ebox_Admin_Action_Scheduler::SCHEDULER_FATAL_TASKS_OPTION_KEY );
		...
		private static function maybe_run_tasks_immediately() {
			...
			if ( defined( 'ebox_RUN_TASKS_IMMEDIATELY' ) && ebox_RUN_TASKS_IMMEDIATELY ) {
				add_action(
					'admin_init',
					function() {
		 ...
		 */
		public static function is_task_in_progress( string $team_name, string $task_name ): bool {
			$tasks_in_progress = as_get_scheduled_actions(
				array(
					'team'  => self::get_full_team_name( $team_name ),
					'hook'   => $task_name,
					'status' => array(
		...
		private static function show_processing_notices() {
			...
			add_action(
				...
				function() {
					...
					$notices = get_option( ebox_Admin_Action_Scheduler::SCHEDULER_NOTICES_OPTION_KEY, array() );
					delete_option( ebox_Admin_Action_Scheduler::SCHEDULER_NOTICES_OPTION_KEY );
