...
if ( ( ! class_exists( 'ebox_Admin_Metabox_Course_Builder' ) ) && ( class_exists( 'ebox_Admin_Builder' ) ) ) {

	/**
	 * Class ebox Course Builder Metabox.
	 *
	 * @since 2.5.0
	 * @uses ebox_Admin_Builder
	 */
	class ebox_Admin_Metabox_Course_Builder extends ebox_Admin_Builder {
		...
		public function show_builder_box( $post ) {
			if ( ( is_a( $post, 'WP_Post' ) ) && ( $this->builder_post_type === $post->post_type ) ) {
				...
				?>
				<style>
					#ebox_builder_box_wrap .ebox_selectors #ebox-selector-post-listing-ebox-modules:empty::after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Lesson.
							esc_html_x( 'Click the \'+\' to add a new %s', 'placeholder: Lesson', 'ebox' ),
							ebox_Custom_Label::get_label( 'Lesson' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						);
						?>
						";
					}
					#ebox_builder_box_wrap .ebox_selectors #ebox-selector-post-listing-ebox-topic:empty::after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Topic.
							esc_html_x( 'Click the \'+\' to add a new %s', 'placeholder: Topic', 'ebox' ),
							ebox_Custom_Label::get_label( 'Topic' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						);
						?>
						";
					}
					#ebox_builder_box_wrap .ebox_selectors #ebox-selector-post-listing-ebox-quiz:empty::after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Quiz.
							esc_html_x( 'Click the \'+\' to add a new %s', 'placeholder: Quiz', 'ebox' ),
					...
					#ebox_builder_box_wrap .ebox_builder_items .ld-course-builder-lesson-items:empty:after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: modules.
							esc_html_x( 'Drop %s Here', 'placeholder: modules', 'ebox' ),
							ebox_Custom_Label::get_label( 'modules' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						);
						?>
						";
					}
					#ebox_builder_box_wrap .ebox_builder_items .ld-course-builder-topic-items:empty:after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Lesson, Topics.
							esc_html_x( 'Drop %1$s %2$s Here', 'placeholder: Lesson, Topics', 'ebox' ),
							ebox_Custom_Label::get_label( 'lesson' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
					...
					#ebox_builder_box_wrap .ebox_builder_items .ld-course-builder-quiz-items:empty:after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Quizzes.
							esc_html_x( 'Drop Global %s Here', 'placeholder: Quizzes', 'ebox' ),
					...
					#ebox_builder_box_wrap .ebox_builder_items .ld-course-builder-lesson-items .ld-course-builder-quiz-items:empty:after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Lesson, Quizzes.
							esc_html_x( 'Drop %1$s %2$s Here', 'placeholder: Lesson, Quizzes', 'ebox' ),
							ebox_Custom_Label::get_label( 'lesson' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
							ebox_Custom_Label::get_label( 'quizzes' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						);
						?>
					...
					#ebox_builder_box_wrap .ebox_builder_items .ld-course-builder-lesson-items .ld-course-builder-topic-items .ld-course-builder-quiz-items:empty:after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Topic, Quizzes.
							esc_html_x( 'Drop %1$s %2$s Here', 'placeholder: Topic, Quizzes', 'ebox' ),
							ebox_Custom_Label::get_label( 'topic' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
							ebox_Custom_Label::get_label( 'quizzes' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						);
						?>
		...
		public function get_build_items_count() {
			?>
			<span class="ebox_builder_items_total">
			<?php
				printf(
					// translators: placeholder: number of steps.
					esc_html_x( 'Total Steps: %s', 'placeholder: number of steps', 'ebox' ),
					'<span class="ebox_builder_items_total_value">' . intval( $this->ld_course_steps_object->get_steps_count() ) . '</span>'
				);
			?>
			</span>
			<?php
		...
		public function builder_admin_footer() {
			...
			$this->builder_assets[ $this->builder_post_type ]['messages']['ebox_unload_message'] = sprintf(
				// translators: placeholder: Course.
				esc_html_x( 'You have unsaved %s Builder changes. Are you sure you want to leave?', 'placeholder: Course', 'ebox' ),
				ebox_Custom_Label::get_label( $builder_post_type_label ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			);
			...
			foreach ( $this->selector_post_types as $selector_post_type ) {
				...
				if ( is_a( $post_type_object, 'WP_Post_Type' ) ) {
					...
					$this->builder_assets[ $this->builder_post_type ]['messages'][ 'confirm_remove_' . $selector_post_type ] = sprintf(
						...
						esc_html_x( 'Are you sure you want to remove this %1$s from the %2$s? (This will also remove all sub-items)', 'placeholders: will be post type labels like Course, Lesson, Topic', 'ebox' ),
						ebox_Custom_Label::get_label( $this->get_label_for_post_type( $selector_post_type ) ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						ebox_Custom_Label::get_label( $builder_post_type_label ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
					);
					...
					$this->builder_assets[ $this->builder_post_type ]['messages'][ 'confirm_trash_' . $selector_post_type ] = sprintf(
						...
						esc_html_x( 'Are you sure you want to move this %s to Trash?', 'placeholder: will be post type label like Course, Lesson, Topic', 'ebox' ),
						ebox_Custom_Label::get_label( $this->get_label_for_post_type( $selector_post_type ) ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
					);
				}
			}
		...
		public function build_selector_query( $args = array() ) {
			$per_page = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'per_page' );
			if ( empty( $per_page ) ) {
				$per_page = 10;
			}

			$step_post_statuses = ebox_get_step_post_statuses();
			...
			if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) !== 'yes' ) {
				 ...
				 */
				$include_orphaned_steps = apply_filters( 'ebox_course_builder_include_orphaned_steps', true, $args );
			 ...
			 * @param array $args An array of query arguments.
			 */
			return apply_filters( 'ebox_course_builder_selector_args', $args );
		}
		...
		public function build_selector_pages_buttons( $post_type_query ) {
			...
			if ( $post_type_query instanceof WP_Query ) {
				...
				$pager_buttons .= '<button ' . $prev_disabled . ' class="button button-simple first" data-page="' . $first_page . '" title="' . esc_attr__( 'First Page', 'ebox' ) . '">&laquo;</button>';
				$pager_buttons .= '<button ' . $prev_disabled . ' class="button button-simple prev" data-page="' . $prev_page . '" title="' . esc_attr__( 'Previous Page', 'ebox' ) . '">&lsaquo;</button>';
				...
				$pager_buttons .= '<button ' . $next_disabled . ' class="button button-simple next" data-page="' . $next_page . '" title="' . esc_attr__( 'Next Page', 'ebox' ) . '">&rsaquo;</button>';
				$pager_buttons .= '<button ' . $next_disabled . ' class="button button-simple last" data-page="' . $last_page . '" title="' . esc_attr__( 'Last Page', 'ebox' ) . '" >&raquo;</button>';
			}
			...
			if ( $post_type_query instanceof WP_Query ) {
				...
				foreach ( $post_type_query->posts as $p ) {
					$selector_rows[] = array(
						...
						'post_status' => ebox_get_step_post_status_slug( $p ),
		...
		protected function build_selector_row_single( $p = null, $selector_post_type = '' ) {
			...
			$selector_sub_actions .= '<a target="_blank" class="ld-course-builder-action ld-course-builder-action-edit ld-course-builder-action-' . $selector_slug . '-edit dashicons" href="' . $edit_post_link . '"><span class="screen-reader-text">' . sprintf(
				...
				esc_html_x( 'Edit %s Settings (new window)', 'placeholder: will contain post type label', 'ebox' ),
				ebox_Custom_Label::get_label( $selector_slug )
			) . '</span></a>';
			...
			$selector_sub_actions .= '<a target="_blank" class="ld-course-builder-action ld-course-builder-action-view ld-course-builder-action-' . $selector_slug . '-view dashicons" href="' . $view_post_link . '"><span class="screen-reader-text">' . sprintf(
				// translators: placeholder: will contain post type label.
				esc_html_x( 'View %s (new window)', 'placeholder: will contain post type label', 'ebox' ),
				ebox_Custom_Label::get_label( $selector_slug )
			) . '</span></a>';
			...
			if ( current_user_can( 'delete_courses' ) ) {
				...
				$selector_sub_actions .= '<span class="ld-course-builder-action ld-course-builder-action-trash ld-course-builder-action-' . $selector_slug . '-trash dashicons" title="' . sprintf(
					// translators: placeholder: will contain post type label.
					esc_html_x( 'Move %s to Trash', 'placeholder: will contain post type label', 'ebox' ),
					ebox_Custom_Label::get_label( $selector_slug )
				) . '"></span>';
			}
			$selector_sub_actions .= '<span class="ld-course-builder-action ld-course-builder-action-remove ld-course-builder-action-' . $selector_slug . '-remove dashicons" title="' . sprintf(
				...
				esc_html_x( 'Remove %1$s from %2$s', 'placeholders: will contain post type label, Course', 'ebox' ),
				ebox_Custom_Label::get_label( $selector_slug ),
			...
			if ( 'ebox-modules' === $selector_post_type ) {
				...
				$selector_action_expand = '<span class="ld-course-builder-action ld-course-builder-action-show-hide ld-course-builder-action-show ld-course-builder-action-' . $selector_slug . '-show dashicons" title="' . esc_html__( 'Expand/Collapse Section', 'ebox' ) . '"></span>';

			} elseif ( 'ebox-topic' === $selector_post_type ) {
			...
			$selector_row .= '<li id="ld-post-' . $p_id . '" class="ld-course-builder-item ld-course-builder-' . $selector_slug . '-item " data-ld-type="' . $selector_post_type . '" data-ld-id="' . $p_id . '">
				...
				// translators: placeholder: will contain post type label.
					esc_html_x( 'Move %s', 'placeholder: will contain post type label', 'ebox' ),
					ebox_Custom_Label::get_label( $selector_slug )
				) . '"></span>
					...
					<span class="ld-course-builder-title"><span class="ld-course-builder-title-text">' . $p_title . '</span>
						<span class="ld-course-builder-action ld-course-builder-edit-title-pencil dashicons" title="' . esc_html__( 'Edit Title', 'ebox' ) . '" ></span>
						<span class="ld-course-builder-action ld-course-builder-edit-title-ok dashicons" title="' . esc_html__( 'Ok', 'ebox' ) . '" ></span>
						<span class="ld-course-builder-action ld-course-builder-edit-title-cancel dashicons" title="' . esc_html__( 'Cancel', 'ebox' ) . '" ></span>
					</span>
					' . $selector_action_expand . '
				</div>
		 ...
		 * @param string $steps_parent_type Parent post type slug. Default is 'ebox-courses'.
		 *
		 * @return string Steps HTML.
		 */
		protected function process_course_steps( $steps = array(), $steps_parent_type = 'ebox-courses' ) {
			$steps_section_html = '';
				...
				foreach ( $steps as $steps_type => $steps_items ) {
					...
					if ( ! empty( $steps_items ) ) {
						foreach ( $steps_items as $steps_id => $steps_set ) {
							...
							$view_post_link = ebox_get_step_permalink( $steps_id, $this->builder_post_id );

							if ( 'ebox-modules' === $steps_type ) {
								$steps_section_item_html = '<div id="ld-post-' . $steps_id . '" class="ld-course-builder-item ld-course-builder-lesson-item" data-ld-type="' . $steps_type . '" data-ld-id="' . $steps_id . '">
									<div class="ld-course-builder-lesson-header ld-course-builder-header">
										<span class="ld-course-builder-actions">
											...
											// translators: placeholder: Lesson.
											sprintf( esc_html_x( 'Move %s', 'placeholder: Lesson', 'ebox' ), ebox_Custom_Label::get_label( 'Lesson' ) ) . '"></span>
											<span class="ld-course-builder-sub-actions">
												...
												// translators: placeholder: Lesson.
												sprintf( esc_html_x( 'Edit %s Settings (new window)', 'placeholder: Lesson', 'ebox' ), ebox_Custom_Label::get_label( 'Lesson' ) ) . '</span></a>
												...
												// translators: placeholder: Lesson.
												sprintf( esc_html_x( 'View %s (new window)', 'placeholder: Lesson', 'ebox' ), ebox_Custom_Label::get_label( 'Lesson' ) ) . '"</span></a>
												...
												sprintf( esc_html_x( 'Remove %1$s from %2$s', 'placeholders: Lesson, Course', 'ebox' ), ebox_Custom_Label::get_label( 'Lesson' ), ebox_Custom_Label::get_label( 'Course' ) ) . '"></span>
											</span>
										</span>
										...
										<span class="ld-course-builder-action ld-course-builder-action-show-hide ld-course-builder-action-show ld-course-builder-action-lesson-show dashicons" title="' . esc_html__( 'Expand/Collapse Section', 'ebox' ) . '"></span>

									</div>
									<div class="ld-course-builder-sub-items" style="display:none">' . $steps_section_item_html . '</div>
								</div>';
							} elseif ( 'ebox-topic' === $steps_type ) {
								$steps_section_item_html = '<div id="ld-post-' . $steps_id . '" class="ld-course-builder-item ld-course-builder-topic-item" data-ld-type="' . $steps_type . '" data-ld-id="' . $steps_id . '">
									<div class="ld-course-builder-topic-header ld-course-builder-header">
										<span class="ld-course-builder-actions">
											<span class="ld-course-builder-action ld-course-builder-action-move ld-course-builder-action-topic-move dashicons" title="' . esc_html__( 'Move', 'ebox' ) . '"></span>
											<span class="ld-course-builder-sub-actions">
												...
												// translators: placeholder: Topic.
												sprintf( esc_html_x( 'Edit %s Settings (new window)', 'placeholder: Topic', 'ebox' ), ebox_Custom_Label::get_label( 'Topic' ) ) . '" ></span></a>
												...
												// translators: placeholder: Topic.
												sprintf( esc_html_x( 'View %s (new window)', 'placeholder: Topic', 'ebox' ), ebox_Custom_Label::get_label( 'Topic' ) ) . '</span></a>
												...
												// translators: placeholders: Lesson, Course.
												sprintf( esc_html_x( 'Remove %1$s from %2$s', 'placeholders: Lesson, Course', 'ebox' ), ebox_Custom_Label::get_label( 'Topic' ), ebox_Custom_Label::get_label( 'Course' ) ) . '"></span>
											</span>
										</span>
										<span class="ld-course-builder-title"><span class="ld-course-builder-title-text">' . get_the_title( $steps_id ) . '</span>
											<span class="ld-course-builder-action ld-course-builder-edit-title-pencil dashicons" title="' . esc_html__( 'Edit Title', 'ebox' ) . '" ></span>
										...
										<span class="ld-course-builder-action ld-course-builder-action-show-hide ld-course-builder-action-show ld-course-builder-action-topic-show dashicons" title="' . esc_html__( 'Expand/Collapse Section', 'ebox' ) . '"></span>
									</div>
									<div class="ld-course-builder-sub-items" style="display:none">' . $steps_section_item_html . '</div>
								</div>';
							} elseif ( 'ebox-quiz' === $steps_type ) {
								$steps_section_item_html = '<div id="ld-post-' . $steps_id . '" class="ld-course-builder-item ld-course-builder-quiz-item" data-ld-type="' . $steps_type . '" data-ld-id="' . $steps_id . '">
									<div class="ld-course-builder-quiz-header ld-course-builder-header">
										<span class="ld-course-builder-actions">
											<span class="ld-course-builder-action ld-course-builder-action-move ld-course-builder-action-quiz-move dashicons" title="' . esc_html__( 'Move', 'ebox' ) . '"></span>
											<span class="ld-course-builder-sub-actions">
												...
												// translators: placeholder: Topic.
												sprintf( esc_html_x( 'Edit %s Settings (new window)', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'Quiz' ) ) . '" ></span></a>
												...
												sprintf( esc_html_x( 'View %s (new window)', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'Quiz' ) ) . '"></span></a>
												<span class="ld-course-builder-action ld-course-builder-action-remove ld-course-builder-action-quiz-remove dashicons" title="' .
												// translators: placeholders: Lesson, Course.
												sprintf( esc_html_x( 'Remove %1$s from %2$s', 'placeholders: Lesson, Course', 'ebox' ), ebox_Custom_Label::get_label( 'Quiz' ), ebox_Custom_Label::get_label( 'Course' ) ) . '"></span>
											</span>
										</span>
										<span class="ld-course-builder-title"><span class="ld-course-builder-title-text">' . get_the_title( $steps_id ) . '</span>
											<span class="ld-course-builder-action ld-course-builder-edit-title-pencil dashicons" title="' . esc_html__( 'Edit Title', 'ebox' ) . '" ></span>
					...
					if ( 'ebox-courses' === $steps_parent_type ) {
						if ( 'ebox-modules' === $steps_type ) {
					...
					} elseif ( 'ebox-modules' === $steps_parent_type ) {
						if ( 'ebox-topic' === $steps_type ) {
					...
					} elseif ( 'ebox-topic' === $steps_parent_type ) {
						if ( 'ebox-quiz' === $steps_type ) {
		 ...
		 */
		public function ebox_builder_selector_pager( $query_args = array() ) {
		 ...
		 * @param array $query_args array of values for AJAX request.
		 */
		public function ebox_builder_selector_search( $query_args = array() ) {
		 ...
		 * @param array $query_args array of values for AJAX request.
		 */
		public function ebox_builder_selector_step_new( $query_args = array() ) {
			global $wpdb;
			...
			if ( ( isset( $query_args['new_steps'] ) ) && ( ! empty( $query_args['new_steps'] ) ) ) {
				foreach ( $query_args['new_steps'] as $old_step_id => $step_set ) {
					if ( ( isset( $step_set['post_type'] ) ) && ( ! empty( $step_set['post_type'] ) ) && ( false !== in_array( $step_set['post_type'], array( 'ebox-modules', 'ebox-topic', 'ebox-quiz' ), true ) ) ) {
						$post_args = array(
						...
						if ( $new_step_id ) {
							...
							if ( 'ebox-quiz' === $post_args['post_type'] ) {
								...
								ebox_update_setting( $new_step_id, 'quiz_pro', $quiz_pro_id );

								// Set the 'View Statistics on Profile' for the new quiz.
								update_post_meta( $new_step_id, '_viewProfileStatistics', 1 );
							}

							ebox_update_setting( $new_step_id, 'course', absint( $this->builder_post_id ) );
						}
					}
				}
			}
		 ...
		 * @param array $query_args array of values for AJAX request.
		 */
		public function ebox_builder_selector_step_trash( $query_args = array() ) {
			$reply_data = array();
			...
			if ( ( empty( $post_args['post_type'] ) ) || ( empty( $post_args['post_id'] ) ) ) {
				...
				$reply_data['error_message'] = esc_html__( '#1: Invalid post data', 'ebox' );
			} elseif ( in_array( $post_args['post_type'], $this->selector_post_types, true ) === false ) {
				$reply_data['status']        = false;
				$reply_data['error_message'] = esc_html__( '#2: Invalid post data', 'ebox' );
			} else {
		 ...
		 * @param array $query_args array of values for AJAX request.
		 */
		public function ebox_builder_selector_step_title( $query_args = array() ) {
...
add_action(
	'ebox_builders_init',
	function() {
		ebox_Admin_Metabox_Course_Builder::add_instance();
	}
);
