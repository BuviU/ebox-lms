...
if ( ( ! class_exists( 'ebox_Admin_Metabox_Quiz_Builder' ) ) && ( class_exists( 'ebox_Admin_Builder' ) ) ) {

	/**
	 * Class ebox Quiz Builder Metabox.
	 *
	 * @since 2.6.0
	 * @uses ebox_Admin_Builder
	 */
	class ebox_Admin_Metabox_Quiz_Builder extends ebox_Admin_Builder {

		/**
		 * ebox quiz question object
		 *
		 * @var object
		 */
		...
		public function __construct() {
			$this->builder_post_type   = 'ebox-quiz';
		...
		public function show_builder_box( $post ) {
			if ( ( is_a( $post, 'WP_Post' ) ) && ( $this->builder_post_type === $post->post_type ) ) {
				...
				<style>
					#ebox_builder_box_wrap .ebox_selectors #ebox-selector-post-listing-ebox-question:empty::after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Question.
							esc_html_x( 'Click the \'+\' to add a new %s', 'placeholder: Question', 'ebox' ),
							ebox_Custom_Label::get_label( 'question' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						);
						?>
						";
					}
					#ebox_builder_box_wrap .ebox_builder_items .ld-course-builder-question-items:empty::after {
						content: "
						<?php
						echo sprintf(
							// translators: placeholder: Questions.
							esc_html_x( 'Drop %s Here', 'placeholder: Questions', 'ebox' ),
		...
		public function get_build_items_count() {
			...
			<?php
				printf(
					...
					esc_html_x( 'Total %1$s: %2$s', 'placeholder: Questions label, number of questions', 'ebox' ),
					ebox_Custom_Label::get_label( 'questions' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
					'<span class="ebox_builder_items_total_value">' . intval( $this->ld_quiz_questions_object->get_questions_count() ) . '</span>'
				);
			?>
			</span>
			<?php
		...
		public function builder_admin_footer() {
			...
			$this->builder_assets[ $this->builder_post_type ]['messages']['ebox_unload_message'] = sprintf(
				...
				esc_html_x( 'You have unsaved %s Builder changes. Are you sure you want to leave?', 'placeholder: Quiz', 'ebox' ),
			...
			foreach ( $this->selector_post_types as $selector_post_type ) {
				...
				if ( is_a( $post_type_object, 'WP_Post_Type' ) ) {
					...
					$this->builder_assets[ $this->builder_post_type ]['messages'][ 'confirm_remove_' . $selector_post_type ] = sprintf(
						...
						esc_html_x( 'Are you sure you want to remove this %1$s from the %2$s?', 'placeholders: post type labels like Question, second Quiz', 'ebox' ),
					...
					$this->builder_assets[ $this->builder_post_type ]['messages'][ 'confirm_trash_' . $selector_post_type ] = sprintf(
						...
						esc_html_x( 'Are you sure you want to move this %s to Trash?', 'placeholders: post type labels like Question', 'ebox' ),
						ebox_Custom_Label::get_label( $this->get_label_for_post_type( $selector_post_type ) ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
					);
				}
			}
		...
		public function build_selector_query( $args = array() ) {
			$per_page = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'per_page' );
			if ( empty( $per_page ) ) {
				$per_page = 10;
			}
			...
			if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'shared_questions' ) !== 'yes' ) {
				 ...
				 */
				$include_orphaned_questions = apply_filters( 'ebox_quiz_builder_include_orphaned_questions', true, $args );
			 ...
			 * @param array $args An array of query arguments.
			 */
			return apply_filters( 'ebox_quiz_builder_selector_args', $args );
		}
		...
		protected function build_selector_row_single( $p = null, $selector_post_type = '' ) {
			...
			if ( ! empty( $question_pro_id ) ) {
				$question_pro_fields = ebox_get_question_pro_fields( $question_pro_id, array( 'quiz_id', 'points', 'answer_type' ) );
			}

			$question_type_string = '';
			if ( ( ! isset( $question_pro_fields['answer_type'] ) ) || ( empty( $question_pro_fields['answer_type'] ) ) || ( ! isset( $ebox_question_types[ $question_pro_fields['answer_type'] ] ) ) ) {
				$question_pro_fields['answer_type'] = 'single';
			}

			$question_type_string = $ebox_question_types[ $question_pro_fields['answer_type'] ];
			...
			$question_points_string = sprintf( _nx( '(%dpt)', '(%dpts)', $question_points, 'placeholder: Question Points', 'ebox' ), number_format_i18n( $question_points ) );
			...
			$selector_sub_actions .= '<span class="ld-course-builder-action ld-course-builder-action-remove ld-course-builder-action-' . $selector_slug . '-remove dashicons" title="' . sprintf(
				...
				esc_html_x( 'Remove %1$s from %2$s', 'placeholders: Question, Quiz', 'ebox' ),
				ebox_Custom_Label::get_label( $selector_slug ),
		...
		public function show_builder_header_right() {
			...
			?>
			<div class="ebox-header-right">
				<?php
				printf(
					// translators: placeholder: Total of question points.
					esc_html_x( 'Total Points: %s', 'placeholder: Total of question points', 'ebox' ),
					'<span class="ebox_builder_points_total_value">' . esc_html( $total_question_points ) . '</span>'
				);
				?>
			</div>
			<?php
		...
		protected function process_quiz_questions( $questions = array() ) {
			...
			if ( ! empty( $questions ) ) {
				foreach ( $questions as $question_id => $q_pro_id ) {
					...
					$view_post_link = ebox_get_step_permalink( $question_id, $this->builder_post_id );

					$question_pro_fields = ebox_get_question_pro_fields( $q_pro_id, array( 'quiz_id', 'points', 'answer_type' ) );
					...
					$questions_section_item_html = '<div id="ld-post-' . $question_id . '" class="ld-course-builder-item ld-course-builder-question-item" data-ld-type="' . $steps_type . '" data-ld-id="' . $question_id . '">
									<div class="ld-course-builder-quiz-header ld-course-builder-header">
										<span class="ld-course-builder-actions">
											<span class="ld-course-builder-action ld-course-builder-action-move ld-course-builder-action-question-move dashicons" title="' . esc_html__( 'Move', 'ebox' ) . '"></span>
											<span class="ld-course-builder-sub-actions">
												...
												sprintf( esc_html_x( 'Edit %s Settings (new window)', 'placeholder: Topic', 'ebox' ), ebox_Custom_Label::get_label( 'Quiz' ) ) . '" ></span></a>
												<span class="ld-course-builder-action ld-course-builder-action-remove ld-course-builder-action-quiz-remove dashicons" title="' .
												// translators: placeholders: Question, Quiz.
												sprintf( esc_html_x( 'Remove %1$s from %2$s', 'placeholders: Question, Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'question' ), ebox_Custom_Label::get_label( 'quiz' ) ) . '"></span>
											</span>
										</span>
										<span class="ld-course-builder-title">
		...
		public function ebox_builder_selector_step_new( $query_args = array() ) {
			...
			if ( ( isset( $query_args['new_steps'] ) ) && ( ! empty( $query_args['new_steps'] ) ) ) {
				foreach ( $query_args['new_steps'] as $old_step_id => $step_set ) {
					if ( ( isset( $step_set['post_type'] ) ) && ( ! empty( $step_set['post_type'] ) ) && ( false !== in_array( $step_set['post_type'], array( 'ebox-question' ), true ) ) ) {
						$post_args = array(
							'action'       => 'new_step',
						...
						/** This filter is documented in includes/admin/classes-builders/class-ebox-admin-course-builder-metabox.php */
						...
						if ( $new_step_id ) {
							...
							if ( 'ebox-question' === $post_args['post_type'] ) {
								$question_pro_id = ebox_update_pro_question( 0, $post_args );
								if ( ! empty( $question_pro_id ) ) {
									update_post_meta( $new_step_id, 'question_pro_id', absint( $question_pro_id ) );
									ebox_proquiz_sync_question_fields( $new_step_id, $question_pro_id );
								}
							}
							...
							ebox_update_setting( $new_step_id, 'quiz', '0' );
		...
		public function ebox_builder_selector_step_title( $query_args = array() ) {
			...
			if ( ( empty( $post_args['post_title'] ) ) || ( empty( $post_args['post_type'] ) ) || ( empty( $post_args['post_id'] ) ) ) {
				...
				if ( 'ebox-question' === $post_args['post_type'] ) {
					...
					$question_pro_id_new = ebox_update_pro_question( $question_pro_id, $post_args );
					...
					if ( ( ! empty( $question_pro_id_new ) ) && ( $question_pro_id_new !== $question_pro_id ) ) {
						...
						ebox_set_question_quizzes_dirty( $post_args['post_id'] );
					}
				}
			}
...
add_action(
	'ebox_builders_init',
	function() {
		ebox_Admin_Metabox_Quiz_Builder::add_instance();
	}
);
