<?php
/**
 * ebox Admin Import/Export Base File Handler.
 *
 * @since 4.3.0
 *
 * @package ebox
...
if ( ! class_exists( 'ebox_Admin_Import_Export_File_Handler' ) ) {
	/**
	 * Class ebox Admin Import/Export Base File Handler.
	 *
	 * @since 4.3.0
	 */
	abstract class ebox_Admin_Import_Export_File_Handler {
		...
		protected function create_work_directory( string $directory ): void {
			...
			if ( ! empty( $upload_dir['error'] ) ) {
				throw new Exception(
					sprintf(
						...
						__( 'Unable to create the files: %s', 'ebox' ),
						$upload_dir['error']
					)
				);
			}
			...
			if ( ! $path_status ) {
				throw new Exception( __( 'Unable to create the files.', 'ebox' ) );
			}

			ebox_put_directory_index_file( trailingslashit( $base_path ) . 'index.php' );
		}
		...
		protected function create_media_directory(): void {
			...
			if ( ! $path_status ) {
				throw new Exception( __( 'Unable to create the media directory.', 'ebox' ) );
			}
		}
	}
}
