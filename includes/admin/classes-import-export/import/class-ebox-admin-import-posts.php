...
if (
	...
	/**
	 * Class ebox Admin Import Posts.
	 *
	 * @since 4.3.0
	 */
	class ebox_Admin_Import_Posts extends ebox_Admin_Import {
		...
		protected function map_post_data( array $data ): array {
			...
			$post_data['meta_input']  = array(
				ebox_Admin_Import::META_KEY_IMPORTED_FROM_POST_ID => $old_id,
				ebox_Admin_Import::META_KEY_IMPORTED_FROM_USER_ID => $post_data['post_author'],
			);
		...
		protected function update_meta_after_insertion( WP_Post $post ): void {
			...
			foreach ( $this->get_ebox_fields_with_media() as $field ) {
				$field_value = ebox_get_setting( $post, $field );
		...
		protected function map_meta_key( string $meta_key, int $post_id ): string {
			if (
				...
				1 === preg_match( '/ebox_team_users_(\d*)/', $meta_key )
			) {
				$meta_key = "ebox_team_users_$post_id";
			}
		 ...
		 * @param array $terms   Term ids teamed by taxonomies.
		 *
		 * @return void
		 */
			...
			foreach ( $terms as $taxonomy_name => $term_ids ) {
				...
				$new_term_ids = get_terms(
					array(
						...
						'fields'     => 'ids',
						'meta_key'   => ebox_Admin_Import::META_KEY_IMPORTED_FROM_TERM_ID,
						'meta_value' => $term_ids,
						'hide_empty' => false,
					)
				);
