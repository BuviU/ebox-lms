<?php
/**
 * ebox Admin Import Associations Handler.
 *
 * @since   4.3.0
 *
 * @package ebox
...
if ( ! class_exists( 'ebox_Admin_Import_Associations_Handler' ) ) {
	/**
	 * Class ebox Admin Import Associations Handler.
	 *
	 * @since 4.3.0
	 */
	class ebox_Admin_Import_Associations_Handler {
		const META_KEY_QUIZ_PROGRESS = '_ebox-quizzes';
		...
		public function handle(): void {
			...
			$this->old_user_id_new_user_id_hash = ebox_Admin_Import::get_old_user_id_new_user_id_hash();
		 ...
		 * @return void
		 */
		protected function update_teams(): void {
			foreach ( $this->get_imported_post_ids( LDLMS_Post_Types::GROUP ) as $post_id ) {
				...
				ebox_set_teams_users(
					$post_id,
					ebox_get_teams_user_ids( $post_id, true )
				);
			}
		}
		...
		protected function update_courses(): void {
			foreach ( $this->get_imported_post_ids( LDLMS_Post_Types::COURSE ) as $post_id ) {
				...
				ebox_course_set_steps_dirty( $post_id ); // Force recreation of the course steps.
			}
		}
		 ...
		 * @return void
		 */
		protected function update_modules(): void {
		...
		protected function update_coupons(): void {
			foreach ( $this->get_imported_post_ids( LDLMS_Post_Types::COUPON ) as $post_id ) {
				foreach ( ebox_COUPON_ASSOCIATED_FIELDS as $field ) {
					$old_ids = ebox_get_setting( $post_id, $field );
					...
					foreach ( $old_ids as $old_id ) {
						$new_id = ebox_Admin_Import::get_new_post_id_by_old_post_id( $old_id );
					...
					ebox_sync_coupon_associated_metas( $post_id, $field, $new_ids );
					ebox_update_setting( $post_id, $field, $new_ids );
				}
			}
		}
		...
		protected function update_transactions(): void {
			foreach ( $this->get_imported_post_ids( LDLMS_Post_Types::TRANSACTION ) as $post_id ) {
				...
				if ( empty( $old_purchased_post_id ) ) {
					$purchased_post_field  = 'team_id';
					$old_purchased_post_id = get_post_meta( $post_id, $purchased_post_field, true );
				}

				$new_purchased_post_id = (int) ebox_Admin_Import::get_new_post_id_by_old_post_id(
				...
				wp_update_post(
					array(
						...
						'post_parent' => (int) ebox_Admin_Import::get_new_post_id_by_old_post_id(
				...
				delete_post_meta( $post_id, 'team_id' );
			}
		}
			...
			foreach ( $this->old_user_id_new_user_id_hash as $user_id ) {
				...
				foreach ( $quiz_attempts as $quiz_attempt ) {
					...
					foreach ( $keys_with_id as $key_with_id ) {
						$new_post_id = ebox_Admin_Import::get_new_post_id_by_old_post_id(
		...
		protected function get_imported_post_ids( string $post_type_name = '' ): array {
			$args = array(
				...
				'meta_query'  => array(
					array(
						'key'     => ebox_Admin_Import::META_KEY_IMPORTED_FROM_POST_ID,
						'compare' => 'EXISTS',
					),
				),
			);
		...
		private function update_shared_steps_associations( int $post_id ): void {
			...
			foreach ( $shared_metas as $shared_meta ) {
				...
				$new_post_id = ebox_Admin_Import::get_new_post_id_by_old_post_id( $old_post_id );
		...
		protected function update_setting( int $post_id, string $setting ): void {
			$old_id = (int) ebox_get_setting( $post_id, $setting );
			...
			ebox_update_setting(
				$post_id,
				$setting,
				ebox_Admin_Import::get_new_post_id_by_old_post_id( $old_id )
			);
		}
