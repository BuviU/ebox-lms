...
if (
	...
	! class_exists( 'ebox_Admin_Import_Export_Users' )
) {
	/**
	 ...
	 * @since 4.3.0
	 */
	class ebox_Admin_Import_Users extends ebox_Admin_Import {
		use ebox_Admin_Import_Export_Users;

		const META_KEY_COURSE_PROGRESS = '_ebox-course_progress';
		...
		const META_KEYS_COURSE_REGEXP = array(
			...
			'ebox_course_expired_' . self::REGEXP_POST_ID,
			'course_completed_' . self::REGEXP_POST_ID,
			'completed_' . self::REGEXP_POST_ID,
		);

		const META_KEYS_GROUP_REGEXP = array(
			'ebox_team_leaders_' . self::REGEXP_POST_ID,
			'ebox_team_users_' . self::REGEXP_POST_ID,
		);
		...
		protected function map_user_data( array $data ): array {
			...
			$user_data['meta_input'] = array(
				ebox_Admin_Import::META_KEY_IMPORTED_FROM_USER_ID => $old_id,
			);
		...
		protected function map_course_progress_meta_value( array $meta_value ): array {
			...
			foreach ( $meta_value as $course_id => $course_progress ) {
				...
				// Map new ids for modules.
				$modules = array();
				foreach ( $course_progress['modules'] as $lesson_id => $value ) {
				...
				$course_progress['modules'] = $modules;
