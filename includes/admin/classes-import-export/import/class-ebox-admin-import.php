...
if (
	trait_exists( 'ebox_Admin_Import_Export_Utils' ) &&
	! class_exists( 'ebox_Admin_Import' )
) {
	/**
	 * Class ebox Admin Import.
	 *
	 * @since 4.3.0
	 */
	abstract class ebox_Admin_Import {
		use ebox_Admin_Import_Export_Utils;

		const META_KEY_IMPORTED_FROM_POST_ID = 'ebox_imported_from_post_id';
		const META_KEY_IMPORTED_FROM_URL     = 'ebox_imported_from_url';
		const META_KEY_IMPORTED_FROM_USER_ID = 'ebox_imported_from_user_id';
		const META_KEY_IMPORTED_FROM_TERM_ID = 'ebox_imported_from_term_id';

		const TRANSIENT_KEY_STATISTIC_REF_IDS = 'ebox_statistic_ref_ids';
		...
		protected function load_and_decode_file(): array {
			 ...
			 */
			return apply_filters( 'ebox_import_decoded_file_data', $decoded_file_data, $file_path );
		}
