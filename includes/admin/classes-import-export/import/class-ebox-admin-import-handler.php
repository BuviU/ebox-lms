...
if (
	class_exists( 'ebox_Admin_Import_Export_Handler' ) &&
	! class_exists( 'ebox_Admin_Import_Handler' )
) {
	/**
	 * Class ebox Admin Import Handler.
	 *
	 * @since 4.3.0
	 */
	class ebox_Admin_Import_Handler extends ebox_Admin_Import_Export_Handler {
		...
		public function handle(): void {
			 ...
			 * @since 4.3.0
			 */
			do_action( 'ebox_import_task_enqueued' );
		...
		public function handle_action( array $options, string $working_directory = '', int $user_id = 0 ): void {
			...
			try {
				 ...
				 * @since 4.3.0
				 */
				do_action( 'ebox_import_task_handled' );
			}
		}
		...
		protected function clean( string $working_directory ): void {
			...
			delete_metadata(
				'post',
				0,
				ebox_Admin_Import::META_KEY_IMPORTED_FROM_POST_ID,
				null,
				true
			);
			...
			delete_metadata(
				'post',
				0,
				ebox_Admin_Import::META_KEY_IMPORTED_FROM_URL,
				null,
				true
			);
			...
			delete_metadata(
				'post',
				0,
				ebox_Admin_Import::META_KEY_IMPORTED_FROM_USER_ID,
				null,
				true
			);
			...
			delete_metadata(
				'term',
				0,
				ebox_Admin_Import::META_KEY_IMPORTED_FROM_TERM_ID,
				null,
				true
			);

			delete_transient( ebox_Admin_Import::TRANSIENT_KEY_STATISTIC_REF_IDS );
		}
		...
		protected function import( array $options, string $working_directory, int $user_id ): void {
			...
			$importers_mapper = new ebox_Admin_Import_Mapper( $this->file_handler, $this->logger );
			...
			foreach ( $importers_mapper->map( $options, $user_id ) as $importer ) {
				 ...
				 *
				 * @param ebox_Admin_Import $importer The ebox_Admin_Import instance.
				 *
				 * @since 4.3.0
				 */
				do_action( 'ebox_import_importer_processed', $importer );
			}

			( new ebox_Admin_Import_Associations_Handler() )->handle();

			ebox_Admin_Action_Scheduler::add_admin_notice(
				__( 'Import completed successfully.', 'ebox' ),
				'success',
				$this->get_scheduler_action_name()
			);
		}
		...
		protected function validate(): void {
			if (
				...
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Invalid request.', 'ebox' ),
					)
				);
			}
			...
			if (
				...
				$error_message = esc_html__( 'Uploading of your import archive failed.', 'ebox' );
		...
		public function enqueue_import_task( string $file_path ) {
			...
			$options_file_name = ebox_Admin_Export_Configuration::FILE_NAME . ebox_Admin_Import_Export_File_Handler::FILE_EXTENSION;
			...
			if ( false === $import_options ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Invalid import archive. Import configuration file "%s" not found.', 'ebox' ),
						$options_file_name
					)
				);
			}
			...
			for ( $i = 0; $i < $zip_archive->numFiles; $i++ ) {
				...
				if (
					...
					&& ebox_Admin_Import_Export_File_Handler::FILE_EXTENSION === mb_substr( $file_name, - mb_strlen( ebox_Admin_Import_Export_File_Handler::FILE_EXTENSION ) )
			...
			if ( ! $import_files_found ) {
				return new WP_Error(
					...
					sprintf(
						...
						__( 'Invalid import archive. Files with the "%1$s" extension except the configuration file "%2$s" were not found.', 'ebox' ),
						ebox_Admin_Import_Export_File_Handler::FILE_EXTENSION,
						$options_file_name
					)
				);
			}
			...
			try {
				...
				return new WP_Error(
					'ld_import_failed',
					esc_html__( 'Failed to run import.', 'ebox' )
				);
			}
		}
	}
