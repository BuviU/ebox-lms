...
if (
	...
	trait_exists( 'ebox_Admin_Import_Export_Posts' ) &&
	interface_exists( 'ebox_Admin_Export_Has_Media' ) &&
	! class_exists( 'ebox_Admin_Export_Posts' )
) {
	/**
	 * Class ebox Admin Export Posts.
	 *
	 * @since 4.3.0
	 */
	class ebox_Admin_Export_Posts extends ebox_Admin_Export_Chunkable implements ebox_Admin_Export_Has_Media {
		use ebox_Admin_Import_Export_Posts;
		...
		public function get_data(): string {
			 ...
			 */
			$query_args = apply_filters( 'ebox_export_post_query_args', $query_args );
			...
			foreach ( $posts as $post ) {
				 ...
				 */
				$post_data = apply_filters( 'ebox_export_post_object', $post_data );
		...
		public function get_media(): array {
			 ...
			 * @return array Query args.
			 */
			$query_args = apply_filters( 'ebox_export_post_media_query_args', $query_args );
			...
			foreach ( $posts as $post ) {
				 ...
				 */
				$media_ids = apply_filters( 'ebox_export_post_media_ids', $media_ids, $post );
		 ...
		 * @param WP_Post $post The post.
		 *
		 * @return array Term IDs teamed by a taxonomy name.
		 */
		...
		protected function get_excluded_meta_keys( WP_Post $post ): array {
			...
			return apply_filters(
				'ebox_export_post_excluded_meta_keys',
		...
		protected function get_special_media_ids( WP_Post $post ): array {
			$fields = $this->get_ebox_fields_with_media();
