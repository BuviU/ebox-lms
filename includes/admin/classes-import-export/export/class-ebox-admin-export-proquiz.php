...
if (
	...
	trait_exists( 'ebox_Admin_Import_Export_Proquiz' ) &&
	interface_exists( 'ebox_Admin_Export_Has_Media' ) &&
	! class_exists( 'ebox_Admin_Export_Proquiz' )
) {
	/**
	 * Class ebox Admin Export Pro Quiz.
	 *
	 * @since 4.3.0
	 */
	class ebox_Admin_Export_Proquiz extends ebox_Admin_Export_Chunkable implements ebox_Admin_Export_Has_Media {
		use ebox_Admin_Import_Export_Proquiz;
		...
		public function get_data(): string {
			 ...
			 */
			$query_args = apply_filters( 'ebox_export_proquiz_query_args', $query_args );
			...
			foreach ( $post_ids as $post_id ) {
				 ...
				 */
				$proquiz_data = apply_filters( 'ebox_export_proquiz_object', $proquiz_data );
		...
		public function get_media(): array {
			...
			$question_media_fields = $wpdb->get_results(
				$wpdb->prepare(
					...
					FROM {$wpdb->prefix}ebox_pro_quiz_question
		...
		public function map_statistics( int $quiz_id ): array {
			...
			$statistics_teamed_by_ref_id = array();
			foreach ( $statistics as $statistic ) {
				$statistics_teamed_by_ref_id[ $statistic->getStatisticRefId() ][] = $statistic->get_object_as_array();
			}
			...
			foreach ( $statistic_refs as $statistic_ref ) {
				...
				$result[] = array(
					...
					'statistic_models'    => $statistics_teamed_by_ref_id[ $statistic_ref_id ] ?? array(),
				);
			}
