...
if (
	...
	! class_exists( 'ebox_Admin_Export_Handler' )
) {
	/**
	 * Class ebox Admin Export Handler.
	 *
	 * @since 4.3.0
	 */
	class ebox_Admin_Export_Handler extends ebox_Admin_Import_Export_Handler {
		const TYPE_ALL      = 'all';
		const TYPE_SELECTED = 'selected';

		const AJAX_ACTION_NAME      = 'ebox_export';
		const SCHEDULER_ACTION_NAME = 'ebox_export_action';
		...
		public function handle(): void {
			...
			try {
				...
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Failed to run export.', 'ebox' ),
					)
				);
			}
			 ...
			 * @since 4.3.0
			 */
			do_action( 'ebox_export_task_enqueued' );
		...
		public function handle_action( array $options ): void {
			...
			try {
				$exporters_mapper = new ebox_Admin_Export_Mapper( $this->file_handler, $this->logger );
				...
				foreach ( $exporters_mapper->map( $options ) as $exporter ) {
					 ...
					 *
					 * @param ebox_Admin_Export $exporter The ebox_Admin_Export instance.
					 *
					 * @since 4.3.0
					 */
					do_action( 'ebox_export_exporter_processed', $exporter );
				}
				...
				ebox_Admin_Action_Scheduler::add_admin_notice(
					sprintf(
						...
						__( 'Export completed successfully. %1$sDownload file%2$s', 'ebox' ),
				 ...
				 * @since 4.3.0
				 */
				do_action( 'ebox_export_task_handled' );
			}
		}
		...
		protected function map_export_options( string $type ): array {
			 ...
			 */
			return apply_filters( 'ebox_export_options', $export_options );
		}
				...
				if (
					...
					if ( ! $all_required_post_types_are_presented ) {
						wp_send_json_error(
							array(
								'message' => sprintf(
									// translators: placeholder: courses, modules, topics, quizzes.
									esc_html_x(
										'It is not possible to export user progress without %1$s, %2$s, %3$s or %4$s.',
										'placeholder: courses, modules, topics, quizzes',
										'ebox'
									),
									ebox_Custom_Label::label_to_lower( 'courses' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									ebox_Custom_Label::label_to_lower( 'modules' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									ebox_Custom_Label::label_to_lower( 'topics' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									ebox_Custom_Label::label_to_lower( 'quizzes' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								),
							)
						);
					}
					...
					wp_send_json_error(
						array(
							...
							// translators: placeholder: quizzes, questions.
								...
								ebox_Custom_Label::label_to_lower( 'quizzes' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								ebox_Custom_Label::label_to_lower( 'questions' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							),
						)
					);
				}
