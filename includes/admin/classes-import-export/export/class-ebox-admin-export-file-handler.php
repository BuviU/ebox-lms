<?php
/**
 * ebox Admin Export File Handler.
 *
 * @since 4.3.0
 *
 * @package ebox
...
if (
	class_exists( 'ebox_Admin_Import_Export_File_Handler' ) &&
	! class_exists( 'ebox_Admin_Export_File_Handler' )
) {
	/**
	 * Class ebox Admin Export File Handler.
	 *
	 * @since 4.3.0
	 */
	class ebox_Admin_Export_File_Handler extends ebox_Admin_Import_Export_File_Handler {
		const EXPORT_DIRECTORY             = 'ebox' . DIRECTORY_SEPARATOR . 'export';
		const ebox_EXPORT_ZIP_PATH_ID = 'ebox_export_zip';
		...
		public function open( string $file ): void {
			...
			if ( ! $file_stream ) {
				throw new Exception( __( 'Unable to create the files to export.', 'ebox' ) );
			}
		...
		public function add_content( string $data ): void {
			...
			if ( false === $result ) {
				throw new Exception( __( 'Unable to add data to the export file.', 'ebox' ) );
			}
		}
		...
		public function add_media_file( int $media_id ): void {
			...
			if ( ! $result ) {
				throw new Exception( __( 'Unable to copy the media file.', 'ebox' ) );
			}
			 ...
			 */
			$media_data = apply_filters( 'ebox_export_media_object', $media_data );
		...
		public function close(): void {
			...
			if ( false === $result ) {
				throw new Exception( __( 'Unable to close the export file.', 'ebox' ) );
			}
		}
		...
		public function init_zip() {
			...
			if ( ! $zip_dir_status ) {
				throw new Exception( __( 'Unable to create the zip directory.', 'ebox' ) );
			}

			ebox_put_directory_index_file( trailingslashit( $this->zip_dir_path ) . 'index.php' );
			...
			if ( ! empty( $zip_files ) ) {
				...
				$this->zip_archive_url  = ebox_Admin_File_Download_Handler::get_download_url(
					self::ebox_EXPORT_ZIP_PATH_ID,
					basename( $this->zip_archive_path )
				);
			} else {
				$zip_file_name          = 'ebox-export-' . gmdate( 'Ymd' ) . '-' . uniqid() . '.zip';
		...
		public function generate_zip_archive(): void {
			...
			if ( true !== $result ) {
				throw new Exception( __( 'Unable to create the zip file.', 'ebox' ) );
			}
			...
			foreach ( $this->files as $name => $path ) {
				...
				if ( true !== $result ) {
					throw new Exception( __( 'Unable to add file to the zip file.', 'ebox' ) );
				}
			}
			...
			if ( true !== $result ) {
				throw new Exception( __( 'Unable to save the zip file.', 'ebox' ) );
			}
			 ...
			 * @since 4.3.0
			 */
			do_action( 'ebox_export_archive_created' );
		}
