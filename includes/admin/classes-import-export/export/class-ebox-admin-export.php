...
if (
	...
	/**
	 * Class ebox Admin Export.
	 *
	 * @since 4.3.0
	 */
	abstract class ebox_Admin_Export {
		...
		public function generate_export_file(): void {
			...
			if ( $this instanceof ebox_Admin_Export_Chunkable ) {
		...
		public function export_media_files(): void {
			if ( ! $this instanceof ebox_Admin_Export_Has_Media ) {
				return;
			}
			...
			// legacy media export.
			if ( $this instanceof ebox_Admin_Export_Posts ) {
				if ( ebox_get_post_type_slug( 'assignment' ) === $this->post_type ) {
					// assignments.
					$this->file_handler->add_raw_folder( 'assignments' );
				} elseif ( ebox_get_post_type_slug( 'essay' ) === $this->post_type ) {
					// essays.
