...
if (
	class_exists( 'ebox_Admin_Filter' ) &&
	 ...
	 * @since 4.2.0
	 */
	abstract class ebox_Admin_Filter_Meta extends ebox_Admin_Filter {
		/**
		 * Meta key name.
		 *
		 * @since 4.2.0
		...
		public function get_label(): string {
			return __( 'Where', 'ebox' ) . ' ' . $this->label;
		}
		...
		public function get_sql_where_clause( $filter_value ): string {
			...
			if ( '' !== $filter_value ) {
				// regex explanation:
				// example of array meta_value: 'modules_lesson_video_enabled";s:2:"on";'.
				// so, we have:
				// meta_value_index followed by '";' + a character that represents the data type + ':'. Ex: 'modules_course";i:1877;'
				...
				// regex explanation:
				// example of array meta_value: 'modules_lesson_video_enabled";s:0:"";'.
				// so, we have:
