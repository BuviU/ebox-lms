...
if ( ! class_exists( 'ebox_BitBucket_API' ) ) {
	class ebox_BitBucket_API {
		...
		private $repo_url_base     = 'https://api.bitbucket.org/2.0/repositories/ebox';
		private $download_url_base = 'https://bitbucket.org/ebox';
		...
		public function http_request_args( $parsed_args = array(), $url = '' ) {
			if ( ! empty( $url ) ) {
				if ( substr( $url, 0, strlen( $this->download_url_base ) ) === $this->download_url_base ) {
					if ( substr( $url, 0, strlen( $this->download_url_base . '/ebox-add-ons/get/stable.zip' ) ) === $this->download_url_base . '/ebox-add-ons/get/stable.zip' ) {
						$parsed_args['timeout'] = ebox_HTTP_BITBUCKET_README_DOWNLOAD_TIMEOUT;
					}

					/**
					 * Filter the timeout for ebox BitBucket downloads.
					 *
					 * @since 3.1.8
					 *
					 ...
					 */
					$parsed_args['timeout'] = apply_filters( 'ebox_bitbucket_request_timeout', absint( $parsed_args['timeout'] ), $parsed_args, $url );

				} elseif ( 'https://support.ebox.com' === substr( $url, 0, strlen( 'https://support.ebox.com' ) ) ) {
					if ( ( isset( $parsed_args['method'] ) ) && ( 'GET' === strtoupper( $parsed_args['method'] ) ) ) {
						$parsed_args['timeout'] = ebox_HTTP_REMOTE_GET_TIMEOUT;
					} else {
						$parsed_args['timeout'] = ebox_HTTP_REMOTE_POST_TIMEOUT;
					}

					/**
					 * Filter the timeout for ebox Support site connections.
					 *
					 * @since 3.1.8
					 *
					 * @param int    $timeout     Current timeout in seconds.
					 ...
					 * @param string $url         URL for request.
					 */
					$parsed_args['timeout'] = apply_filters( 'ebox_support_request_timeout', absint( $parsed_args['timeout'] ), $parsed_args, $url );
				}
			}
		...
		public function get_addon_directory() {
			if ( empty( $this->ld_addons_dir ) ) {
				...
				$this->ld_addons_dir = trailingslashit( $wp_upload_dir['basedir'] ) . 'ebox/add-ons';
		...
		public function get_bitbucket_repositories( $override_cache = false ) {
			...
			if ( true !== $override_cache ) {
				...
				$repo_error_count = 3;
				if ( ( defined( 'ebox_REPO_ERROR_THRESHOLD_COUNT' ) ) && ( ebox_REPO_ERROR_THRESHOLD_COUNT > 1 ) ) {
					$repo_error_count = absint( ebox_REPO_ERROR_THRESHOLD_COUNT );
				}

				$repo_error_time = 7200;
				if ( ( defined( 'ebox_REPO_ERROR_THRESHOLD_TIME' ) ) && ( ebox_REPO_ERROR_THRESHOLD_TIME > 1 ) ) {
					$repo_error_time = absint( ebox_REPO_ERROR_THRESHOLD_TIME );
				}

				$log_error_count = $this->get_error_count_bitbucket_repository( 'ebox-add-ons' );
				if ( absint( $log_error_count ) >= $repo_error_count ) {
					// Check the last time the log file was updated.
					$log_error_time      = $this->get_error_update_timestamp_bitbucket_repository( 'ebox-add-ons' );
					...
					$this->clear_error_bitbucket_repository( 'ebox-add-ons' );
				}
			} else {
				// f we are in cach orderride mode we clear the log.
				$this->clear_error_bitbucket_repository( 'ebox-add-ons' );
				$this->remove_lock_bitbucket_repository( 'ebox-add-ons' );
			}

			// Check if we can lock the processing file.
			if ( ! $this->lock_bitbucket_repository( 'ebox-add-ons' ) ) {
				return $repository_data;
			}

			$request_url = $this->get_bitbucket_repository_download_url( 'ebox-add-ons' );
			...
			if ( ! empty( $request_url ) ) {
				...
				if ( ! is_wp_error( $download ) ) {
					if ( is_file( $download ) ) {
						$addon_dest_file = $this->get_addon_directory() . '/ebox-add-ons.zip';
						...
						if ( true === $unzip_ret ) {
							...
							$unzip_files = ebox_scandir_recursive( $ld_addons_dir_unzip );
					...
					$this->error_log_bitbucket_repository( 'ebox-add-ons', $download );
				}

				// Unlock the lock file for other processes.
				$this->unlock_bitbucket_repository( 'ebox-add-ons' );
			}
		...
		public function get_bitbucket_repository_readme( $plugin_key = '' ) {
			if ( ! empty( $plugin_key ) ) {
				...
				if ( ( 200 === $code ) && ( ! empty( $body ) ) ) {
					$readme_parser = new eboxWPReadmeParser();
...
if ( ! class_exists( 'eboxWPReadmeParser' ) ) {

	class eboxWPReadmeParser {
