<?php
/**
 * ebox class to handle Search integration
 *
 * @package ebox\Search
 * @since 3.1.7
 */
...
if ( ! class_exists( 'ebox_Search' ) ) {

	/**
	 * Class for handling the ebox Search.
	 */
	class ebox_Search {
		...
		public function pre_get_posts( $query ) {
			...
			if ( ( $query->is_main_query() ) && ( ! is_admin() ) && ( $query->is_search ) ) {
				...
				$ld_post_types        = ebox_get_post_types( 'course_steps' );
				...
				if ( ! empty( $ld_post_types ) ) {
					...
					if ( is_user_logged_in() ) {
						...
						$this->enrolled_courses = ebox_user_get_enrolled_courses( $this->user_id, array(), true );
						if ( ! empty( $this->enrolled_courses ) ) {
							foreach ( $ld_post_types as $ld_post_type ) {
								if ( isset( $wp_post_types[ $ld_post_type ] ) ) {
									if ( ebox_post_type_search_param( $ld_post_type, 'search_enrolled_only' ) ) {
						...
						// If we don't have any enrolled courses we remove the ebox CPTs from the search.
						foreach ( $ld_post_types as $ld_post_type ) {
							if ( isset( $wp_post_types[ $ld_post_type ] ) ) {
								if ( ebox_post_type_search_param( $ld_post_type, 'search_login_only' ) ) {
									$wp_post_types[ $ld_post_type ]->exclude_from_search = true;
								}
							}
						}
