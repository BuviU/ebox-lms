 ...
 */
function ebox_purchase_invoice_filename( int $user_id = 0, int $post_id = 0 ) {
	...
	if ( ! in_array( $post->post_type, ebox_get_post_type_slug( array( 'course', 'team' ) ), true ) ) {
		return false;
	}
	 ...
	 * @param int    $user_id    User ID
	 */
	$filename = apply_filters( 'ebox_purchase_invoice_filename', $filename, $post_id, $file_time, $user_id );
 ...
 */
function ebox_purchase_invoice_filepath( int $post_id = 0 ) {
	$post_id = absint( $post_id );
	if ( empty( $post_id ) ) {
		return false;
	}
	...
	$ld_upload_dir   = $upload_dir_base . '/ebox';
	...
	if ( file_exists( $ld_upload_dir ) && is_writable( $ld_upload_dir ) ) {
		...
		if ( ! file_exists( $ld_upload_purchase_invoices_dir ) ) {
			if ( wp_mkdir_p( $ld_upload_purchase_invoices_dir ) !== false ) {
				...
				ebox_put_directory_index_file( trailingslashit( $ld_upload_purchase_invoices_dir ) . 'index.php' );
			}
		}
	 ...
	 */
	return apply_filters( 'ebox_purchase_invoice_filepath', $filepath, $ld_upload_purchase_invoices_dir );
}

if ( ! function_exists( 'ebox_generate_purchase_invoice' ) ) {
	/**
	 ...
	 */
	function ebox_generate_purchase_invoice( int $transaction_id ) {
		$email_setting = ebox_Settings_Section_Emails_Purchase_Invoice::get_section_settings_all();
		...
		if ( ebox_get_post_type_slug( LDLMS_Post_Types::TRANSACTION ) !== $transaction_post->post_type ) {
			return false;
		}
		 ...
		 * @param int   $post_id      Post ID.
		 */
		$placeholders = apply_filters( 'ebox_purchase_invoice_email_placeholders', $placeholders, $user_id, $post_id );
		...
		$filepath = ebox_purchase_invoice_filepath( $transaction_id );

		add_filter(
			'ebox_purchase_invoice_filepath',
			/**
		...
		$pdf_data = array(
			'purchaser_name'  => array(
				'label' => esc_html__( 'Purchased by: ', 'ebox' ),
				'value' => ebox_emails_parse_placeholders( $email_setting['purchaser_name'], $placeholders ),
			),
			'purchase_date'   => array(
				'label' => esc_html__( 'Date: ', 'ebox' ),
				'value' => $purchase_date,
			),
...
if ( ! function_exists( 'ebox_purchase_invoice_pdf' ) ) {
	/**
	 ...
	 */
	function ebox_purchase_invoice_pdf( array $pdf_content = array() ) {
		require_once ebox_LMS_PLUGIN_DIR . 'includes/ld-convert-post-pdf.php';

		$purchase_invoice_defaults = array(
			'post_id'       => 0,     // The Course/Team Post ID.
		...
		if ( empty( $pdf_content['pdf_data'] ) ) {
			wp_die( esc_html__( 'No data to generate PDF.', 'ebox' ) );
		}
		...
		if ( is_null( $purchase_invoice_args['purchase_invoice_post'] ) ) {
			wp_die( esc_html__( 'Post does not exist.', 'ebox' ) );
		}
		...
		if ( ( ! $purchase_invoice_args['user'] ) || ( ! is_a( $purchase_invoice_args['user'], 'WP_User' ) ) ) {
			wp_die( esc_html__( 'User does not exist.', 'ebox' ) );
		}
		...
		if ( ! empty( $config_lang_tmp ) && strlen( $config_lang_tmp ) === 3 ) {
			$ld_purchase_invoice_lang_dir = ebox_LMS_LIBRARY_DIR . '/tcpdf/config/lang';
		 ...
		 */
		$font = apply_filters( 'ebox_purchase_invoice_font', $font );
		 ...
		 */
		$font_size = apply_filters( 'ebox_purchase_invoice_font_size', $font_size );
		 ...
		 */
		$ebox_pdf_username = apply_filters(
			'ebox_purchase_invoice_pdf_username',
			$purchase_invoice_args['user']->user_login,
			$transaction_user_id,
			$purchase_invoice_args['purchase_invoice_id']
		);

		$purchase_invoice_args['pdf_title'] = __( 'Purchase Invoice - ', 'ebox' ) . get_the_title( $transaction_id ) . ' - ' . $pdf_content['purchase_date']['value'];
		...
		if ( ! $transaction ) {
			wp_die( esc_html__( 'Transaction now found.', 'ebox' ) );
		}

		try {
			$transaction_pricing = $transaction->get_pricing();
		} catch ( ebox_DTO_Validation_Exception $e ) {
			wp_die( esc_html__( 'Something went wrong.', 'ebox' ) );
		}
		...
		if ( $transaction->has_coupon() ) {
			...
			$pricing .= __( 'Original Price: ', 'ebox' ) . ebox_get_price_formatted( $transaction_pricing->price, $transaction_pricing->currency ) . '<br />';
			$pricing .= __( 'Coupon: ', 'ebox' ) . $coupon_data->code . '<br />';
			$pricing .= __( 'Discount: ', 'ebox' ) . $transaction_pricing->discount . '<br />';
			$pricing .= __( 'Discounted Price: ', 'ebox' ) . $transaction_pricing->discounted_price . '<br />';
			...
			$pricing .= sprintf(
				...
				_x( 'Trial: %1$s for %2$s %3$s then <br/ >', 'placeholder: Course', 'ebox' ),
				ebox_get_price_formatted( $transaction_pricing->trial_price, $transaction_pricing->currency ),
				$transaction_pricing->trial_duration_value,
				ebox_get_grammatical_number_label_for_interval( $transaction_pricing->trial_duration_value, $transaction_pricing->trial_duration_length )
			);
			$pricing .= sprintf(
				...
				_x( '%1$s every %2$s %3$s for %4$s %5$s', 'placeholder: Course', 'ebox' ),
				ebox_get_price_formatted( $transaction_pricing->price, $transaction_pricing->currency ),
				$transaction_pricing->duration_value,
				ebox_get_grammatical_number_label_for_interval( $transaction_pricing->duration_value, $transaction_pricing->duration_length ),
				$transaction_pricing->recurring_times * $transaction_pricing->duration_value,
				ebox_get_grammatical_number_label_for_interval( $transaction_pricing->recurring_times * $transaction_pricing->duration_value, $transaction_pricing->duration_length )
			);
		} elseif ( $transaction->is_subscription() && ! $transaction->has_trial() ) {
			$pricing .= sprintf(
				// translators: item price, duration value, duration length, product of recurring times multiplied by duration value, duration length.
			...
			// Normal transaction.
			$pricing .= ebox_get_price_formatted( $transaction_pricing->price, $transaction_pricing->currency );
		}
		 ...
		 */
		$fields = apply_filters( 'ebox_purchase_invoice_fields', $fields );
		...
		<tr>
			<th align="left"><i>' . __( 'Item Name', 'ebox' ) . '</i></th>
			<th align="right"><i>' . __( 'Price', 'ebox' ) . '</i></th>
		</tr>
		<tr>
		...
		$purchase_invoice_content = (string) preg_replace_callback(
			...
			'ebox_post2pdf_conv_image_align_center',
			$purchase_invoice_content
		);
		...
		if ( ! class_exists( 'TCPDF' ) ) {
			require_once ebox_LMS_LIBRARY_DIR . '/tcpdf/config/lang/' . $purchase_invoice_args['lang'] . '.php';
			require_once ebox_LMS_LIBRARY_DIR . '/tcpdf/tcpdf.php';
		}
		 ...
		 * @param int   $purchase_invoice_id      purchase_invoice post ID.
		 */
		$tcpdf_params = apply_filters( 'ebox_purchase_invoice_params', $tcpdf_params, $purchase_invoice_args['purchase_invoice_id'] );
		 ...
		 * @param int   $purchase_invoice_id purchase_invoice post ID.
		 */
		do_action( 'ebox_purchase_invoice_created', $pdf, $purchase_invoice_args['purchase_invoice_id'] );
		 ...
		 * @param int    $purchase_invoice_id     purchase_invoice post ID.
		 */
		$pdf->SetCreator( apply_filters( 'ebox_purchase_invoice_pdf_creator', PDF_CREATOR, $pdf, $purchase_invoice_args['purchase_invoice_id'] ) );
		 ...
		 * @param int    $purchase_invoice_id         purchase_invoice post ID.
		 */
		$pdf->SetAuthor( apply_filters( 'ebox_purchase_invoice_pdf_author', $purchase_invoice_args['pdf_author_name'], $pdf, $purchase_invoice_args['purchase_invoice_id'] ) );
		 ...
		 * @param int    $purchase_invoice_id   purchase_invoice post ID.
		 */
		$pdf->SetTitle( apply_filters( 'ebox_purchase_invoice_pdf_title', $purchase_invoice_args['pdf_title'], $pdf, $purchase_invoice_args['purchase_invoice_id'] ) );
		 ...
		 * @param int    $purchase_invoice_id     purchase_invoice post ID.
		 */
		$pdf->SetSubject( apply_filters( 'ebox_purchase_invoice_pdf_subject', wp_strip_all_tags( get_the_category_list( ',', '', $purchase_invoice_args['purchase_invoice_id'] ) ), $pdf, $purchase_invoice_args['purchase_invoice_id'] ) );
		 ...
		 * @param int    $purchase_invoice_id      purchase_invoice post ID.
		 */
		$pdf->SetKeywords( apply_filters( 'ebox_purchase_invoice_pdf_keywords', $purchase_invoice_args['pdf_keywords'], $pdf, $purchase_invoice_args['purchase_invoice_id'] ) );
		 ...
		 * @param int   $post_id Post ID.
		 */
		do_action( 'ebox_purchase_invoice_after', $pdf, $purchase_invoice_args['purchase_invoice_id'] );
		...
		if ( '' !== $img_file ) {
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_purchase_invoice_thumbnail_processing_start', $img_file, $purchase_invoice_args, $pdf );
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_purchase_invoice_thumbnail_before', $img_file, $purchase_invoice_args, $pdf );
			 ...
			 */
			$logo_params = apply_filters( 'ebox_purchase_invoice_logo_params', $logo_params );
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_purchase_invoice_thumbnail_after', $img_file, $purchase_invoice_args, $pdf );
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_purchase_invoice_thumbnail_processing_end', $img_file, $purchase_invoice_args, $pdf );
		}
		 ...
		 * @param array  $purchase_invoice_args Array of purchase_invoice args.
		 */
		do_action( 'ebox_purchase_invoice_content_write_cell_before', $pdf, $purchase_invoice_args );
		...
		$pdf_cell_args = apply_filters(
			'ebox_purchase_invoice_content_write_cell_args',
			$pdf_cell_args,
			$purchase_invoice_args,
			$tcpdf_params,
			$pdf
		 ...
		 * @param array  $purchase_invoice_args Array of purchase_invoice args.
		 */
		do_action( 'ebox_purchase_invoice_content_write_cell_after', $pdf, $purchase_invoice_args );
		...
		$filepath = ebox_purchase_invoice_filepath( $purchased_post_id );
		$filename = ebox_purchase_invoice_filename( $transaction_user_id, $purchased_post_id );
		$file     = array(
			'filepath' => $filepath,
			'filename' => $filename,
		);
