...
/**
 * Send the course/team purchase success email
 *
 * @since 3.6.0
 *
 * @param int $user_id User ID.
 * @param int $post_id Course/Team post ID.
 *
 * @return void
 */
function ebox_send_purchase_success_email( int $user_id = 0, int $post_id = 0 ): void {
	if ( empty( $user_id ) ) {
		return;
	}
	 ...
	 */
	$placeholders = apply_filters( 'ebox_purchase_email_placeholders', $placeholders, $user_id, $post_id );

	if ( in_array( $post->post_type, ebox_get_post_type_slug( array( 'course' ) ), true ) ) {
		$email_setting = ebox_Settings_Section_Emails_Course_Purchase_Success::get_section_settings_all();
	} elseif ( in_array( $post->post_type, ebox_get_post_type_slug( array( 'team' ) ), true ) ) {
		$email_setting = ebox_Settings_Section_Emails_Team_Purchase_Success::get_section_settings_all();
	} else {
		return;
	}
	...
	if ( 'on' === $email_setting['enabled'] ) {
		 ...
		 * @param int    $post_id       Post ID.
		 */
		$email_setting['subject'] = apply_filters( 'ebox_purchase_email_subject', $email_setting['subject'], $user_id, $post_id );
		 ...
		 * @param int    $post_id       Post ID.
		 */
		$email_setting['message'] = apply_filters( 'ebox_purchase_email_message', $email_setting['message'], $user_id, $post_id );
		...
		if ( ( ! empty( $email_setting['subject'] ) ) && ( ! empty( $email_setting['message'] ) ) ) {
			ebox_emails_send( $user->user_email, $email_setting );
		}
	}
}
 ...
 */
function ebox_emails_parse_placeholders( string $content = '', array $placeholders = array() ): string {
 ...
 */
function ebox_emails_from_name( string $from_name = '' ): string {
	$ebox_from_name = ebox_Settings_Section::get_section_setting(
		'ebox_Settings_Section_Emails_Sender_Settings',
		'from_name'
	);

	return ! empty( $ebox_from_name )
		? sanitize_text_field( $ebox_from_name )
		: $from_name;
}
 ...
 */
function ebox_emails_from_email( string $from_email = '' ): string {
	$ebox_from_email = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Emails_Sender_Settings', 'from_email' );

	return sanitize_email(
		! empty( $ebox_from_email ) ? $ebox_from_email : $from_email
	);
}
 ...
 */
function ebox_emails_send( string $user_email = '', array $email_args = array(), string $headers = '', array $attachments = array() ): bool {
	if ( empty( $user_email ) ) {
		return false;
	}
	...
	remove_filter( 'wp_mail_from', 'ebox_emails_from_email' );
	remove_filter( 'wp_mail_from_name', 'ebox_emails_from_name' );
...
add_action(
	...
	function ( int $transaction_id ): void {
		 ...
		 */
		$placeholders = apply_filters( 'ebox_purchase_invoice_email_placeholders', $placeholders, $user->ID, $product_id );
		 ...
		 * @param int    $product_id    Transaction Product ID.
		 */
		$email_setting['subject'] = apply_filters( 'ebox_purchase_invoice_email_subject', $email_setting['subject'], $user->ID, $product_id );
		 ...
		 * @param int    $product_id    Transaction Product ID.
		 */
		$email_setting['message'] = apply_filters( 'ebox_purchase_invoice_email_message', $email_setting['message'], $user->ID, $product_id );

		if ( ! empty( $email_setting['message'] ) ) {
			$email_setting['message'] = ebox_emails_parse_placeholders( $email_setting['message'], $placeholders );
		}

		$pdf = ebox_generate_purchase_invoice( $transaction_id );
...
add_action(
	...
	function ( int $transaction_id ): void {
		...
		ebox_send_purchase_success_email( $transaction->get_user()->ID, $product->get_id() );
	}
);
