 ...
 * Everything except the button adding is handled in includes/payments/class-ebox-paypal-ipn.php.
 *
 * @since 4.5.0
 *
 * @package ebox
...
if ( ! class_exists( 'ebox_Paypal_IPN_Gateway' ) && class_exists( 'ebox_Payment_Gateway' ) ) {
	/**
	 ...
	 * }
	 */
	class ebox_Paypal_IPN_Gateway extends ebox_Payment_Gateway {
		...
		public function cancel_subscription( string $subscription_id ): WP_Error {
			return new WP_Error(
				self::$wp_error_code,
				__( 'Subscription management is not supported by PayPal IPN payment gateway.', 'ebox' )
			);
		}
		...
		protected function configure(): void {
			$this->settings = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_PayPal' );
		}
		...
		public function process_webhook(): void {
			if (
			( ! isset( $_SERVER['REQUEST_URI'] ) && ! isset( $_GET['ebox-integration'] ) && ! isset( $_GET['ebox-lms'] ) ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			( isset( $_GET['ebox-integration'] ) && $this->get_name() !== $_GET['ebox-integration'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			( isset( $_GET['ebox-lms'] ) && 'paypal' !== $_GET['ebox-lms'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			( isset( $_SERVER['REQUEST_URI'] ) && ! isset( $_GET['ebox-integration'] ) && ! isset( $_GET['ebox-lms'] ) && strpos( sanitize_text_field( wp_unslash( $_SERVER['REQUEST_URI'] ) ), 'ebox-lms/paypal' ) === false ) // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			) {
				return;
			}
			...
			if ( ! empty( $this->transaction_data['post_id'] ) ) {
				 ...
				 *
				 * Use find_many() method to prepare for ebox cart system update.
				 * The post_id will be changed later to posts_ids that holds
				 * multiple post_id values after cart system update.
				 */
				$this->products = ebox_Product_Model::find_many( array( $this->transaction_data['post_id'] ) );
			}
			...
			if ( ! empty( $this->transaction_data['txn_type'] ) ) {
				...
				switch ( $this->transaction_data['txn_type'] ) {
					...
					case 'subscr_payment':
						$transactions = ebox_Transaction_Model::find_many_by_meta(
							array(
								'user_id'   => $user_id,
								'post_id'   => $post_id,
		...
		protected function map_payment_button_markup( array $params, WP_Post $post ): string {
			try {
				$product = ebox_Product_Model::create_from_post( $post );
			...
			$price               = apply_filters( 'ebox_get_price_by_coupon', $pricing->price, $product->get_id(), $user_id );
			$button_label        = esc_html(
				$this->map_payment_button_label( __( 'Use PayPal', 'ebox' ), $post )
			);
		 ...
		 * @param ebox_Pricing_DTO $product_pricing Product pricing DTO.
		 *
		 * @return string
		 */
		private function generate_user_purchase_hash( int $product_id, ebox_Pricing_DTO $product_pricing ): string {
			...
			set_transient(
				...
				array(
					...
					'time'       => time(),
					'nonce'      => $hash_nonce,
					ebox_Transaction_Model::$meta_key_pricing_info => $product_pricing->to_array(),
				),
			...
			switch ( $this->webhook_action ) {
				case self::RETURN_ACTION_NAME_SUCCESS:
					...
					if ( isset( $this->user_hash['product_id'] ) ) {
						...
						if ( ebox_get_post_type_slug( 'course' ) === $this->transaction_data['post_type'] ) {
							$this->transaction_data['course_id'] = $product_id;
						} elseif ( ebox_get_post_type_slug( 'team' ) === $this->transaction_data['post_type'] ) {
							$this->transaction_data['team_id'] = $product_id;
						}
						 ...
						 * multiple post_id values after cart system update.
						 */
						$this->products = ebox_Product_Model::find_many( array( $product_id ) );
					}
					...
					if ( empty( $this->products ) ) {
						$this->products = ebox_Product_Model::find_many( array( $item_number ) );
					}
				...
				case self::RETURN_ACTION_NAME_NOTIFY:
					...
					$transactions = ebox_Transaction_Model::find_many_by_meta( $query_args );
		...
		private function process_webhook_data_init(): void {
			...
			$this->log_info( 'ebox Version: ' . ebox_VERSION ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		}
		...
		private function process_webhook_ipn_listener(): void {
			...
			if ( ! file_exists( ebox_LMS_LIBRARY_DIR . '/paypal/ipnlistener.php' ) ) {
				$message = 'Required file not found: ' . ebox_LMS_LIBRARY_DIR . '/paypal/ipnlistener.php';
				$this->log_error( $message );
		...
		private function process_webhook_data(): void {
			...
			if ( ebox_is_course_post( $this->transaction_data['post_id'] ) ) {
				$this->log_info( 'Purchased Course access [' . $this->transaction_data['post_id'] . ']' );
			} elseif ( ebox_is_team_post( $this->transaction_data['post_id'] ) ) {
				$this->log_info( 'Purchased Team access [' . $this->transaction_data['post_id'] . ']' );
			}
		...
		protected function map_transaction_meta( $data, ebox_Product_Model $product ): ebox_Transaction_Meta_DTO {
			...
			$pricing_info = ! empty( $this->user_hash[ ebox_Transaction_Model::$meta_key_pricing_info ] ) ?
				ebox_Pricing_DTO::create(
					// @phpstan-ignore-next-line -- Variable array key name.
					$this->user_hash[ ebox_Transaction_Model::$meta_key_pricing_info ]
				) :
				ebox_Pricing_DTO::create(
					$pricing_array
				);
			...
			$meta = array(
				...
				ebox_Transaction_Model::$meta_key_price_type => ! $is_subscription_event ? ebox_PRICE_TYPE_PAYNOW : ebox_PRICE_TYPE_SUBSCRIBE,
				ebox_Transaction_Model::$meta_key_pricing_info => $pricing_info,
				ebox_Transaction_Model::$meta_key_has_trial => $is_subscription_event && ! empty( $data['period1'] ),
				ebox_Transaction_Model::$meta_key_has_free_trial => $is_subscription_event && ! empty( $data['period1'] ) && isset( $data['mc_amount1'] ) && ( '0.00' === strval( $data['mc_amount1'] ) || '0' === strval( $data['mc_amount1'] ) ),
