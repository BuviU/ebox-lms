...
if ( ! class_exists( 'ebox_Unknown_Gateway' ) && class_exists( 'ebox_Payment_Gateway' ) ) {
	/**
	 * Unknown gateway class.
	 *
	 * @since 4.5.0
	 */
	class ebox_Unknown_Gateway extends ebox_Payment_Gateway {
		...
		public function cancel_subscription( string $subscription_id ): WP_Error {
			return new WP_Error(
				self::$wp_error_code,
				__( 'Subscription management is not possible because the payment gateway is not enabled or configured.', 'ebox' )
			);
		}
		...
		public static function get_label(): string {
			return esc_html__( 'Unknown', 'ebox' );
		}
		 ...
		 *
		 * @throws ebox_DTO_Validation_Exception Exception.
		 *
		...
		protected function map_transaction_meta( $data, ebox_Product_Model $product ): ebox_Transaction_Meta_DTO {
			return ebox_Transaction_Meta_DTO::create();
		}
