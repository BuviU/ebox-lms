...
if ( ! class_exists( 'ebox_Payment_Gateway' ) ) {
	/**
	 * Payment gateway class.
	 *
	 * @since 4.5.0
	 */
	abstract class ebox_Payment_Gateway {
		/**
		 ...
		 * @var string
		 */
		protected static $wp_error_code = 'ebox_payment_gateway_error';
		...
		/**
		 * ebox_Transaction_Logger instance or null.
		 *
		 * @since 4.5.0
		 *
		 * @var ebox_Transaction_Logger|null
		 */
		...
		public function __construct() {
			...
			$this->currency_code = mb_strtoupper( ebox_get_currency_code() );
			$this->log_prefix    = uniqid();

			// add transaction loggers.
			if ( $this->supports_logger() ) {
				$this->logger = new ebox_Transaction_Logger( $this );
		...
		public function init(): void {
			...
			add_action(
				'ebox_payment_button_added',
				function() {
					wp_enqueue_script( 'ebox-payments' );

					$this->enqueue_scripts();
				}
			);

			add_filter( 'ebox_payment_buttons', array( $this, 'add_payment_button' ), 11, 3 );
		...
		public function cancel_subscription( string $subscription_id ) {
			return new WP_Error(
				...
				sprintf(
					...
					esc_html_x( 'Subscription cancellation is not supported by %1$s gateway.', 'placeholder: Gateway name', 'ebox' ),
					static::get_name()
				)
			);
		}
		 ...
		 */
		abstract protected function map_transaction_meta( $data, ebox_Product_Model $product ): ebox_Transaction_Meta_DTO;
		...
		protected function get_ajax_action_name_setup(): string {
			return 'ebox_payment_gateway_setup_' . static::get_name();
		}
		 ...
		 *
		 * @return ebox_Payment_Gateway Instance of the payment gateway. ebox_Unknown_Gateway if not found or not ready.
		 */
		public static function get_active_payment_gateway_by_name( string $payment_gateway_name ): ebox_Payment_Gateway {
			if ( ! isset( self::$active_gateways[ $payment_gateway_name ] ) ) {
				return self::$active_gateways[ ebox_Unknown_Gateway::get_name() ];
			}
		...
		public static function get_url_success( array $products, string $gateway_url = '' ): string {
			$products = array_filter(
				$products,
				function ( $product ) {
					return $product instanceof ebox_Product_Model;
				}
			);
			...
			if ( 1 === count( $products ) ) {
				...
				if ( ebox_is_course_post( $product->get_post() ) ) {
					$enrollment_url = ebox_get_course_enrollment_url( $product->get_post() );
				} elseif ( ebox_is_team_post( $product->get_post() ) ) {
					$enrollment_url = ebox_get_team_enrollment_url( $product->get_post() );
				}
				...
				if ( ! empty( $enrollment_url ) ) {
					 ...
					 * @param string                    $payment_gateway_name Payment gateway name.
					 * @param ebox_Product_Model[] $products             Purchased products.
					 *
					 * @return string The URL, where user will be redirected after the successful payment.
					 */
					return apply_filters( 'ebox_payment_option_url_success', $enrollment_url, static::get_name(), $products );
				}
			}
			...
			if ( ! empty( $gateway_url ) ) {
				/** This filter is documented in includes/payments/gateways/class-ebox-payment-gateway.php */
				return apply_filters( 'ebox_payment_option_url_success', $gateway_url, static::get_name(), $products );
			}

			// "Registration success" page link in settings.

			$registration_success_page_id = (int) ebox_Settings_Section::get_section_setting(
			...
			if ( ! empty( $registration_success_page_id ) ) {
				...
				if ( ! empty( $registration_success_page_url ) ) {
					/** This filter is documented in includes/payments/gateways/class-ebox-payment-gateway.php */
					return apply_filters( 'ebox_payment_option_url_success', $registration_success_page_url, static::get_name(), $products );
				}
			}
			...
			if ( 1 === count( $products ) ) {
				...
				if ( ! empty( $post_url ) ) {
					/** This filter is documented in includes/payments/gateways/class-ebox-payment-gateway.php */
					return apply_filters( 'ebox_payment_option_url_success', $post_url, static::get_name(), $products );
				}
			}
			...
			/** This filter is documented in includes/payments/gateways/class-ebox-payment-gateway.php */
			return apply_filters( 'ebox_payment_option_url_success', get_home_url(), static::get_name(), $products );
		}
		...
		public static function get_url_fail( array $products, string $gateway_url = '' ): string {
			...
			if ( ! empty( $gateway_url ) ) {
				 ...
				 */
				return apply_filters( 'ebox_payment_option_url_fail', $gateway_url, static::get_name(), $products );
			}
			...
			/** This filter is documented in includes/payments/gateways/class-ebox-payment-gateway.php */
			return apply_filters( 'ebox_payment_option_url_fail', $url, static::get_name(), $products );
		}
		...
		protected function map_description( array $products = array() ): string {
			...
			$product_titles = array_map(
				function( ebox_Product_Model $product ): string {
		...
		protected function get_image_url( array $products = array() ): string {
			...
			if ( empty( $image_url ) ) {
				$logo_image_id = ebox_Settings_Section::get_section_setting(
					'ebox_Settings_Theme_LD30',
					'login_logo'
				);
			 ...
			 */
			$image_url = (string) apply_filters( 'ebox_payment_option_image_url', $image_url, static::get_name() );
		...
		protected function record_transaction( array $meta, WP_Post $post, WP_User $user ): int {
			$transaction_id = ebox_transaction_create( $meta, $post, $user, $this->parent_transaction_id );
		...
		protected function get_nonce_name(): string {
			return 'ebox-payment-gateway-' . static::get_name() . '-nonce';
		}
		...
		protected function get_form_class_name( string $additional_class = '' ): string {
			$form_class = 'ebox-payment-gateway-form-' . static::get_name();
		...
		private function create_user( string $email ): ?WP_User {
			 ...
			 */
			$shorten = (bool) apply_filters( 'ebox_payment_gateway_get_username_from_email', false );

			if ( ebox_Stripe_Gateway::get_name() === static::get_name() ) {
				/**
				 * Filters whether to shorten a username from an email or keep an email. Default false.
				 *
				 * @since 4.0.0
				 * @deprecated 4.5.0 Use the {@see 'ebox_payment_gateway_get_username_from_email'} filter instead.
				 *
				...
				$shorten = apply_filters_deprecated(
					'ebox_stripe_create_short_username',
					array( $shorten ),
					'4.5.0',
					'ebox_get_username_from_email'
				);
			}
			...
			if ( ebox_Razorpay_Gateway::get_name() === static::get_name() ) {
				/**
				 * Fires after a user is created with Razorpay.
				 *
				 * @since 4.2.0
				 * @deprecated 4.5.0 Use the {@see 'ebox_payment_gateway_user_created'} action instead.
				 *
				 * @param int $user_id User ID.
				 */
				do_action_deprecated(
					'ebox_user_created_with_razorpay',
					array( $user_id ),
					'4.5.0',
					'ebox_payment_gateway_user_created'
				);
			}
			 ...
			 * @param string  $payment_gateway_name Payment gateway name.
			 */
			do_action( 'ebox_payment_gateway_user_created', $user, static::get_name() );
		...
		protected function map_payment_button_label( string $gateway_button_label, WP_Post $post ): string {
			...
			$active_gateways = array_filter(
				...
				function ( $gateway ) {
					return ! $gateway instanceof ebox_Unknown_Gateway;
				}
			);

			if ( count( $active_gateways ) > 1 ) {
				$button_label = $gateway_button_label;
			} elseif ( ebox_is_course_post( $post ) ) {
				$button_label = ebox_Custom_Label::get_label( ebox_Custom_Label::$button_take_course );
			} elseif ( ebox_is_team_post( $post ) ) {
				$button_label = ebox_Custom_Label::get_label( ebox_Custom_Label::$button_take_team );
			}
			...
			if ( ebox_Razorpay_Gateway::get_name() === static::get_name() ) {
				 ...
				 *
				 * @since 4.5.0
				 * @deprecated 4.5.0 Use the {@see 'ebox_payment_button_label'} filter instead.
				 *
				...
				$button_label = apply_filters_deprecated(
					'ebox_button_label_razorpay',
					array( $button_label ),
					'4.5.0',
					'ebox_payment_button_label'
				);
			} elseif ( ebox_Stripe_Gateway::get_name() === static::get_name() ) {
				/**
				 ...
				 */
				$button_label = apply_filters_deprecated(
					'ebox_stripe_purchase_button_text',
					array( $button_label ),
					'4.5.0',
					'ebox_payment_button_label'
				);
			 ...
			 */
			return apply_filters( 'ebox_payment_button_label', $button_label, static::get_name() );
		}
		...
		protected function maybe_ignore_event( array $event ): bool {
			if ( ebox_Razorpay_Gateway::get_name() === static::get_name() ) {
				...
				if (
					! apply_filters_deprecated(
						'ebox_process_webhook_razorpay',
						array( true, $event ),
						'4.5.0',
						'ebox_payment_gateway_event_ignore'
					)
				) {
					return true;
				}
			 ...
			 * @return bool
			 */
			return apply_filters( 'ebox_payment_gateway_event_ignore', false, static::get_name(), $event );
		}
		...
		protected function finish_webhook_processing( array $event, bool $processed ): void {
			 ...
			 * @param string $gateway_name Gateway name.
			 */
			do_action( 'ebox_payment_gateway_event_processed', $event, $processed, static::get_name() );
