...
if ( ! class_exists( 'ebox_Stripe_Gateway' ) && class_exists( 'ebox_Payment_Gateway' ) ) {
	/**
	 * Stripe Connect gateway class.
	 *
	 * @since 4.5.0
	 */
	class ebox_Stripe_Gateway extends ebox_Payment_Gateway {
		...
		public static function get_label(): string {
			return esc_html__( 'Stripe Connect', 'ebox' );
		}
		...
		public function setup_payment(): void {
			...
			if ( ! $product ) {
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Product not found.', 'ebox' ),
					)
				);
			}
			...
			try {
				...
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Stripe session was not created.', 'ebox' ) . ' ' . esc_html( $e->getMessage() ),
					)
				);
			}
		...
		protected function configure(): void {
			$this->settings = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_Stripe_Connect' );

			$this->currency_code = mb_strtolower( $this->currency_code );

			$this->customer_id_meta_key = $this->is_test_mode()
				? ebox_Settings_Section_Stripe_Connect::STRIPE_CUSTOMER_ID_META_KEY_TEST
				: ebox_Settings_Section_Stripe_Connect::STRIPE_CUSTOMER_ID_META_KEY;
			...
			if ( ! class_exists( 'Stripe\Stripe' ) ) {
				require_once ebox_LMS_LIBRARY_DIR . '/stripe-php/init.php';
			}
		...
		public function map_payment_button_markup( array $params, WP_Post $post ): string {
			...
			$button .= '<input class="' . esc_attr( ebox_Payment_Button::map_button_class_name() ) . '" id="' . esc_attr( ebox_Payment_Button::map_button_id() ) . '" type="submit" value="' . esc_attr( $button_label ) . '">';
			$button .= '</form>';
			...
			<script>
				...
				jQuery(document).ready(function ($) {
					...
					$(document).on('submit', '.<?php echo esc_attr( $this->get_form_class_name() ); ?>', function (e) {
						...
						$(this).closest('.ebox_checkout_buttons').addClass('ld-loading');
						...
						$.ajax({
							...
							$('.ebox_checkout_buttons').removeClass('ld-loading');
		...
		public function process_webhook(): void {
			if (
				! isset( $_GET['ebox-integration'] ) || // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				(
					$this->get_name() !== $_GET['ebox-integration'] && // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					'stripe-connect' !== $_GET['ebox-integration'] // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Legacy integration name.
				)
			) {
				return;
			}
		 ...
		 * @param ebox_Product_Model[] $products Products.
		...
		private function process_webhook_event( Event $event, array $products, WP_User $user ): void {
			...
			switch ( $event->type ) {
				...
				case self::EVENT_INVOICE_PAYMENT_SUCCEEDED:
					...
					if ( ! isset( $subscription_meta[ ebox_Transaction_Model::$meta_key_pricing_info ] ) ) {
					...
					$subscription_meta[ ebox_Transaction_Model::$meta_key_pricing_info ] = json_decode(
						$subscription_meta[ ebox_Transaction_Model::$meta_key_pricing_info ],
						true
					);

					try {
						$transaction_meta_dto = ebox_Transaction_Meta_DTO::create( $subscription_meta );
					} catch ( Exception $e ) {
					...
					foreach ( $products as $index => $product ) {
						if ( isset( $entity->metadata->has_recurring_limit ) && $entity->metadata->has_recurring_limit ) {
							...
							$remove_post_access = (bool) apply_filters(
								'ebox_stripe_remove_user_course_access_on_recurring_limit',
								false,
								$product->get_id(),
								$this->user->ID
							);
		 ...
		 * @since 4.5.0
		 *
		 * @param ebox_Product_Model $product Product.
		 *
		 ...
		 */
		private function get_legacy_subscription_params( ebox_Product_Model $product ): array {
			...
			$post_settings = ebox_get_setting( $product_post );
		 ...
		 * @param string                  $ebox_version     ebox version that was used to create the event. Can be empty for older versions.
		 * @param ebox_Product_Model $product               Product.
		 *
		 * @return array<mixed>
		 */
		private function process_legacy_meta(
			...
			string $ebox_version,
			ebox_Product_Model $product
		): array {
			if ( empty( $ebox_version ) ) {
				if ( ! isset( $event_meta[ ebox_Transaction_Model::$meta_key_gateway_name ] ) ) {
					$event_meta[ ebox_Transaction_Model::$meta_key_gateway_name ] = self::get_name();
				}

				if ( ! isset( $event_meta[ ebox_Transaction_Model::$meta_key_price_type ] ) ) {
					$event_meta[ ebox_Transaction_Model::$meta_key_price_type ] = $is_subscription_event
						? ebox_PRICE_TYPE_SUBSCRIBE
						: ebox_PRICE_TYPE_PAYNOW;
				}

				if ( ! isset( $event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ] ) ) {
					$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ] = array(
					...
					if ( $is_subscription_event ) {
						...
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ] = array_merge(
							$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ],
						...
						$has_free_trial = $has_trial && 0. === ebox_get_price_as_float( $legacy_subscription_params['trial_price'] );

						$event_meta[ ebox_Transaction_Model::$meta_key_has_trial ]      = $has_trial;
						$event_meta[ ebox_Transaction_Model::$meta_key_has_free_trial ] = $has_free_trial;
					}

					// Encode to decode later, just for compatibility with the new code.
					$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ] = wp_json_encode(
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]
					);
				}
			}
		...
		private function is_legacy_data( $entity ): bool {
			return empty( $entity->metadata->is_ebox );
		}
		...
		private function setup_products_or_fail( $entity ): array {
			...
			if ( ! empty( $entity->lines->data ) ) {
				...
				$products = ebox_Product_Model::find_many(
					array_filter( $post_ids )
				);
			} else {
				$post_id    = (int) ( $entity->metadata[ $product_meta_key ] ?? 0 );
				$post_ids[] = $post_id;
				$product    = ebox_Product_Model::find( $post_id );
			...
			$this->log_info(
				'Products IDs: ' . array_reduce(
					$products,
					function ( string $carry, ebox_Product_Model $product ): string {
		...
		public function show_successful_message(): void {
			...
			$message = is_user_logged_in()
				? __( 'Your transaction was successful.', 'ebox' )
				: __( 'Your transaction was successful. Please log in to access your content.', 'ebox' );
			?>
		 ...
		 * @param ebox_Product_Model $product Product.
		 *
		 * @throws ebox_DTO_Validation_Exception Transaction data validation exception.
		 *
		 * @return ebox_Transaction_Meta_DTO
		 */
		protected function map_transaction_meta( $entity, ebox_Product_Model $product ): ebox_Transaction_Meta_DTO {
			...
			$meta = array_merge(
				...
				array(
					ebox_Transaction_Model::$meta_key_gateway_transaction => ebox_Transaction_Gateway_Transaction_DTO::create(
						array(
			...
			$meta = $this->process_legacy_meta(
				...
				$meta,
				$is_subscription,
				$entity->metadata->ebox_version ?? '',
				$product
			);

			// It was encoded to allow arrays in the metadata.
			if ( is_string( $meta[ ebox_Transaction_Model::$meta_key_pricing_info ] ) ) {
				$meta[ ebox_Transaction_Model::$meta_key_pricing_info ] = json_decode(
					$meta[ ebox_Transaction_Model::$meta_key_pricing_info ],
					true
				);
			}

			return ebox_Transaction_Meta_DTO::create( $meta );
		}
		...
		private function get_payment_methods(): array {
			 ...
			 */
			return apply_filters( 'ebox_stripe_payment_method_types', $enabled_payment_methods );
		}
		 ...
		 * @param ebox_Product_Model $product Product.
		 *
		 * @throws ebox_DTO_Validation_Exception If order creation fails.
		 *
		 * @return array{metadata: array<mixed>, items: array<mixed>, payment_data: array<mixed>}
		 */
		private function get_order_data( float $amount, ebox_Product_Model $product ): array {
			$transaction_meta_dto = ebox_Transaction_Meta_DTO::create(
				array(
					ebox_Transaction_Model::$meta_key_gateway_name => $this::get_name(),
					ebox_Transaction_Model::$meta_key_price_type   => ebox_PRICE_TYPE_PAYNOW,
					ebox_Transaction_Model::$meta_key_pricing_info => ebox_Pricing_DTO::create(
						array(
		 ...
		 *
		 * @param float                   $amount  Amount.
		 * @param ebox_Pricing_DTO   $pricing Pricing DTO.
		 * @param ebox_Product_Model $product Product.
		 *
		 ...
		 */
		private function get_subscription_data( float $amount, ebox_Pricing_DTO $pricing, ebox_Product_Model $product ): array {
			if ( empty( $pricing->duration_length ) ) {
				throw new Exception( __( 'The Billing Cycle Interval value must be set.', 'ebox' ) );
			} elseif ( 0 === $pricing->duration_value ) {
				throw new Exception( __( 'The minimum Billing Cycle value is 1.', 'ebox' ) );
			}
			...
			$transaction_meta_dto = ebox_Transaction_Meta_DTO::create(
				array(
					ebox_Transaction_Model::$meta_key_gateway_name   => $this::get_name(),
					ebox_Transaction_Model::$meta_key_price_type     => ebox_PRICE_TYPE_SUBSCRIBE,
					ebox_Transaction_Model::$meta_key_pricing_info   => $pricing,
					ebox_Transaction_Model::$meta_key_has_trial      => $has_trial,
					ebox_Transaction_Model::$meta_key_has_free_trial => $has_trial && 0. === $course_trial_price,
				)
			);
				...
				$items = array(
					array(
						'name'     => sprintf(
							...
							_n( '%d Day Trial', '%d Days Trial', $trial_duration_in_days, 'ebox' ),
		 ...
		 * @param ebox_Product_Model $product Product.
		 *
		 * @throws ebox_DTO_Validation_Exception|ApiErrorException|InvalidArgumentException Invalid pricing or validation or API error.
		 *
		 * @return string Stripe session ID.
		 */
		private function create_payment_session( ebox_Product_Model $product ): string {
			$product_pricing = $product->get_pricing( $this->user );

			/**
			 * Filters course/team price.
			 *
			 * @since 4.1.0
			 *
			 ...
			 */
			$course_price = apply_filters( 'ebox_get_price_by_coupon', $product_pricing->price, $product->get_id(), $this->user->ID );
			...
			if ( ebox_PRICE_TYPE_PAYNOW === $product_pricing_type ) {
				$payment_intent_data = $this->get_order_data( $course_price, $product );
			} elseif ( ebox_PRICE_TYPE_SUBSCRIBE === $product_pricing_type ) {
		 ...
		 * @return string
		 */
		private function get_plan_id( float $amount, ebox_Pricing_DTO $pricing, ebox_Product_Model $product ): string {
