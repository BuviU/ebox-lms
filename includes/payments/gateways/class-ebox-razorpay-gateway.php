...
if ( ! class_exists( 'ebox_Razorpay_Gateway' ) && class_exists( 'ebox_Payment_Gateway' ) ) {
	/**
	 * Razorpay gateway class.
	 *
	 * @since 4.5.0
	 */
	class ebox_Razorpay_Gateway extends ebox_Payment_Gateway {
		private const GATEWAY_NAME = 'razorpay';

		private const META_KEY_CUSTOMER_ID_LIVE = 'ebox_razorpay_live_customer_id';
		private const META_KEY_CUSTOMER_ID_TEST = 'ebox_razorpay_test_customer_id';

		private const META_KEY_PLANS_LIVE = 'ebox_razorpay_live_plans';
		private const META_KEY_PLANS_TEST = 'ebox_razorpay_test_plans';
		...
		public static function get_label(): string {
			return esc_html__( 'Razorpay', 'ebox' );
		}
		...
		protected function configure(): void {
			$this->settings = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_Razorpay' );
			...
			if ( ! class_exists( 'Razorpay\Api\Api' ) ) {
				require_once ebox_LMS_LIBRARY_DIR . '/razorpay-php/Razorpay.php';
			}
		 ...
		 * }
		 */
		private function map_payment_options( ebox_Product_Model $product ): array {
			...
			/** This filter is documented in includes/payments/gateways/class-ebox-stripe-gateway.php */
			$price = apply_filters(
				'ebox_get_price_by_coupon',
				$pricing->price,
				$product->get_id(),
				$this->user->ID
			);
			...
			if ( $price_in_subunits < 1 ) {
				throw new Exception( __( 'The minimum Price is 1.', 'ebox' ) );
			}
			 ...
			 */
			$options = apply_filters( 'ebox_payment_options_razorpay', $options );
		...
		public function process_webhook(): void {
			...
			if ( ! isset( $_GET['ebox-integration'] ) || $this->get_name() !== $_GET['ebox-integration'] ) {
				return;
			}
		 ...
		 *     payload: array{
		 *         id: string,
		 *         notes: array{is_ebox?: bool, ebox_version?: string, post_id?: int, operation_id?: string},
		...
		public function map_payment_button_markup( array $params, WP_Post $post ): string {
			...
			>
				<input class="<?php echo esc_attr( ebox_Payment_Button::map_button_class_name() ); ?>" id="<?php echo esc_attr( ebox_Payment_Button::map_button_id() ); ?>" type="submit" value="<?php echo esc_attr( $button_label ); ?>">
			</form>
			<?php
			$buffer = ob_get_clean();
		...
		private function find_or_create_customer_id(): string {
			...
			try {
				...
				if ( is_null( $customer ) ) {
					throw new Exception(
						__( 'Razorpay customer creation failed. And an existing related Razorpay customer was not found.', 'ebox' )
					);
				}
			}
		...
		private function create_subscription_id(
			...
			ebox_Pricing_DTO $pricing,
			ebox_Product_Model $product,
			array $required_notes
		): string {
			if ( 0 === $pricing->recurring_times ) {
				throw new Exception( __( 'Razorpay does not support infinite subscriptions.', 'ebox' ) );
				...
				throw new Exception( __( 'For daily plans, the minimum billing cycle value is 7.', 'ebox' ) );
			}
			...
			$transaction_meta_dto = ebox_Transaction_Meta_DTO::create(
				array(
					...
					ebox_Transaction_Model::$meta_key_has_free_trial => $has_trial && 0.0 === $pricing->trial_price,
				)
			);
		 ...
		 */
		private function find_or_create_plan_id( int $amount, int $interval, string $period, ebox_Product_Model $product ): string {
			$plan_options = array(
		 ...
		 *     event: string,
		 *     contains: array<string>,
		 *     payload: array{id: string, notes: array{is_ebox?: bool, ebox_version?: string, post_id?: int, operation_id?: string}}
		 * } $data
		 *
		 ...
		 */
		protected function map_transaction_meta( $data, ebox_Product_Model $product ): ebox_Transaction_Meta_DTO {
			...
			if ( empty( $ebox_version ) ) {
				...
				if ( isset( $event_meta['pricing'] ) ) {
					...
					if ( $is_subscription_event ) {
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]                          = $legacy_pricing;
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]['recurring_times']       = $legacy_pricing['no_of_cycles'] ?? 0;
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]['duration_value']        = $legacy_pricing['pricing_billing_p3'] ?? 0;
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]['duration_length']       = $legacy_pricing['pricing_billing_t3'] ?? '';
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]['trial_price']           = $legacy_pricing['trial_price'] ?? 0;
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]['trial_duration_value']  = $legacy_pricing['trial_duration_p1'] ?? 0;
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ]['trial_duration_length'] = $legacy_pricing['trial_duration_t1'] ?? '';
					} else {
						$event_meta[ ebox_Transaction_Model::$meta_key_pricing_info ] = $legacy_pricing;
					}
		...
		private function setup_products_or_fail( int $post_id ): array {
			...
			$products = ebox_Product_Model::find_many( array( $post_id ) );
		 ...
		 *     payload: array{
		 *         id: string,
		 *         notes: array{is_ebox?: bool, ebox_version?: string, post_id?: int, user_id?: int, operation_id?: string},
		 *         order?: array{entity: array<mixed>},
		 *         subscription?: array{entity: array<mixed>},
		 *     }
		 * }
		...
		private function validate_webhook_event_or_fail(): array {
			 ...
			 * Event.
			 *
			 * @var array{event: string, contains: string[], payload: array{id: string, notes: array{is_ebox?: bool, ebox_version?: string, post_id?: int, operation_id?: string}}} $event
			 */
			...
			if ( empty( $entity['notes']['is_ebox'] ) ) {
				$this->log_error( 'Webhook event notes validation failed. Missing key "is_ebox" in notes. This event was not created by ebox.' );

				wp_send_json_error(
					new WP_Error(
						'bad_request',
						'Event notes validation failed. Missing key "is_ebox" in notes. This event was not created by ebox.'
					),
					422
				);
			}
