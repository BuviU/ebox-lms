 ...
 */
function ebox_transaction_create( array $meta_fields, WP_Post $post, WP_User $user, int $parent_transaction_id = 0 ): int {
	...
	if ( $parent_transaction_id <= 0 ) {
		...
		$parent_transaction_id = wp_insert_post(
			array(
				'post_title'  => __( 'Parent transaction', 'ebox' ),
		...
		update_post_meta( $parent_transaction_id, ebox_Transaction_Model::$meta_key_is_parent, true );
	 ...
	 */
	$transaction_title = apply_filters( 'ebox_transaction_post_title', $post->post_title, $post, $user, $meta_fields );
	 ...
	 */
	do_action( 'ebox_transaction_created', $transaction_id );
