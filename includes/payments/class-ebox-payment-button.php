...
if ( ! class_exists( 'ebox_Payment_Button' ) ) {
	/**
	 * Class for mapping payment button HTML markup.
	 *
	 * @since 4.5.0
	 */
	class ebox_Payment_Button {
		...
		public function __construct( $post ) {
			...
			$this->registration_page_id     = ebox_registration_page_get_id();
			$this->registration_page_is_set = ebox_registration_page_is_set();
		...
		public function map(): string {
			...
			try {
				$product = ebox_Product_Model::create_from_post( $this->post );
			} catch ( Exception $e ) {
				return '';
			}
			...
			switch ( $product_pricing_type ) {
				case ebox_PRICE_TYPE_OPEN:
					return $this->button_open();

				case ebox_PRICE_TYPE_CLOSED:
					return $this->button_closed();

				case ebox_PRICE_TYPE_FREE:
					return $this->button_free();

				case ebox_PRICE_TYPE_PAYNOW:
				case ebox_PRICE_TYPE_SUBSCRIBE:
		...
		private function button_open(): string {
			 ...
			 */
			return (string) apply_filters( 'ebox_payment_button_open', '', $this->default_payment_params );
		}
		...
		private function button_closed(): string {
			 ...
			 * } $post_settings
			 */
			$post_settings = ebox_get_setting( $this->post->ID ); // @phpstan-ignore-line -- $this->post is checked before in the map().
			...
			if ( ! empty( $button_url ) ) {
				...
				$button = sprintf(
					...
					esc_attr( $this->map_button_class_name( 'ebox-button-closed' ) ),
			 ...
			 *
			 * @since 2.1.0
			 * @deprecated 4.5.0 Use the {@see 'ebox_payment_button_closed'} filter instead.
			 *
		...
		private function button_free(): string {
			...
			$button = '
				<form action="' . esc_url( $permalink ) . '" method="post">
					...
					<input type="submit" class="' . esc_attr( $this->map_button_class_name( 'ebox-button-free' ) ) . '" id="' . esc_attr( $this->map_button_id() ) . '" value="' . esc_attr( $this->button_label ) . '" />
				</form>
			';
			 ...
			 * @return string Payment button HTML markup.
			 */
			return (string) apply_filters( 'ebox_payment_button_free', $button, $this->default_payment_params );
		}
		...
		private function button_paid(): string {
			...
			if ( empty( $buttons ) ) {
				 ...
				 * @return string Payment button HTML markup.
				 */
				return (string) apply_filters( 'ebox_payment_button', '', $this->default_payment_params );
			}

			$wrapper_start = '<div class="ebox_checkout_buttons">';
			$wrapper_end   = '</div>';

			/** This filter is documented in includes/payments/class-ebox-payment-button.php */
			...
			$button_dropdown_trigger = sprintf(
				'<button class="%s" id="%s" data-jq-dropdown="#ebox-payment-button-dropdown">%s</button>',
				esc_attr( $this->map_button_class_name( 'ebox_checkout_button' ) ),
				esc_attr( $this->map_button_id() ),
			 ...
			 *
			 * @deprecated 4.5.0 Use the {@see 'ebox_payment_button_dropdown_trigger'} filter instead.
			 *
			 ...
			 */
			$button = (string) apply_filters_deprecated(
				'ebox_dropdown_payment_button',
				array( $wrapper_start . $button_dropdown_trigger . $wrapper_end ),
				'4.5.0',
				'ebox_payment_button_dropdown_trigger'
			);
			 ...
			 */
			return (string) apply_filters( 'ebox_payment_button_dropdown_trigger', $button );
		}
		...
		private function map_label(): string {
			if ( ebox_is_course_post( $this->post ) ) {
				$label_key = ebox_Custom_Label::$button_take_course;
			} elseif ( ebox_is_team_post( $this->post ) ) {
				$label_key = ebox_Custom_Label::$button_take_team;
			} else {
				return '';
			}

			return ebox_Custom_Label::get_label( $label_key );
		}
		...
		private function print_dropdown_buttons_in_footer( string $button_html ): void {
			$dropdown_buttons  = '<div id="ebox-payment-button-dropdown" class="jq-dropdown jq-dropdown-tip checkout-dropdown-button">';
			 ...
			 */
			$dropdown_buttons = (string) apply_filters( 'ebox_payment_button_dropdown', $dropdown_buttons );
