<?php
/**
 * Utility class to contain all the custom databases used within ebox.
 *
 * @since 2.6.0
 * @package ebox
 */
	...
	class LDLMS_DB {
		...
		public static function init( $force_reload = false ) {
			...
			if ( ( true === $force_reload ) || ( ! isset( self::$tables[ $blog_id ] ) ) || ( empty( self::$tables[ $blog_id ] ) ) ) {
				 ...
				 */
				self::$tables_base = apply_filters( 'ebox_custom_database_tables', self::$tables_base );
		...
		public static function get_table_prefix( $table_section = '' ) {
			 ...
			 * @param string $table_section Table section prefix.
			 */
			return apply_filters( 'ebox_table_prefix', $table_prefix, $table_section );
		}
		...
		public static function get_table_sub_prefix( $table_section = '' ) {
			...
			switch ( $table_section ) {
				...
				case 'wpproquiz':
					if ( ( defined( 'ebox_PROQUIZ_DATABASE_PREFIX_SUB' ) ) && ( ebox_PROQUIZ_DATABASE_PREFIX_SUB ) ) {
						$table_sub_prefix = esc_attr( ebox_PROQUIZ_DATABASE_PREFIX_SUB );
					} else {
						...
						$data_upgrade_proquiz_tables = ebox_Admin_Data_Upgrades::get_instance( 'ebox_Admin_Data_Upgrades_Rename_WPProQuiz_Tables' );
						...
						if ( isset( $data_settings['prefixes']['current'] ) ) {
							...
							if ( ( defined( 'ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT' ) ) && ( ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT ) ) { // @phpstan-ignore-line
								$table_sub_prefix = esc_attr( ebox_PROQUIZ_DATABASE_PREFIX_SUB_DEFAULT );
							} else {
				...
				case 'activity':
					if ( ( defined( 'ebox_LMS_DATABASE_PREFIX_SUB' ) ) && ( ebox_LMS_DATABASE_PREFIX_SUB ) ) { // @phpstan-ignore-line
						$table_sub_prefix = esc_attr( ebox_LMS_DATABASE_PREFIX_SUB );
					} else {
						$table_sub_prefix = 'ebox_';
					}
					break;
			 ...
			 * @param string $table_section Table section prefix.
			 */
			return apply_filters( 'ebox_table_sub_prefix', $table_sub_prefix, $table_section );
		}
		...
		public static function get_table_status_info( $table_name = '' ) {
			...
			if ( ! empty( $table_name ) ) {
				 ...
				 */
				if ( true === apply_filters( 'ebox_support_db_tables_info', true ) ) {
					if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table_name ) ) === $table_name ) { // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
						 ...
						 */
						if ( true === apply_filters( 'ebox_support_db_tables_rows', true ) ) {
		...
		public static function get_db_server_info() {
			...
			$db_server_info = array(
				...
				'mysqldb_version_min' => ebox_MIN_MYSQL_VERSION,
				'mariadb_active'      => false,
				'mariadb_version_min' => ebox_MIN_MARIA_VERSION,
				'db_version_found'    => '',
			);
