...
if ( ( ! class_exists( 'ebox_Admin_Settings_Data_Upgrades' ) ) && ( class_exists( 'ebox_Admin_Data_Upgrades' ) ) ) {
	/**
	 * Deprecated Class for admin settings data upgrades
	 */
	class ebox_Admin_Settings_Data_Upgrades {
		// phpcs:ignore Squiz.Commenting.FunctionComment
		public static function get_instance( $instance_key = '' ) {
			if ( function_exists( '_deprecated_function' ) ) {
				_deprecated_function( 'ebox_Admin_Settings_Data_Upgrades::get_instance()', '2.6.0', 'ebox_Admin_Data_Upgrades::get_instance()' );
			}

			return ebox_Admin_Data_Upgrades::get_instance();
		}
	}
}
