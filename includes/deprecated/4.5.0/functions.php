...
if ( ! function_exists( 'ebox_footer_payment_buttons' ) ) {
	/**
	 ...
	 */
	function ebox_footer_payment_buttons() {
...
if ( ! function_exists( 'ebox_get_footer' ) ) {
	/**
...
if ( ! function_exists( 'ebox_get_payment_button_label' ) ) {
	/**
	 ...
	 * @return string
	 */
	function ebox_get_payment_button_label( $post ): string {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		if ( class_exists( 'ebox_Custom_Label' ) ) {
			if ( ebox_is_course_post( $post ) ) {
				$button_label = ebox_Custom_Label::get_label( 'button_take_this_course' );
			} elseif ( ebox_is_team_post( $post ) ) {
				$button_label = ebox_Custom_Label::get_label( 'button_take_this_team' );
			}
		} else {
			if ( ebox_is_course_post( $post ) ) {
				$button_label = __( 'Take This Course', 'ebox' );
			} elseif ( ebox_is_team_post( $post ) ) {
				$button_label = __( 'Enroll in Team', 'ebox' );
			}
		}
...
if ( ! function_exists( 'ebox_paypal_init_user_purchase_hash' ) ) {
	/**
	 ...
	 * @return string
	 */
	function ebox_paypal_init_user_purchase_hash( $user_id = 0, $product_id = 0 ) {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
...
if ( ! function_exists( 'ebox_paypal_get_purchase_success_redirect_url' ) ) {
	/**
	 ...
	 * @deprecated 4.5.0
	 *
	 * @param int $post_id Course or Team post ID purchased.
	 *
	 * @return string $return_url
	 */
	function ebox_paypal_get_purchase_success_redirect_url( $post_id = 0 ) {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		if ( ! empty( $post_id ) ) {
			...
			$type_slug = '';
			if ( ebox_get_post_type_slug( 'course' ) === get_post_type( $post_id ) ) {
				$type_slug = 'course';
			} elseif ( ebox_get_post_type_slug( 'team' ) === get_post_type( $post_id ) ) {
				$type_slug = 'team';
			}

			if ( ! empty( $type_slug ) ) {
				$price_type = ebox_get_setting( $post_id, $type_slug . '_price_type' );
				if ( ! empty( $price_type ) ) {
					$enrollment_url = ebox_get_setting( $post_id, $type_slug . '_price_type_' . $price_type . '_enrollment_url' );
		...
		if ( empty( $return_url ) ) {
			$paypal_settings = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_PayPal' );
		...
		if ( empty( $return_url ) ) {
			$ld_registration_success_page_id = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Registration_Pages', 'registration_success' );
		...
		if ( ( empty( $return_url ) ) && ( ! empty( $post_id ) ) ) {
			 ...
			 * we return the customer to the course/team.
			 */
		 ...
		 * @param int    $post_id      The Course/Team Post ID.
		 */
		$return_url = apply_filters( 'ebox_paypal_purchase_success_url', $return_url, $post_id );

		return $return_url;
	}
}

if ( ! function_exists( 'ebox_paypal_get_purchase_cancel_redirect_url' ) ) {
	/**
	 ...
	 * @return string $return_url
	 */
	function ebox_paypal_get_purchase_cancel_redirect_url( $post_id = 0 ) {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		if ( empty( $return_url ) ) {
			if ( ! empty( $post_id ) ) {
				if ( empty( $return_url ) ) {
					/**
					 * If the PayPal cancel URL is empty we return the customer to the course/team.
					 */
					$return_url = get_permalink( $post_id );
				}
			}
		 ...
		 * @param int    $post_id      The Course/Team Post ID.
		 */
		$return_url = apply_filters( 'ebox_paypal_purchase_cancel_url', $return_url, $post_id );

		return $return_url;
	}
}

if ( ! function_exists( 'ebox_send_purchase_invoice_email' ) ) {
	/**
	 * Sends the course/team purchase invoice email
	 *
	 * @since      4.2.0
	 * @deprecated 4.5.0
	 *
	 ...
	 * @return void
	 */
	function ebox_send_purchase_invoice_email( int $transaction_id ): void {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		if ( ! in_array( $purchased_post->post_type, ebox_get_post_type_slug( array( LDLMS_Post_Types::COURSE, LDLMS_Post_Types::GROUP ) ), true ) ) {
			return;
		}
		 ...
		 * @param int    $post_id       Post ID.
		 */
		$email_setting['subject'] = apply_filters( 'ebox_purchase_invoice_email_subject', $email_setting['subject'], $user_id, $post_id );
		if ( ! empty( $email_setting['subject'] ) ) {
			$email_setting['subject'] = ebox_emails_parse_placeholders( $email_setting['subject'], $placeholders );
		}
		 ...
		 * @param int    $post_id       Post ID.
		 */
		$email_setting['message'] = apply_filters( 'ebox_purchase_invoice_email_message', $email_setting['message'], $user_id, $post_id );
		...
		$pdf_data = array(
			'purchaser_name'   => ebox_emails_parse_placeholders( $email_setting['purchaser_name'], $placeholders ),
			'user_id'          => $user_id,
			'purchase_date'    => $purchase_date,
			'transaction_id'   => $transaction_id,
			'transaction_meta' => ebox_transaction_get_payment_meta( $transaction_id ),
			...
			'filename'         => ebox_purchase_invoice_filename( $user_id, $post_id ),
			'filepath'         => ebox_purchase_invoice_filepath( $post_id ),
		);
...
if ( ! function_exists( 'ebox_transaction_add_ebox_version' ) ) {
	/**
	 ...
	 * @return void
	 */
	function ebox_transaction_add_ebox_version( int $transaction_id ): void {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}

		update_post_meta( $transaction_id, 'ebox_version', ebox_VERSION );
	}
}

if ( ! function_exists( 'ebox_transaction_get_payment_meta' ) ) {
	/**
	 ...
	 */
	function ebox_transaction_get_payment_meta( int $transaction_id = 0 ): array {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		if ( true === array_key_exists( 'coupon', $transaction_meta ) ) {
			$meta = ebox_transaction_get_coupon_meta( $transaction_id );
		} else {
			if ( 'stripe' === $transaction_processor ) {
				$meta = ebox_transaction_get_stripe_meta( $transaction_id );
			} elseif ( 'paypal' === $transaction_processor ) {
				$meta = ebox_transaction_get_paypal_meta( $transaction_id );
			} elseif ( 'razorpay' === $transaction_processor ) {
				$meta = ebox_transaction_get_razorpay_meta( $transaction_id );
			} else {
				$meta = array();
			}
		}
...
if ( ! function_exists( 'ebox_transaction_get_razorpay_meta' ) ) {
	/**
	 ...
	 * @return array Array of transaction meta.
	 */
	function ebox_transaction_get_razorpay_meta( int $transaction_id = 0 ): array {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		$meta = array(
			...
			'item_price' => ebox_get_price_formatted( $pricing_meta['price'], $pricing_meta['currency'] ),
		);
		...
		if ( ! empty( $pricing_meta['trial_price'] ) ) {
			$trial_meta = array(
				'trial_price'           => ebox_get_price_formatted( $pricing_meta['trial_price'], $pricing_meta['currency'] ),
...
if ( ! function_exists( 'ebox_transaction_get_paypal_meta' ) ) {
	/**
	 ...
	 * @return array Array of transaction meta.
	 */
	function ebox_transaction_get_paypal_meta( int $transaction_id = 0 ): array {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		$meta = array(
			'item_name'  => get_the_title( $transaction_meta['post_id'][0] ),
			'item_price' => ebox_get_price_formatted( $transaction_meta['mc_gross'][0], $currency ),
		);
		...
		if ( 'subscr_payment' === $transaction_meta['txn_type'][0] ) {
			...
			$subscribe_meta  = array(
				'item_price'      => ebox_get_price_formatted( $transaction_meta['amount3'][0], $currency ),
		...
		if ( ! empty( $transaction_meta['trial_price'][0] ) ) {
			...
			$trial_meta            = array(
				'trial_price'           => ebox_get_price_formatted( $transaction_meta['trial_price'][0], $currency ),
...
if ( ! function_exists( 'ebox_transaction_get_final_price' ) ) {
	/**
	 ...
	 */
	function ebox_transaction_get_final_price( int $transaction_id ): int {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		$transaction_meta      = get_post_meta( $transaction_id );
		$transaction_processor = $transaction_meta[ ebox_Transaction_Model::$meta_key_gateway_name ][0];
		// remove Stripe's metadata from meta array.
		if ( true === array_key_exists( 'stripe_metadata', $transaction_meta ) ) {
			unset( $transaction_meta['stripe_metadata'] );
		}
...
if ( ! function_exists( 'ebox_transaction_get_stripe_meta' ) ) {
	/**
	 ...
	 * @return array Array of transaction meta.
	 */
	function ebox_transaction_get_stripe_meta( int $transaction_id = 0 ): array {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		$meta = array(
			'item_name'  => get_the_title( $transaction_meta['post_id'][0] ),
			'item_price' => ebox_get_price_formatted( $transaction_meta['stripe_price'][0], $currency ),
		);
		if ( ! empty( $transaction_meta['pricing_billing_p3'][0] ) ) {
			...
			$subscribe_meta  = array(
				'item_price'      => ebox_get_price_formatted( $transaction_meta['subscribe_price'][0], $currency ),
...
if ( ! function_exists( 'ebox_transaction_get_coupon_meta' ) ) {
	/**
	 ...
	 * @return array Array of transaction meta.
	 */
	function ebox_transaction_get_coupon_meta( int $transaction_id = 0 ): array {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '4.5.0' );
		}
		...
		$discount = ebox_COUPON_TYPE_FLAT === $coupon_meta['type']
			? ebox_get_price_formatted( $coupon_meta['discount'], $currency )
			: $coupon_meta['amount'] . '%';

		return array(
			'item_name'             => get_the_title( $transaction_data['post_id'][0] ),
			'item_price'            => ebox_get_price_formatted( $coupon_meta['price'], $currency ),
			'item_coupon'           => $coupon_meta['code'],
			'item_discount'         => $discount,
			'item_discounted_price' => ebox_get_price_formatted( $coupon_meta['discounted_price'], $currency ),
		);
	}
}
