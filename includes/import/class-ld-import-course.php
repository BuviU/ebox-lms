<?php
/**
 * ebox Import Courses CPT
 *
 * This file contains functions to handle import of the ebox CPT Course
 *
 * @package ebox\Import
 * @since 1.0.0
 */
...
if ( ( ! class_exists( 'ebox_Import_Course' ) ) && ( class_exists( 'ebox_Import_Post' ) ) ) {
	/**
	 * Class to import courses.
	 */
	class ebox_Import_Course extends ebox_Import_Post {
		/**
		 * Version
		 *
		 * @var string Version.
		 ...
		 */
		protected $dest_post_type = 'ebox-courses';
		 ...
		 */
		protected $source_post_type = 'ebox-courses';
		...
		public function set_post_prerequisite( $dest_post_id = 0, $prerequisite_post_id = 0 ) {
			if ( ( ! empty( $dest_post_id ) ) && ( ! empty( $prerequisite_post_id ) ) ) {
				...
				$prerequisite_posts   = ebox_get_course_prerequisite( $dest_post_id );
		...
		public function set_course_prerequisite( $course_id = 0, $course_prerequisites = array() ) {
			if ( ! empty( $course_id ) ) {
				...
				return ebox_update_setting( $course_id, 'course_prerequisite', (array) $course_prerequisites );
			}
			return false;
		}
		...
		public function add_user_progress( $user_id = 0, $course_id = 0, $args = array(), $force = false ) {
			if ( ( ! empty( $user_id ) ) && ( ! empty( $course_id ) ) ) {
				...
				if ( is_a( $user, 'WP_User' ) ) {
					$user_course_meta = get_user_meta( $user_id, '_ebox-course_progress', true );
					...
					if ( true === $_changed ) {
						update_user_meta( $user_id, '_ebox-course_progress', $user_course_meta );
					}
				}
			}
		}
