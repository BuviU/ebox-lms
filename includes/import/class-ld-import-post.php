 ...
 * ebox Import CPT
 *
 * This file contains functions to handle import of the ebox CPT Courses, modules, Topics, Quizzes
 *
 * @package ebox\Import
 * @since 1.0.0
 */
...
if ( ! class_exists( 'ebox_Import_Post' ) ) {
	/**
	 * Class to import posts.
	 */
	class ebox_Import_Post {
			...
			if ( ! empty( $source_post_id ) ) {
				...
				if ( ( $source_post ) && ( $source_post->post_type == $this->source_post_type ) && ( is_a( $source_post, 'WP_Post' ) ) ) {
					...
					if ( ( empty( $previous_imported_post_id ) ) || ( true == $force_copy ) ) {
						 ...
						 */
						$dest_post    = apply_filters( 'ebox_duplicate_post_array', $dest_post, $source_post );
						...
						if ( $dest_post_id > 0 ) {
							...
							$dest_post_meta = ebox_CPT_Instance::$instances[ $dest_post->post_type ]->get_settings_values( $dest_post->post_type );
							 ...
							 */
							$dest_post_meta = apply_filters( 'ebox_sensei_import_meta', $dest_post_meta, $source_post, $dest_post );
