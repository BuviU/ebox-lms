<?php
/**
 * ebox Import Quiz Statistics
 *
 * This file contains functions to handle import of the ebox Quiz Statistics
 *
 * @package ebox\Import
 * @since 1.0.0
 */
...
if ( ( ! class_exists( 'ebox_Import_Quiz_Statistics' ) ) && ( class_exists( 'ebox_Import_Post' ) ) ) {
	/**
	 * Class to import quiz statistics
	 */
	class ebox_Import_Quiz_Statistics extends ebox_Import_Post {
		/**
		 * Version
		 *
		 * @var string Version.
		...
		public function migrate_file_upload_to_essay( $file_upload_full = '', $question_id = 0 ) {
			if ( ! empty( $file_upload_full ) ) {
				...
				// This logic was copied from LD core includes/quiz/ld-quiz-essay.php ebox_essay_fileupload_process().
				$filename = ebox_clean_filename( basename( $file_upload_full ) );
				...
				while ( file_exists( $upload_dir_path . '/' . $filename ) ) {
					 ...
					 * @param string $extension   File extension.
					 */
					$filename = apply_filters( 'ebox_essay_upload_filename_dup', $filename, $question_id, $file_title, $i, $filetype['ext'] );
				}
