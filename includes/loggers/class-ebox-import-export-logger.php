...
if ( ! class_exists( 'ebox_Import_Export_Logger' ) && class_exists( 'ebox_Logger' ) ) {
	/**
	 * Import/Export logger class.
	 *
	 * @since 4.5.0
	 */
	class ebox_Import_Export_Logger extends ebox_Logger {
		/**
		 * Log type import.
		 *
		 * @since 4.5.0
		...
		public function get_label(): string {
			switch ( $this->log_type ) {
				case self::$log_type_import:
					return esc_html__( 'Data Importing', 'ebox' );

				case self::$log_type_export:
					return esc_html__( 'Data Exporting ', 'ebox' );
