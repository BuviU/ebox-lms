...
if ( ! class_exists( 'ebox_Logger' ) ) {
	/**
	 * Logger class.
	 *
	 * @since 4.5.0
	 */
	abstract class ebox_Logger {
		private const PATH_ID                   = 'ebox_logs';
		private const PATH                      = 'ebox' . DIRECTORY_SEPARATOR . 'logs';
		private const EXTENSION                 = '.log';
		private const SUFFIXES_META_KEY         = 'ebox_log_suffixes';
		...
		private function maybe_rotate_file() {
			 ...
			 * @param ebox_Logger $ebox_logger Logger instance.
			 */
			$max_file_size = apply_filters( 'ebox_logger_max_file_size', self::DEFAULT_MAX_SIZE_IN_BYTES, $this );
		...
		public function get_download_url(): string {
			return ebox_Admin_File_Download_Handler::get_download_url(
				self::PATH_ID,
		...
		public static function init_log_directory() {
			...
			ebox_put_directory_index_file( trailingslashit( $log_dir ) . 'index.php' );

			ebox_Admin_File_Download_Handler::register_file_path( self::PATH_ID, $log_dir );

			self::$file_protection_instructions = ebox_Admin_File_Download_Handler::try_to_protect_file_path( $log_dir );
		}
		...
		public function is_enabled(): bool {
			...
			$enabled = ebox_Settings_Section::get_section_setting(
				'ebox_Settings_Section_Logs',
				$this->get_name()
			);
