<?php
/**
 * ebox class for displaying the course wizard.
 *
 * @package    ebox
 * @since      4.1.0
 */
...
if ( ! class_exists( 'ebox_Course_Wizard' ) ) {
	/**
	 * Course wizard class.
	 */
	class ebox_Course_Wizard {

		const PLAYLIST_PROCESS_SERVER_ENDPOINT   = 'https://licensing.ebox.com/services/wp-json/ebox-playlist-parser/v1';
		const PLAYLIST_PROCESS_SERVER_SSL_VERIFY = true; // false only for local testing.

		const HANDLE = 'ebox-course-wizard';

		const LICENSE_KEY       = 'nss_plugin_license_ebox_lms';
		const LICENSE_EMAIL_KEY = 'nss_plugin_license_email_ebox_lms';
		...
		public function add_wizard_button() {
			...
			if ( is_object( $screen ) && 'edit-' . ebox_get_post_type_slug( 'course' ) === $screen->id ) {
				?>
				<script>
					window.onload = function() {
						...
						if (settingsElem.length === 1 && newEntityElem.length === 1) {
							...
							var wizardText = document.createTextNode('<?php esc_html_e( 'Create from Video Playlist', 'ebox' ); ?>');
		...
		public function register_menu() {
			add_menu_page(
				__( 'Course Creation Wizard', 'ebox' ),
				__( 'Course Creation Wizard', 'ebox' ),
		...
		public function enqueue_scripts() {
			...
			if ( is_object( $screen ) && 'toplevel_page_' . self::HANDLE === $screen->id ) {
				wp_register_style(
					'ld-tailwindcss',
					ebox_LMS_PLUGIN_URL . 'assets/css/ld-tailwind.css',
					array(),
				...
				wp_register_script(
					self::HANDLE,
					ebox_LMS_PLUGIN_URL . 'assets/js/ebox-course-wizard' . ebox_min_asset() . '.js',
					array(),
					ebox_SCRIPT_VERSION_TOKEN,
					true
				);
				...
				wp_localize_script(
					...
					'ldCourseWizard',
					array(
						'valid_recurring_paypal_day_max'   => ebox_billing_cycle_field_frequency_max( 'D' ),
						'valid_recurring_paypal_week_max'  => ebox_billing_cycle_field_frequency_max( 'W' ),
						'valid_recurring_paypal_month_max' => ebox_billing_cycle_field_frequency_max( 'M' ),
						'valid_recurring_paypal_year_max'  => ebox_billing_cycle_field_frequency_max( 'Y' ),
						'buttons'                          => array(
							'youtube' => array(
								'label'     => __( 'Load data from YouTube', 'ebox' ),
								'img_class' => 'ld-w-8 ld-inline ld-mr-1',
								'img_src'   => esc_url( ebox_LMS_PLUGIN_URL . 'assets/images/youtube_icon.png' ),
								'img_alt'   => esc_attr(
									__( 'YouTube icon', 'ebox' )
								),
							),
							'vimeo'   => array(
								'label'     => __( 'Load data from Vimeo', 'ebox' ),
								'img_class' => 'ld-w-6 ld-inline ld-mr-1',
								'img_src'   => esc_url( ebox_LMS_PLUGIN_URL . 'assets/images/vimeo_icon.png' ),
								'img_alt'   => esc_attr(
									__( 'Vimeo icon', 'ebox' )
								),
							),
							'wistia'  => array(
								'label'     => __( 'Load data from Wistia', 'ebox' ),
								'img_class' => 'ld-w-7 ld-inline',
								'img_src'   => esc_url( ebox_LMS_PLUGIN_URL . 'assets/images/wistia_icon.png' ),
								'img_alt'   => esc_attr(
									__( 'Wistia icon', 'ebox' )
								),
							),
		...
		public function process_url_action() {
			if ( ! isset( $_REQUEST['ld_course_wizard_playlist_process'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['ld_course_wizard_playlist_process'] ) ), 'ld_course_wizard_playlist_process' ) ) {
				ebox_safe_redirect( admin_url( 'admin.php?page=' . self::HANDLE ) ); // wrong call.
			}

			$playlist_url = isset( $_REQUEST['playlist_url'] ) ? esc_url_raw( wp_unslash( $_REQUEST['playlist_url'] ) ) : '';
			if ( empty( $playlist_url ) ) {
				ebox_safe_redirect( admin_url( 'admin.php?page=' . self::HANDLE ) ); // no playlist URL.
			}
		...
		public function create_course_action() {
			...
			if ( empty( $transient_data['playlist_data'] ) ) {
				ebox_safe_redirect( admin_url( 'admin.php?page=' . self::HANDLE ) ); // no transient data.
			}
			...
			ebox_safe_redirect( admin_url( 'edit.php?post_type=' . ebox_get_post_type_slug( 'course' ) ) );
		}
		...
		public function process_url( $playlist_url ) {
			...
			if ( ! $body || ! empty( $body->message ) ) {
				$this->update_processing_data( $playlist_url, 'error_message', ! empty( $body->message ) ? $body->message : __( 'Error on access ebox service. Please try it again in a few minutes.', 'ebox' ) );
		...
		private function get_processing_data() {
			...
			if ( false !== $transient_data ) {
				...
				if ( ! isset( $_GET['refresh'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					$process_data['error_message'] = __( 'Request expired. Please try it again.', 'ebox' );
				}
			}
		...
		private function get_playlist_data( $playlist_url ) {
			...
			$body = json_decode( wp_remote_retrieve_body( $request ) );
			if ( ! $body || ! empty( $body->message ) ) {
				return isset( $body->message ) ? $body->message : __( 'Error on access ebox service. Please try it again in a few minutes.', 'ebox' );
			}
		...
		<div class="ld-container ld-mx-auto">
			<div class="ld-flex ld-flex-wrap ld-flex-col ld-items-center">
				<div class="ld-flex ld-mt-6">
					<p class="ld-text-4xl">
						...
						echo sprintf(
							...
							esc_html( ebox_Custom_Label::label_to_lower( 'course' ) )
						);
						?>
					</p>
				</div>
				<div class="ld-flex ld-mt-2">
					<p class="ld-text-xl">
						...
						echo sprintf(
							// translators: course.
							esc_html_x(
								'You can use a YouTube Playlist, a Vimeo Showcase or a Wistia Project URL to create a ebox %s in a few minutes.',
								'placeholder: course',
								'ebox'
							),
							esc_html( ebox_Custom_Label::label_to_lower( 'course' ) )
				...
				<div class="ld-flex ld-flex-wrap ld-flex-col ld-items-center ld-mt-10 ld-w-full">
					...
					if ( ! ebox_is_ebox_license_valid() ) {
						...
						<div class="notice notice-error">
							<p><?php echo esc_html__( 'Please activate your license to use this feature.', 'ebox' ); ?></p>
						</div>
						<?php
					} else {
				...
				<div class="ld-flex ld-mt-8">
					<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=' . ebox_get_post_type_slug( 'course' ) ) ); ?>" class="button">
						<?php
						echo sprintf(
							// translators: course.
							esc_html_x(
					...
					<?php if ( $process_data['can_create_course'] ) { ?>
						<form id="ld_cw_create_course_form" method="post" action="<?php echo esc_url_raw( admin_url( 'admin-post.php' ) ); ?>">
								...
								<button id="ld_cw_create_course_btn" style="margin-left: 10px" type="button" class="button button-primary"><?php echo esc_html__( 'Create the course', 'ebox' ); ?></button>
						</form>
					<?php } ?>
				</div>
			</div>
		...
		private function render_url_process_step( $process_data ) {
			...
			<div class="ld-flex ld-flex-wrap ld-flex-col ld-bg-white ld-box-content ld-w-1/2 ld-p-8 ld-rounded-md ld-shadow-lg" id="ld-course-wizard_<?php echo esc_attr( self::STEP_URL_PROCESS ); ?>">
				<form method="post" action="<?php echo esc_url_raw( admin_url( 'admin-post.php' ) ); ?>" class="ld-flex">
					...
					<input placeholder="<?php echo esc_attr__( 'Enter playlist URL', 'ebox' ); ?>" id="ld_cw_playlist_url" class="ld-w-full ld-mr-3" type="url" required name="playlist_url" value="<?php echo esc_url( $process_data['playlist_url'] ); ?>" required />
					<button id="ld_cw_load_data_button" class="ld-rounded-md ld-bg-blue-600 ld-text-white ld-py-3 ld-px-4 ld-font-medium" type="submit">
						<span><?php echo esc_html__( 'Load', 'ebox' ); ?></span>
					</button>
				</form>
			</div>
			<?php
		...
		private function render_course_config_step( $process_data ) {
			...
			<div class="ld-flex ld-flex-wrap ld-flex-col ld-bg-white ld-box-content ld-p-8 ld-border-4" id="ld-course-wizard_<?php echo esc_attr( self::STEP_COURSE_CONFIG ); ?>">
				<?php if ( ! empty( $process_data['error_message'] ) ) { ?>
					...
					<a class="button button-primary" href="<?php echo esc_url( $process_data['try_again_url'] ); ?>"><?php echo esc_html__( 'Try it again', 'ebox' ); ?></a>
				<?php } else { ?>
					...
					<div class="w-full ld-flex ld-flex-wrap ld-flex-col ld-py-5 ld-mb-3">
						<div class="w-full ld-flex">
							<p class="ld-text-xl"><?php echo esc_html__( 'How users will gain access to the course?', 'ebox' ); ?></p>
						</div>
							...
							<fieldset>
								<div>
									...
									<label class="ld-font-bold" for="ld_cw_course_price_type_open">
										<?php echo esc_html__( 'Open', 'ebox' ); ?>
									</label>
									<p class="ld-ml-6 ld-mb-3"><?php esc_html_e( 'The course is not protected. Any user can access its content without the need to be logged-in or enrolled.', 'ebox' ); ?></p>
								</div>
								...
								<div>
									...
									<label class="ld-font-bold" for="ld_cw_course_price_type_free">
										<?php echo esc_html__( 'Free', 'ebox' ); ?>
									</label>
									<p class="ld-ml-6 ld-mb-3"><?php esc_html_e( 'The course is protected. Registration and enrollment are required in order to access the content.', 'ebox' ); ?></p>
								</div>
								...
								<div>
									...
									<label class="ld-font-bold" for="ld_cw_course_price_type_buy_now">
										<?php echo esc_html__( 'Buy now', 'ebox' ); ?>
									</label>
									<p class="ld-ml-6 ld-mb-3"><?php esc_html_e( 'The course is protected via the ebox built-in PayPal and/or Stripe. Users need to purchase the course (one-time fee) in order to gain access.', 'ebox' ); ?></p>
									<div id="ld_cw_paynow_div" class="ld-ml-6 ld-py-4 ld-border-l-4" style="display: none">
										<div class="ld-ml-2">
											<label class="ld-mr-10" for="ld_cw_course_price_type_paynow_price">
												<?php echo esc_html__( 'Course Price', 'ebox' ); ?>
											</label>
								...
								<div>
									...
									<label class="ld-font-bold" for="ld_cw_course_price_type_subscribe">
										<?php echo esc_html__( 'Recurring', 'ebox' ); ?>
									</label>
									<p class="ld-ml-6 ld-mb-3"><?php esc_html_e( 'The course is protected via the ebox built-in PayPal and/or Stripe. Users need to purchase the course (recurring fee) in order to gain access.', 'ebox' ); ?></p>
									<div id="ld_cw_subscribe_div" class="ld-ml-6 ld-py-4 ld-border-l-4" style="display: none">
										...
										<div class="ld-mt-4 ld-ml-2">
											<span class="ld-mr-10">
												<?php echo esc_html__( 'Billing Cycle', 'ebox' ); ?>
											</span>
											...
											<select id="ld_cw_course_price_billing_interval" name="ld_cw_course_price_billing_interval" value="">
												<option value=""><?php echo esc_html__( 'select interval', 'ebox' ); ?></option>
												<option value="D"><?php echo esc_html__( 'day(s)', 'ebox' ); ?></option>
												<option value="W"><?php echo esc_html__( 'week(s)', 'ebox' ); ?></option>
												<option value="M"><?php echo esc_html__( 'month(s)', 'ebox' ); ?></option>
												<option value="Y"><?php echo esc_html__( 'year(s)', 'ebox' ); ?></option>
											</select>
										</div>
									</div>
								</div>
								...
								<div>
									...
									<label class="ld-font-bold" for="ld_cw_course_price_type_closed">
										<?php echo esc_html__( 'Closed', 'ebox' ); ?>
									</label>
									<p class="ld-ml-6 ld-mb-3"><?php esc_html_e( 'The course can only be accessed through admin enrollment (manual), team enrollment, or integration (shopping cart or membership) enrollment.', 'ebox' ); ?></p>
								</div>
							</fieldset>
						</div>
					</div>
					...
					<div class="w-full ld-flex ld-flex-wrap ld-flex-col">
						<div class="w-full ld-flex">
							<p class="ld-text-xl"><?php echo esc_html__( 'How users will interact with the content?', 'ebox' ); ?></p>
						</div>
						<div class="w-full ld-flex ld-mt-2">
							<fieldset>
								<div>
									...
									<label class="ld-font-bold" for="ld_cw_course_progression_linear">
										<?php echo esc_html__( 'Linear form', 'ebox' ); ?>
									</label>
									<p class="ld-ml-6 ld-mb-3"><?php esc_html_e( 'Requires the user to progress through the course in the designated step sequence.', 'ebox' ); ?></p>
								</div>
								<div>
									...
									<label class="ld-font-bold" for="ld_cw_course_progression_free">
										<?php echo esc_html__( 'Free form', 'ebox' ); ?>
									</label>
									<p class="ld-ml-6 ld-mb-3"><?php esc_html_e( 'Allows the user to move freely through the course without following the designated step sequence.', 'ebox' ); ?></p>
								</div>
							</fieldset>
						</div>
					</div>
		...
		private function generate_course_creation_message( $process_data ) {
			...
			$course_qty_modules = $process_data['playlist_data']->playlist_count;

			return sprintf(
				// translators: placeholders: course name, modules qty description.
				esc_html_x( 'The course "%1$s" will be created with %2$s.', 'placeholders: course name, modules qty description', 'ebox' ),
				$course_name,
				// translators: placeholders: number of modules.
				sprintf( _n( '%s lesson', '%s modules', $course_qty_modules, 'ebox' ), $course_qty_modules )
			);
		...
		private function create_course_from_playlist( $playlist_data, $course_price_type, $course_disable_lesson_progression, $course_price = null, $course_price_billing_number = null, $course_price_billing_interval = null ) {
			...
			$shared_steps = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) === 'yes';
			...
			foreach ( $playlist_data->playlist_items as $video_data ) {
				...
				ebox_update_setting( $lesson_id, 'lesson_video_enabled', 'on' );
				ebox_update_setting( $lesson_id, 'lesson_video_url', sanitize_text_field( $video_data->video_url ) );
				if ( ! $shared_steps ) {
					ebox_update_setting( $lesson_id, 'course', $course_id );
				}
			}

			ebox_update_setting( $course_id, 'course_price_type', sanitize_text_field( $course_price_type ) );
			ebox_update_setting( $course_id, 'course_disable_lesson_progression', sanitize_text_field( $course_disable_lesson_progression ) );
			if ( 'paynow' === $course_price_type || 'subscribe' === $course_price_type ) {
				ebox_update_setting( $course_id, 'course_price', sanitize_text_field( $course_price ) );
				if ( 'subscribe' === $course_price_type ) {
					ebox_update_setting( $course_id, 'course_price_billing_p3', sanitize_text_field( $course_price_billing_number ) );
					ebox_update_setting( $course_id, 'course_price_billing_t3', sanitize_text_field( $course_price_billing_interval ) );
				}
			}
