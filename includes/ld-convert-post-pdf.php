...
if ( ! function_exists( 'ebox_get_thumb_path' ) ) {
	 ...
	 */
	function ebox_get_thumb_path( $post_id = 0 ) {
		if ( ! empty( $post_id ) ) {
			...
			if ( post_type_supports( $post_type, 'thumbnail' ) ) {
				...
				if ( $thumbnail_id ) {
					 ...
					 *
					 * Filter to allow URLs to be used for certificate background images. ebox-3430
					 *
					 * @since 3.2.0
					 *
...
if ( ! function_exists( 'ebox_post2pdf_conv_image_align_center' ) ) {
	/**
	 ...
	 */
	function ebox_post2pdf_conv_image_align_center( $matches ) {
...
if ( ! function_exists( 'ebox_post2pdf_conv_img_size' ) ) {
	/**
	 ...
	 */
	function ebox_post2pdf_conv_img_size( $matches ) {
		global $q_config;
		$size = null;
...
if ( ! function_exists( 'ebox_certificate_post_shortcode' ) ) {
	 ...
	 */
	function ebox_certificate_post_shortcode( $cert_args = array() ) {
		...
		if ( ( ! $cert_args['cert_post'] ) || ( ! is_a( $cert_args['cert_post'], 'WP_Post' ) ) || ( ebox_get_post_type_slug( 'certificate' ) !== $cert_args['cert_post']->post_type ) ) {
			wp_die( esc_html__( 'Certificate Post does not exist.', 'ebox' ) );
		}

		$cert_args['post_post'] = get_post( $cert_args['post_id'] );
		if ( ( ! $cert_args['post_post'] ) || ( ! is_a( $cert_args['post_post'], 'WP_Post' ) ) ) {
			wp_die( esc_html__( 'Awarded Post does not exist.', 'ebox' ) );
		}
		...
		if ( ( ! empty( $config_lang_tmp ) ) && ( strlen( $config_lang_tmp ) === 3 ) ) {
			$ld_cert_lang_dir = ebox_LMS_LIBRARY_DIR . '/tcpdf/config/lang';
		 ...
		 */
		$ebox_pdf_username = apply_filters( 'ebox_pdf_username', $cert_args['user']->display_name, $cert_args['user_id'], $cert_args['cert_id'] );
		if ( ! empty( $ebox_pdf_username ) ) {
			if ( ! empty( $cert_args['pdf_title'] ) ) {
				$cert_args['pdf_title'] .= " $sep ";
			}
			$cert_args['pdf_title'] .= $ebox_pdf_username;
		}
		...
		if ( 'title' === $cert_args['filename_type'] ) {
			 ...
			 * @param int    $cert_id Certificate post ID.
			 */
			$filename = apply_filters( 'ebox_pdf_filename', $filename, $cert_args['cert_id'] );
		 ...
		 */
		$query_string_params_allowed = apply_filters( 'ebox_certificate_query_string_params_allowed', $query_string_params_allowed, $query_string_params_supported );
		...
		if ( 'F' === $destination ) {
			if ( ( defined( 'ebox_UPLOADS_BASE_URL' ) ) && ( ! empty( ebox_UPLOADS_BASE_URL ) ) ) {
				$cert_args['filename_url'] = ebox_UPLOADS_BASE_URL . '/certificates/' . $filename;
			}

			if ( ( defined( 'ebox_UPLOADS_BASE_DIR' ) ) && ( ! empty( ebox_UPLOADS_BASE_DIR ) ) && ( file_exists( ebox_UPLOADS_BASE_DIR ) ) && ( is_writable( ebox_UPLOADS_BASE_DIR ) ) ) {
				$ld_upload_certificates_dir = trailingslashit( ebox_UPLOADS_BASE_DIR ) . 'certificates';
				if ( ! file_exists( $ld_upload_certificates_dir ) ) {
					if ( wp_mkdir_p( $ld_upload_certificates_dir ) !== false ) {
						...
						ebox_put_directory_index_file( trailingslashit( $ld_upload_certificates_dir ) . 'index.php' );
					}
				}
		 ...
		 */
		if ( ! defined( 'ebox_TCPDF_LEGACY_LD322' ) ) {
			$use_LD322_define = apply_filters( 'ebox_tcpdf_legacy_ld322', true, $cert_args ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase

			/**
			 * Define ebox LMS - Set to enable legacy TCPDF processing logic.
			 *
			 ...
			 */
			define( 'ebox_TCPDF_LEGACY_LD322', $use_LD322_define ); // phpcs:ignore WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
		}
		...
		$cert_content = preg_replace_callback( "/(<img[^>]*?class=['\"][^'\"]*?aligncenter[^'\"]*?['\"][^>]*?>)/i", 'ebox_post2pdf_conv_image_align_center', $cert_content );

		// Add width and height into image tag.
		$cert_content = preg_replace_callback( "/(<img[^>]*?src=['\"]((http:\/\/|https:\/\/|\/)[^'\"]*?(jpg|jpeg|gif|png))['\"])([^>]*?>)/i", 'ebox_post2pdf_conv_img_size', $cert_content );
		 ...
		 * @param int     $cert_id                   Certificate post ID.
		 */
		if ( apply_filters( 'ebox_certificate_styles', true, $cert_args['cert_id'] ) ) {
			$certificate_styles = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Certificates_Styles', 'styles' );
		 ...
		 * @param int    $cert_id      Certificate post ID.
		 */
		$cert_content = apply_filters( 'ebox_certificate_content', $cert_content, $cert_args['cert_id'] );

		/**
		 * Build the PDF Certificate using TCPDF.
		 */
		if ( ! class_exists( 'TCPDF' ) ) {
			require_once ebox_LMS_LIBRARY_DIR . '/tcpdf/config/lang/' . $cert_args['lang'] . '.php';
			require_once ebox_LMS_LIBRARY_DIR . '/tcpdf/tcpdf.php';
		}

		$ebox_certificate_options = get_post_meta( $cert_args['cert_post']->ID, 'ebox_certificate_options', true );
		if ( ! is_array( $ebox_certificate_options ) ) {
			$ebox_certificate_options = array( $ebox_certificate_options );
		}

		if ( ! isset( $ebox_certificate_options['pdf_page_format'] ) ) {
			$ebox_certificate_options['pdf_page_format'] = PDF_PAGE_FORMAT;
		}

		if ( ! isset( $ebox_certificate_options['pdf_page_orientation'] ) ) {
			$ebox_certificate_options['pdf_page_orientation'] = PDF_PAGE_ORIENTATION;
		}

		// Create a new object.
		$tcpdf_params = array(
			'orientation' => $ebox_certificate_options['pdf_page_orientation'],
			'unit'        => PDF_UNIT,
			'format'      => $ebox_certificate_options['pdf_page_format'],
			'unicode'     => true,
			'encoding'    => 'UTF-8',
			'diskcache'   => false,
			'pdfa'        => false,
		 ...
		 * @param int   $cert_id      Certificate post ID.
		 */
		$tcpdf_params = apply_filters( 'ebox_certificate_params', $tcpdf_params, $cert_args['cert_id'] );
		 ...
		 * @param int   $cert_id Certificate post ID.
		 */
		do_action( 'ebox_certification_created', $pdf, $cert_args['cert_id'] );
		 ...
		 * @param int    $cert_id     Certificate post ID.
		 */
		$pdf->setCreator( apply_filters( 'ebox_pdf_creator', PDF_CREATOR, $pdf, $cert_args['cert_id'] ) );
		 ...
		 * @param int    $cert_id         Certificate post ID.
		 */
		$pdf->setAuthor( apply_filters( 'ebox_pdf_author', $cert_args['pdf_author_name'], $pdf, $cert_args['cert_id'] ) );
		 ...
		 * @param int    $cert_id   Certificate post ID.
		 */
		$pdf->setTitle( apply_filters( 'ebox_pdf_title', $cert_args['pdf_title'], $pdf, $cert_args['cert_id'] ) );
		 ...
		 * @param int    $cert_id     Certificate post ID.
		 */
		$pdf->setSubject( apply_filters( 'ebox_pdf_subject', wp_strip_all_tags( get_the_category_list( ',', '', $cert_args['cert_id'] ) ), $pdf, $cert_args['cert_id'] ) );
		 ...
		 * @param int    $cert_id      Certificate post ID.
		 */
		$pdf->setKeywords( apply_filters( 'ebox_pdf_keywords', $cert_args['pdf_keywords'], $pdf, $cert_args['cert_id'] ) );
		 ...
		 * @param int   $post_id Post ID.
		 */
		do_action( 'ebox_certification_after', $pdf, $cert_args['cert_id'] );

		// get featured image.
		$img_file = ebox_get_thumb_path( $cert_args['cert_id'] );
		...
		if ( '' !== $img_file ) {
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_certification_thumbnail_processing_start', $img_file, $cert_args, $pdf );
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_certification_thumbnail_before', $img_file, $cert_args, $pdf );
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_certification_thumbnail_after', $img_file, $cert_args, $pdf );
			 ...
			 * @param TCPDF  $pdf       `TCPDF` class instance.
			 */
			do_action( 'ebox_certification_thumbnail_processing_end', $img_file, $cert_args, $pdf );
		}
		 ...
		 * @param array  $cert_args Array of certificate args.
		 */
		do_action( 'ebox_certification_content_write_cell_before', $pdf, $cert_args );
		 ...
		 * @param TCPDF $pdf           `TCPDF` class instance.
		 */
		$pdf_cell_args = apply_filters( 'ebox_certification_content_write_cell_args', $pdf_cell_args, $cert_args, $tcpdf_params, $pdf );
		 ...
		 * @param array  $cert_args Array of certificate args.
		 */
		do_action( 'ebox_certification_content_write_cell_after', $pdf, $cert_args );
