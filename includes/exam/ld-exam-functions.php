...
if ( ! defined( 'ebox_EXAM_CHALLENGE_POST_META_KEY' ) ) {
	define( 'ebox_EXAM_CHALLENGE_POST_META_KEY', '_ld_exam_challenge' );
}
 ...
 * @return void
 */
function ebox_course_exam_challenge_redirect( $course_id = 0 ) {
	if ( empty( $course_id ) ) {
		return;
	}
	...
	$course_status = ebox_course_status( $course_id, $user_id, true );
	...
	$exam_id = (int) ebox_get_course_exam_challenge( $course_id );
	if ( ! empty( $exam_id ) ) {
		...
		if ( true === ebox_can_user_bypass(
			$user_id,
			'ebox_course_exam_challenge_redirect',
			array(
				'user_id'       => $user_id,
				'course_id'     => $course_id,
				'exam_id'       => $exam_id,
		...
		$exam_status = ebox_get_user_course_exam_challenge_status( $user_id, $course_id );
		if ( 'not_taken' === $exam_status ) {

			$exam_show_new_enroll = ebox_get_setting( $exam_id, 'show_new_enroll' );
			 ...
			 */
			$exam_link = apply_filters( 'ebox_course_to_exam_challenge_redirect', $exam_link, $course_id, $exam_id, $user_id );
			if ( ! empty( $exam_link ) ) {
				ebox_safe_redirect( $exam_link );
			}
		}
	}
}
 ...
 * @param int $exam_id Exam Post ID.
 */
function ebox_exam_challenge_view_permission( $exam_id = 0 ) {
	$user_id = get_current_user_id();
	if ( ( ! empty( $user_id ) ) && ( ! empty( $exam_id ) ) ) {
		...
		if ( true === ebox_can_user_bypass(
			$user_id,
			'ebox_exam_challenge_view_permission',
			array(
				'user_id' => $user_id,
				'exam_id' => $exam_id,
			)
		...
		$course_id_show = (int) ebox_get_setting( $exam_id, 'exam_challenge_course_show' );
		if ( ! empty( $course_id_show ) ) {
			$exam_status = ebox_get_user_course_exam_challenge_status( $user_id, $course_id_show );
			// If the user has not taken the exam we pass them one to it.
			if ( 'not_taken' === $exam_status ) {
				return;
			} elseif ( 'passed' === $exam_status ) {
				$course_id_passed = (int) ebox_get_setting( $exam_id, 'exam_challenge_course_passed' );
				if ( ( ! empty( $course_id_passed ) ) && ( $course_id_passed !== $course_id_show ) && ( is_post_publicly_viewable( $course_id_passed ) ) ) {
					...
					if ( ! empty( $course_passed_link ) ) {
						...
						*/
						$course_passed_link = apply_filters( 'ebox_exam_challenge_to_course_passed_redirect', $course_passed_link, $course_id_passed, $exam_id, $user_id, $exam_status );
						if ( ! empty( $course_passed_link ) ) {
							ebox_safe_redirect( $course_passed_link );
						}
					}
				}
				...
				if ( is_post_publicly_viewable( $course_id_show ) ) {
					...
					if ( ! empty( $course_show_link ) ) {
						 ...
						 * @param string $exam_status Exam Status slug.
						*/
						$course_show_link = apply_filters( 'ebox_exam_challenge_to_course_show_redirect', $course_show_link, $course_id_show, $exam_id, $user_id, $exam_status );
						if ( ! empty( $course_show_link ) ) {
							ebox_safe_redirect( $course_show_link );
						}
					}
				}
	...
	$course_archive_link = get_post_type_archive_link( ebox_get_post_type_slug( 'course' ) );
	if ( ! empty( $course_archive_link ) ) {
		ebox_safe_redirect( $course_archive_link );
	}
	ebox_safe_redirect( get_home_url() );
}
 ...
 */
function ebox_get_exam_challenge_courses( $exam_id = 0 ) {
	$course_ids = array();
	...
	if ( ! empty( $exam_id ) ) {
		...
		$query_args = array(
			...
			'meta_query'     => array(
				array(
					'key'     => ebox_EXAM_CHALLENGE_POST_META_KEY,
					'value'   => $exam_id,
					'compare' => '=',
				),
			),
 ...
 * @return array An array of course IDs.
 */
function ebox_get_exam_challenge_available_courses() {
	$query_args = array(
		'post_type'      => ebox_get_post_type_slug( 'course' ),
		'fields'         => 'ids',
		'posts_per_page' => -1,
 ...
 */
function ebox_set_exam_challenge_courses( $exam_id = 0, $exam_courses_new = array() ) {
	$exam_id = absint( $exam_id );
	if ( ! empty( $exam_id ) ) {

		$exam_courses_old = ebox_get_exam_challenge_courses( $exam_id, true );
		...
		if ( ! empty( $exam_courses_add ) ) {
			foreach ( $exam_courses_add as $course_id ) {
				ebox_update_course_exam_challenge( $course_id, $exam_id, false );
			}
		}
		...
		if ( ! empty( $exam_courses_remove ) ) {
			foreach ( $exam_courses_remove as $course_id ) {
				ebox_update_course_exam_challenge( $course_id, $exam_id, true );
			}
		}
	}
}
 ...
 */
function ebox_get_course_exam_challenge( $course_id = 0 ) {

	$course_id = absint( $course_id );
	$exam_id   = 0;

	if ( ! empty( $course_id ) ) {
		$exam_id        = get_post_meta( $course_id, ebox_EXAM_CHALLENGE_POST_META_KEY, true );
		$exam_id        = absint( $exam_id );
		$exam_post_type = get_post_type( $exam_id );

		if ( ebox_get_post_type_slug( 'exam' ) !== get_post_type( $exam_id ) ) {
			$exam_id = 0;
		}
	}
 ...
 */
function ebox_update_course_exam_challenge( $course_id = 0, $exam_id = 0, $remove = false ) {
	...
	if ( true === $remove ) {
		$exam_enrolled = ebox_get_course_exam_challenge( $course_id );
		$exam_enrolled = absint( $exam_enrolled );
		if ( ! empty( $exam_enrolled ) ) {
			$action_success = (bool) delete_post_meta( $course_id, ebox_EXAM_CHALLENGE_POST_META_KEY );
			if ( true === $action_success ) {
				 ...
				 * @param int $exam_id   Exam ID.
				 */
				do_action( 'ebox_removed_course_exam_challenge', $course_id, $exam_enrolled );
			}
		}
	} elseif ( ( true !== $remove ) && ( ! empty( $course_id ) ) && ( ! empty( $exam_id ) ) ) {
		$action_success = (bool) update_post_meta( $course_id, ebox_EXAM_CHALLENGE_POST_META_KEY, $exam_id );
		if ( true === $action_success ) {
			/**
			 ...
			 * @param int $exam_id   Exam ID.
			 */
			do_action( 'ebox_added_course_exam_challenge', $course_id, $exam_id );
		}
	}
 ...
 * @return string|null.
 */
function ebox_course_exam_challenge_status_label( $status_slug = '' ) {
	global $ebox_exam_challenge_statuses;

	if ( ( ! empty( $status_slug ) ) && ( isset( $ebox_exam_challenge_statuses[ $status_slug ] ) ) ) {
		return $ebox_exam_challenge_statuses[ $status_slug ];
	}
}
 ...
 * @param int $course_id    Course ID.
 *
 * @return string key from $ebox_exam_challenge_statuses or empty.
 */
function ebox_get_user_course_exam_challenge_status( $user_id = 0, $course_id = 0 ) {
	...
	return ebox_grade_user_course_exam_activity( $exam_activity );
}
 ...
 */
function ebox_grade_user_course_exam_activity( $exam_activity ) {
 ...
 */
function ebox_get_user_course_exam_activity( $user_id = 0, $course_id = 0, $exam_id = 0 ) {
	$user_id   = absint( $user_id );
	$course_id = absint( $course_id );
	$exam_id   = absint( $exam_id );
	...
	if ( ( ! empty( $user_id ) ) && ( ! empty( $exam_id ) ) ) {
		...
		$activity = ebox_get_user_activity( $args );
		if ( ( is_object( $activity ) ) && ( property_exists( $activity, 'activity_id' ) ) && ( ! empty( $activity->activity_id ) ) ) {
			...
			$activity->activity_meta = (array) ebox_get_user_activity_meta( $activity->activity_id );
		}
	}
 ...
 * @since 4.0.0
 **/
function ebox_exam_deregister_post_type_blocks() {
	$post_type = get_post_type( get_the_ID() );
	if ( ebox_get_post_type_slug( 'exam' ) !== $post_type ) {
		wp_enqueue_script(
			'ebox-deregister-post-type-blocks',
...
add_action( 'enqueue_block_editor_assets', 'ebox_exam_deregister_post_type_blocks' );
 ...
 * @since 4.0.0
 */
function ebox_exam_legacy_theme_warning_message() {
	$general_setting_link = '<a href="' . esc_url( add_query_arg( array( 'page' => 'ebox_lms_settings' ), admin_url( 'admin.php' ) ) . '#' ) . '">' . esc_html__( 'General Settings', 'ebox' ) . '</a>';

	// translators: placeholders: Challenge Exams.
	$message = '<div class="notice notice-error is-dismissible"><p>' . sprintf( esc_html_x( 'You are using the Legacy ebox theme which does not support %1$s, please visit %2$s to select a newer theme.', 'placeholders: Challenge Exams', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'exams' ) ), $general_setting_link ) . '</p></div>';
	 ...
	 */
	return apply_filters( 'ebox_exam_legacy_theme_warning_message', $message );
}
 ...
 * @since 4.0.0
 */
function ebox_exam_process_ajax() {
	...
	if ( ! wp_verify_nonce( esc_attr( $_POST['exam_nonce'] ), 'ebox-exam_nonce-' . $user_id . '-' . $exam_id . '-' . $course_id ) ) {
		return;
	}
	...
	if ( ( ! $exam ) || ( ! is_a( $exam, 'WP_Post' ) ) || ( ebox_get_post_type_slug( 'exam' ) !== $exam->post_type ) ) {
		return;
	}

	$course = get_post( $course_id );
	if ( ( ! $course ) || ( ! is_a( $course, 'WP_Post' ) ) || ( ebox_get_post_type_slug( 'course' ) !== $course->post_type ) ) {
		return;
	}

	$course_exam_challenge = (int) ebox_get_course_exam_challenge( $course_id );
	...
	switch ( $exam_action ) {
		case 'ebox_exam_process_reset':
			$exam_activity = ebox_get_user_activity( $exam_activity_args );
			if ( ( is_a( $exam_activity, 'LDLMS_Model_Activity' ) ) && ( property_exists( $exam_activity, 'activity_id' ) ) && ( ! empty( $exam_activity->activity_id ) ) ) {
				ebox_delete_user_activity( $exam_activity->activity_id );

				$json_data['link_text'] = esc_html__( '(pending)', 'ebox' );
				wp_send_json_success( $json_data );
			}

			break;

		case 'ebox_exam_process_complete':
			$exam_activity = ebox_get_user_activity( $exam_activity_args, true );
			if ( ( is_a( $exam_activity, 'LDLMS_Model_Activity' ) ) && ( property_exists( $exam_activity, 'activity_id' ) ) && ( ! empty( $exam_activity->activity_id ) ) ) {
				...
				if ( true !== (bool) $exam_activity['activity_status'] ) {
					...
					ebox_update_user_activity( $exam_activity );

					// Set an activity meta field so we know the activity was completed manually.
					ebox_update_user_activity_meta( $exam_activity['activity_id'], 'manual_complete_user_id', get_current_user_id() );
					ebox_update_user_activity_meta( $exam_activity['activity_id'], 'manual_complete_time', $activity_timestamp );

					// Now set the associated course as complete.
					$course_exam_challenge_passed = (int) ebox_get_setting( $exam_id, 'exam_challenge_course_passed' );
					if ( empty( $course_exam_challenge_passed ) ) {
						$course_exam_challenge_passed = $course_id;
					}
					ebox_user_course_complete_all_steps( $user_id, $course_exam_challenge_passed );
...
add_action( 'wp_ajax_ebox_exam_process_reset', 'ebox_exam_process_ajax' );
add_action( 'wp_ajax_ebox_exam_process_complete', 'ebox_exam_process_ajax' );
