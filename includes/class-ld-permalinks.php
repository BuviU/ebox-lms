...
if ( ! class_exists( 'ebox_Permalinks' ) ) {
	/**
	 * Class for ebox Permalinks
	 */
	class ebox_Permalinks {
		 ...
		 * when we try to match the /courses/course-slug/modules/lesson-slug/topics/topic-slug/quizzes/quiz-slug/ the queried 'post_type' will be set to
		...
		public function generate_rewrite_rules( $wp_rewrite ) {
			if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) == 'yes' ) {
				$courses_cpt = get_post_type_object( ebox_get_post_type_slug( 'course' ) );
				$modules_cpt = get_post_type_object( ebox_get_post_type_slug( 'lesson' ) );
				$topics_cpt  = get_post_type_object( ebox_get_post_type_slug( 'topic' ) );
				$quizzes_cpt = get_post_type_object( ebox_get_post_type_slug( 'quiz' ) );

				$ld_rewrite_values = array(
					ebox_get_post_type_slug( 'course' ) => array(
						'name' => array(
					...
					ebox_get_post_type_slug( 'lesson' ) => array(
						'name' => array(
							'placeholder' => '{{modules_cpt_name}}',
							'value'       => $modules_cpt->name,
						),
						'slug' => array(
							'placeholder' => '{{modules_cpt_slug}}',
							'value'       => $modules_cpt->rewrite['slug'],
						),
					),
					ebox_get_post_type_slug( 'topic' ) => array(
						'name' => array(
					...
					ebox_get_post_type_slug( 'quiz' ) => array(
						'name' => array(
				...
				$ld_rewrite_patterns = array(
					...
					// Course > Lesson.
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)/comment-page-([0-9]{1,})/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&cpage=$matches[3]',
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)(?:/([0-9]+))?/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&page=$matches[3]',

					// Course > Lesson > Quiz.
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)/{{quizzes_cpt_slug}}/([^/]+)/comment-page-([0-9]{1,})/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&{{quizzes_cpt_name}}=$matches[3]&cpage=$matches[4]',
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)/{{quizzes_cpt_slug}}/([^/]+)(?:/([0-9]+))?/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&{{quizzes_cpt_name}}=$matches[3]&page=$matches[4]',

					// Course > Lesson > Topic.
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)/{{topics_cpt_slug}}/([^/]+)/comment-page-([0-9]{1,})/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&{{topics_cpt_name}}=$matches[3]&cpage=$matches[4]',
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)/{{topics_cpt_slug}}/([^/]+)(?:/([0-9]+))?/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&{{topics_cpt_name}}=$matches[3]&page=$matches[4]',

					// Course > Lesson > Topic > Quiz.
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)/{{topics_cpt_slug}}/([^/]+)/{{quizzes_cpt_slug}}/([^/]+)/comment-page-([0-9]{1,})/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&{{topics_cpt_name}}=$matches[3]&{{quizzes_cpt_name}}=$matches[4]&cpage=$matches[5]',
					'{{courses_cpt_slug}}/([^/]+)/{{modules_cpt_slug}}/([^/]+)/{{topics_cpt_slug}}/([^/]+)/{{quizzes_cpt_slug}}/([^/]+)(?:/([0-9]+))?/?$' => 'index.php?{{courses_cpt_name}}=$matches[1]&{{modules_cpt_name}}=$matches[2]&{{topics_cpt_name}}=$matches[3]&{{quizzes_cpt_name}}=$matches[4]&page=$matches[5]',
				);
				 ...
				 */
				$ld_rewrite_rules = apply_filters( 'ebox_permalinks_nested_urls', $ld_rewrite_rules, $ld_rewrite_patterns, $ld_rewrite_values );
		...
		public function post_type_link( $post_link = '', $post = null, $leave_name = false, $sample = false ) {
			...
			if ( ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) == 'yes' ) && ( in_array( $post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) ) {
				...
				if ( ( is_admin() ) && ( 'edit.php' == $pagenow ) ) {
					if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
						...
						$course_id = apply_filters( 'ebox_post_link_course_id', $course_id, $post_link, $post );
						if ( empty( $course_id ) ) {
							return $post_link;
						}
					}
				 ...
				 */
				$courses_cpt->rewrite['slug_alt'] = apply_filters( 'ebox_post_type_rewrite_slug', $courses_cpt->rewrite['slug'], ebox_get_post_type_slug( 'course' ) );
				/** This filter is documented in includes/class-ld-permalinks.php */
				$modules_cpt->rewrite['slug_alt'] = apply_filters( 'ebox_post_type_rewrite_slug', $modules_cpt->rewrite['slug'], ebox_get_post_type_slug( 'lesson' ) );
				/** This filter is documented in includes/class-ld-permalinks.php */
				$topics_cpt->rewrite['slug_alt'] = apply_filters( 'ebox_post_type_rewrite_slug', $topics_cpt->rewrite['slug'], ebox_get_post_type_slug( 'topic' ) );
				/** This filter is documented in includes/class-ld-permalinks.php */
				$quizzes_cpt->rewrite['slug_alt'] = apply_filters( 'ebox_post_type_rewrite_slug', $quizzes_cpt->rewrite['slug'], ebox_get_post_type_slug( 'quiz' ) );

				$draft_or_pending = in_array( $post->post_status, array( 'draft', 'pending', 'auto-draft', 'future' ), true );

				if ( $modules_cpt->name == $post->post_type ) {
					...
					if ( ! empty( $course_id ) ) {
						...
						if ( $course instanceof WP_Post ) {
							...
							if ( false === $sample ) {
								...
								$lesson_post_name = '%' . $modules_cpt->name . '%';
							}

							if ( ( $wp_rewrite->using_permalinks() ) && ( ! $draft_or_pending ) ) {
								// Old URL part.
								if ( strstr( $post_link, $modules_cpt->rewrite['slug'] . '/' . $lesson_post_name ) ) {
									$url_part_old = '/' . $modules_cpt->rewrite['slug'] . '/' . $lesson_post_name;
								} elseif ( strstr( $post_link, $modules_cpt->rewrite['slug_alt'] . '/' . $lesson_post_name ) ) {
									$url_part_old = '/' . $modules_cpt->rewrite['slug_alt'] . '/' . $lesson_post_name;
								}
								...
								$url_part_new .= '/' . $modules_cpt->rewrite['slug_alt'] . '/' . $lesson_post_name;
							} else {
								// Old URL part.
								$url_part_old = add_query_arg( $modules_cpt->name, $lesson->post_name, $url_part_old );

								// New URL part.
								$url_part_new = add_query_arg( $courses_cpt->name, $course->post_name, $url_part_new );
								$url_part_new = add_query_arg( $modules_cpt->name, $lesson->post_name, $url_part_new );
							}
						}
					}
					...
					/** This filter is documented in includes/class-ld-permalinks.php */
					$course_id = apply_filters( 'ebox_post_link_course_id', ebox_get_course_id( $topic->ID ), $post_link, $post );
					if ( ! empty( $course_id ) ) {
						if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
							$lesson_id = ebox_course_get_single_parent_step( $course_id, $topic->ID );
						} else {
							$lesson_id = ebox_get_lesson_id( $topic->ID );
						}
						...
						if ( ! empty( $lesson_id ) ) {
							...
							if ( ( $course instanceof WP_Post ) && ( $lesson instanceof WP_Post ) ) {
								...
								if ( ( $wp_rewrite->using_permalinks() ) && ( ! $draft_or_pending ) ) {
									...
									if ( ! strstr( $post_link, $modules_cpt->rewrite['slug_alt'] . '/' . $lesson_post_name ) ) {
										$url_part_new .= '/' . $modules_cpt->rewrite['slug_alt'] . '/' . $lesson_post_name;
									}
					...
					/** This filter is documented in includes/class-ld-permalinks.php */
					$course_id = apply_filters( 'ebox_post_link_course_id', ebox_get_course_id( $quiz->ID ), $post_link, $post );
					...
					if ( ! empty( $course_id ) ) {
						...
						if ( $course instanceof WP_Post ) {
							...
							if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
								$quiz_parents = ebox_course_get_all_parent_step_ids( $course_id, $quiz->ID );
							} else {
								$lesson_id = ebox_get_lesson_id( $quiz->ID );
								if ( ! empty( $lesson_id ) ) {
									if ( get_post_type( $lesson_id ) == $topics_cpt->name ) {
										$topic_id  = $lesson_id;
										$lesson_id = ebox_get_lesson_id( $topic_id );
										if ( ! empty( $lesson_id ) ) {
											if ( get_post_type( $lesson_id ) == $modules_cpt->name ) {
							...
							if ( ! empty( $quiz_parents ) ) {
								foreach ( $quiz_parents as $quiz_parent_id ) {
									...
									if ( $quiz_parent_post->post_type == $modules_cpt->name ) {
										if ( ( $wp_rewrite->using_permalinks() ) && ( ! $draft_or_pending ) ) {
											$parent_slug = $modules_cpt->rewrite['slug_alt'];
										} else {
											$parent_slug = $modules_cpt->name;
										}
		...
		public function row_actions( $actions = array(), $post = '' ) {
			...
			if ( ( is_admin() ) && ( 'edit.php' == $pagenow ) ) {
				if ( ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) ) && ( in_array( $typenow, ebox_get_post_types( 'course_steps' ), true ) ) ) {
					$course_id = 0;
		...
		public function get_edit_post_link( $link = '', $post_id = 0, $context = '' ) {
			...
			if ( ( ! empty( $post_id ) ) && ( ! is_admin() ) || ( ( is_admin() && ( in_array( $pagenow, array( 'post.php', 'edit.php' ), true ) ) ) ) ) {
				...
				if ( ( ! empty( $post_type_name ) ) && ( in_array( $post_type_name, ebox_get_post_types( 'course_steps' ), true ) ) ) {
					if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
				...
				if ( ( ! empty( $post_type_name ) ) && ( in_array( $post_type_name, LDLMS_Post_Types::get_post_types( 'quiz_questions' ), true ) ) ) {
					$quiz_id = 0;
					$quiz_id = ebox_get_quiz_id( $post_id );
					if ( ! empty( $quiz_id ) ) {
		...
		public function get_sample_permalink( $permalink = '', $post_id = 0, $title = '', $name = '', $post = '' ) {
			...
			if ( ( is_admin() ) && ( 'post.php' === $pagenow ) && ( is_a( $post, 'WP_Post' ) ) && ( in_array( $post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) ) {
		...
		public function attachment_link( $attachment_link = '', $attachment_id = 0 ) {
			...
			if ( ( ! empty( $attachment_link ) ) && ( ! empty( $attachment_id ) ) ) {
				if ( ( $wp_rewrite->using_permalinks() ) && ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) ) ) {
					...
					if ( ( $attachment_post ) && ( is_a( $attachment_post, 'WP_Post' ) ) ) {
						if ( ( property_exists( $attachment_post, 'post_parent' ) ) && ( ! empty( $attachment_post->post_parent ) ) ) {
							if ( in_array( get_post_type( $attachment_post->post_parent ), ebox_get_post_types( 'course' ), true ) ) {
		...
		public function comment_form_top() {
			...
			if ( ( is_a( $queried_object, 'WP_Post' ) ) && ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) == 'yes' ) ) {
				if ( in_array( $queried_object->post_type, ebox_get_post_types( 'course_steps' ), true ) ) {
					echo '<input type="hidden" name="step_id" value="' . absint( $queried_object->ID ) . '" />';

					$course_id = ebox_get_course_id( $queried_object->ID );
					if ( ! empty( $course_id ) ) {
						echo '<input type="hidden" name="course_id" value="' . absint( $course_id ) . '" />';

						$redirect_to = ebox_get_step_permalink( $queried_object->ID, $course_id );
 ...
 */
function ebox_get_step_permalink( $step_id = 0, $step_course_id = null ) {
 ...
 */
function ebox_redirect_post_location( $location = '', $post_id = 0 ) {
	if ( ( is_admin() ) && ( ! empty( $location ) ) ) {
		...
		if ( in_array( $typenow, ebox_get_post_types( 'course_steps' ), true ) ) {
		...
		} elseif ( ebox_get_post_type_slug( 'question' ) === $typenow ) {
...
add_filter( 'redirect_post_location', 'ebox_redirect_post_location', 10, 2 );
 ...
 * then be flushed.
 */
function ebox_setup_rewrite_flush() {
	update_option( 'ebox_lms_rewrite_flush', true, false );
}
