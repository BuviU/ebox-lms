  ...
  $.wpProQuizFront = function (element, options) {
    ...
    const timelimit = {
      ...
      start() {
        ...
        const resume_data = ebox_prepare_quiz_resume_data(config);
        ...
        const $timeDiv = globalElements.timelimit.find(".wpProQuiz_progress");
    ...
    const reviewBox = new (function () {
      ...
      this.init = function () {
        ...
        if (config.mode != 2) {
          $(
            ".wpProQuiz_reviewLegend li.ebox-quiz-review-legend-item-correct"
          ).hide();
          $(
            ".wpProQuiz_reviewLegend li.ebox-quiz-review-legend-item-incorrect"
          ).hide();
        }
      ...
      this.toggle = function () {
        if (bitOptions.reviewQustion) {
          ...
          $(
            ".wpProQuiz_reviewLegend li.ebox-quiz-review-legend-item-current"
          ).hide();
          $(
            ".wpProQuiz_reviewLegend li.ebox-quiz-review-legend-item-review"
          ).hide();
          $(
            ".wpProQuiz_reviewLegend li.ebox-quiz-review-legend-item-answered"
          ).hide();
    ...
    const readResponses = function (
      ...
      const func = {
        singleMulti() {
          const input = $questionList.find(".wpProQuiz_questionInput");
        ...
        matrix_sort_answer() {
          ...
          $items.each(function () {
            ...
            const $stringUl = $this.find(".wpProQuiz_maxtrixSortCriterion");
        ...
        cloze_answer() {
          ...
          $questionList.find(".wpProQuiz_cloze").each(function (i, v) {
            ...
            const inputText = plugin.methode.cleanupCurlyQuotes(input.val());
    ...
    function setResponse(
      ...
      if (question_data.type == "single" || question_data.type == "multiple") {
        ...
        question_value = ebox_decodeHTML(question_value);
        ...
        jQuery('span.wpProQuiz_cloze input[type="text"]', $questionList).each(
          function (index) {
            if (typeof question_value[index] !== "undefined") {
              $(this).val(ebox_decodeHTML(question_value[index]));
              if (lockQuestion) {
            ...
            if (resume_data[formId].type === type) {
              switch (type) {
                ...
                case that.typeConst.EMAIL:
                  ...
                  form_value = ebox_decodeHTML(form_value);
                  $this.val(form_value);
                  break;
                case that.typeConst.CHECKBOX:
    ...
    plugin.methode = {
      ...
      startQuiz(loadData) {
        ...
        quiz_resume_data = ebox_prepare_quiz_resume_data(config);
      ...
      showSinglePage(page) {
        ...
        /**
         * ebox-6591 On Quiz Start the scroll to the top of the page
      ...
      nextQuestion() {
        if (
          ...
          if (
            currentQuestion.find(globalNames.next).data("question-lock") ===
            true
          ) {
        ...
        if (config.quiz_resume_enabled === "1") {
          ...
          plugin.methode.saveMetaDataToCookie({
            lastQuestion: jQuery(currentQuestion[0]).data("question-meta")
              .question_pro_id,
          });
          ...
          if (typeof nextQuestion.data("question-meta") !== "undefined") {
            plugin.methode.saveMetaDataToCookie({
              nextQuestion: nextQuestion.data("question-meta").question_pro_id,
            });
          }
        }
        ...
        if (
          ...
          globalElements.questionList.children().each(function () {
            if (
              ...
              currentQuestion.data("question-meta").question_pro_id
            ) {
              $(this).hide();
            }
          });
      ...
      uploadFile(event) {
        ...
        if (typeof file !== "undefined") {
          ...
          const uploadEssaySubmit = $("#uploadEssaySubmit_" + question_id);

          const uploadEssayMessage = $("#uploadEssayMessage_" + question_id);
          uploadEssayMessage.removeClass("uploadEssayMessage_fail");
          uploadEssayMessage.removeClass("uploadEssayMessage_success");
          uploadEssayMessage.html(config.essayUploading);

          const data = new FormData();
          data.append("action", "ebox_upload_essay");
          ...
          $.ajax({
            ...
            success(response) {
              ...
              if (
                ...
                const $item = $("#uploadEssayFile_" + question_id).parents(
                  ".wpProQuiz_listItem"
                );
                $e.trigger({
                  type: "questionSolved",
      ...
      showQuizSummary() {
        ...
        $e.find(".wpProQuiz_checkPage .wpProQuiz_reviewSummary").append(
          $e.find(".wpProQuiz_reviewDiv .wpProQuiz_reviewQuestion").clone()
        );

        $e.find(".wpProQuiz_checkPage .wpProQuiz_reviewSummary").append(
          $e.find(".wpProQuiz_reviewDiv .wpProQuiz_reviewLegend").clone()
        );
      ...
      finishQuiz(timeover) {
        // ebox-4412 : Disable final buttons on submit.
      ...
      finishQuizEnd() {
        ...
        if (
          ...
          const certificateContainer = $e.find(".wpProQuiz_certificate");
          if (
            results.comp.result >=
        ...
        if (
          ...
          if (
            ...
            $e.trigger({
              type: "ebox-quiz-finished-passed",
              values: {
                status: "passed",
                item: $e,
                results,
            ...
            $e.trigger({
              type: "ebox-quiz-finished-failed",
              values: {
                status: "failed",
                item: $e,
                results,
      ...
      sendCompletedQuiz() {
        ...
        var quiz_resume_data = ebox_prepare_quiz_resume_data(config);
        if (config.ld_script_debug == true) {
      ...
      saveRandomQuestions(questions) {
        ...
        jQuery(questions).each(function (index, question) {
          const id = jQuery(question).data("question-meta").question_pro_id;
      ...
      restartQuiz() {
        ...
        // ebox-3201 - Added reload to force check on Quiz Repeats / Run Once logic.
              ...
              if (typeof json[question_id] !== "undefined") {
                ...
                if (result.c) {
                  if (typeof result.e.AnswerMessage !== "undefined") {
                    ...
                    $this
                      .find(".wpProQuiz_correct")
                      .trigger("ebox-quiz-answer-response-contentchanged");
                  }
        ...
        switch (result.e.type) {
          ...
          case "multiple":
            $questionList.children().each(function (i) {
              ...
              if (result.e.c[index]) {
                const checked = $("input.wpProQuiz_questionInput", $item).is(
                  ":checked"
                );
                if (checked) {
            ...
            if (
              ...
              jQuery.each(
                ...
                function (correct_item_idx, correct_item_key) {
                  const student_item_el = $questionList.children(
                    "li.wpProQuiz_questionListItem"
                  )[correct_item_idx];

                  if (
                    typeof student_item_el !== "undefined" &&
                    student_item_el !== ""
                  ) {
                    if (
                      correct_item_key == jQuery(student_item_el).data("pos")
                    ) {
              ...
              jQuery(sortlist_container)
                .find(".wpProQuiz_questionList_container_student")
                .append($questionList);
              jQuery(sortlist_container)
                .find(".wpProQuiz_questionList_container_correct")
              ...
              jQuery(sortlist_container)
                .find("input.wpProQuiz_questionList_containers_view_student")
                .on("click", function (e) {
                  jQuery(sortlist_container)
                    .find(
                      "input.wpProQuiz_questionList_containers_view_student"
                    )
                    .addClass("wpProQuiz_questionList_containers_view_active");
                  jQuery(sortlist_container)
                    .find(
              ...
              jQuery(sortlist_container)
                .find("input.wpProQuiz_questionList_containers_view_correct")
                .on("click", function (e) {
                  jQuery(sortlist_container)
                    .find(
                      "input.wpProQuiz_questionList_containers_view_student"
          ...
          if (dataObject.hasOwnProperty("checked" + data.id)) {
            for (const element in dataObject) {
              if (element.startsWith("checked")) {
                const elementId = element.replace("checked", "");
                  ...
                  if (questionResult.c) {
                    if (typeof questionResult.e.AnswerMessage !== "undefined") {
                      question
                        .find(".wpProQuiz_correct")
                        .find(".wpProQuiz_AnswerMessage")
                        .html(ebox_decodeHTML(questionResult.e.AnswerMessage));
                      question
                        .find(".wpProQuiz_correct")
                        .trigger("ebox-quiz-answer-response-contentchanged");
                    }
      ...
      handleToplistData(json) {
        ...
        const $addBox = $tp.find(".wpProQuiz_addBox").show().children("div");
      ...
      nextQuestionClicked() {
        const $questionList = currentQuestion.find(globalNames.questionList);
        const data = config.json[$questionList.data("question_id")];
      ...
      calculateCookieExpiry() {
        ...
        if (config.timelimitcookie && !config.quiz_resume_cookie_expiration) {
          cookieExpireDate.setTime(
            cookieExpireDate.getTime() + config.timelimitcookie * 1000
          );
        }
      ...
      CookieSetResponses() {
        ...
        if (
          (cookie_value == undefined || !Object.keys(cookie_value).length) &&
        ...
        list.each(function () {
          ...
          if (cookie_value[form_question_id] != undefined) {
            const cookie_question_data = cookie_value[form_question_id];
      ...
      startCookieSendTimer() {
        ...
        if (
          !cookieSendTimer &&
          (undefined === quiz_resume_data || quiz_resume_data.length === 0)
        ) {
      ...
      prepareSendCookieData() {
        ...
        if (cookieLength > 0) {
          const cookieKeys = plugin.methode.getObjectKeys(cookie_value);
      ...
      addTimelimitCookieData() {
        const quizTimelimitCookie = jQuery.cookie(timelimit.timer_cookie);
        if ("undefined" !== typeof quizTimelimitCookie) {
          plugin.methode.saveMetaDataToCookie({
            [timelimit.timer_cookie]: JSON.parse(quizTimelimitCookie),
          });
        }
      },
      moveToNextUnansweredQuestion(data) {
        ...
        if (typeof data !== "undefined") {
          let nextQuestion =
            typeof data.nextQuestion === "number" ? data.nextQuestion : 0;
          const lastQuestion =
            typeof data.lastQuestion === "number" ? data.lastQuestion : 0;
          ...
          if (config.mode === 3) {
            if (!config.qpp) {
              ...
              jQuery(globalElements.listItems).each(function (index, listItem) {
                if (
                  $(listItem).data("question-meta").question_pro_id ===
                  lastQuestion
                ) {
      ...
      singlePageNavigationQuizResume(index) {
        ...
        if (activePage <= maxPage) {
          const $listItem = globalElements.questionList.children().hide();
          const pl = $e.find(globalNames.singlePageLeft).hide();
          const pr = $e.find(globalNames.singlePageRight).hide();
          const cs = $e.find('input[name="checkSingle"]').hide();
          ...
          plugin.methode.scrollTo(globalElements.listItems.eq(index), 0);
        }
      },
      sendCookieData(keys) {
        jQuery.ajax({
          ...
          success(response) {
            if (response.success) {
              if (
                plugin.methode.compareObjectKeys(response.data, cookie_value)
              ) {
          ...
          if ("matrix_sort_answer" === question_type) {
            ...
            $("ul.wpProQuiz_maxtrixSortCriterion", questionItem).each(function (
              ...
              if (typeof parent_td !== "undefined") {
                const parent_td_height = $(parent_td).height();
                if (parent_td_height) {
...
var ebox_prepare_quiz_resume_data = function (config) {
...
var ebox_decodeHTML = function (html) {
  html = html || "";
