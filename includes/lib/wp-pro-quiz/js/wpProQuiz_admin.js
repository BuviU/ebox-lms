jQuery(function ($) {
  ...
  $.fn.wpProQuiz_quizOverall = function () {
        ...
        $('input[name="exportItems"]').each(function () {
          ...
          if (this.checked) {
            var text = $this
              .parent()
              .parent()
              .find(".wpProQuiz_quizName a:eq(0)")
              .text();
    ...
    var init = function () {
      ...
      $('input[name="exportItemsAll"]').on("change", function () {
        ...
        if (this.checked) $input.attr("checked", true);
      ...
      $("#exportStart").on("click", function (e) {
        if (!methods.startExport()) e.preventDefault();
      });
    };
  ...
  $.fn.wpProQuiz_questionOverall = function () {
    var methode = {
      saveSort: function () {
        ...
        var url =
          location.replace("admin.php", "admin-ajax.php") + "&action=save_sort";
      ...
      sortUpdate: function (e, ui) {
        $(".wpProQuiz_questionOverall tbody")
          .children()
          .each(function () {
            $t = $(this)
              .children()
      ...
      loadQuestionCopy: function () {
        ...
        var location = window.location.pathname + window.location.search;
        var url =
          location.replace("admin.php", "admin-ajax.php") +
        ...
        $.post(
          ...
          function (json) {
            $.each(json, function (i, v) {
              var team = $(document.createElement("optteam"));

              team.attr("label", v.name);
  ...
  $.fn.wpProQuiz_quizEdit = function () {
    var methode = {
        ...
        $("#resultList")
          ...
          .each(function () {
            if ($(this).css("display") == "none") {
              ...
              var $this = $(this);
              var $text = $this.find(
                'textarea[name="resultTextGrade[text][]"]'
              );
              var id = $text.attr("id");
              var hidden = true;
              ...
              $this
                .find('input[name="resultTextGrade[activ][]"]')
                .val("1")
                .keyup();

              if (
                typeof tinymce != "undefined" &&
                tinymce.editors[id] != undefined &&
                !tinymce.editors[id].isHidden()
              ) {
                hidden = false;
              }
      ...
      validResultInput: function (input) {
        ...
        if (!isNaN(input) && Number(input) <= 100 && Number(input) >= 0) {
          if (input.match(/\./) != null) return input.split(".")[1].length < 3;
      ...
      validInput: function () {
        ...
        if (
          typeof tinymce != "undefined" &&
          tinymce.editors.text != undefined &&
          !tinymce.editors.text.isHidden()
        ) {
        ...
        if ($("#wpProQuiz_resultGradeEnabled:checked").length) {
          ...
          $("#resultList")
            ...
            .each(function () {
              if ($(this).is(":visible")) {
                if (
                  !methode.validResultInput(
                    $(this)
                      .find('input[name="resultTextGrade[prozent][]"]')
                      .val()
                  )
                ) {
                  rCheck = false;
    ...
    var init = function () {
      ...
      $('input[name="resultTextGrade[prozent][]"]')
      ...
      if (window.postL10n) {
        $("#wpProQuiz_save,input[name=save]").on("click", function (e) {
          ...
          $('select[name="prerequisiteList[]"] option').attr(
            "selected",
            "selected"
          );
        });
      ...
      $("#btnPrerequisiteAdd").on("click", function () {
        $('select[name="quizList"] option:selected')
          .removeAttr("selected")
          .appendTo('select[name="prerequisiteList[]"]');
      });

      $("#btnPrerequisiteDelete").on("click", function () {
        $('select[name="prerequisiteList[]"] option:selected')
          .removeAttr("selected")
          .appendTo('select[name="quizList"]');
      });

      $('input[name="prerequisite"]')
        .on("change", function () {
          if (this.checked) $("#prerequisiteBox").show();
          else $("#prerequisiteBox").hide();
        })
        .change();

      $('input[name="toplistDataAddMultiple"]')
        .on("change", function () {
          if (this.checked) $("#toplistDataAddBlockBox").show();
          else $("#toplistDataAddBlockBox").hide();
        })
        .change();

      $('input[name="toplistActivated"]')
        .on("change", function () {
          if (this.checked) $("#toplistBox > tr:gt(0)").show();
          else $("#toplistBox > tr:gt(0)").hide();
        })
        .change();

      $('input[name="showReviewQuestion"]')
        .on("change", function () {
          if (this.checked) {
      ...
      $("#form_add").on("click", function () {
        $("#form_table tbody > tr:eq(0)")
          .clone(true)
          .appendTo("#form_table tbody")
          .show();
      ...
      $(".editDropDown").on("click", function () {
        $(".dropDownEditBox")
          .not($(this).siblings(".dropDownEditBox").toggle())
          .hide();
  ...
  $.fn.wpProQuiz_statistics = function () {
    ...
    var methode = {
      loadStatistics: function (userId) {
        ...
        $(
          "#wpProQuiz_statistics_content, #wpProQuiz_statistics_overview"
        ).hide();

        $.post(url, data, methode.setStatistics, "json");
      },
      ...
      setStatistics: function (json) {
        ...
        var setItem = function (i, j, r) {
          i.find(".wpProQuiz_cCorrect").text(
            j.cCorrect + " (" + j.pCorrect + "%)"
          );
          i.find(".wpProQuiz_cIncorrect").text(
            j.cIncorrect + " (" + j.pIncorrect + "%)"
          );
      ...
      loadOverview: function () {
        $(
          ".wpProQuiz_statistics_table, #wpProQuiz_statistics_content, #wpProQuiz_statistics_overview"
        ).hide();
        $("#wpProQuiz_loadData").show();
          ...
          function (json) {
            ...
            $.each(json.items, function (i, v) {
              ...
              if (v.completed) {
                ...
                d.find(".wpProQuiz_cCorrect").text(
                  v.cCorrect + " (" + v.pCorrect + "%)"
                );
                d.find(".wpProQuiz_cIncorrect").text(
                  v.cIncorrect + " (" + v.pIncorrect + "%)"
                );
      ...
      resetStatistic: function (complete) {
        var userId =
          currectTab == "wpProQuiz_typeRegisteredUser"
            ? $("#userSelect").val()
            : 0;
        var location = window.location.pathname + window.location.search;
        var url =
          location.replace("admin.php", "admin-ajax.php") + "&action=reset";
        var data = {
    ...
    var init = function () {
      ...
      $("#wpProQuiz_pageLimit, #wpProQuiz_onlyCompleted").on(
        "change",
        function () {
      ...
      $("#wpProQuiz_currentPage").on("change", function () {
        ...
        if ($("#wpProQuiz_currentPage option").length == 1) {
          ...
          $("#wpProQuiz_currentPage option:first-child:selected").length
        ) {
          $("#wpProQuiz_pageRight").show();
        } else if (
          $("#wpProQuiz_currentPage option:last-child:selected").length
        ) {
      ...
      $("#wpProQuiz_pageRight").on("click", function () {
        $("#wpProQuiz_currentPage option:selected")
          .next()
  ...
  $.fn.wpProQuiz_toplist = function () {
      ...
      handleDataRequest: function (json) {
        ...
        $(".wpProQuiz_delete").on("click", function () {
          if (confirm(wpProQuizLocalize.confirm_delete_entry)) {
            var id = new Array(
              $(this).closest("tr").find('input[name="checkedData[]"]').val()
            );
        ...
        $(".inline_editCancel").on("click", function () {
          ...
          $contain
            ...
            .val($contain.find(".wpProQuiz_username").text());
          $contain
            .find(".inline_editEmail")
            .val($contain.find(".wpProQuiz_email").text());
    ...
    var init = function () {
      ...
      $("#wpProQuiz_action").on("click", function () {
        ...
        if (name != "0") {
          var ids = $('input[name="checkedData[]"]:checked')
            .map(function () {
              return $(this).val();
            })
            .get();
      ...
      $("#wpProQuiz_checkedAll").on("change", function () {
        ...
        else $('input[name="checkedData[]"]').removeAttr("checked", "checked");
      });
  ...
  function WpProQuiz_Admin() {
      ...
      gobalSettings: function () {
        var methode = {
          ...
          categoryEdit: function (id, name) {
            ...
            global.ajaxPost("categoryEdit", data, function (json) {
              ...
              $('select[name="category"] option[value="' + id + '"]').text(
                data.categoryName
              );
          ...
          templateDelete: function (id, type) {
            ...
            global.ajaxPost("templateDelete", data, function (json) {
              ...
              if (!type) {
                ...
                $(
                  'select[name="templateQuiz"] option[value="' + id + '"]'
                ).remove();
                ...
                $(
                  'select[name="templateQuestion"] option[value="' + id + '"]'
                ).remove();
        ...
        var init = function () {
          ...
          $('input[name="templateQuizEdit"]').on("click", function () {
            var template_id = $(
              'select[name="templateQuiz"] option:selected'
            ).val();
            if (template_id != "") {
          ...
          $('input[name="templateQuestionEdit"]').on("click", function () {
            var template_id = $(
              'select[name="templateQuestion"] option:selected'
            ).val();
            if (template_id != "") {
      ...
      questionEdit: function () {
        ...
        methode = {
          generateArrayIndex: function () {
            ...
            type =
              type == "single" || type == "multiple" ? "classic_answer" : type;
                ...
                .each(function (i, v) {
                  $(this)
                    ...
                    .each(function () {
                      ...
                      if (x > 0) {
                        this.name =
                          "answerData[" + n + name.substring(x, name.length);
                      }
                    });
                });
            });
          ...
          globalValidate: function () {
            ...
            if (typenow !== "undefined" && typenow == "ebox-question") {
          ...
          answerRemove: function () {
            ...
            if (li.parent().children().length > 1) li.remove();
            else alert(wpProQuizLocalize.no_delete_answer);
          ...
          addMediaClick: function () {
            ...
            var htmlCheck = closest.find(
              'input[name="answerData[][html]"]:eq(0)'
            );
            ...
            media_upload_frame.on("select", function () {
              ...
              if (json.type === "audio") {
                field.val(
                  field.val() + '[audio src="' + json.url + '"][/audio]'
                );
                htmlCheck.attr("checked", true);
              }
              ...
              if (json.type === "video") {
                field.val(
                  field.val() + '[video src="' + json.url + '"][/video]'
                );
                htmlCheck.attr("checked", true);
              }
            });
        ...
        var validate = {
          classic_answer: function () {
            ...
            $(".classic_answer .answerList")
              .children()
              .each(function () {
                var t = $(this);
                if (
                  !global.isEmpty(
                    t.find("textarea.wpProQuiz_single_text").val()
                  )
                ) {
                  findText++;
            ...
            if (
              !findCorrect &&
              !(
                $('input[name="disableCorrect"]').is(":checked") &&
                $('input[name="answerPointsDiffModusActivated"]').is(
                  ":checked"
                ) &&
                $('input[name="answerPointsActivated"]').is(":checked") &&
                ($('input[name="answerType"]:checked').val() == "single" ||
                  $('input[name="answerType"]:checked').val() == "multiple")
              )
            ) {
          ...
          free_answer: function () {
            if (
              global.isEmpty(
                $(".free_answer textarea.wpProQuiz_free_text").val()
              )
            ) {
              alert(wpProQuizLocalize.no_answer_msg);
              return false;
          ...
          sort_answer: function () {
            ...
            $(".sort_answer .answerList")
              ...
              .each(function () {
                ...
                if (
                  !global.isEmpty(
                    t.find("textarea.wpProQuiz_sort_answer").val()
                  )
                ) {
                  findText++;
          ...
          matrix_sort_answer: function () {
            ...
            $(".matrix_sort_answer .answerList")
              ...
              .each(function () {
                ...
                if (
                  !global.isEmpty(
                    t.find("textarea.wpProQuiz_matrix_answer").val()
                  )
                ) {
                  findText++;
                  menge++;
                  ...
                  if (
                    global.isEmpty(
                      t.find("textarea.wpProQuiz_matrix_sort_string").val()
                    )
                  ) {
                    sortString = false;
                  }
                  ...
                  if (
                    !global.isEmpty(
                      t.find("textareawpProQuiz_matrix_sort_string").val()
                    )
                  ) {
                    menge++;
        ...
        var formListener = function () {
          ...
          $('input[name="answerType"]').on("click", function () {
            ...
            var answerPointsActivated_checked = $(
              'input[name="answerPointsActivated"]'
            ).is(":checked");
            ...
            if (v == "single") {
              if (typenow !== "undefined" && typenow == "ebox-question") {
                $("#ebox_question_single_choice_options").show();
              } else {
              ...
              if (typenow !== "undefined" && typenow == "ebox-question") {
                $("#ebox_question_single_choice_options").hide();
              } else {
                $("#singleChoiceOptions").hide();
              }
              $(".classic_answer .wpProQuiz_classCorrect")
                .parent()
                .parent()
                .show();
            }
            ...
            if (v == "single" || v == "multiple") {
              ...
              $("#ebox_question_message_incorrect_answer").hide();
            } else if (v == "essay") {
              ...
              $(
                "#ebox_question_message_correct_answer .postbox-header h2"
              ).html(wpProQuizLocalize.metabox_title_correct_message_essay);
              $(
                "#ebox_question_message_correct_answer .inside .ebox-same-answer-text"
              ).hide();
              $(
                "#ebox_question_message_correct_answer .inside p.description"
              ).hide();
              ...
              $("#ebox_question_message_correct_answer").show();
              $(
                "#ebox_question_message_correct_answer .postbox-header h2"
              ).html(wpProQuizLocalize.metabox_title_correct_message);
              $(
                "#ebox_question_message_correct_answer .inside .ebox-same-answer-text"
              ).show();
              $(
              ...
              $("#ebox_question_message_incorrect_answer").show();
            }
          ...
          $(
            ".sort_answer ul, .classic_answer ul, .matrix_sort_answer ul"
          ).sortable({
            handle: ".wpProQuiz_move",
          });
          ...
          $(elements.pointsModus)
            .on("change", function () {
              ...
              global.displayChecked(
                this,
                $('input[name="answerPointsDiffModusActivated"]'),
                true,
                true
              );
          ...
          $('input[name="answerPointsDiffModusActivated"]')
            .on("change", function () {
              global.displayChecked(
                this,
                $('input[name="disableCorrect"]'),
                true,
                true
              );

              if (this.checked) $('input[name="disableCorrect"]').change();
              else
                $(".classic_answer .wpProQuiz_classCorrect")
                  .parent()
                  .parent()
          ...
          $('input[name="disableCorrect"]')
            .on("change", function () {
              global.displayChecked(
                this,
                $(".classic_answer .wpProQuiz_classCorrect").parent().parent(),
                true
              );
            })
            .change();
      ...
      statistic: function () {
        ...
        methode = {
          ...
          loadUsersStatistic: function () {
            ...
            global.ajaxPost("statisticLoad", data, function (json) {
              ...
              $.each(json.tests, function () {
                ...
                if (json.testId == this.id) $option.attr("selected", true);
          ...
          loadStatisticOverview: function (nav) {
            var data = {
              ...
              onlyCompleted: Number(
                $("#wpProQuiz_onlyCompleted").is(":checked")
              ),
              page: elements.currentPage.val(),
              nav: Number(nav),
            };
            ...
            global.ajaxPost("statisticLoadOverview", data, function (json) {
              ...
              if (json.page != undefined) methode.handleNav(json.page);
            });
          },
          ...
          handleNav: function (nav) {
            ...
            for (var i = 1; i <= nav; i++) {
              $(document.createElement("option")).val(i).text(i).appendTo($p);
            }
          ...
          loadFormsOverview: function (nav) {
            ...
            global.ajaxPost("statisticLoadFormOverview", data, function (json) {
              ...
              if (json.page != undefined) methode.handleFormNav(json.page);
            });
          },
        ...
        var init = function () {
          ...
          $("#wpProQuiz_formUser, #wpProQuiz_fromPageLimit").on(
            "change",
            function () {
          ...
          $("#wpProQuiz_formPageLeft").on("click", function () {
            $("#wpProQuiz_formCurrentPage").val(
              Number(elements.currentPage.val()) - 1
            );
      ...
      statisticNew: function () {
        ...
        var overviewFilter = {
          ...
          changeFilter: function () {
            $.extend(this.data, {
              ...
              onlyCompleted: Number(
                $("#wpProQuiz_overviewOnlyCompleted").is(":checked")
              ),
              generateNav: 1,
            });
          },
        ...
        var deleteMethode = {
          deleteUserStatistic: function (refId, userId) {
            if (!confirm(wpProQuizLocalize.reset_statistics_msg)) return false;
        ...
        var methode = {
          loadHistoryAjax: function () {
            var data = $.extend(
              {
                page: historyFilter.data.generateNav
                  ? 1
                  : historyNavigator.getCurrentPage(),
              },
              historyFilter.data
            );
            ...
            global.ajaxPost("statisticLoadHistory", data, function (json) {
              ...
              if (json.navi) historyNavigator.setNumPage(json.navi);
              ...
              content.find(".wpProQuiz_delete").on("click", function () {
                deleteMethode.deleteUserStatistic(
                  $(this).parents("tr").find(".user_statistic").data("ref_id"),
                  0
                );
          ...
          loadUserAjax: function (userId, refId, avg) {
            ...
            global.ajaxPost("statisticLoadUser", data, function (json) {
              ...
              content
                .find("#wpProQuiz_resetUserStatistic")
                .on("click", function () {
              ...
              jQuery("body").trigger("ebox-statistics-contentchanged");
          ...
          loadBox: function (show, contain) {
            if (show) $("#wpProQuiz_loadDataHistory").show();
            else $("#wpProQuiz_loadDataHistory").hide();
          },

          loadOverviewAjax: function () {
            var data = $.extend(
              {
                page: overviewFilter.data.generateNav
                  ? 1
                  : overviewNavigator.getCurrentPage(),
              },
              overviewFilter.data
            );
            ...
            global.ajaxPost("statisticLoadOverviewNew", data, function (json) {
              ...
              if (json.navi) overviewNavigator.setNumPage(json.navi);
              ...
              content.find(".wpProQuiz_delete").on("click", function () {
                deleteMethode.deleteUserStatistic(
                  0,
                  $(this).parents("tr").find(".user_statistic").data("user_id")
                );
          ...
          usersSelect2Ajax: function (el) {
            ...
            return {
              ...
              data: function (params) {
                return {
                  action: "ebox_quiz_statistics_users_select2",
                  nonce: nonce || "",
                  search: params.term || "",
                  page: params.page || 1,
        ...
        var init = function () {
          ...
          if (wpProQuizLocalize.select2_enabled) {
            if (
              jQuery(
                '#wpProQuiz_tabHistory select#wpProQuiz_historyUser[data-ld-select2="1"]'
              ).length
            ) {
              jQuery(
                '#wpProQuiz_tabHistory select#wpProQuiz_historyUser[data-ld-select2="1"]'
              ).each(function (idx, item) {
                var select2_args = ebox_get_base_select2_args();
          ...
          if ($("#datepickerFrom").length || $("#datepickerTo").length) {
            ...
            $(document).on("DOMNodeInserted", function (e) {
              if (e.target.id == "ui-datepicker-div") {
                $("#ui-datepicker-div").addClass("ebox-datepicker");
              }
            });
          ...
          $("#wpProQuiz_tabOverview .wpProQuiz_update").on(
            "click",
            function () {
              overviewFilter.changeFilter();
              methode.loadOverviewAjax();
    ...
    var init = function () {
      ...
      var m = $.noop;
      if (
        $(".wpProQuiz_questionEdit").length ||
        $("body.ebox-post-type.ebox-question").length
      ) {
  ...
  function Navigator(obj, option) {
    ...
    var checkNavBar = function () {
      ...
      if (cur > 1) elements.pageLeft.show();

      if (cur + 1 <= num) elements.pageRight.show();
    };
