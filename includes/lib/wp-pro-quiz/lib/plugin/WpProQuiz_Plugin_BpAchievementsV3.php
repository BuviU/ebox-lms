	...
	class WpProQuiz_Plugin_BpAchievementsV3 extends DPA_Extension {
		...
		public function __construct() {
			$this->actions = array(
				...
				'wp_pro_quiz_completed_quiz'             => sprintf( esc_html_x( 'The user completed a %s.', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) ),
				// translators: placeholder: quiz.
				'wp_pro_quiz_completed_quiz_100_percent' => sprintf( esc_html_x( 'The user completed a %s with 100 percent.', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) ),
			);
			...
			// translators: placeholder: quiz.
			$this->description     = sprintf( esc_html_x( 'A powerful and beautiful %s plugin for WordPress.', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) );
			$this->id              = 'wp-pro-quiz';
			$this->image_url       = WPPROQUIZ_URL . '/img/wp_pro_quiz.jpg';
			$this->name            = esc_html__( 'WP-Pro-Quiz', 'ebox' );
