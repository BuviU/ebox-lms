...
class WpProQuiz_Controller_GlobalSettings extends WpProQuiz_Controller_Controller {
	...
	private function edit() {
		...
		if ( isset( $this->_post['submit'] ) ) {
			...
			//WpProQuiz_View_View::admin_notices(__('Settings saved', 'ebox'), 'info');

			$toplistDateFormat = $this->_post['toplist_date_format'];

			if ( 'custom' == $toplistDateFormat ) {
		...
		} elseif ( isset( $this->_post['databaseFix'] ) ) {
			//WpProQuiz_View_View::admin_notices(__('Database repaired', 'ebox'), 'info');
