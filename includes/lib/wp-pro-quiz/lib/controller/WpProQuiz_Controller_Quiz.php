...
class WpProQuiz_Controller_Quiz extends WpProQuiz_Controller_Controller {
	...
	private function addEditQuiz( $get = null, $post = null ) {
		...
		if ( $quizId && $quizMapper->exists( $quizId ) == 0 ) {
			WpProQuiz_View_View::admin_notices(
				...
				// translators: placeholder: Quiz.
					esc_html_x( '%s not found', 'placeholder: Quiz', 'ebox' ),
		...
		if ( isset( $this->_post['template'] ) || ( isset( $this->_post['templateLoad'] ) && isset( $this->_post['templateLoadId'] ) ) ) {
			...
			if ( null !== $data ) {
				...
				if ( empty( $quizId ) && ! empty( $get['post_id'] ) ) {
					ebox_update_setting( $get['post_id'], 'quiz_pro', $quiz->getId() );
				}
				$quizId = $quiz->getId();
			...
			if ( $this->checkValidit( $this->_post ) ) {
				...
				//WpProQuiz_View_View::admin_notices( sprintf( esc_html_x('%s title or %s description are not filled', 'Quiz title or quiz description are not filled', 'ebox'), ebox_Custom_Label::get_label( 'quiz' ), ebox_get_custom_label_lower( 'quiz' )));
			}
		} elseif ( $quizId ) {
		...
		// translators: placeholder: quiz.
		$this->view->header = $quizId ? sprintf( esc_html_x( 'Edit %s', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) ) : sprintf( esc_html_x( 'Create %s', 'Create quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) );
	...
	private function addUpdateQuiz( $get = null, $post = null ) {
		...
		if ( $quizId && $quizMapper->exists( $quizId ) == 0 ) {
			// translators: placeholder: Quiz.
			WpProQuiz_View_View::admin_notices( sprintf( esc_html_x( '%s not found', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'quiz' ) ), 'error' );
		...
		if ( isset( $this->_post['form'] ) ) {
			...
			if ( $this->checkValidit( $this->_post ) ) {
				...
				if ( ( empty( $quizId ) ) && ( isset( $get['post_id'] ) ) && ( ! empty( $get['post_id'] ) ) ) {
					...
					$quiz_id_primary_new = absint( ebox_get_quiz_primary_shared( $quiz->getId(), false ) );
	...
	private function getEditQuiz( $get = null, $post = null ) {
		...
		if ( $quizId && $quizMapper->exists( $quizId ) == 0 ) {
			WpProQuiz_View_View::admin_notices( esc_html__( 'Missing ProQuiz Associated Settings.', 'ebox' ), 'error' );
		...
		// translators: placeholder: quiz.
		? sprintf( esc_html_x( 'Edit %s', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) )
		// translators: placeholder: quiz.
		: sprintf( esc_html_x( 'Create %s', 'Create quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) );
	...
	public function isLockQuiz( $quizId ) {
		...
		$bypass_course_limits_admin_users = ebox_can_user_bypass( $userId, 'ebox_quiz_lock', $quizId, $this->_post );
			...
			if ( empty( $userId ) ) {
				...
				if(isset($this->_cookie['wpProQuiz_lock']) && $userId == 0 && $quiz->isQuizRunOnceCookie()) {
					...
					if ( ( isset( $this->_post['quiz'] ) ) && ( ! empty( $this->_post['quiz'] ) ) ) {
						$repeats = ebox_quiz_get_repeats( $this->_post['quiz'] );
					}
					if ( '' !== $repeats ) {
						...
						if ( ( $cookieJson !== false ) && ( isset( $cookieJson[ $this->_post['quizId'] ] ) ) ) {
							...
							$cookie_quiz = ebox_quiz_convert_lock_cookie( $cookie_quiz );
		...
		if ( $quiz->isPrerequisite() ) {
			...
			if ( ! empty( $quizIds ) ) {
				...
				foreach ( $quizIds as $pro_quiz_id ) {
					$post_quiz_id = ebox_get_quiz_id_by_pro_quiz_id( $pro_quiz_id );
				...
				if ( ! empty( $post_quiz_ids ) ) {
					...
					//  'ebox_quiz_prerequisite_course_check',
					//  $post_course_id,
					//  $userId,
					// 	$post_quiz_ids
					// );

					$post_quiz_ids = ebox_is_quiz_notcomplete( $userId, $post_quiz_ids, true, -1 );
					if ( ! empty( $post_quiz_ids ) ) {
	...
	private function editAction( $id ) {
		...
		$this->view = new WpProQuiz_View_QuizEdit();
		// translators: placeholder: quiz.
		$this->view->header = sprintf( esc_html_x( 'Edit %s', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) );
	...
	private function createAction() {
		...
		$this->view = new WpProQuiz_View_QuizEdit();
		// translators: placeholder: quiz.
		$this->view->header = sprintf( esc_html_x( 'Create %s', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) );
	...
	private function saveTemplate() {
		...
		if ( ! empty( $quiz_post_id ) ) {
			$data[ '_' . ebox_get_post_type_slug( 'quiz' ) ] = ebox_get_setting( $quiz_post_id );
		}

		// Zero out the ProQuiz Post ID and the reference for the associated settings.
		$data['quiz']->setPostId( 0 );
		$data[ '_' . ebox_get_post_type_slug( 'quiz' ) ]['quiz_pro'] = 0;
	...
	private function checkValidit( $post ) {
		...
		if ( ( isset( $post['post_ID'] ) ) && ( ! empty( $post['post_ID'] ) ) ) {
			if ( ebox_get_post_type_slug( 'quiz' ) === get_post_type( absint( $post['post_ID'] ) ) ) {
				return true;
			}
		}
	}
	...
	private function filterResultTextGrade( $post ) {
		...
		if ( isset( $post['resultTextGrade'] ) ) {
			$sorted = ebox_quiz_result_message_sort( $post['resultTextGrade'] );
			return $sorted;
		}
	...
	public function completedQuiz() {
		...
		if ( empty( $userId ) ) {
			...
			if ( '' !== $repeats ) {
				...
				$usermeta = get_user_meta( $userId, '_ebox-quizzes', true );
				...
				if ( ! empty( $usermeta ) ) {
					...
					if ( $attempts_count > $repeats ) {
						$bypass_course_limits_admin_users = ebox_can_user_bypass( $userId, 'ebox_course_lesson_access_from', $this->_post['course_id'], get_post( $this->_post['quiz'] ) );

						// For logged in users to allow an override filter.
						/** This filter is documented in includes/course/ld-course-progress.php */
						$bypass_course_limits_admin_users = apply_filters( 'ebox_prerequities_bypass', $bypass_course_limits_admin_users, $userId, $this->_post['course_id'], $this->_post['quiz'] );
						if ( true !== $bypass_course_limits_admin_users )  {
							$lockIp = true;
						}
					}
		...
		if ( ! $lockIp && ! $lockCookie ) {
			$statistics = new WpProQuiz_Controller_Statistics();
			ebox_quiz_debug_log_message( 'calling completeQuiz->save' );
			ebox_quiz_debug_log_message( 'quiz<pre>' . print_r( $quiz, true ) . '</pre>' );

			$statisticRefMapper_id = $statistics->save( $quiz );

			ebox_quiz_debug_log_message( 'calling do_action(wp_pro_quiz_completed_quiz) statisticRefMapper_id ' . $statisticRefMapper_id );
	...
	private function emailNote( WpProQuiz_Model_Quiz $quiz, $result, $categories ) {
		...
		$email_settings_admin = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Quizzes_Admin_Email' );

		$email_settings_user = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Quizzes_User_Email' );
		...
		if ( $quiz->isUserEmailNotification() ) {
			 ...
			 */
			$user_mail_message = apply_filters( 'ebox_quiz_email_note_user_message', $user_mail_message, $r, $quiz, $result, $categories );
			 ...
			 * @param WpProQuiz_Model_Quiz  $quiz         Quiz object.
			 */
			$email_params = apply_filters( 'ebox_quiz_email', $email_params, $quiz );
		...
		if ( $quiz->getEmailNotification() == WpProQuiz_Model_Quiz::QUIZ_EMAIL_NOTE_ALL
			 ...
			 * @param array                $categories Quiz categories
			 */
			$admin_mail_message = apply_filters( 'ebox_quiz_email_note_admin_message', $admin_mail_message, $r, $quiz, $result, $categories );
			$headers            = '';
			 ...
			 * @param WpProQuiz_Model_Quiz  $quiz         Quiz object.
			 */
			$email_params = apply_filters( 'ebox_quiz_email_admin', $email_params, $quiz );
			wp_mail( $email_params['email'], $email_params['subject'], $email_params['msg'], $email_params['headers'] );
		...
		if ( ( ! empty( $category_scores ) ) && ( ! empty( $question_cats ) ) ) {
			...
			foreach ( $question_cats as $cat ) {
				if ( ! $cat->getCategoryId() ) {
					$cat->setCategoryName( esc_html__( 'Not categorized', 'ebox' ) );
				}
