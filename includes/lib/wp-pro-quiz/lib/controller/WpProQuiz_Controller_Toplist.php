...
class WpProQuiz_Controller_Toplist extends WpProQuiz_Controller_Controller {
		...
		foreach ( $toplist as $tp ) {
			$j['data'][] = array(
				...
				'date'   => WpProQuiz_Helper_Until::convertTime(
					$tp->getDate(),
					ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Management_Display', 'toplist_time_format' )
	...
	private function editAdminToplist() {
		...
		if ( empty( $name ) || empty( $email ) || filter_var( $email, FILTER_VALIDATE_EMAIL ) === false ) {
			return array( 'error' => esc_html__( 'No name or e-mail entered.', 'ebox' ) );
		}
	...
	public function addInToplist() {
		...
		if ( ! empty( $user_id ) ) {
			if ( ebox_is_admin_user( $user_id ) ) {
				$include_admin_in_reports = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Admin_User', 'reports_include_admin_users' );
			 ...
			 */
			$include_admin_in_reports = apply_filters( 'ebox_quiz_add_toplist_bypass', $include_admin_in_reports, $user_id, $this->_post );
		}
		...
		if ( true === $r ) {
			$r = array(
				'text'  => esc_html__( 'You signed up successfully.', 'ebox' ),
				'clear' => true,
			);
		}
	...
	private function handleAddInToplist( WpProQuiz_Model_Quiz $quiz ) {
		...
		if ( $userId > 0 ) {
			if ( $toplistMapper->countUser( $quizId, $userId, $clearTime ) ) {
				return array(
					'text'  => esc_html__( 'You can not enter again.', 'ebox' ),
					'clear' => true,
				);
			}
			...
			if ( empty( $name ) || empty( $email ) || filter_var( $email, FILTER_VALIDATE_EMAIL ) === false ) {
				return array(
					'text'  => esc_html__( 'No name or e-mail entered.', 'ebox' ),
					'clear' => false,
				);
			}
			...
			if ( strlen( $name ) > 15 ) {
				return array(
					'text'  => esc_html__( 'Your name can not exceed 15 characters.', 'ebox' ),
					'clear' => false,
				);
			}
			...
			if ( $quiz->isToplistDataCaptcha() ) {
				...
				if ( $captcha->isSupported() ) {
					if ( ! $captcha->check( $prefix, $captchaAnswer ) ) {
						return array(
							'text'  => esc_html__( 'You entered wrong captcha code.', 'ebox' ),
							'clear' => false,
						);
					}
				}
