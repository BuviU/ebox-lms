...
class WpProQuiz_Controller_Admin {
	...
	public function loadQuizData() {
		...
		if ( ! wp_verify_nonce( $_POST['quiz_nonce'], 'ebox-quiz-nonce-' . $quiz_post_id . '-' . $id . '-' . $user_id ) ) {
	...
	public function adminToplist() {
		if ( ( isset( $_POST['nonce'] ) ) && ( ! empty( $_POST['nonce'] ) ) && ( wp_verify_nonce( $_POST['nonce'], 'ebox-wpproquiz-toplist' ) ) ) {
	...
	public function resetLock() {
		if ( ( isset( $_POST['nonce'] ) ) && ( ! empty( $_POST['nonce'] ) ) && ( wp_verify_nonce( $_POST['nonce'], 'ebox-wpproquiz-reset-lock' ) ) ) {
	...
	public function cookieSaveQuiz() {
		if ( is_user_logged_in() ) {
			...
			wp_send_json_error( array( 'message' => esc_html__( 'Your login has expired. Please log in and reload this page to continue.', 'ebox' ) ), 403 );
		}
		...
		if ( ( empty( $quiz_nonce ) ) || ( ! wp_verify_nonce( $quiz_nonce, 'ebox-quiz-nonce-' . $quiz_post_id . '-' . $id . '-' . $user_id ) ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Your login has expired. Please log in and reload this page to continue.', 'ebox' ) ), 403 );
		}
			...
			wp_send_json_error(
				array(
					'message' => sprintf(
						...
						esc_html_x( '%s data could not be saved to the server. Please reload the page and try again. If this error persists, please contact support.', 'placeholder: Quiz', 'ebox' ),
						esc_html( ebox_Custom_Label::get_label( 'quiz' ) )
					),
				)
			);
		}
	...
	public function completedQuiz() {
		...
		ebox_quiz_debug_log_message( '_POST<pre>' . print_r( $_POST, true ) . '</pre>' );
		...
		if ( ! wp_verify_nonce( $_POST['quiz_nonce'], 'ebox-quiz-nonce-' . $quiz_post_id . '-' . $id . '-' . $user_id ) ) {
			ebox_quiz_debug_log_message( 'quiznonce verify failed' );
			die();
		}
		...
		if ( ( isset( $_POST['results'] ) ) && ( ! empty( $_POST['results'] ) ) && ( is_string( $_POST['results'] ) ) ) {
			...
			ebox_quiz_debug_log_message( '_POST[results]<pre>' . print_r( $_POST['results'], true ) . '</pre>' );
		}
		...
		if ( ! isset( $_POST['results'] ) ) {
			return array(
				'text'  => esc_html__( 'An error has occurred.', 'ebox' ),
				'clear' => true,
			);
		}
		...
		ebox_quiz_debug_log_message( 'Verifying submitted results' );
		...
		foreach ( $_POST['results'] as $r_idx => $result ) {
			ebox_quiz_debug_log_message( '[' . $r_idx . '] result<pre>' . print_r( $result, true ) . '</pre>' );
			...
			if ( $r_idx !== 'comp' ) {
				...
				if ( ( isset( $result['p_nonce'] ) ) && ( ! empty( $result['p_nonce'] ) ) ) {
					...
					if ( ! wp_verify_nonce( $result['p_nonce'], $points_nonce ) ) {
						ebox_quiz_debug_log_message( 'invalid points nonce (p_nonce). Clearing points values.' );
						ebox_quiz_debug_log_message( 'p_nonce[' . $result['a_nonce'] . ' points_nonce[' . $points_nonce . ']' );
					...
					ebox_quiz_debug_log_message( 'missing/empty answer p_nonce. Clearing points values.' );
					$_POST['results'][ $r_idx ]['points']         = 0;
					$_POST['results'][ $r_idx ]['correct']        = 0;
					$_POST['results'][ $r_idx ]['possiblePoints'] = 0;
				...
				if ( ( isset( $result['a_nonce'] ) ) && ( ! empty( $result['a_nonce'] ) ) ) {
					global $ebox_completed_question;
					$ebox_completed_question = $question;
						...
						array_map(
							function ( $array_item ) {
								...
								if ( is_string( $array_item ) ) {
									...
									$question_legacy_sanitize_scheme = apply_filters( 'ebox_quiz_question_legacy_sanitize_scheme', false, $array_item, $ebox_completed_question );
					...
					if ( ! wp_verify_nonce( $result['a_nonce'], $answers_nonce ) ) {
						ebox_quiz_debug_log_message( 'invalid answer a_nonce. Clearing answer/response values.' );
						ebox_quiz_debug_log_message( 'a_nonce[' . $result['a_nonce'] . ' answers_nonce[' . $answers_nonce . ']' );
						$_POST['results'][ $r_idx ]['data'] = array();
					}
				} else {
					ebox_quiz_debug_log_message( 'missing/empty answer a_nonce. Clearing answer/response values.' );
					$_POST['results'][ $r_idx ]['data'] = array();
				}
			}
		}
		...
		if ( floatval( $comp_result ) !== floatval( $_POST['results']['comp']['result'] ) ) {
			ebox_quiz_debug_log_message( 'invalid or mismatched [comp][result] percentage sent [' . floatval( $_POST['results']['comp']['result'] ) . ']' );
			ebox_quiz_debug_log_message( 'Recalculated result value [' . floatval( $comp_result ) . '] will be used.' );

			$_POST['results']['comp']['result'] = $comp_result;
		}

		$quiz = new WpProQuiz_Controller_Quiz();
		ebox_quiz_debug_log_message( 'calling completeQuiz' );
		$quiz->completedQuiz();
	}
	...
	private function localizeScript() {
		...
		$translation_array = array(
			// translators: placeholder: quiz.
			'delete_msg'                          => sprintf( esc_html_x( 'Do you really want to delete the %s/question?', 'placeholder: quiz.', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) ),
			'no_title_msg'                        => esc_html__( 'Title is not filled!', 'ebox' ),
			// translators: placeholder: question.
			'no_question_msg'                     => sprintf( esc_html_x( 'No %s deposited!', 'placeholder: question', 'ebox' ), ebox_get_custom_label_lower( 'question' ) ),
			'no_correct_msg'                      => esc_html__( 'Correct answer was not selected!', 'ebox' ),
			'no_answer_msg'                       => esc_html__( 'No answer deposited!', 'ebox' ),
			// translators: placeholder: quiz.
			'no_quiz_start_msg'                   => sprintf( esc_html_x( 'No %s description filled!', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) ),
			'fail_grade_result'                   => esc_html__( 'The percent values in result text are incorrect.', 'ebox' ),
			'no_nummber_points'                   => esc_html__( 'No number in the field "Points" or less than 1', 'ebox' ),
			'no_nummber_points_new'               => esc_html__( 'No number in the field "Points" or less than 0', 'ebox' ),
			// translators: placeholder: quiz.
			'no_selected_quiz'                    => sprintf( esc_html_x( 'No %s selected', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) ),
			'reset_statistics_msg'                => esc_html__( 'Do you really want to reset the statistic?', 'ebox' ),
			'no_data_available'                   => esc_html__( 'No data available', 'ebox' ),
			'no_sort_element_criterion'           => esc_html__( 'No sort element in the criterion', 'ebox' ),
			'dif_points'                          => esc_html__( '"Different points for every answer" is not possible at "Free" choice', 'ebox' ),
			'category_no_name'                    => esc_html__( 'You must specify a name.', 'ebox' ),
			'confirm_delete_entry'                => esc_html__( 'This entry should really be deleted?', 'ebox' ),
			'not_all_fields_completed'            => esc_html__( 'Not all fields completed.', 'ebox' ),
			'temploate_no_name'                   => esc_html__( 'You must specify a template name.', 'ebox' ),
			'no_delete_answer'                    => esc_html__( 'Cannot delete only answer', 'ebox' ),

			'metabox_title_correct_message'       => esc_html__( 'Message with the correct answer', 'ebox' ) . ' ' . esc_html__( '(optional)', 'ebox' ),
			'metabox_title_correct_message_essay' => esc_html__( 'Message after Essay is submitted - optional', 'ebox' ),

			'closeText'                           => esc_html__( 'Close', 'ebox' ),
			'currentText'                         => esc_html__( 'Today', 'ebox' ),
			...
			'isRTL'                               => $isRtl,
			'select2_enabled'                     => ebox_use_select2_lib(),
			'select2_fetch_enabled'               => ebox_use_select2_lib_ajax_fetch(),
		);
	...
	public function register_page() {
		// translators: placeholder: Quiz.
		$quiz_title = sprintf( esc_html_x( 'Advanced %s', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'quiz' ) );
