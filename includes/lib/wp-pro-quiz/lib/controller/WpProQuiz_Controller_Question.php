...
class WpProQuiz_Controller_Question extends WpProQuiz_Controller_Controller {
	...
	private function addEditQuestion( $quizId ) {
		...
		if ( $questionId && $questionMapper->existsAndWritable( $questionId ) == 0 ) {
			WpProQuiz_View_View::admin_notices(
				...
				// translators: placeholder: Question.
					esc_html_x( '%s not found', 'placeholder: Question', 'ebox' ),
		...
		if ( isset( $this->_post['template'] ) || ( isset( $this->_post['templateLoad'] ) && isset( $this->_post['templateLoadId'] ) ) ) {
			...
			// translators: placeholder: question.
			$add_new_question = "<a href='" . $add_new_question_url . "'>" . sprintf( esc_html_x( 'Click here to add another %s.', 'placeholder: question', 'ebox' ), ebox_Custom_Label::get_label( 'question' ) ) . '</a>';
		...
		// translators: placeholder: question.
		$this->view->header = $questionId ? sprintf( esc_html_x( 'Edit %s', 'placeholder: question', 'ebox' ), ebox_get_custom_label( 'question' ) ) : sprintf( esc_html_x( 'New %s', 'placeholder: question', 'ebox' ), ebox_get_custom_label( 'question' ) );
	...
	public function getPostQuestionModel( $quizId, $questionId ) {
		...
		if ( ( isset( $post['title'] ) ) && ( empty( $post['title'] ) ) ) {
			...
			$post['title'] = sprintf( esc_html_x( '%1$s: %2$d', 'placeholder: Question, question count', 'ebox' ), ebox_get_custom_label( 'question' ), $count + 1 );
		}
	...
	public function editAction( $id ) {
		...
		// translators: placeholder: question.
		$this->view->header = sprintf( esc_html_x( 'Edit %s', 'placeholder: question', 'ebox' ), ebox_get_custom_label( 'question' ) );
	...
	public function editPostAction( $id ) {
		...
		if ( isset( $this->_post['submit'] ) && $mapper->existsAndWritable( $id ) ) {
			...
			if ( empty( $post['title'] ) ) {
				...
				$post['title'] = sprintf( esc_html_x( 'Question: %d', 'placeholder: Quiz sequence', 'ebox' ), $question->getSort() + 1 );
			}
	...
	public function createAction() {
		...
		// translators: placeholder: question.
		$this->view->header = sprintf( esc_html_x( 'New %s', 'placeholder: question', 'ebox' ), ebox_get_custom_label( 'question' ) );
	...
	public function clearPost( $post ) {
		...
		if ( ( isset( $post['answerType'] ) ) && ( 'cloze_answer' == $post['answerType'] ) && ( isset( $post['answerData']['cloze'] ) ) ) {
			$question_cloze_data = ebox_question_cloze_fetch_data( $post['answerData']['cloze']['answer'] );
		...
		if ( ( isset( $post['answerType'] ) ) && ( 'assessment_answer' == $post['answerType'] ) && ( isset( $post['answerData']['assessment'] ) ) ) {
			if ( isset( $post['ebox-question_quiz'] ) ) {
				$quiz_id = absint( $post['ebox-question_quiz'] );
			} else {
				$quiz_id = 0;
			}
			...
			$cloze_answer_data = ebox_question_assessment_fetch_data( $post['answerData']['assessment']['answer'], $quiz_id, $question_id );
		...
		if ( ( isset( $post['answerType'] ) ) && ( 'free_answer' == $post['answerType'] ) && ( isset( $post['answerData'] ) ) ) {
			foreach ( $post['answerData'] as $k => $v ) {
				...
				$answer_data = ebox_question_free_get_answer_data( $answerType );
