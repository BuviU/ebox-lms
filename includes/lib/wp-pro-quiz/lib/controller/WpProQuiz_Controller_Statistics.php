...
class WpProQuiz_Controller_Statistics extends WpProQuiz_Controller_Controller {
	...
	private function show( $quizId ) {
		...
		$categoryEmpty->setCategoryName( __( 'No category', 'ebox' ) );
			...
			if ( has_action( 'pre_user_query', 'ure_exclude_administrators' ) ) {
				...
				if ( ! ebox_is_admin_user( get_current_user_id() ) ) {
					if ( ebox_is_team_leader_user( get_current_user_id() ) ) {
						...
						$include_user_ids = ebox_get_teams_leaders_users_for_course_step( $quiz->getPostId() );
					} else {
	...
	public function save( $quiz = null ) {
		ebox_quiz_debug_log_message( 'in ' . __CLASS__ . ' - ' . __FUNCTION__ );
		...
		ebox_quiz_debug_log_message( 'quizId ' . $quizId );
		...
		ebox_quiz_debug_log_message( 'course_post_id ' . $course_post_id );

		$array  = $this->_post['results'];
		$lockIp = $this->getIp();
		ebox_quiz_debug_log_message( 'lockIp ' . $lockIp );

		$userId = get_current_user_id();

		if ( false === $lockIp ) {
			ebox_quiz_debug_log_message( 'lockIp === false - aborting' );
			return false;
		}
		...
		if ( ! $quiz->isStatisticsOn() ) {
			ebox_quiz_debug_log_message( 'quiz->isStatisticsOn not enabled? - aborting' );
			return false;
		}
		...
		$statisticRefMapper = new WpProQuiz_Model_StatisticRefMapper();
		ebox_quiz_debug_log_message( 'calling statisticRefMapper->statisticSave' );
		ebox_quiz_debug_log_message( 'values<pre>' . print_r( $values, true ) . '</pre>' );

		$statisticRefMapper_id = $statisticRefMapper->statisticSave( $statisticRefModel, $values );

		ebox_quiz_debug_log_message( 'statisticRefMapper_id ' . $statisticRefMapper_id );
	...
	public static function ajaxLoadHistory( $data, $func ) {
		...
		foreach ( $statisticModel as $model ) {
			if ( ! $model->getUserId() ) {
				...
				$model->setUserName( __( 'Deleted user', 'ebox' ) );
			}
			...
			$date_format = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Management_Display', 'statistics_time_format' );
	...
	public static function ajaxLoadStatisticUser( $data, $func ) {
		 ...
		 */
		$quiz_post_id = ebox_get_quiz_id_by_pro_quiz_id( $quizId );
		$quiz_post_id = absint( $quiz_post_id );
		if ( ( ! empty( $statisticUsers ) ) && ( ! empty( $quiz_post_id ) ) ) {
			$quiz_questions = ebox_get_quiz_questions( $quiz_post_id );
		...
		foreach ( $statisticUsers as $statistic ) {
			if ( ! isset( $output[ $statistic->getCategoryId() ] ) ) {
				$output[ $statistic->getCategoryId() ] = array(
					...
					'categoryName' => $statistic->getCategoryId() ? $statistic->getCategoryName() : esc_html__( 'No category', 'ebox' ),
				);
			}
			...
			// the result is the incorrect field will be set to 1. See ebox-212
			if ( 'essay' == $question_item['answerType'] ) {
				if ( ( isset( $question_item['statistcAnswerData']['graded_id'] ) ) && ( ! empty( $question_item['statistcAnswerData']['graded_id'] ) ) ) {
					...
					if ( ( $graded_post ) && ( is_a( $graded_post, 'WP_Post' ) ) ) {
						$graded_data = ebox_get_submitted_essay_data( $data['quizId'], $question_item['questionId'], $graded_post );
						if ( isset( $graded_data['status'] ) ) {
							if ( 'graded' === $graded_data['status'] ) {
								if ( isset( $graded_data['points_awarded'] ) ) {
									...
									$question_item['result']    = esc_html__( 'Graded', 'ebox' );
								}
							} else {
								$question_item['result'] = esc_html__( 'Ungraded', 'ebox' );
							}
						}
					}
				}
			 ...
			 */
			$question_item['result'] = apply_filters( 'ebox-quiz-statistics-result', $question_item['result'], $question_item );
		...
		$view->userName = esc_html__( 'Anonymous', 'ebox' );
		...
		if ( $view->statisticModel->getUserId() ) {
			...
			if ( false !== $userInfo ) {
				...
				$view->userName = esc_html__( 'Deleted user', 'ebox' );
			}
		}
