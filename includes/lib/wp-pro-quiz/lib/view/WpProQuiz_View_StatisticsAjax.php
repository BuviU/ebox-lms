...
class WpProQuiz_View_StatisticsAjax extends WpProQuiz_View_View {
	...
	public function showHistoryTable() {
		...
		<table class="wp-list-table widefat">
			...
			<tbody id="wpProQuiz_statistics_form_data">
				...
				<tr>
					<td colspan="6" style="text-align: center; font-weight: bold; padding: 10px;"><?php esc_html_e( 'No data available', 'ebox' ); ?></td>
				</tr>
				<?php } else { ?>
	...
	public function showUserTable() {
		$filepath = ebox_LMS::get_template( 'ebox_quiz_statistics.css', null, null, true );
		...
		printf( esc_html_x( 'User statistics: %s', 'placeholder: user name', 'ebox' ), esc_html( $this->userName ) );
		?>
		</h2>
		<?php if ( $this->avg ) { ?>
		<h2>
			...
			echo date_i18n( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Management_Display', 'statistics_time_format' ),
		...
		<table class="wp-list-table widefat" style="margin-top: 20px;">
			<thead>
				<tr>
					...
					<th scope="col" style="width: 95px;"><?php esc_html_e( 'Results', 'ebox' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				...
				<tr class="categoryTr">
					...
					foreach ( $cat['questions'] as $q ) {
						 ...
						 */
						$q = apply_filters( 'ebox_question_statistics_data', $q, $quiz, $_POST );  // phpcs:ignore WordPress.Security.NonceVerification.Missing
				...
				<tr>
					...
					<th>
						...
						if ( ! $this->avg && null !== $q['statistcAnswerData'] ) {
							...
							*/
							 ...
							 * @param array  $http_post_data An array of global http post data.
							 */
							$q['questionName'] = apply_filters( 'ebox_quiz_statistics_questionName', $q['questionName'], $q, $_POST );  // phpcs:ignore WordPress.Security.NonceVerification.Missing
							...
							?>
							<a href="#" class="statistic_data"><?php esc_html_e( '(view)', 'ebox' ); ?></a>
							<?php
					...
					<tr style="display: none;">
						<th colspan="9">
							 ...
							 * @param array   $http_post_data An array of global http post data.
							 */
							$show_messages = apply_filters( 'ebox_quiz_statistics_show_feedback_messages', $q['questionShowMsgs'], $q, $quiz, $_POST );  // phpcs:ignore WordPress.Security.NonceVerification.Missing
							if ( $show_messages ) {
								$answerText = '';
								if ( true == $q['correct'] ) {
									 ...
									 */
									$q['questionCorrectMsg'] = apply_filters( 'ebox_quiz_statistics_questionCorrectMsg', $q['questionCorrectMsg'], $q, $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
									 ...
									 * @param array  $http_post       An array of global http post data.
									 */
									$q['questionIncorrectMsg'] = apply_filters( 'ebox_quiz_statistics_questionIncorrectMsg', $q['questionIncorrectMsg'], $q, $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		<div style="margin-top: 10px;">
			...
			<div style="float: right;">
				<?php if ( current_user_can( 'wpProQuiz_reset_statistics' ) ) { ?>
					<a class="button-secondary" href="#" id="wpProQuiz_resetUserStatistic"><?php esc_html_e( 'Reset statistics', 'ebox' ); ?></a>
				<?php } ?>
			</div>
		...
		<ul class="wpProQuiz_questionList">
			...
			for ( $i = 0; $i < $count_answer_data; $i++ ) {
				...
				<li class="<?php echo esc_attr( $correct ); ?>">
					...
					$questionData = ebox_question_free_get_answer_data( $qAnswerData[ $i ] );
					...
					if ( ! $quiz->isDisabledAnswerMark() ) {
						...
						if ( ( ! empty( $questionData['correct'] ) ) && ( '' !== $userResponse_filtered ) ) {
							foreach ( $questionData['correct'] as $questionData_correct ) {
								...
								if ( apply_filters( 'ebox_quiz_question_free_answers_to_lowercase', true, null ) ) {
				...
				<li>
					...
					$cloze_output = ebox_question_cloze_prepare_output( $cloze_data );
					...
					$assessment = apply_filters( 'ebox_quiz_question_answer_postprocess', $assessment, 'assessment' );
					...
					if ( ( ! isset( $sAnswerData['graded_id'] ) ) || ( empty( $sAnswerData['graded_id'] ) ) ) {
						...
						if ( ( ! empty( $userId ) ) && ( ! empty( $quizId ) ) && ( ! empty( $statisticRefId ) ) ) {
							$user_quizzes = get_user_meta( $userId, '_ebox-quizzes', true );
					...
					if ( ( isset( $sAnswerData['graded_id'] ) ) && ( ! empty( $sAnswerData['graded_id'] ) ) ) {
							...
							<li class="<?php echo esc_attr( $correct ); ?>">
								<div class="wpProQuiz_sortable">
									...
									if ( 'graded' == $essay_post->post_status ) {
										esc_html_e( 'Status: Graded', 'ebox' );
									} else {
										esc_html_e( 'Status: Not Graded', 'ebox' );
									}

									if ( ( ebox_is_team_leader_user() ) || ( ebox_is_admin_user() ) || ( get_current_user_id() == $essay_post->post_author ) ) {
										?>
											(<a target="_blank" href="<?php echo esc_url( get_permalink( $sAnswerData['graded_id'] ) ); ?>"><?php esc_html_e( 'view', 'ebox' ); ?></a>)
											<?php
									}

									if ( current_user_can( 'edit_post', $sAnswerData['graded_id'] ) ) {
										?>
											(<a target="_blank" href="<?php echo esc_url( get_edit_post_link( $sAnswerData['graded_id'] ) ); ?>"><?php esc_html_e( 'edit', 'ebox' ); ?></a>)
											<?php
									}
									?>
								</div>
						...
						<li class="<?php echo esc_attr( $correct ); ?>">
							<div class="wpProQuiz_sortable">
							<?php echo esc_html__( 'Essay not submitted', 'ebox' ); ?>
							</div>
						</li>
						<?php
					}
	...
	private function fetchAssessment( $answerText, $answerData ) {
		...
		$assessment_data = ebox_question_assessment_fetch_data( $answerText, 0, 0 );
		if ( ( isset( $assessment_data['correct'] ) ) && ( is_array( $assessment_data['correct'] ) ) && ( ! empty( $assessment_data['correct'] ) ) ) {
			...
			return ebox_question_assessment_prepare_output( $assessment_data );
		}
	}
	...
	private function fetchCloze( $answer_text, $answerData, $question_pro_id = 0 ) {
		...
		$question_cloze_data = ebox_question_cloze_fetch_data( $answer_text );
					...
					if ( $question_model->isAnswerPointsActivated() ) {
						if ( isset( $question_cloze_data['points'][ $correct_key ][ $correct_set_idx ] ) ) {
							...
							$correct_set_text .= ' <span class="wpProQuiz_cloze_answerPoints">' . sprintf(
								...
								_n( '- %dpt', '- %dpts', absint( $points ), 'ebox' ),
										...
										<td>
											...
											switch ( $form->getType() ) {
												...
												case WpProQuiz_Model_Form::FORM_TYPE_CHECKBOX:
													echo '1' == $str ? esc_html__( 'ticked', 'ebox' ) : esc_html__( 'not ticked', 'ebox' );
													break;
												case WpProQuiz_Model_Form::FORM_TYPE_YES_NO:
													echo 1 == $str ? esc_html__( 'Yes', 'ebox' ) : esc_html__( 'No', 'ebox' );
													break;
	...
	public function showOverviewTable() {
		...
		<table class="wp-list-table widefat">
			...
			<tbody>
				...
				<tr>
					<td colspan="7" style="text-align: center; font-weight: bold; padding: 10px;"><?php esc_html_e( 'No data available', 'ebox' ); ?></td>
				</tr>
				<?php } else { ?>
					...
					foreach ( $this->statisticModel as $model ) {
						...
						if ( ! $model->getUserId() ) {
							$model->setUserName( __( 'Anonymous', 'ebox' ) );
						}
