...
<div class="wrap wpProQuiz_importOverall">
	...
	<div style="padding: 10px; background-color: #C7E4FF; margin-top: 20px; border: 1px dotted;">
		<h3 style="margin-top: 0;"><?php esc_html_e( 'Successfully', 'ebox' ); ?></h3>
		<div>
			<?php
				$edit_link = '';
			if ( $this->import_post_id ) {
				...
				echo wpautop( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					sprintf(
						...
						esc_html_x( 'Import completed successfully: %s', 'placeholder: link to Imported Quiz.', 'ebox' ),
						$edit_link
					)
				);
			?>
	...
	<form method="post">
		<table class="wp-list-table widefat">
			<thead>
				<tr>
					...
					echo sprintf( esc_html_x( '%s name', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'quiz' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
					?>
					</th>
					<th scope="col"><?php esc_html_e( 'Questions', 'ebox' ); ?></th>
				</tr>
			</thead>
			<tbody>
		...
		<input style="margin-top: 20px;" class="button-primary" name="importSave" value="<?php esc_html_e( 'Start import', 'ebox' ); ?>" type="submit">
	</form>
	<?php } ?>
</div>
