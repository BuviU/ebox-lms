...
class WpProQuiz_View_FrontQuiz extends WpProQuiz_View_View {
	...
	private function getFreeCorrect( $data, $question = null ) {
		...
		foreach ( $t as $idx => $item ) {
			...
			if ( '' == $item ) {
				...
				if ( apply_filters( 'ebox_quiz_question_free_answers_to_lowercase', true, $question ) ) {
	...
	public function show( $preview = false ) {
		...
		if ( is_user_logged_in() ) {
			$quiz_resume_enabled = (bool) ebox_get_setting(
				$this->quiz->getPostId(),
				'quiz_resume'
			);
			...
			if ( true === $quiz_resume_enabled ) {
				...
				if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) !== 'yes' ) {
					$course_id = ebox_get_setting( $this->quiz->getPostId(), 'course' );
	...
	public function script( $preview = false ) {
		...
		if ( ( isset( $this->_shortcode_atts['quiz_id'] ) ) && ( ! empty( $this->_shortcode_atts['quiz_id'] ) ) ) {
			...
			if ( 'ebox-quiz' != @$post->post_type ) {
				$quiz_post_id = ebox_get_quiz_id_by_pro_quiz_id( $quiz_pro_id );
			}
		}

		if ( ( isset( $quiz_post_id ) ) && ( ! empty( $quiz_post_id ) ) ) {
			$quiz_meta = get_post_meta( $quiz_post_id, '_ebox-quiz', true );
		} else {
			$quiz_meta = array();
		}

		if ( ( isset( $quiz_meta['ebox-quiz_passingpercentage'] ) ) && ( ! empty( $quiz_meta['ebox-quiz_passingpercentage'] ) ) ) {
			$quiz_meta_ebox_quiz_passingpercentage = floatval( $quiz_meta['ebox-quiz_passingpercentage'] );
		} else {
			$quiz_meta_ebox_quiz_passingpercentage = 0;
		}
		...
		if ( ( isset( $this->_shortcode_atts['course_id'] ) ) && ( ! empty( $this->_shortcode_atts['course_id'] ) ) ) {
			...
			if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) !== 'yes' ) {
				$course_id = ebox_get_setting( $quiz_post_id, 'course' );
				$course_id = absint( $course_id );
			}
		}
		...
		if ( ( isset( $this->_shortcode_atts['lesson_id'] ) ) && ( ! empty( $this->_shortcode_atts['lesson_id'] ) ) ) {
			...
			if ( ! empty( $course_id ) ) {
				$lesson_id = ebox_course_get_single_parent_step( $course_id, $quiz_post_id, 'ebox-modules' );
			}
		}
		...
		if ( ( isset( $this->_shortcode_atts['topic_id'] ) ) && ( ! empty( $this->_shortcode_atts['topic_id'] ) ) ) {
			...
			if ( ! empty( $course_id ) ) {
				$topic_id = ebox_course_get_single_parent_step( $course_id, $quiz_post_id, 'ebox-topic' );
			}
		}
		...
		$quiz_nonce = '';
		if ( ! empty( $user_id ) ) {
			$quiz_nonce = wp_create_nonce( 'ebox-quiz-nonce-' . $quiz_post_id . '-' . $quiz_pro_id . '-' . $user_id );
		} else {
			$quiz_nonce = wp_create_nonce( 'ebox-quiz-nonce-' . $quiz_post_id . '-' . $quiz_pro_id . '-0' );
		}
		...
		$quiz_resume_cookie_send_timer = ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_MIN;
		$quiz_resume_cookie_expiration = 604800; // 7 days.
		$quiz_resume_quiz_started      = 0;

		if ( is_user_logged_in() ) {
			$quiz_resume_enabled = (bool) ebox_get_setting( $quiz_post_id, 'quiz_resume' );
			if ( true === $quiz_resume_enabled ) {
				$quiz_resume_cookie_send_timer = (int) ebox_get_setting( $quiz_post_id, 'quiz_resume_cookie_send_timer' );
				if ( ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_MIN < $quiz_resume_cookie_send_timer ) {
					$quiz_resume_cookie_send_timer = ebox_QUIZ_RESUME_COOKIE_SEND_TIMER_MIN;
				}
		...
		$quiz_resume_data = ebox_prepare_quiz_resume_data_to_js( $quiz_resume_data );
		...
		function load_wpProQuizFront" . esc_attr( $this->quiz->getId() ) . "() {
			jQuery('#wpProQuiz_" . esc_attr( $this->quiz->getId() ) . "').wpProQuizFront({
				...
				bo: ' . (int) $bo . ',
				passingpercentage: ' . (int) $quiz_meta_ebox_quiz_passingpercentage . ',
				user_id: ' . (int) $user_id . ',
				...
				essayUploading: '" . esc_html(
					ebox_LMS::get_template(
						...
						array(
							...
							'message'      => esc_html__( 'Uploading', 'ebox' ),
						)
					)
				) . "',
				essaySuccess: '" . esc_html(
					ebox_LMS::get_template(
						...
						array(
							...
							'message'      => esc_html__( 'Success', 'ebox' ),
						)
					)
				) . "',
				lbn: " . wp_json_encode(
					( $this->quiz->isShowReviewQuestion() && ! $this->quiz->isQuizSummaryHide() ) ? ebox_LMS::get_template(
						'ebox_quiz_messages',
						array(
							'quiz_post_id' => $this->quiz->getID(),
				 ...
				 */
				(int) apply_filters( 'ebox_quiz_scroll_sensitivity', 10, $quiz_post_id, $user_id ) . "',
				scrollSpeed: '" .
				/**
				 ...
				 * @param int $user_id      User ID
				 */
				(int) apply_filters( 'ebox_quiz_scroll_speed', 10, $quiz_post_id, $user_id ) . "',
				quiz_resume_enabled:  '" .
				/**
				 ...
				 *
				 */
				(int) apply_filters( 'ebox_quiz_resume_enabled', $quiz_resume_enabled, $quiz_post_id, $user_id ) . "',
				 ...
				 *
				 */
				wp_json_encode( apply_filters( 'ebox_quiz_resume_data', $quiz_resume_data, $quiz_post_id, $user_id ), JSON_HEX_APOS ) . "',
				 ...
				 *
				 */
				(int) apply_filters( 'ebox_quiz_resume_cookie_expiration', $quiz_resume_cookie_expiration, $quiz_post_id, $user_id ) . "',
				 ...
				 *
				 */
				(int) apply_filters( 'ebox_quiz_resume_cookie_send_timer', $quiz_resume_cookie_send_timer, $quiz_post_id, $user_id ) . "',
			});
		}
	...
	public function max_question_script() {
		...
		if ( 'ebox-quiz' != @$post->post_type ) {
			...
			$quiz_post_id = ebox_get_quiz_id_by_pro_quiz_id( $quiz_id );
		} else {
		...
		if ( ( isset( $quiz_meta['ebox-quiz_passingpercentage'] ) ) && ( ! empty( $quiz_meta['ebox-quiz_passingpercentage'] ) ) ) {
			$quiz_meta_ebox_quiz_passingpercentage = intval( $quiz_meta['ebox-quiz_passingpercentage'] );
		} else {
			$quiz_meta_ebox_quiz_passingpercentage = 0;
		}
		...
		$quiz_nonce = '';
		if ( ! empty( $user_id ) ) {
			$quiz_nonce = wp_create_nonce( 'ebox-quiz-nonce-' . $quiz_post_id . '-' . $this->quiz->getId() . '-' . $user_id );
		} else {
			$quiz_nonce = wp_create_nonce( 'ebox-quiz-nonce-' . $quiz_post_id . '-' . $this->quiz->getId() . '-0' );
		}
		...
		jQuery( function($) {
			$('#wpProQuiz_" . (int) $this->quiz->getId() . "').wpProQuizFront({
				...
				quiz_resume_data: '" .
				/** This filter is documented in includes/lib/wp-pro-quiz/lib/view/WpProQuiz_ViewFrontQuiz.php */
				wp_json_encode( apply_filters( 'ebox_quiz_resume_data', $quiz_resume_data, $quiz_post_id, $user_id ) ) . "',
				quiz_resume_cookie_expiration: '" .
				/** This filter is documented in includes/lib/wp-pro-quiz/lib/view/WpProQuiz_ViewFrontQuiz.php */
				(int) apply_filters( 'ebox_quiz_resume_cookie_expiration', $quiz_resume_cookie_expiration, $quiz_post_id, $user_id ) . "',
				quiz_resume_cookie_send_timer: '" .
	 ...
	 *
	 * @deprecated 3.5.0 Use {@see 'ebox_question_cloze_fetch_data'} instead.
	 *
	 * @param string $answer_text Question answer text.
	 */
	private function fetchCloze( $answer_text ) {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '3.5.0', 'ebox_question_cloze_fetch_data' );
		}

		return ebox_question_cloze_fetch_data( $answer_text );
	}
	 ...
	 *
	 * @deprecated 3.5.0 Use {@see 'ebox_question_assessment_fetch_data'} instead.
	 *
	...
	private function fetchAssessment( $answerText, $quizId, $questionId ) {
		if ( function_exists( '_deprecated_function' ) ) {
			_deprecated_function( __FUNCTION__, '3.5.0', 'ebox_question_assessment_fetch_data' );
		}
		return ebox_question_assessment_fetch_data( $answerText, $quizId, $questionId );
	}
	...
	private function showUserQuizStatisticsBox() {
		...
		if ( current_user_can( 'wpProQuiz_show_statistics' ) ) {
			$user_quizzes = get_user_meta( get_current_user_id(), '_ebox-quizzes', true );
											...
											echo esc_html(
												ebox_LMS::get_template(
													...
													array(
														...
														// translators: placeholder: Quiz.
														'message'       => sprintf( esc_html_x( 'View %s Statistics', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'quiz' ) ),
													)
												)
											);
	...
	public function showQuizBox( $questionCount ) {
		...
		$filepath = ebox_LMS::get_template(
