...
<div class="wrap wpProQuiz_questionOverall">
	<h1><?php echo ebox_Custom_Label::get_label( 'quiz' ); ?>: <?php echo wp_kses_post( $this->quiz->getName() ); ?></h1> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output ?>
	<div id="sortMsg" class="updated" style="display: none;"><p><strong>
		<?php
		printf(
			// translators: placeholder: Questions.
			esc_html_x( '%s sorted', 'placeholder: Questions', 'ebox' ),
	...
	<p>
		...
		<a class="button-secondary" href="admin.php?page=ldAdvQuiz&module=question&action=addEdit&quiz_id=<?php echo absint( $this->quiz->getId() ); ?>&post_id=<?php echo absint( $post_id ); ?>">
			...
			// translators: placeholder: Question.
				esc_html_x( 'Add %s', 'placeholder: Question', 'ebox' ),
				ebox_Custom_Label::get_label( 'questions' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			);
			?>
		</a>
	...
	<table class="wp-list-table widefat">
		<thead>
			<tr>
				...
				<th scope="col"><?php esc_html_e( 'Name', 'ebox' ); ?></th>
				<th scope="col"><?php esc_html_e( 'Type', 'ebox' ); ?></th>
				<th scope="col"><?php esc_html_e( 'Category', 'ebox' ); ?></th>
				<th scope="col"><?php esc_html_e( 'Points', 'ebox' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
			...
			if ( count( $this->question ) ) {
				...
				<tr id="wpProQuiz_questionId_<?php echo absint( $question->getId() ); ?>">
					...
					<td>
						...
						if ( current_user_can( 'wpProQuiz_edit_quiz' ) ) {
							...
							<?php if ( current_user_can( 'wpProQuiz_edit_quiz' ) ) { ?>
								<span><a href="admin.php?page=ldAdvQuiz&module=question&action=addEdit&quiz_id=<?php echo absint( $this->quiz->getId() ); ?>&questionId=<?php echo absint( $question->getId() ); ?>&post_id=<?php echo absint( $post_id ); ?>"><?php esc_html_e( 'Edit', 'ebox' ); ?></a></span>
							...
							<span>
								...
								?>
								"><?php esc_html_e( 'Delete', 'ebox' ); ?></a> |
							</span>
							<?php } if ( current_user_can( 'wpProQuiz_edit_quiz' ) ) { ?>
							<span>
								<a class="wpProQuiz_move" href="#" style="cursor:move;"><?php esc_html_e( 'Move', 'ebox' ); ?></a>
							</span>
							<?php } ?>
						</div>
					</td>
					<td>
						...
						if ( isset( $ebox_question_types[ $question_type ] ) ) {
							echo esc_html( $ebox_question_types[ $question_type ] );
						}
						?>
					</td>
					<td>
				...
				?>
				<tr>
					<td colspan="5" style="text-align: center; font-weight: bold; padding: 10px;"><?php esc_html_e( 'No data available', 'ebox' ); ?></td>
				</tr>
				<?php
			}
			?>
		...
		<tfoot>
			<tr>
				<th></th>
				<th style="font-weight: bold;"><?php esc_html_e( 'Total', 'ebox' ); ?></th>
				<th></th>
				<th></th>
	...
	<p>
		 ...
		 */
		do_action( 'ebox_questions_buttons_before' );
		?>
		<?php if ( current_user_can( 'wpProQuiz_edit_quiz' ) ) { ?>
		<a class="button-secondary" href="admin.php?page=ldAdvQuiz&module=question&action=addEdit&quiz_id=<?php echo absint( $this->quiz->getId() ); ?>&post_id=<?php echo absint( $post_id ); ?>">
			<?php
		...
		<a class="button-secondary" href="#" id="wpProQuiz_saveSort"><?php esc_html_e( 'Save order', 'ebox' ); ?></a>
		<a class="button-secondary" href="#" id="wpProQuiz_questionCopy">
			<?php
			// translators: placeholder: questions, quiz.
			printf( esc_html_x( 'Copy %1$s from another %2$s', 'placeholder: questions, quiz', 'ebox' ), ebox_get_custom_label_lower( 'questions' ), ebox_get_custom_label_lower( 'quiz' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			?>
		</a>
		<?php } ?>
		<?php
		 ...
		 */
		do_action( 'ebox_questions_buttons_after' );
		?>
	</p>
		<?php
		/**
		 ...
		 */
		do_action( 'ebox_questions_toolbox_before' );
		?>
	<div class="wpProQuiz_questionCopy">
		<form action="admin.php?page=ldAdvQuiz&module=question&quiz_id=<?php echo absint( $this->quiz->getId() ); ?>&action=copy_question" method="POST">
			...
			<?php
			// translators: placeholder: questions, quiz.
			echo sprintf( esc_html_x( 'Copy %1$s from another %2$s', 'placeholder: questions, quiz', 'ebox' ), ebox_get_custom_label_lower( 'questions' ), ebox_get_custom_label_lower( 'quiz' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			?>
			</h2>
			<p>
			<?php
			// translators: placeholders: questions, quiz, quiz.
			echo sprintf( esc_html_x( 'Here you can copy %1$s from another %2$s into this %3$s. (Multiple selection enabled)', 'placeholders: questions, quiz, quiz', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'questions' ) ), esc_html( ebox_get_custom_label_lower( 'quiz' ) ), esc_html( ebox_get_custom_label_lower( 'quiz' ) ) );
			?>
			</p>
			...
			<input class="button-primary" name="questionCopy" value="<?php sprintf( esc_html_x( 'Copy %s', 'placeholder: questions', 'ebox' ), ebox_get_custom_label_lower( 'questions' ) ); ?>" type="submit">
		</form>
	</div>
</div>
		<?php
