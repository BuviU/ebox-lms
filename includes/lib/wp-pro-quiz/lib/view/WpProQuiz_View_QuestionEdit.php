...
<div class="wrap wpProQuiz_questionEdit">
	...
	<form action="admin.php?page=ldAdvQuiz&module=question&action=addEdit&quiz_id=<?php echo intval( $this->quiz->getId() ); ?>&questionId=<?php echo intval( $this->question->getId() ); ?>&post_id=<?php echo intval( $post_id ); ?>" method="POST">
		...
		printf(
			...
			esc_html_x( 'Return to %s Overview', 'placeholder: Question', 'ebox' ),
		...
		<div id="poststuff">
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Title', 'ebox' ); ?> <?php esc_html_e( '(optional)', 'ebox' ); ?></h3>
				<div class="inside">
					<p class="description">
						<?php
						// translators: placeholder: quiz.
						echo sprintf( esc_html_x( 'The title is used for overview, it is not visible in %s. If you leave the title field empty, a title will be generated.', 'placeholder: quiz', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'quiz' ) ) );
						?>
					</p>
			...
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Points', 'ebox' ); ?> <?php esc_html_e( '(required)', 'ebox' ); ?></h3>
				<div class="inside">
					<div id="wpProQuiz_questionPoints>
						...
						<label>
							<input name="points" class="small-text" value="<?php echo esc_attr( $this->question->getPoints() ); ?>" type="number" min="1"> <?php esc_html_e( 'Points', 'ebox' ); ?>
						</label>
						<p class="description">
							<?php
								printf(
									// translators: placeholder: question
									esc_html_x( 'These points will be rewarded, only if the user answers the %s correctly.', 'placeholder: question', 'ebox' ),
									esc_html( ebox_get_custom_label_lower( 'question' ) )
								)
							?>
						</p>
			...
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Category', 'ebox' ); ?> <?php esc_html_e( '(optional)', 'ebox' ); ?></h3>
				<div class="inside">
					<p class="description">
					...
					<div>
						<select name="category">
							...
							<option value="0" <?php echo $this->question->getCategoryId() == 0 ? 'selected="selected"' : ''; ?>>--- <?php esc_html_e( 'No category', 'ebox' ); ?> ---</option>
							<?php
			...
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Question', 'ebox' ); ?> <?php esc_html_e( '(required)', 'ebox' ); ?></h3>
				<div class="inside">
					<?php
			...
			<div class="postbox" style="<?php echo $this->quiz->isHideAnswerMessageBox() ? '' : 'display: none;'; ?>">
				<h3 class="hndle"><?php esc_html_e( 'Message with the correct / incorrect answer', 'ebox' ); ?></h3>
				<div class="inside">
					<?php
					// translators: placeholder: quiz.
					echo sprintf( esc_html_x( 'Deactivated in %s settings.', 'placeholder: quiz', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'quiz' ) ) );
					?>
				</div>
			</div>
			<div style="<?php echo $this->quiz->isHideAnswerMessageBox() ? 'display: none;' : ''; ?>">
				<div class="postbox" id="wpProQuiz_correctMessageBox">
					<h3 class="hndle"><?php esc_html_e( 'Message with the correct answer', 'ebox' ); ?> <?php esc_html_e( '(optional)', 'ebox' ); ?></h3>
					<div class="inside">
						<p class="description">
				...
				<div class="postbox" id="wpProQuiz_incorrectMassageBox">
					<h3 class="hndle"><?php esc_html_e( 'Message with the incorrect answer', 'ebox' ); ?> <?php esc_html_e( '(optional)', 'ebox' ); ?></h3>
					<div class="inside">
						<p class="description">
			...
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Hint', 'ebox' ); ?> <?php esc_html_e( '(optional)', 'ebox' ); ?></h3>
				<div class="inside">
					<p class="description">
			...
			<div class="postbox">
				<h3 class="hndle"><?php esc_html_e( 'Answer type', 'ebox' ); ?></h3>
				<div class="inside">
				<?php
			...
			<div class="postbox" id="wpProQuiz_answers">
				...
				<div class="inside answer_felder">
					...
					<div class="sort_answer">
						...
						<input type="button" class="button-primary addAnswer" data-default-value="<?php echo esc_attr( ebox_LMS_DEFAULT_ANSWER_POINTS ); ?>" value="<?php esc_html_e( 'Add new answer', 'ebox' ); ?>">
					</div>
			...
			<div style="float: left;">
				<input type="submit" name="submit" id="saveQuestion" class="button-primary" value="<?php esc_html_e( 'Save', 'ebox' ); ?>">
			</div>
			<div style="float: right;">
	...
	<li style="border-bottom:1px dotted #ccc; padding-bottom: 5px; background-color: whiteSmoke;" id="TEST">
		<table style="width: 100%;border: 1px solid #9E9E9E;border-collapse: collapse; margin-bottom: 20px;">
			<thead>
				<tr>
					<th width="160px" style=" border-right: 1px solid #9E9E9E; padding: 5px; "><?php esc_html_e( 'Options', 'ebox' ); ?></th>
					<th style="padding: 5px;"><?php esc_html_e( 'Answer', 'ebox' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<tr>
		...
		<input type="button" name="submit" class="button-primary deleteAnswer" value="<?php esc_html_e( 'Delete answer', 'ebox' ); ?>">
		<input type="button" class="button-secondary addMedia" value="<?php esc_html_e( 'Add Media', 'ebox' ); ?>">
		<a href="#" class="button-secondary wpProQuiz_move" style="cursor: move;"><?php esc_html_e( 'Move', 'ebox' ); ?></a>
		...
		foreach ( $data as $d ) {
			...
			<li style="border-bottom:1px dotted #ccc; padding-bottom: 5px; background-color: whiteSmoke;">
				<table style="width: 100%;border: 1px solid #9E9E9E;border-collapse: collapse; margin-bottom: 20px;">
					<thead>
						<tr>
							<th width="130px" style=" border-right: 1px solid #9E9E9E; padding: 5px; "><?php esc_html_e( 'Options', 'ebox' ); ?></th>
							<th style=" border-right: 1px solid #9E9E9E; padding: 5px; "><?php esc_html_e( 'Criterion', 'ebox' ); ?></th>
							<th style="padding: 5px;"><?php esc_html_e( 'Sort elements', 'ebox' ); ?></th>
						</tr>
					</thead>
					<tbody>
						<tr>
	...
	<div class="answerList">
		<p class="description">
			<?php echo esc_html_x( 'Correct answers (one per line) (answers will be converted to lower case). If mode "Different points for each answer" is activated, you can assign points to each answer using "|". Example: One|15. The default point value is 1.', 'Question type description for Free Answers', 'ebox' ); ?>
		</p>
	...
	public function clozeChoice( $data ) {
		...
		?>
		<p class="description">
			<?php echo wp_kses_post( __( 'Enclose the searched words with <code>{</code> <code>}</code> e.g. <code>I {play} soccer</code>. Capital and small letters will be ignored.', 'ebox' ) ); ?>
		</p>
		<p class="description">
			<?php echo wp_kses_post( __( 'You can specify multiple options for a search word. Enclose the word with <code>[</code> <code>]</code> e.g. <code>I {[play][love][hate]} soccer</code>. In this case answers "play", "love" OR "hate" are correct.', 'ebox' ) ); ?>
		</p>
		<p class="description" style="margin-top: 10px;">
			<?php echo wp_kses_post( __( 'If mode "Different points for each answer" is activated, you can assign points to each answer using <code>|</code>. The default point value is be 1.', 'ebox' ) ); ?>
		</p>
		<p class="description">
			<?php echo wp_kses_post( '<code>I play {[football][soccer|2][golf|5]}</code>. In this example there are three possible answer, "football" is 1 point, "soccer" is 2 points, and "golf" is 5 point.', 'ebox' ); ?>
		</p>
		<p class="description">
			<?php echo wp_kses_post( 'The total possible points will be the total of the highest points values from each answer set.', 'ebox' ); ?>
		</p>
		<?php
			wp_editor(
				$single->getAnswer(),
	...
	public function assessmentChoice( $data ) {
		...
		?>
		<p class="description">
			<?php esc_html_e( 'Here you can create an assessment question.', 'ebox' ); ?>
		</p>
		<p class="description">
			<?php esc_html_e( 'Enclose a assesment with {}. The individual assessments are marked with [].', 'ebox' ); ?>
			<br>
			<?php esc_html_e( 'The number of options in the maximum score.', 'ebox' ); ?>
		</p>
		<p>
			<?php esc_html_e( 'Examples:', 'ebox' ); ?>
			<br>
			* <?php esc_html_e( 'less true { [1] [2] [3] [4] [5] } more true', 'ebox' ); ?>
		</p>

		<p>
			* <?php esc_html_e( 'less true { [a] [b] [c] } more true', 'ebox' ); ?>
		</p>
	...
	public function essayChoice( $data ) {
		...
		if ( is_a( $data, 'WpProQuiz_Model_AnswerTypes' ) ) {
			?>
				<p class="description"><?php esc_html_e( 'How should the user submit their answer?', 'ebox' ); ?></p>
				<select name="answerData[essay][type]" id="essay-type">
					<option value="text" <?php selected( $data->getGradedType(), 'text' ); ?>><?php esc_html_e( 'Text Box', 'ebox' ); ?></option>
					<option value="upload" <?php selected( $data->getGradedType(), 'upload' ); ?>><?php esc_html_e( 'Upload', 'ebox' ); ?></option>
				</select>

				<p class="description" style="margin-top: 10px">
					<?php
					// translators: placeholder: question, course.
					echo sprintf( esc_html_x( 'This is a %1$s that can be graded and potentially prevent a user from progressing to the next step of the %2$s.', 'placeholder: question, course', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'question' ) ), esc_html( ebox_get_custom_label_lower( 'course' ) ) )
					?>
					<br />
					<?php esc_html_e( 'The user can only progress if the essay is marked as "Graded" and if the user has enough points to move on.', 'ebox' ); ?><br />
					<?php
					// translators: placeholder: question, quiz.
					echo sprintf( esc_html_x( 'How should the answer to this %1$s be marked and graded upon %2$s submission?', 'placeholder: question, quiz', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'question' ) ), esc_html( ebox_get_custom_label_lower( 'quiz' ) ) );
					?>
					<br />
				</p>
				<select name="answerData[essay][progression]" id="essay-progression">
					<option value=""><?php esc_html_e( '-- Select --', 'ebox' ); ?></option>
					<option value="not-graded-none" <?php selected( $data->getGradingProgression(), 'not-graded-none' ); ?>><?php esc_html_e( 'Not Graded, No Points Awarded', 'ebox' ); ?></option>
					<option value="not-graded-full" <?php selected( $data->getGradingProgression(), 'not-graded-full' ); ?>><?php esc_html_e( 'Not Graded, Full Points Awarded', 'ebox' ); ?></option>
					<option value="graded-full" <?php selected( $data->getGradingProgression(), 'graded-full' ); ?>><?php esc_html_e( 'Graded, Full Points Awarded', 'ebox' ); ?></option>
				</select>
	...
	<div class="postbox" id="singleChoiceOptions">
		<h3 class="hndle"><?php esc_html_e( 'Single choice options', 'ebox' ); ?></h3>
		<div class="inside">
			<p class="description">
...
.pointDia td {
	...
	<tr>
		<th>
			<?php esc_html_e( '"Different points for each answer" enabled', 'ebox' ); ?>
			<br>
