...
class WpProQuiz_Helper_ImportXml {
	...
	public function setImportFileUpload( $file ) {
		if ( ! is_uploaded_file( $file['tmp_name'] ) ) {
			//$this->setError( __( 'File was not uploaded', 'ebox' ) );
			return false;
		}
	...
	private function checkCode() {
		...
		if ( false === $xml ) {
			$this->_error = esc_html__( 'XML could not be loaded.', 'ebox' );
			return false;
		}
	...
	public function getImportData() {
		...
		if ( isset( $xml->data ) && isset( $xml->data->quiz ) ) {
			foreach ( $xml->data->quiz as $quiz ) {
				$quiz      = ebox_array_sanitize_keys_and_values( $quiz );
				...
				if ( null !== $quizModel ) {
					...
					if ( $quiz->forms->form ) {
						foreach ( $quiz->forms->form as $form ) {
							$form                                = ebox_array_sanitize_keys_and_values( $form );
					...
					if ( isset( $quiz->questions ) ) {
						foreach ( $quiz->questions->question as $question ) {
							$question      = ebox_array_sanitize_keys_and_values( $question );
					...
					if ( ( isset( $_POST['importSave'] ) ) && ( ! empty( $_POST['importSave'] ) ) ) {
						if ( isset( $quiz->post_meta ) ) {
							...
							foreach ( $quiz->post_meta as $post_meta ) {
								...
								if ( ! empty( $meta_key ) ) {
									...
									if ( ( defined( 'ebox_QUIZ_EXPORT_LEGACY' ) ) && ( true === ebox_QUIZ_EXPORT_LEGACY ) ) {
										$a['post_meta'][ $quizModel->getId() ][ $meta_key ] = maybe_unserialize( $meta_value );
									} else {
										if ( ebox_is_valid_JSON( $meta_value ) ) {
											$a['post_meta'][ $quizModel->getId() ][ $meta_key ] = json_decode( $meta_value, true );
										}
									}
								}
							}
							$a['post_meta'][ $quizModel->getId() ] = ebox_array_sanitize_keys_and_values( $a['post_meta'][ $quizModel->getId() ] );
						}
						...
						if ( isset( $quiz->post ) ) {
							...
							$a['post'][ $quizModel->getId() ] = ebox_array_sanitize_keys_and_values( $a['post'][ $quizModel->getId() ] );
						}
					}
				}
			}
	...
	public function saveImport( $ids ) {
		...
		foreach ( $data['master'] as $quiz ) {
			...
			if ( ( isset( $data['post'][ $oldId ] ) ) && ( ! empty( $data['post'][ $oldId ] ) ) ) {
				...
				$post_import_keys = apply_filters( 'ebox_quiz_import_post_keys', $post_import_keys );
			...
			/** This filter is documented in includes/lib/wp-pro-quiz/lib/helper/WpProQuiz_Helper_Import.php */
			$quiz_insert_data = apply_filters( 'ebox_quiz_import_post_data', $quiz_insert_data, 'xml' );
			...
			if ( ! empty( $quiz_post_id ) ) {
				...
				/** This filter is documented in includes/lib/wp-pro-quiz/lib/helper/WpProQuiz_Helper_Import.php */
				$post_meta_import_keys = apply_filters( 'ebox_quiz_import_post_meta_keys', $post_meta_import_keys );
				...
				ebox_update_setting( $quiz_post_id, 'quiz_pro', $quiz->getId() );

				if ( $quiz->isStatisticsOn() ) {
					ebox_update_setting( $quiz_post_id, 'statisticsOn', '1' );
				} else {
					ebox_update_setting( $quiz_post_id, 'statisticsOn', '0' );
				}
			}
			...
			if ( ( isset( $data['question'][ $oldId ] ) ) && ( ! empty( $data['question'][ $oldId ] ) ) ) {
				foreach ( $data['question'][ $oldId ] as $question ) {
					...
					if ( ! empty( $question_post_id ) ) {
						...
						ebox_update_setting( $question_post_id, 'quiz', $quiz_post_id );
