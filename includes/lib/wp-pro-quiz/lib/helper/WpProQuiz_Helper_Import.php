...
class WpProQuiz_Helper_Import {
	...
	private function checkCode() {
		...
		if ( 'WPQ' !== $c ) {
			$this->setError( __( 'File have wrong format', 'ebox' ) );
			return false;
		}

		if ( $v2 < 3 ) {
			$this->setError( __( 'File is not compatible with the current version', 'ebox' ) );
			return false;
		}
	...
	public function getImportData() {
		...
		if ( null === $this->_content ) {
			$this->setError( __( 'File cannot be processed', 'ebox' ) );
			return false;
		}
	...
	private function importData( $o, $ids = false, $version = '1' ) {
		...
		foreach ( $o['master'] as $master ) {
			 ...
			 */
			$quiz_insert_data = apply_filters( 'ebox_quiz_import_post_data', $quiz_insert_data, 'wpq' );
			...
			if ( ! empty( $quiz_post_id ) ) {
				...
				ebox_update_setting( $quiz_post_id, 'quiz_pro', $master->getId() );
	...
	private function saveUnserialize( $str, &$into ) {
		if ( ( defined( 'ebox_QUIZ_EXPORT_LEGACY' ) ) && ( true === ebox_QUIZ_EXPORT_LEGACY ) ) {
			...
			$import = ebox_array_sanitize_keys_and_values( $import );
