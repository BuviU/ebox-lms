...
class WpProQuiz_Helper_ExportXml {
	...
	public function export( $ids ) {
		...
		$header->setAttribute( 'ld_version', ebox_VERSION );
		$header->setAttribute( 'ebox_SETTINGS_DB_VERSION', ebox_SETTINGS_DB_VERSION );
	...
	private function getPostContentElement( $dom, $quizModel ) {
		...
		if ( ! empty( $quiz_post_id ) ) {
			...
			$post_export_keys = apply_filters( 'ebox_quiz_export_post_keys', $post_export_keys, $quiz_post_id );
	...
	private function addPostMetaElement( $dom, $quizModel, $quizElement ) {
		...
		if ( ! empty( $quiz_post_id ) ) {
			...
			/** This filter is documented in includes/lib/wp-pro-quiz/lib/helper/WpProQuiz_Helper_Export.php */
			$post_meta_export_keys = apply_filters( 'ebox_quiz_export_post_meta_keys', $post_meta_export_keys, $quiz_post_id );
	...
	private function getQuizElement( $dom, $quiz, $forms ) {
		...
		if ( is_array( $quiz->getResultText() ) ) {
			...
			if ( ( isset( $resultArray['text'] ) ) && ( ! empty( $resultArray['text'] ) ) ) {
				$count_text = count( $resultArray['text'] );
