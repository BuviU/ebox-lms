...
class WpProQuiz_Helper_Until {
	...
	public static function saveUnserialize($str, &$into) {
		...
		$into = @unserialize($str);
		if ( false === $into ) {
			$str_fixed = ebox_recount_serialized_bytes( $str );
			if ( $str_fixed !== $str ) {
	...
	*/
	...
	public static function getDatePicker( $format, $namePrefix ) {
		...
		$day = ' <select name="' . $namePrefix . '_day"><option value="">' . esc_html__( 'day', 'ebox' ) . '</option>';
		...
		$monthNumber = ' <select name="' . $namePrefix . '_month"><option value="">' . esc_html__( 'month', 'ebox' ) . '</option>';
		...
		$monthName = ' <select name="' . $namePrefix . '_month"><option value="">' . esc_html__( 'month', 'ebox' ) . '</option>';
		...
		$year = ' <select name="' . $namePrefix . '_year"><option value="">' . esc_html__( 'year', 'ebox' ) . '</option>';
		 ...
		 */
		$date_year_min = (int) apply_filters( 'ebox_quiz_custom_field_year_min', 1900 );
		 ...
		 */
		$date_year_max = (int) apply_filters( 'ebox_quiz_custom_field_year_max', date( 'Y' ) + 20 );
