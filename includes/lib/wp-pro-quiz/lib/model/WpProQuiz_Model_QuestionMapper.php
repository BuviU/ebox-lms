...
class WpProQuiz_Model_QuestionMapper extends WpProQuiz_Model_Mapper {
	...
	public function updateSort( $id, $sort ) {
		...
		if ( true === ebox_is_data_upgrade_quiz_questions_updated() ) {
			$question_post_id = ebox_get_question_post_by_pro_id( $id );
			if ( ! empty( $question_post_id ) ) {
				...
				ebox_set_question_quizzes_dirty( $question_post_id );
			}
		}
	}
			...
			if ( empty( $question_post_id ) ) {
				...
				if ( ( $question_pro ) && ( is_a( $question_pro, 'WpProQuiz_Model_Question' ) ) ) {
					...
					if ( false !== $question_insert_post_id ) {
						...
						ebox_update_setting( $question_insert_post_id, 'quiz', $quiz_post_id );
						ebox_proquiz_sync_question_fields( $question_insert_post_id, $question_pro );
						ebox_set_question_quizzes_dirty( $question_insert_post_id );
					}
				}
			}
		}
	...
	public function fetchAll( $quizId = 0, $rand = false, $max = 0, $offset = 0, $only_online = true ) {
		...
		if ( is_a( $quizId, 'WpProQuiz_Model_Quiz' ) ) {
			...
			$quiz_post_id = ebox_get_question_post_by_pro_id( $quizId );
			if ( empty( $quiz_post_id ) ) {
				if ( ( isset( $_GET['post'] ) ) && ( ! empty( $_GET['post'] ) ) ) {
					$quiz_post_id = ebox_get_quiz_id( absint( $_GET['post'] ) );
				}
			}
		}

		if ( ( ! empty( $quiz_post_id ) ) && ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Quizzes_Builder', 'enabled' ) === 'yes' ) && ( true === ebox_is_data_upgrade_quiz_questions_updated() ) ) {
			...
			if ( $ld_quiz_questions_object ) {
				 ...
				 */
				$pro_questions = apply_filters( 'ebox_fetch_quiz_questions', $pro_questions, $quizId, $rand, $max );
		...
		if ( ! $stat_id ) {
			throw new Exception(
				...
				// translators: questions
					esc_html_x( 'Statistics ID cannot be empty in order to fetch %s for statistics.', 'placeholder: questions', 'ebox' ),
