 ...
 */
function ebox_question_cloze_fetch_data( $answer_text, $convert_to_lower = true ) {
	 ...
	 */
	$answer_text = apply_filters( 'ebox_quiz_question_answer_preprocess', $answer_text, 'cloze' );
	...
	foreach ( $matches as $k => $v ) {
		...
		if ( preg_match_all( '#\[(.*?)\]#im', $text, $multiTextMatches ) ) {
			foreach ( $multiTextMatches[1] as $multiText ) {
				 ...
				 */
				if ( apply_filters( 'ebox_quiz_question_cloze_answers_to_lowercase', $convert_to_lower ) ) {
	 ...
	 * @param string $context The context of type of question.
	 */
	$data['replace'] = apply_filters( 'ebox_quiz_question_answer_postprocess', $data['replace'], 'cloze' );

	return $data;
}


function ebox_question_cloze_prepare_output( $cloze_data = array() ) {
	$cloze_output = '';
...
function ebox_question_assessment_fetch_data( $answerText, $quizId = 0, $questionId = 0 ) {

	/** This filter is documented in includes/lib/wp-pro-quiz/wp-pro-quiz.php */
	$answerText = apply_filters( 'ebox_quiz_question_answer_preprocess', $answerText, 'assessment' );
...
function ebox_question_assessment_prepare_output( $assessment_data = array() ) {
	$assessment_output = '';
 ...
 */
function ebox_cast_WpProQuiz_Model_AnswerTypes( $instance, $className ) {
...
function ebox_question_free_get_answer_data( $data, $question = null ) {
	$question_data = array();
