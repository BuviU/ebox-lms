 ...
 * @param string $team The team to assign this job to.
 * @return int The action ID.
 */
function as_enqueue_async_action( $hook, $args = array(), $team = '' ) {
	if ( ! ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return 0;
	}
	return ActionScheduler::factory()->async( $hook, $args, $team );
}
 ...
 * @return int The action ID.
 */
function as_schedule_single_action( $timestamp, $hook, $args = array(), $team = '' ) {
	if ( ! ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return 0;
	}
	return ActionScheduler::factory()->single( $hook, $args, $timestamp, $team );
}
 ...
 * @return int The action ID.
 */
function as_schedule_recurring_action( $timestamp, $interval_in_seconds, $hook, $args = array(), $team = '' ) {
	if ( ! ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return 0;
	}
	return ActionScheduler::factory()->recurring( $hook, $args, $timestamp, $interval_in_seconds, $team );
}
 ...
 * @return int The action ID.
 */
function as_schedule_cron_action( $timestamp, $schedule, $hook, $args = array(), $team = '' ) {
	if ( ! ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return 0;
	}
	return ActionScheduler::factory()->cron( $hook, $args, $timestamp, $schedule, $team );
}
 ...
 * @param string $team The team the job is assigned to.
 */
function as_unschedule_all_actions( $hook, $args = array(), $team = '' ) {
	if ( ! ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return;
	}
	if ( empty( $args ) ) {
		if ( ! empty( $hook ) && empty( $team ) ) {
			ActionScheduler_Store::instance()->cancel_actions_by_hook( $hook );
			return;
		}
		if ( ! empty( $team ) && empty( $hook ) ) {
			ActionScheduler_Store::instance()->cancel_actions_by_team( $team );
			return;
		}
	}
	do {
		$unscheduled_action = as_unschedule_action( $hook, $args, $team );
	} while ( ! empty( $unscheduled_action ) );
}

/**
 * Check if there is an existing action in the queue with a given hook, args and team combination.
 *
 ...
 */
function as_next_scheduled_action( $hook, $args = null, $team = '' ) {
	if ( ! ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return false;
	}
 ...
 */
function as_has_scheduled_action( $hook, $args = null, $team = '' ) {
	if ( ! ActionScheduler::is_initialized( __FUNCTION__ ) ) {
		return false;
	}
 ...
 *        'team' => '' - the team the action belongs to
 *        'status' => '' - ActionScheduler_Store::STATUS_COMPLETE or ActionScheduler_Store::STATUS_PENDING
 *        'claimed' => NULL - TRUE to find claimed actions, FALSE to find unclaimed actions, a string to find a specific claim ID
 *        'per_page' => 5 - Number of results to return
 *        'offset' => 0
 *        'orderby' => 'date' - accepted values are 'hook', 'team', 'modified', 'date' or 'none'
 *        'order' => 'ASC'
 *
