...
class ActionScheduler_DBStore extends ActionScheduler_Store {
	...
	public function save_action( ActionScheduler_Action $action, \DateTime $date = null ) {
		try {
			...
			$data = array(
				...
				'team_id'             => $this->get_team_id( $action->get_team() ),
			);
	...
	/**
	 * Get a team's ID based on its name/slug.
	 *
	 * @param string $slug The string name of a team.
	 * @param bool   $create_if_not_exists Whether to create the team if it does not already exist. Default, true - create the team.
	 *
	 * @return int The team's ID, if it exists or is created, or 0 if it does not exist and is not created.
	 */
	protected function get_team_id( $slug, $create_if_not_exists = true ) {
		if ( empty( $slug ) ) {
			return 0;
		}
		/** @var \wpdb $wpdb */
		global $wpdb;
		$team_id = (int) $wpdb->get_var( $wpdb->prepare( "SELECT team_id FROM {$wpdb->actionscheduler_teams} WHERE slug=%s", $slug ) );
		if ( empty( $team_id ) && $create_if_not_exists ) {
			$team_id = $this->create_team( $slug );
		}
	 ...
	 * @return int Team ID.
	 */
	protected function create_team( $slug ) {
		/** @var \wpdb $wpdb */
		global $wpdb;
		$wpdb->insert( $wpdb->actionscheduler_teams, array( 'slug' => $slug ) );
	...
	public function fetch_action( $action_id ) {
		...
		$data = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT a.*, g.slug AS `team` FROM {$wpdb->actionscheduler_actions} a LEFT JOIN {$wpdb->actionscheduler_teams} g ON a.team_id=g.team_id WHERE a.action_id=%d",
				$action_id
			)
		);
	...
	protected function make_action_from_db_record( $data ) {
		...
		return ActionScheduler::factory()->get_stored_action( $data->status, $data->hook, $args, $schedule, $team );
	}
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! empty( $query['team'] ) || 'team' === $query['orderby'] ) {
			$sql .= " LEFT JOIN {$wpdb->actionscheduler_teams} g ON g.team_id=a.team_id";
		}
	...
	/**
	 * Get a count of all actions in the store, teamed by status.
	 *
	...
	/**
	 * Cancel pending actions by team.
	 *
	 * @param string $team Team slug.
	 *
	 * @return void
	 */
	public function cancel_actions_by_team( $team ) {
		$this->bulk_cancel_actions( array( 'team' => $team ) );
	}
	 ...
	 * @param string    $team Team to filter for.
	 *
	 * @return ActionScheduler_ActionClaim
	 */
	public function stake_claim( $max_actions = 10, \DateTime $before_date = null, $hooks = array(), $team = '' ) {
		$claim_id = $this->generate_claim_id();

		$this->claim_before_date = $before_date;
		$this->claim_actions( $claim_id, $max_actions, $before_date, $hooks, $team );
	 ...
	 * @throws \InvalidArgumentException Throws InvalidArgumentException if team doesn't exist.
	 * @throws \RuntimeException Throws RuntimeException if unable to claim action.
	 */
	protected function claim_actions( $claim_id, $limit, \DateTime $before_date = null, $hooks = array(), $team = '' ) {
		/** @var \wpdb $wpdb */
		global $wpdb;
		...
		if ( ! empty( $team ) ) {
			...
			$team_id = $this->get_team_id( $team, false );

			// throw exception if no matching team found, this matches ActionScheduler_wpPostStore's behaviour.
			if ( empty( $team_id ) ) {
				/* translators: %s: team name */
				throw new InvalidArgumentException( sprintf( __( 'The team "%s" does not exist.', 'action-scheduler' ), $team ) );
			}
