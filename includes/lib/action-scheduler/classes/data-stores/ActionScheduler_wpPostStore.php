...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	const GROUP_TAXONOMY    = 'action-team';
	...
	public function save_action( ActionScheduler_Action $action, DateTime $scheduled_date = null ) {
		try {
			...
			$this->save_action_team( $post_id, $action->get_team() );
	 ...
	 *
	 * @param int    $post_id Post ID.
	 * @param string $team   Team to save.
	 * @return void
	 */
	protected function save_action_team( $post_id, $team ) {
		if ( empty( $team ) ) {
			wp_set_object_terms( $post_id, array(), self::GROUP_TAXONOMY, false );
		} else {
			wp_set_object_terms( $post_id, array( $team ), self::GROUP_TAXONOMY, false );
		}
	}
	...
	protected function make_action_from_post( $post ) {
		...
		$team = wp_get_object_terms( $post->ID, self::GROUP_TAXONOMY, array( 'fields' => 'names' ) );
		$team = empty( $team ) ? '' : reset( $team );

		return ActionScheduler::factory()->get_stored_action( $this->get_action_status_by_post_status( $post->post_status ), $hook, $args, $schedule, $team );
	}
	 ...
	 * @param string   $team       Claim only actions in the given team.
	 *
	 * @return ActionScheduler_ActionClaim
	 * @throws RuntimeException When there is an error staking a claim.
	 * @throws InvalidArgumentException When the given team is not valid.
	 */
	...
	protected function claim_actions( $claim_id, $limit, DateTime $before_date = null, $hooks = array(), $team = '' ) {
		...
		$limit_ids = ! empty( $team );
		$ids       = $limit_ids ? $this->get_actions_by_team( $team, $limit, $date ) : array();
	...
	/**
	 * Get IDs of actions within a certain team and up to a certain date/time.
	 *
	 * @param string   $team The team to use in finding actions.
	 ...
	 *
	 * @return array IDs of actions in the appropriate team and before the appropriate time.
	 * @throws InvalidArgumentException When the team does not exist.
	 */
	protected function get_actions_by_team( $team, $limit, DateTime $date ) {
		// Ensure the team exists before continuing.
		if ( ! term_exists( $team, self::GROUP_TAXONOMY ) ) {
