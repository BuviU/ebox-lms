...
class ActionScheduler_WPCLI_Scheduler_command extends WP_CLI_Command {
	 ...
	 *
	 * [--team=<team>]
	 * : Only run actions from the specified team. Omitting this option runs actions from all teams.
	 *
	...
	public function run( $args, $assoc_args ) {
		...
		$team   = \WP_CLI\Utils\get_flag_value( $assoc_args, 'team', '' );
		...
		try {
			...
			$total = $runner->setup( $batch, $hooks, $team, $force );
			...
			while ( $total > 0 ) {
				...
				$total = ( $unlimited || $batches_completed < $batches ) ? $runner->setup( $batch, $hooks, $team, $force ) : 0;
			}
		} catch ( Exception $e ) {
			$this->print_error( $e );
		}
