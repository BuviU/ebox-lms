...
class ActionScheduler_ActionFactory {
	 ...
	 * @param string $team A team to put the action in
	 *
	 * @return ActionScheduler_Action An instance of the stored action
	 */
	public function get_stored_action( $status, $hook, array $args = array(), ActionScheduler_Schedule $schedule = null, $team = '' ) {
		...
		$action_class = apply_filters( 'action_scheduler_stored_action_class', $action_class, $status, $hook, $args, $schedule, $team );

		$action = new $action_class( $hook, $args, $schedule, $team );

		/**
		 * Allow 3rd party code to change the instantiated action for a given hook, args, schedule and team.
		 *
		 ...
		 * @param string $team The instantiated action's team.
		 */
		return apply_filters( 'action_scheduler_stored_action_instance', $action, $hook, $args, $schedule, $team );
	}
	 ...
	 */
	public function async( $hook, $args = array(), $team = '' ) {
		$schedule = new ActionScheduler_NullSchedule();
		$action = new ActionScheduler_Action( $hook, $args, $schedule, $team );
	 ...
	 * @return int The ID of the stored action
	 */
	public function single( $hook, $args = array(), $when = null, $team = '' ) {
	 ...
	 * @return int The ID of the stored action
	 */
	public function recurring( $hook, $args = array(), $first = null, $interval = null, $team = '' ) {
		if ( empty($interval) ) {
			return $this->single( $hook, $args, $first, $team );
		}
	 ...
	 * @return int The ID of the stored action
	 */
	public function cron( $hook, $args = array(), $base_timestamp = null, $schedule = null, $team = '' ) {
		if ( empty($schedule) ) {
			return $this->single( $hook, $args, $base_timestamp, $team );
		}
	...
	public function repeat( $action ) {
		...
		$new_action = new ActionScheduler_Action( $action->get_hook(), $action->get_args(), $new_schedule, $action->get_team() );
