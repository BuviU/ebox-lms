...
//  * automatic page numbering and page teams;
 ...
 * <li>automatic page numbering and page teams;</li>
...
class TCPDF {
	...
	/**
	 * Array of page numbers were a new page team was started (the page numbers are the keys of the array).
	 * @protected
	 * @since 3.0.000 (2008-03-27)
	 */
	protected $newpageteam = array();

	/**
	 * Array that contains the number of pages in each page team.
	 * @protected
	 * @since 3.0.000 (2008-03-27)
	 */
	protected $pageteams = array();

	/**
	 * Current page team number.
	 * @protected
	 * @since 3.0.000 (2008-03-27)
	 */
	protected $currpageteam = 0;
	 ...
	 * @since 4.8.001 (2009-09-09)
	 */
	protected $radiobutton_teams = array();

	/**
	 * List of radio team objects IDs.
	 * @protected
	 * @since 4.8.001 (2009-09-09)
	 */
	protected $radio_teams = array();
	...
	/**
	 * Boolean value true when in SVG defs team.
	 * @protected
	 * @since 5.0.000 (2010-05-02)
	 */
	...
	public function Footer() {
		...
		if (empty($this->pageteams)) {
			$pagenumtxt = $w_page.$this->getAliasNumPage().' / '.$this->getAliasNbPages();
		} else {
			$pagenumtxt = $w_page.$this->getPageNumTeamAlias().' / '.$this->getPageTeamAlias();
		}
		$this->setY($cur_y);
		//Print page number
		if ($this->getRTL()) {
	...
	protected function getAllInternalPageNumberAliases() {
		$basic_alias = array(TCPDF_STATIC::$alias_tot_pages, TCPDF_STATIC::$alias_num_page, TCPDF_STATIC::$alias_team_tot_pages, TCPDF_STATIC::$alias_team_num_page, TCPDF_STATIC::$alias_right_shift);
		$pnalias = array();
	...
	protected function _putpages() {
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$gdiff = 0; // difference used for right shift alignment of page team numbers
			if (!empty($this->pageteams)) {
				if (isset($this->newpageteam[$n])) {
					$pageteamnum = 0;
					++$teamnum;
					$ptga = TCPDF_STATIC::formatPageNumber($this->pageteams[$teamnum]);
					$ptgu = TCPDF_FONTS::UTF8ToUTF16BE($ptga, false, $this->isunicode, $this->CurrentFont);
					$ptg_num_chars = $this->GetNumChars($ptga);
				}
				++$pageteamnum;
				$pnga = TCPDF_STATIC::formatPageNumber($pageteamnum);
			...
			if (!$this->pdfa_mode || $this->pdfa_version >= 2) {
				$out .= ' /Team << /Type /Team /S /Transparency /CS /DeviceRGB >>';
			}
	...
	protected function _getannotsrefs($n) {
		...
		if (isset($this->PageAnnots[$n])) {
			foreach ($this->PageAnnots[$n] as $key => $val) {
				if (!in_array($val['n'], $this->radio_teams)) {
					$out .= ' '.$val['n'].' 0 R';
				}
			}
			// add radiobutton teams
			if (isset($this->radiobutton_teams[$n])) {
				foreach ($this->radiobutton_teams[$n] as $key => $data) {
					if (isset($data['n'])) {
						$out .= ' '.$data['n'].' 0 R';
					}
				}
		...
		for ($n=1; $n <= $this->numpages; ++$n) {
			if (isset($this->PageAnnots[$n])) {
				...
				foreach ($this->PageAnnots[$n] as $key => $pl) {
					...
					// create annotation object for teaming radiobuttons
					if (isset($this->radiobutton_teams[$n][$pl['txt']]) AND is_array($this->radiobutton_teams[$n][$pl['txt']])) {
						$radio_button_obj_id = $this->radiobutton_teams[$n][$pl['txt']]['n'];
						$annots = '<<';
						$annots .= ' /Type /Annot';
						$annots .= ' /Subtype /Widget';
						$annots .= ' /Rect [0 0 0 0]';
						if ($this->radiobutton_teams[$n][$pl['txt']]['#readonly#']) {
							// read only
							$annots .= ' /F 68';
							$annots .= ' /Ff 49153';
						} else {
						...
						$annots .= ' /Kids [';
						$defval = '';
						foreach ($this->radiobutton_teams[$n][$pl['txt']] as $key => $data) {
							if (isset($data['kid'])) {
								$annots .= ' '.$data['kid'].' 0 R';
								if ($data['def'] !== 'Off') {
									$defval = $data['def'];
						...
						$this->radiobutton_teams[$n][$pl['txt']] = $radio_button_obj_id;
					}
					$formfield = false;
					...
					switch (strtolower($pl['opt']['subtype'])) {
						...
						case 'widget': {
							...
							if (isset($this->radiobutton_teams[$n][$pl['txt']])) {
								// set parent
								$annots .= ' /Parent '.$this->radiobutton_teams[$n][$pl['txt']].' 0 R';
							}
							if (isset($pl['opt']['t']) AND is_string($pl['opt']['t'])) {
					...
					if ($formfield AND !isset($this->radiobutton_teams[$n][$pl['txt']])) {
	...
	protected function _putxobjects() {
		foreach ($this->xobjects as $key => $data) {
			if (isset($data['outdata'])) {
				...
				$out .= ' >>'; //end resources
				if (isset($data['team']) AND ($data['team'] !== false)) {
					// set transparency team
					$out .= ' /Team << /Type /Team /S /Transparency';
					if (is_array($data['team'])) {
						if (isset($data['team']['CS']) AND !empty($data['team']['CS'])) {
							$out .= ' /CS /'.$data['team']['CS'];
						}
						if (isset($data['team']['I'])) {
							$out .= ' /I /'.($data['team']['I']===true?'true':'false');
						}
						if (isset($data['team']['K'])) {
							$out .= ' /K /'.($data['team']['K']===true?'true':'false');
						}
					}
					$out .= ' >>';
				}
	...
	protected function _beginpage($orientation='', $format='') {
		...
		$this->y = $this->tMargin;
		if (isset($this->newpageteam[$this->page])) {
			// start a new team
			$this->currpageteam = $this->newpageteam[$this->page];
			$this->pageteams[$this->currpageteam] = 1;
		} elseif (isset($this->currpageteam) AND ($this->currpageteam > 0)) {
			++$this->pageteams[$this->currpageteam];
		}
	}
	...
	public function RadioButton($name, $w, $prop=array(), $opt=array(), $onvalue='On', $checked=false, $x=null, $y=null, $js=false) {
		...
		// set data for parent team
		if (!isset($this->radiobutton_teams[$this->page])) {
			$this->radiobutton_teams[$this->page] = array();
		}
		if (!isset($this->radiobutton_teams[$this->page][$name])) {
			$this->radiobutton_teams[$this->page][$name] = array();
			++$this->n;
			$this->radiobutton_teams[$this->page][$name]['n'] = $this->n;
			$this->radio_teams[] = $this->n;
		}
		$kid = ($this->n + 1);
		// save object ID to be added on Kids entry on parent object
		$this->radiobutton_teams[$this->page][$name][] = array('kid' => $kid, 'def' => $defval);
		// get default style
		$prop = array_merge($this->getFormDefaultProp(), $prop);
		...
		// store readonly flag
		if (!isset($this->radiobutton_teams[$this->page][$name]['#readonly#'])) {
			$this->radiobutton_teams[$this->page][$name]['#readonly#'] = false;
		}
		$this->radiobutton_teams[$this->page][$name]['#readonly#'] |= ($opt['f'] & 64);
	...
	/**
	 * Create a new page team.
	 * NOTE: call this function before calling AddPage()
	 * @param int|null $page starting team page (leave empty for next page).
	 * @public
	 * @since 3.0.000 (2008-03-27)
	 */
	public function startPageTeam($page=null) {
		if (empty($page)) {
			$page = $this->page + 1;
		}
		$this->newpageteam[$page] = sizeof($this->newpageteam) + 1;
	}
	...
	/**
	 * Return the alias for the total number of pages in the current page team.
	 * If the current font is unicode type, the returned string is surrounded by additional curly braces.
	 * This alias will be replaced by the total number of pages in this team.
	 * @return string alias of the current page team
	 * @public
	 * @since 3.0.000 (2008-03-27)
	 */
	public function getPageTeamAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.TCPDF_STATIC::$alias_team_tot_pages.'}';
		}
		return TCPDF_STATIC::$alias_team_tot_pages;
	}

	/**
	 * Return the alias for the page number on the current page team.
	 * If the current font is unicode type, the returned string is surrounded by additional curly braces.
	 * This alias will be replaced by the page number (relative to the belonging team).
	 * @return string alias of the current page team
	 * @public
	 * @since 4.5.000 (2009-01-02)
	 */
	public function getPageNumTeamAlias() {
		if ($this->isUnicodeFont()) {
			return '{'.TCPDF_STATIC::$alias_team_num_page.'}';
		}
		return TCPDF_STATIC::$alias_team_num_page;
	}

	/**
	 * Return the current page in the team.
	 * @return int current page in the team
	 * @public
	 * @since 3.0.000 (2008-03-27)
	 */
	public function getTeamPageNo() {
		return $this->pageteams[$this->currpageteam];
	}

	/**
	 * Returns the current team page number formatted as a string.
	 * @public
	 * @since 4.3.003 (2008-11-18)
	 * @see PaneNo(), formatPageNumber()
	 */
	public function getTeamPageNoFormatted() {
		return TCPDF_STATIC::formatPageNumber($this->getTeamPageNo());
	}
	...
	function _putshaders() {
		...
		foreach ($this->gradients as $id => $grad) {
			...
			if ($grad['transparency']) {
				...
				$out .= ' /BBox [0 0 '.$rect.']';
				$out .= ' /Team << /Type /Team /S /Transparency /CS /DeviceGray >>';
				$out .= ' /Resources <<';
	...
	protected function getHtmlDomArray($html) {
		...
		if ( ( ! defined( 'ebox_TCPDF_LEGACY_LD322' ) ) || ( true !== ebox_TCPDF_LEGACY_LD322 ) ) {
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
		...
		switch($tag['value']) {
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					 ...
					 * The problem this creates is related to query string params which might be themselves a URL that need to be properly encoded/decoded. See ebox-5234 for more details.
					 */
					if ( ( defined( 'ebox_TCPDF_LEGACY_LD322' ) ) && ( true === ebox_TCPDF_LEGACY_LD322 ) ) {
	...
	public function movePage($frompage, $topage) {
		...
		if (isset($this->newpageteam) AND !empty($this->newpageteam)) {
			for ($i = $frompage; $i > $topage; --$i) {
				if (isset($this->newpageteam[$i]) AND (($i + $this->pageteams[$this->newpageteam[$i]]) > $frompage)) {
					--$this->pageteams[$this->newpageteam[$i]];
					break;
				}
			}
			for ($i = $topage; $i > 0; --$i) {
				if (isset($this->newpageteam[$i]) AND (($i + $this->pageteams[$this->newpageteam[$i]]) > $topage)) {
					++$this->pageteams[$this->newpageteam[$i]];
					break;
				}
			}
		...
		for ($i = $frompage; $i > $topage; --$i) {
			...
			if (isset($this->newpageteam[$j])) {
				$this->newpageteam[$i] = $this->newpageteam[$j];
				unset($this->newpageteam[$j]);
			}
			if ($this->currpageteam == $j) {
				$this->currpageteam = $i;
			}
		}
	...
	public function deletePage($page) {
		...
		if (isset($this->newpageteam) AND !empty($this->newpageteam)) {
			for ($i = $page; $i > 0; --$i) {
				if (isset($this->newpageteam[$i]) AND (($i + $this->pageteams[$this->newpageteam[$i]]) > $page)) {
					--$this->pageteams[$this->newpageteam[$i]];
					break;
				}
			}
		...
		if ($page < $this->numpages) {
			...
			if (isset($this->newpageteam[$this->numpages])) {
				unset($this->newpageteam[$this->numpages]);
			}
			if ($this->currpageteam == $this->numpages) {
				$this->currpageteam = ($this->numpages - 1);
			}
			if (isset($this->pageteams[$this->numpages])) {
				unset($this->pageteams[$this->numpages]);
			}
	...
	public function copyPage($page=0) {
		...
		if (isset($this->newpageteam[$page])) {
			// start a new team
			$this->newpageteam[$this->page] = sizeof($this->newpageteam) + 1;
			$this->currpageteam = $this->newpageteam[$this->page];
			$this->pageteams[$this->currpageteam] = 1;
		} elseif (isset($this->currpageteam) AND ($this->currpageteam > 0)) {
			++$this->pageteams[$this->currpageteam];
	...
	*/
	 ...
	 * @param mixed $team Set transparency team. Can be a boolean value or an array specifying optional parameters: 'CS' (solour space name), 'I' (boolean flag to indicate isolated team) and 'K' (boolean flag to indicate knockout team).
	 ...
	 */
	public function startTemplate($w=0, $h=0, $team=false) {
		if ($this->inxobj) {
		...
		$this->bMargin = 0;
		// set team mode
		$this->xobjects[$this->xobjid]['team'] = $team;
		return $this->xobjid;
	}
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		switch($name) {
			...
			case 'g': {
				// team together related graphics elements
				array_push($this->svgstyles, $svgstyle);
				$this->StartTransform();
	...
	protected function endSVGElementHandler($parser, $name) {
		...
		switch($name) {
			...
			case 'g': {
				// unteam: remove last style from array
