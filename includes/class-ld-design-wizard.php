<?php
/**
 * ebox class for displaying the design wizard.
 *
 * @package    ebox
 * @since 4.4.0
 */
...
if ( ! class_exists( 'ebox_Design_Wizard' ) ) {
	/**
	 * Design wizard class.
	 */
	class ebox_Design_Wizard {
		/**
		 * The opened status, can be completed, ongoing or closed.
		 */
		const STATUS_KEY = 'ebox_design_wizard_status';

		const STATUS_COMPLETED = 'completed';
		const STATUS_ONGOING   = 'ongoing';
		const STATUS_CLOSED    = 'closed';

		const DATA_KEY = 'ebox_design_wizard';

		const HANDLE = 'ebox-design-wizard';
		...
		public function register_templates() : array {
			$this->templates = array(
				'kadence_seo_skills'              => array(
					...
					'plugins'           => array(
						'kadence-starter-templates' => __( 'Starter Templates by Kadence WP', 'ebox' ),
					),
				...
				'astra_meditation_courses'        => array(
					...
					'color_scheme'      => 'light',
					'plugins'           => array(
						'astra-sites' => __( 'Starter Templates by Astra', 'ebox' ),
					),
					'preview_url'       => 'https://websitedemos.net/learn-meditation-08/',
				),
				'astra_ebox_academy'         => array(
					'id'                => 'astra_ebox_academy',
					'label'             => 'ebox Academy',
					'theme'             => 'astra',
					'theme_label'       => 'Astra',
					...
					'preview_url'       => 'https://websitedemos.net/ebox-academy-08/',
				),
			 ...
			 */
			return apply_filters( 'ebox_design_wizard_templates', $this->templates );
		}
		...
		public function load_fonts() : void {
			if (
				...
				isset( $_GET['page'] ) && 'ebox-design-wizard' === $_GET['page']
				// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		...
		public function enqueue_admin_scripts() : void {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( isset( $_GET['page'] ) && 'ebox-design-wizard' === $_GET['page'] ) {
				...
				wp_enqueue_style(
					'ebox-design-wizard',
					ebox_LMS_PLUGIN_URL . '/assets/css/design-wizard.css',
					array(),
					ebox_VERSION
				);
				...
				wp_enqueue_script(
					'ebox-design-wizard',
					ebox_LMS_PLUGIN_URL . '/assets/js/design-wizard.js',
				...
				ob_start();
				include_once ebox_LMS_PLUGIN_DIR . '/includes/views/design-wizard/actions-success.php';
				$actions_success = ob_get_clean();

				ob_start();
				include_once ebox_LMS_PLUGIN_DIR . '/includes/views/design-wizard/actions-error.php';
				...
				wp_localize_script(
					...
					array(
						...
						'messages'                    => array(
							'dw_error_prefix'  => '<strong>' . __( 'Error', 'ebox' ) . '</strong>',
							// phpcs:ignore Generic.Files.LineLength.TooLong
							'dw_error_default' => __( 'There\'s unknown error with the design wizard. Please try again later or contact our support if the issue persists.', 'ebox' ),
						),
						'templates'                   => $templates,
					)
				);
			...
			if (
				...
				wp_enqueue_style(
					'ebox-design-wizard-gfonts', // cspell:disable-line.
					// phpcs:ignore Generic.Files.LineLength.TooLong
		...
		public function render_step( int $step ) : void {
			...
			ebox_LMS::get_view( $template_path, $args, true );
		}
		...
		public function get_theme_palettes( string $theme = '' ) : array {
			...
			$astra_palettes = array(
				'dark'  => array(
					'default'  => array(
						'slug'   => 'default',
						'title'  => __( 'Default', 'ebox' ),
						'colors' => array(),
					),
					'style-1'  => array(
						'slug'   => 'style-1',
						'title'  => __( 'Style 1', 'ebox' ),
						'colors' => array(
							'#8E43F0',
							'#7215EA',
							'#FFFFFF',
		...
		public function get_template_preview_image_url( string $template ) : string {
			...
			$image_dir_url  = ebox_LMS_PLUGIN_URL . '/assets/images/design-wizard/previews/';
			$image_dir_path = ebox_LMS_PLUGIN_DIR . '/assets/images/design-wizard/previews/';
			 ...
			 */
			return apply_filters( 'ebox_design_wizard_template_preview_image_url', $image_url, $template );
		}
		...
		public function install_theme( string $theme ) : array {
			if ( empty( $theme ) ) {
				wp_send_json_error(
					array(
						...
						'errorMessage' => __( 'No theme specified.', 'ebox' ),
					)
				);
			}
			...
			if ( ! current_user_can( 'install_themes' ) ) {
				$status['errorMessage'] =
					__( 'Sorry, you are not allowed to install themes on this site.', 'ebox' );
			...
			if ( is_wp_error( $result ) ) {
				...
				$status['errorMessage'] =
					__( 'Unable to connect to the filesystem. Please confirm your credentials.', 'ebox' );
		...
		public function install_plugin( string $plugin ) : array {
			if ( empty( $plugin ) ) {
				wp_send_json_error(
					array(
						...
						'errorMessage' => __( 'No plugin specified.', 'ebox' ),
					)
				);
			}
			...
			if ( ! current_user_can( 'install_plugins' ) ) {
				$status['errorMessage'] =
					__( 'Sorry, you are not allowed to install plugins on this site.', 'ebox' );
				wp_send_json_error( $status );
			}
		...
		private function install_plugin_fallback( $plugin_key = '', &$status = array() ) : void {
			...
			$file     = ebox_LMS_PLUGIN_DIR . 'plugins/' . $filename;
			...
			if ( file_exists( $file ) ) {
				...
				if ( $zip->open( $file ) === true ) {
					...
					$status['errorMessage'] = __( 'We can\'t extract plugin file. Please check your file and directory permission.', 'ebox' );
				}
			}
		}
		...
		public function ajax_ld_dw_build_template() : void {
			...
			if ( ! current_user_can( 'switch_themes' ) ) {
				wp_send_json_error(
					array(
						'message' => __( 'User doesn\'t have enough capability', 'ebox' ),
			...
			if ( isset( $response['completed'] ) && $response['completed'] ) {
				$message = __( 'The template has been built and is ready to use.', 'ebox' );

				update_option( 'ebox_design_wizard_status', 'completed' );
		...
		public function ajax_install_theme() {
			...
			if ( isset( $install['success'] ) ) {
				$response['next_step_message'] = __( 'Install plugin(s)', 'ebox' );
			}
		...
		public function ajax_install_plugin() {
			...
			if ( isset( $install['success'] ) ) {
				$response['next_step_message'] = __( 'Activate theme', 'ebox' );
			}
		...
		public function ajax_activate_theme() {
			...
			return array(
				'next_step_message' => __( 'Activate plugin(s)', 'ebox' ),
			);
		}
		...
		public function ajax_activate_plugin() {
			...
			return array(
				'next_step_message' => __( 'Build template', 'ebox' ),
			);
		}
		...
		public function ajax_build_template() {
			...
			if ( isset( $this->ajax_template['theme'] ) && $this->ajax_template['theme'] === 'kadence' ) {
				$message = __( 'Run Kadence template building process', 'ebox' );
			} elseif ( isset( $this->ajax_template['theme'] ) && $this->ajax_template['theme'] === 'astra' ) {
				$message = __( 'Run Astra template building process', 'ebox' );
			}
