 ...
 * @since 2.1.0
 */
function ebox_assignment_process_init() {
	...
	if ( ( isset( $_POST['uploadfile'] ) ) && ( ! empty( $_POST['uploadfile'] ) ) && ( isset( $_POST['post'] ) ) && ( ! empty( $_POST['post'] ) ) && ( isset( $_POST['course_id'] ) && ( ! empty( $_POST['course_id'] ) ) ) ) {
		...
		if ( 'on' !== ebox_get_setting( $post_id, 'lesson_assignment_upload' ) ) {
			return;
		}

		// 3. Verify the lesson/topic is from the correct course.
		$courses = ebox_get_courses_for_step( $post_id, true );
		...
		if ( ! is_user_logged_in() ) {
			 ...
			 */
			if ( ! apply_filters( 'ebox_assignment_upload_user_check', false, $course_id, $post_id ) ) {
				return;
			}
		}
		...
		if ( ( ! empty( $file['name'][0] ) ) && ( ebox_check_upload( $file, $post_id ) ) ) {
			$file_desc = ebox_fileupload_process( $file, $post_id );
	...
	if ( ! empty( $_GET['ebox_delete_attachment'] ) ) {
		$assignment_post = get_post( intval( $_GET['ebox_delete_attachment'] ) );
		if ( ( isset( $assignment_post ) ) && ( $assignment_post instanceof WP_Post ) && ( ebox_get_post_type_slug( 'assignment' ) === $assignment_post->post_type ) ) {
			$current_user_id = get_current_user_id();

			if ( ( $assignment_post->post_author == $current_user_id ) || ( ebox_is_admin_user( $current_user_id ) ) || ( ebox_is_team_leader_of_user( $current_user_id, $assignment_post->post_author ) ) ) {

				$course_id = get_post_meta( $assignment_post->ID, 'course_id', true );
				if ( empty( $course_id ) ) {
					$course_id = ebox_get_course_id( $assignment_post->ID );
				}
				$course_step_id = get_post_meta( $assignment_post->ID, 'lesson_id', true );

				ebox_process_mark_incomplete( $current_user_id, $course_id, $course_step_id );
				 ...
				 */
				wp_delete_post( $assignment_post->ID, apply_filters( 'ebox_assignment_force_delete', true, $assignment_post->ID, $assignment_post ) );
				...
				update_user_meta(
					...
					array(
						array(
							'type'    => 'success',
							'message' => esc_html__( 'Assignment successfully deleted.', 'ebox' ),
						),
					)
				);

				$return_url = remove_query_arg( 'ebox_delete_attachment' );
				ebox_safe_redirect( $return_url );
			}
		}
	}
	...
	if ( ! empty( $_POST['attachment_mark_complete'] ) && ! empty( $_POST['userid'] ) ) {
		...
		if ( ( ebox_is_admin_user( $current_user_id ) ) || ( ebox_is_team_leader_of_user( $current_user_id, $user_id ) ) ) {
			ebox_approve_assignment( $user_id, $lesson_id );
		}
	}
}

add_action( 'parse_request', 'ebox_assignment_process_init', 1 );
 ...
 */
function ebox_get_user_assignments( $post_id, $user_id, $course_id = 0, string $fields = 'all' ) {
	if ( empty( $course_id ) ) {
 ...
 *
 * How is this different from ebox_assignment_process_init() ?
 *
 * @since 2.1.0
 *
 * @param int $post_id Post ID.
 ...
 */
function ebox_upload_assignment_init( $post_id, $fname ) {
	...
	update_post_meta( $post_id, 'ebox_modules-assignment', $new_assignment_meta );
	...
	$points_enabled = ebox_get_setting( $post, 'lesson_assignment_points_enabled' );
	...
	$auto_approve       = ebox_get_setting( $post, 'auto_approve_assignment' );
	...
	if ( $assignment_post_id ) {
		 ...
		 */
		do_action( 'ebox_assignment_uploaded', $assignment_post_id, $assignment_meta );
		...
		if ( empty( $auto_approve ) ) {
			...
			update_user_meta(
				...
				array(
					array(
						'type'    => 'success',
						'message' => esc_html__( 'Assignment successfully uploaded.', 'ebox' ),
					),
				)
			);

			ebox_safe_redirect( get_permalink( $post->ID ), 303 );
		}
	}

	if ( ! empty( $auto_approve ) ) {
		ebox_approve_assignment( $current_user->ID, $post_id, $assignment_post_id );

		// assign full points if auto approve & points are enabled.
		if ( 'on' === $points_enabled ) {
			$points = ebox_get_setting( $post, 'lesson_assignment_points_amount' );
			update_post_meta( $assignment_post_id, 'points', intval( $points ) );
		}

		ebox_get_next_lesson_redirect( $post );
	}
}
 ...
 */
function ebox_assignments_comments_open( $open, $post_id ) {
	if ( ebox_get_post_type_slug( 'assignment' ) === get_post_type( $post_id ) ) {
		$comment_status = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Assignments_CPT', 'comment_status' );
...
add_filter( 'comments_open', 'ebox_assignments_comments_open', 10, 2 );
 ...
 */
function ebox_assignments_comments_on( $data ) {
	if ( ebox_get_post_type_slug( 'assignment' ) === $data['post_type'] ) {
		$data['comment_status'] = 'open';
	}

	return $data;
}
add_filter( 'wp_insert_post_data', 'ebox_assignments_comments_on' );
 ...
 */
function ebox_clean_filename( $string ) {
 ...
 */
function ebox_fileupload_process( $uploadfiles, $post_id ) {
	...
	if ( is_array( $uploadfiles ) ) {
		...
		foreach ( $uploadfiles['name'] as $key => $value ) {
			...
			if ( 0 == $uploadfiles['error'][ $key ] ) {
				...
				// clean filename.
				$filename = ebox_clean_filename( $uploadfiles['name'][ $key ] );
				...
				// Before this function we have already validated the file extension/type via the function ebox_check_upload
				// @2.5.4.
				...
				if ( ! file_exists( $upload_dir_path ) ) {
					if ( is_writable( dirname( $upload_dir_path ) ) ) {
						...
						die( esc_html__( 'Unable to write to UPLOADS directory. Is this directory writable by the server?', 'ebox' ) );
					}
				}
				...
				if ( ! file_exists( $_index ) ) {
					ebox_put_directory_index_file( $_index );
				}
				...
				$filename = apply_filters(
					'ebox_assignment_upload_filename',
					$filename,
					$post_id,
					$file_time,
					$file_title,
				...
				if ( ! is_writeable( $upload_dir_path ) ) {
					die( esc_html__( 'Unable to write to directory. Is this directory writable by the server?', 'ebox' ) );
				}
				...
				if ( ! @move_uploaded_file( $file_tmp, $file_dest ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged -- Better not to touch it for now.
					echo sprintf(
						...
						esc_html__( 'Error, the file %1$s could not be moved to: %2$s', 'ebox' ),
						esc_html( $file_tmp ),
						esc_html( $file_dest )
					);
					continue;
				 ...
				 */
				ebox_upload_assignment_init( $post_id, $filename, $file_dest );
				$file_desc             = array();
 ...
 * @return boolean
 */
function ebox_lesson_hasassignments( $post ) {
	$post_id     = $post->ID;
 ...
 */
function ebox_approve_assignment_by_id( $assignment_id ) {
	...
	return ebox_approve_assignment( $user_id, $lesson_id, $assignment_id );
}
 ...
 * @return boolean Returns true if the assignment is approved otherwise false.
 */
function ebox_approve_assignment( $user_id, $lesson_id, $assignment_post_id = 0 ) {
	 ...
	 */
	$ebox_approve_assignment = apply_filters( 'ebox_approve_assignment', true, $user_id, $lesson_id, $assignment_post_id );

	if ( $ebox_approve_assignment ) {
		$assignment_course_id            = get_post_meta( $assignment_post_id, 'course_id', true );
		$ebox_process_mark_complete = ebox_process_mark_complete( $user_id, $lesson_id, null, $assignment_course_id );
		if ( $ebox_process_mark_complete ) {
			...
			foreach ( $assignment_ids as $assignment_id ) {
				...
				if ( $assignment->post_author == $user_id ) {
					ebox_assignment_mark_approved( $assignment_id );
					 ...
					 * @param int $assignment_id Assignment ID.
					 */
					do_action( 'ebox_assignment_approved', $assignment_id );
				}
			}
		}

		return $ebox_process_mark_complete;
	}
 ...
 * @param int $assignment_id Assignment ID.
 */
function ebox_assignment_mark_approved( $assignment_id ) {
 ...
 */
function ebox_is_assignment_approved_by_meta( $assignment_id ) {
 ...
 */
function ebox_is_assignment_approved( $assignment_id ) {
	$assignment = get_post( $assignment_id );
	...
	$lesson_id = ebox_get_lesson_id( $assignment->ID );

	if ( empty( $lesson_id ) ) {
		return '';
	}

	$lesson_completed = ebox_is_lesson_notcomplete( $assignment->post_author, array( $lesson_id => 1 ) );
 ...
 *
 * Only allow admins, team leaders, and assignment owners to see the assignment.
 * Fires on `wp` hook.
 *
 ...
 * @since 2.1.0
 */
function ebox_assignment_permissions() {
	if ( is_singular( ebox_get_post_type_slug( 'assignment' ) ) ) {
		...
		if ( ebox_is_admin_user( $user_id ) ) {
			...
			/**
			 * For the Team Leader we check for common teams between
			 * Leader + Author + Course
			 */
			$course_id = get_post_meta( $post->ID, 'course_id', true );
			$course_id = absint( $course_id );

			if ( ebox_check_team_leader_course_user_intersect( $user_id, $post->post_author, $course_id ) ) {
				return;
			}
		 ...
		 */
		ebox_safe_redirect( apply_filters( 'ebox_assignment_permissions_redirect_url', get_bloginfo( 'url' ) ) );
	} elseif ( ( is_home() ) || ( is_front_page() ) ) {
		/**
		 * Prevents the user from forcing the query on the home page
		 * with http://www.site.com?post_type=ebox-assignment to access an archive.
		 *
		 * It would be nice if this is controllable via WP register_post_type() settings.
		 *
		 * See ebox-6390 for more details.
		 */
		if ( get_query_var( 'post_type', '' ) === ebox_get_post_type_slug( 'assignment' ) ) {
			...
			if ( ! empty( $redirect_to_url ) ) {
				ebox_safe_redirect( $redirect_to_url );
			}
		}
	}
}

add_action( 'wp', 'ebox_assignment_permissions' );
 ...
 * @since 2.1.0
 */
function ebox_register_assignment_upload_type() {

	$exclude_from_search = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Assignments_CPT', 'exclude_from_search' );
	if ( 'yes' === $exclude_from_search ) {
		$exclude_from_search = true;
	} else {
		$exclude_from_search = false;
	}
	$publicly_queryable = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Assignments_CPT', 'publicly_queryable' );
	...
	$show_in_rest = ebox_REST_API::enabled( ebox_get_post_type_slug( 'assignment' ) ) || ebox_REST_API::gutenberg_enabled( ebox_get_post_type_slug( 'assignment' ) );

	$labels = array(
		'name'               => esc_html__( 'Assignments', 'ebox' ),
		'singular_name'      => esc_html__( 'Assignment', 'ebox' ),
		'edit_item'          => esc_html__( 'Edit Assignment', 'ebox' ),
		'view_item'          => esc_html__( 'View Assignment', 'ebox' ),
		'view_items'         => esc_html__( 'View Assignments', 'ebox' ),
		'search_items'       => esc_html__( 'Search Assignments', 'ebox' ),
		'not_found'          => esc_html__( 'No assignment found', 'ebox' ),
		'not_found_in_trash' => esc_html__( 'No assignment found in Trash', 'ebox' ),
		'parent_item_colon'  => esc_html__( 'Parent:', 'ebox' ),
		'menu_name'          => esc_html__( 'Assignments', 'ebox' ),
	);
	 ...
	 */
	$args = apply_filters( 'ebox-cpt-options', $args, 'ebox-assignment' ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores -- Better to keep it this way for now.

	register_post_type( 'ebox-assignment', $args );
}

add_action( 'init', 'ebox_register_assignment_upload_type' );
 ...
 * @return void
 */
function ebox_init_assignments_capabilities(): void {
	...
	if ( $team_leader_role instanceof WP_Role ) {
		$team_leader_role_capabilities = array(
			'read_assignment',
			'edit_assignments',
			'edit_others_assignments',
			'edit_published_assignments',
		...
		foreach ( $team_leader_role_capabilities as $capability ) {
			if ( ! $team_leader_role->has_cap( $capability ) ) {
				$team_leader_role->add_cap( $capability );
			}
		}
	}
}

add_action( 'admin_init', 'ebox_init_assignments_capabilities' );
 ...
 */
function ebox_before_delete_assignment( $post_id ) {

	if ( ( ! empty( $post_id ) ) && ( ebox_get_post_type_slug( 'assignment' ) === get_post_type( $post_id ) ) ) {
...
add_action( 'before_delete_post', 'ebox_before_delete_assignment' );

/**
 * Returns the number of points awarded for an assignment.
 *
 * Displayed on single modules under the submitted assignment
 *
 * @param int $assignment_id ID of the assignment.
 *
 * @return string Returns the number of points awarded string.
 */
function ebox_assignment_points_awarded( $assignment_id ) {
	$points_enabled = ebox_assignment_is_points_enabled( $assignment_id );

	if ( $points_enabled ) {
		$current = ebox_get_assignment_points_awarded( $assignment_id );
		...
		return apply_filters(
			'ebox_points_awarded_output',
			sprintf(
				// translators: placeholder: points awarded values (30/100) 30%.
				esc_html_x( 'Points Awarded: %s', 'placeholder: points awarded values (30/100) 30%', 'ebox' ),
				$current
			),
			$current
		);
 ...
 */
function ebox_get_assignment_points_awarded( $assignment_id ) {
	...
	if ( ( ! empty( $current ) ) && ( ! is_numeric( $current ) ) ) {
		return esc_html__( 'Pending', 'ebox' );
	}
	...
	if ( is_numeric( $current ) ) {
		 ...
		 */
		return apply_filters(
			'ebox_points_awarded_output_format',
			sprintf(
				'(%1$d/%2$d) %3$d&#37; ',
				$current,
				$max_points,
 ...
 */
function ebox_assignment_is_points_enabled( $assignment ) {
	...
	$assignment_settings_id = intval( get_post_meta( $assignment_id, 'lesson_id', true ) );
	$points_enabled         = ebox_get_setting( $assignment_settings_id, 'lesson_assignment_points_enabled' );
 ...
 */
function ebox_return_bytes_from_shorthand( $val = 0 ) {
 ...
 * @return boolean
 */
function ebox_check_upload( $uploadfiles = array(), $post_id = 0 ) {
	$post_settings = array();
	...
	if ( ( is_array( $uploadfiles ) ) && ( ! empty( $post_id ) ) ) {
		...
		$post_settings = ebox_get_setting( $post_id );
		if ( ( isset( $post_settings['assignment_upload_limit_size'] ) ) && ( ! empty( $post_settings['assignment_upload_limit_size'] ) ) ) {
			...
			$limit_file_size = ebox_return_bytes_from_shorthand( $limit_file_size );
		...
		if ( ( empty( $limit_file_size ) ) || ( intval( $uploadfiles['size'][0] ) > $limit_file_size ) ) {
			update_user_meta(
				...
				array(
					array(
						'type'    => 'error',
						'message' => esc_html__( 'Uploaded file size exceeds allowed limit.', 'ebox' ),
					),
				)
			);
			return false;
		...
		$limit_file_exts = ebox_get_allowed_upload_mime_extensions_for_post( $post_id );
		...
		if ( ( empty( $filetype_mime ) ) || ( empty( $filetype_mime['ext'] ) ) || ( empty( $filetype_mime['type'] ) ) || ( ! $limit_file_exts[ strtolower( $filetype_mime['ext'] ) ] ) ) {
			update_user_meta(
				...
				array(
					array(
						'type'    => 'error',
						'message' => esc_html__( 'The uploaded file type is not allowed.', 'ebox' ),
					),
				)
			);
			return false;
		...
		if ( isset( $post_settings['assignment_upload_limit_count'] ) ) {
			...
			if ( $assignment_upload_limit_count > 0 ) {
				$assignments = ebox_get_user_assignments( $post_id, get_current_user_id() );
				if ( ( ! empty( $assignments ) ) && ( count( $assignments ) >= $assignment_upload_limit_count ) ) {
					update_user_meta(
						...
						array(
							array(
								'type'    => 'error',
								'message' => esc_html__( 'Number of allowed assignment uploads reached.', 'ebox' ),
							),
						)
					);
					return false;
	 ...
	 * );
	 */
	return (bool) apply_filters( 'ebox_assignment_check_upload', true, $uploadfiles, $post_id, $post_settings );
}
 ...
 */
function ebox_assignment_list_approved( array $assignment_ids, int $step_id, int $user_id ): bool {
	...
	if ( ! empty( $step_id ) ) {
		$course_id      = absint( ebox_get_course_id( $step_id ) );
		$assignment_ids = ebox_get_user_assignments( $step_id, $user_id, $course_id, 'ids' );
	}

	foreach ( $assignment_ids as $assignment ) {
		$approval_status = ebox_is_assignment_approved_by_meta( $assignment );
