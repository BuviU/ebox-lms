 ...
 * as the [course_progress] shortcode used within ebox.
 *
 * @package ebox
 * @since 2.5.9
 */
...
if ( ( class_exists( 'ebox_Gutenberg_Block' ) ) && ( ! class_exists( 'ebox_Gutenberg_Block_Course_Progress' ) ) ) {
	/**
	 * Class for handling ebox Course Progress Block
	 */
	class ebox_Gutenberg_Block_Course_Progress extends ebox_Gutenberg_Block {

		/**
		 * Object constructor
		 */
		public function __construct() {
			$this->shortcode_slug   = 'ebox_course_progress';
				...
				if ( empty( $block_attributes['course_id'] ) ) {
					return $this->render_block_wrap(
						...
						// translators: placeholder: Course, Quiz, Course.
							_x( '%1$s ID or %2$s ID is required when not used within a %3$s.', 'placeholder: Course, Quiz, Course', 'ebox' ),
							ebox_Custom_Label::get_label( 'course' ),
...
new ebox_Gutenberg_Block_Course_Progress();
