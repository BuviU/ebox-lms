 ...
 * as the ld_materials shortcode used within ebox.
 *
 * @package ebox
 * @since 4.0.0
 */
...
if ( ( class_exists( 'ebox_Gutenberg_Block' ) ) && ( ! class_exists( 'ebox_Gutenberg_Block_Materials' ) ) ) {
	/**
	 * Class for handling ebox Materials Block
	 */
	class ebox_Gutenberg_Block_Materials extends ebox_Gutenberg_Block {
		...
		public function render_block( $block_attributes = array(), $block_content = '', WP_block $block = null ) {
			...
			if ( $this->block_attributes_is_editing_post( $block_attributes ) ) {
				...
				if ( ! empty( $block_attributes['post_id'] ) ) {
					...
					if ( ( ! $post ) || ( ! is_a( $post, 'WP_Post' ) ) || ( ! in_array( $post->post_type, ebox_get_post_types(), true ) ) ) {
						return $this->render_block_wrap(
							'<span class="ebox-block-error-message">' . esc_html__( 'Invalid ebox Post ID.', 'ebox' ) . '</span>'
						);
					}
				}
			}
...
new ebox_Gutenberg_Block_Materials();
