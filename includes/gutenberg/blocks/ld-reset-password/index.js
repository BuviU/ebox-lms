...
const block_key = "ebox/ld-reset-password";
const block_title = __("ebox Reset Password", "ebox");
registerBlockType(block_key, {
  title: block_title,
  description: __("Shows the reset password form", "ebox"),
  icon: "id-alt",
  ...
  edit: function (props) {
    const {
      attributes: { preview_show, example_show, width },
      setAttributes,
    } = props;
    const inspectorControls = (
      <InspectorControls key="controls">
        <PanelBody title={__("Styling", "ebox")} initialOpen={true}>
          <TextControl
            label={__("Form Width", "ebox")}
            help={__("Sets the width of the reset password form.", "ebox")}
            value={width || ""}
            type={"string"}
