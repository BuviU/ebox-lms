...
const block_key = "ebox/ld-course-info";
const block_title = sprintf(
  // translators: placeholder: Course.
  _x("ebox %s Info [ld_course_info]", "placeholder: Course", "ebox"),
    ...
    const panelbody_header = (
      <PanelBody title={__("Settings", "ebox")}>
        ...
        <ToggleControl
          label={sprintf(
            // translators: placeholder: Courses.
            _x("Show Registered %s", "placeholder: Courses", "ebox"),
        ...
        <ToggleControl
          label={sprintf(
            // translators: placeholder: Course.
            _x("Show %s Progress", "placeholder: Course", "ebox"),
            ldlms_get_custom_label("course")
          )}
        ...
        <ToggleControl
          label={sprintf(
            ...
            _x("Show %s Attempts", "placeholder: Quiz", "ebox"),
    ...
    if (registered_show === true) {
      panelbody_registered = (
        <PanelBody
          title={sprintf(
            // translators: placeholder: Courses.
            _x("Registered %s", "placeholder: Courses", "ebox"),
            ldlms_get_custom_label("courses")
          )}
          initialOpen={false}
        >
          <ToggleControl
            ...
            onChange={(registered_show_thumbnail) =>
              setAttributes({ registered_show_thumbnail })
            }
          />
          <TextControl
            label={__("per page", "ebox")}
            help={sprintf(
          ...
          <SelectControl
            ...
            options={[
              {
                label: __("Title - Order by post title (default)", "ebox"),
                value: "title",
              },
              {
                label: __("ID - Order by post id", "ebox"),
                value: "ID",
              },
              {
            ...
            onChange={(registered_orderby) =>
              setAttributes({ registered_orderby })
            }
          />
          <SelectControl
            ...
            options={[
              {
                label: __("ASC - lowest to highest values (default)", "ebox"),
                value: "ASC",
              },
              {
                label: __("DESC - highest to lowest values", "ebox"),
                value: "DESC",
              },
            ]}
    ...
    if (progress_show === true) {
      panelbody_progress = (
        <PanelBody
          title={sprintf(
            // translators: placeholder: Course.
            _x("%s Progress", "placeholder: Course", "ebox"),
            ldlms_get_custom_label("course")
          )}
          initialOpen={false}
        >
          <TextControl
            ...
            help={sprintf(
              ...
              ldlms_get_per_page("progress_num")
            )}
            value={progress_num || ""}
            min={0}
            max={100}
    ...
    if (quiz_show === true) {
      panelbody_quiz = (
        <PanelBody
          title={sprintf(
            // translators: placeholder: Quiz.
            _x("%s Attempts", "placeholder: Quiz", "ebox"),
            ldlms_get_custom_label("quiz")
          )}
          initialOpen={false}
        >
          ...
          <SelectControl
            ...
            options={[
              {
                label: __("Date Taken (default) - Order by date taken", "ebox"),
                value: "taken",
              },
              {
