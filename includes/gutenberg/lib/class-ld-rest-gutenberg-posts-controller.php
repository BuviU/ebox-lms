<?php
/**
 * ebox Gutenberg Posts Controller.
 *
 * @package ebox
 * @since 2.5.8
 */
...
if ( ! class_exists( 'LD_REST_Posts_Gutenberg_Controller' ) ) {
	...
	class LD_REST_Posts_Gutenberg_Controller extends WP_REST_Posts_Controller {
		...
		public function register_routes() {
			...
			if ( isset( $schema['properties']['password'] ) ) {
				$get_item_args['password'] = array(
					'description' => __( 'The password for the post if it is password protected.', 'ebox' ),
					'type'        => 'string',
				);
			}
					...
					array(
						...
						'args'                => array(
							'force' => array(
								'type'        => 'boolean',
								'default'     => false,
								'description' => __( 'Whether to bypass trash and force deletion.', 'ebox' ),
							),
						),
					),
		...
		public function get_item_permissions_check( $request ) {
			 ...
			 * @since 3.2.0
			 */
			if ( ( defined( 'ebox_BLOCK_WORDPRESS_CPT_ROUTES' ) ) && ( true === ebox_BLOCK_WORDPRESS_CPT_ROUTES ) ) {
				...
				if ( $post && ! $this->check_update_permission( $post ) ) {
					return new WP_Error(
						...
						__( 'Sorry, you are not allowed to edit this post.', 'ebox' ),
				...
				if ( $post && ! empty( $request['password'] ) ) {
					...
					if ( ! hash_equals( $post->post_password, $request['password'] ) ) {
						return new WP_Error(
							...
							__( 'Incorrect post password.', 'ebox' ),
							array( 'status' => 403 )
						);
					}
				}
		...
		public function get_items_permissions_check( $request ) {
			if ( ebox_is_valid_post_type( $this->post_type ) ) {
				$post_type_object = get_post_type_object( $this->post_type );
				if ( ( ! $post_type_object ) || ( ! is_a( $post_type_object, 'WP_Post_Type' ) ) ) {
					return new WP_Error(
						'rest_type_invalid',
						__( 'Invalid post type.', 'ebox' ),
						array( 'status' => 404 )
					);
				}
				...
				if ( ( ! property_exists( $post_type_object, 'show_in_rest' ) ) || ( true !== $post_type_object->show_in_rest ) ) {
					return new WP_Error(
						...
						__( 'Cannot view post type.', 'ebox' ),
						array( 'status' => rest_authorization_required_code() )
					);
				}

				$can_view_archive = false;
				if ( ebox_post_type_has_archive( $this->post_type ) ) {
				 ...
				 */
				$can_view_archive = apply_filters( 'ebox_rest_wp_archive_viewable', $can_view_archive, $this->post_type, $request );
				if ( ! $can_view_archive ) {
					return new WP_Error(
						'rest_cannot_view',
						__( 'Sorry, you are not allowed to edit posts in this post type.', 'ebox' ),
						array( 'status' => rest_authorization_required_code() )
					);
				}
			}
		...
		public function get_items( $request ) {
			 ...
			 */
			$data = apply_filters( 'ebox_rest_wp_archive_repsonse', $data, $this->post_type, $request ); // cspell:disable-line.
