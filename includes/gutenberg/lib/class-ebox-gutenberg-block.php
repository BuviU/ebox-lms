<?php
/**
 * Base class for all ebox Gutenberg Blocks.
 *
 * @package ebox
 * @since 2.5.9
 */
...
if ( ! class_exists( 'ebox_Gutenberg_Block' ) ) {
	/**
	 * Abstract Parent class to hold common functions used by specific ebox Blocks.
	 *
	 * @since 2.5.9
	 */
	class ebox_Gutenberg_Block {
		...
		public function init() {
			if ( function_exists( 'register_block_type' ) ) {
				...
				add_filter( 'ebox_block_markers_shortcode_atts', array( $this, 'ebox_block_markers_shortcode_atts_filter' ), 30, 4 );

				if ( ( defined( 'ebox_GUTENBERG_CONTENT_PARSE_LEGACY' ) ) && ( true === ebox_GUTENBERG_CONTENT_PARSE_LEGACY ) ) {
					/**
					...
					add_filter( 'ebox_convert_block_markers_to_shortcode_content', array( $this, 'convert_block_markers_to_shortcode_content_filter' ), 30, 4 );
				}
			}
		}
		...
		public function render_block_wrap( $content = '', $with_inner = true ) {
			...
			if ( true === $with_inner ) {
				 ...
				 */
				$extra_classes = '';
				if ( ! strstr( $content, 'ebox-wrap' ) ) {
					$extra_classes = ' ebox-wrap';
				}
				if ( ! strstr( $content, 'ebox-wrapper' ) ) {
					$extra_classes = ' ebox-wrapper';
				}
				$return_content .= '<div class="ebox-block-inner' . $extra_classes . '">';
			}
		...
		public function convert_block_markers_to_shortcode( $content = '', $block_slug = '', $shortcode_slug = '', $self_closing = false ) {
			if ( ( ! empty( $content ) ) && ( ! empty( $block_slug ) ) && ( ! empty( $shortcode_slug ) ) ) {
				...
				if ( true === $self_closing ) {
						...
						if ( ( isset( $ar[1] ) ) && ( is_array( $ar[1] ) ) && ( ! empty( $ar[1] ) ) ) {
							foreach ( $ar[1] as $pattern_key => $pattern_atts_json ) {
								...
								if ( ! empty( $pattern_atts_json ) ) {
									...
									$pattern_atts_array = apply_filters( 'ebox_block_markers_shortcode_atts', $pattern_atts_array, $shortcode_slug, $block_slug, $content );
									if ( ( is_array( $pattern_atts_array ) ) && ( ! empty( $pattern_atts_array ) ) ) {
										$shortcode_atts = '';
										foreach ( $pattern_atts_array as $attr_key => $attr_value ) {
											if ( 'meta' === $attr_key ) {
								...
								if ( ! empty( $replacement_text ) ) {
									 ...
									 */
									$content = apply_filters( 'ebox_convert_block_markers_to_shortcode_content', $content, $pattern_atts_array, $shortcode_slug, $block_slug );
								}
							}
						}
					}
					 ...
					 *
					 * <!-- wp:ebox/ld-student {"course_id":"109"} -->
					 * <!-- wp:paragraph -->
						...
						if ( ( isset( $ar[1] ) ) && ( is_array( $ar[1] ) ) && ( ! empty( $ar[1] ) ) ) {
							foreach ( $ar[1] as $pattern_key => $pattern_atts_json ) {
								...
								if ( ( isset( $ar[2][ $pattern_key ] ) ) && ( ! empty( $ar[2][ $pattern_key ] ) ) ) {
									...
									if ( ! empty( $replacement_text ) ) {
										$content = str_replace( $ar[0][ $pattern_key ], $replacement_text, $content );
										/** This filter is documented in includes/gutenberg/lib/class-ebox-gutenberg-block.php */
										$content = apply_filters( 'ebox_convert_block_markers_to_shortcode_content', $content, $pattern_atts_array, $shortcode_slug, $block_slug );
									}
								}
							}
					...
					if ( ( 'preview_user_id' === $key ) && ( '' !== $val ) ) {
						...
						if ( ! ebox_is_admin_user( get_current_user_id() ) ) {
							if ( ebox_is_team_leader_user( get_current_user_id() ) ) {
								// If team leader user we ensure the preview user_id is within their team(s).
								if ( ! ebox_is_team_leader_of_user( get_current_user_id(), $val ) ) {
									continue;
								}
							} else {
								// If neither admin or team leader then we don't see the user_id for the shortcode.
								continue;
							}
						}
		...
		public function get_example_user_id() {
			 ...
			 */
			$user_id = apply_filters( 'ebox_gutenberg_block_example_id', $user_id, 'user_id', 'user', $this->block_slug );
		...
		public function get_example_post_id( $post_type = '' ) {
			$post_id = 0;
			/** This filter is documented in includes/gutenberg/lib/class-ebox-gutenberg-block.php */
			$post_id = apply_filters( 'ebox_gutenberg_block_example_id', $post_id, 'post_id', $post_type, $this->block_slug );
			...
			if ( empty( $post_id ) ) {
				$post_id = ebox_get_single_post( $post_type );
			}
		 ...
		 *
		 * @param array  $block_attributes The block attributes.
		 * @param string $post_prefix     The post prefix to use. Example: 'course', 'quiz', 'team', etc.
		 *
		 * @return int Post ID.
		 */
