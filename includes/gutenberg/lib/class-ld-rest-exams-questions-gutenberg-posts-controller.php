<?php
/**
 * ebox REST API Questions Gutenberg Controller.
 *
...
if ( ( ! class_exists( 'LD_REST_Exams_Questions_Gutenberg_Controller' ) ) && ( class_exists( 'WP_REST_Posts_Controller' ) ) ) {
	...
	/**
	 * Class ebox REST API Questions Gutenberg Controller.
	 *
	 * @since 4.0.0
 ...
 // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
		...
		public function __construct( $post_type = '' ) {
			if ( empty( $post_type ) ) {
				$post_type = ebox_get_post_type_slug( 'exam_question' );
			}
		...
		public function get_item( $request ) {
			try {
				...
				return new WP_Error(
					...
					__( 'Error while trying to get the item', 'ebox' ),
					array( 'status' => 400 )
				);
			}
		}
		...
		private function get_ld_question_block_content( $post_id ) {
			...
			return serialize_block(
				array(
					'blockName'    => 'ebox/ld-exam-question',
			...
			try {
				...
				if ( empty( $question['question_title'] ) ) {
					return new WP_Error(
						...
						__( 'Missing question_title', 'ebox' ),
						array( 'status' => 400 )
					);
				}
		...
		/**
		 * Get ebox/ld-question block from a ld_question post.
		 *
		 * @since 4.0.0
		 *
		...
		private function get_block_ld_question( $request ) {
			if ( empty( $request['id'] ) || empty( $request['content'] ) ) {
				return new WP_Error(
					'rest_post-ld_question',
					__( 'Missing content', 'ebox' ),
					array( 'status' => 400 )
				);
			}
			$block = parse_blocks( trim( $request['content'] ) );
			if ( empty( $block[0] ) || 'ebox/ld-exam-question' !== $block[0]['blockName'] ) {
				return new WP_Error(
					'rest_post-ld_question',
					__( 'Missing ebox/ld-exam-question block', 'ebox' ),
					array( 'status' => 400 )
				);
			}
