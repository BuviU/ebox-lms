...
if ( ( ! class_exists( 'Quiz_Widget' ) ) && ( class_exists( 'WP_Widget' ) ) ) {
	...
	class Quiz_Widget extends WP_Widget /* phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound */ {
		...
		public function __construct() {
			...
			$this->post_name = ebox_Custom_Label::get_label( 'quiz' );

			if ( empty( $args['description'] ) ) {
				// translators: placeholder: Quiz.
				$args['description'] = sprintf( esc_html_x( 'Displays a list of %s', 'placeholder: Quiz', 'ebox' ), $this->post_name );
			}
