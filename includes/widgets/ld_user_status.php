...
if ( ( ! class_exists( 'ebox_User_Status_Widget' ) ) && ( class_exists( 'WP_Widget' ) ) ) {
	/**
	 * Class for ebox `User_Status` Widget.
	 *
	 * @since 3.0.0
	 * @uses WP_Widget
	 */
	class ebox_User_Status_Widget extends WP_Widget {
		...
		public function __construct() {
			$widget_ops  = array(
				...
				'description' => sprintf(
					...
					esc_html_x( 'ebox - Registered %s and progress information of users. Visible only to users logged in.', 'placeholders: courses', 'ebox' ),
					ebox_Custom_Label::get_label( 'courses' )
				),
			);
			$control_ops = array();
			parent::__construct( 'lduserstatus', __( 'User Status', 'ebox' ), $widget_ops, $control_ops );
		}
		...
		public function widget( $args, $instance ) {
			...
			$user_status = ebox_LMS::get_template(
		...
		public function form( $instance ) {
			...
			?>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'registered_show_thumbnail' ) ); ?>"><?php esc_html_e( 'Registered show thumbnail:', 'ebox' ); ?></label>
					<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'registered_show_thumbnail' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'registered_show_thumbnail' ) ); ?>">
						<option value="" <?php selected( $registered_show_thumbnail, '' ); ?>><?php esc_html_e( 'Yes (default)', 'ebox' ); ?></option>
						<option value="false" <?php selected( $registered_show_thumbnail, 'false' ); ?>><?php esc_html_e( 'No', 'ebox' ); ?></option>
					</select>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'registered_num' ) ); ?>"><?php esc_html_e( 'Registered per page:', 'ebox' ); ?></label>
					...
					<?php
						printf(
							...
							esc_html_x( 'Default is %d. Set to zero for no pagination.', 'placeholders: default per page', 'ebox' ),
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'registered_orderby' ) ); ?>"><?php esc_html_e( 'Registered order by:', 'ebox' ); ?></label>
					<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'registered_orderby' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'registered_orderby' ) ); ?>">
						<option value="" <?php selected( $registered_orderby, '' ); ?>><?php esc_html_e( 'Title (default) - Order by post title', 'ebox' ); ?></option>
						<option value="id" <?php selected( $registered_orderby, 'id' ); ?>><?php esc_html_e( 'ID - Order by post id', 'ebox' ); ?></option>
						<option value="date" <?php selected( $registered_orderby, 'date' ); ?>><?php esc_html_e( 'Date - Order by post date', 'ebox' ); ?></option>
						<option value="menu_order" <?php selected( $registered_orderby, 'menuorder' ); ?>><?php esc_html_e( 'Menu - Order by Page Order Value', 'ebox' ); ?></option>
					</select>
				</p>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'registered_order' ) ); ?>"><?php esc_html_e( 'Registered order:', 'ebox' ); ?></label>
					<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'registered_order' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'registered_order' ) ); ?>">
						<option value="" <?php selected( $registered_order, '' ); ?>><?php esc_html_e( 'ASC (default) - lowest to highest values', 'ebox' ); ?></option>
						<option value="DESC" <?php selected( $registered_order, 'DESC' ); ?>><?php esc_html_e( 'DESC - highest to lowest values', 'ebox' ); ?></option>
					</select>
				</p>
	...
	add_action(
		'widgets_init',
		function() {
			return register_widget( 'ebox_User_Status_Widget' );
		}
	);
}
