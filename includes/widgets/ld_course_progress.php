<?php
/**
 * ebox `Course Progress` Widget Class.
 *
 * @since 2.1.0
 ...
 */
if ( ( ! class_exists( 'ebox_Course_Progress_Widget' ) ) && ( class_exists( 'WP_Widget' ) ) ) {

	/**
	 * Class for ebox `Course Progress` Widget.
	 *
	 * @since 2.1.0
	 * @uses WP_Widget
	 */
	class ebox_Course_Progress_Widget extends WP_Widget {
		...
		public function __construct() {
			$widget_ops  = array(
				...
				'description' => sprintf( esc_html_x( 'ebox %s progress bar', 'placeholders: course', 'ebox' ), ebox_get_custom_label_lower( 'course' ) ),
			);
			$control_ops = array();
			// translators: placeholder: Course.
			parent::__construct( 'ldcourseprogress', sprintf( esc_html_x( '%s Progress Bar', 'Course Progress Bar Label', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) ), $widget_ops, $control_ops );
		}
		...
		public function widget( $args, $instance ) {
			...
			$progressbar = ebox_course_progress( $args );
		...
		public function form( $instance ) {
			...
			?>
			<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'ebox' ); ?></label>
	...
	add_action(
		'widgets_init',
		function() {
			return register_widget( 'ebox_Course_Progress_Widget' );
		}
	);
}
