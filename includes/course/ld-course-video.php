 ...
 *
 * @package ebox\Video_Progression
 * @since 2.4.0
 */
...
if ( ! class_exists( 'ebox_Course_Video' ) ) {
	/**
	 * Class for handling the ebox Video Progression.
	 *
	 * @since 2.4.0
	 */
	class ebox_Course_Video {
		...
		/**
		 * ebox Vide Progress constructor.
		 *
		 * @since 2.4.0
		 */
		public function __construct() {
			add_action( 'wp_footer', array( $this, 'action_wp_footer' ), 1 );
			add_filter( 'ebox_process_mark_complete', array( $this, 'process_mark_complete' ), 99, 3 );
		}
		...
		/**
		 * Hook into the ebox template logic to insert the Video Progression output
		 *
		 * @since 2.4.3
		 *
		...
		public function add_video_to_content( $content, $post, $settings = array() ) {
			...
			$this->course_id     = (int) ebox_get_course_id( $post->ID );
			...
			if ( ( isset( $this->step_settings['lesson_video_enabled'] ) ) && ( 'on' === $this->step_settings['lesson_video_enabled'] ) ) {
				if ( ( isset( $this->step_settings['lesson_video_url'] ) ) && ( ! empty( $this->step_settings['lesson_video_url'] ) ) ) {
					// Because some copy/paste can result in leading whitespace. ebox-3819.
					...
					$bypass_course_limits_admin_users = ebox_can_user_bypass( $this->user_id, 'ebox_video_progression', $post->ID, $post );
					...
					if ( ! $bypass_course_limits_admin_users ) {
						...
						if ( 'ebox-modules' === $post->post_type ) {
							$progress = ebox_get_course_progress( $this->user_id, $post->ID );
							...
							if ( ( ! empty( $progress['this'] ) ) && ( $progress['this'] instanceof WP_Post ) && ( true === (bool) $progress['this']->completed ) ) {
								...
								if ( 'BEFORE' === $this->step_settings['lesson_video_shown'] ) {
									$show_video           = true;
									$logic_video          = true;
									$complete_child_steps = ebox_user_progression_get_complete_child_steps( $this->user_id, $this->course_id, $post->ID );
									...
									$incomplete_child_steps = ebox_user_progression_get_incomplete_child_steps( $this->user_id, $this->course_id, $post->ID );
									...
									if ( ! empty( $video_preg_pattern ) ) {
										...
										if ( ( is_array( $matches ) ) && ( isset( $matches[1] ) ) && ( ! empty( $matches[1] ) ) ) {
											...
											if ( 'youtube' === $this->video_data['videos_found_provider'] ) {
												 ...
												 * @param array   $settings       An array of ebox settings for a post.
												 */
												...
												$this->video_content = str_replace( '<iframe ', '<iframe data-ebox-video-wistia-id="' . $video_id . '" ', $this->video_content );
											} elseif ( 'vooplayer' === $this->video_data['videos_found_provider'] ) {
												/** This filter is documented in includes/course/ld-course-video.php */
												$ld_video_params = apply_filters(
													'ld_video_params',
										...
										if ( ( isset( $this->step_settings['lesson_video_auto_complete'] ) ) && ( 'on' === $this->step_settings['lesson_video_auto_complete'] ) ) {
											...
											if ( isset( $this->step_settings['lesson_video_auto_complete_delay'] ) ) {
												...
												'<p class="ld-video-delay-message">' . sprintf(
													...
													esc_html_x( '%1$s will auto complete in %2$s seconds', 'placeholders: 1. Lesson or Topic label, 2. span for counter', 'ebox' ),
				...
				if ( ! empty( $this->video_content ) ) {
					if ( false !== $this->video_data['videos_found_provider'] ) {
						...
						$video_post_url       = ebox_get_step_permalink( $post );
						 ...
						 * @param array  $settings       An array of ebox settings for a post.
						 */
						$this->video_data = apply_filters( 'ebox_lesson_video_data', $this->video_data, $this->step_settings );
		...
		public function action_wp_footer() {
			if ( false !== $this->video_data['videos_found_provider'] ) {
				...
				wp_enqueue_script(
					'ebox_cookie_script_js',
					ebox_LMS_PLUGIN_URL . 'assets/vendor-libs/js-cookie/js.cookie' . ebox_min_asset() . '.js',
					array(),
					ebox_SCRIPT_VERSION_TOKEN,
					true
				);
				$ebox_assets_loaded['scripts']['ebox_cookie_script_js'] = __FUNCTION__;

				wp_enqueue_script(
					'ebox_video_script_js',
					ebox_LMS_PLUGIN_URL . 'assets/js/ebox_video_script' . ebox_min_asset() . '.js',
					array( 'jquery' ),
					ebox_SCRIPT_VERSION_TOKEN,
					true
				);
				$ebox_assets_loaded['scripts']['ebox_video_script_js'] = __FUNCTION__;

				wp_localize_script( 'ebox_video_script_js', 'ebox_video_data', $this->video_data );

				if ( 'youtube' === $this->video_data['videos_found_provider'] ) {
					wp_enqueue_script( 'youtube_iframe_api', 'https://www.youtube.com/iframe_api', array( 'ebox_video_script_js' ), ebox_SCRIPT_VERSION_TOKEN, true );
				} elseif ( 'vimeo' === $this->video_data['videos_found_provider'] ) {
					wp_enqueue_script( 'vimeo_iframe_api', 'https://player.vimeo.com/api/player.js', array( 'ebox_video_script_js' ), ebox_SCRIPT_VERSION_TOKEN, true );
				}
			}
		}
		...
		public function process_mark_complete( $process_complete, $post, $current_user ) {
			if ( ( isset( $_GET['quiz_redirect'] ) ) && ( ! empty( $_GET['quiz_redirect'] ) ) && ( isset( $_GET['quiz_type'] ) ) && ( 'lesson' === $_GET['quiz_type'] ) ) {
				...
				if ( ( ! empty( $lesson_id ) ) && ( ! empty( $quiz_id ) ) ) {
					$lesson_settings = ebox_get_setting( $lesson_id );
					if ( ( isset( $lesson_settings['lesson_video_enabled'] ) ) && ( 'on' === $lesson_settings['lesson_video_enabled'] ) ) {
						if ( ( isset( $lesson_settings['lesson_video_shown'] ) ) && ( 'AFTER' === $lesson_settings['lesson_video_shown'] ) ) {
							...
							add_filter( 'ebox_completion_redirect', array( $this, 'ebox_completion_redirect' ), 99 );
						}
					}
				}
			}
		 ...
		 */
		public function ebox_completion_redirect( $link ) {
			if ( ( isset( $_GET['quiz_redirect'] ) ) && ( ! empty( $_GET['quiz_redirect'] ) ) && ( isset( $_GET['quiz_type'] ) ) && ( 'lesson' === $_GET['quiz_type'] ) ) {
				$lesson_id = 0;
				$quiz_id   = 0;
				...
				if ( ( ! empty( $lesson_id ) ) && ( ! empty( $quiz_id ) ) ) {
					...
					if ( ( isset( $lesson_settings['lesson_video_enabled'] ) ) && ( 'on' === $lesson_settings['lesson_video_enabled'] ) ) {
						if ( ( isset( $lesson_settings['lesson_video_shown'] ) ) && ( 'AFTER' === $lesson_settings['lesson_video_shown'] ) ) {
							$link = ebox_get_step_permalink( $lesson_id );

							remove_filter( 'ebox_completion_redirect', array( $this, 'ebox_completion_redirect' ), 99 );
						}
					}
				}
			}
		...
		public function build_video_cookie_key() {
			...
			$cookie_key = 'ebox-video-progress-' . md5( $cookie_key );
		...
		protected function get_nonce_slug() {
			return 'ebox_video_' . $this->user_id . '_' . $this->course_id . '_' . $this->step_id;
		}
		...
		public function check_video_complete( $step_id = 0, $course_id = 0, $user_id = 0 ) {
			...
			if ( empty( $this->course_id ) ) {
				$this->course_id = (int) ebox_get_course_id( $this->step_id );
				if ( empty( $this->course_id ) ) {
					return;
				}
			}

			$this->step_settings = ebox_get_setting( $this->step_id );

			// Check if any sub-step is complete.
			if ( 'BEFORE' === $this->step_settings['lesson_video_shown'] ) {
				$complete_child_steps = ebox_user_progression_get_complete_child_steps( $this->user_id, $this->course_id, $this->step_id );
...
add_action(
	'ebox_init',
	function() {
		ebox_Course_Video::get_instance();
	}
);
 ...
 * @return bool true if complete.
 */
function ebox_video_complete_for_step( $step_id = 0, $course_id = 0, $user_id = 0 ) {
	$ld_video_instance = ebox_Course_Video::get_instance();
 ...
 * @param int $user_id   User ID.
 */
function ebox_video_delete_cookie_for_step( $step_id = 0, $course_id = 0, $user_id = 0 ) {
	$ld_video_instance = ebox_Course_Video::get_instance();
	if ( $ld_video_instance ) {
