 ...
 */
function ebox_mark_complete( $post, $atts = array() ) {
	...
	if ( isset( $_POST['ebox_mark_complete'] ) && isset( $_POST['post'] ) && intval( $_POST['post'] ) == $post->ID ) {
		return '';
	}

	$bypass_course_limits_admin_users = ebox_can_user_bypass( $user_id, 'ebox_course_progression', $post->ID, $post );
	 ...
	 */
	$bypass_course_limits_admin_users = apply_filters( 'ebox_prerequities_bypass', $bypass_course_limits_admin_users, $user_id, $post->ID, $post ); // cspell:disable-line -- prerequities are prerequisites...

	$course_id = ebox_get_course_id( $post->ID );

	if ( ( ebox_lesson_progression_enabled() ) && ( ! $bypass_course_limits_admin_users ) ) {

		if ( ! ebox_lms_has_access( $course_id, $user_id ) ) {
			return '';
		}

		// Check Course Prerequisites.
		if ( ! ebox_is_course_prerequities_completed( $course_id, $user_id ) ) { // cspell:disable-line -- prerequities are prerequisites...
			return '';
		}

		if ( ! ebox_check_user_course_points_access( $course_id, $user_id ) ) {
			return '';
		}

		$step_quiz_list = ebox_get_lesson_quiz_list( $post->ID, $user_id, $course_id );
		...
		if ( 'ebox-modules' === $post->post_type ) {
			$progress = ebox_get_course_progress( $user_id, $post->ID );

			if ( ! empty( $progress['this']->completed ) ) {
				/** This filter is documented in includes/class-ld-cpt-instance.php */
				if ( ! apply_filters( 'ebox_previous_step_completed', false, $progress['this']->ID, $user_id ) ) {
					return ebox_show_mark_incomplete( $post, $atts );
				}
			}

			if ( ! empty( $progress['prev'] ) && empty( $progress['prev']->completed ) && ebox_lesson_progression_enabled() ) {
				/** This filter is documented in includes/class-ld-cpt-instance.php */
				if ( ! apply_filters( 'ebox_previous_step_completed', false, $progress['prev']->ID, $user_id ) ) {
					return '';
				}
			}

			if ( ! ebox_lesson_topics_completed( $post->ID ) ) {
				/** This filter is documented in includes/class-ld-cpt-instance.php */
				if ( ! apply_filters( 'ebox_previous_step_completed', false, $post->ID, $user_id ) ) {
					return '';
				}
			}
		}

		if ( 'ebox-topic' === $post->post_type ) {
			$progress = ebox_get_course_progress( $user_id, $post->ID );
			...
			if ( ebox_lesson_progression_enabled() ) {
				...
				if ( ! ebox_is_previous_complete( $lesson ) ) {
					/** This filter is documented in includes/class-ld-cpt-instance.php */
					if ( ! apply_filters( 'ebox_previous_step_completed', false, $lesson->ID, $user_id ) ) {
						return '';
					}
				}
			}
		...
		$previous_item_id = ebox_user_progress_get_previous_incomplete_step( $user_id, $course_id, $post->ID );
	...
	if ( ebox_lesson_hasassignments( $post ) ) { // cspell:disable-line.
		global $ebox_assignment_upload_message;

		$ret = ebox_LMS::get_template(
			'ebox_lesson_assignment_upload_form.php',
			array(
				'course_step_post'                => $post,
				'user_id'                         => $user_id,
				'assignment_upload_error_message' => $ebox_assignment_upload_message,
			)
		);
		if ( ! is_null( $ret ) ) {
			return $ret;
		...
		if ( ( ! ebox_is_admin_user( $user_id ) ) || ( ! $bypass_course_limits_admin_users ) ) {
			$assignments = ebox_get_user_assignments( $post->ID, $user_id, $course_id );
	...
	$button_disabled = '';
	$time            = 0;
	$time_value      = ebox_forced_lesson_time( $post );

	if ( ! empty( $time_value ) ) {
		$time = ebox_convert_lesson_time_time( $time_value );
	}

	if ( ( ! ebox_is_admin_user( $user_id ) ) || ( ! $bypass_course_limits_admin_users ) ) {

		if ( ! empty( $time ) ) {
			$time_cookie_key = ebox_forced_lesson_time_cookie_key( $post );
			if ( ! empty( $time_cookie_key ) ) {
				 ...
				 * external.
				 */
				if ( ( isset( $_COOKIE[ 'ebox_timer_cookie_' . $time_cookie_key ] ) ) && ( '0' === $_COOKIE[ 'ebox_timer_cookie_' . $time_cookie_key ] ) ) {
					$time = 0;
				}
			}
	 ...
	 */
	$atts = apply_filters( 'ebox_mark_complete_form_atts', $atts, $post );
	...
	if ( isset( $atts['form']['class'] ) ) {
		$form_class = ' class="ebox-mark-complete ' . esc_attr( $atts['form']['class'] ) . '" ';
	} else {
		$form_class = ' class="ebox-mark-complete" ';
	}
	...
	if ( isset( $atts['button']['class'] ) ) {
		$button_class = ' class="ebox_mark_complete_button ' . esc_attr( $atts['button']['class'] ) . '" ';
	} else {
		$button_class = ' class="ebox_mark_complete_button" ';
	}

	$form_fields = '<input type="hidden" value="' . $post->ID . '" name="post" />
				<input type="hidden" value="' . ebox_get_course_id( $post->ID ) . '" name="course_id" />
				<input type="hidden" value="' . wp_create_nonce( 'ebox_mark_complete_' . get_current_user_id() . '_' . $post->ID ) . '" name="ebox_mark_complete" />
				<input type="submit" ' . $button_id . ' value="' . ebox_Custom_Label::get_label( 'button_mark_complete' ) . '" ' . $button_class . ' ' . $button_disabled . '/>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
	/**
	 ...
	 * @param WP_Post $post       WP_Post object being displayed.
	 */
	$form_fields = apply_filters( 'ebox_mark_complete_form_fields', $form_fields, $post );
	...
	if ( ( ! ebox_is_admin_user( $user_id ) ) || ( ! $bypass_course_limits_admin_users ) ) {
		if ( ! empty( $time ) ) {
			...
			$timer_class = ' class="ebox_timer';
			if ( isset( $atts['timer']['class'] ) ) {
				$timer_class .= ' ' . esc_attr( $atts['timer']['class'] );
			}
			$timer_class .= '" ';

			$return .= '<span ' . $timer_id . ' ' . $timer_class . ' data-timer-seconds="' . $time . '" data-button="input.ebox_mark_complete_button" data-cookie-key="' . $time_cookie_key . '"></span>';
		}
	}
	 ...
	 * @param WP_Post $post   WP_Post object being displayed.
	 */
	return apply_filters( 'ebox_mark_complete', $return, $post );
}
 ...
 */
function ebox_ajax_mark_complete( $quiz_id = null, $lesson_id = null ) {
	...
	$can_attempt_again = ebox_can_attempt_again( $user_id, $quiz_id );

	if ( $can_attempt_again ) {
		$link = ebox_next_lesson_quiz( false, $user_id, $lesson_id, null );
	} else {
		$link = ebox_next_lesson_quiz( false, $user_id, $lesson_id, array( $quiz_id ) );
	}
 ...
 */
function ebox_lesson_topics_completed( $lesson_id, $mark_lesson_complete = false ) {
	$topics = ebox_get_topic_list( $lesson_id );

	if ( empty( $topics[0]->ID ) ) {
		return true;
	}

	$progress = ebox_get_course_progress( null, $topics[0]->ID );
	...
	if ( $mark_lesson_complete ) {
		$user_id = get_current_user_id();
		ebox_process_mark_complete( null, $lesson_id );
	}
 ...
 */
function ebox_mark_complete_process( $post = null ) {
	...
	if ( ( isset( $_POST['ebox_mark_complete'] ) ) && ( ! empty( $_POST['ebox_mark_complete'] ) ) && ( isset( $_POST['post'] ) ) && ( ! empty( $_POST['post'] ) ) ) {
		...
		if ( isset( $_POST['course_id'] ) ) {
			...
			$course_id = ebox_get_course_id( $post_id );
		}
		 ...
		 * @since 2.2.1.2
		 */
		if ( ! wp_verify_nonce( $_POST['ebox_mark_complete'], 'ebox_mark_complete_' . $userid . '_' . $post_id ) ) {
			return;
		}

		$return = ebox_process_mark_complete( $userid, $post_id, false, $course_id );

		if ( $return ) {
			// Remove the lesson/topic timer cookie once the step is completed.
			$timer_cookie_key = ebox_forced_lesson_time_cookie_key( $post_id );
			if ( ! empty( $timer_cookie_key ) ) {
				if ( isset( $_COOKIE[ 'ebox_timer_cookie_' . $timer_cookie_key ] ) ) {
					unset( $_COOKIE[ 'ebox_timer_cookie_' . $timer_cookie_key ] );
				}
				// empty value and expiration one hour before.
				$res = setcookie( 'ebox_timer_cookie_' . $timer_cookie_key, '', time() - 3600 );
			}

			// Remove the lesson/topic video progress cookie once the step is completed.
			ebox_video_delete_cookie_for_step( $post_id, $course_id, $userid );

			$next_lesson_redirect = ebox_get_next_lesson_redirect();
		} else {
		...
		if ( ! empty( $next_lesson_redirect ) ) {
			 ...
			 */
			$next_lesson_redirect = apply_filters( 'ebox_completion_redirect', $next_lesson_redirect, $post_id );
			if ( ! empty( $next_lesson_redirect ) ) {
				ebox_safe_redirect( $next_lesson_redirect );
			}
		}
	}
}
add_action( 'wp', 'ebox_mark_complete_process' );
 ...
 */
function ebox_get_course_url( $id = null ) {

	if ( empty( $id ) ) {
		$id = ebox_get_course_id();
	}
 ...
 */
function ebox_get_next_lesson_redirect( $post = null ) {
	if ( empty( $post->ID ) ) {
		global $post;
	}

	$next = ebox_next_post_link( '', true, $post );
	...
	if ( ! empty( $next ) ) {
		...
		if ( 'ebox-topic' === $post->post_type ) {
			...
			$link      = ebox_next_global_quiz( true, null, $course_id );
		}
	}
	...
	if ( ! empty( $link ) ) {
		...
		$link = apply_filters( 'ebox_completion_redirect', $link, $post->ID );
		if ( ! empty( $link ) ) {
			ebox_safe_redirect( $link );
		}
	}
...
function ebox_quiz_redirect() {
	...
	if ( ! empty( $_GET['quiz_redirect'] ) && ! empty( $_GET['quiz_id'] ) && ! empty( $_GET['quiz_type'] ) && ! empty( $_GET['course_id'] ) && 'global' == $_GET['quiz_type'] ) {
		...
		if ( $can_attempt_again ) {
			$link = ebox_next_global_quiz();
		} else {
			$link = ebox_next_global_quiz( true, null, null, array( $quiz_id ) );
		}

		/** This filter is documented in includes/course/ld-course-progress.php */
		$link = apply_filters( 'ebox_completion_redirect', $link, $quiz_id );
		if ( ! empty( $link ) ) {
			ebox_safe_redirect( $link );
		}
	} else {
		...
		if ( ! empty( $_GET['quiz_redirect'] ) && ! empty( $_GET['quiz_id'] ) && ! empty( $_GET['quiz_type'] ) && ! empty( $_GET['lesson_id'] ) && 'lesson' == $_GET['quiz_type'] ) {
			...
			if ( ( 1 === $next_step ) && ( ! empty( $user_id ) ) && ( ! empty( $course_id ) ) && ( ! empty( $quiz_id ) ) ) {
				$next_incomplete_step_id = ebox_user_progress_get_next_incomplete_step( $user_id, $course_id, $quiz_id );
				if ( ! empty( $next_incomplete_step_id ) ) {
					$link = ebox_get_step_permalink( $next_incomplete_step_id, $course_id );
				}
				...
				$link = ebox_next_lesson_quiz( true, $user_id, $lesson_id, null );
				if ( empty( $link ) ) {
					$link = ebox_next_post_link( '', true );
				}

				if ( empty( $link ) ) {
					$lesson_post = get_post( $lesson_id );
					if ( 'ebox-topic' === $lesson_post->post_type ) {
						if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
							$course_id = ebox_get_course_id( $lesson_post->ID );
							$lesson    = ebox_course_get_single_parent_step( $course_id, $lesson_post->ID );
						} else {
							$lesson = ebox_get_setting( $lesson_post, 'lesson' );
						}
						$link = ebox_get_step_permalink( $lesson, $course_id );
					} else {
						$link = ebox_next_global_quiz();
					}
				}
...
add_action( 'wp', 'ebox_quiz_redirect' );
 ...
 */
function ebox_can_attempt_again( $user_id, $quiz_id ) {
	$quizmeta = get_post_meta( $quiz_id, '_ebox-quiz', true );

	if ( isset( $quizmeta['ebox-quiz_repeats'] ) ) {
		$repeats = $quizmeta['ebox-quiz_repeats'];
	} else {
		$repeats = '';
	}
	 ...
	 * @param int $quiz_id Quiz ID.
	 */
	$repeats = apply_filters( 'ebox_allowed_repeats', $repeats, $user_id, $quiz_id );

	if ( '' == $repeats ) {
		return true;
	}

	$quiz_results = get_user_meta( $user_id, '_ebox-quizzes', true );
 ...
 */
function ebox_is_previous_complete( $post ) {
	$progress = ebox_get_course_progress( null, $post->ID );
 ...
 */
function ebox_get_previous( $post ) {
	$progress = ebox_get_course_progress( null, $post->ID );
 ...
 */
function ebox_process_mark_complete( $user_id = null, $postid = null, $onlycalculate = false, $course_id = 0 ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_process_mark_complete_legacy( $user_id, $postid, $onlycalculate, $course_id );
	}
	...
	if ( ! $onlycalculate ) {
		 ...
		 */
		$process_completion = apply_filters( 'ebox_process_mark_complete', true, $post, $current_user );
	...
	if ( empty( $course_id ) ) {
		$course_id = ebox_get_course_id( $postid );
	}
	...
	$course_progress = ebox_user_get_course_progress( $user_id, $course_id, 'legacy' );
	...
	if ( ( ! isset( $course_progress['modules'] ) ) || ( empty( $course_progress['modules'] ) ) ) {
		$course_progress['modules'] = array();
	}
	...
	if ( 'ebox-topic' === $post->post_type && empty( $course_progress['topics'][ $lesson_id ] ) ) {
	...
	if ( ! $onlycalculate && 'ebox-modules' === $post->post_type && empty( $course_progress['modules'][ $postid ] ) ) {
		$course_progress['modules'][ $postid ] = 1;
		$lesson_completed                      = true;
	}

	if ( ! $onlycalculate && 'ebox-topic' === $post->post_type && empty( $course_progress['topics'][ $lesson_id ][ $postid ] ) ) {
	...
	$completed = ebox_course_get_completed_steps( $user_id, $course_id, $course_progress );

	$course_progress['completed'] = $completed;
	$course_progress['total']     = ebox_get_course_steps_count( $course_id );
	 ...
	 * @since 2.1.0
	 */
	if ( in_array( $post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) {
		$course_progress['last_id'] = $post->ID;
	}

	$course_completed_time = time();

	$course_earliest_completed_time = ebox_activity_course_get_earliest_started( $current_user->ID, $course_id, $course_completed_time );
	...
	ebox_user_set_course_progress( $user_id, $course_id, $course_progress );

	if ( ! empty( $topic_completed ) ) {
		$course_activity = ebox_activity_start_course( $current_user->ID, $course_id, $course_earliest_completed_time );
		if ( $course_activity ) {
			ebox_activity_update_meta_set(
				$course_activity->activity_id,
				array(
					'steps_completed' => ebox_course_get_completed_steps( $current_user->ID, $course_id ),
					'steps_last_id'   => $post->ID,
				)
			);
		}
		ebox_activity_start_lesson( $current_user->ID, $course_id, $lesson_id, $course_completed_time );
		ebox_activity_complete_topic( $current_user->ID, $course_id, $post->ID, $course_completed_time );
	}
	...
	if ( ! empty( $lesson_completed ) ) {
		...
		ebox_activity_complete_lesson( $current_user->ID, $course_id, $post->ID, $course_completed_time );
	}
	...
	/**
	 * ebox-5883 - Always return true if we've made it this far.
	 */
	$return = true;
 ...
 */
function ebox_update_completion( $user_id = null, $postid = null ) {
	if ( empty( $postid ) ) {
		global $post;
	...
	if ( ! empty( $postid ) ) {
		ebox_process_mark_complete( $user_id, $postid, true );
	}
}
 ...
 */
function ebox_is_quiz_complete( $user_id = null, $quiz_id = 0, $course_id = 0 ) {
	return ! ebox_is_quiz_notcomplete( $user_id, array( $quiz_id => 1 ), false, $course_id );
}
 ...
 */
function ebox_is_quiz_notcomplete( $user_id = null, $quizzes = null, $return_incomplete_quiz_ids = false, $course_id = 0 ) {
	$user_id   = (int) $user_id;
	...
	if ( ( ! empty( $quiz_results ) ) && ( is_array( $quiz_results ) ) ) {
		foreach ( $quiz_results as $quiz ) {
			...
			if ( ( -1 !== $quiz_course_id ) && ( empty( $quiz_course_id ) ) ) {
				$quiz_course_id = (int) ebox_get_course_id( intval( $quiz['quiz'] ) );
			}
			...
			if ( ! isset( $quiz['course'] ) ) {
				if ( ! ebox_is_course_shared_steps_enabled() ) {
					 ...
					 */
					$quiz['course'] = (int) ebox_get_setting( $quiz['quiz'], 'course' );
				} else {
					$quiz['course'] = ebox_get_course_id( $quiz['quiz'] );
				}
			}
			...
			if ( ( -1 === $course_id ) || ( $course_id === $quiz['course'] ) ) {
				if ( isset( $quiz['pass'] ) ) {
					...
					$passingpercentage = (int) ebox_get_setting( $quiz['quiz'], 'passingpercentage' );
 ...
 */
function ebox_get_course_progress( $user_id = null, $postid = null, $course_id = null ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_get_course_progress_legacy( $user_id, $postid, $course_id );
	}
	...
	if ( 'ebox-modules' === $this_post->post_type ) {
		$posts = $ld_course_object->get_modules( array( 'nopaging' => true ) );

		if ( empty( $course_progress ) || empty( $course_progress['modules'] ) ) {
			$completed_posts = array();
		} else {
			$completed_posts = $course_progress['modules'];
		}
	} elseif ( 'ebox-topic' === $this_post->post_type ) {
		$lesson_id = ebox_course_get_single_parent_step( $course_id, $this_post->ID );
		if ( ! $lesson_id ) {
			return null;
		}
 ...
 */
function ebox_is_lesson_complete( $user_id = null, $lesson_id = 0, $course_id = 0 ) {
	return ! ebox_is_lesson_notcomplete( $user_id, array( $lesson_id => 1 ), $course_id );
}
 ...
 *
 * @param int|null $user_id   Optional. User ID. Defaults to the current logged-in user. Default null.
 * @param array    $modules   An array of lesson IDs.
 * @param int      $course_id Optional. Course ID. Default 0.
 *
 * @return boolean Returns true if the lesson is not complete otherwise false.
 */
function ebox_is_lesson_notcomplete( $user_id = null, $modules = array(), $course_id = 0 ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_is_lesson_notcomplete_legacy( $user_id, $modules, $course_id );
	}
	...
	if ( ! empty( $modules ) ) {
		foreach ( $modules as $lesson => $v ) {
			if ( empty( $course_id ) ) {
				$course_id = ebox_get_course_id( $lesson );
			}

			if ( ! empty( $course_id ) ) {
				$course_progress = ebox_user_get_course_progress( $user_id, $course_id, 'legacy' );

				if ( ( isset( $course_progress['modules'][ $lesson ] ) ) && ( ! empty( $course_progress['modules'][ $lesson ] ) ) ) {
					unset( $modules[ $lesson ] );
				}
			}
		}
	}
 ...
 */
function ebox_is_topic_complete( $user_id = null, $topic_id = 0, $course_id = 0 ) {
	return ! ebox_is_topic_notcomplete( $user_id, array( $topic_id => 1 ), $course_id );
}
 ...
 */
function ebox_is_topic_notcomplete( $user_id = null, $topics = array(), $course_id = 0 ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_is_topic_notcomplete_legacy( $user_id, $topics, $course_id );
	}
	...
	if ( ! empty( $topics ) ) {
		foreach ( $topics as $topic_id => $v ) {
			if ( empty( $course_id ) ) {
				$course_id = ebox_get_course_id( $topic_id );
			}
			...
			if ( ! empty( $course_id ) ) {
				...
				$lesson_id = ebox_course_get_single_parent_step( $course_id, $topic_id );
				if ( ! empty( $lesson_id ) ) {
 ...
 */
function ebox_course_status( $course_id, $user_id = null, $return_slug = false ) {
	global $ebox_course_statuses;

	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_course_status_legacy( $course_id, $user_id, $return_slug );
	}
	$course_status_slug = '';
	...
	if ( in_array( $course_status_slug, array( 'in_progress', 'not_started' ), true ) ) {
		if ( ( $course_progress['total'] > 0 ) && ( $course_progress['completed'] === $course_progress['total'] ) ) {
			if ( ebox_process_mark_complete( $user_id, $course_id ) ) {
	...
	if ( true === $return_slug ) {
		...
		if ( isset( $ebox_course_statuses[ $course_status_slug ] ) ) {
			/**
			...
			return apply_filters(
				'ebox_course_status',
				ebox_course_status_label( $course_status_slug ),
				$course_id,
				$user_id,
 ...
 *
 * @global array $ebox_course_statuses An array of course status.
 *
 * @since 2.3.0
 *
 * @param string $course_status_label Optional. The current translatable text for course status. Default empty.
 *
 * @return string The index/key of the course status string if found in the `$ebox_course_statuses` global array.
 */
function ebox_course_status_idx( $course_status_label = '' ) {
	global $ebox_course_statuses;

	return array_search( $course_status_label, $ebox_course_statuses, true );
}
 ...
 * @return string|null.
 */
function ebox_course_status_label( $course_status_slug = '' ) {
	global $ebox_course_statuses;

	if ( isset( $ebox_course_statuses[ $course_status_slug ] ) ) {
		return $ebox_course_statuses[ $course_status_slug ];
	}
}
 ...
 */
function ebox_is_quiz_accessable( $user_id = null, $post = null, $return_incomplete = false, $course_id = 0 ) {

	// Allow using the legacy function in case of issues with new logic.
	if ( ( defined( 'ebox_IS_QUIZ_ACCESSABLE_LEGACY' ) && ( ebox_IS_QUIZ_ACCESSABLE_LEGACY === true ) ) ) {
		return ebox_is_quiz_accessable_legacy( $user_id, $post, $course_id );
	}
	...
	$bypass_course_limits_admin_users = ebox_can_user_bypass( $user_id, 'ebox_quiz_accessable', $post, $course_id );
	if ( true === $bypass_course_limits_admin_users ) {
		return 1;
	}

	$course_progress_co = ebox_user_get_course_progress( $user_id, $course_id, 'co' );

	if ( ebox_is_course_builder_enabled() ) {
		$quiz_parent_id = ebox_course_get_single_parent_step( $course_id, $post->ID );
	} else {
		$quiz_parent_id = ebox_get_setting( $post, 'lesson' );
	}
	...
	if ( ! empty( $quiz_parent_id ) ) {
		...
		if ( is_a( $quiz_parent_post, 'WP_Post' ) ) {
			if ( ebox_get_post_type_slug( 'topic' ) === $quiz_parent_post->post_type ) {
				$quiz_parent_topic_post  = $quiz_parent_post;
				$quiz_parent_lesson_id   = ebox_get_setting( $quiz_parent_topic_post, 'lesson' );
				$quiz_parent_lesson_post = get_post( $quiz_parent_lesson_id );

				$parent_topic_quizzes = ebox_get_lesson_quiz_list( $quiz_parent_topic_post, $user_id, $course_id );
				...
				$lesson_topics_progress = ebox_get_course_progress( $user_id, $quiz_parent_topic_post->ID );
				...
				if ( 'on' === ebox_get_setting( $quiz_parent_topic_post->ID, 'lesson_video_enabled' ) ) {
					if ( ! empty( ebox_get_setting( $quiz_parent_topic_post->ID, 'lesson_video_url' ) ) ) {
						if ( 'BEFORE' === ebox_get_setting( $quiz_parent_topic_post->ID, 'lesson_video_shown' ) ) {
							if ( ! ebox_video_complete_for_step( $quiz_parent_topic_post->ID, $course_id, $user_id ) ) {
								if ( true === $return_incomplete ) {
				...
				$lesson_progress = ebox_get_course_progress( $user_id, $quiz_parent_lesson_post->ID, $course_id );
			...
			} elseif ( ebox_get_post_type_slug( 'lesson' ) === $quiz_parent_post->post_type ) {
				$quiz_parent_lesson_post = $quiz_parent_post;
				$sibling_completed_steps = 0;

				$lesson_topics = ebox_get_topic_list( $quiz_parent_lesson_post->ID );
				if ( ! empty( $lesson_topics ) ) {
					$lesson_topics_progress = ebox_get_course_progress( $user_id, $lesson_topics[0]->ID );
					if ( ( isset( $lesson_topics_progress['posts'] ) ) && ( ! empty( $lesson_topics_progress['posts'] ) ) ) {
						foreach ( $lesson_topics_progress['posts'] as $topic ) {
							if ( empty( $topic->completed ) ) {
								if ( true === $return_incomplete ) {
				...
				$quizzes = ebox_get_lesson_quiz_list( $quiz_parent_lesson_post, $user_id, $course_id );
				if ( ! empty( $quizzes ) ) {
					// loop until we get to the first status == 'notcompleted'.
				...
				$lesson_progress = ebox_get_course_progress( $user_id, $quiz_parent_lesson_post->ID );
				if ( ( isset( $lesson_progress['posts'] ) ) && ( ! empty( $lesson_progress['posts'] ) ) ) {
					foreach ( $lesson_progress['posts'] as $lesson ) {
						if ( $lesson->ID === $quiz_parent_lesson_post->ID ) {
							if ( ! empty( $lesson->completed ) ) {
				...
				if ( empty( $sibling_completed_steps ) ) {
					if ( 'on' === ebox_get_setting( $quiz_parent_lesson_post->ID, 'lesson_video_enabled' ) ) {
						if ( ! empty( ebox_get_setting( $quiz_parent_lesson_post->ID, 'lesson_video_url' ) ) ) {
							if ( 'BEFORE' === ebox_get_setting( $quiz_parent_lesson_post->ID, 'lesson_video_shown' ) ) {
								if ( ! ebox_video_complete_for_step( $quiz_parent_lesson_post->ID, $course_id, $user_id ) ) {
									if ( true === $return_incomplete ) {
		...
		// First we check if all course modules are completed.
		$modules = ebox_get_course_modules_list( $course_id, $user_id, array( 'num' => 0 ) );
		if ( ! empty( $modules ) ) {
		...
		$quizzes = ebox_get_global_quiz_list( $course_id );
		if ( ! empty( $quizzes ) ) {
			// loop until we get to the first status == 'notcompleted'.
			foreach ( $quizzes as $quiz ) {
				if ( $quiz->ID === $post->ID ) {
					return 1;
				} elseif ( ! ebox_is_quiz_complete( $user_id, $quiz->ID, $course_id ) ) {
					if ( true === $return_incomplete ) {
						return $quiz;
					} else {
						return 0;
 ...
 * @return boolean
 */
function ebox_is_all_global_quizzes_complete( $user_id = null, $id = null ) {
	$quizzes = ebox_get_global_quiz_list( $id );
	$return  = true;

	if ( ! empty( $quizzes ) ) {
		foreach ( $quizzes as $quiz ) {
			if ( ebox_is_quiz_notcomplete( $user_id, array( $quiz->ID => 1 ), false, $id ) ) {
				$return = false;
			}
		}
	}
 ...
 */
function ebox_next_global_quiz( $url = true, $user_id = null, $id = null, $exclude = array() ) {
	if ( empty( $id ) ) {
		$id = ebox_get_course_id();
	}
	...
	if ( ! empty( $quizzes ) ) {
		foreach ( $quizzes as $quiz ) {
			if ( ! in_array( $quiz->ID, $exclude, true ) && ebox_is_quiz_notcomplete( $user_id, array( $quiz->ID => 1 ), false, $id ) && ebox_can_attempt_again( $user_id, $quiz->ID ) ) {
				if ( $url ) {
	 ...
	 * @param int        $course_id Course ID.
	 */
	$return = apply_filters( 'ebox_course_completion_url', $return, $id );
	return $return;
}
 ...
 * @return int|string The ID or the URL of the quiz.
 */
function ebox_next_lesson_quiz( $url = true, $user_id = null, $lesson_id = null, $exclude = array() ) {
	global $post;
	...
	// Assumption here is the ebox_get_lesson_quiz_list returns the quizzes in the order they should be taken.
	$quizzes = ebox_get_lesson_quiz_list( $lesson_id, $user_id );
	if ( ( ! empty( $quizzes ) ) && ( is_array( $quizzes ) ) ) {
		foreach ( $quizzes as $quiz ) {
			...
			if ( ebox_can_attempt_again( $user_id, $quiz['post']->ID ) ) {
	...
	if ( empty( $return ) ) {
		if ( ( ebox_lesson_progression_enabled( $course_id ) ) && ( ! ebox_can_complete_step( $user_id, $lesson_id, $course_id ) ) ) {
			$return = ebox_get_step_permalink( $lesson_id, $course_id );
		} elseif ( ebox_user_is_course_children_progress_complete( $user_id, $course_id, $lesson_id ) ) {
			ebox_process_mark_complete( $user_id, $lesson_id, false, $course_id );
		}
	}
 ...
 */
function ebox_can_complete_step( $user_id = 0, $step_id = 0, $course_id = 0, $ignore_lesson_timer = false ) {
	$user_id = absint( $user_id );
	if ( empty( $user_id ) ) {
		$user_id = get_current_user_id();
	}
	...
	if ( ( ! empty( $user_id ) ) && ( ! empty( $step_id ) ) && ( ! empty( $course_id ) ) ) {
		...
		if ( ! ebox_lesson_progression_enabled( $course_id ) ) {
			$incomplete_child_steps = ebox_user_progression_get_incomplete_child_steps( $user_id, $course_id, $step_id );
			if ( empty( ! $incomplete_child_steps ) ) {
				return false;
			}
		} else {
			$incomplete_step_id = ebox_user_progress_get_previous_incomplete_step( $user_id, $course_id, $step_id );
			if ( ! empty( $incomplete_step_id ) && ( $incomplete_step_id !== $step_id ) ) {
				return false;
			}
		}

		if ( in_array( get_post_type( $step_id ), ebox_get_post_type_slug( array( 'lesson', 'topic' ) ), true ) ) {

			// Check the Lesson Timer...
			if ( false === $ignore_lesson_timer ) {
				$step_timer_time = ebox_forced_lesson_time( $step_id );
				if ( ! empty( $step_timer_time ) ) {
					$time_cookie_key = ebox_forced_lesson_time_cookie_key( $step_id );
					if ( ! empty( $time_cookie_key ) ) {
			...
			if ( 'on' === ebox_get_setting( $step_id, 'lesson_assignment_upload' ) ) {
				$assignments = ebox_get_user_assignments( $step_id, $user_id, $course_id );
				if ( empty( $assignments ) ) {
					return false;
				} else {
					foreach ( $assignments as $assignment ) {
			...
			if ( ! ebox_user_is_course_children_progress_complete( $user_id, $course_id, $step_id ) ) {
				return false;
			}
		}
 ...
 */
function ebox_has_global_quizzes( $id = null ) {
	$quizzes = ebox_get_global_quiz_list( $id );
 ...
 */
function ebox_course_progress_widget( $atts ) {
	echo ebox_course_progress( $atts ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Need to output HTML
}
 ...
 */
function ebox_lesson_progression_enabled( $course_id = 0 ) {
	...
	if ( ! empty( $course_id ) ) {
		$setting = ebox_get_setting( $course_id, 'course_disable_lesson_progression' );
		 ...
		 * @param int  $course_id Course ID.
		 */
		return apply_filters( 'ebox_course_progression_enabled', empty( $setting ), $course_id );
	}
 ...
 */
function ebox_forced_lesson_time( $lesson_topic_post = '' ) {
	...
	if ( ! in_array( $lesson_topic_post->post_type, array( ebox_get_post_type_slug( 'lesson' ), ebox_get_post_type_slug( 'topic' ) ), true ) ) {
		return 0;
	}
 ...
 */
function ebox_forced_lesson_time_cookie_key( $lesson_topic_post = '' ) {
	...
	if ( 'on' === $meta[ $lesson_topic_post->post_type . '_forced_lesson_time_enabled' ] ) {
		if ( ( isset( $meta[ $lesson_topic_post->post_type . '_forced_lesson_time' ] ) ) && ( ! empty( $meta[ $lesson_topic_post->post_type . '_forced_lesson_time' ] ) ) ) {
			$cookie_key = get_current_user_id() . '_' . ebox_get_course_id( $lesson_topic_post ) . '_' . $lesson_topic_post->ID;
 ...
 */
function ebox_course_completed( $user_id, $course_id ) {
	if ( ebox_course_status( $course_id, $user_id ) == esc_html__( 'Completed', 'ebox' ) ) {
		return true;
	} else {
		return false;
	}
 ...
 */
function ebox_course_completed_store_time( $data ) {
...
add_action( 'ebox_before_course_completed', 'ebox_course_completed_store_time', 10, 1 );
 ...
 * @param int $user_id   User ID.
 */
function ebox_delete_course_progress( $course_id, $user_id ) {
	global $wpdb;
	$usermeta = get_user_meta( $user_id, '_ebox-course_progress', true );

	if ( isset( $usermeta[ $course_id ] ) ) {
		unset( $usermeta[ $course_id ] );
		update_user_meta( $user_id, '_ebox-course_progress', $usermeta );
	}
	...
	$quizzes          = array();
	$usermeta_quizzes = get_user_meta( $user_id, '_ebox-quizzes', true );
	...
	if ( ! empty( $quizzes ) ) {
		foreach ( $quizzes as $quiz_id ) {
			ebox_delete_quiz_progress( $user_id, $quiz_id );
		}
	}
}
 ...
 * @param int $quiz_id Quiz ID.
 */
function ebox_delete_quiz_progress( $user_id, $quiz_id ) {
	global $wpdb;
	...
	if ( ! empty( $usermeta ) && is_array( $usermeta ) ) {
		...
		update_user_meta( $user_id, '_ebox-quizzes', $usermeta_new );
	}

	// ProQuiz Data.
	$pro_quiz_id = ebox_get_setting( $quiz_id, 'quiz_pro' );
 ...
 */
function ebox_quiz_remove_user_statistics_by_ref( $ref_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_quiz_remove_user_toplist( $user_id = 0, $quiz_time = 0, $pro_quizid = 0 ) {
	global $wpdb;
 ...
 */
function ebox_process_mark_incomplete( $user_id = 0, $course_id = 0, $step_id = 0, $step_complete = false ) {
	$user_id = absint( $user_id );
	if ( empty( $user_id ) ) {
		return;
	}
	...
	if ( empty( $step_id ) ) {
		global $post;
		if ( ( isset( $post ) ) && ( $post instanceof WP_Post ) && ( ( in_array( $post->post_type, array( 'ebox-modules', 'ebox-topic', 'ebox-quiz' ), true ) ) ) ) {
			$step_id = $post->ID;
		} else {
			return;
		}
	...
	$course_step_parents = ebox_course_get_all_parent_step_ids( $course_id, $step_id );

	$user_course_progress = get_user_meta( $user_id, '_ebox-course_progress', true );
	if ( ! isset( $user_course_progress[ $course_id ] ) ) {
		return;
	}

	$step_post_type = get_post_type( $step_id );
	if ( 'ebox-quiz' === $step_post_type ) {
		if ( ! empty( $course_step_parents ) ) {
			if ( ( 2 == count( $course_step_parents ) ) && ( 'ebox-modules' == get_post_type( $course_step_parents[0] ) ) && ( 'ebox-topic' == get_post_type( $course_step_parents[1] ) ) ) {
				...
				if ( ( isset( $user_course_progress[ $course_id ]['topics'][ $lesson_id ][ $topic_id ] ) ) && ( true == $user_course_progress[ $course_id ]['topics'][ $lesson_id ][ $topic_id ] ) ) {
					 ...
					 * @param int $topic_id  Topic ID.
					 */
					do_action( 'ebox_mark_incomplete_process', $user_id, $course_id, $topic_id );
					...
					$topic_activity = ebox_get_user_activity( $topic_args );
					if ( $topic_activity ) {
						$topic_activity = (array) $topic_activity;

						$topic_activity['activity_status']    = false;
						$topic_activity['activity_completed'] = false;
						ebox_update_user_activity( $topic_activity );
					}
				}

				if ( ( isset( $user_course_progress[ $course_id ]['modules'][ $lesson_id ] ) ) && ( true == $user_course_progress[ $course_id ]['modules'][ $lesson_id ] ) ) {
					$user_course_progress[ $course_id ]['modules'][ $lesson_id ] = 0;
					$user_course_progress[ $course_id ]['completed']            -= 1;

					/** This filter is documented in includes/course/ld-course-progress.php */
					do_action( 'ebox_mark_incomplete_process', $user_id, $course_id, $lesson_id );
					...
					$lesson_activity = ebox_get_user_activity( $lesson_args );
					if ( $lesson_args ) {
						$lesson_args                       = (array) $lesson_args;
						$lesson_args['activity_status']    = false;
						$lesson_args['activity_completed'] = false;
						ebox_update_user_activity( $lesson_args );
					}
				}
			} elseif ( ( 1 == count( $course_step_parents ) ) && ( 'ebox-modules' == get_post_type( $course_step_parents[0] ) ) ) {
				$lesson_id = $course_step_parents[0];
	...
	} elseif ( 'ebox-topic' === $step_post_type ) {
		$step_parent_id = ebox_course_get_single_parent_step( $course_id, $step_id );
		if ( ! empty( $step_parent_id ) ) {
			if ( ( isset( $user_course_progress[ $course_id ]['topics'][ $step_parent_id ][ $step_id ] ) ) && ( true == $user_course_progress[ $course_id ]['topics'][ $step_parent_id ][ $step_id ] ) ) {
				...
				/** This filter is documented in includes/course/ld-course-progress.php */
				do_action( 'ebox_mark_incomplete_process', $user_id, $course_id, $step_id );
			...
			if ( ( isset( $user_course_progress[ $course_id ]['modules'][ $step_parent_id ] ) ) && ( true == $user_course_progress[ $course_id ]['modules'][ $step_parent_id ] ) ) {
				$user_course_progress[ $course_id ]['modules'][ $step_parent_id ] = 0;
				$user_course_progress[ $course_id ]['completed']                 -= 1;

				/** This filter is documented in includes/course/ld-course-progress.php */
				do_action( 'ebox_mark_incomplete_process', $user_id, $course_id, $step_parent_id );
	...
	} elseif ( 'ebox-modules' === $step_post_type ) {
		if ( ( isset( $user_course_progress[ $course_id ]['modules'][ $step_id ] ) ) && ( true == $user_course_progress[ $course_id ]['modules'][ $step_id ] ) ) {
			$user_course_progress[ $course_id ]['modules'][ $step_id ] = 0;
			$user_course_progress[ $course_id ]['completed']          -= 1;
	...
	if ( $user_course_progress[ $course_id ]['completed'] !== $user_course_progress[ $course_id ]['total'] ) {
		...
		/** This filter is documented in includes/course/ld-course-progress.php */
		do_action( 'ebox_mark_incomplete_process', $user_id, $course_id, $course_id );
		$course_args     = array(
			'course_id'     => $course_id,
			'user_id'       => $user_id,
			'post_id'       => $course_id,
		...
		if ( $course_args ) {
			...
			ebox_update_user_activity( $course_args );
		}
	}

	return update_user_meta( $user_id, '_ebox-course_progress', $user_course_progress );
 ...
 */
function ebox_get_user_quiz_attempt( $user_id = 0, $args = array() ) {
	if ( ( ! empty( $user_id ) ) && ( ! empty( $args ) ) ) {
		$user_quizzes = get_user_meta( $user_id, '_ebox-quizzes', true );
 ...
 */
function ebox_remove_user_quiz_attempt( $user_id = 0, $args = array() ) {
	if ( ( ! empty( $user_id ) ) && ( ! empty( $args ) ) ) {
		...
		if ( ! empty( $user_quizzes ) ) {
			...
			foreach ( $user_quizzes as $idx => $user_quiz ) {
				...
				if ( true === $match_found ) {
					 ...
					 */
					do_action( 'ebox_user_quiz_delete_single_before', $user_id, $user_quiz );

					if ( ( isset( $user_quiz['time'] ) ) && ( ! empty( $user_quiz['time'] ) ) ) {
						if ( ( isset( $user_quiz['pro_quizid'] ) ) && ( ! empty( $user_quiz['pro_quizid'] ) ) ) {
							ebox_quiz_remove_user_toplist( $user_id, $user_quiz['time'], $user_quiz['pro_quizid'] );
						}
					}
					...
					if ( ( isset( $user_quiz['statistic_ref_id'] ) ) && ( ! empty( $user_quiz['statistic_ref_id'] ) ) ) {
						ebox_quiz_remove_user_statistics_by_ref( $user_quiz['statistic_ref_id'] );
					}

					if ( ( ! isset( $user_quiz['course'] ) ) || ( empty( $user_quiz['course'] ) ) ) {
						$user_quiz['course'] = ebox_get_course_id( $user_quiz['quiz'] );
					}
					...
					$quiz_activity = ebox_get_user_activity( $quiz_args );
					if ( ! empty( $quiz_activity ) ) {
						ebox_delete_user_activity( $quiz_activity->activity_id );
					}
					 ...
					 * @param array $user_quiz User meta quiz item to be deleted.
					 */
					do_action( 'ebox_user_quiz_delete_single_after', $user_id, $user_quiz );
				}
			}
			...
			if ( true === $changes ) {
				...
				update_user_meta( $user_id, '_ebox-quizzes', $user_quizzes );
				...
				if ( ! empty( $changed_user_quizzes ) ) {
					 ...
					 */
					do_action( 'ebox_user_quiz_delete_all_after', $user_id, $changed_user_quizzes );

					foreach ( $changed_user_quizzes as $user_quiz ) {

						if ( ! ebox_is_quiz_complete( $user_id, $user_quiz['quiz'], $user_quiz['course'] ) ) {
							ebox_process_mark_incomplete( $user_id, $user_quiz['course'], $user_quiz['quiz'], false );
						}

						/**
						 * Legacy: Call the `ebox_process_mark_complete` function after the quiz entry is removed.
						 *
						 ...
						 * @param int   $user_id       User ID for the Quiz.
						 */
						if ( true === apply_filters( 'ebox_quiz_call_mark_complete_after_remove', false, $user_quiz, $user_id ) ) {
							ebox_process_mark_complete( $user_id, $user_quiz['quiz'], false, $user_quiz['course'] );
						}
					}
				}
			}
 ...
 */
function ebox_show_mark_incomplete( $post, $atts = array() ) {
	$course_mark_incomplete_enabled = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Management_Display', 'course_mark_incomplete_enabled', false );
	 ...
	 */
	if ( ! apply_filters( 'ebox_show_mark_incomplete_form', true, $course_id, $post, $atts ) ) {
		return '';
	}
	 ...
	 * @param WP_Post $post `WP_Post` object being displayed.
	 */
	$atts = apply_filters( 'ebox_mark_incomplete_form_atts', $atts, $post );
	...
	if ( isset( $atts['form']['class'] ) ) {
		$form_class = ' class="ebox-mark-incomplete ebox-mark-complete ' . esc_attr( $atts['form']['class'] ) . '" ';
	} else {
		$form_class = ' class="ebox-mark-incomplete ebox-mark-complete" ';
	}
	...
	$button_disabled = '';
	if ( isset( $atts['button']['class'] ) ) {
		$button_class = ' class="ebox_mark_incomplete_button ebox_mark_complete_button ' . esc_attr( $atts['button']['class'] ) . '" ';
	} else {
		$button_class = ' class="ebox_mark_incomplete_button ebox_mark_complete_button" ';
	}

	$button_label = ebox_Custom_Label::get_label( 'button_mark_incomplete' );
	if ( empty( $button_label ) ) {
		$button_label = esc_html__( 'Mark Incomplete', 'ebox' );
	}
	$form_fields = '<input type="hidden" value="' . $post->ID . '" name="post" />
				<input type="hidden" value="' . ebox_get_course_id( $post->ID ) . '" name="course_id" />
				<input type="hidden" value="' . wp_create_nonce( 'ebox_mark_incomplete_' . $user_id . '_' . $post->ID ) . '" name="ebox_mark_incomplete" />
 ...
 * @global WP_Post $post Global post object.
 */
function ebox_mark_incomplete_process() {
	if ( ( isset( $_POST['ebox_mark_incomplete'] ) ) && ( ! empty( $_POST['ebox_mark_incomplete'] ) ) && ( isset( $_POST['post'] ) ) && ( ! empty( $_POST['post'] ) ) ) {
		$post_id = intval( $_POST['post'] );
		 ...
		 * @since 3.1.4
		 */
		if ( ! wp_verify_nonce( $_POST['ebox_mark_incomplete'], 'ebox_mark_incomplete_' . $user_id . '_' . $post_id ) ) {
			return;
		}

		return ebox_process_mark_incomplete( $user_id, $course_id, $post_id, false );
	}
	return false;
}
add_action( 'wp', 'ebox_mark_incomplete_process' );
