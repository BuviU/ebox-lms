...
/**
 * Gets all the modules and topics for a given course ID.
 *
 ...
 */
function ebox_get_course_steps( $course_id = 0, $include_post_types = array( 'ebox-modules', 'ebox-topic' ) ) {

	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_get_course_steps_legacy( $course_id, $include_post_types );
	}
...
/**
 * Gets the total count of modules and topics for a given course ID.
 *
 * @since 2.3.0
 *
 * @param int $course_id Optional. The ID of the course. Default 0.
 ...
 */
function ebox_get_course_steps_count( $course_id = 0 ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_get_course_steps_count_legacy( $course_id );
	}

	return ebox_course_get_steps_count( $course_id );
}
 ...
 */
function ebox_course_get_completed_steps( $user_id = 0, $course_id = 0, $course_progress = array() ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_course_get_completed_steps_legacy( $user_id, $course_id, $course_progress );
	}
 ...
 */
function ebox_course_get_sections( $course_id = 0, $query_args = array() ) {
	$sections = array();
 ...
 */
function ebox_course_get_modules( $course_id = 0, $query_args = array() ) {
	$modules = array();
	...
	if ( ! empty( $course_id ) ) {
		...
		if ( ( $ld_course_object ) && ( is_a( $ld_course_object, 'LDLMS_Model_Course' ) ) ) {
			...
			$modules = $ld_course_object->get_modules( $query_args );
		}
	}
 ...
 */
function ebox_course_get_topics( $course_id = 0, $lesson_id = 0, $query_args = array() ) {
	$topics = array();
 ...
 */
function ebox_course_get_quizzes( $course_id = 0, $parent_id = 0, $query_args = array() ) {
	$quizzes = array();
 ...
 */
function ebox_course_get_steps_count( $course_id = 0 ) {
	$steps_count = 0;
 ...
 */
function ebox_course_get_all_parent_step_ids( $course_id = 0, $step_id = 0 ) {
	$step_parents = array();
	...
	if ( ( ! empty( $course_id ) ) && ( ! empty( $step_id ) ) ) {
		if ( ebox_is_course_builder_enabled() ) {
			...
			if ( ! empty( $parent_step_id ) ) {
				...
				if ( 'ebox-topic' === get_post_type( $parent_step_id ) ) {
 ...
 */
function ebox_course_get_single_parent_step( $course_id = 0, $step_id = 0, $step_type = '' ) {
	$parent_step_id = 0;
	...
	if ( ( ! empty( $course_id ) ) && ( ! empty( $step_id ) ) ) {
		if ( ebox_is_course_builder_enabled() ) {
			...
			if ( empty( $step_type ) ) {
				...
				if ( 'ebox-topic' === $step_id_post_type ) {
					$parent_step_id = get_post_meta( $step_id, 'lesson_id', true );

				} elseif ( 'ebox-quiz' === $step_id_post_type ) {
					$lesson_id      = 0;
					$topic_id       = 0;
					$parent_step_id = get_post_meta( $step_id, 'lesson_id', true );
					if ( ! empty( $parent_step_id ) ) {
						$parent_step_id_post_type = get_post_type( $parent_step_id );
						if ( 'ebox-topic' === $parent_step_id_post_type ) {
							$topic_id  = $parent_step_id;
							$lesson_id = get_post_meta( $topic_id, 'lesson_id', true );
						} elseif ( 'ebox-modules' === $parent_step_id_post_type ) {
							$lesson_id = $parent_step_id;
						}

						if ( 'ebox-modules' === $step_type ) {
							$parent_step_id = $lesson_id;
						} elseif ( 'ebox-topic' === $step_type ) {
 ...
 */
function ebox_course_get_steps_by_type( $course_id = 0, $step_type = '' ) {
	$course_steps_return = array();

	if ( ( ! empty( $course_id ) ) && ( ! empty( $step_type ) ) ) {
		$ld_course_steps_object = LDLMS_Factory_Post::course_steps( intval( $course_id ) );
		if ( $ld_course_steps_object ) {
			if ( in_array( $step_type, ebox_get_post_types( 'course_steps' ), true ) ) {
 ...
 */
function ebox_course_get_children_of_step( $course_id = 0, $step_id = 0, $child_type = '', $return_type = 'ids', $nested = false ) {
 ...
 */
function ebox_get_courses_for_step( $step_id = 0, $return_flat_array = false ) {
	global $wpdb;
	...
	if ( ! empty( $step_id ) ) {
		...
		// ebox-6567 : If shared steps is not enabled then clear the array node.
		...
		if ( ! empty( $course_ids['primary'] ) ) {
			...
			foreach ( $course_ids['secondary'] as $s_course_id => $s_course_title ) {
				...
				ebox_update_setting( $step_id, 'course', $s_course_id );
				break;
			}
		}
 ...
 */
function ebox_check_primary_course_for_step( $step_id = 0 ) {
	$step_id = absint( $step_id );
	if ( ( ! empty( $step_id ) ) && ( ebox_is_course_shared_steps_enabled() ) ) {
		if ( in_array( get_post_type( $step_id ), array( ebox_get_post_type_slug( 'lesson' ), ebox_get_post_type_slug( 'topic' ), ebox_get_post_type_slug( 'quiz' ) ), true ) ) {
			$course_id = ebox_get_primary_course_for_step( $step_id );
			if ( empty( $course_id ) ) {
				$post_courses = ebox_get_courses_for_step( $step_id );
				if ( ( isset( $post_courses['secondary'] ) ) && ( ! empty( $post_courses['secondary'] ) ) ) {
					foreach ( $post_courses['secondary'] as $course_id => $course_title ) {
						ebox_set_primary_course_for_step( $step_id, $course_id );
						break;
					}
				}
			}
 ...
 */
function ebox_get_primary_course_for_step( $step_id = 0 ) {
	$course_id = null;
	$step_id   = absint( $step_id );
	if ( ! empty( $step_id ) ) {
		$course_id = get_post_meta( $step_id, 'course_id', true );
		if ( empty( $course_id ) ) {
			$step_courses = ebox_get_courses_for_step( $step_id );
 ...
 * @param integer $course_id Course ID.
 */
function ebox_set_primary_course_for_step( $step_id = 0, $course_id = 0 ) {
	$step_id   = absint( $step_id );
	$course_id = absint( $course_id );
	...
	if ( ( ! empty( $step_id ) ) && ( ! empty( $course_id ) ) ) {
		...
		if ( ( ! isset( $step_courses['primary'][ $course_id ] ) ) && ( isset( $step_courses['secondary'][ $course_id ] ) ) ) {
			ebox_update_setting( $step_id, 'course', $course_id );
		}
	}
}
 ...
 */
function ebox_check_course_step( $wp ) {
	if ( is_single() ) {
		global $post;
		if ( ( in_array( $post->post_type, array( 'ebox-modules', 'ebox-topic', 'ebox-quiz' ), true ) === true ) && ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) ) ) {
			...
			if ( ! empty( $course_slug ) ) {
				...
				if ( ( ! empty( $course_post ) ) && ( is_a( $course_post, 'WP_Post' ) ) && ( 'ebox-courses' === $course_post->post_type ) ) {
					$step_courses = ebox_get_courses_for_step( $post->ID, true );
					if ( ( ! empty( $step_courses ) ) && ( isset( $step_courses[ $course_post->ID ] ) ) ) {

						if ( in_array( $post->post_type, array( 'ebox-topic', 'ebox-quiz' ), true ) === true ) {

							$parent_steps = ebox_course_get_all_parent_step_ids( $course_post->ID, $post->ID );

							if ( 'ebox-quiz' === $post->post_type ) {
								$topic_slug = get_query_var( 'ebox-topic' );
								if ( ! empty( $topic_slug ) ) {
									$topic_post = ebox_get_page_by_path( $topic_slug, 'ebox-topic' );
									if ( ( ! empty( $topic_post ) ) && ( is_a( $topic_post, 'WP_Post' ) ) && ( 'ebox-topic' === $topic_post->post_type ) ) {
										if ( ! in_array( $topic_post->ID, $parent_steps, true ) ) {
											$course_link = get_permalink( $course_post->ID );
											if ( ! empty( $course_link ) ) {
												ebox_safe_redirect( $course_link );
											}
										}
									} else {
										$course_link = get_permalink( $course_post->ID );
								...
								$lesson_slug = get_query_var( 'ebox-modules' );
								if ( ! empty( $lesson_slug ) ) {
									$lesson_post = ebox_get_page_by_path( $lesson_slug, 'ebox-modules' );
									if ( ( ! empty( $lesson_post ) ) && ( is_a( $lesson_post, 'WP_Post' ) ) && ( 'ebox-modules' === $lesson_post->post_type ) ) {
							...
							} elseif ( 'ebox-topic' === $post->post_type ) {
								$lesson_slug = get_query_var( 'ebox-modules' );
								if ( ! empty( $lesson_slug ) ) {
									$lesson_post = ebox_get_page_by_path( $lesson_slug, 'ebox-modules' );
									if ( ( ! empty( $lesson_post ) ) && ( is_a( $lesson_post, 'WP_Post' ) ) && ( 'ebox-modules' === $lesson_post->post_type ) ) {
				...
				if ( ebox_is_admin_user() ) {
					...
					$step_courses = ebox_get_courses_for_step( $post->ID, false );

					// If we do have a primary (legacy) then we redirect the user there.
					if ( ! empty( $step_courses['primary'] ) ) {
						$primary_courses = array_keys( $step_courses['primary'] );
						$step_permalink  = ebox_get_step_permalink( $post->ID, $primary_courses[0] );
						if ( ! empty( $step_permalink ) ) {
							ebox_safe_redirect( $step_permalink );
						} else {
							$courses_archive_link = get_post_type_archive_link( 'ebox-courses' );
						...
						if ( ebox_is_admin_user() ) {
							...
							return;
						} elseif ( ( 'ebox-quiz' === $post->post_type ) && ( empty( $step_courses['secondary'] ) ) ) {
							// If here we have a quiz with no primary or secondary courses. So it is standalone and allowed.
							return;
						} else {
							$courses_archive_link = get_post_type_archive_link( 'ebox-courses' );
							if ( ! empty( $courses_archive_link ) ) {
								ebox_safe_redirect( $courses_archive_link );
							}
						}
					}
				}
...
add_action( 'wp', 'ebox_check_course_step' );
 ...
 */
function ebox_transition_course_step_post_status( $new_status, $old_status, $post ) {
	global $wpdb;

	if ( $new_status !== $old_status ) {
		if ( ( $post ) && ( is_a( $post, 'WP_Post' ) ) && ( in_array( $post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) ) {
			...
			if ( ! empty( $course_ids ) ) {
				...
				foreach ( $course_ids as $course_id ) {
					ebox_course_set_steps_dirty( $course_id );
				}
			}
		}
	}
}
add_action( 'transition_post_status', 'ebox_transition_course_step_post_status', 10, 3 );
 ...
 * @param integer $course_id Course ID.
 */
function ebox_course_set_steps_dirty( $course_id = 0 ) {
	$course_id = absint( $course_id );
	if ( ! empty( $course_id ) ) {
 ...
 */
function ebox_get_step_post_statuses() {
	...
	$ld_post_statuses['password'] = esc_html_x( 'Password', 'Password Protected post_status label', 'ebox' );
	 ...
	 */
	return apply_filters( 'ebox_course_steps_post_statuses', $ld_post_statuses );
}
 ...
 * @return string
 */
function ebox_get_step_post_status_slug( $post ) {
 ...
 * @return string
 */
function ebox_get_step_post_status_label( $post ) {
	$post_status_label = '';
	if ( ( $post ) && ( is_a( $post, 'WP_Post' ) ) ) {
		$post_status_slug = ebox_get_step_post_status_slug( $post );
		$post_statuses    = ebox_get_step_post_statuses();
 ...
 */
function ebox_format_step_post_title_with_status_label( $post, $skip_statuses = array( 'publish' ), $before_label = '(', $after_label = ')' ) {
	$post_title = '';
	...
	if ( is_a( $post, 'WP_Post' ) ) {
		...
		if ( ! in_array( $post_status_slug, $skip_statuses, true ) ) {
			...
			if ( isset( $post_statuses[ $post_status_slug ] ) ) {
				...
				$post_status_label = esc_html( $before_label ) . esc_html__( 'Unknown', 'ebox' ) . esc_html( $after_label );
			}

			if ( ! empty( $post_status_label ) ) {
				$post_title = sprintf(
					// translators: placeholder: post title, post status.
					esc_html_x( '%1$s %2$s', 'placeholder: post title, post status', 'ebox' ),
					$post_title,
 ...
 * ebox. But this function will also be triggered when adding
 * a new step post within ebox.
 *
 * @since 3.4.0.3
 *
 ...
 */
function ebox_new_step_insert( $post_id, $post, $update ) {
	$post_id = absint( $post_id );
	if ( ( ! empty( $post_id ) ) && ( ! $update ) && ( $post ) && ( is_a( $post, 'WP_Post' ) ) && ( in_array( $post->post_type, ebox_get_post_types( 'course_steps' ), true ) ) ) {
		$ebox_course_dirty = get_option( 'ebox_course_dirty', array() );
		if ( ! is_array( $ebox_course_dirty ) ) {
			$ebox_course_dirty = array();
		}
		$ebox_course_dirty[] = $post_id;
		update_option( 'ebox_course_dirty', $ebox_course_dirty );
	}
}
add_action( 'wp_insert_post', 'ebox_new_step_insert', 30, 3 );

/**
 * Handler function to post-process new inserted steps from ebox_insert_new_step.
 *
 * @since 3.4.0.3
 */
function ebox_new_step_init() {
	$ebox_course_dirty = get_option( 'ebox_course_dirty', array() );
	if ( ! is_array( $ebox_course_dirty ) ) {
		$ebox_course_dirty = array();
	}

	while ( ! empty( $ebox_course_dirty ) ) {
		foreach ( $ebox_course_dirty as $post_id ) {
			if ( in_array( get_post_type( $post_id ), ebox_get_post_types( 'course_steps' ), true ) ) {
			...
			$ebox_course_dirty = array_diff( $ebox_course_dirty, array( $post_id ) );
			break;
		}
	}
	update_option( 'ebox_course_dirty', $ebox_course_dirty );
}
add_action( 'ebox_init', 'ebox_new_step_init', 30 );
 ...
 */
function ebox_course_steps_update_post_meta( $meta_id = 0, $object_id = '', $meta_key = '', $meta_value = '' ) {
	...
	if ( ( ! empty( $object_id ) ) && ( ! empty( $meta_key ) ) ) {
		...
		if ( 'course_id' === $meta_key ) {
			if ( in_array( get_post_type( $object_id ), ebox_get_post_types( 'course_steps' ), true ) ) {
				$in_process = true;
				...
				if ( $course_id_current !== $course_id_new ) {
					if ( ! empty( $course_id_current ) ) {
						ebox_course_set_steps_dirty( $course_id_current );
					}

					if ( ! empty( $course_id_new ) ) {
						ebox_course_set_steps_dirty( $course_id_new );
					}
				}
...
add_action( 'update_post_meta', 'ebox_course_steps_update_post_meta', 20, 4 );
 ...
 * @param mixed      $meta_value Optional. Meta value. Default empty.
 */
function ebox_course_steps_delete_post_meta( $meta_id = 0, $object_id = '', $meta_key = '', $meta_value = '' ) {
	// Simple static var to prevent recursive calls.
	static $in_process = false;
	if ( true === $in_process ) {
		return;
...
add_action( 'delete_post_meta', 'ebox_course_steps_delete_post_meta', 20, 4 );
 ...
 * @param mixed      $meta_value Optional. Meta value. Default empty.
 */
function ebox_course_steps_add_post_meta( $object_id, $meta_key, $meta_value ) {
	// Simple static var to prevent recursive calls.
	static $in_process = false;
	if ( true === $in_process ) {
		return;
...
add_action( 'add_post_meta', 'ebox_course_steps_add_post_meta', 20, 3 );
