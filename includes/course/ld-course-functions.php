 ...
 */
function ebox_get_course_id( $id = null, $bypass_cb = false ) {
	...
	if ( empty( $id ) ) {
		if ( ! defined( 'REST_REQUEST' ) ) {
			if ( is_admin() ) {
				...
				if ( ( ! in_array( $pagenow, array( 'post.php', 'post-new.php' ), true ) ) || ( ! in_array( $post_type, ebox_get_post_types( 'course' ), true ) ) ) {
					return false;
				}
	...
	if ( ebox_get_post_type_slug( 'course' ) === $p->post_type ) {
		return $p->ID;
	}

	// Somewhat a kludge. Here we try and assume the course_id being handled.
	if ( ( ebox_is_course_shared_steps_enabled() ) && ( false === $bypass_cb ) ) {
		if ( ! is_admin() ) {
			$course_slug = get_query_var( 'ebox-courses' );
			if ( ! empty( $course_slug ) ) {
				$course_post = ebox_get_page_by_path( $course_slug, 'ebox-courses' );
		...
		if ( ( isset( $_GET['course_id'] ) ) && ( ! empty( $_GET['course_id'] ) ) ) {
			...
			if ( get_post_type( intval( $_GET['post'] ) ) == 'ebox-courses' ) {
 ...
 */
function ebox_get_lesson_id( $post_id = null, $course_id = null ) {
	...
	if ( ( ! $post ) || ( ! is_a( $post, 'WP_Post' ) ) || ( ! in_array( $post->post_type, ebox_get_post_types( 'course' ), true ) ) ) {
		return false;
	}
	...
	if ( ebox_is_course_shared_steps_enabled() ) {
		$lesson_slug = get_query_var( ebox_get_post_type_slug( 'lesson' ) );
		if ( ! empty( $lesson_slug ) ) {
			$lesson_post = ebox_get_page_by_path( $lesson_slug, ebox_get_post_type_slug( 'lesson' ) );
			...
			if ( ! empty( $course_id ) ) {
				return ebox_course_get_single_parent_step( $course_id, $post->ID );
			}
		}
	} else {
		if ( in_array( $post->post_type, ebox_get_post_type_slug( array( 'topic', 'quiz' ) ), true ) ) {
 ...
 */
function ebox_is_course_prerequities_completed( $post_id = 0, $user_id = 0 ) {
	...
	if ( ! empty( $post_id ) ) {
		...
		if ( ( ! empty( $course_id ) ) && ( ! empty( $user_id ) ) && ( ebox_get_course_prerequisite_enabled( $course_id ) ) ) {
			$course_pre = ebox_get_course_prerequisites( $course_id, $user_id );
			if ( ! empty( $course_pre ) ) {
				$course_pre_compare = ebox_get_course_prerequisite_compare( $course_id );
 ...
 */
function ebox_get_course_prerequisites( $post_id = 0, $user_id = 0 ) {
	...
	if ( ! empty( $post_id ) ) {
		...
		if ( ( ! empty( $course_id ) ) && ( ! empty( $user_id ) ) && ( ebox_get_course_prerequisite_enabled( $course_id ) ) ) {
			...
			$course_pre = ebox_get_course_prerequisite( $course_id );
			if ( ! empty( $course_pre ) ) {
				$course_pre_compare = ebox_get_course_prerequisite_compare( $course_id );
				...
				foreach ( $course_pre as $c_id ) {
					...
					$course_status = ebox_course_status( $c_id, $user_id, true );
 ...
 */
function ebox_get_course_prerequisite( $course_id = 0 ) {
	$course_pre = array();

	if ( ! empty( $course_id ) ) {
		$transient_key        = 'ebox_course_pre_' . $course_id;
		...
		if ( false !== $course_pre_transient ) {
			...
			$course_pre = ebox_get_setting( $course_id, 'course_prerequisite' );
			if ( empty( $course_pre ) ) {
				$course_pre = array();
			}
			...
			if ( ! empty( $course_pre ) ) {
				$post_status = ebox_get_step_post_statuses();
 ...
 */
function ebox_set_course_prerequisite( $course_id = 0, $course_pre = array() ) {
	if ( ! empty( $course_id ) ) {
		...
		return ebox_update_setting( $course_id, 'course_prerequisite', $course_pre );
	}
 ...
 */
function ebox_get_course_prerequisite_enabled( $course_id ) {
	$course_pre_enabled = false;

	$course_id = ebox_get_course_id( $course_id );
	if ( ! empty( $course_id ) ) {
		$course_pre_enabled = ebox_get_setting( $course_id, 'course_prerequisite_enabled' );
		if ( 'on' === $course_pre_enabled ) {
			$course_pre_courses = ebox_get_setting( $course_id, 'course_prerequisite' );
 ...
 */
function ebox_set_course_prerequisite_enabled( $course_id, $enabled = true ) {
	if ( true === $enabled ) {
		$enabled = 'on';
	}
	...
	return ebox_update_setting( $course_id, 'course_prerequisite_enabled', $enabled );
}
 ...
 */
function ebox_get_course_prerequisite_compare( $post_id ) {
	...
	if ( ! empty( $post_id ) ) {
		$course_id = ebox_get_course_id( $post_id );
		if ( ! empty( $course_id ) ) {
			$course_prerequisite_compare = ebox_get_setting( $course_id, 'course_prerequisite_compare' );
 ...
 */
function ebox_get_course_points_enabled( $post_id = 0 ) {
	$course_points_enabled = false;

	if ( ! empty( $post_id ) ) {
		$course_id = ebox_get_course_id( $post_id );
		if ( ! empty( $course_id ) ) {
			$course_points_enabled = ebox_get_setting( $course_id, 'course_points_enabled' );
 ...
 */
function ebox_get_course_points( $post_id = 0, $decimals = 1 ) {
	$course_points = false;

	if ( ! empty( $post_id ) ) {
		$course_id = ebox_get_course_id( $post_id );
		if ( ! empty( $course_id ) ) {
			if ( ebox_get_course_points_enabled( $course_id ) ) {
				$course_points = 0;

				$course_points = ebox_get_setting( $course_id, 'course_points' );
				if ( ! empty( $course_points ) ) {
					$course_points = ebox_format_course_points( $course_points, $decimals );
				}
			}
		}
	}
 ...
 * @return int|false Returns false if the course points are disabled otherwise returns course points.
 */
function ebox_get_course_points_access( $post_id = 0 ) {
	...
	if ( ! empty( $post_id ) ) {
		...
		if ( ! empty( $course_id ) ) {
			if ( ebox_get_course_points_enabled( $course_id ) ) {
				...
				$course_points_access = ebox_format_course_points( ebox_get_setting( $course_id, 'course_points_access' ) );
			}
		}
	}
 ...
 */
function ebox_check_user_course_points_access( $post_id, $user_id = 0 ) {
	...
	if ( ! empty( $post_id ) ) {
		...
		if ( ( ! empty( $course_id ) ) && ( ! empty( $user_id ) ) ) {
			if ( ebox_get_course_points_enabled( $course_id ) ) {
				$course_access_points = ebox_get_course_points_access( $course_id );

				if ( ! empty( $course_access_points ) ) {
					$user_course_points = ebox_get_user_course_points( $user_id );
 ...
 * @since 2.1.0
 */
function ebox_process_course_join() {
	$user_id = get_current_user_id();
	...
	if ( ( isset( $_POST['course_join'] ) ) && ( isset( $_POST['course_id'] ) ) ) {
		...
		$post              = get_post( $post_id );
		if ( ( ! $post ) || ( ! is_a( $post, 'WP_Post' ) ) || ( ebox_get_post_type_slug( 'course' ) !== $post->post_type ) ) {
			return;
		}
	} elseif ( ( isset( $_POST['team_join'] ) ) && ( isset( $_POST['team_id'] ) ) ) {
		$post_label_prefix = 'team';
		$post_id           = intval( $_POST['team_id'] );
		$post              = get_post( $post_id );
		if ( ( ! $post ) || ( ! is_a( $post, 'WP_Post' ) ) || ( ebox_get_post_type_slug( 'team' ) !== $post->post_type ) ) {
			return;
		}
	} else {
		return;
	...
	if ( empty( $user_id ) ) {
		 ...
		 * @param int    $post_id Course or Team ID.
		 */
		$login_url = apply_filters( 'ebox_' . $post_label_prefix . '_join_redirect', $login_url, $post_id );
		if ( ! empty( $login_url ) ) {
			ebox_safe_redirect( $login_url );
		}
	}
	...
	$settings = ebox_get_setting( $post_id );

	if ( ebox_get_post_type_slug( 'team' ) === get_post_type( $post_id ) ) {
		if ( ! isset( $settings['team_price_type'] ) ) {
			if ( ! defined( 'ebox_DEFAULT_GROUP_PRICE_TYPE' ) ) {
				$settings['team_price_type'] = ebox_DEFAULT_GROUP_PRICE_TYPE;
			} else {
				$settings['team_price_type'] = '';
			}
		}

		if ( 'free' === $settings['team_price_type'] || 'paynow' === $settings['team_price_type'] && empty( $settings['team_price'] ) && ! empty( $_POST['team_join'] ) || ebox_is_user_in_team( $user_id, $post_id ) ) {
			ld_update_team_access( $user_id, $post_id );
		}
	} elseif ( ebox_get_post_type_slug( 'course' ) === get_post_type( $post_id ) ) {
		if ( ! isset( $settings['course_price_type'] ) ) {
			if ( ! defined( 'ebox_DEFAULT_COURSE_PRICE_TYPE' ) ) {
				$settings['course_price_type'] = ebox_DEFAULT_COURSE_PRICE_TYPE;
			} else {
				$settings['course_price_type'] = '';
			}
		}

		if ( 'free' === $settings['course_price_type'] || 'paynow' === $settings['course_price_type'] && empty( $settings['course_price'] ) && ! empty( $settings['course_join'] ) || ebox_lms_has_access( $post_id, $user_id ) ) {
			ld_update_course_access( $user_id, $post_id );
		}
	}
}

add_action( 'wp', 'ebox_process_course_join' );

/**
 * Gets all the courses with the price type open.
 *
 * Logic for this query was taken from the `ebox_lms_has_access_fn()` function
 *
 * @since 2.3.0
 *
 ...
 */
function ebox_get_open_courses( $bypass_transient = false ) {
	return ebox_get_posts_by_price_type( ebox_get_post_type_slug( 'course' ), 'open', $bypass_transient );
}
 ...
 * @return array An array of course IDs.
 */
function ebox_get_paynow_courses( $bypass_transient = false ) {
	return ebox_get_posts_by_price_type( ebox_get_post_type_slug( 'course' ), 'paynow', $bypass_transient );
}
 ...
 */
function ebox_get_course_expired_access_from_meta( $course_id = 0 ) {
	global $wpdb;
	...
	if ( ! empty( $course_id ) ) {
		$expired_user_ids = $wpdb->get_col(
			$wpdb->prepare(
				...
				'ebox_course_expired_' . $course_id
			)
		);
	}
 ...
 * @since 2.6.4
 *
 * @TODO Need to convert all references to get_post_meta for '_ebox-courses' to use this function.
 *
 * @param int    $course_id   Optional. The ID of the course. Default 0.
 ...
 */
function ebox_get_course_meta_setting( $course_id = 0, $setting_key = '' ) {
	...
	$meta = get_post_meta( $course_id, '_ebox-courses', true );
	...
	if ( ( empty( $setting_key ) ) || ( 'course_access_list' === $setting_key ) ) {
		if ( ! isset( $meta['ebox-courses_course_access_list'] ) ) {
			$meta['ebox-courses_course_access_list'] = '';
		}

		if ( ! empty( $meta['ebox-courses_course_access_list'] ) ) {
			if ( is_string( $meta['ebox-courses_course_access_list'] ) ) {
				$meta['ebox-courses_course_access_list'] = array_map( 'absint', explode( ',', $meta['ebox-courses_course_access_list'] ) );
			} elseif ( is_array( $meta['ebox-courses_course_access_list'] ) ) {
				$meta['ebox-courses_course_access_list'] = array_map( 'absint', $meta['ebox-courses_course_access_list'] );
			} else {
				// Not sure how we can get here. Just in case.
				$meta['ebox-courses_course_access_list'] = array();
			}
		} else {
			$meta['ebox-courses_course_access_list'] = array();
		}
		...
		$meta['ebox-courses_course_access_list'] = array_diff( $meta['ebox-courses_course_access_list'], array( 0, '' ) );
	}

	if ( empty( $setting_key ) ) {
		return $meta;
	} elseif ( isset( $meta[ 'ebox-courses_' . $setting_key ] ) ) {
		return $meta[ 'ebox-courses_' . $setting_key ];
	}
}

add_filter(
	'ebox-courses_display_options',
	function( $options, $location ) {
		if ( ( ! isset( $options[ $location . '_course_prerequisite_enabled' ] ) ) || ( empty( $options[ $location . '_course_prerequisite_enabled' ] ) ) ) {
			...
			if ( $post instanceof WP_Post ) {
				$settings = get_post_meta( $post->ID, '_ebox-courses', true );
				...
				if ( ( isset( $settings[ $location . '_course_prerequisite' ] ) ) && ( ! empty( $settings[ $location . '_course_prerequisite' ] ) ) ) {
					...
					update_post_meta( $post->ID, '_ebox-courses', $settings );
				}
			}
		}
...
/**
 * Updates the users team course access.
 *
 * Fires on `ebox_update_course_access` hook.
 *
 * @since 2.4.0
 *
 ...
 * @param boolean $remove      Whether to user team from course access.
 */
function ebox_update_course_users_teams( $user_id, $course_id, $access_list, $remove ) {
	if ( ( ! empty( $user_id ) ) && ( ! empty( $course_id ) ) && ( true !== (bool) $remove ) ) {

		$teams = ebox_get_course_teams( $course_id, true );
		if ( ! empty( $teams ) ) {
			foreach ( $teams as $team_id ) {
				$ld_auto_enroll_team_courses = get_post_meta( $team_id, 'ld_auto_enroll_team_courses', true );
				/**
				 * See settings in includes/settings/settings-metaboxes/class-ld-settings-metabox-team-courses-enroll.php
				 * If the checkbox is set then ALL courses can be used to enroll into team.
				 */
				if ( 'yes' === $ld_auto_enroll_team_courses ) {
					/**
					 * Filters whether to enroll into team for the course.
					 *
					 * @since 3.2.0
					 *
					 * @param boolean $enroll_in_team Whether to enroll the user into the team.
					 * @param integer $team_id        The Team ID.
					 * @param integer $course_id       The Course ID.
					 */
					if ( apply_filters( 'ebox_team_course_auto_enroll', true, $team_id, $course_id ) ) {
						ld_update_team_access( $user_id, $team_id );
					}
				} else {
					/**
					...
					if ( ( is_array( $ld_auto_enroll_team_course_ids ) ) && ( ! empty( $ld_auto_enroll_team_course_ids ) ) ) {
						$ld_auto_enroll_team_course_ids = array_map( 'absint', $ld_auto_enroll_team_course_ids );

						if ( in_array( $course_id, $ld_auto_enroll_team_course_ids, true ) ) {
...
add_action( 'ebox_update_course_access', 'ebox_update_course_users_teams', 50, 4 );
 ...
 */
function ebox_user_get_course_completed_date( $user_id = 0, $course_id = 0 ) {
	...
	if ( ( ! empty( $user_id ) ) && ( ! empty( $course_id ) ) ) {
		...
		if ( empty( $completed_on_timestamp ) ) {
			...
			$activity = ebox_reports_get_activity( $activity_query_args );
 ...
 */
function ebox_get_page_by_path( $slug = '', $post_type = '' ) {
	$course_post = null;
...
/**
 * Gets the course modules per page setting.
 *
 * This function will initially source the per_page from the course. But if we are using the
 * default lesson options setting we will use that. Then if the modules options
 ...
 */
function ebox_get_course_modules_per_page( $course_id = 0 ) {
	$course_modules_per_page = 0;

	// From the WP > Settings > Reading > Posts per page.
	$course_modules_per_page = (int) get_option( 'posts_per_page' );

	// From the ebox > Settings > General > Global Pagination Settings > Shortcodes & Widgets per page.
	$course_modules_per_page = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'per_page', $course_modules_per_page );

	// From the ebox > Courses > Settings > Global Course Management > Course Table Pagination > modules per page.
	$course_settings = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Courses_Management_Display' );
	if ( ( isset( $course_settings['course_pagination_enabled'] ) ) && ( 'yes' === $course_settings['course_pagination_enabled'] ) ) {
		if ( isset( $course_settings['course_pagination_modules'] ) ) {
			$course_modules_per_page = $course_settings['course_pagination_modules'];
		} elseif ( isset( $course_settings['posts_per_page'] ) ) {
			$course_modules_per_page = $course_settings['posts_per_page'];
		}
	}

	// From the specific Course Settings > Custom Pagination.
	if ( ! empty( $course_id ) ) {
		$course_settings = ebox_get_setting( intval( $course_id ) );
		if ( ( isset( $course_settings['course_lesson_per_page'] ) ) && ( 'CUSTOM' === $course_settings['course_lesson_per_page'] ) && ( isset( $course_settings['course_lesson_per_page_custom'] ) ) ) {
			$course_modules_per_page = $course_settings['course_lesson_per_page_custom'];
		}
	}

	return absint( $course_modules_per_page );
}

/**
 * Called from within the Course modules List processing query ebox_CPT::loop_shortcode.
...
/**
 * Handles the course modules list pager.
 *
 * @since 2.5.5
 *
 * Fires on `ebox_course_modules_list_pager` hook.
 *
 ...
 */
function ebox_course_modules_list_pager( $query_result = null, $pager_context = '' ) {
	global $course_pager_results;
...
add_action( 'ebox_course_modules_list_pager', 'ebox_course_modules_list_pager', 10, 2 );
 ...
 */
function ebox_get_lesson_topic_paged_values() {
	$paged_values = array(
		'lesson' => 0,
		'paged'  => 1,
	);
 ...
 */
function ebox_process_lesson_topics_pager( $topics = array(), $args = array() ) {
	global $course_pager_results;
	...
	if ( ! empty( $topics ) ) {
		if ( ! isset( $args['per_page'] ) ) {
			$topics_per_page = ebox_get_course_topics_per_page( $args['course_id'], $args['lesson_id'] );
		} else {
		...
		if ( ( $topics_per_page > 0 ) && ( count( $topics ) > $topics_per_page ) ) {
			...
			} elseif ( get_post_type() === ebox_get_post_type_slug( 'topic' ) ) {
				/**
			...
			} elseif ( get_post_type() === ebox_get_post_type_slug( 'quiz' ) ) {
				$parent_step_ids = ebox_course_get_all_parent_step_ids( $args['course_id'], get_the_ID() );
				if ( ! empty( $parent_step_ids ) ) {
					$parent_step_ids = array_map( 'absint', $parent_step_ids );
					$parent_step_ids = array_reverse( $parent_step_ids );

					if ( get_post_type( $parent_step_ids[0] ) === ebox_get_post_type_slug( 'topic' ) ) {
					...
					} elseif ( get_post_type( $parent_step_ids[0] ) === ebox_get_post_type_slug( 'lesson' ) ) {
						/**
...
/**
 * Gets the course modules order query arguments.
 *
 * The course modules order can be set in the course or globally defined in
 ...
 * @param int $course_id Optional. The ID of the course. Default 0.
 *
 * @return array An array of course modules order query arguments.
 */
function ebox_get_course_modules_order( $course_id = 0 ) {
	...
	if ( ! empty( $course_id ) ) {
		...
		if ( ( isset( $course_settings['course_lesson_order'] ) ) && ( ! empty( $course_settings['course_lesson_order'] ) ) ) {
			$course_modules_args['order'] = $course_settings['course_lesson_order'];
		}

		if ( ( isset( $course_settings['course_lesson_orderby'] ) ) && ( ! empty( $course_settings['course_lesson_orderby'] ) ) ) {
			$course_modules_args['orderby'] = $course_settings['course_lesson_orderby'];
		}
	}
	 ...
	 */
	return apply_filters( 'ebox_course_modules_order', $course_modules_args, $course_id );
}
 ...
 */
function ebox_convert_course_access_list( $course_access_list = '', $return_array = false ) {
 ...
 */
function ebox_get_course_topics_per_page( $course_id = 0, $lesson_id = 0 ) {
	...
	// From the ebox > Settings > General > Global Pagination Settings > Shortcodes & Widgets per page.
	$course_topics_per_page = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Per_Page', 'per_page', $course_topics_per_page );
 ...
 */
function ebox_use_legacy_course_access_list() {
	$use_legacy_course_access_list = true;

	$data_course_access_convert = ebox_data_upgrades_setting( 'course-access-lists-convert' );
	 ...
	 */
	return apply_filters( 'ebox_use_legacy_course_access_list', $use_legacy_course_access_list );
}
 ...
 */
function ebox_get_last_active_course( $user_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_user_course_last_step( $user_id = 0, $course_id = 0 ) {
	global $wpdb;
	...
	if ( ! empty( $user_id ) ) {
		if ( empty( $course_id ) ) {
			$course_id = ebox_get_last_active_course( $user_id );
		}
		if ( ! empty( $course_id ) ) {
			$query_result        = $wpdb->get_var(
				$wpdb->prepare(
 ...
 */
function ebox_can_user_bypass( $user_id = 0, $context = 'ebox_course_progression', $args = array() ) {
	if ( empty( $user_id ) ) {
		if ( is_user_logged_in() ) {
			$user_id = get_current_user_id();
		}
	...
	$can_bypass = false;
	if ( ! empty( $user_id ) ) {
		if ( ( ebox_is_admin_user( $user_id ) ) && ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Admin_User', 'bypass_course_limits_admin_users' ) ) ) {
			$can_bypass = true;
		} elseif ( ( ebox_is_team_leader_user( $user_id ) ) && ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Teams_Team_Leader_User', 'bypass_course_limits' ) ) ) {
			$can_bypass = true;
		}
	}
	 ...
	 * context. Typically starting with an step ID, Course ID, etc.
	 */
	$can_bypass = apply_filters( 'ebox_user_can_bypass', $can_bypass, $user_id, $context, $args );
 ...
 */
function ebox_can_user_autoenroll_courses( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		if ( is_user_logged_in() ) {
			$user_id = get_current_user_id();
		}
	...
	$auto_enroll = false;
	if ( ! empty( $user_id ) ) {
		if ( ebox_is_admin_user( $user_id ) ) {
			$auto_enroll = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Admin_User', 'courses_autoenroll_admin_users' );
		} elseif ( ebox_is_team_leader_user( $user_id ) ) {
			if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Teams_Team_Leader_User', 'courses_autoenroll' ) ) {
				$auto_enroll = 'yes';
			}
		}
	}
	 ...
	 */
	return apply_filters( 'ebox_override_course_auto_enroll', $auto_enroll, $user_id );
}
 ...
 * @since 3.4.0
 */
function ebox_is_course_builder_enabled() {
	if ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Management_Display', 'course_builder_enabled' ) ) {
		return true;
	}
 ...
 * @since 3.4.0
 */
function ebox_is_course_shared_steps_enabled() {
	if ( ( true === ebox_is_course_builder_enabled() ) && ( 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Management_Display', 'course_builder_shared_steps' ) ) ) {
		return true;
	}
...
/**
 * Get Courses/Teams by Price Type.
 *
 * @since 3.4.1
 *
 * @param string  $post_type        Post Type slug: ebox-courses or team.
 * @param string  $price_type       Price Type: open, free, closed, paynow, etc.
 * @param boolean $bypass_transient Optional. Whether to bypass transient cache. Default false.
 *
 * @return @array Array of Course IDs.
 */
function ebox_get_posts_by_price_type( $post_type = '', $price_type = '', $bypass_transient = false ) {
	global $wpdb;
	...
	if ( ( ! empty( $post_type ) ) && ( in_array( $post_type, ebox_get_post_type_slug( array( 'course', 'team' ) ), true ) ) ) {

		$new_logic = false;
		if ( ebox_get_post_type_slug( 'course' ) === $post_type ) {
			if ( empty( $price_type ) ) {
				$price_type = ebox_DEFAULT_COURSE_PRICE_TYPE;
			}

			$transient_key = 'ebox_' . $price_type . '_courses';

		} elseif ( ebox_get_post_type_slug( 'team' ) === $post_type ) {
			if ( empty( $price_type ) ) {
				$price_type = ebox_DEFAULT_GROUP_PRICE_TYPE;
			}

			$transient_key = 'ebox_' . $price_type . '_teams';
		}
		...
		if ( false === $post_ids_transient ) {
			if ( ebox_post_meta_processed( $post_type ) ) {
				...
				$sql_str = $wpdb->prepare(
					"SELECT postmeta.post_id as post_id FROM {$wpdb->postmeta} as postmeta
						...
						AND ( postmeta.meta_value REGEXP '\"" . $post_type . '_' . ebox_get_post_type_key( $post_type ) . '_price_type";s:' . strlen( $price_type ) . ':"' . $price_type . "\";' )",
					$post_type,
					'_' . $post_type
				);
 ...
 * @return boolean.
 */
function ebox_post_meta_processed( $post_type = '' ) {
	if ( ( ! empty( $post_type ) ) && ( ebox_is_valid_post_type( $post_type ) ) ) {
		$data_settings = ebox_data_upgrades_setting( ebox_get_post_type_key( $post_type ) . '-post-meta' );
		 ...
		 */
		return apply_filters( 'ebox_post_meta_processed', $post_meta_processed, $post_type );
	}
	return false;
}
 ...
 * @return bool
 */
function ebox_is_course_post( $post ): bool {
	if ( empty( $post ) ) {
		return false;
	}
 ...
 * @return string
 */
function ebox_get_course_enrollment_url( $post ): string {
	if ( empty( $post ) ) {
		return '';
	}
	...
	/** This filter is documented in includes/course/ld-course-functions.php */
	return apply_filters( 'ebox_course_join_redirect', $url, $post->ID );
}
