 ...
 */
function ebox_update_user_activity( $args = array() ) {
	 ...
	 */
	$args = apply_filters( 'ebox_update_user_activity_args', $args );
	if ( empty( $args ) ) {
		return;
	}
	...
	if ( ( ! empty( $args['activity_id'] ) ) && ( ! empty( $args['activity_meta'] ) ) ) {
		foreach ( $args['activity_meta'] as $meta_key => $meta_value ) {
			ebox_update_user_activity_meta( $args['activity_id'], $meta_key, $meta_value );
		}
	}
	 ...
	 */
	do_action( 'ebox_update_user_activity', $args );
 ...
 */
function ebox_get_user_activity( $args = array(), $activity_create = false ) {
	global $wpdb;
	...
	if ( ( ! isset( $args['activity_type'] ) ) || ( empty( $args['activity_type'] ) ) ) {
		if ( ( isset( $args['post_id'] ) ) && ( ! empty( $args['post_id'] ) ) ) {
			...
			if ( ebox_is_valid_post_type( $post_type ) ) {
				$args['activity_type'] = ebox_get_post_type_key( $post_type );
			}
		}
	}
	...
	/**
	 * Filter for ebox_get_user_activity.
	 *
	 * @since 3.2.3
	 *
	 ...
	 */
	$activity = apply_filters( 'ebox_get_user_activity', $activity, $args );
 ...
 */
function ebox_get_user_activity_meta( $activity_id = 0, $activity_meta_key = '', $return_activity_meta_value_only = true, $return_activity_meta_value_unserialized = false ) {
 ...
 */
function ebox_update_user_activity_meta( $activity_id = 0, $meta_key = '', $meta_value = null ) {
	global $wpdb;
	...
	$activity = ebox_get_user_activity_meta( $activity_id, $meta_key, false );
	if ( null !== $activity ) {
		$wpdb->update(
 ...
 */
function ebox_delete_user_activity_meta( $activity_id = 0, $meta_key = '' ) {
	global $wpdb;
 ...
 * @param int $activity_id Optional. Activity ID. Default 0.
 */
function ebox_delete_user_activity( $activity_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_activity_course_get_earliest_started( $user_id = 0, $course_id = 0, $default_started = 0 ) {
	$user_id   = absint( $user_id );
	$course_id = absint( $course_id );

	if ( ( ! empty( $user_id ) ) && ( ! empty( $course_id ) ) ) {
		$user_activity = ebox_user_get_course_progress( $user_id, $course_id, 'activity' );
		if ( ! empty( $user_activity ) ) {
			if ( isset( $user_activity[ ebox_get_post_type_slug( 'course' ) . ':' . $course_id ] ) ) {
				unset( $user_activity[ ebox_get_post_type_slug( 'course' ) . ':' . $course_id ] );
			}
 ...
 */
function ebox_activity_start_step( $user_id = 0, $course_id = 0, $step_id = 0, $type = '', $start_time = 0 ) {
	if ( empty( $start_time ) ) {
		$start_time = time();
	}
	$args = array(
	...
	return ebox_get_user_activity( $args, true );
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_complete_step( $user_id = 0, $course_id = 0, $step_id = 0, $type = '', $complete_time = 0 ) {
	...
	$activity = ebox_get_user_activity( $args, true );
	if ( ! empty( $activity ) ) {
	...
	$activity_id = ebox_update_user_activity( $activity );
	if ( ! empty( $activity_id ) ) {
		return ebox_get_user_activity( $activity, true );
	}
	return null;
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_start_course( $user_id = 0, $course_id = 0, $start_time = 0, $force_start_time = true ) {
	$activity = ebox_activity_start_step( $user_id, $course_id, $course_id, 'course', $start_time );
	if ( $activity ) {

		if ( ( ! empty( absint( $start_time ) ) ) && ( empty( absint( $activity->activity_started ) ) ) ) {
			$activity->activity_started = absint( $start_time );
			ebox_update_user_activity( $activity );
		}
	}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_complete_course( $user_id = 0, $course_id = 0, $complete_time = 0 ) {
	return ebox_activity_complete_step( $user_id, $course_id, $course_id, 'course', $complete_time );
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_start_lesson( $user_id = 0, $course_id = 0, $lesson_id = 0, $start_time = 0 ) {
	return ebox_activity_start_step( $user_id, $course_id, $lesson_id, 'lesson', $start_time );
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_complete_lesson( $user_id = 0, $course_id = 0, $lesson_id = 0, $complete_time = 0 ) {
	return ebox_activity_complete_step( $user_id, $course_id, $lesson_id, 'lesson', $complete_time );
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_start_topic( $user_id = 0, $course_id = 0, $topic_id = 0, $start_time = 0 ) {
	return ebox_activity_start_step( $user_id, $course_id, $topic_id, 'topic', $start_time );
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_complete_topic( $user_id = 0, $course_id = 0, $topic_id = 0, $complete_time = 0 ) {
	return ebox_activity_complete_step( $user_id, $course_id, $topic_id, 'topic', $complete_time );
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_start_quiz( $user_id = 0, $course_id = 0, $quiz_id = 0, $start_time = 0 ) {
	return ebox_activity_start_step( $user_id, $course_id, $quiz_id, 'quiz', $start_time );
}
 ...
 * @return object Instance of LDLMS_Model_Activity or null;
 */
function ebox_activity_complete_quiz( $user_id = 0, $course_id = 0, $quiz_id = 0, $complete_time = 0 ) {
	return ebox_activity_complete_step( $user_id, $course_id, $quiz_id, 'quiz', $complete_time );
}
 ...
 */
function ebox_activity_update_meta_set( $activity_id = 0, $activity_meta = array() ) {
	if ( ( ! empty( $activity_id ) ) && ( is_array( $activity_meta ) ) ) {
		foreach ( $activity_meta as $meta_key => $meta_value ) {
			ebox_update_user_activity_meta( $activity_id, $meta_key, $meta_value );
		}
	}
}
 ...
 * @param int $course_id Course ID.
 */
function ebox_activity_course_get_latest_completed_step( $user_id = 0, $course_id = 0 ) {
	global $wpdb;
