 ...
 */
function ebox_get_lesson_list( $id = null, $atts = array() ) {
	global $post;
	...
	$course_id = ebox_get_course_id( $id );
	...
	$orderby             = ( isset( $course_modules_args['orderby'] ) ) ? $course_modules_args['orderby'] : 'title';
	$order               = ( isset( $course_modules_args['order'] ) ) ? $course_modules_args['order'] : 'ASC';
	...
	$modules_args = array_merge( $modules_args, $atts );
	...
	if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
		...
		if ( ( isset( $course_steps[ $modules_args['post_type'] ] ) ) && ( ! empty( $course_steps[ $modules_args['post_type'] ] ) ) ) {
			$modules_args['post__in'] = $course_steps[ $modules_args['post_type'] ];
			$modules_args['orderby']  = 'post__in';

			unset( $modules_args['order'] );
			unset( $modules_args['meta_key'] );
			unset( $modules_args['meta_value'] );
		} else {
			return array();
		}
	}
	 ...
	 */
	$modules_args = apply_filters( 'ebox_get_lesson_list_args', $modules_args, $id, $course_id );
	if ( ! empty( $modules_args ) ) {
		return ld_lesson_list( $modules_args );
	}
 ...
 *
 * Replaced by `ebox_get_course_modules_list` in 3.4.0.
 *
 * @since 2.1.0
 *
 ...
 * @param array            $modules_args Optional. An array of query arguments to get lesson list. Default empty array.
 *
 * @return array The lesson list array.
 */
function ebox_get_course_modules_list_legacy( $course = null, $user_id = null, $modules_args = array() ) {
	if ( empty( $course ) ) {
	...
	$orderby = ( empty( $course_settings['course_lesson_orderby'] ) ) ? ( $modules_options['orderby'] ?? '' ) : $course_settings['course_lesson_orderby'];
	$order   = ( empty( $course_settings['course_lesson_order'] ) ) ? ( $modules_options['order'] ?? '' ) : $course_settings['course_lesson_order'];

	$lesson_query_pagination = 'true';
	if ( ( isset( $modules_args['num'] ) ) && ( $modules_args['num'] !== false ) ) {
		if ( intval( $modules_args['num'] ) == 0 ) {
			$lesson_query_pagination = '';
			$posts_per_page          = -1;
		} else {
			$posts_per_page = intval( $modules_args['num'] );
		}
	} else {
		$posts_per_page = ebox_get_course_modules_per_page( $course->ID );
	...
	$lesson_paged = 1;
	if ( isset( $modules_args['paged'] ) ) {
		$lesson_paged = intval( $modules_args['paged'] );
	...
	$opt = wp_parse_args( $modules_args, $opt );
	...
	$modules = ebox_CPT::loop_shortcode( $opt );
	return $modules;
}
 ...
 *
 * Replaced by `ebox_get_topic_list` in 3.4.0.
 *
 * @since 2.1.0
 *
 ...
 */
function ebox_get_topic_list_legacy( $for_lesson_id = null, $course_id = null ) {
	if ( empty( $course_id ) ) {
		$course_id = ebox_get_course_id( $for_lesson_id );
	}

	if ( ( ! empty( $for_lesson_id ) ) && ( ! empty( $course_id ) ) ) {
		$transient_key = 'ebox_lesson_topics_' . $course_id . '_' . $for_lesson_id;
	} elseif ( ! empty( $for_lesson_id ) ) {
		$transient_key = 'ebox_lesson_topics_' . $for_lesson_id;
	} else {
		$transient_key = 'ebox_lesson_topics_all';
	}
	...
	if ( false === $topics_array ) {
		...
		if ( ! empty( $for_lesson_id ) ) {
			...
			$orderby         = $modules_options['orderby'];
			$order           = $modules_options['order'];
		...
		if ( ebox_is_course_shared_steps_enabled() ) {
			if ( ! empty( $course_id ) ) {
				...
				if ( ( isset( $steps['ebox-modules'][ $for_lesson_id ]['ebox-topic'] ) ) && ( ! empty( $steps['ebox-modules'][ $for_lesson_id ]['ebox-topic'] ) ) ) {
					$topic_ids                    = array_keys( $steps['ebox-modules'][ $for_lesson_id ]['ebox-topic'] );
		...
		if ( ! empty( $topics ) ) {
			if ( empty( $for_lesson_id ) ) {
				...
				foreach ( $topics as $topic ) {
					if ( ebox_is_course_shared_steps_enabled() ) {
						$course_id = ebox_get_course_id( $topic->ID );
						$lesson_id = ebox_course_get_single_parent_step( $course_id, $topic->ID );
					} else {
						$lesson_id = ebox_get_setting( $topic, 'lesson' );
					}
 ...
 *
 * Replaced by `ebox_get_course_quiz_list` in 3.4.0.
 *
 * @since 2.1.0
 *
 * @param int|WP_Post|null $course  Optional. The `WP_Post` course object or course ID. Default null.
 ...
 */
function ebox_get_course_quiz_list_legacy( $course = null, $user_id = null ) {
	if ( empty( $course ) ) {
		$course_id = ebox_get_course_id();
		$course    = get_post( $course_id );
	}
	...
	$quizzes = ebox_CPT::loop_shortcode( $opt );
	return $quizzes;
}
 ...
 *
 * Replaced by `ebox_get_lesson_quiz_list` in 3.4.0.
 *
 * @since 2.1.0
 *
 ...
 */
function ebox_get_lesson_quiz_list_legacy( $lesson, $user_id = null, $course_id = null ) {
 ...
 *
 * Replaced by `ebox_get_global_quiz_list` in 3.4.0.
 *
 * @global WP_Post $post Global post object.
 *
 * @since 2.1.0
 ...
 */
function ebox_get_global_quiz_list_legacy( $id = null ) {
	global $post;
	...
	if ( ! empty( $course_id ) ) {
		if ( ebox_is_course_shared_steps_enabled() ) {
			$quiz_ids = ebox_course_get_children_of_step( $course_id, $course_id, 'ebox-quiz' );
			if ( ! empty( $quiz_ids ) ) {
				return get_posts(
					array(
						'post_type'      => 'ebox-quiz',
			...
			$transient_key = 'ebox_quiz_course_' . $course_id;
			...
			if ( false === $quizzes_new ) {
				...
				foreach ( $quizzes as $k => $quiz ) {
					$quiz_lesson = ebox_get_setting( $quiz, 'lesson' );
 ...
 *
 * Replaced by `ebox_get_course_data` in 3.4.0.
 *
 * @since 3.4.0
 *
 ...
 */
function ebox_get_course_data_legacy( $data ) {
	...
	if ( ( 'post.php' === $pagenow ) && ( ebox_get_post_type_slug( 'course' ) === $typenow ) ) {
		...
		if ( ! empty( $course_id ) ) {
			...
			$modules        = ebox_get_course_modules_list( $course_id, null, array( 'num' => 0 ) );
			$output_modules = array();
			$lesson_topics  = array();
			...
			if ( ( is_array( $modules ) ) && ( ! empty( $modules ) ) ) {
				...
				foreach ( $modules as $lesson ) {
					$post = $lesson['post'];
					// Get lesson's topics.
					$topics        = ebox_topic_dots( $post->ID, false, 'array', null, $course_id );
					$output_topics = array();
					...
					if ( ( is_array( $topics ) ) && ( ! empty( $topics ) ) ) {
						...
						foreach ( $topics as $topic ) {
							// Get topic's quizzes.
							$topic_quizzes        = ebox_get_lesson_quiz_list( $topic->ID, null, $course_id );
							...
							$output_topics[] = array(
								...
								'url'        => ebox_get_step_permalink( $topic->ID, $course_id ),
					...
					$quizzes        = ebox_get_lesson_quiz_list( $post->ID, null, $course_id );
					$output_quizzes = array();
			...
			if ( ( is_array( $quizzes ) ) && ( ! empty( $quizzes ) ) ) {
				...
				foreach ( $quizzes as $quiz ) {
					...
					$output_quizzes[] = array(
						...
						'post_title' => $post->post_title,
						'type'       => $post->post_type,
						'url'        => ebox_get_step_permalink( $post->ID, $course_id ),
						'edit_link'  => get_edit_post_link( $post->ID, '' ),
						'tree'       => array(),
					);
				}
...
/**
 * LEGACY: Gets the total count of modules and topics for a given course ID.
 *
 * Replaced by `ebox_get_course_steps_count` in 3.4.0.
 *
 * @since 2.3.0
 *
 ...
 */
function ebox_get_course_steps_count_legacy( $course_id = 0 ) {
	...
	if ( ! isset( $courses_steps[ $course_id ] ) ) {
		...
		$course_steps = ebox_get_course_steps( $course_id );
		if ( ! empty( $course_steps ) ) {
			$courses_steps[ $course_id ] = count( $course_steps );
		}

		if ( ebox_has_global_quizzes( $course_id ) ) {
 ...
 *
 * Replaced by `ebox_course_status` in 3.4.0.
 *
 * @since 2.1.0
 ...
 */
function ebox_course_status_legacy( $id, $user_id = null, $return_slug = false ) {
	$course_status_str = '';
	...
	if ( ! empty( $completed_on ) ) {
		if ( true === $return_slug ) {
			...
			$course_status_str = esc_html__( 'Completed', 'ebox' );
		}
	} else {
		$course_progress = get_user_meta( $user_id, '_ebox-course_progress', true );
		/**
		 ...
		 * @param int   $course_id                      Course ID.
		 */
		if ( apply_filters( 'ebox_course_status_recalc_total_steps', true, $course_progress[ $id ], $user_id, $id ) ) {
			$course_steps_count = ebox_get_course_steps_count( $id );
		...
		if ( ! empty( $quizzes ) ) {
			...
			if ( ! empty( $quizzes_incomplete ) ) {
				...
				if ( has_filter( 'ebox_post_args_teams' ) ) {
					 ...
					 * @param int $user_id User ID
					 */
					apply_filters_deprecated( 'ebox_prevent_course_autocompletion_multiple_final_quizzes', array( false, $id, $user_id ), '3.2.3', 'ebox_course_autocompletion_multiple_final_quizzes_step' );
				}
				...
				$quiz_notstarted = apply_filters(
					'ebox_course_autocompletion_multiple_final_quizzes_step',
					false,
					$id,
					$user_id,
					$quizzes,
		...
		if ( ( empty( $course_progress[ $id ] ) || empty( $course_progress[ $id ]['modules'] ) && ! $has_completed_topic ) && $quiz_notstarted ) {
			if ( true === $return_slug ) {
				$course_status_str = 'not-started';
			} else {
				$course_status_str = esc_html__( 'Not Started', 'ebox' );
			}
		} elseif (
			...
			if ( true === $return_slug ) {
				...
				$course_status_str = esc_html__( 'In Progress', 'ebox' );
			}
		} elseif (
			isset( $course_progress[ $id ]['completed'] )
			&& isset( $course_progress[ $id ]['total'] )
			 ...
			 */
			ebox_process_mark_complete( $user_id, $id, false, $id );
		}
	}
 ...
 *
 * Replaced by `ebox_is_lesson_notcomplete` in 3.4.0.
 *
 * @since 2.1.0
 *
 * @param int|null $user_id   Optional. User ID. Defaults to the current logged-in user. Default null.
 * @param array    $modules   Optional. An array of lesson IDs.
 ...
 */
function ebox_is_lesson_notcomplete_legacy( $user_id = null, $modules = array(), $course_id = 0 ) {
	if ( empty( $user_id ) ) {
	...
	if ( ! empty( $modules ) ) {
		foreach ( $modules as $lesson => $v ) {
			...
			if ( ! empty( $course_progress[ $course_id ] ) && ! empty( $course_progress[ $course_id ]['modules'] ) && ! empty( $course_progress[ $course_id ]['modules'][ $lesson ] ) ) {
 ...
 *
 * Replaced by `ebox_is_topic_notcomplete` in 3.4.0.
 *
 * @since 2.3.1
 ...
 */
function ebox_is_topic_notcomplete_legacy( $user_id = null, $topics = array(), $course_id = 0 ) {
	if ( empty( $user_id ) ) {
		$current_user = wp_get_current_user();
		$user_id      = $current_user->ID;
	}
	...
	if ( ! empty( $topics ) ) {
		foreach ( $topics as $topic_id => $v ) {
			...
			$lesson_id = ebox_get_lesson_id( $topic_id );
 ...
 *
 * Replaced by `ebox_is_quiz_accessable` in 3.4.0.
 *
 * @since 2.4.0
 *
 * @param int|null     $user_id   Optional. User ID. Default null.
 ...
 */
function ebox_is_quiz_accessable_legacy( $user_id = null, $post = null, $course_id = 0 ) {
	if ( empty( $user_id ) ) {
		$current_user = wp_get_current_user();
	...
	if ( ( ! empty( $post ) ) && ( $post instanceof WP_Post ) ) {
		...
		if ( ebox_is_course_shared_steps_enabled() ) {
			$quiz_lesson = ebox_course_get_single_parent_step( $course_id, $post->ID );
		} else {
			$quiz_lesson = ebox_get_setting( $post, 'lesson' );
		}

		if ( ! empty( $quiz_lesson ) ) {
			$quiz_lesson_post = get_post( $quiz_lesson );
			if ( ( $quiz_lesson_post instanceof WP_Post ) && ( 'ebox-topic' === $quiz_lesson_post->post_type ) ) {
				return 1;
			} elseif ( ebox_lesson_topics_completed( $quiz_lesson ) ) {
				return 1;
			} else {
				return 0;
			}
			...
			$modules = ebox_get_lesson_list( $course_id, array( 'num' => 0 ) );
 ...
 *
 * Replaced by `ebox_get_course_progress` in 3.4.0.
 *
 * @since 2.1.0
 *
 * @param int|null $user_id   Optional. User ID. Default null.
 ...
 */
function ebox_get_course_progress_legacy( $user_id = null, $postid = null, $course_id = null ) {
	if ( empty( $user_id ) ) {
		$current_user = wp_get_current_user();
	...
	if ( 'ebox-modules' === $this_post->post_type ) {
		$posts = ebox_get_lesson_list( $postid, array( 'num' => 0 ) );

		if ( empty( $course_progress ) || empty( $course_progress[ $course_id ]['modules'] ) ) {
			$completed_posts = array();
		} else {
			$completed_posts = $course_progress[ $course_id ]['modules'];
		}
	} elseif ( 'ebox-topic' === $this_post->post_type ) {
		if ( ebox_is_course_shared_steps_enabled() ) {
			$lesson_id = ebox_course_get_single_parent_step( $course_id, $this_post->ID );
		} else {
			$lesson_id = ebox_get_setting( $this_post, 'lesson' );
		}
		$posts = ebox_get_topic_list( $lesson_id, $course_id );
...
/**
 * LEGACY: Gets all the modules and topics for a given course ID.
 *
 * For now excludes quizzes at lesson and topic level.
 *
 * Replaced by `ebox_get_course_steps` in 3.4.0.
 *
 * @since 2.3.0
 *
 * @param int   $course_id          Optional. The ID of the course. Default 0.
 * @param array $include_post_types Optional. An array of post types to include in course steps. Default array contains 'ebox-modules' and 'ebox-topic'.
 *
 * @return array An array of all course steps.
 */
function ebox_get_course_steps_legacy( $course_id = 0, $include_post_types = array( 'ebox-modules', 'ebox-topic' ) ) {
	...
	if ( ! empty( $course_id ) ) {
		if ( ebox_is_course_builder_enabled() ) {
			foreach ( $include_post_types as $post_type ) {
				$steps[ $post_type ] = ebox_course_get_steps_by_type( $course_id, $post_type );
			}
		} else {
			if ( ( in_array( 'ebox-modules', $include_post_types, true ) ) || ( in_array( 'ebox-topic', $include_post_types, true ) ) ) {
				...
				if ( $lesson_steps_query->have_posts() ) {
					$steps['ebox-modules'] = $lesson_steps_query->posts;
				}
			}

			// For Topics we still require the parent modules items.
			if ( in_array( 'ebox-topic', $include_post_types, true ) ) {

				if ( ! empty( $steps['ebox-modules'] ) ) {
					...
					if ( ( isset( $steps['ebox-modules'] ) ) && ( ! empty( $steps['ebox-modules'] ) ) ) {
						$topic_steps_query_args['meta_query'][] = array(
							'key'     => 'lesson_id',
							'value'   => $steps['ebox-modules'],
							'compare' => 'IN',
							'type'    => 'NUMERIC',
						);
					}
					...
					if ( $topic_steps_query->have_posts() ) {
						$steps['ebox-topic'] = $topic_steps_query->posts;
					}
				} else {
					$steps['ebox-topic'] = array();
				}
			}
		}
	}
 ...
 *
 * Replaced by `ebox_process_mark_complete` in 3.4.0.
 *
 * @since 2.1.0
 *
 * @param int|null $user_id       Optional. User ID. Default null.
 ...
 */
function ebox_process_mark_complete_legacy( $user_id = null, $postid = null, $onlycalculate = false, $course_id = 0 ) {
	if ( empty( $user_id ) ) {
		if ( is_user_logged_in() ) {
			$current_user = wp_get_current_user();
			$user_id      = $current_user->ID;
	...
	if ( ( ! isset( $course_progress[ $course_id ]['modules'] ) ) || ( empty( $course_progress[ $course_id ]['modules'] ) ) ) {
		$course_progress[ $course_id ]['modules'] = array();
	}
	...
	if ( 'ebox-topic' === $post->post_type && empty( $course_progress[ $course_id ]['topics'][ $lesson_id ] ) ) {
	...
	if ( ! $onlycalculate && 'ebox-modules' === $post->post_type && empty( $course_progress[ $course_id ]['modules'][ $postid ] ) ) {
		$course_progress[ $course_id ]['modules'][ $postid ] = 1;
		$lesson_completed                                    = true;
	}

	if ( ! $onlycalculate && 'ebox-topic' === $post->post_type && empty( $course_progress[ $course_id ]['topics'][ $lesson_id ][ $postid ] ) ) {
	...
	$completed = ebox_course_get_completed_steps( $user_id, $course_id, $course_progress[ $course_id ] );

	$course_progress[ $course_id ]['completed'] = $completed;

	// New logic includes modules and topics.
	$course_progress[ $course_id ]['total'] = ebox_get_course_steps_count( $course_id );
	...
	if ( ! empty( $topic_completed ) ) {
		...
		if ( ! $course_activity ) {
			...
			ebox_update_user_activity_meta( $course_activity->activity_id, 'steps_total', $course_progress[ $course_id ]['total'] );
			ebox_update_user_activity_meta( $course_activity->activity_id, 'steps_completed', $course_progress[ $course_id ]['completed'] );
			ebox_update_user_activity_meta( $course_activity->activity_id, 'steps_last_id', $post->ID );
		}
	}
 ...
 *
 * Replaced by `ebox_course_get_completed_steps` in 3.4.0
 *
 * @since 2.3.0
 *
 ...
 */
function ebox_course_get_completed_steps_legacy( $user_id = 0, $course_id = 0, $course_progress = array() ) {
	$steps_completed_count = 0;

	if ( ( ! empty( $user_id ) ) && ( ! empty( $course_id ) ) ) {

		if ( empty( $course_progress ) ) {
			$course_progress_all = get_user_meta( $user_id, '_ebox-course_progress', true );
		...
		if ( ! empty( $course_modules ) ) {
			if ( isset( $course_progress['modules'] ) ) {
				foreach ( $course_progress['modules'] as $lesson_id => $lesson_completed ) {
					if ( in_array( $lesson_id, $course_modules, true ) ) {
		...
		$course_topics = ebox_course_get_steps_by_type( $course_id, 'ebox-topic' );
		...
		if ( ebox_has_global_quizzes( $course_id ) ) {
			if ( ebox_is_all_global_quizzes_complete( $user_id, $course_id ) ) {
