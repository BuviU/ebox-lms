...
function ld_course_check_user_access( $course_id, $user_id = null ) {
	return ebox_lms_has_access( $course_id, $user_id );
}
...
function ld_get_mycourses( $user_id = null, $atts = array() ) {
	...
	return ebox_user_get_enrolled_courses(
		$user_id,
		$atts,
		true
	);
 ...
 */
function ebox_lms_has_access( $post_id, $user_id = null ) {
	 ...
	 */
	return apply_filters( 'ebox_lms_has_access', ebox_lms_has_access_fn( $post_id, $user_id ), $post_id, $user_id );
}
 ...
 * @return bool Returns true if the user has access.
 */
function ebox_lms_has_access_fn( $post_id, $user_id = null ) {
	if ( empty( $user_id ) ) {
	...
	if ( ! empty( $post_id ) && ebox_is_sample( $post_id ) ) {
		return true;
	}

	$meta = ebox_get_setting( $course_id );
	...
	if ( true === ebox_use_legacy_course_access_list() ) {
		if ( ! empty( $meta['course_access_list'] ) ) {
			$course_access_list = ebox_convert_course_access_list( $meta['course_access_list'], true );
		} else {
			$course_access_list = array();
		}
		if ( ( in_array( $user_id, $course_access_list ) ) || ( ebox_user_team_enrolled_to_course( $user_id, $course_id ) ) ) {
		...
		if ( ( ! empty( $course_user_meta ) ) || ( ebox_user_team_enrolled_to_course( $user_id, $course_id ) ) ) {
			$expired = ld_course_access_expired( $course_id, $user_id );
			return ! $expired; // True if not expired.
		} else {
			return false;
 ...
 */
function ebox_lms_access_redirect( $post_id ) {
	$access = ebox_lms_has_access( $post_id );
	if ( true === $access ) {
		return true;
	}

	$link = get_permalink( ebox_get_course_id( $post_id ) );
	/**
	 ...
	 * @param int    $post_id Post ID.
	 */
	$link = apply_filters( 'ebox_access_redirect', $link, $post_id );
	if ( ! empty( $link ) ) {
...
function ld_course_access_expired( $course_id, $user_id ) {
	...
	if ( empty( $course_access_upto ) ) {
		...
		if ( time() >= $course_access_upto ) {
			 ...
			 */
			if ( apply_filters( 'ebox_process_user_course_access_expire', true, $user_id, $course_id, $course_access_upto ) ) {

				/**
				 * As of ebox 2.3.0.3 we store the GMT timestamp as the meta value. In prior versions we stored 1
				*/
				update_user_meta( $user_id, 'ebox_course_expired_' . $course_id, time() );
				 ...
				 * @param int $course_id Course ID.
				 */
				do_action( 'ebox_user_course_access_expired', $user_id, $course_id );

				$delete_course_progress = ebox_get_setting( $course_id, 'expire_access_delete_progress' );
				if ( ! empty( $delete_course_progress ) ) {
					ebox_delete_course_progress( $course_id, $user_id );
				}
				return true;
			} else {
				return false;
...
function ld_course_access_expired_alert() {
	...
	if ( ! is_singular() || empty( $post->ID ) || ebox_get_post_type_slug( 'course' ) !== $post->post_type ) {
		return;
	}
	...
	$expired = get_user_meta( $user_id, 'ebox_course_expired_' . $post->ID, true );

	if ( empty( $expired ) ) {
		return;
	}

	$has_access = ebox_lms_has_access( $post->ID, $user_id );

	if ( $has_access ) {
		delete_user_meta( $user_id, 'ebox_course_expired_' . $post->ID );
		return;
	} else {
		echo '<script>
			setTimeout(function() {
				...
				printf(
					...
					esc_html_x( 'Your access to this %s has expired.', 'placeholder: Course', 'ebox' ),
...
function ld_course_access_expires_on( $course_id, $user_id ) {
	...
	// If the course_id + user_id is not set we check the team courses.
	if ( empty( $courses_access_from ) ) {
		$courses_access_from = ebox_user_team_enrolled_to_course_from( $user_id, $course_id );
	}
	...
	if ( abs( intval( $courses_access_from ) ) ) {
		...
		if ( ! empty( $expire_access ) ) {
			$expire_access_days = ebox_get_setting( $course_id, 'expire_access_days' );
...
function ld_course_access_from( $course_id = 0, $user_id = 0 ) {
	...
	if ( ! isset( $courses[ $course_id ][ $user_id ] ) ) {
		...
		if ( empty( $courses[ $course_id ][ $user_id ] ) ) {
			 ...
			 * @param int     $course_id          Course ID.
			 */
			if ( ( 'open' === ebox_get_course_meta_setting( $course_id, 'course_price_type' ) ) && ( apply_filters( 'ebox_course_open_set_user_access_from', true, $user_id, $course_id ) ) ) {
				$enrolled_teams = ebox_user_team_enrolled_to_course_from( $user_id, $course_id );
				if ( ! empty( $enrolled_teams ) ) {
					$courses[ $course_id ][ $user_id ] = absint( $enrolled_teams );
				}
			}
		}
		if ( empty( $courses[ $course_id ][ $user_id ] ) ) {
			...
			$course_activity = ebox_get_user_activity( $course_activity_args );
...
function ld_course_access_from_update( $course_id, $user_id, $access = '', $is_gmt = false ) {
	if ( ( ! empty( $course_id ) ) && ( ! empty( $user_id ) ) && ( ! empty( $access ) ) ) {
		...
		if ( ! is_numeric( $access ) ) {
			...
			$access_time = ebox_get_timestamp_from_date_string( $access, ! $is_gmt );
...
function ld_update_course_access( $user_id, $course_id, $remove = false ): bool {
	...
	if ( true === ebox_use_legacy_course_access_list() ) {
		...
		$course_access_list = ebox_convert_course_access_list( $course_access_list, true );
		...
		$course_access_list = ebox_convert_course_access_list( $course_access_list );
		ebox_update_setting( $course_id, 'course_access_list', $course_access_list );
	}
	...
	if ( true === $course_activity_args['changed'] ) {
		...
		if ( true !== $skip ) {
			...
			ebox_update_user_activity( $course_activity_args );
		}
	}
	 ...
	 * Note: Used if `ebox_use_legacy_course_access_list()` returns true. Otherwise null is sent.
	 * @param boolean      $remove             Whether to remove course access from the user.
	 */
	do_action( 'ebox_update_course_access', $user_id, $course_id, $course_access_list, $remove );
...
function ld_lesson_access_from( $lesson_id, $user_id, $course_id = null, $bypass_transient = false ) {
	...
	$courses_access_from = ld_course_access_from( $course_id, $user_id );
	if ( empty( $courses_access_from ) ) {
		$courses_access_from = ebox_user_team_enrolled_to_course_from( $user_id, $course_id, $bypass_transient );
	}

	$visible_after = ebox_get_setting( $lesson_id, 'visible_after' );
	if ( $visible_after > 0 ) {
		...
		$visible_after_specific_date = ebox_get_setting( $lesson_id, 'visible_after_specific_date' );
		if ( ! empty( $visible_after_specific_date ) ) {
			if ( ! is_numeric( $visible_after_specific_date ) ) {
				// If we a non-numeric value like a date stamp Y-m-d hh:mm:ss we want to convert it to a GMT timestamp.
				$visible_after_specific_date = ebox_get_timestamp_from_date_string( $visible_after_specific_date, true );
			}
...
function lesson_visible_after( string $content = '', $post = null ) {
	...
	$bypass_course_limits_admin_users = ebox_can_user_bypass( $user_id, 'ebox_course_lesson_not_available', $post->ID, $post );
	...
	$lesson_access_from = ebox_course_step_available_date( $post->ID, $course_id, $user_id, true );
	if ( ! empty( $lesson_access_from ) ) {
		...
		$content = ebox_LMS::get_template(
			...
			array(
				...
				'lesson_access_from_date' => ebox_adjust_date_time_display( $lesson_access_from ),
				'context'                 => $context,
			),
			false
		);
...
add_filter( 'ebox_content', 'lesson_visible_after', 1, 2 );
 ...
 */
function ebox_get_users_for_course( $course_id = 0, $query_args = array(), $exclude_admin = true ) {
	...
	$course_price_type = ebox_get_course_meta_setting( $course_id, 'course_price_type' );
	...
	if ( 'open' === $course_price_type ) {
		...
		if ( true === ebox_use_legacy_course_access_list() ) {
			$course_access_list = ebox_get_course_meta_setting( $course_id, 'course_access_list' );
		...
		$course_teams_users = ebox_get_course_teams_users_access( $course_id );
		$course_user_ids     = array_merge( $course_user_ids, $course_teams_users );

		if ( ! empty( $course_user_ids ) ) {
			$course_user_ids = array_unique( $course_user_ids );
		}

		$course_expired_access_users = ebox_get_course_expired_access_from_meta( $course_id );
 ...
 */
function ebox_set_users_for_course( $course_id = 0, $course_users_new = array() ) {

	if ( ! empty( $course_id ) ) {

		if ( ! empty( $course_users_new ) ) {
			$course_users_new = ebox_convert_course_access_list( $course_users_new, true );
		} else {
			$course_users_new = array();
		}

		$course_users_old = ebox_get_course_users_access_from_meta( $course_id );
		if ( ! empty( $course_users_old ) ) {
			$course_users_old = ebox_convert_course_access_list( $course_users_old, true );
		} else {
 ...
 */
function ebox_get_course_users_access_from_meta( $course_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_user_get_course_childen_progress( $user_id = 0, $course_id = 0, $step_id = 0 ) {
	$user_id   = absint( $user_id );
	$course_id = absint( $course_id );
	...
	if ( ( ! empty( $course_id ) ) && ( ! empty( $step_id ) ) && ( ! empty( $user_id ) ) ) {
		$course_children_steps = ebox_course_get_children_of_step( $course_id, $step_id );
		if ( ! empty( $course_children_steps ) ) {
			$course_children_steps = array_map( 'absint', $course_children_steps );

			$user_course_progress_co = ebox_user_get_course_progress( $user_id, $course_id, 'co' );
 ...
 */
function ebox_user_is_course_children_progress_complete( $user_id = 0, $course_id = 0, $step_id = 0 ) {
	$user_id   = absint( $user_id );
	$course_id = absint( $course_id );
	$step_id   = absint( $step_id );

	if ( ( ! empty( $course_id ) ) && ( ! empty( $step_id ) ) && ( ! empty( $user_id ) ) ) {
		$user_children_progress = ebox_user_get_course_childen_progress( $user_id, $course_id, $step_id ); // cspell:disable-line.
 ...
 * @return int.
 */
function ebox_course_step_available_date( int $step_id = 0, int $course_id = 0, int $user_id = 0, bool $parent_steps = false ) {
	...
	$step_post = get_post();
	if ( ( ! is_a( $step_post, 'WP_Post' ) ) || ( ! in_array( $step_post->post_type, ebox_get_post_types(), true ) ) ) {
		return $available_timestamp;
	}
	...
	if ( ebox_can_user_bypass( $user_id, 'ebox_course_lesson_not_available', $step_post->ID, $step_post ) ) {
		return $available_timestamp;
	}

	$step_ids = array();
	if ( true === $parent_steps ) {
		$step_ids = ebox_course_get_all_parent_step_ids( $course_id, $step_id, true, true );
		if ( count( $step_ids ) > 1 ) {
