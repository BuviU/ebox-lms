 ...
 */
function ebox_previous_post_link( $prev_link = '', $url = false, $post = null ) {
	if ( empty( $post ) ) {
		global $post;
	}
	...
	if ( 'ebox-modules' === $post->post_type ) {
		$link_name = ebox_get_label_course_step_previous( ebox_get_post_type_slug( 'lesson' ) );

		$course_id = ebox_get_course_id( $post );
		$posts     = ebox_course_get_modules( $course_id, array( 'per_page' => 0 ) );

	} elseif ( 'ebox-topic' === $post->post_type ) {
		$link_name = ebox_get_label_course_step_previous( ebox_get_post_type_slug( 'topic' ) );
		...
		if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) {
			...
			$course_id = ebox_get_setting( $post, 'course' );
		}
		$posts = ebox_course_get_topics( $course_id, $lesson_id, array( 'per_page' => 0 ) );
	} else {
		return $prev_link;
	}
	...
	if ( ( $prev_post ) && ( is_a( $prev_post, 'WP_Post' ) ) ) {
		if ( 'id' === $url ) {
			 ...
			 */
			return apply_filters( 'ebox_previous_post_link', $link, $permalink, $link_name, $post );
		}
	} else {
		return $prev_link;
	}
 ...
 */
function ebox_next_post_link( $prev_link = '', $url = false, $post = null ) {
	if ( empty( $post ) ) {
		global $post;
	}
	...
	if ( 'ebox-modules' === $post->post_type ) {
		$link_name = ebox_get_label_course_step_next( ebox_get_post_type_slug( 'lesson' ) );
		$course_id = ebox_get_course_id( $post );
		$posts     = ebox_course_get_modules( $course_id, array( 'per_page' => 0 ) );
	} elseif ( 'ebox-topic' === $post->post_type ) {
		$link_name = ebox_get_label_course_step_next( ebox_get_post_type_slug( 'topic' ) );
	...
	if ( ( $next_post ) && ( is_a( $next_post, 'WP_Post' ) ) ) {
		if ( 'id' === $url ) {
			 ...
			 * @param WP_Post $post      Post object.
			 */
			return apply_filters( 'ebox_next_post_link', $link, $permalink, $link_name, $next_post );
		}
	} else {
		return $prev_link;
	}
 ...
 */
function ebox_clear_prev_next_links( $prev_link = '' ) {
	global $post;

	if ( ! is_singular() || empty( $post->post_type ) || ! in_array( $post->post_type, array( 'ebox-quiz', 'ebox-courses', 'ebox-topic', 'ebox-assignment' ), true ) ) {
		return $prev_link;
	} else {
		return '';
	}
}
add_filter( 'previous_post_link', 'ebox_clear_prev_next_links', 1 );
add_filter( 'next_post_link', 'ebox_clear_prev_next_links', 1 );
 ...
 */
function ebox_quiz_continue_link( $id ) {
	$course_id = ebox_get_course_id( $id );
	if ( ( ! empty( $course_id ) ) && ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Builder', 'shared_steps' ) == 'yes' ) ) {
		$lesson_id = ebox_course_get_single_parent_step( $course_id, $id );
		if ( empty( $lesson_id ) ) {
		...
		if ( ( isset( $url ) ) && ( ! empty( $url ) ) ) {
			$return_link = '<a id="quiz_continue_link" href="' . esc_url( $url ) . '">' . ebox_Custom_Label::get_label( 'button_click_here_to_continue' ) . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
		}
	} else {
		$quizmeta = get_post_meta( $id, '_ebox-quiz', true );

		if ( ! empty( $quizmeta['ebox-quiz_lesson'] ) ) {
			$return_id   = (int) $quizmeta['ebox-quiz_lesson'];
			$quiz_lesson = (int) $quizmeta['ebox-quiz_lesson'];
		}
	...
	if ( $version >= '1.5.1' ) {
		 ...
		 */
		return apply_filters( 'ebox_quiz_continue_link', $return_link, $url );

	} else {

		/** This filter is documented in includes/course/ld-course-navigation.php */
		return apply_filters( 'ebox_quiz_continue_link', $return_link );

	}
}

/**
 * Outputs the ebox topic dots.
 *
 ...
 */
function ebox_topic_dots( $lesson_id, $show_text = false, $type = 'dots', $user_id = null, $course_id = null ) {
	if ( empty( $lesson_id ) ) {
		return '';
	}
	...
	$topics = ebox_get_topic_list( $lesson_id, $course_id );
	if ( empty( $topics[0]->ID ) ) {
		return '';
	}

	$topics_progress = ebox_get_course_progress( $user_id, $topics[0]->ID, $course_id );
	...
	$html = "<div id='ebox_topic_dots-" . esc_attr( $lesson_id ) . "' class='ebox_topic_dots type-" . esc_attr( $type ) . "'>";
	...
	switch ( $type ) {
		...
		default:
			...
			foreach ( $topics as $topic ) {
				...
				/** This filter is documented in includes/course/ld-course-navigation.php */
				$html .= apply_filters( 'ebox_topic_dots_item', '<a class="' . esc_attr( $completed ) . '" href="' . esc_url( get_permalink( $topic->ID ) ) . '"><span title="' . esc_html( apply_filters( 'the_title', $topic->post_title, $topic->ID ) ) . '"></span></a>', $topic, $completed, $type, $sn ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP Core Hook
			}
 ...
 */
function ebox_get_topic_list( $lesson_id = null, $course_id = null ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_get_topic_list_legacy( $lesson_id, $course_id );
	}
	...
	if ( ( ! empty( $course_id ) ) && ( ! empty( $lesson_id ) ) ) {
		$course_topics = ebox_course_get_topics( $course_id, $lesson_id );
		/**
		 * See the part from ebox_topic_dots() where this function is called.
 ...
 */
function ebox_get_global_quiz_list( $id = null ) {
	$quizzes = array();

	$course_id = ebox_get_course_id( $id );
	if ( ! empty( $course_id ) ) {
		if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
			return ebox_get_global_quiz_list_legacy( $id );
		}

		$quizzes = ebox_course_get_quizzes( $course_id, $course_id );
	}
	return $quizzes;
}
 ...
 */
function ebox_get_course_modules_list( $course_id = null, $user_id = null, $query_args = array() ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_get_course_modules_list_legacy( $course_id, $user_id, $query_args );
	}
	...
	if ( ! empty( $course_id ) ) {
		...
		if ( ( $ld_course_object ) && ( is_a( $ld_course_object, 'LDLMS_Model_Course' ) ) ) {
			$course_modules = $ld_course_object->get_modules( $query_args );
			if ( ! empty( $course_modules ) ) {
				$modules_pager = $ld_course_object->get_pager( $course_id, ebox_get_post_type_slug( 'lesson' ) );
				$sno           = 1;
				if ( ( isset( $modules_pager['per_page'] ) ) && ( isset( $modules_pager['paged'] ) ) ) {
					$sno = 1 + ( absint( $modules_pager['per_page'] ) * ( absint( $modules_pager['paged'] ) - 1 ) );
				}
				foreach ( $course_modules as $lesson_post ) {
					$lesson_item = array(
						'sno'                => $sno,
						'id'                 => $lesson_post->ID,
						'post'               => $lesson_post,
					...
					if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) == 'yes' ) {
						$lesson_item['permalink'] = ebox_get_step_permalink( $lesson_post->ID, $course_id );
					} else {
						$lesson_item['permalink'] = get_permalink( $lesson_post->ID );
					}

					$lesson_item['sample'] = ( ebox_is_sample( $lesson_post->ID ) ) ? 'is_sample' : 'is_not_sample';

					if ( ! ebox_is_lesson_notcomplete( $user_id, array( $lesson_post->ID => 1 ), $course_id ) ) {
						...
						if ( empty( $lesson_item['lesson_access_from'] ) ) {
							...
							$lesson_item['sub_title'] = ebox_LMS::get_template(
								...
								array(
									...
									'lesson_access_from_date' => ebox_adjust_date_time_display( $lesson_item['lesson_access_from'] ),
			...
			$modules_pager = $ld_course_object->get_pager( $course_id, ebox_get_post_type_slug( 'lesson' ) );
			if ( is_array( $modules_pager ) ) {
				if ( isset( $modules_pager['paged'] ) ) {
					$query_posts->query_vars['paged'] = intval( $modules_pager['paged'] );
				}
				if ( isset( $modules_pager['total_items'] ) ) {
					$query_posts->found_posts = absint( $modules_pager['total_items'] );
				}
				if ( isset( $modules_pager['total_pages'] ) ) {
					$query_posts->max_num_pages = $modules_pager['total_pages'];
				}
			}
 ...
 */
function ebox_get_course_quiz_list( $course = null, $user_id = null ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_get_course_quiz_list_legacy( $course, $user_id );
	}
	...
	if ( ! empty( $course_id ) ) {
		$course_quizzes = ebox_course_get_quizzes( $course_id, $course_id );
		if ( ! empty( $course_quizzes ) ) {
			...
			foreach ( $course_quizzes as $quiz_post ) {
				...
				if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' ) == 'yes' ) {
					$quiz_item['permalink'] = ebox_get_step_permalink( $quiz_post->ID, $course_id );
				} else {
					$quiz_item['permalink'] = get_permalink( $quiz_post->ID );
				}

				$quiz_item['sample'] = ( ebox_is_sample( $quiz_post->ID ) ) ? 'is_sample' : 'is_not_sample';
				$quiz_item['status'] = ( ebox_is_quiz_notcomplete( $user_id, array( $quiz_post->ID => 1 ), false, $course_id ) ) ? 'notcompleted' : 'completed';
 ...
 */
function ebox_get_lesson_quiz_list( $lesson, $user_id = null, $course_id = null ) {
	if ( ( defined( 'ebox_COURSE_FUNCTIONS_LEGACY' ) ) && ( true === ebox_COURSE_FUNCTIONS_LEGACY ) ) {
		return ebox_get_lesson_quiz_list_legacy( $lesson, $user_id, $course_id );
	}
	...
	if ( is_a( $lesson, 'WP_Post' ) ) {
		...
		if ( ( is_a( $lesson_post, 'WP_Post' ) ) && ( in_array( $lesson_post->post_type, ebox_get_post_type_slug( array( 'lesson', 'topic' ) ), true ) ) ) {
			$lesson_id = $lesson_post->ID;
		}
	}
	...
	if ( ( ! empty( $course_id ) ) && ( ! empty( $lesson_id ) ) ) {
		$course_quizzes = ebox_course_get_quizzes( $course_id, $lesson_id );
		if ( ! empty( $course_quizzes ) ) {
