 ...
 * @since 2.1.0
 */
function ebox_mce_init() {
	$screen = get_current_screen();
	if ( ( $screen ) && ( $screen instanceof WP_Screen ) ) {
		if ( ( 'post' === $screen->base ) && ( 'ebox-certificates' === $screen->post_type ) ) {
			add_filter( 'tiny_mce_before_init', 'ebox_wp_tiny_mce_before_init' );
			add_filter( 'mce_css', 'ebox_filter_mce_css' );
		}
	}
}
 ...
 */
add_action( 'load-post.php', 'ebox_mce_init' );
add_action( 'load-post-new.php', 'ebox_mce_init' );
 ...
 */
function ebox_filter_mce_css( $mce_css = '' ) {
	$ld_mce_css = plugins_url( 'assets/css/ebox_editor.css', ebox_LMS_PLUGIN_DIR . 'index.php' );
	if ( ! empty( $ld_mce_css ) ) {
		if ( ! empty( $mce_css ) ) {
			$mce_css .= ',';
		}
 ...
 */
function ebox_wp_tiny_mce_before_init( $init_array ) {
	...
	if ( ! empty( $post_id ) ) {
		$img_path = ebox_get_thumb_url( $post_id );
		if ( ! empty( $img_path ) ) {
			$init_array['setup'] = <<<JS
[function(ed) {
 ...
 */
function ebox_get_thumb_url( $post_id = 0, $size = 'full' ) {
