...
if ( ! class_exists( 'ebox_CPT_Instance' ) ) {
	 ...
	 */
	class ebox_CPT_Instance extends ebox_CPT {
		...
		public function __construct( $args ) {
			...
			if ( empty( $name ) ) {
				$name = ! empty( $options_page_title ) ? $options_page_title : $post_name . esc_html__( ' Options', 'ebox' );
			}
			...
			if ( empty( $default_options ) ) {
				...
				$this->default_options = array(
					...
					'order'          => array(
						'name'            => esc_html__( 'Sort Direction', 'ebox' ),
						'type'            => 'select',
						'initial_options' => array(
							''     => esc_html__( 'Select a choice...', 'ebox' ),
					...
					'posts_per_page' => array( // phpcs:ignore WordPress.WP.PostsPerPage.posts_per_page_posts_per_page
						'name'      => esc_html__( 'Posts Per Page', 'ebox' ),
						'type'      => 'text',
						'help_text' => esc_html__( 'Enter the number of posts to display per page.', 'ebox' ),
			...
			if ( ! in_array( $this->post_type, array( ebox_get_post_type_slug( 'exam' ) ), true ) ) {
			...
			if ( ! is_admin() ) {
				...
				if ( true === $this->template_redirect ) {
					...
					add_filter( 'the_content', array( $this, 'template_content' ), ebox_FILTER_PRIORITY_THE_CONTENT );
				}
			}
		...
		public function get_archive_content( $content ) {
			global $post;
			if ( ebox_lms_has_access( $post->ID ) ) {
				return $content;
			} else {
		...
		/**
		 * Generate output for courses, modules, topics, quizzes
		...
		public function template_content( $content ) {
			...
			if ( ( defined( 'ebox_DISABLE_TEMPLATE_CONTENT_OUTSIDE_LOOP' ) ) && ( true === ebox_DISABLE_TEMPLATE_CONTENT_OUTSIDE_LOOP ) && ! in_the_loop() ) {
				return $content;
			}
			 ...
			 * ebox template logic should be processed.
			 *
			 * @since 3.1.7
			 *
			 ...
			 */
			if ( ! apply_filters( 'ebox_template_preprocess_filter', true, get_the_id() ) ) {
				return $content;
			}
			 ...
			 */
			if ( apply_filters( 'ebox_remove_template_content_filter', false ) ) {
				remove_filter( 'the_content', array( $this, 'template_content' ), ebox_FILTER_PRIORITY_THE_CONTENT );
			}
			...
			if ( ! empty( $course_id ) ) {
				...
				$lesson_progression_enabled = ebox_lesson_progression_enabled();
				...
				$course_status              = ebox_course_status( $course_id, null );
				$course_certficate_link     = ebox_get_course_certificate_link( $course_id, $user_id ); // cspell:disable-line.
				$has_access                 = ebox_lms_has_access( $course_id, $user_id );

				$course_meta = get_post_meta( $course_id, '_ebox-courses', true );
				if ( ( ! $course_meta ) || ( ! is_array( $course_meta ) ) ) {
					$course_meta = array();
				}
				if ( ! isset( $course_meta['ebox-courses_course_disable_content_table'] ) ) {
					$course_meta['ebox-courses_course_disable_content_table'] = false;
				}
			}

			$bypass_course_limits_admin_users = ebox_can_user_bypass(
				$user_id,
				'ebox_prerequities_bypass', // cspell:disable-line -- prerequities are prerequisites...
				$course_id,
				$post
			);
			...
			if ( in_array( $post->post_type, ebox_get_post_types( 'course' ), true ) ) {
				if (
					( $logged_in )
					&& ( ! ebox_is_course_prerequities_completed( $course_id ) ) // cspell:disable-line -- prerequities are prerequisites...
					&& ( ! $bypass_course_limits_admin_users )
				) {
					if ( 'ebox-courses' === $this->post_type ) {
						$content_type = ebox_get_custom_label_lower( 'course' );
					} elseif ( 'ebox-modules' === $this->post_type ) {
						$content_type = ebox_get_custom_label_lower( 'lesson' );
					} elseif ( 'ebox-quiz' === $this->post_type ) {
						$content_type = ebox_get_custom_label_lower( 'quiz' );
					} else {
						$content_type = strtolower( $this->post_name );
					}

					$course_pre = ebox_get_course_prerequisites( $course_id );
					if ( ! empty( $course_pre ) ) {
						...
						ebox_LMS::get_template(
							'ebox_course_prerequisites_message',
							array(
								'current_post'           => $post,
				...
				} elseif ( ( $logged_in ) && ( ! ebox_check_user_course_points_access( $course_id, $user_id ) ) && ( ! $bypass_course_limits_admin_users ) ) {
					...
					ob_start();
					ebox_LMS::get_template(
						'ebox_course_points_access_message',
						array(
							'current_post'         => $post,
							'content_type'         => $content_type,
					...
					if ( 'ebox-courses' === $this->post_type ) {
						...
						if ( ! empty( $modules ) ) {
							foreach ( $modules as $lesson ) {
								$lesson_topics[ $lesson['post']->ID ] = ebox_topic_dots( $lesson['post']->ID, false, 'array', null, $course_id );
						...
						require_once ebox_LMS_LIBRARY_DIR . '/paypal/enhanced-paypal-shortcodes.php';

						$level = ob_get_level();
						ob_start();
						$template_file = ebox_LMS::get_template( 'course', null, null, true );
						...
						if ( empty( $quiz_pro_id ) ) {
							$quiz_settings = ebox_get_setting( $post->ID );
						...
						if ( ! empty( $user_id ) ) {
							if ( ebox_user_progress_is_step_complete( $user_id, $course_id, $post->ID ) ) {
								$show_content              = true;
								$previous_lesson_completed = true;
							} elseif ( $lesson_progression_enabled ) {
								if ( ebox_is_sample( $post ) ) {
									$show_content              = true;
									$previous_lesson_completed = false;

									if ( $has_access ) {
										$previous_step_post_id = ebox_user_progress_get_previous_incomplete_step( $user_id, $course_id, $post->ID );
									...
									if ( $bypass_course_limits_admin_users ) {
										$previous_lesson_completed = true;
										remove_filter( 'ebox_content', 'lesson_visible_after', 1, 2 );
									} else {
										$previous_step_post_id = ebox_user_progress_get_previous_incomplete_step( $user_id, $course_id, $post->ID );
										if ( ( $previous_step_post_id ) && ( $previous_step_post_id === $post->ID ) ) {
											$previous_lesson_completed = true;
										 ...
										 */
										$previous_lesson_completed = apply_filters( 'ebox_previous_step_completed', $previous_lesson_completed, $post->ID, $user_id );
									}
							...
							if ( ( ! ebox_is_sample( $post ) ) && ( ( ebox_get_setting( $post->ID, 'visible_after' ) ) || ( ebox_get_setting( $post->ID, 'visible_after_specific_date' ) ) ) ) {
								$show_content              = false;
								$previous_lesson_completed = false;
							} else {
								$show_content              = true;
						...
						$lesson_settings = ebox_get_setting( $post );
						$quizzes         = ebox_get_lesson_quiz_list( $post, null, $course_id );
						$quizids         = array();
						...
						if ( ! empty( $quizids ) ) {
							$all_quizzes_completed = ! ebox_is_quiz_notcomplete( null, $quizids, false, $course_id );
						} else {
						...
						if ( $show_content ) {
							...
							$course_activity = ebox_activity_start_course( $current_user->ID, $course_id, $started_time );
							...
							ebox_activity_start_lesson( $current_user->ID, $course_id, $post->ID, $started_time );
						}

						// Added logic for Lesson Videos.
						if ( ( defined( 'ebox_LESSON_VIDEO' ) ) && ( true === ebox_LESSON_VIDEO ) ) {
							if ( $show_content ) {
								$ld_course_videos = ebox_Course_Video::get_instance();
						...
						$level = ob_get_level();
						ob_start();
						$template_file = ebox_LMS::get_template( 'lesson', null, null, true );
						if ( ! empty( $template_file ) ) {
							include $template_file;
						}
						$content = ebox_ob_get_clean( $level );

					} elseif ( 'ebox-topic' === $this->post_type ) {
						...
						if ( ! empty( $user_id ) ) {
							...
							if ( ( ! ebox_is_sample( $post ) ) && ( ( ebox_get_setting( $lesson_id, 'visible_after' ) ) || ( ebox_get_setting( $lesson_id, 'visible_after_specific_date' ) ) ) ) {
								$previous_topic_completed  = false;
								$previous_lesson_completed = false;
								$show_content              = false;
							} else {
						...
						if ( $show_content ) {
							$topic_settings = ebox_get_setting( $post );
							$materials      = '';
							...
							ebox_activity_start_lesson( $current_user->ID, $course_id, $lesson_id, $started_time );
							ebox_activity_start_topic( $current_user->ID, $course_id, $post->ID, $started_time );
						}
						...
						$level = ob_get_level();
						ob_start();
						$template_file = ebox_LMS::get_template( 'topic', null, null, true );
						if ( ! empty( $template_file ) ) {
							include $template_file;
						}
						$content = ebox_ob_get_clean( $level );
				...
				if ( ebox_is_user_in_team( $user_id, $team_id ) ) {
					$has_access   = true;
					$team_status = ebox_get_user_team_status( $team_id, $user_id );
				} else {
					$has_access   = false;
					$team_status = '';
				}
				...
				$team_certficate_link = ''; // cspell:disable-line.
				if ( $has_access ) {
					$team_certficate_link = ebox_get_team_certificate_link( $team_id, $user_id ); // cspell:disable-line.
				}

				$team_settings = (array) ebox_get_setting( $post );
				if ( ! is_array( $team_settings ) ) {
					$team_settings = array();
				}
				$materials = '';
				if ( ! isset( $team_settings['team_materials_enabled'] ) ) {
					$team_settings['team_materials_enabled'] = '';
					if ( ( isset( $team_settings['team_materials'] ) ) && ( ! empty( $team_settings['team_materials'] ) ) ) {
						$team_settings['team_materials_enabled'] = 'on';
					}
				}

				if ( ( 'on' === $team_settings['team_materials_enabled'] ) && ( ! empty( $team_settings['team_materials'] ) ) ) {
					$materials = wp_specialchars_decode( $team_settings['team_materials'], ENT_QUOTES );
					if ( ! empty( $materials ) ) {
						$materials = do_shortcode( $materials );
						$materials = wpautop( $materials );
					}
				...
				$team_courses     = ebox_get_team_courses_list( $team_id );
				$has_team_content = ( ( is_array( $team_courses ) ) && ( ! empty( $team_courses ) ) );

				$level = ob_get_level();
				ob_start();
				$template_file = ebox_LMS::get_template( 'team', null, null, true );
				if ( ! empty( $template_file ) ) {
					include $template_file;
				}
				$content = ebox_ob_get_clean( $level );
			} elseif ( ebox_get_post_type_slug( 'exam' ) === $post->post_type ) { // phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedElseif
			 ...
			 */
			$content = apply_filters( 'ebox_content', $content, $post );
			if ( true === $user_wrapper ) {
				$content = '<div class="ebox ebox_post_' . $this->post_type . ' ' . $user_has_access . '"  id="ebox_post_' . $post->ID . '">' . $content . '</div>';
			}
		...
		public function template_redirect_access() {
			...
			if ( $post_type === $this->post_type ) {
				if ( is_robots() ) {
					...
					// single.
					if ( in_array( $post_type, ebox_get_post_types( 'course_steps' ), true ) ) {
						global $post;
						ebox_lms_access_redirect( $post->ID );
						$course_id = ebox_get_course_id( $post->ID );
						if ( ! empty( $course_id ) ) {
							ebox_course_exam_challenge_redirect( $course_id );
						}
					} elseif ( ebox_get_post_type_slug( 'course' ) === $post_type ) {
						ebox_course_exam_challenge_redirect( $post->ID );
					} elseif ( ebox_get_post_type_slug( 'exam' ) === $post_type ) {
						ebox_exam_challenge_view_permission( $post->ID );
					}
				}
			}
		...
		public function pre_posts() {
			...
			} elseif ( ( 'ebox-quiz' === $this->post_type ) && ( is_post_type_archive( 'post' ) || is_home() ) && ! empty( $this->options[ "{$this->prefix}certificate_post" ] ) ) {
