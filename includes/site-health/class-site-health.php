...
if ( ! class_exists( 'ebox_Site_Health' ) ) {
	/**
	 * Handles Site Health data.
	 *
	 * @since 4.4.1
	 */
	class ebox_Site_Health {
		private const SITE_HEALTH_KEY = 'ebox';
		...
		private function map_general_fields(): array {
			$license_is_valid = ebox_is_ebox_license_valid();
			$last_updated     = array_keys( ebox_data_upgrades_setting( 'version_history' ) )[0] ?? 0;

			return array(
				'version'           => array(
					'label' => __( 'Version', 'ebox' ),
					'value' => ebox_VERSION,
				),
				'last_updated'      => array(
					'label' => __( 'Last updated', 'ebox' ),
					'value' => $last_updated > 0 ? ebox_adjust_date_time_display( $last_updated ) : __( 'Never', 'ebox' ),
					'debug' => $last_updated,
				),
				'previous_version'  => array(
					'label' => __( 'Previous version', 'ebox' ),
					'value' => ebox_data_upgrades_setting( 'prior_version' ),
				),
				'license_validated' => array(
					'label' => __( 'License validated', 'ebox' ),
		...
		private function map_settings_fields(): array {
			$shared_steps_enabled  = ebox_is_course_shared_steps_enabled();
			$focus_mode_enabled    = $this->focus_mode_is_enabled();
			$is_rtl                = is_rtl();
			$registration_page_set = ebox_registration_page_is_set();
			$currency_code         = ebox_get_currency_code();
			$nested_urls_enabled   = 'yes' === ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Permalinks', 'nested_urls' );
			$paypal_configured     = ( new ebox_Paypal_IPN_Gateway() )->is_ready();
			$stripe_configured     = ( new ebox_Stripe_Gateway() )->is_ready();
			$razorpay_configured   = ( new ebox_Razorpay_Gateway() )->is_ready();

			return array(
				'shared_course_steps'            => array(
					'label' => __( 'Shared course steps', 'ebox' ),
					'value' => $this->bool_to_on_off_string( $shared_steps_enabled ),
					'debug' => $shared_steps_enabled,
				),
				'active_template'                => array(
					'label' => __( 'Active template', 'ebox' ),
					'value' => esc_html( ebox_Theme_Register::get_active_theme_name() ),
					'debug' => esc_attr( ebox_Theme_Register::get_active_theme_key() ),
				),
				'focus_mode'                     => array(
					'label' => __( 'Focus mode', 'ebox' ),
		...
		private function map_statistics_fields(): array {
			return array(
				'course_using_free_form_progression_count' => array(
					'label' => __( 'Number of courses using free form progression', 'ebox' ),
				...
				'course_using_linear_progression_count'    => array(
					'label' => __( 'Number of courses using linear progression', 'ebox' ),
				...
				'lesson_using_video_progression_count'     => array(
					'label' => __( 'Number of modules using video progression', 'ebox' ),
				...
				'lesson_using_drip_content_count'          => array(
					'label' => __( 'Number of modules using drip content', 'ebox' ),
				...
				'topic_using_drip_content_count'           => array(
					'label' => __( 'Number of topics using drip content', 'ebox' ),
		...
		private function map_constant_fields(): array {
			$ebox_debug        = defined( 'ebox_DEBUG' ) && ebox_DEBUG; // @phpstan-ignore-line -- Constant can be true/false.
			$ebox_script_debug = defined( 'ebox_SCRIPT_DEBUG' ) && ebox_SCRIPT_DEBUG; // @phpstan-ignore-line -- Constant can be true/false.

			return array(
				'ebox_DEBUG'        => array(
					'label' => __( 'ebox_DEBUG', 'ebox' ),
					'value' => $this->bool_to_yes_no_string( $ebox_debug ),
					'debug' => $ebox_debug,
				),
				'ebox_SCRIPT_DEBUG' => array(
					'label' => __( 'ebox_SCRIPT_DEBUG', 'ebox' ),
					'value' => $this->bool_to_yes_no_string( $ebox_script_debug ),
					'debug' => $ebox_script_debug,
				),
			);
		}
		...
		private function get_enrolled_user_count(): int {
			...
			if ( $open_course_query->found_posts > 0 ) {
				return ebox_students_enrolled_count();
			}
			...
			return (int) $wpdb->get_var(
				...
				// The 2nd select is: user ids that were "enrolled" via team -> course access.
				$wpdb->prepare(
					"
					...
					$wpdb->esc_like( 'ebox_team_users_' ) . '%',
					'ebox_team_enrolled_',
					$wpdb->esc_like( 'ebox_team_enrolled_' ) . '%'
				)
			);
		}
		...
		private function bool_to_on_off_string( bool $value ): string {
			return $value ? __( 'On', 'ebox' ) : __( 'Off', 'ebox' );
		}
		...
		private function bool_to_yes_no_string( bool $value ): string {
			return $value ? __( 'Yes', 'ebox' ) : __( 'No', 'ebox' );
		}
		...
		private function focus_mode_is_enabled(): bool {
			if ( ! ebox_is_active_theme( 'ld30' ) ) {
				return false;
			}

			return 'yes' === ebox_Settings_Section::get_section_setting(
		...
		private function map_meta_value_from_setting( string $setting_key, string $setting_value ): string {
			// Regular expressions are explained in includes/admin/classes-filters/class-ebox-admin-filter-meta.php.
