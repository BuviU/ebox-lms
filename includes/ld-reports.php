<?php
/**
 * ebox Reports functions
 *
 * @since 2.3.0
 *
 * @package ebox\Reports
 */
 ...
 */
function ebox_get_users_query( $query_args = array() ) {
	 ...
	 */
	$query_args = apply_filters( 'ebox_get_users_query_args', $query_args );
	if ( ! empty( $query_args ) ) {
 ...
 * team leader, it will show the only user within the leader's teams. For admin, it will
 * show all users.
 *
 * @since 2.3.0
 *
 ...
 */
function ebox_get_report_user_ids( $user_id = 0, $query_args = array() ) {
	if ( empty( $user_id ) ) {
	...
	if ( ebox_is_admin_user( $user_id ) ) {
		if ( ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_General_Admin_User', 'reports_include_admin_users' ) != 'yes' ) {
			$query_args['role__not_in'] = 'administrator';
		}
	} elseif ( ebox_is_team_leader_user( $user_id ) ) {
		$include_user_ids = ebox_get_team_leader_teams_users( $user_id );

		// Even though we have the users ids from the ebox_get_team_leader_teams_users() we need to validate them
	 ...
	 */
	$query_args      = apply_filters( 'ebox_get_report_users_query_args', $query_args );
	$report_user_ids = ebox_get_users_query( $query_args );
	/**
	 * Filters list of get report user ids.
	 *
	 * @param array|null $report_user_ids An array of report user ids.
	 */
	return apply_filters( 'ebox_get_report_user_ids', $report_user_ids );
}
 ...
 */
function ebox_get_courses_count( $query_args = array(), $return_field = 'found_posts' ) {
	$return = 0;
	 ...
	 */
	$query_args = apply_filters( 'ebox_courses_count_args', $query_args );
...
/**
 * Gets the count of pending ebox-assignment posts.
 *
 * @since 2.3.0
 *
 ...
 *               otherwise the specified `WP_Query` return field.
 */
function ebox_get_assignments_pending_count( $query_args = array(), $return_field = 'found_posts' ) {
	$return = 0;
	...
	// added logic for non-admin user like team leaders who will only see a sub-set of assignments.
	$user_id = get_current_user_id();
	if ( ebox_is_team_leader_user( $user_id ) ) {
		...
		if ( ! empty( $team_ids ) && is_array( $team_ids ) ) {
			foreach ( $team_ids as $team_id ) {
				$team_users = ebox_get_teams_user_ids( $team_id );

				if ( ! empty( $team_users ) && is_array( $team_users ) ) {
					foreach ( $team_users as $team_user_id ) {
						$user_ids[ $team_user_id ] = $team_user_id;
					}
				}
	 ...
	 */
	$query_args = apply_filters( 'ebox_get_assignments_pending_count_query_args', $query_args );
...
/**
 * Gets the link to admin assignments(ebox-assignment) posts listing.
 *
 ...
 */
function ebox_admin_get_assignments_listing_link( $link_args = array() ) {
...
/**
 * Gets the link to admin pending assignments(ebox-assignment) posts listing.
 *
 * @since 2.3.0
 *
 * @return string  The URL to pending assignments admin page with filters.
 */
function ebox_admin_get_assignments_pending_listing_link() {
	return ebox_admin_get_assignments_listing_link( 'approval_status=0' );
}


/**
 * Gets the count of pending Essays(ebox-essays) posts.
 *
 * @since 2.3.0
 *
 ...
 *               otherwise the specified `WP_Query` return field.
 */
function ebox_get_essays_pending_count( $query_args = array(), $return_field = 'found_posts' ) {
	$return = 0;
	 ...
	 */
	$query_args = apply_filters( 'ebox_get_essays_pending_count_query_args', $query_args );
...
/**
 * Gets the link to admin Essays(ebox-essays) posts listing.
 *
 ...
 */
function ebox_admin_get_essays_listing_link( $link_args = array() ) {
...
/**
 * Gets the link to admin pending Essays(ebox-essays) posts listing.
 *
 * @since 2.3.0
 *
 * @return string The URL to essays admin page with filters.
 */
function ebox_admin_get_essays_pending_listing_link() {
	return ebox_admin_get_essays_listing_link( 'post_status=not_graded' );
}
 ...
 */
function ebox_students_enrolled_count( $user_query_args = array() ) {
	...
	$user_query_args = apply_filters(
		'ebox_students_enrolled_count_qrgs', // cspell:disable-line.
...
/**
 * Gets the list or count of team users for a team leader.
 *
 * @param int     $user_id     Optional. Team leader user ID. Defaults to the current user ID. Default 0.
 * @param boolean $by_team    Optional. Whether to get user IDs or count sorted by team. Default false.
 ...
 */
function ebox_get_team_leader_teams_users( $user_id = 0, $by_team = false, $totals_only = false ) {
	...
	if ( ebox_is_team_leader_user( $user_id ) ) {
		...
		if ( ! empty( $team_ids ) ) {
			...
			foreach ( $team_ids as $team_id ) {
				...
				if ( true == $by_team ) {
					if ( true == $totals_only ) {
						$user_ids[ $team_id ] = count( $team_user_ids );
					} else {
						$user_ids[ $team_id ] = $team_user_ids;
					}
				} else {
					if ( true == $totals_only ) {
						$user_ids += count( $team_user_ids );
					} else {
						$user_ids = array_merge( $user_ids, $team_user_ids );
					}
				}
			}
		}
...
/**
 * Gets the list or count of team courses for a team leader.
 *
 * @param int     $team_leader_user_id Optional. Team leader user ID. Defaults to the current user ID. Default 0.
 * @param boolean $by_team             Optional. Whether to get user IDs or count sorted by team. Default false.
 * @param boolean $totals_only          Optional. Whether to get the only count of courses. Default false.
 *
 * @return int|array An array of user IDs or user count.
 */
function ebox_get_team_leader_teams_courses( $team_leader_user_id = 0, $by_team = false, $totals_only = false ) {
	...
	if ( empty( $team_leader_user_id ) ) {
		$team_leader_user_id = get_current_user_id();
	}

	if ( ebox_is_team_leader_user( $team_leader_user_id ) ) {

		$team_ids = ebox_get_administrators_team_ids( $team_leader_user_id );
		...
		if ( ! empty( $team_ids ) ) {
			...
			foreach ( $team_ids as $team_id ) {
				...
				if ( true == $by_team ) {
					if ( true == $totals_only ) {
						$course_ids[ $team_id ] = count( $team_course_ids );
					} else {
						$course_ids[ $team_id ] = $team_course_ids;
					}
				} else {
					if ( true == $totals_only ) {
						$course_ids += count( $team_course_ids );
					} else {
						$course_ids = array_merge( $course_ids, $team_course_ids );
					}
				}
 ...
 *
 * This function will query the new ebox_course_user_activity table for user/course Activity.
 *
 ...
 */
function ebox_reports_get_activity( $query_args = array(), $current_user_id = 0 ) {
	global $wpdb, $ebox_post_types;
	...
	$defaults = array(
		// array or comma lst of team ids to use in query. Default is all teams.
		'team_ids'                   => '',
		...
		// array or comma list of LD specific post types. See $ebox_post_types for possible values.
		'post_types'                  => '',

		// array or comma list of post statuses. See $ebox_post_types for possible values.
		'post_status'                 => '',
		...
		'date_format'                 => apply_filters( 'ebox_date_time_formats', get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) ),
	...
	// Clean the team_ids arg.
	if ( '' != $query_args['team_ids'] ) {
		if ( ! is_array( $query_args['team_ids'] ) ) {
			$query_args['team_ids'] = explode( ',', $query_args['team_ids'] );
		}
		$query_args['team_ids'] = array_map( 'trim', $query_args['team_ids'] );
	}
	...
	if ( '' != $query_args['post_types'] ) {
		...
		$query_args['post_types'] = array_intersect( $query_args['post_types'], $ebox_post_types );
	} else {
		// If not provides we set this to our internal defined ebox_post_types.
		$query_args['post_types'] = $ebox_post_types;
	}
	...
	if ( ( '' == $query_args['team_ids'] ) && ( '' == $query_args['post_ids'] ) && ( '' == $query_args['user_ids'] ) ) {
		// If no filters were provided.
		// If the view user is a team leader we just return all the activity for all the managed users.
		if ( ebox_is_team_leader_user( $current_user_id ) ) {
			$query_args['user_ids'] = ebox_get_team_leader_teams_users( $current_user_id );
		}
	} else {
		if ( ! ebox_is_team_leader_user( $current_user_id ) ) {
			if ( ebox_is_admin_user( $current_user_id ) ) {
				// If the team_ids parameter is passed in we need to determine the course_ids contains in the team_ids.
				if ( '' != $query_args['team_ids'] ) {
					$query_args['post_ids'] = ebox_get_teams_courses_ids( $current_user_id, $query_args['team_ids'] );
				}
			} else {
				/**
				 * If the user if not a team leader and not admin then abort until we have added support for those roles.
				 * return $activity_results;
				 */
				...
				if ( empty( $query_args['post_ids'] ) ) {
					$query_args['post_ids'] = ebox_user_get_enrolled_courses( get_current_user_id() );
					if ( empty( $query_args['post_ids'] ) ) {
						return $activity_results;
					}
				}
	...
	if ( ( true != $query_args['suppress_filters_all'] ) && ( true != $query_args['suppress_filters_query_args'] ) ) {
		 ...
		 */
		$query_args = apply_filters( 'ebox_get_activity_query_args', $query_args );
	}
	...
	if ( ( true != $query_args['suppress_filters_all'] ) && ( true != $query_args['suppress_filters_query_str'] ) ) {
		 ...
		 * @param array  $query_args      An array of user query arguments.
		 */
		$sql_str_fields = apply_filters( 'ebox_user_activity_query_fields', $sql_str_fields, $query_args );
		 ...
		 * @param array  $query_args     An array of user query arguments.
		 */
		$sql_str_tables = apply_filters( 'ebox_user_activity_query_tables', $sql_str_tables, $query_args );
		 ...
		 * @param array  $query_args      An array of user query arguments.
		 */
		$sql_str_joins = apply_filters( 'ebox_user_activity_query_joins', $sql_str_joins, $query_args );
		 ...
		 * @param array  $query_args      An array of user query arguments.
		 */
		$sql_str_where = apply_filters( 'ebox_user_activity_query_where', $sql_str_where, $query_args );
		 ...
		 * @param array  $query_args      An array of user query arguments.
		 */
		$sql_str_order = apply_filters( 'ebox_user_activity_query_order', $sql_str_order, $query_args );
		 ...
		 * @param array  $query_args      An array of user query arguments.
		 */
		$sql_str_limit = apply_filters( 'ebox_user_activity_query_limit', $sql_str_limit, $query_args );
	}
	...
	if ( true != $query_args['suppress_filters_query_str'] ) {
		 ...
		 * @param array $query_args An array of user query arguments.
		 */
		$sql_str = apply_filters( 'ebox_user_activity_query_str', $sql_str, $query_args );
	}
		...
		if ( ( ! is_wp_error( $activity_query_results ) ) && ( count( $activity_query_results ) ) ) {
			...
			foreach ( $activity_results['results'] as &$result_item ) {
				...
				if ( true == $query_args['include_meta'] ) {
					$result_item->activity_meta = ebox_get_activity_meta_fields( $result_item->activity_id, $query_args['meta_fields'] );
				}
			}
		} else {
 ...
 */
function ebox_report_course_users_progress( $course_id = 0, $user_query_args = array(), $activity_query_args = array() ) {
	...
	if ( ! empty( $course_id ) ) {
		...
		if ( ( ! isset( $activity_query_args['user_ids'] ) ) || ( empty( $activity_query_args['user_ids'] ) ) ) {
			$course_user_query = ebox_get_users_for_course( intval( $course_id ), $user_query_args );
 ...
 */
function ebox_report_clear_user_activity_by_types( $user_id = 0, $activity_types = '' ) {
	$activity_ids = ebox_report_get_activity_by_user_id( $user_id, $activity_types );
	if ( ! empty( $activity_ids ) ) {
		ebox_report_clear_by_activity_ids( $activity_ids );
	}
}
 ...
 *                              following: 'access', 'course', 'lesson', 'topic', 'quiz'. Default empty.
 */
function ebox_report_clear_post_activity_by_types( $post_id = 0, $activity_types = '' ) {
	$activity_ids = ebox_report_get_activity_by_post_id( $post_id, $activity_types );
	if ( ! empty( $activity_ids ) ) {
		ebox_report_clear_by_activity_ids( $activity_ids );
	}
}
 ...
 */
function ebox_report_clear_by_activity_ids( $activity_ids = array() ) {
	global $wpdb;
 ...
 * @since 2.5.0
 */
function ebox_activity_clear_mismatched_users() {
	global $wpdb;
 ...
 * @since 2.5.0
 */
function ebox_activity_clear_mismatched_posts() {
	global $wpdb;
	...
	if ( ! empty( $process_posts ) ) {
		foreach ( $process_posts as $post_id ) {
			ebox_report_clear_post_activity_by_types( $post_id );
		}
	}
}
 ...
 */
function ebox_report_get_activity_by_user_id( $user_id = 0, $activity_types = '' ) {
	global $wpdb;
 ...
 * @return array|void Returns an array of activity IDs.
 */
function ebox_report_get_activity_by_post_id( $post_id = 0, $activity_types = '' ) {
	global $wpdb;
 ...
 */
function ebox_report_user_courses_progress( $user_id = 0, $course_query_args = array(), $activity_query_args = array() ) {
	...
	if ( ( ! isset( $activity_query_args['post_ids'] ) ) || ( empty( $activity_query_args['post_ids'] ) ) ) {
		$activity_query_args['post_ids'] = ebox_user_get_enrolled_courses( intval( $user_id ), $course_query_args );
	}
 ...
 */
function ebox_get_user_quiz_attempts( $user_id = 0, $quiz_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_get_user_quiz_attempts_count( $user_id, $quiz_id ) {
	$quiz_attempts = ebox_get_user_quiz_attempts( $user_id, $quiz_id );
 ...
 */
function ebox_get_user_quiz_attempts_time_spent( $user_id, $quiz_id ) {
	$total_time_spent = 0;

	$attempts = ebox_get_user_quiz_attempts( $user_id, $quiz_id );
 ...
 */
function ebox_get_user_course_attempts( $user_id = 0, $course_id = 0 ) {
	global $wpdb;
 ...
 */
function ebox_get_user_course_attempts_time_spent( $user_id = 0, $course_id = 0 ) {
	$total_time_spent = 0;

	$attempts = ebox_get_user_course_attempts( $user_id, $course_id );
 ...
 * @return array
 */
function ebox_get_activity_meta_fields( $activity_id = 0, $activity_meta_keys = array() ) {
	global $wpdb;
 ...
 */
function ebox_get_activity_human_time_diff( $activity_started = 0, $activity_completed = 0, $minimum_diff = 60 ) {
