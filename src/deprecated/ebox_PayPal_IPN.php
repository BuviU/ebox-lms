<?php
/**
 * Deprecated. Use ebox_Paypal_IPN_Gateway instead.
...
_deprecated_file(
	basename( __FILE__ ),
	'4.5.0',
	esc_html( ebox_LMS_PLUGIN_DIR . '/includes/payments/gateways/class-ebox-paypal-ipn-gateway.php' )
);

if ( ! class_exists( 'ebox_PayPal_IPN' ) ) {
	/**
	 * Class to create the instance.
	 *
	 * @deprecated 4.5.0
	 */
	class ebox_PayPal_IPN {
		...
		public function is_ready(): bool {
			...
			$settings = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_PayPal' );
		...
		public static function ipn_process() {
			...
			if ( self::$ipn_transaction_post_id > 0 ) {
				...
				do_action( 'ebox_transaction_created', self::$ipn_transaction_post_id );
			}
		...
		public static function ipn_init_post_data() {
			...
			self::ipn_debug( 'ebox Version: ' . ebox_VERSION ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
			...
			switch ( self::$hash_action ) {
				case 'return-success':
					...
					if ( isset( self::$hash_user_meta_values['product_id'] ) ) {
						...
						if ( ebox_get_post_type_slug( 'course' ) === self::$ipn_transaction_data['post_type'] ) {
							self::$ipn_transaction_data['course_id'] = $product_id;
						} elseif ( ebox_get_post_type_slug( 'team' ) === self::$ipn_transaction_data['post_type'] ) {
							self::$ipn_transaction_data['team_id'] = $product_id;
						}
					}
					...
					$redirect_url = ebox_paypal_get_purchase_success_redirect_url( $product_id );
				...
				case 'return-cancel':
					...
					$redirect_url = ebox_paypal_get_purchase_cancel_redirect_url( $product_id );
		...
		/**
		 * Load ebox general PayPal Settings.
		 *
		 * @since 3.2.3
		 * @deprecated 4.5.0
		 */
		public static function ipn_init_settings() {
			...
			self::$ld_paypal_settings = ebox_Settings_Section::get_section_settings_all( 'ebox_Settings_Section_PayPal' );
			...
			self::ipn_debug( 'ebox Paypal Settings<pre>' . print_r( self::$ld_paypal_settings, true ) . '</pre>' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
		...
		public static function ipn_init_listener() {
			...
			if ( true !== self::$ld_debug_enabled ) {
				...
				if ( ! file_exists( ebox_LMS_LIBRARY_DIR . '/paypal/ipnlistener.php' ) ) {
					$message = 'Required IPN listener file not found ' . ebox_LMS_LIBRARY_DIR . '/paypal/ipnlistener.php';
					self::ipn_debug( $message );
					self::ipn_exit( '', true, $message, 404 );
				}

				if ( ! class_exists( 'IpnListener' ) ) {
					require ebox_LMS_LIBRARY_DIR . '/paypal/ipnlistener.php';
				}

				$ebox_paypal_ipn_listener = new IpnListener();
				 ...
				 * @since 2.2.1.2
				 *
				 * @param Object  $ebox_paypal_ipn_listener An instance of IpnListener Class.
				 */
				do_action_ref_array( 'ebox_ipnlistener_init', array( &$ebox_paypal_ipn_listener ) );

				self::ipn_debug( 'IPN Listener Loaded' );

				if ( ! empty( self::$ld_paypal_settings['paypal_sandbox'] ) ) {
					self::ipn_debug( 'PayPal Sandbox Enabled.' );
					$ebox_paypal_ipn_listener->use_sandbox = true;
				} else {
					self::ipn_debug( 'PayPal Live Enabled.' );
					$ebox_paypal_ipn_listener->use_sandbox = false;
				}

				try {
					self::ipn_debug( 'Checking IPN Post Method.' );
					$ebox_paypal_ipn_listener->requirePostMethod();
					$ebox_paypal_ipn_verified = $ebox_paypal_ipn_listener->processIpn();
					self::ipn_debug( 'IPN Post method check completed.' );
					if ( ! $ebox_paypal_ipn_verified ) {
						/**
		...
		public static function ipn_process_post_data() {
			...
			if ( ebox_get_post_type_slug( 'course' ) === self::$ipn_transaction_data['post_type'] ) {
				...
				self::$ipn_transaction_data['post_type_prefix'] = 'team';
				self::$ipn_transaction_data['team_id']         = absint( self::$ipn_transaction_data['post_id'] );
				self::ipn_debug( 'Purchased Team access [' . self::$ipn_transaction_data['post_id'] . ']' );
			} else {
			...
			if ( empty( self::$ipn_transaction_data['post_id'] ) ) {
				$message = 'Invalid \'post_id\' in IPN data. Unable to determine related Course/Team post.';
				self::ipn_debug( $message );
				self::ipn_exit( '', true, $message, 422 );
			}
			...
			if ( in_array( self::$ipn_transaction_data['txn_type'], array( 'web_accept', 'subscr_payment' ), true ) ) {
				$post_settings = ebox_get_setting( self::$ipn_transaction_data['post_id'] );
		...
		public static function ipn_debug( $msg = '' ) {
			...
			if ( ! empty( $msg ) ) {
				if ( '---' === $msg ) {
					...
					$datetime = ebox_adjust_date_time_display( time(), 'Y-m-d H:i:s' );
				}
		...
		public static function ipn_exit( $redirect_url = '', $error = false, $message = '', $status_code = 200 ) {
			...
			if ( ! empty( self::$ipn_transaction_log ) ) {
				...
				if ( ! empty( $transaction_post_id ) ) {
					 ...
					 */
					if ( apply_filters( 'ebox_paypal_save_processing_log', true ) ) {
		...
		public static function ipn_init_transaction() {
			...
			if ( empty( self::$ipn_transaction_post_id ) ) {
				if ( ( isset( self::$hash_user_meta_values['transaction_id'] ) ) && ( ! empty( self::$hash_user_meta_values['transaction_id'] ) ) ) {
					...
					if ( ( $transaction_post ) && ( is_a( $transaction_post, 'WP_Post' ) ) && ( ebox_get_post_type_slug( 'transaction' ) === $transaction_post->post_type ) ) {
		...
		public static function ipn_get_transaction_title() {
			...
			// translators: placeholders: Course/Team Label, Course/Team Post title, Transaction Label, User name.
				esc_html_x( '%1$s %2$s %3$s By %4$s', 'placeholders: Course/Team Label, Course/Team Post title, Transaction Label, User name', 'ebox' ),
		...
		protected static function get_transaction_post_label() {
			...
			if ( ! empty( self::$ipn_transaction_data['post_id'] ) ) {
				if ( ebox_get_post_type_slug( 'course' ) === self::$ipn_transaction_data['post_type'] ) {
					$transaction_post_label = ebox_get_custom_label( 'course' );
				} elseif ( ebox_get_post_type_slug( 'team' ) === self::$ipn_transaction_data['post_type'] ) {
					$transaction_post_label = ebox_get_custom_label( 'team' );
				}
			}
		...
		protected static function get_transaction_type_label() {
			...
			if ( 'web_accept' === self::$ipn_transaction_data['txn_type'] ) {
				$transaction_type_label = esc_html__( 'Purchased', 'ebox' );
			} elseif ( 'subscr_signup' === self::$ipn_transaction_data['txn_type'] ) {
				$transaction_type_label = esc_html__( 'Subscription Signup', 'ebox' );
			} elseif ( 'subscr_payment' === self::$ipn_transaction_data['txn_type'] ) {
				$transaction_type_label = esc_html__( 'Subscription Payment', 'ebox' );
			} elseif ( 'subscr_cancel' === self::$ipn_transaction_data['txn_type'] ) {
				$transaction_type_label = esc_html__( 'Subscription Cancel', 'ebox' );
			} elseif ( 'subscr_failed' === self::$ipn_transaction_data['txn_type'] ) {
				$transaction_type_label = esc_html__( 'Subscription Failed', 'ebox' );
			} elseif ( 'subscr_eot' === self::$ipn_transaction_data['txn_type'] ) {
				$transaction_type_label = esc_html__( 'Subscription EOT', 'ebox' );
			}
		 ...
		 * Grant user access.
		 *
		 * This function will enroll the user in the Course/Team.
		 *
		 * @since 3.2.3
		 * @deprecated 4.5.0
		 */
		public static function ipn_grant_access() {
			...
			if ( ( ! empty( self::$ipn_transaction_data['user_id'] ) ) && ( ! empty( self::$ipn_transaction_data['post_id'] ) ) ) {
				if ( ebox_get_post_type_slug( 'course' ) === get_post_type( self::$ipn_transaction_data['post_id'] ) ) {
					...
					if ( ! ebox_lms_has_access( self::$ipn_transaction_data['post_id'], self::$ipn_transaction_data['user_id'] ) ) {
						// record in course.
						ld_update_course_access( absint( self::$ipn_transaction_data['user_id'] ), self::$ipn_transaction_data['post_id'] );
						ebox_send_purchase_success_email( absint( self::$ipn_transaction_data['user_id'] ), self::$ipn_transaction_data['post_id'] );
						self::ipn_debug( 'User enrolled in Course success.' );

					} else {
						self::ipn_debug( 'User previously enrolled in Course.' );
					}
				} elseif ( ebox_get_post_type_slug( 'team' ) === get_post_type( self::$ipn_transaction_data['post_id'] ) ) {
					self::ipn_debug( 'Starting to give Team access: User ID[' . absint( self::$ipn_transaction_data['user_id'] ) . '] Team[' . self::$ipn_transaction_data['post_id'] . ']' );

					$user_enrolled = get_user_meta( self::$ipn_transaction_data['user_id'], 'ebox_team_users_' . self::$ipn_transaction_data['post_id'], true );
					if ( ! $user_enrolled ) {
						// record in team.
						ld_update_team_access( absint( self::$ipn_transaction_data['user_id'] ), self::$ipn_transaction_data['post_id'] );
						ebox_send_purchase_success_email( absint( self::$ipn_transaction_data['user_id'] ), self::$ipn_transaction_data['post_id'] );
						self::ipn_debug( 'User enrolled in Team success.' );
					} else {
						self::ipn_debug( 'User previously enrolled Team.' );
					}
				}
			}
		}
