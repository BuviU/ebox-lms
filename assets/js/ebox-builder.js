jQuery(function ($) {
  ...
  if (jQuery("#ebox_builder_box_wrap").length) {
    course_id = jQuery("#ebox_builder_box_wrap").data("ld-course-id");
    ld_typenow = jQuery("#ebox_builder_box_wrap").data("ld-typenow");
  }

  if (jQuery("#ebox_builder_box_wrap .ebox_selectors").length) {
    // set the first selector set to open by default.
    jQuery(
      "#ebox_builder_box_wrap .ebox_selectors .ebox-selector-container"
    ).each(function (index, item) {
      if (index == 0) {
        jQuery(".ebox-selector-post-listing", item).slideDown(
          "slow",
          function () {
            jQuery(
        ...
        jQuery(".ebox-selector-post-listing", item).hide();
        jQuery(
          ".ebox-selector-header .ld-course-builder-action-show-hide",
          item
    ...
    /*
		function course_builder_box_wrap_resize( e ) {
			jQuery( window ).resize(function() {
				...
				//var ld_course_builder_box_wrap_width = jQuery( '#ebox_builder_box_wrap' ).width();
				//console.log('ld_course_builder_box_wrap_width[%o]', ld_course_builder_box_wrap_width);
				//if ( ld_course_builder_box_wrap_width < 500 ) {
				//	jQuery( '#ebox_builder_box_wrap .ebox_selectors' ).css( 'width', '40%' );
				//	jQuery( '#ebox_builder_box_wrap .ebox_builder_items' ).css( 'width', 'calc(60% - 5px)' );
				//} else {
				//	jQuery( '#ebox_builder_box_wrap .ebox_selectors' ).css( 'width', '30%' );
				//	jQuery( '#ebox_builder_box_wrap .ebox_builder_items' ).css( 'width', 'calc(70% - 5px)' );
				//}
				...
				//var ld_course_builder_selectors_width = jQuery( '#ebox_builder_box_wrap .ebox_selectors' ).outerWidth();
				...
				//jQuery( '#ebox_builder_box_wrap .ebox_builder_items' ).width(new_Width-25);
    ...
    function selector_show_hide(event, action) {
      ...
      var post_listing = jQuery(parent_items_el).find(
        ".ebox-selector-post-listing"
      );
      ...
      if (action == "open") {
        jQuery(post_listing).slideDown("slow", function () {
          jQuery(event.currentTarget).removeClass(
    ...
    jQuery("#ebox_builder_box_wrap").on(
      ...
      selector_pager_click
    );
    function selector_pager_click(e) {
      e.stopImmediatePropagation();
      var selector_container = jQuery(e.currentTarget).parents(
    ...
    function selector_pager_process(
      selector_paged,
      selector_type,
      selector_container
    ) {
        ...
        success: function (reply_data) {
          if (typeof reply_data !== "undefined") {
            if (typeof reply_data.selector_pager !== "undefined") {
              jQuery(".pager-info", selector_container).html(
            ...
            if (typeof reply_data.selector_rows !== "undefined") {
              jQuery(
                "ul.ebox-selector-post-listing",
                selector_container
              ).html(reply_data.selector_rows);
              jQuery(
                "ul.ebox-selector-post-listing li",
                selector_container
              ).draggable(draggable_objects[selector_type]);
      ...
      jQuery(
        ...
        if (typeof selector_container !== "undefined") {
          ...
          if (typeof selector_type !== "undefined") {
            ...
            jQuery(".ebox-selector-pager", selector_container).hide();
            jQuery(".ebox-selector-search", selector_container).width(
              "100%"
            );
            ...
            search_interval_ref = setInterval(function () {
              ...
              if (!search_el.is(":focus")) {
                ...
                jQuery(".ebox-selector-pager", selector_container).show();
                jQuery(".ebox-selector-search", selector_container).width(
                  search_width + "px"
                );
              }
  ...
  function selector_search(selector_container, search_value) {
    if (
      typeof selector_container !== "undefined" &&
  ...
  function build_html_element_map() {
    ...
    if (
      ...
      jQuery(
        ...
        .children()
        .each(function (lesson_item_idx, lesson_item) {
          var lesson_id = jQuery(lesson_item).data("ld-id");

          builder_items_count += 1;
          builder_items["ebox-modules:" + lesson_id.toString()] = {};

          jQuery(".ld-course-builder-lesson-topic-items", lesson_item)
            .children()
            .each(function (topic_item_idx, topic_item) {
              var topic_id = jQuery(topic_item).data("ld-id");

              builder_items_count += 1;
              builder_items["ebox-modules:" + lesson_id.toString()][
                "ebox-topic:" + topic_id.toString()
              ] = {};

              jQuery(".ld-course-builder-topic-quiz-items", topic_item)
                .children()
                .each(function (topic_quiz_item_idx, topic_quiz_item) {
                  var topic_quiz_id = jQuery(topic_quiz_item).data("ld-id");
                  builder_items["ebox-modules:" + lesson_id.toString()][
                    "ebox-topic:" + topic_id.toString()
                  ]["ebox-quiz:" + topic_quiz_id.toString()] = {};
                });
            });

          jQuery(".ld-course-builder-lesson-quiz-items", lesson_item)
            .children()
            .each(function (lesson_quiz_item_idx, lesson_quiz_item) {
              var lesson_quiz_id = jQuery(lesson_quiz_item).data("ld-id");
              builder_items["ebox-modules:" + lesson_id.toString()][
                "ebox-quiz:" + lesson_quiz_id.toString()
              ] = {};
            });
        });
      ...
      jQuery(
        ...
        .children()
        .each(function (quiz_item_idx, quiz_item) {
          var quiz_id = jQuery(quiz_item).data("ld-id");
          has_global_quizzes = true;
          builder_items["ebox-quiz:" + quiz_id.toString()] = {};
        });
      ...
      jQuery(
        ...
        .children()
        .each(function (question_item_idx, question_item) {
          var question_id = jQuery(question_item).data("ld-id");
          var points = jQuery(".ld-course-builder-points", question_item).data(
            "ld-points"
          );

          builder_items_points =
            parseInt(builder_items_points) + parseInt(points);

          builder_items_count += 1;
          builder_items["ebox-question:" + question_id.toString()] = {};
        });
    }
  ...
  draggable_objects["ebox-modules"] = {
    cancel: ".ld-disabled",
  ...
  draggable_objects["ebox-question"] = {
    cancel: ".ld-disabled",
  ...
  function ld_course_builder_draggable_helper(selected) {
    var container = $("<div/>").attr("id", "ld-selector-draggable-team");
  ...
  sortable_objects["ebox-modules"] = {
    //containment: "#ebox_builder_box_wrap",
    ...
    stop: function (event, ui) {
      ...
      if (element_id == "ld-selector-draggable-team") {
        var new_modules = "";

        var ld_selected = jQuery(
        ...
        ui.item.children("li").each(function () {
          ...
          new_modules =
            ...
            element_ld_id +
            '">' +
            jQuery(inner_el).html() +
            "</div>";

          if (typeof element_ld_id !== "undefined") {
            selector_set_item_disabled("ebox-modules", element_ld_id, true);
            selector_set_item_selected("ebox-modules", element_ld_id, false);
          }
        });
        jQuery(ui.item).replaceWith(new_modules);

        // after the Lesson(s) added we reset the jQuery UI sortable logic to include the new child items
        jQuery(
          "#ebox_builder_box_wrap .ld-course-builder-topic-items"
        ).sortable(sortable_objects["ebox-topic"]);
        jQuery(
          "#ebox_builder_box_wrap .ld-course-builder-quiz-items"
        ).sortable(sortable_objects["ebox-quiz"]);

        ld_selected = jQuery.unique(ld_selected);
        jQuery(
          "#ebox_builder_box_wrap .ebox_selectors .ebox-selector-container-ebox-modules"
        ).data("ld-selected", ld_selected);
    ...
    stop: function (event, ui) {
      ...
      if (element_id == "ld-selector-draggable-team") {
        ...
        ui.item.children("li").each(function () {
          ...
          if (typeof element_ld_id !== "undefined") {
            selector_set_item_disabled("ebox-topic", element_ld_id, true);
            jQuery(
    ...
    stop: function (event, ui) {
      ...
      if (element_id == "ld-selector-draggable-team") {
        ...
        ui.item.children("li").each(function () {
          ...
          if (typeof element_ld_id !== "undefined") {
            selector_set_item_disabled("ebox-quiz", element_ld_id, true);
            jQuery(
        ...
        update_builder_items_element();
      } else if (element_id == "ld-draggable-builder-team") {
  ...
  sortable_objects["ebox-question"] = {
    //containment: "#ebox_builder_box_wrap",
    ...
    stop: function (event, ui) {
      ...
      if (element_id == "ld-selector-draggable-team") {
        ...
        ui.item.children("li").each(function () {
          ...
          if (typeof element_ld_id !== "undefined") {
            selector_set_item_disabled("ebox-question", element_ld_id, true);
            jQuery(
  ...
  // Draggable / Sortable modules
  jQuery(
    "#ebox_builder_box_wrap .ebox_selectors ul#ebox-selector-post-listing-ebox-modules li"
  ).draggable(draggable_objects["ebox-modules"]);
  jQuery(
    "#ebox_builder_box_wrap .ebox_builder_items .ld-course-builder-lesson-items"
  ).sortable(sortable_objects["ebox-modules"]);

  // Draggable / Sortable Topics
  jQuery(
    "#ebox_builder_box_wrap .ebox_selectors ul#ebox-selector-post-listing-ebox-topic li"
  ).draggable(draggable_objects["ebox-topic"]);
  jQuery("#ebox_builder_box_wrap .ld-course-builder-topic-items").sortable(
    sortable_objects["ebox-topic"]
  );
  ...
  ).draggable(draggable_objects["ebox-quiz"]);
  jQuery("#ebox_builder_box_wrap .ld-course-builder-quiz-items").sortable(
    sortable_objects["ebox-quiz"]
  );
  ...
  ).draggable(draggable_objects["ebox-question"]);
  jQuery(
    "#ebox_builder_box_wrap .ld-course-builder-question-items"
  ).sortable(sortable_objects["ebox-question"]);
  ...
  function update_builder_items_element() {
    ...
    ).html(builder_items_count);

    jQuery(
      "#ebox_builder_box_wrap .ebox_builder_items .ebox_builder_points_total_value"
    ).html(builder_items_points);

    var builder_elements_str = JSON.stringify(builder_elements);
    if ("ebox-courses" == ld_typenow) {
      jQuery("#ebox_builder_box_wrap input#ebox_builder_data").val(
        builder_elements_str
      );
    } else if ("ebox-quiz" == ld_typenow) {
      jQuery("#ebox_builder_box_wrap input#ebox_builder_data").val(
        builder_elements_str
      );
    }
  ...
  jQuery("#ebox_builder_box_wrap .ebox_builder_items").on(
    "click",
    ".ld-course-builder-action-show-hide",
    show_hide_sub_elements
  );
  function show_hide_sub_elements(e, action) {
    e.stopImmediatePropagation();

    var builder_item = jQuery(e.currentTarget)
      .parents(".ld-course-builder-item")
      .first();
    var closest_sub_items = jQuery(builder_item)
      .find(".ld-course-builder-sub-items")
      .first();
  ...
  jQuery("#ebox_builder_box_wrap .ebox_builder_items").on(
    ...
    builder_remove_element
  );

  function builder_remove_element(event) {
    var remove_item = jQuery(event.currentTarget).closest(
    ...
    var confirm_title = get_builder_asset_message(
      "confirm_remove_" + remove_item_type
    );
    if (confirm_title === "") {
    ...
    var remove_item_title = jQuery(
      "span.ld-course-builder-title-text",
      remove_item
    ).html();
  ...
  function builder_trash_element(event) {
    ...
    var confirm_title = get_builder_asset_message(
      "confirm_trash_" + trash_item_type
    );
    if (confirm_title === "") {
      confirm_title = "Confirm delete";
    }
  ...
  window.onbeforeunload = function () {
    if (
      typeof ebox_builder_assets[ld_typenow].post_data.builder_editor !==
        "undefined" &&
      ebox_builder_assets[ld_typenow].post_data.builder_editor ===
        "classic"
    ) {
      if (cb_form_unsaved === true) {
        var ebox_unload_message = get_builder_asset_message(
          "ebox_unload_message"
        );
        if (
          typeof ebox_unload_message !== "undefined" &&
          ebox_unload_message !== ""
        ) {
          return ebox_unload_message;
        }
      }
    }
  };
  ...
  var parent_form = jQuery("#ebox_builder_box_wrap").parents("form");
  ...
  if (touchEvent == "hover") {
    jQuery("#ebox_builder_box_wrap .ebox_builder_items").on(
      ...
      builder_step_show_element_actions
    );
    jQuery("#ebox_builder_box_wrap .ebox_builder_items").on(
      "mouseleave",
      ".ld-course-builder-actions",
      builder_step_hide_element_actions
    );
  ...
  function builder_step_show_element_actions(event) {
    ...
    var sub_actions = jQuery(
      ".ld-course-builder-sub-actions",
      element_move_action
    );
    if (typeof sub_actions !== "undefined") {
      // We need to check if the element is being dragged. If it is being dragged it will have
      // a parent container div#ld-selector-draggable-team in that case we hide the sub-actions.
      var parent_id = jQuery(element_move_action)
        .closest("div#ld-selector-draggable-team")
        .attr("id");
  ...
  jQuery("#ebox_builder_box_wrap").on(
    ...
    builder_edit_title_show_pencil
  );
  jQuery("#ebox_builder_box_wrap").on(
    "mouseleave",
    ".ld-course-builder-title",
    builder_edit_title_hide_pencil
  );
  function builder_edit_title_show_pencil(event) {
    ...
    var parent_el = jQuery(event.currentTarget).closest(
      ".ld-course-builder-item"
    )[0];
  ...
  function builder_edit_title(event) {
    ...
    if (typeof parent_el !== "undefined") {
      ...
      jQuery(".ld-course-builder-title-text", title_el).html(
        '<input type="text" value="' + title_org + '" />'
      );
      ...
      function handle_title_save(event_confirm) {
        ...
        var parent_el = jQuery(event_confirm.currentTarget).parents(
          ".ld-course-builder-item"
        );

        jQuery(parent_el).removeClass("ld-course-builder-title-edit");
        var title_new = jQuery(
          ".ld-course-builder-title-text input",
          title_el
        ).val();
            ...
            jQuery.ajax({
              ...
              success: function (reply_data) {
                if (
                  ...
                  var selector_item = jQuery(
                    "#ebox_builder_box_wrap .ebox_selectors li#ld-post-" +
                      step_id
                  );
                  jQuery(".ld-course-builder-title-text", selector_item).html(
                    title_new
                  );

                  //var builder_item = jQuery( '#ebox_builder_box_wrap .ebox_builder_items div#ld-course-builder-lesson-item-'+step_id );
                  var builder_item = jQuery(
                    "#ebox_builder_box_wrap .ebox_builder_items #ld-post-" +
                      step_id
                  );
                  jQuery(".ld-course-builder-title-text", builder_item)
                    .first()
                    .html(title_new);
                }
              },
      ...
      jQuery(title_el).on(
        ...
        function (event_cancel) {
          ...
          var parent_el = jQuery(event_cancel.currentTarget).parents(
            ".ld-course-builder-item"
          );
          ...
          if (jQuery(".ld-course-builder-action-show-hide", parent_el).length) {
            var selector_container =
              course_buider_get_parent_selector_container(parent_el); // cspell:disable-line
            if (
              typeof selector_container !== "undefined" &&
              jQuery(selector_container).hasClass("ebox_builder_items")
            ) {
  ...
  function course_buider_get_parent_selector_container(el) {
    ...
    if (typeof el !== "undefined") {
      var selector_container = jQuery(el).closest(".ebox_selectors");
      if (
        typeof selector_container !== "undefined" &&
        selector_container.length > 0
      ) {
        return selector_container[0];
      }
      var builder_container = jQuery(el).closest(".ebox_builder_items");
      if (
        typeof builder_container !== "undefined" &&
        builder_container.length > 0
      ) {
  ...
  function selector_update_empty(selector_type) {
    if (selector_type !== "") {
      ...
      if (
        jQuery(
          "ul#ebox-selector-post-listing-" + selector_type + " li",
          selector_container
        ).length
      ) {
        jQuery(".ebox-selector-pager", selector_container).show();
        jQuery(".ebox-selector-search", selector_container).show();
      } else {
        jQuery(".ebox-selector-pager", selector_container).hide();
        jQuery(".ebox-selector-search", selector_container).hide();
      }
    }
  }
    ...
    if (selector_type !== "" && selected_id !== "") {
      ...
      if (changed == true) {
        jQuery(
          "#ebox_builder_box_wrap .ebox_selectors .ebox-selector-container-" +
            selector_type
        ).data("ld-selected", disabled_items);
      }
    }
  }
  ...
  function selector_get_type_from_item(el) {
    if (el !== "") {
      var selector_container = jQuery(el).closest(
        ".ebox-selector-container"
      );
      if (typeof selector_container !== "undefined") {
        var selector_type = jQuery(selector_container).data("ld-type");
  ...
  function builder_add_new_step(event) {
    ...
    if (
      !jQuery(event.currentTarget).hasClass(
      ...
      var selector_container = jQuery(event.currentTarget).closest(
        ".ebox-selector-container"
      );
      if (typeof selector_container !== "undefined") {
        var selector_show_hide = jQuery(selector_container).find(
          ".ld-course-builder-action-show-hide"
        );
        if (
          jQuery(selector_show_hide).hasClass("ld-course-builder-action-show")
        ) {
          jQuery(
            ".ld-course-builder-action-show-hide",
            selector_container
        ...
        if (typeof selector_type !== "undefined") {
          ...
          var first_item = jQuery(
            ".ebox-row-placeholder li",
            selector_container
          ).clone();
  ...
  function builder_trigger_new_steps_ajax() {
    if (ld_builder_new_step_ajax_pid == "") {
      if (jQuery("#ebox_builder_box_wrap .ld-new-step").length) {
        var new_items_set = {};

        //jQuery( '#ebox_builder_box_wrap .ld-new-step' ).slice(0, 2).each( function( item_idx, item_el ) {
            ...
            success: function (reply_data) {
              if (typeof reply_data !== "undefined") {
                if (typeof reply_data.new_steps !== "undefined") {
                  jQuery.each(
                    reply_data.new_steps,
                    function (old_step_id, new_step_set) {
                      if (
                        jQuery("#ebox_builder_box_wrap #" + old_step_id)
                          .length
                      ) {
                        if (
                          jQuery(
                            "#ebox_builder_box_wrap #" + old_step_id
                          ).hasClass("ld-new-step")
                        ) {
                          jQuery(
                            "#ebox_builder_box_wrap #" + old_step_id
                          ).removeClass("ld-new-step");
                          jQuery(
                            "#ebox_builder_box_wrap #" + old_step_id
                          ).attr("data-ld-id", new_step_set.post_id);
                          jQuery(
                            "#ebox_builder_box_wrap #" +
                              old_step_id +
                            ...
                            "href",
                            new_step_set.view_url.replace(/&amp;/g, "&")
                          );
                          jQuery(
                            "#ebox_builder_box_wrap #" +
                              old_step_id +
                            ...
                            "href",
                            new_step_set.edit_url.replace(/&amp;/g, "&")
                          );
                          jQuery(
                            "#ebox_builder_box_wrap #" + old_step_id
                          ).attr("id", "ld-post-" + new_step_set.post_id);
                        }
                      }
                    }
                  );
  ...
  function get_builder_asset_message(message_key) {
    if (typeof ebox_builder_assets[ld_typenow].messages !== "undefined") {
      if (
        typeof message_key !== "undefined" &&
        typeof ebox_builder_assets[ld_typenow].messages[message_key] !==
          "undefined"
      ) {
        return ebox_builder_assets[ld_typenow].messages[message_key];
      }
      return ebox_builder_assets[ld_typenow].messages;
    }
  }

  function get_builder_asset_post_data(data_key) {
    if (typeof ebox_builder_assets[ld_typenow].post_data !== "undefined") {
      if (
        typeof data_key !== "undefined" &&
        typeof ebox_builder_assets[ld_typenow].post_data[data_key] !==
          "undefined"
      ) {
        return ebox_builder_assets[ld_typenow].post_data[data_key];
      }
      return ebox_builder_assets[ld_typenow].post_data;
    }
  }
