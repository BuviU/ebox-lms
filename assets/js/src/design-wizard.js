...
jQuery(function () {
	...
	jQuery(document).on('click', '.next-button', function (e) {
		...
		if (step === 1) {
			...
			nonce = eboxDesignWizard.ajax_init_nonce;
					...
					},
					{
						...
						click() {
							window.location.href =
								eboxDesignWizard.ebox_setup_url;
						},
					},
				],
			});
	...
	jQuery(document).on('click', '.exit', function (e) {
		...
		window.location.href = eboxDesignWizard.ebox_setup_url;
	});
			...
			if (response.success) {
				if (!response.data.complete) {
					...
					const actionsTemplate =
						eboxDesignWizard.templates.actions_success;
	...
	if (currentStep) {
		...
		switch (currentStep) {
			...
			case 'astra_sites_set_site_data-site_colors':
				...
				data.security = eboxDesignWizard.ajax_set_data_nonce;
		...
		jQuery
			.ajax({
				...
				data: {
					action: step,
					_ajax_nonce: eboxDesignWizard.ajax_nonce,
					...data,
				},
				success(response) {
					if (currentStep === 'astra-wxr-import') {
						...
						if (response.success) {
							...
							if (
								...
								message =
									eboxDesignWizard.messages
										.dw_error_default;
							}

							message =
								eboxDesignWizard.messages.dw_error_prefix +
								': ' +
								message;
							...
							const actionsTemplate =
								eboxDesignWizard.templates.actions_error;
	...
	if (currentStep) {
		...
		jQuery
			.ajax({
				...
				data: {
					action: step,
					security: eboxDesignWizard.ajax_kadence_security_nonce,
					...data,
				},
				success(response) {
					const ajaxUrl = '',
...
function getFontDetails(theme, key) {
	...
	if (
		Object.prototype.hasOwnProperty.call(
			eboxDesignWizard.fonts[theme][key],
			'details'
		)
	) {
		details = eboxDesignWizard.fonts[theme][key].details;
	}
...
function getPaletteDetails(theme, key, colorScheme = '') {
	...
	if (colorScheme.length > 0) {
		palettes = eboxDesignWizard.palettes[theme][colorScheme][key];
	} else {
		palettes = eboxDesignWizard.palettes[theme][key];
	}
