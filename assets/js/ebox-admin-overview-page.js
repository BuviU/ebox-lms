...
/**
 * @namespace ebox.admin.overview
 */
window.ebox.admin.overview = {
	...
	openFirstIncompleteAccordionPanel() {
		...
		accordionPanels.forEach( function( panel ) {
			if ( panel.classList.contains( '-completed' ) ) {
				...
				if ( markCompleteButton && markCompleteButton.innerHTML !== eboxOverviewPageData['mark_incomplete'] ) {
					markCompleteButton.innerHTML = eboxOverviewPageData['mark_incomplete'];
				};
				return;
			}
	...
	markComplete: function( e ) {
			if ( 'BUTTON' === e.target.tagName && ( e.target.classList.contains( 'ld-bootcamp__mark-complete--toggle' ) || e.target.classList.contains( 'ld-bootcamp__mark-complete--toggle-indicator' ) ) ) {
				...
				eboxOverviewPageData['mark_complete'] === markCompleteButton.innerHTML ?
						markCompleteButton.innerHTML = eboxOverviewPageData['mark_incomplete'] :
						markCompleteButton.innerHTML = eboxOverviewPageData['mark_complete'];
	...
	searchSupportSite: function( e ) {
		if ( 'FORM' === e.target.tagName && e.target.id === 'ld-overview--search-documentation-form' ) {
			...
			if ( searchTerm && searchTerm.value.length > 3 ) {
				window.open( 'https://www.ebox.com/support/docs/?s=' + searchTerm.value, '_blank' );
			}
		}
	},
	eventListeners: function() {
			document.querySelector( '.ld-bootcamp' ).addEventListener( 'click', ebox.admin.overview.toggleAccordion.bind( ebox.admin.overview ) );
			document.querySelector( 'body' ).addEventListener( 'click', ebox.admin.overview.toggleBootcamp.bind( ebox.admin.overview ) );
			document.querySelector( 'body' ).addEventListener( 'click', ebox.admin.overview.markComplete.bind( ebox.admin.overview ) );
			document.querySelector( 'body' ).addEventListener( 'submit', ebox.admin.overview.searchSupportSite.bind( ebox.admin.overview ) );
	}
};

document.addEventListener( 'DOMContentLoaded', ebox.admin.overview.eventListeners );
document.addEventListener( 'DOMContentLoaded', ebox.admin.overview.openFirstIncompleteAccordionPanel.bind( ebox.admin.overview ) );
document.addEventListener( 'DOMContentLoaded', ebox.admin.overview.maybeDisplayShowBootcampButton.bind( ebox.admin.overview ) );
