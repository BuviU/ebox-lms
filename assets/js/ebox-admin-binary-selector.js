jQuery( function() {
	var selectors_array = [];
	jQuery( '.ebox-binary-selector' ).each( function() {
		var selector_id = jQuery( this ).prop( 'id' );
		if ( typeof selector_id !== 'undefined' ) {
			selectors_array[selector_id] = new ebox_binary_selector( this );
			selectors_array[selector_id].init();
		}
	} );
} );

function ebox_binary_selector( selector_div ) {
	var self = this;
	...
	this.init_actions = function() {
		jQuery( '.ebox-binary-selector-section-middle', self.selector_div ).on( 'click', 'a.ebox-binary-selector-button-add', self.add_selected_items );
		jQuery( '.ebox-binary-selector-section-middle', self.selector_div ).on( 'click', 'a.ebox-binary-selector-button-remove', self.remove_selected_items );
	};

	this.init_pager = function() {
		if ( jQuery( 'ul.ebox-binary-selector-pager', self.selector_div ).length ) {
			jQuery( 'ul.ebox-binary-selector-pager', self.selector_div ).each( function( e ) {
				var section_pager_el = this;

				if ( jQuery( section_pager_el ).hasClass( 'ebox-binary-selector-pager-left' ) ) {
					var position = 'left';
				} else if ( jQuery( section_pager_el ).hasClass( 'ebox-binary-selector-pager-right' ) ) {
					var position = 'right';
				}
			...
			jQuery( '.ebox-binary-selector-section ul.ebox-binary-selector-pager a', self.selector_div ).on( 'click', function( e ) {
				e.preventDefault();
	...
	this.init_search = function() {
		if ( jQuery( '.ebox-binary-selector-search', self.selector_div ).length ) {
			...
			jQuery( '.ebox-binary-selector-search', self.selector_div ).on( 'focus', function() {
				var search_el = this;
				...
				if ( jQuery( search_el ).hasClass( 'ebox-binary-selector-search-left' ) ) {
					var position = 'left';
				} else if ( jQuery( search_el	).hasClass( 'ebox-binary-selector-search-right' ) ) {
					var position = 'right';
				} else {
					return;
				}
				...
				interval_ref = setInterval( function() {
					...
					if ( search_values[position].current_value == '' ) {
						if ( search_values[position].query_data.query_vars.search != search_values[position].current_value ) {
							...
							var post_data = {
								action: 'ebox_binary_selector_pager',
								nonce: self.nonce,
							...
							jQuery.ajax( {
								...
								success: function( reply_data ) {
									if ( typeof reply_data.html_options !== 'undefined' ) {
										jQuery( '.ebox-binary-selector-items', section_el ).empty().append( reply_data.html_options );
									}
									var section_pager_el = jQuery( 'ul.ebox-binary-selector-pager', section_el );
							...
							//if (jQuery('.ebox-binary-selector-pager', section_el).length) {
							//	jQuery('.ebox-binary-selector-pager', section_el).show();
							//}
						}
					} else {
						//if (jQuery('.ebox-binary-selector-pager', section_el).length) {
						//	jQuery('.ebox-binary-selector-pager', section_el).hide();
						//}
							...
							jQuery.ajax( {
								...
								success: function( reply_data ) {
									if ( typeof reply_data.html_options !== 'undefined' ) {
										...
										jQuery( '.ebox-binary-selector-items', section_el ).empty().append( '' );
									}

									if ( typeof reply_data.pager !== 'undefined' ) {
										var section_pager_el = jQuery( '.ebox-binary-selector-pager', section_el );
										if ( typeof section_pager_el !== 'undefined' ) {
											jQuery( section_pager_el ).show();
											self.selector_data[position].pager = reply_data.pager;
						...
						if ( ! jQuery( '.ebox-binary-selector-search', self.section_el ).is( ':focus' ) ) {
							clearInterval( interval_ref );
						}
					}
				}, search_timeout );
	...
	this.update_legend = function() {
		var options_size = jQuery( 'select.ebox-binary-selector-items-left option', self.selector_div ).length;
		jQuery( '.ebox-binary-selector-legend-left span.items-total-count', self.selector_div ).html( options_size );

		//var total_items_count = self.get_selector_data_element('total_items');
		//jQuery('.ebox-binary-selector-legend-left span.items-total-count', self.selector_div).html(total_items_count);

		var options_size = jQuery( 'select.ebox-binary-selector-items-right option', self.selector_div ).length;
		//jQuery('.ebox-binary-selector-legend-right span.items-loaded-count', self.selector_div).html(options_size);

		//var selected_items_length = Object.keys(self.selected_items).length;
		jQuery( '.ebox-binary-selector-legend-right span.items-total-count', self.selector_div ).html( options_size );
	};
	...
	this.handle_pager = function( clicked_el ) {
		...
		var section_pager_el = jQuery( clicked_el ).parents( 'ul.ebox-binary-selector-pager' );
		if ( typeof section_pager_el === 'undefined' ) {
			return;
		}
		...
		if ( jQuery( clicked_el ).hasClass( 'ebox-binary-selector-pager-prev' ) ) {
			if ( selector_pager.current_page == 1 ) {
				return;
			}

			query_data.query_vars.paged = parseInt( selector_pager.current_page ) - 1;
		} else if ( jQuery( clicked_el ).hasClass( 'ebox-binary-selector-pager-next' ) ) {
	...
	this.update_pager = function( section_pager_el, section_pager_data ) {
		if ( typeof section_pager_el !== 'undefined' ) {
			...
			if ( ( current_page >= 1 ) && ( total_pages >= 1 ) ) {
				jQuery( '.ebox-binary-selector-pager-info span.current_page', section_pager_el ).html( current_page );
				jQuery( '.ebox-binary-selector-pager-info span.current_page', section_pager_el ).show();

				jQuery( '.ebox-binary-selector-pager-info span.total_pages', section_pager_el ).html( total_pages );
				jQuery( '.ebox-binary-selector-pager-info span.total_pages', section_pager_el ).show();

				jQuery( '.ebox-binary-selector-pager-info span.total_items span.total_items_count', section_pager_el ).html( total_items );
				jQuery( '.ebox-binary-selector-pager-info span.total_items', section_pager_el ).show();

				jQuery( '.ebox-binary-selector-pager-info', section_pager_el ).show();

				if ( current_page == 1 ) {
					jQuery( '.ebox-binary-selector-pager-prev a', section_pager_el ).hide();
				} else {
					jQuery( '.ebox-binary-selector-pager-prev a', section_pager_el ).show();
				}

				if ( current_page == total_pages ) {
					jQuery( '.ebox-binary-selector-pager-next a', section_pager_el ).hide();
				} else {
					jQuery( '.ebox-binary-selector-pager-next a', section_pager_el ).show();
				}
			} else {
				jQuery( '.ebox-binary-selector-pager-info span.current_page', section_pager_el ).hide();
				jQuery( '.ebox-binary-selector-pager-info span.total_pages', section_pager_el ).hide();
				jQuery( '.ebox-binary-selector-pager-prev a', section_pager_el ).hide();
				jQuery( '.ebox-binary-selector-pager-next a', section_pager_el ).hide();
			}
		}
	...
	this.init_lazy_load = function() {
		...
		//if (jQuery(self.selector_div).hasClass('ebox_lazy_load') ) {
	...
	this.add_selected_items = function( e ) {
		...
		var items_changed = [];
		if ( ( jQuery( '.ebox-binary-selector-items-left', self.selector_div ).length ) && ( jQuery( '.ebox-binary-selector-items-right', self.selector_div ).length ) ) {
			jQuery( '.ebox-binary-selector-items-left option:selected', self.selector_div ).each( function() {
				var option_left_el = jQuery( this );

				if ( ! option_left_el.hasClass( 'ebox-binary-selector-item-disabled' ) ) {
					jQuery( option_left_el, self.selector_div ).removeClass( 'ebox-binary-selector-item-selected' );
					jQuery( option_left_el, self.selector_div ).clone().appendTo( jQuery( '.ebox-binary-selector-items-right', self.selector_div ) );
					jQuery( option_left_el, self.selector_div ).addClass( 'ebox-binary-selector-item-disabled' );
	...
	this.remove_selected_items = function( e ) {
		...
		if ( ( jQuery( '.ebox-binary-selector-items-left', self.selector_div ).length ) && ( jQuery( '.ebox-binary-selector-items-right', self.selector_div ).length ) ) {
			jQuery( '.ebox-binary-selector-items-right option:selected', self.selector_div ).each( function() {
				...
				if ( jQuery( '.ebox-binary-selector-items-left option[data-value="' + data_id + '"]', self.selector_div ).length ) {
					jQuery( '.ebox-binary-selector-items-left option[data-value="' + data_id + '"]', self.selector_div ).removeClass( 'ebox-binary-selector-item-disabled' );
					jQuery( '.ebox-binary-selector-items-left option[data-value="' + data_id + '"]', self.selector_div ).removeClass( 'ebox-binary-selector-item-selected' );
					jQuery( '.ebox-binary-selector-items-left option[data-value="' + data_id + '"]', self.selector_div ).prop( 'disabled', false );
				}
	...
	this.get_section_el = function( child_el ) {
		if ( typeof child_el !== 'undefined' ) {
			return jQuery( child_el ).parents( '.ebox-binary-selector-section' );
		}
	};
	...
	this.update_selected_form_element = function() {
		...
		//jQuery('.ebox-binary-selector-items-right li', self.selector_div).each(function() {
		jQuery('.ebox-binary-selector-items-right option', self.selector_div).each(function() {
		...
		jQuery('input.ebox-binary-selector-form-element', self.selector_div).val(JSON.stringify(self.selected_items));
...
*/
	this.get_selector_form_element = function( return_raw ) {
		...
		var selected_items = jQuery( 'input.ebox-binary-selector-form-element', self.selector_div ).val();
	...
	this.save_selector_form_element = function( ) {
		jQuery( 'input.ebox-binary-selector-form-element', self.selector_div ).val( JSON.stringify( self.selected_items ) );
		jQuery( 'input.ebox-binary-selector-form-changed', self.selector_div ).val( '1' );
	};
	...
	this.sort_right_options = function() {
		...
		/*
		var options = jQuery('.ebox-binary-selector-items-right li', self.selector_div);
	...
	this.lazy_load_items = function( query_data ) {
		...
		var post_data = {
			action: 'ebox_binary_selector_lazy_loader',
			nonce: self.nonce,
				...
				if ( typeof reply_data !== 'undefined' ) {
					if ( typeof reply_data.html_options !== 'undefined' ) {
						if ( reply_data.html_options != '' ) {
							jQuery( 'select.ebox-binary-selector-items-left', self.selector_div ).append( reply_data.html_options );
	...
	this.set_selected_items = function() {
		...
		jQuery( 'select.ebox-binary-selector-items-right', self.selector_div ).empty();
		...
		jQuery( 'select.ebox-binary-selector-items-left > option.ebox-binary-selector-item ', self.selector_div ).removeClass( 'ebox-binary-selector-item-disabled' );
		jQuery( 'select.ebox-binary-selector-items-left > option.ebox-binary-selector-item ', self.selector_div ).prop( 'selected', false );
		jQuery( 'select.ebox-binary-selector-items-left > option.ebox-binary-selector-item ', self.selector_div ).prop( 'disabled', false );

		//selected_items = JSON.parse(selected_items);

		jQuery.each( self.selected_items, function( index, item ) {
			jQuery( '.ebox-binary-selector-items-left option.ebox-binary-selector-item[value="' + item + '"]', self.selector_div ).each( function() {
				var option_left_el = jQuery( this );
				jQuery( option_left_el, self.selector_div ).clone().appendTo( jQuery( '.ebox-binary-selector-items-right', self.selector_div ) );
				jQuery( option_left_el, self.selector_div ).addClass( 'ebox-binary-selector-item-disabled' );
				jQuery( option_left_el, self.selector_div ).prop( 'selected', false );
