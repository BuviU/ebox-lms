...
if ( ! class_exists( 'ebox_Theme_Register' ) ) {

	/**
	 * Class to create the settings section.
	 */
	class ebox_Theme_Register {
		...
		/**
		 * Theme Base URL.
		 * Relative to ebox plugin directory.
		 *
		 * @since 4.0.0
		 * @var string
		 */
		...
		final public static function load_themes_settings_sections() {
			if ( ! empty( self::$_instances ) ) {
				foreach ( self::$_instances as $theme_key => $theme_instance ) {
					 ...
					 */
					do_action( 'ebox_theme_settings_load', $theme_instance->get_theme_key() );
				}
				do_action( 'ebox_settings_themes_init' );
			}
		}
		...
		final public static function get_active_theme_instance() {
			$theme_key = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Courses_Themes', 'themes' );
			if ( empty( $theme_key ) ) {
				$themes = get_option( 'ebox_settings_courses_themes' );
			...
			if ( ! isset( self::$_instances[ $theme_key ] ) ) {
				...
				if ( 'new' === $ld_prior_version ) {
					$theme_key = ebox_DEFAULT_THEME;
				} else {
					$theme_key = ebox_LEGACY_THEME;
				}
			}
			return self::$_instances[ $theme_key ];
		}
		...
		final public static function load_active_theme() {
			...
			if ( ( $theme_instance ) && ( is_a( $theme_instance, 'ebox_Theme_Register' ) ) ) {
				 ...
				 * @param string $theme_key Theme key.
				 */
				do_action( 'ebox_theme_load', $theme_instance->get_theme_key() );
			}
		}
 ...
 * @return boolean true if theme_key is the active theme, otherwise false.
 */
function ebox_is_active_theme( $theme_key = '' ) {
	if ( ( ! empty( $theme_key ) ) && ( ebox_Theme_Register::get_active_theme_key() === $theme_key ) ) {
		return true;
	}
...
add_action(
	'after_setup_theme',
	function() {
		ebox_Theme_Register::init();
		ebox_Theme_Register::load_active_theme();
		ebox_Theme_Register::load_themes_settings_sections();
	},
	20
);
