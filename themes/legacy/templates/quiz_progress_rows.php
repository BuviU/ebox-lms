...
<?php foreach ( $quizzes as $k => $v ) : ?>
	...
	<?php
	if ( ( ! ( $quiz instanceof WP_Post ) ) || ( $quiz->post_type != 'ebox-quiz' ) ) {
		if ( ( isset( $v['pro_quizid'] ) ) && ( ! empty( $v['pro_quizid'] ) ) ) {
			$quiz_post_id = ebox_get_quiz_id_by_pro_quiz_id( intval( $v['pro_quizid'] ) );
	...
	$c = ebox_certificate_details( $v['quiz'], $user_id );
	...
	if ( isset( $v['course'] ) ) {
		...
		$quiz_course_id = ebox_get_course_id( $quiz, true );
	}
	?>
	...
	<?php if ( ! empty( $quiz_title ) ) : ?>
		<p id="ld-quiz-<?php echo $v['time']; ?>">
			<strong><a href="<?php echo esc_url( ebox_get_step_permalink( $quiz->ID, $quiz_course_id ) ); ?>"><?php echo $quiz_title; ?></a></strong>
			<?php
			if ( ( isset( $v['course'] ) ) && ( intval( $v['course'] ) != ebox_get_course_id( $quiz, true ) ) ) {
			...
			<?php
			if ( ( ( get_current_user_id() == $user_id ) || ( ebox_is_admin_user() ) || ( ebox_is_team_leader_user() ) ) && ( ! empty( $certificateLink ) ) ) {
				if ( ( ( isset( $v['pass'] ) ) && ( 1 == $v['pass'] ) ) &&
					...
					?>
						 - <a href='<?php echo esc_url( $certificateLink ); ?>' target='_blank'><?php echo esc_html__( 'Certificate', 'ebox' ); ?></a>
					<?php
				}
			}
			...
			if ( ( get_current_user_id() == $user_id ) || ( ebox_is_admin_user() ) || ( ebox_is_team_leader_user() ) ) {
				if ( ( ! isset( $v['statistic_ref_id'] ) ) || ( empty( $v['statistic_ref_id'] ) ) ) {
					$v['statistic_ref_id'] = ebox_get_quiz_statistics_ref_for_quiz_attempt( $user_id, $v );
				}
			...
			if ( isset( $v['m_edit_by'] ) ) {
				...
				if ( $manual_edit_user instanceof WP_User ) {
					$manual_edit_str = sprintf(
						...
						_x( 'Manual Edit by: %1$s on %2$s', 'placeholders: User, Date', 'ebox' ),
						$manual_edit_user->display_name,
						/** This filter is documented in includes/ld-misc-functions.php */
						date_i18n( apply_filters( 'ebox_date_time_formats', get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) ), $v['m_edit_time'] + get_option( 'gmt_offset' ) * 3600 )
					);

					?>
					<abbr title="<?php echo esc_attr( $manual_edit_str ); ?>"><?php esc_html_e( '(m)', 'ebox' ); ?></abbr>
					<?php
				}
			}
			?>
			...
			if ( current_user_can( 'wpProQuiz_edit_quiz' ) ) {
				?>
				<a href="<?php echo esc_url( add_query_arg( 'course_id', $quiz_course_id, get_edit_post_link( $quiz->ID ) ) ); ?>"><?php echo esc_html_x( '(edit)', 'profile edit quiz link label', 'ebox' ); ?></a>
				<?php if ( ebox_show_user_course_complete( $user_id ) ) { ?>
				<a class="remove-quiz" data-quiz-user-id="<?php echo (int) $user_id; ?>" data-quiz-nonce="<?php echo esc_attr( wp_create_nonce( 'remove_quiz_' . $user_id . '_' . $v['quiz'] . '_' . $v['time'] ) ); ?>" href="#" title="
																	 <?php
																		...
																		echo sprintf( esc_html_x( 'remove this %s item', 'placeholder: quiz', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'quiz' ) ) )
																		?>
					"><?php echo esc_html_x( '(remove)', 'profile remove quiz link label', 'ebox' ); ?></a>
					<?php
				}
			}
			?>
				...
				foreach ( $v['graded'] as $quiz_question_id => $graded ) {
					...
					if ( isset( $graded['post_id'] ) ) {
						...
						if ( $graded_post instanceof WP_Post ) {
							...
							echo /* $post_type_object_label_name .': '. */ get_the_title( $graded['post_id'] ) . ', ' . esc_html__( 'Status', 'ebox' ) . ': <span style="' . $graded_color . '">' . esc_html( $post_status_object_label ) . '</span>, ' . esc_html__( 'Points', 'ebox' ) . ': ' . esc_html( $graded['points_awarded'] );

							if ( current_user_can( 'edit_essays' ) ) {
								echo ' <a target="_blank" href="' . esc_url( get_edit_post_link( $graded['post_id'] ) ) . '">' . esc_html__( 'edit', 'ebox' ) . '</a>';
							}
							echo ' <a target="_blank" href="' . esc_url( get_permalink( $graded['post_id'] ) ) . '">' . esc_html__( 'view', 'ebox' ) . '</a>';

							echo ' <a target="_blank" href="' . esc_url( get_permalink( $graded['post_id'] ) ) . '#comments">' . esc_html__( 'comments', 'ebox' ) . ' ' . esc_html( get_comments_number( $graded['post_id'] ) ) . '</a>';
							echo '<br />';
						}
					}
				}
			...
			<?php if ( isset( $v['rank'] ) && is_numeric( $v['rank'] ) ) : ?>
				<?php echo esc_html__( 'Rank: ', 'ebox' ); ?> <?php echo esc_html( $v['rank'] ); ?>,
			<?php endif; ?>

			<?php echo esc_html__( 'Score ', 'ebox' ); ?><?php echo esc_html( $v['score'] ); ?> <?php echo esc_html__( ' out of ', 'ebox' ); ?> <?php
			...
			?>
			<?php echo esc_html__( ' question(s)', 'ebox' ); ?>

			<?php if ( isset( $v['points'] ) && isset( $v['total_points'] ) ) : ?>
				<?php echo esc_html__( ' . Points: ', 'ebox' ); ?> <?php echo esc_html( $v['points'] ); ?>/<?php echo esc_html( $v['total_points'] ); ?>
			<?php endif; ?>

			<?php echo esc_html__( ' on ', 'ebox' ); ?>
			<?php
			/** This filter is documented in includes/ld-misc-functions.php */
			echo date_i18n( apply_filters( 'ebox_date_time_formats', get_option( 'date_format' ) . ' ' . get_option( 'time_format' ) ), $v['time'] + get_option( 'gmt_offset' ) * 3600 );

			if ( is_admin() ) {
				echo ' <a onClick="return false;" title="' . esc_html__( 'copy link to use in [quizinfo] block.', 'ebox' ) . '" href="data:quizinfo:quiz:' . absint( $quiz->ID ) . ':user:' . absint( $user_id ) . ':time:' . absint( $v['time'] ) . '">' . esc_html__( '#', 'ebox' ) . '</a>';
			}
