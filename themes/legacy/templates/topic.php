...
?>
<?php if ( ! empty( $topics ) ) : ?>
	<div id='ebox_topic_dots-<?php echo esc_attr( $lesson_id ); ?>' class="ebox_topic_dots type-dots">

		<b>
		<?php
		printf(
			// translators: placeholder: Topic.
			esc_html_x( '%s Progress:', 'placeholder: Topic', 'ebox' ),
...
<div id="ebox_back_to_lesson"><a href='<?php echo esc_url( ebox_get_step_permalink( $lesson_id, $course_id ) ); ?>'>&larr;
													<?php
														echo ebox_get_label_course_step_back( get_post_type( $lesson_id ) );
													?>
	</a></div>
<?php } ?>
...
<?php if ( $lesson_progression_enabled && ! $previous_topic_completed ) : ?>
	...
	if ( ( ! empty( $previous_item ) ) && ( $previous_item instanceof WP_Post ) ) {
		...
		} elseif ( 'ebox-topic' === $previous_item->post_type ) {
			echo sprintf(
		...
		echo sprintf(
			...
			esc_html_x( 'Please go back and complete the previous %s.', 'placeholder: lesson', 'ebox' ),
...
<?php elseif ( $lesson_progression_enabled && ! $previous_lesson_completed ) : ?>
	...
	if ( ( ! empty( $previous_item ) ) && ( $previous_item instanceof WP_Post ) ) {
		...
		// translators: placeholder: lesson.
		echo sprintf( esc_html_x( 'Please go back and complete the previous %s.', 'placeholder: lesson', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'lesson' ) ) );
	}
	?>
	</span>
	<br />
...
<?php if ( $show_content ) : ?>
	<?php if ( ( isset( $materials ) ) && ( ! empty( $materials ) ) ) : ?>
		<div id="ebox_topic_materials" class="ebox_topic_materials">
			<h4>
			<?php
			// translators: placeholder: Topic.
			printf( esc_html_x( '%s Materials', 'placeholder: Topic', 'ebox' ), ebox_Custom_Label::get_label( 'topic' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			?>
			</h4>
	...
	<?php if ( ! empty( $quizzes ) ) : ?>
		<div id="ebox_quizzes" class="ebox_quizzes">
			<div id="quiz_heading"><span><?php echo ebox_Custom_Label::get_label( 'quizzes' ); ?></span><span class="right"><?php esc_html_e( 'Status', 'ebox' ); ?></span></div> <?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output ?>
