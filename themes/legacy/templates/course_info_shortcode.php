...
<div id="ld_course_info" class="ld_course_info" data-shortcode-atts="<?php echo $shortcode_atts_json; ?>">
	...
	<?php if ( ( 'profile.php' !== $pagenow ) && ( 'user-edit.php' !== $pagenow ) ) { ?>
		<?php if ( $courses_registered ) : ?>
			<div id='ld_course_info_mycourses_list' class="ld_course_info_mycourses_list">
				...
				echo sprintf( esc_html_x( 'You are registered for the following %s', 'placeholder: courses', 'ebox' ), esc_html( ebox_get_custom_label_lower( 'courses' ) ) );
				?>
				</h4>
	...
	if ( is_admin() ) {
		if ( function_exists( 'get_current_screen' ) ) {
			...
			if ( ( 'profile.php' === $pagenow ) || ( 'user-edit.php' === $pagenow ) || ( 'ebox-lms_page_team_admin_page' === $current_screen->id ) ) {
				echo do_shortcode( '[ld_user_course_points user_id="' . $user_id . '" context="profile"]' );

				if ( ( ebox_is_admin_user() ) || ( ( ebox_is_team_leader_user() ) && ( ebox_is_team_leader_of_user( get_current_user_id(), $user_id ) ) ) ) {
					?>
						<p><label for="ebox-course-points-user"><strong>
						<?php
						// translators: placeholder: Course.
						printf( esc_html_x( 'Extra %s points', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
						?>
						</strong></label> <input id="ebox-course-points-user" name="ebox_course_points" type="number" min="0" step="any" value="<?php echo ebox_format_course_points( get_user_meta( $user_id, 'course_points', true ) ); ?>" /><?php } ?></p>
						<?php
			}
		}
	}
	...
	<?php if ( ! empty( $course_progress ) ) : ?>
		<div id="course_progress_details" class="course_progress_details">
			...
			<?php
			// translators: placeholder: Course.
			printf( esc_html_x( '%s progress details:', 'placeholder: Course', 'ebox' ), ebox_Custom_Label::get_label( 'course' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
			?>
			</h4>
			<?php
	...
	<?php if ( $quizzes ) : ?>
		<div id="quiz_progress_details" class="quiz_progress_details">
			...
			echo sprintf( _x( 'You have taken the following %s:', 'placeholder: quizzes', 'ebox' ), ebox_get_custom_label_lower( 'quizzes' ) );
			?>
			</h4>
			...
			echo sprintf( _x( 'Are you sure that you want to remove this %s item?', 'placeholder: quiz', 'ebox' ), ebox_get_custom_label_lower( 'quiz' ) )
			?>
			</div>
