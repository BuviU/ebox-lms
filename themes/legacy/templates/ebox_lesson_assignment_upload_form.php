...
if ( ( isset( $course_step_post ) ) && ( $course_step_post instanceof WP_Post ) ) {
	...
	$post_settings = ebox_get_setting( $course_step_post );
	...
	$upload_message = sprintf( esc_html_x( 'Maximum upload file size: %s', 'placeholder: PHP file upload size', 'ebox' ), $php_max_upload );
	...
	if ( ( isset( $post_settings['assignment_upload_limit_extensions'] ) ) && ( ! empty( $post_settings['assignment_upload_limit_extensions'] ) ) ) {
		...
		if ( ! empty( $limit_file_exts ) ) {
			...
			$upload_message .= ' ' . sprintf( esc_html_x( 'Allowed file types: %s', 'placeholder: comma list of file extentions', 'ebox' ), implode( ', ', $limit_file_exts ) );
		}
	}
	...
	$ret .= '
			<table id="leardash_upload_assignment">
				<tr><td><u>' . esc_html__( 'Upload Assignment', 'ebox' ) . '</u></td></tr>
				<tr>
					<td>
						<form name="uploadfile" id="uploadfile_form" method="POST" enctype="multipart/form-data" action="" accept-charset="utf-8" >
							...
							<input type="hidden" name="MAX_FILE_SIZE" value="' . ebox_return_bytes_from_shorthand( $php_max_upload ) . '" />
							<input type="hidden" value="' . $course_step_post->ID . '" name="post"/>
							<input type="hidden" value="' . ebox_get_course_id( $course_step_post->ID ) . '" name="course_id"/>
							<input type="hidden" name="uploadfile" value="' . wp_create_nonce( 'uploadfile_' . get_current_user_id() . '_' . $course_step_post->ID ) . '"  />
							<input class="button-primary" type="submit" id="uploadfile_btn" value="' . esc_html__( 'Upload', 'ebox' ) . '"  onClick="this.form.submit(); this.disabled=true;;" />
						</form>
					</td>
				</tr>
