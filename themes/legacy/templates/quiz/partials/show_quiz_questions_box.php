						...
						ebox_LMS::get_template(
							...
							array(
								...
								'message'      => sprintf(
									...
									esc_html_x( '%1$s %2$s of %3$s', 'placeholder: question, question number, questions total', 'ebox' ),
						...
						echo wp_kses_post(
							ebox_LMS::get_template(
								...
								array(
									...
									'message'      => '<span>' . $index . '</span>. ' . esc_html__( 'Question', 'ebox' ),
						...
						echo wp_kses_post(
							ebox_LMS::get_template(
								...
								array(
									...
									'message'      => sprintf( esc_html_x( '%s point(s)', 'placeholder: total quiz points', 'ebox' ), '<span>' . $question->getPoints() . '</span>' ),
							...
							echo wp_kses_post(
								ebox_LMS::get_template(
									...
									array(
										...
										'message'      => sprintf( esc_html_x( 'Category: %s', 'placeholder: Quiz Category', 'ebox' ), '<span>' . esc_html( $question->getCategoryName() ) . '</span>' ),
				...
				<div class="wpProQuiz_question" style="margin: 10px 0px 0px 0px;">
							...
							echo wp_kses_post(
								ebox_LMS::get_template(
									...
									array(
										...
										'message'      => esc_html__( 'Sort elements', 'ebox' ),
									)
								)
							);
							?>
							...
							foreach ( $answer_array as $v_idx => $v ) {
								...
								<li class="wpProQuiz_questionListItem" data-pos="<?php echo esc_attr( $datapos ); ?>">
									...
									if ( $question->getAnswerType() === 'single' || $question->getAnswerType() === 'multiple' ) {
										...
										$question_answer_data = ebox_question_free_get_answer_data( $v, $question );
										...
										$cloze_data   = ebox_question_cloze_fetch_data( $v->getAnswer() );
										...
										$assessment_data = ebox_question_assessment_fetch_data( $v->getAnswer(), $quiz->getId(), $question->getId() );
										...
										$assessment_output = ebox_question_assessment_prepare_output( $assessment_data );
										...
										if ( $v->getGradedType() === 'text' ) :
												...
												placeholder="<?php echo wp_kses_post( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
													ebox_LMS::get_template(
														...
														array(
															...
															'message'      => esc_html__( 'Type your response here', 'ebox' ),
														)
													)
											...
											<?php elseif ( $v->getGradedType() === 'upload' ) : ?>
													...
													echo wp_kses_post(
														ebox_LMS::get_template(
															...
															array(
																...
																'message'      => '<p>' . esc_html__( 'Upload your answer to this question.', 'ebox' ) . '</p>',
															)
														)
													);
												?>
												<form enctype="multipart/form-data" method="post" name="uploadEssay">
													...
													<input type="submit" id='uploadEssaySubmit_<?php echo esc_attr( $question->getId() ); ?>' value="<?php esc_html_e( 'Upload', 'ebox' ); ?>" />
													<input type="hidden" id="_uploadEssay_nonce_<?php echo esc_attr( $question->getId() ); ?>" name="_uploadEssay_nonce" value="<?php echo esc_attr( wp_create_nonce( 'ebox-upload-essay-' . $question->getId() ) ); ?>" />
											...
											<?php else : ?>
												<?php esc_html_e( 'Essay type not found', 'ebox' ); ?>
											<?php endif; ?>
													...
													echo wp_kses_post(
														ebox_LMS::get_template(
															...
															array(
																...
																'message'      => esc_html__( 'This response will be awarded full points automatically, but it can be reviewed and adjusted after submission.', 'ebox' ),
															)
														)
													);
													?>
														...
														echo wp_kses_post(
															ebox_LMS::get_template(
																...
																array(
																	...
																	'message'      => esc_html__( 'This response will be awarded full points automatically, but it will be reviewed and possibly adjusted after submission.', 'ebox' ),
																)
															)
														);
													?>
														...
														echo wp_kses_post(
															ebox_LMS::get_template(
																...
																array(
																	...
																	'message'      => esc_html__( 'This response will be reviewed and graded after submission.', 'ebox' ),
																)
															)
														);
													?>
					...
					<?php if ( $question->getAnswerType() === 'sort_answer' ) { ?>
						<div class="wpProQuiz_questionList_containers">
							<p><?php esc_html_e( 'View Answers', 'ebox' ); ?>: <input type="button" class="wpProQuiz_questionList_containers_view_student wpProQuiz_questionList_containers_view_active wpProQuiz_button2" value="<?php esc_html_e( 'Student', 'ebox' ); ?>"> <input type="button" class="wpProQuiz_questionList_containers_view_correct wpProQuiz_button2" value="<?php esc_html_e( 'Correct', 'ebox' ); ?>" /></p>
				...
				<?php if ( ! $quiz->isHideAnswerMessageBox() ) { ?>
					<div class="wpProQuiz_response" style="display: none;">
						<div style="display: none;" class="wpProQuiz_correct">
										...
										echo wp_kses_post(
											ebox_LMS::get_template(
												...
												array(
													...
													'message'      => esc_html__( 'Points', 'ebox' ),
												)
											)
										);
										?>
								...
								echo wp_kses_post(
									ebox_LMS::get_template(
										...
										array(
											...
											'message'      => esc_html__( 'Grading can be reviewed and adjusted.', 'ebox' ),
										)
									)
								);
								?>
							...
							<?php } ?>
							<<?php echo esc_attr( ebox_QUIZ_ANSWER_MESSAGE_HTML_TYPE ); ?> class="wpProQuiz_AnswerMessage"></<?php echo esc_attr( ebox_QUIZ_ANSWER_MESSAGE_HTML_TYPE ); ?>>
						</div>
					...
					<input type="button" name="skip" value="<?php echo wp_kses_post( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
						ebox_LMS::get_template(
							...
							array(
								...
								'message'      => sprintf( esc_html_x( 'Skip %s', 'placeholder: question', 'ebox' ), ebox_get_custom_label_lower( 'question' ) ),
							)
						)
					...
					ebox_LMS::get_template(
						...
						array(
							...
							'message'      => sprintf(
								...
								esc_html_x( '%s Summary', 'Quiz Summary', 'ebox' ),
				...
				<input type="button" name="checkSingle" value="<?php echo wp_kses_post( // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentBeforeOpen,Squiz.PHP.EmbeddedPhp.ContentAfterOpen
					ebox_LMS::get_template(
						...
						array(
							...
							'message'      => sprintf(
								// translators: placeholder: Quiz.
								esc_html_x( 'Finish %s', 'placeholder: Quiz', 'ebox' ),
								ebox_Custom_Label::get_label( 'quiz' ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output
							),
						)
					)
