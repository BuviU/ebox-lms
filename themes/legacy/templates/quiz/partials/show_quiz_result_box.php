...
<div style="display: none;" class="wpProQuiz_sending">
	<h4 class="wpProQuiz_header"><?php esc_html_e( 'Results', 'ebox' ); ?></h4>
	<p>
		<div>
		<?php
		echo wp_kses_post(
			ebox_LMS::get_template(
				...
				array(
					...
					'message'      => sprintf( esc_html_x( '%s complete. Results are being recorded.', 'placeholder: Quiz', 'ebox' ), ebox_Custom_Label::get_label( 'quiz' ) ),
				)
			)
		);
		?>
...
<div style="display: none;" class="wpProQuiz_results">
		...
		echo wp_kses_post(
			ebox_LMS::get_template(
				...
				array(
					...
					'message'      => '<p>' . sprintf( esc_html_x( '%1$s of %2$s %3$s answered correctly', 'placeholder: correct answer, question count, questions', 'ebox' ), '<span class="wpProQuiz_correct_answer">0</span>', '<span>' . $question_count . '</span>', ebox_get_custom_label( 'questions' ) ) . '</p>',
			...
			echo wp_kses_post(
				ebox_LMS::get_template(
					...
					array(
						...
						'message'      => sprintf( esc_html_x( 'Your time: %s', 'placeholder: quiz time.', 'ebox' ), '<span></span>' ),
					)
				)
			);
		?>
		...
		echo wp_kses_post(
			ebox_LMS::get_template(
				...
				array(
					...
					'message'      => esc_html__( 'Time has elapsed', 'ebox' ),
				)
			)
		);
		?>
			...
			echo wp_kses_post(
				ebox_LMS::get_template(
					...
					array(
						...
						'message'      => sprintf( esc_html_x( 'You have reached %1$s of %2$s point(s), (%3$s)', 'placeholder: points earned, points total', 'ebox' ), '<span>0</span>', '<span>0</span>', '<span>0</span>' ),
			...
			echo wp_kses_post(
				ebox_LMS::get_template(
					...
					array(
						...
						'message'      => sprintf( esc_html_x( 'Earned Point(s): %1$s of %2$s, (%3$s)', 'placeholder: points earned, points total, points percentage', 'ebox' ), '<span>0</span>', '<span>0</span>', '<span>0</span>' ),
						'placeholders' => array( '0', '0', '0' ),
					)
				)
			);
			...
			echo wp_kses_post(
				ebox_LMS::get_template(
					...
					array(
						...
						'message'      => sprintf( esc_html_x( '%1$s Essay(s) Pending (Possible Point(s): %2$s)', 'placeholder: number of essays, possible points', 'ebox' ), '<span>0</span>', '<span>0</span>' ),
					...
					echo wp_kses_post(
						ebox_LMS::get_template(
							...
							array(
								...
								'message'      => esc_html__( 'Average score', 'ebox' ),
							)
						)
					);
				?>
				...
				echo wp_kses_post(
					ebox_LMS::get_template(
						...
						array(
							...
							'message'      => esc_html__( 'Your score', 'ebox' ),
						)
					)
				);
			?>
	...
	<div class="wpProQuiz_catOverview" <?php $quiz_view->isDisplayNone( $quiz->isShowCategoryScore() ); ?>>
		...
		echo wp_kses_post(
			ebox_LMS::get_template(
				...
				array(
					'quiz_post_id' => $quiz->getID(),
					'context'      => 'ebox_categories_header',
					'message'      => esc_html__( 'Categories', 'ebox' ),
				)
			)
		);
		?>
					...
					$cat->setCategoryName(
						wp_kses_post(
							ebox_LMS::get_template(
								...
								array(
									'quiz_post_id' => $quiz->getID(),
									'context'      => 'ebox_not_categorized_messages',
									'message'      => esc_html__( 'Not categorized', 'ebox' ),
								)
							)
						)
					);
	...
	<div class="ld-quiz-actions" style="margin: 10px 0px;">
		<?php
			/**
			 *  See snippet https://developers.ebox.com/hook/show_quiz_continue_buttom_on_fail/
			 *
			 * @since 2.3.0.2
			 */
			$show_quiz_continue_buttom_on_fail = apply_filters( 'show_quiz_continue_buttom_on_fail', false, ebox_get_quiz_id_by_pro_quiz_id( $quiz->getId() ) );
		?>
							...
							ebox_LMS::get_template(
								...
								array(
									...
									'message'      => sprintf(
										...
										esc_html_x( 'Restart %s', 'Restart Quiz Button Label', 'ebox' ),
							...
							ebox_LMS::get_template(
								...
								array(
									...
									'message'      => sprintf(
										...
										esc_html_x( 'View %s', 'View Questions Button Label', 'ebox' ),
				...
				echo wp_kses_post(
					ebox_LMS::get_template(
						...
						array(
							...
							'message'      => esc_html__( 'Show leaderboard', 'ebox' ),
						)
					)
				); // phpcs:ignore Generic.WhiteSpace.ScopeIndent.Incorrect
					?>" /><?php // phpcs:ignore Squiz.PHP.EmbeddedPhp.ContentAfterEnd ?>
