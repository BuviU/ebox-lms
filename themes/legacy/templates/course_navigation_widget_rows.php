...
<?php
if ( ! empty( $modules ) ) {
	foreach ( $modules as $course_lesson ) {

		$current_topic_ids = '';
		$topics            = ebox_topic_dots( $course_lesson['post']->ID, false, 'array' );

		if ( ( isset( $widget_instance['show_lesson_quizzes'] ) ) && ( $widget_instance['show_lesson_quizzes'] == true ) ) {
			$lesson_quiz_list = ebox_get_lesson_quiz_list( $course_lesson['post']->ID, get_current_user_id(), $course_id );
		} else {
		...
		if ( $is_current_lesson ) {
			$lesson_list_class .= ' ebox-current-menu-ancestor ';
		}
		...
		<div class='<?php echo esc_attr( $lesson_list_class ); ?>' id='lesson_list-<?php echo esc_attr( $course_lesson['post']->ID ); ?>'>
			...
			if ( $is_current_lesson ) {
				$current_lesson_class .= ' ebox-current-menu-item ';
			}
			?>

			<div class="list_modules">
				<div class="lesson <?php echo $current_lesson_class; ?>" >
					<a href='<?php echo esc_attr( ebox_get_step_permalink( $course_lesson['post']->ID, $course_id ) ); ?>'><?php echo $course_lesson['post']->post_title; ?></a>
				</div>

				<?php if ( ! empty( $topics ) ) { ?>
					<div id='ebox_topic_dots-<?php echo esc_attr( $course_lesson['post']->ID ); ?>' class="flip ebox_topic_widget_list"  style='<?php echo ( strpos( $list_arrow_class, 'collapse' ) !== false ) ? 'display:none' : ''; ?>'>
						<ul>
							<?php
								$odd_class = '';
							...
							foreach ( $topics as $key => $topic ) {
								...
								$current_topic_class = ( $topic->ID == $widget_instance['current_step_id'] ) ? 'ebox-current-menu-item' : '';
								?>
									<li class="<?php echo $current_topic_class; ?>">
										<span class="topic_item"><a class='<?php echo esc_attr( $completed_class ); ?>' href='<?php echo esc_url( ebox_get_step_permalink( $topic->ID, $course_id ) ); ?>' title='<?php echo esc_html( $topic->post_title ); ?>'><span><?php echo apply_filters( 'the_title', $topic->post_title, $topic->ID ); ?></span></a></span>
									<?php
									if ( ( isset( $widget_instance['show_topic_quizzes'] ) ) && ( $widget_instance['show_topic_quizzes'] == true ) ) {
										$topic_quiz_list = ebox_get_lesson_quiz_list( $topic->ID, get_current_user_id(), $course_id );
										if ( ! empty( $topic_quiz_list ) ) {
											?>
												<ul id="ebox-lesson-quiz-list-<?php echo $topic->ID; ?>" class="ebox-topic-quiz-list">
												<?php
												foreach ( $topic_quiz_list as $quiz ) {
													$quiz_completed      = ebox_is_quiz_complete( get_current_user_id(), $quiz['post']->ID, $course_id );
													$completed_class     = empty( $quiz_completed ) ? 'topic-notcompleted' : 'topic-completed';
													$current_topic_class = ( $quiz['post']->ID == $widget_instance['current_step_id'] ) ? 'ebox-current-menu-item' : '';
													if ( ! empty( $current_topic_class ) ) {
																							$lesson_topic_child_item_active = true;
													}
													?>
													<li class="quiz-item <?php echo $current_topic_class; ?>">
														<span class="topic_item"><a class='<?php echo esc_attr( $completed_class ); ?>' href='<?php echo esc_url( ebox_get_step_permalink( $quiz['post']->ID, $course_id ) ); ?>' title='<?php echo esc_html( apply_filters( 'the_title', $quiz['post']->post_title, $quiz['post']->ID ) ); ?>'><span><?php echo apply_filters( 'the_title', $quiz['post']->post_title, $quiz['post']->ID ); ?></span></a></span>
													</li>
													<?php
												}
												?>
										...
										foreach ( $lesson_quiz_list as $quiz ) {
											...
											?>
												<li class="quiz-item <?php echo $current_topic_class; ?>">
													...
													href="<?php echo esc_url( ebox_get_step_permalink( $quiz['post']->ID, $course_id ) ); ?>"
				...
				<script type="text/javascript">
					jQuery( function() {
						...
						if ( jQuery('#course_navigation #lesson_list-<?php echo $course_lesson['post']->ID; ?>').hasClass('inactive') ) {
							...
							jQuery('#course_navigation #lesson_list-<?php echo $course_lesson['post']->ID; ?>').addClass('ebox-current-menu-ancestor');
...
if ( $show_course_quizzes == true ) {
	if ( ( isset( $widget_instance['show_course_quizzes'] ) ) && ( $widget_instance['show_course_quizzes'] == true ) ) {
		...
		$course_quiz_list = ebox_get_course_quiz_list( $course_id, get_current_user_id() );
		if ( ! empty( $course_quiz_list ) ) {
			?>
			<ul id="ebox-course-quiz-list-<?php echo $course_id; ?>" class="ebox-course-quiz-list ebox_navigation_lesson_topics_list">
			<?php
