<?php
/**
 * ebox LD30 Displays a single topic row
 *
 * Available Variables:
 *
...
$attributes = ebox_get_course_step_attributes( $topic_id, $course_id, $user_id );
...
$topic_class = apply_filters(
	'ebox-topic-row-class',
	'ld-table-list-item-preview ld-primary-color-hover ld-topic-row ' .
								( $topic->completed ? 'ebox-complete' : 'ebox-incomplete' )
 ...
 */
$topic_status = apply_filters( 'ebox-topic-status', ( $topic->completed ? 'completed' : 'not-completed' ), $topic, $course_id );

$ebox_topic_class_visible_after = '';
$ebox_topic_available_date = ebox_course_step_available_date( $topic->ID, $course_id, $user_id, true );
if ( ! empty( $ebox_topic_available_date ) ) {
	$ebox_topic_class_visible_after .= ' ebox-not-available';
}
 ...
 */
do_action( 'ebox-topic-row-before', $topic->ID, $course_id, $user_id ); ?>
<div class="ld-table-list-item <?php echo esc_attr( 'ld-table-list-item-' . $topic->ID ); ?> <?php echo esc_attr( $ebox_topic_class_visible_after ); ?>" id="<?php echo esc_attr( 'ld-table-list-item-' . $topic->ID ); ?>">
	<a class="<?php echo esc_attr( $topic_class ); ?>" href="<?php echo esc_url( ebox_get_step_permalink( $topic->ID, $course_id ) ); ?>">
		<?php
		/**
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-topic-row-status-before', $topic->ID, $course_id, $user_id );
		?>

		<?php ebox_status_icon( $topic_status, $topic->post_type, null, true ); ?>
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-topic-row-title-before', $topic->ID, $course_id, $user_id );
		?>
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-topic-row-title-after', $topic->ID, $course_id, $user_id );
		?>
	</a>
	<?php
		if ( ! empty( $attributes ) ) :
 ...
 * @param int $user_id   User ID.
 */
do_action( 'ebox-topic-quiz-row-before', $topic->ID, $course_id, $user_id );

$topic_quizzes = ebox_get_lesson_quiz_list( $topic, null, $course_id );
 ...
 * @param int $user_id   User ID.
 */
do_action( 'ebox-topic-quiz-row-after', $topic->ID, $course_id, $user_id );
 ...
 * @param int $user_id   User ID.
 */
do_action( 'ebox-topic-row-after', $topic->ID, $course_id, $user_id );
