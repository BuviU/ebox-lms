<?php
/**
 * ebox LD30 Displays a single quiz row
 *
 * Available Variables:
 *
...
$quiz_classes = ebox_quiz_row_classes( $quiz, $context );
$is_sample    = ( isset( $lesson['sample'] ) ? $lesson['sample'] : false );

$attributes = ebox_get_course_step_attributes( $quiz['post']->ID, $course_id, $user_id );
 ...
 */
$atts = apply_filters( 'ebox_quiz_row_atts', ( isset( $has_access ) && ! $has_access && ! $is_sample ? 'data-ld-tooltip="' . esc_html__( "You don't currently have access to this content", 'ebox' ) . '"' : '' ) );

$ebox_quiz_available_date = ebox_course_step_available_date( $quiz['post']->ID, $course_id, $user_id, true );
if ( ! empty( $ebox_quiz_available_date ) ) {
	$quiz_classes['wrapper'] .= ' ebox-not-available';
}
 ...
 */
do_action( 'ebox-quiz-row-before', $quiz['post']->ID, $course_id, $user_id ); ?>
<div id="<?php echo esc_attr( 'ld-table-list-item-' . $quiz['post']->ID ); ?>" class="<?php echo esc_attr( $quiz_classes['wrapper'] ); ?> <?php echo esc_attr( 'ld-table-list-item-' . $quiz['post']->ID ); ?>" <?php echo wp_kses_post( $atts ); ?>>
	<div class="<?php echo esc_attr( $quiz_classes['preview'] ); ?>">
		<a class="<?php echo esc_attr( $quiz_classes['anchor'] ); ?>" href="<?php echo esc_url( ebox_get_step_permalink( $quiz['post']->ID, $course_id ) ); ?>">
			<?php
			/**
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-quiz-row-status-before', $quiz['post']->ID, $course_id, $user_id );

			ebox_status_icon( $quiz['status'], 'ebox-quiz', null, true );
			/**
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-quiz-row-title-before', $quiz['post']->ID, $course_id, $user_id );
			?>
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-quiz-row-title-after', $quiz['post']->ID, $course_id, $user_id );
			?>
		</a>
		<?php
		if ( ! empty( $attributes ) ) :
 ...
 * @param int $user_id   User ID.
 */
do_action( 'ebox-quiz-row-after', $quiz['post']->ID, $course_id, $user_id );
