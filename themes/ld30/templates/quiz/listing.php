<?php
/**
 * ebox LD30 Displays a quiz listing
 *
 * Available Variables:
 *
...
if ( ! empty( $quizzes ) ) :
	 ...
	 */
	do_action( 'ebox-' . $context . '-quiz-list-before', get_the_ID(), $course_id, $user_id );
	$is_sample = false;
	if ( ( isset( $lesson_id ) ) && ( ! empty( $lesson_id ) ) ) {
		$is_sample = ebox_get_setting( $lesson_id, 'sample_lesson' );
	}
	...
	<div class="<?php echo esc_attr( $table_class ); ?>">
		...
		<div class="ld-table-list-header ld-primary-background">
			 ...
			 * @param int       $user_id   User ID.
			 */
			do_action( 'ebox-' . $context . '-quiz-list-heading-before', get_the_ID(), $course_id, $user_id );
			?>
			<?php // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Method escapes output?>
			<div class="ld-table-list-title"><?php echo ebox_Custom_Label::get_label( 'quizzes' ); ?></div>
			 ...
			 * @param int       $user_id   User ID.
			 */
			do_action( 'ebox-' . $context . '-quiz-list-progress-before', get_the_ID(), $course_id, $user_id );
			?>
			...
			<div class="ld-table-list-lesson-details"></div>
				 ...
				 * @since 3.0.0
				 */
				do_action( 'ebox-' . $context . '-quiz-list-progress-after', get_the_ID(), $course_id, $user_id );
				?>
				 ...
				 * @since 3.0.0
				 */
				do_action( 'ebox-' . $context . '-quiz-list-heading-after', get_the_ID(), $course_id, $user_id );
				?>
	 ...
	 * @since 3.0.0
	 */
	do_action( 'ebox-' . $context . '-quiz-list-after', get_the_ID(), $course_id, $user_id );
	?>
