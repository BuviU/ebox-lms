<?php
/**
 * ebox LD30 Displays a lesson content (topics and quizzes)
 *
 * Available Variables:
 *
...
$lesson_progress = ebox_lesson_progress( $lesson['post'], $course_id );
$is_sample       = ebox_is_sample( $lesson['post'] );
 ...
 */
do_action( 'ebox-topic-list-before', $lesson['post']->ID, $course_id, $user_id ); ?>
...
<div class="
	...
	<div class="ld-table-list-header ld-primary-background">
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-topic-list-heading-before', $lesson['post']->ID, $course_id, $user_id );
		?>
		...
		<div class="ld-table-list-title">
			...
			<span class="ld-text">
				...
				echo sprintf( esc_html_x( '%s Content', 'Lesson Content Label', 'ebox' ), esc_attr( ebox_Custom_Label::get_label( 'lesson' ) ) );
				?>
			</span>
		...
		<div class="ld-table-list-lesson-details">
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-topic-list-progress-before', $lesson['post']->ID, $course_id, $user_id );
			?>
			...
			<?php if ( $lesson_progress ) : ?>
				 ...
				 * @param int     $user_id       User ID
				 */
				if ( true === (bool) apply_filters( 'ebox_show_lesson_list_progress', true, $lesson['post']->ID, $course_id, $user_id ) ) {
					?>
					<span class="ld-lesson-list-progress">
					<?php
					echo sprintf(
						// translators: placeholder: Lesson Complete Percentage.
						esc_html_x( '%s%% Complete', 'Lesson Complete Percentage', 'ebox' ),
				 ...
				 * @param int     $user_id    User ID
				 */
				if ( true === (bool) apply_filters( 'ebox_show_lesson_list_steps', true, $lesson['post']->ID, $course_id, $user_id ) ) {
					?>
					<span class="ld-lesson-list-steps">
					<?php
					echo sprintf(
						// translators: placeholder: %1$s: Lesson Steps Complete %2$s: Total lesson steps.
						esc_html_x( '%1$d/%2$d Steps', '%1$s: Lesson Steps Complete %2$s: Total lesson steps', 'ebox' ),
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-topic-list-progress-after', $lesson['post']->ID, $course_id, $user_id );
			?>

			<?php if ( 'ebox-lesson' === get_post_type() ) : ?>
				<span class="ld-expand-button" data-ld-expands="<?php echo esc_attr( 'ld-topic-list-' . $lesson['post']->ID ); ?>">
					...
					<span class="ld-text"><?php esc_html_e( 'Expand', 'ebox' ); ?></span>
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-topic-list-heading-after', $lesson['post']->ID, $course_id, $user_id );
		?>
	...
	<div class="ld-table-list-footer">
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-lesson-pagination-before', $lesson['post']->ID, $course_id, $user_id );
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-lesson-pagination-after', $lesson['post']->ID, $course_id, $user_id );
		?>
 ...
 * @param int $user_id   User ID.
 */
do_action( 'ebox-topic-list-after', $lesson['post']->ID, $course_id, $user_id ); ?>
