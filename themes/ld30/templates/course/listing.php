<?php
/**
 * ebox LD30 Displays the listing of course content
 *
 * Available Variables:
...
if ( ! empty( $modules ) || ! empty( $quizzes ) ) :

	/**
	 * Filters ebox Course table CSS class.
	 *
	 * @since 3.0.0
	 *
	 * @param string $course_table_class CSS classes for course table.
	 */
	$table_class = apply_filters( 'ebox_course_table_class', 'ld-item-list-items ' . ( isset( $lesson_progression_enabled ) && $lesson_progression_enabled ? 'ld-lesson-progression' : '' ) );
	...
	<div class="<?php echo esc_attr( $table_class ); ?>" id="<?php echo esc_attr( 'ld-item-list-' . $course_id ); ?>" data-ld-expand-id="<?php echo esc_attr( 'ld-item-list-' . $course_id ); ?>" data-ld-expand-list="true">
		 ...
		 */
		do_action( 'ebox-course-listing-before', $course_id, $user_id );
		...
		if ( $modules && ! empty( $modules ) ) :
			...
			$sections = ebox_30_get_course_sections( $course_id );
			$i        = 0;
		 ...
		 * @param int     $user_id             User ID.
		 */
		$show_course_quizzes = apply_filters( 'ebox-show-course-quizzes', $show_course_quizzes, $course_id, $user_id );
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-course-listing-after', $course_id, $user_id );
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-course-pagination-before', $course_id, $user_id );
		...
		if ( isset( $course_pager_results['pager'] ) ) :
			ebox_get_template_part(
				...
				array(
					...
					'pager_context' => ( isset( $context ) ? $context : 'course_modules' ),
					'course_id'     => $course_id,
				),
				true
			);
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-course-pagination-after', $course_id, $user_id );
		?>
