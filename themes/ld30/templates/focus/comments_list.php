<?php
/**
 * ebox LD30 focus mode prints the comments list output.
 *
 * @since 3.0.0
 *
...
if ( ! function_exists( 'ebox_focus_mode_comments_list' ) ) {
	/**
	 ...
	 */
	function ebox_focus_mode_comments_list( $comment, $args, $depth ) {
		global $wp_roles;
		global $post;
		...
		if ( ! empty( $roles ) ) {
			foreach ( $roles as $role ) :
				...
				if ( 'administrator' === $role || 'team_leader' === $role ) {
		...
		$ebox_avatar_class = empty( get_avatar( $comment->comment_author_email ) ) ? ' ld-no-avatar-image' : '';

		?>

		<div <?php comment_class( 'ld-focus-comment ptype-' . $post->post_type . ' ' . $role_classes . $ebox_avatar_class ); ?> id="comment-<?php comment_ID(); ?>">
			<div class="ld-comment-wrapper">
			<?php if ( '0' == $comment->comment_approved ) : ?>
				<span class="ld-comment-alert"><?php esc_html_e( 'Your response is awaiting for approval.', 'ebox' ); ?></span>
			<?php endif; ?>
				...
				<div class="ld-comment-avatar">
					...
					<span class="ld-comment-avatar-author">
						...
						printf(
							...
							esc_html_x( '%1$s at %2$s', 'placeholders: comment date, comment time', 'ebox' ),
					...
					<div class="ld-comment-reply">
						...
						comment_reply_link(
							array_merge(
								$args,
								array(
									'reply_text' => esc_html__( 'Reply', 'ebox' ),
									'after'      => '',
									'depth'      => $depth,
...
if ( ! isset( $_GET['replytocom'] ) ) {
	 ...
	 */
	$args = apply_filters(
		'ebox_focus_mode_list_comments_args',
		array(
			'style'    => 'div',
			'page'     => get_query_var( 'cpage', 1 ),
			'callback' => 'ebox_focus_mode_comments_list',
		)
	);
