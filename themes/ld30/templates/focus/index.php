...
if ( have_posts() ) :
	while ( have_posts() ) :
		 ...
		 */
		do_action( 'ebox-focus-header-before', $course_id, $user_id );
	...
	<div class="ld-focus-main">
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-focus-masthead-before', $course_id, $user_id );
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-focus-masthead-after', $course_id, $user_id );
		?>
		...
		<div class="ld-focus-content">
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-focus-content-title-before', $course_id, $user_id );
			?>
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-focus-content-content-before', $course_id, $user_id );
			?>
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-focus-content-content-after', $course_id, $user_id );
			?>

			<?php
				wp_link_pages(
					array(
						'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'ebox' ),
						'after'  => '</div>',
					)
				);
			?>
			 ...
			 */
			if ( comments_open() || ( apply_filters( 'ebox_focus_mode_show_existing_comments', false ) ) ) {
				if ( has_filter( 'ebox_focus_mode_can_view_comments' ) ) {
					/**
					...
					apply_filters_deprecated(
						'ebox_focus_mode_can_view_comments',
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-focus-content-end', $course_id, $user_id );
			?>
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-focus-content-footer-before', $course_id, $user_id );
		 ...
		 * @param int $user_id   User ID.
		 */
		do_action( 'ebox-focus-content-footer-after', $course_id, $user_id );
	...
	ebox_get_template_part(
		...
		array(
			'type'    => 'warning',
			'icon'    => 'alert',
			'message' => esc_html__( 'No content found at this address', 'ebox' ),
		),
		true
	);
