...
$header['logo'] = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Theme_LD30', 'login_logo' );
if ( ! empty( $header['logo'] ) ) {
	 ...
	 * This filter will be called only if there is a logo set in ebox plugin settings.
	 *
	 * @since 3.1.0
	 *
	 ...
	 */
	$header['logo_alt'] = apply_filters( 'ebox_focus_header_logo_alt', $header['logo_alt'], $course_id, $user_id );
	/**
	 ...
	 * @param int    $user_id   User ID.
	 */
	$header['logo_url'] = apply_filters( 'ebox_focus_header_logo_url', get_home_url(), $course_id, $user_id );
} else {
	/**
	 * Filters Focus mode header text. This text is used to display in place of the logo.
	 * This filter will be called only if there is no logo set in ebox plugin settings.
	 *
	 * @since 3.1.0
	 *
	 ...
	 * @param int    $user_id     User ID.
	 */
	$header['text'] = apply_filters( 'ebox_focus_header_text', '', $course_id, $user_id );
	if ( ! empty( $header['text'] ) ) {
		 ...
		 * @param int    $user_id         User ID.
		 */
		$header['text_url'] = apply_filters( 'ebox_focus_header_text_url', '', $course_id, $user_id );
	}
}
?>
<div class="ld-focus-header">
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-focus-header-mobile-nav-before', $course_id, $user_id );
	?>

	<div class="ld-mobile-nav">
		<a href="#" class="ld-trigger-mobile-nav" aria-label="<?php esc_attr_e( 'Menu', 'ebox' ); ?>">
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-focus-header-logo-before', $course_id, $user_id );
	?>
	...
	if ( ! empty( $header['logo'] ) ) {
		 ...
		 * @param int    $user_id        User ID.
		 */
		$header_element = apply_filters( 'ebox_focus_header_element', $header_element, $header, $course_id, $user_id );
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-focus-header-logo-after', $course_id, $user_id );
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-focus-header-nav-before', $course_id, $user_id );

	$can_complete = false;
	if ( ( ! ebox_lesson_progression_enabled( $course_id ) ) || ( true === ebox_can_user_bypass( $user_id, 'ebox_course_progression' ) ) ) {
		if ( ebox_user_is_course_children_progress_complete( $user_id, $course_id, $post->ID ) ) {
			$can_complete = true;
		} else {
			$can_complete = false;
		}
	} else {
		$can_complete = ebox_can_complete_step( $user_id, $post->ID, $course_id, true );
	}
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-focus-header-nav-after', $course_id, $user_id );
	?>
	...
	<?php if ( is_user_logged_in() ) : ?>
		<div class="ld-user-menu">
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-focus-header-user-menu-before', $course_id, $user_id );
			...
			echo sprintf(
				...
				esc_html_x( 'Hello, %s!', 'Focus mode welcome placeholder', 'ebox' ),
				/**
			...
			<span class="ld-profile-avatar">
				 ...
				 * @param int $user_id   User ID.
				 */
				do_action( 'ebox-focus-header-avatar-before', $course_id, $user_id );
				 ...
				 * @param int $user_id   User ID.
				 */
				do_action( 'ebox-focus-header-avatar-after', $course_id, $user_id );
				?>
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-focus-header-user-dropdown-before', $course_id, $user_id );
			?>

			<span class="ld-user-menu-items">
				<?php
				$custom_menu_items = ebox_30_get_custom_focus_menu_items();
				...
				$menu_items = array(
					'course-home' => array(
						...
						'label' => sprintf(
							...
							esc_html_x( '%s Home', 'Placeholder for course home link', 'ebox' ),
				 ...
				 * @param int   $user_id   User ID.
				 */
				$menu_items = apply_filters( 'ebox_focus_header_user_dropdown_items', $menu_items, $course_id, $user_id );
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-focus-header-user-dropdown-after', $course_id, $user_id );
			?>
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-focus-header-usermenu-after', $course_id, $user_id );
	?>
</div> <!--/.ld-focus-header-->
