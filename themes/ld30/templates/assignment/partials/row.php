<?php
/**
 * ebox LD30 Display lesson/topic assignment uploads listing row
 *
 * Available Variables:
 *
...
<div class="ld-table-list-item">
	<div class="ld-table-list-item-preview">
		 ...
		 */
		do_action( 'ebox-assignment-row-before', $assignment, get_the_ID(), $course_id, $user_id );
		?>
		...
		<div class="ld-table-list-title">
			 ...
			 * @param int     $user_id    User ID.
			 */
			do_action( 'ebox-assignment-row-delete-before', $assignment, get_the_ID(), $course_id, $user_id );

			// Delete assignment link.
			if ( ! ebox_is_assignment_approved_by_meta( $assignment->ID ) ) :
				if ( ( isset( $post_settings['lesson_assignment_deletion_enabled'] ) && 'on' === $post_settings['lesson_assignment_deletion_enabled'] && absint( $assignment->post_author ) === absint( $user_id ) ) || ( ebox_is_admin_user( $user_id ) ) || ( ebox_is_team_leader_of_user( $user_id, $assignment->post_author ) ) ) :
					?>
				<a href="<?php echo esc_url( add_query_arg( 'ebox_delete_attachment', $assignment->ID ) ); ?>" title="<?php esc_html_e( 'Delete this uploaded Assignment', 'ebox' ); ?>">
					<span class="ld-icon ld-icon-delete" aria-label="<?php esc_html_e( 'Delete Assignment', 'ebox' ); ?>"></span>
				</a>
					<?php
				endif;
			endif;
			 ...
			 * @param int     $user_id    User ID.
			 */
			do_action( 'ebox-assignment-row-title-before', $assignment, get_the_ID(), $course_id, $user_id );
			?>

			<a href='<?php echo esc_url( get_post_meta( $assignment->ID, 'file_link', true ) ); ?>' target="_blank">
				<span class="ld-item-icon">
					<span class="ld-icon ld-icon-download" aria-label="<?php esc_html_e( 'Download Assignment', 'ebox' ); ?>"></span>
				</span>
			</a>
			 ...
			 * @param int     $user_id    User ID.
			 */
			do_action( 'ebox-assignment-row-title-after', $assignment, get_the_ID(), $course_id, $user_id );
			?>
		...
		<div class="ld-table-list-columns">
			...
			if ( post_type_supports( 'ebox-assignment', 'comments' ) && apply_filters( 'comments_open', $assignment->comment_status, $assignment->ID ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- WP Core filter
				...
				if ( true === (bool) $assignment_post_type_object->publicly_queryable ) {
					...
					<a href='<?php echo esc_url( get_comments_link( $assignment->ID ) ); ?>' data-ld-tooltip="
						...
						echo sprintf(
							...
							esc_html_x( '%d Comments', 'placeholder: comment count', 'ebox' ),
				...
				<span class="ld-status ld-status-complete">
					...
					if ( $assignment_points ) :
						echo sprintf(
							...
							esc_html_x( '%1$s/%2$s Points Awarded ', 'placeholders: points current, points max', 'ebox' ),
	 ...
	 * @param int     $user_id    User ID.
	 */
	do_action( 'ebox-assignment-row-after', $assignment, get_the_ID(), $course_id, $user_id );
	?>
	</div>
</div>
