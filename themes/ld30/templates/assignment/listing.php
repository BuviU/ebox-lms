<?php
/**
 * ebox LD30 Display lesson/topic assignment uploads listing.
 *
...
if ( isset( $course_step_post ) && $course_step_post instanceof WP_Post ) :
	 ...
	 */
	do_action( 'ebox-assignment-alerts-before', $course_step_post->ID, $course_id, $user_id );
	...
	if ( ! empty( $assignment_messages ) && is_array( $assignment_messages ) ) :
		foreach ( $assignment_messages as $assignment_message ) :
			if ( ( isset( $assignment_message['message'] ) && ! empty( $assignment_message['message'] ) ) && ( isset( $assignment_message['type'] ) && ! empty( $assignment_message['type'] ) ) ) :
				 ...
				 * @param int $user_id             User ID.
				 */
				$message = apply_filters( 'ebox_assignment_upload_message', $message, $course_step_post->ID, $course_id, $user_id );
	 ...
	 * @param int $user_id             User ID.
	 */
	do_action( 'ebox-assignment-alerts-after', $course_step_post->ID, $course_id, $user_id );
	 ...
	 * @param int $user_id             User ID.
	 */
	do_action( 'ebox-assignment-list-before', $course_step_post->ID, $course_id, $user_id ); ?>

	<?php
	$assignment_stats = ebox_get_assignment_progress( $assignments );
	...
	if ( absint( $assignment_stats['complete'] ) !== absint( $assignment_stats['total'] ) ) :
		 ...
		 * @param int $user_id             User ID.
		 */
		do_action( 'ebox-assignment-list-before-aproval-alert', $course_step_post->ID, $course_id, $user_id );
		...
		ebox_get_template_part(
			...
			array(
				'type'    => 'warning',
				'icon'    => 'alert',
				'message' => ( $approval_needed > 1 ? __( 'You have assignments awaiting approval.', 'ebox' ) : __( 'You have an assignment awaiting approval.', 'ebox' ) ),
			),
			true
		);
		 ...
		 * @param int $user_id             User ID.
		 */
		do_action( 'ebox-assignment-list-after-aproval-alert', $course_step_post->ID, $course_id, $user_id );
	 ...
	 * @param int $user_id             User ID.
	 */
	do_action( 'ebox-assignment-list-after-alert-before-table', $course_step_post->ID, $course_id, $user_id );
	?>
	...
	<div class="ld-table-list ld-assignment-list">
		<div class="ld-table-list-header ld-primary-background">
			<div class="ld-table-list-title">
				 ...
				 * @param int $user_id             User ID.
				 */
				do_action( 'ebox-assignment-list-table-header-before', $course_step_post->ID, $course_id, $user_id );
				?>
				<span class="ld-item-icon">
					<span class="ld-icon ld-icon-assignment"></span>
				</span>
				<?php
				echo esc_html_e( 'Assignments', 'ebox' );
				/**
				 ...
				 * @param int $user_id             User ID.
				 */
				do_action( 'ebox-assignment-list-table-header-after', $course_step_post->ID, $course_id, $user_id );
				?>
			</div>
			<div class="ld-table-list-columns">
				...
				$columns = apply_filters(
					'ebox-assignment-list-columns',
					array(
						'ld-assignment-column-approved' => sprintf(
							// translators: placeholders: assignment count approved, assignment count total.
							esc_html_x( '%1$d/%2$d Approved', 'placeholders: assignment count approved, assignment count total', 'ebox' ),
				...
				foreach ( $columns as $class => $label ) :
					...
					<div class="<?php echo esc_attr( 'ld-table-list-column ' . $class ); ?>">
						 ...
						 * @param int $user_id             User ID.
						 */
						do_action( 'ebox-assignment-list-table-before-column-' . $class, $course_step_post->ID, $course_id, $user_id );
						 ...
						 * @param int $user_id             User ID.
						 */
						do_action( 'ebox-assignment-list-table-after-column-' . $class, $course_step_post->ID, $course_id, $user_id );
						?>
					</div>
				<?php endforeach; ?>
			</div>
		 ...
		 * @param int $user_id             User ID.
		 */
		do_action( 'ebox-assignment-list-header-after', $course_step_post->ID, $course_id, $user_id );
		?>
		...
		<div class="ld-table-list-items">
			...
			if ( ! empty( $assignments ) ) :
				...
				endforeach;
					...
					esc_html_x( 'No assignments submitted at this time', 'No assignments message', 'ebox' );
				 ...
				 * @param int $user_id             User ID.
				 */
				do_action( 'ebox-assignment-list-after', $course_step_post->ID, $course_id, $user_id );
			 ...
			 * @param int $user_id             User ID.
			 */
			do_action( 'ebox-assignment-upload-after', $course_step_post->ID, $course_id, $user_id );
			?>
