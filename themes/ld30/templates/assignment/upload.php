<?php
/**
 * ebox LD30 Display lesson/topic assignment uploads form.
 *
...
if ( isset( $post_settings['assignment_upload_limit_size'] ) && ! empty( $post_settings['assignment_upload_limit_size'] ) ) {
	if ( ebox_return_bytes_from_shorthand( $post_settings['assignment_upload_limit_size'] ) < ebox_return_bytes_from_shorthand( $php_max_upload ) ) {
...
$upload_message = sprintf(
	...
	esc_html_x( 'Maximum upload file size: %s', 'placeholder: PHP file upload size', 'ebox' ),
	$php_max_upload
);

if ( isset( $post_settings['assignment_upload_limit_extensions'] ) && ! empty( $post_settings['assignment_upload_limit_extensions'] ) ) {
	$limit_file_exts = ebox_validate_extensions( $post_settings['assignment_upload_limit_extensions'] );
	if ( ! empty( $limit_file_exts ) ) {
		$upload_message .= ' ' . sprintf(
			// translators: placeholder: Comma separated list of file extentions.
			esc_html_x( 'Allowed file types: %s', 'placeholder: Comma separated list of file extentions', 'ebox' ),
 ...
 */
do_action( 'ebox-assignment-uploads-before', $course_step_post->ID, $course_id, $user_id ); ?>
...
<div class="ld-file-upload">
	...
	<div class="ld-file-upload-heading">
		 ...
		 * @param int $user_id             User ID.
		 */
		do_action( 'ebox-assignment-uploads-heading-before', $course_step_post->ID, $course_id, $user_id );

		esc_html_e( 'Upload Assignment', 'ebox' );
		?>
		 ...
		 * @param int $user_id             User ID.
		 */
		do_action( 'ebox-assignment-uploads-heading-after', $course_step_post->ID, $course_id, $user_id );
		?>
	</div>
	...
	<form name="uploadfile" id="uploadfile_form" class="ld-file-upload-form" method="POST" enctype="multipart/form-data" action="" accept-charset="utf-8" >
		...
		<label for="uploadfiles">
			<strong><?php echo esc_html_e( 'Browse', 'ebox' ); ?></strong>
			<span><?php echo esc_html_e( 'No file selected', 'ebox' ); ?></span>
		</label>
		 ...
		 * @param int $user_id             User ID.
		 */
		do_action( 'ebox-assignment-uploads-form', $course_step_post->ID, $course_id, $user_id );
		?>

		<input class="ld-button" type="submit" value="<?php esc_html_e( 'Upload', 'ebox' ); ?>" id="uploadfile_btn" disabled="true">

		<input type="hidden" name="MAX_FILE_SIZE" value="<?php echo ebox_return_bytes_from_shorthand( $php_max_upload ); ?>" />
		<input type="hidden" value="<?php echo esc_attr( $course_step_post->ID ); ?>" name="post"/>
		<input type="hidden" value="<?php echo esc_attr( ebox_get_course_id( $course_step_post->ID ) ); ?>" name="course_id"/>
	...
	<div class="ld-file-upload-message">
		 ...
		 * @param int $user_id             User ID.
		 */
		do_action( 'ebox-assignment-uploads-message', $course_step_post->ID, $course_id, $user_id );
		?>
	</div>
	 ...
	 * @param int $user_id             User ID.
	 */
	do_action( 'ebox-assignment-uploads-message-after', $course_step_post->ID, $course_id, $user_id );
	?>
