...
<?php
	 ...
	 */
	do_action( 'ebox-quiz-before', $quiz_post->ID, $course_id, $user_id );
	...
	if ( ! empty( $lesson_progression_enabled ) ) :
		$last_incomplete_step = ebox_is_quiz_accessable( $user_id, $quiz_post, true, $course_id );
		if ( ! empty( $user_id ) ) {
			if ( ebox_user_progress_is_step_complete( $user_id, $course_id, $quiz_post->ID ) ) {
				$show_content = true;
			} else {
				if ( $bypass_course_limits_admin_users ) {
					...
					$previous_step_post_id = ebox_user_progress_get_parent_incomplete_step( $user_id, $course_id, $quiz_post->ID );
					if ( ( ! empty( $previous_step_post_id ) ) && ( $previous_step_post_id !== $quiz_post->ID ) ) {
						...
						$previous_step_post_id     = ebox_user_progress_get_previous_incomplete_step( $user_id, $course_id, $quiz_post->ID );
						$previous_lesson_completed = true;
						if ( ( ! empty( $previous_step_post_id ) ) && ( $previous_step_post_id !== $quiz_post->ID ) ) {
							$previous_lesson_completed = false;
					 ...
					 * @param int  $user_id                   User ID.
					 */
					$previous_lesson_completed = apply_filters( 'ebox_previous_step_completed', $previous_lesson_completed, $quiz_post->ID, $user_id );
				}

				$show_content = $previous_lesson_completed;
			}

			if ( ( ebox_is_sample( $quiz_post ) ) /* && ( true !== (bool) $has_access ) */ ) {
				$show_content = true;
			} elseif ( ( $last_incomplete_step ) && ( is_a( $last_incomplete_step, 'WP_Post' ) ) ) {
				$show_content = false;

				$sub_context = '';
				if ( 'on' === ebox_get_setting( $last_incomplete_step->ID, 'lesson_video_enabled' ) ) {
					if ( ! empty( ebox_get_setting( $last_incomplete_step->ID, 'lesson_video_url' ) ) ) {
						if ( 'BEFORE' === ebox_get_setting( $last_incomplete_step->ID, 'lesson_video_shown' ) ) {
							if ( ! ebox_video_complete_for_step( $last_incomplete_step->ID, $course_id, $user_id ) ) {
				 ...
				 * @param int $user_id   User ID.
				 */
				do_action( 'ebox-quiz-progression-before', $quiz_post->ID, $course_id, $user_id );
				 ...
				 * @param int $user_id   User ID.
				 */
				do_action( 'ebox-quiz-progression-after', $quiz_post->ID, $course_id, $user_id );
	...
	if ( $show_content ) :
		...
		if ( $attempts_left ) :
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-quiz-actual-content-before', $quiz_post->ID, $course_id, $user_id );
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-quiz-actual-content-after', $quiz_post->ID, $course_id, $user_id );
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-quiz-attempts-alert-before', $quiz_post->ID, $course_id, $user_id );
			 ...
			 * @param int $user_id   User ID.
			 */
			do_action( 'ebox-quiz-attempts-alert-after', $quiz_post->ID, $course_id, $user_id );
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-quiz-after', $quiz_post->ID, $course_id, $user_id );
