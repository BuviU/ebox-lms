<?php
/**
 * ebox LD30 Displays course navigation quiz row
 *
 * @since 3.0.0
 *
...
$classes = array(
	'container' => 'ld-lesson-item' . ( get_the_ID() === absint( $quiz['post']->ID ) ? ' ld-is-current-lesson ' : '' ) . ( 'completed' === $quiz['status'] ? ' ebox-complete' : ' ebox-incomplete' ),
...
if ( isset( $context ) && 'lesson' === $context ) {
	$classes['container'] = 'ld-table-list-item' . ( 'completed' === $quiz['status'] ? ' ebox-complete' : ' ebox-incomplete' );
...
if ( ! empty( $ebox_quiz_available_date ) ) {
	$classes['wrapper'] .= ' ebox-not-available';
}
?>

<div class="<?php echo esc_attr( $classes['container'] ); ?>">
	<div class="<?php echo esc_attr( $classes['wrapper'] ); ?>">
		<a class="<?php echo esc_attr( $classes['anchor'] ); ?>" href="<?php echo esc_url( ebox_get_step_permalink( $quiz['post']->ID, $course_id ) ); ?>">

			<?php ebox_status_icon( $quiz['status'], 'ebox-quiz', null, true ); ?>
