<?php
/**
 * ebox LD30 Displays course navigation lesson row
 *
 * @since 3.0.0
 *
 * @package ebox\Templates\LD30\Widgets
 */
...
if ( isset( $post ) && is_object( $post ) && isset( $post->post_type ) ) {
	if ( in_array( $post->post_type, array( 'ebox-modules', 'ebox-topic', 'ebox-quiz' ), true ) ) {
		if ( 'ebox-modules' === $post->post_type ) {
			...
			$current_lesson_id = ebox_course_get_single_parent_step( $course_id, $post->ID, 'ebox-modules' );
		}
	}
}
...
$bypass_course_limits_admin_users = ebox_can_user_bypass( $user_id, 'ebox_course_lesson_access_from', $lesson['post']->ID, $course_id );
 ...
 */
$lesson_class = apply_filters( 'ebox-nav-widget-lesson-class', $lesson_class, $lesson );
...
<div class="<?php echo esc_attr( $lesson_class ); ?>">
	<div class="ld-lesson-item-preview">
		<a class="ld-lesson-item-preview-heading ld-primary-color-hover" href="<?php echo esc_url( ebox_get_step_permalink( $lesson['post']->ID, $course_id ) ); ?>">
			...
			ebox_status_icon( $status, 'ebox-lesson', null, true );
			?>
		...
		if ( $expandable ) :
			...
			/**
			 * Filters the auto-expanding control of modules in Focus Mode sidebar.
			 *
			 * @since 3.0.0
			 *
			 ...
			 */
			$expand_class  = apply_filters( 'ebox-nav-widget-expand-class', ( $is_current_lesson ? 'ld-expanded' : '' ), $lesson['post']->ID, $course_id, $user_id );
			...
			echo sprintf( esc_html_x( 'Expand %s', 'placeholder: Lesson', 'ebox' ), esc_html( ebox_get_custom_label( 'lesson' ) ) );
			?>
			...
			<div class="ld-table-list ld-topic-list">
				<div class="ld-table-list-items">
					...
					if ( isset( $widget_instance['show_lesson_quizzes'] ) && true === (bool) $widget_instance['show_lesson_quizzes'] ) :
						...
						$show_lesson_quizzes = apply_filters( 'ebox-show-lesson-quizzes', $show_lesson_quizzes, $lesson['post']->ID, $course_id, $user_id );
