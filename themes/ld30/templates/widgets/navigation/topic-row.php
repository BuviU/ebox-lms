<?php
/**
 * ebox LD30 Displays the course navigation widget topic row.
 *
 * @since 3.0.0
 *
 ...
 */
$wrapper_class = apply_filters( 'ebox-topic-row-wrapper-class', 'ld-table-list-item' . ( $topic->completed ? ' ebox-complete' : ' ebox-incomplete' ), $topic );
...
$topic_class = apply_filters( 'ebox-topic-row-class', $topic_class, $topic );

/** This filter is documented in themes/ld30/templates/topic/partials/row.php */
$topic_status = apply_filters( 'ebox-topic-status', ( $topic->completed ? 'completed' : 'not-completed' ) );

$attributes = ebox_get_course_step_attributes( $topic->ID, $course_id, $user_id );

$ebox_available_date = ebox_course_step_available_date( $topic->ID, $course_id, $user_id, true );
if ( ! empty( $ebox_available_date ) ) {
	$wrapper_class .= ' ebox-not-available';
}
?>
<div class="<?php echo esc_attr( $wrapper_class ); ?>">
	<a class="<?php echo esc_attr( $topic_class ); ?>" href="<?php echo esc_url( ebox_get_step_permalink( $topic->ID, $course_id ) ); ?>">

		<?php ebox_status_icon( $topic_status, 'ebox-topic', null, true ); ?>
