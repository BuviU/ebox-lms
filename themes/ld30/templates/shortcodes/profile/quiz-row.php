<?php
/**
 * ebox LD30 Displays a user's profile quiz row.
 *
 * @since 3.0.0
 *
...
<div class="ld-table-list-item <?php echo esc_attr( $status ); ?>">
	<div class="ld-table-list-item-preview">
		...
		<div class="ld-table-list-title">
			<a href="<?php echo esc_url( $quiz_link ); ?>"><?php echo wp_kses_post( ebox_status_icon( $status, 'ebox-quiz' ) ); ?><span><?php echo wp_kses_post( $quiz_title ); ?></span></a>
		...
		<div class="ld-table-list-columns">
			...
			if ( $certificate_link && ! empty( $certificate_link ) ) {
				$certificate_link = '<a class="ld-certificate-link" href="' . $certificate_link . '" target="_new" aria-label="' . __( 'Certificate', 'ebox' ) . '"><span class="ld-icon ld-icon-certificate"></span></a>';
			}

			/**
			 * Filters ebox profile quiz column list.
			 *
			 * @since 3.0.0
			 *
			...
			$quiz_columns = apply_filters(
				...
				array(
					...
					'date'        => array(
						...
						'content' => ebox_adjust_date_time_display( $quiz_attempt['time'] ),
						'class'   => '',
					),
				),
				$quiz_attempt,
		...
		<div class="ld-table-list-item-expanded">
			<div class="ld-table-list ld-essay-list">
				<div class="ld-table-list-header">
					<div class="ld-table-list-title">
						<?php echo esc_html_e( 'Essays', 'ebox' ); ?>
					</div> <!--/.ld-table-list-title-->
					<div class="ld-table-list-columns">
						<?php
						...
						$columns = apply_filters(
							'ebox-essay-column-headings',
							array(
								array(
									'id'    => 'comments',
									'label' => __( 'Comments', 'ebox' ),
								),
								array(
									'id'    => 'status',
									'label' => __( 'Status', 'ebox' ),
								),
								array(
									'id'    => 'points',
									'label' => __( 'Points', 'ebox' ),
								),
							)
						);
