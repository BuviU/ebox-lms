<?php
/**
 * ebox LD30 Displays the status of a user's course in the ld_profile shortode
 *
 * Available Variables:
...
$since  = ebox_user_team_enrolled_to_course_from( $user_id, $course_id );
if ( empty( $since ) ) {
...
<div class="<?php echo esc_attr( $course_class ); ?>" id="<?php echo esc_attr( 'ld-course-list-item-' . $course_id ); ?>">
	<div class="ld-item-list-item-preview">
		...
		<a href="<?php echo esc_url( get_the_permalink( $course_id ) ); ?>" class="ld-item-name">
			<?php ebox_status_icon( $status, get_post_type( 'ebox-courses' ), null, true ); ?>
			<span class="ld-item-title">
				...
				$components = array(
					...
					'progress' => sprintf( esc_html_x( '%s%% Complete', 'User Status Course Progress', 'ebox' ), $progress['percentage'] ),
					// translators: User Status Course Steps.
					'steps'    => sprintf( esc_html_x( '%1$d/%2$d Steps', 'User Status Course Steps', 'ebox' ), $progress['completed'], $progress['total'] ),
				);

				if ( ! empty( $since ) ) {
					// translators: User Status Course Since.
					$components['since'] = sprintf( esc_html_x( 'Since %s', 'User Status Course Since', 'ebox' ), ebox_adjust_date_time_display( $since ) );
				}
				 ...
				 */
				$components = apply_filters( 'ebox_user_status_course_components', $components );
				?>
