<?php
/**
 * ebox LD30 Displays the login modal
 *
 * @since 3.0.0
 *
...
<div class="ld-modal ld-login-modal<?php echo ( $can_register ? ' ld-can-register' : '' ); ?>">
	...
	<div class="ld-login-modal-login">
		<div class="ld-login-modal-wrapper">
			 ...
			 * @since 3.0.0
			 */
			do_action( 'ebox-login-modal-heading-before' );
			?>
			<div class="ld-modal-heading">
				<?php echo esc_html_e( 'Login', 'ebox' ); ?>
			</div>
			<?php
			/**
			 ...
			 * @since 3.0.0
			 */
			do_action( 'ebox-login-modal-heading-after' );

			if ( in_array( get_post_type(), ebox_get_post_types( 'course' ), true ) ) {
				?>
				<div class="ld-modal-text">
					<?php
					echo sprintf(
						// translators: placeholder: course.
						esc_html_x( 'Accessing this %s requires a login. Please enter your credentials below!', 'placeholder: course', 'ebox' ),
			 ...
			 * @since 3.0.0
			 */
			do_action( 'ebox-login-modal-text-after' );
			if ( isset( $_GET['login'] ) && 'failed' === $_GET['login'] ) :
				...
				ebox_get_template_part(
					 ...
					 * @since 3.0.0
					 */
					do_action( 'ebox-login-modal-alert-after' );
			...
			elseif ( isset( $_GET['ld-resetpw'] ) && 'true' === $_GET['ld-resetpw'] ) : // cspell:disable-line.
				...
				ebox_get_template_part(
					...
					array(
						'type'    => 'warning',
						'icon'    => 'alert',
						'message' => __( 'Please check your email for the password reset link.', 'ebox' ),
					),
					true
				);
			...
			<div class="ld-login-modal-form">
				 ...
				 * @since 3.0.0
				 */
				do_action( 'ebox-login-modal-form-before' );
				 ...
				 */
				$login_form_args = apply_filters( 'ebox-login-form-args', $login_form_args );
				 ...
				 * @since 3.0.0
				 */
				do_action( 'ebox-login-modal-form-after' );
				...
				$lost_password_url = ebox_add_login_hash( $lost_password_url );
				$lost_password_url = wp_lostpassword_url( $lost_password_url );
				if ( ebox_reset_password_is_enabled() ) {
					$lost_password_url = get_permalink( ebox_get_reset_password_page_id() );
				}
				?>
				<a class="ld-forgot-password-link" href="<?php echo esc_url( $lost_password_url ); ?>"><?php esc_html_e( 'Lost Your Password?', 'ebox' ); ?></a>

				<?php
				$logo_id = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Theme_LD30', 'login_logo' );
				 ...
				 * @since 3.0.0
				 */
				do_action( 'ebox-login-modal-after' );
				?>
	...
	if ( $can_register ) :
		...
		if ( ! is_multisite() ) {
			$ld_registration_page_id = (int) ebox_Settings_Section::get_section_setting( 'ebox_Settings_Section_Registration_Pages', 'registration' );

			if ( ! empty( $ld_registration_page_id ) ) {
				$register_url = get_permalink( $ld_registration_page_id );
				if ( in_array( get_post_type(), ebox_get_post_type_slug( array( 'course', 'team' ) ), true ) ) {
					// If we are showing on a Course or Team we inlcude the 'ld_register_id' query string param.
		...
		/**
		 * Filters the ebox login modal registration URL.
		 *
		 * @since 3.1.0
		 *
		 * @param string $register_url Login modal registration url.
		 */
		$register_url = apply_filters( 'ebox_login_model_register_url', $register_url );
		/**
		 * Filters the ebox login modal registration header text.
		 *
		 * @since 3.1.0
		 *
		 * @param string $registration_header Login modal registration header text.
		 */
		$register_header = apply_filters( 'ebox_login_model_register_header', esc_html__( 'Register', 'ebox' ) );
		/**
		 * Filters the ebox login modal registration text.
		 *
		 * @since 3.1.0
		 *
		 * @param string $registration_text   Login Modal registration text.
		 */
		$register_text     = apply_filters( 'ebox_login_model_register_text', esc_html__( 'Don\'t have an account? Register one!', 'ebox' ) );
		$errors_conditions = ebox_login_error_conditions();
		?>
		<div class="ld-login-modal-register">
			<div class="ld-login-modal-wrapper">
				<div class="ld-content">
					 ...
					 * @since 3.0.0
					 */
					do_action( 'ebox-register-modal-heading-before' );
					?>
					<div class="ld-modal-heading">
					 ...
					 * @since 3.0.0
					 */
					do_action( 'ebox-register-modal-heading-after' );
					?>
					 ...
					 * @since 3.0.0
					 */
					do_action( 'ebox-register-modal-text-after' );
					...
					if ( $errors['has_errors'] ) :
						ebox_get_template_part(
							 ...
							 */
							do_action( 'ebox-register-modal-errors-after', $errors );
					...
					elseif ( isset( $_GET['ld-registered'] ) && 'true' === $_GET['ld-registered'] ) :
						...
						ebox_get_template_part(
							...
							array(
								'type'    => 'success',
								'icon'    => 'alert',
								'message' => __( 'Registration successful, please check your email to set your password.', 'ebox' ),
							),
							true
						);
						 ...
						 * @param array $errors An array of error details.
						 */
						do_action( 'ebox-register-successful-after', $errors );

					endif;

					if ( '#ld-user-register' === $register_url ) {
						/**
						 * Filters the ebox Login modal register button CSS class.
						 *
						 * @since 3.1.0
						 *
						 * @param string $register_button_class Register button CSS class.
						 */
						$register_button_class = apply_filters( 'ebox_login_model_register_button_class', 'ld-js-register-account' );
					} else {

						/** This filter is documented in themes/ld30/templates/modules/login-modal.php */
						$register_button_class = apply_filters( 'ebox_login_model_register_button_class', '' );
					}
					?>
					<a href="<?php echo esc_url( $register_url ); ?>" class="ld-button ld-button-reverse <?php echo esc_attr( $register_button_class ); ?>"><?php echo esc_html_e( 'Register an Account', 'ebox' ); ?></a>
					 ...
					 * @since 3.0.0
					 */
					do_action( 'ebox-register-modal-registration-link-after' );
					?>
				...
				if ( '#ld-user-register' === $register_url ) {
					...
					<div id="ld-user-register" class="ld-hide">
						<?php
						if ( has_action( 'ebox_register_modal_register_form_override' ) ) {
							/**
							 * Allow for replacement of the defaut ebox Registration form
							 *
							 * @since 3.2.0
							 */
							do_action( 'ebox_register_modal_register_form_override' );
						} else {
							/**
							 * Fires before the register modal heading.
							 *
							 * @since 3.0.0
							 */
							do_action( 'ebox-register-modal-register-form-before' );
							if ( is_multisite() ) {
							...
							<form name="registerform" id="registerform" action="<?php echo esc_url( $register_action_url ); ?>" method="post" novalidate="novalidate">
								<p>
									<label for="user_reg_login"><?php esc_html_e( 'Username', 'ebox' ); ?><br />
									<input type="text" name="<?php echo esc_attr( $field_name_login ); ?>" id="user_reg_login" class="input" value="" size="20" /></label>
								</p>
								<p>
									<label for="user_reg_email"><?php esc_html_e( 'Email', 'ebox' ); ?><br />
								...
								if ( in_array( $post_type, ebox_get_post_types( 'course' ), true ) ) {
									...
									if ( ( ! empty( $course_id ) ) && ( in_array( ebox_get_setting( $course_id, 'course_price_type' ), array( 'free' ) ) ) && ( apply_filters( 'ebox_registration_form_include_course', true, $course_id ) ) ) {
										?>
										<input name="ebox-registration-form-post" value="<?php echo absint( $course_id ); ?>" type="hidden" />
										<?php
										wp_nonce_field( 'ebox-registration-form-post-' . absint( $course_id ) . '-nonce', 'ebox-registration-form-post-nonce' );
									}
								} elseif ( in_array( $post_type, array( ebox_get_post_type_slug( 'team' ) ), true ) ) {
									$team_id = get_the_ID();

									/** This filter is documented in themes/ld30/includes/login-register-functions.php */
									if ( ( ! empty( $team_id ) ) && ( in_array( ebox_get_setting( $team_id, 'team_price_type' ), array( 'free' ) ) ) && ( apply_filters( 'ebox_registration_form_include_team', true, $team_id ) ) ) {
										?>
										<input name="ebox-registration-form-post" value="<?php echo absint( $team_id ); ?>" type="hidden" />
										<?php
										wp_nonce_field( 'ebox-registration-form-post-' . absint( $team_id ) . '-nonce', 'ebox-registration-form-post-nonce' );
									}
								}
								...
								$redirect_to_url = ebox_add_login_hash( $redirect_to_url );
								?>
								...
								<p id="reg_passmail"><?php esc_html_e( 'Registration confirmation will be emailed to you.', 'ebox' ); ?></p>
								<p class="submit"><input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e( 'Register', 'ebox' ); ?>" /></p>
							</form>
							<?php
							/**
							 * Fires before the register modal heading.
							 ...
							 * @since 3.0.0
							 */
							do_action( 'ebox-register-modal-register-form-after' );
						}
						?>
				 ...
				 * @since 3.0.0
				 */
				do_action( 'ebox-register-modal-register-wrapper-after' );
				?>
			</div> <!--/.ld-login-modal-wrapper-->
