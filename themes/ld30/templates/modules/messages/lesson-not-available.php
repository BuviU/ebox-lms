<?php
/**
 * ebox LD30 Displays the Course Lesson Not Available message
 *
 * Available Variables:
...
$message = sprintf(
	wp_kses_post(
		...
		__( '<span class="ld-display-label">Available on:</span> <span class="ld-display-date">%s</span>', 'ebox' )
	),
	esc_html( ebox_adjust_date_time_display( $lesson_access_from_int ) )
);
...
if ( ( 'lesson' === $context ) || ( 'topic' === $context ) || ( 'quiz' === $context ) ) {
	...
	if ( ! empty( $course_id ) ) {
		$button = array(
			...
			'label'         => ebox_get_label_course_step_back( ebox_get_post_type_slug( 'course' ) ),
			'icon'          => 'arrow-left',
			'icon-location' => 'left',
		);
	}
...
<div class="ebox-wrapper">
	...
	ebox_get_template_part(
		...
		array(
			 ...
			 */
			'message' => apply_filters( 'ebox_lesson_available_from_text', $message, get_post( $lesson_id ), $lesson_access_from_int ),
		),
		true
	);
	?>
