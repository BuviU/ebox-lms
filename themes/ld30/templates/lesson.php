...
$in_focus_mode = ebox_Settings_Section::get_section_setting( 'ebox_Settings_Theme_LD30', 'focus_mode_enabled' );
...
<div class="<?php echo esc_attr( ebox_the_wrapper_class() ); ?>">
	 ...
	 */
	do_action( 'ebox-lesson-before', get_the_ID(), $course_id, $user_id );
	...
	if ( $show_content ) :
		...
		if ( ( defined( 'ebox_TEMPLATE_CONTENT_METHOD' ) ) && ( 'shortcode' === ebox_TEMPLATE_CONTENT_METHOD ) ) {
			...
			if ( ! empty( $topics ) || ! empty( $quizzes ) ) :
				 ...
				 * @param int $user_id   User ID.
				 */
				do_action( 'ebox-lesson-content-list-before', get_the_ID(), $course_id, $user_id );
				 ...
				 * @param int $user_id   User ID.
				 */
				do_action( 'ebox-lesson-content-list-after', get_the_ID(), $course_id, $user_id );
	...
	if ( ( defined( 'ebox_TEMPLATE_CONTENT_METHOD' ) ) && ( 'shortcode' === ebox_TEMPLATE_CONTENT_METHOD ) ) {
		...
		if ( $all_quizzes_completed && $logged_in && ! empty( $course_id ) ) :
			 ...
			 * @param int     $user_id      User ID.
			 */
			$can_complete = apply_filters( 'ebox-lesson-can-complete', $can_complete, get_the_ID(), $course_id, $user_id );
		endif;
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-lesson-after', get_the_ID(), $course_id, $user_id );
