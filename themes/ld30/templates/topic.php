...
<div class="<?php echo esc_attr( ebox_the_wrapper_class() ); ?>">
	 ...
	 */
	do_action( 'ebox-topic-before', get_the_ID(), $course_id, $user_id );

	if ( ( defined( 'ebox_TEMPLATE_CONTENT_METHOD' ) ) && ( 'shortcode' === ebox_TEMPLATE_CONTENT_METHOD ) ) {
		$shown_content_key = 'ebox-shortcode-wrap-ld_infobar-' . absint( $course_id ) . '_' . (int) get_the_ID() . '_' . absint( $user_id );
	...
	$sub_context = '';
	if ( ( $lesson_progression_enabled ) && ( ! ebox_user_progress_is_step_complete( $user_id, $course_id, $post->ID ) ) ) {
		$previous_item = ebox_get_previous( $post );
		if ( ( ! $previous_topic_completed ) || ( empty( $previous_item ) ) ) {
			if ( 'on' === ebox_get_setting( $lesson_post->ID, 'lesson_video_enabled' ) ) {
				if ( ! empty( ebox_get_setting( $lesson_post->ID, 'lesson_video_url' ) ) ) {
					if ( 'BEFORE' === ebox_get_setting( $lesson_post->ID, 'lesson_video_shown' ) ) {
						if ( ! ebox_video_complete_for_step( $lesson_post->ID, $course_id, $user_id ) ) {
	...
	if ( ( $lesson_progression_enabled ) && ( ! empty( $sub_context ) || ! $previous_topic_completed || ! $previous_lesson_completed ) && ! ebox_can_user_bypass() ) {

		if ( ( ! ebox_is_sample( $post ) ) /* || ( ebox_is_sample( $post ) && true === (bool) $has_access ) */ ) {
	...
	if ( $show_content ) :
		...
		if ( ( defined( 'ebox_TEMPLATE_CONTENT_METHOD' ) ) && ( 'shortcode' === ebox_TEMPLATE_CONTENT_METHOD ) ) {
			$shown_content_key = 'ebox-shortcode-wrap-course_content-' . absint( $course_id ) . '_' . (int) get_the_ID() . '_' . absint( $user_id );
			if ( false === strstr( $content, $shown_content_key ) ) {
			 ...
			 */
			if ( ebox_lesson_hasassignments( $post ) && ! empty( $user_id ) ) : // cspell:disable-line.
				$bypass_course_limits_admin_users = ebox_can_user_bypass( $user_id, 'ebox_lesson_assignment' );
				$course_children_steps_completed  = ebox_user_is_course_children_progress_complete( $user_id, $course_id, $post->ID );
				if ( ( ebox_lesson_progression_enabled() && $course_children_steps_completed ) || ! ebox_lesson_progression_enabled() || $bypass_course_limits_admin_users ) :
					 ...
					 * @param int $user_id   User ID.
					 */
					do_action( 'ebox-lesson-assignment-before', get_the_ID(), $course_id, $user_id );
					 ...
					 * @param int $user_id   User ID.
					 */
					do_action( 'ebox-lesson-assignment-after', get_the_ID(), $course_id, $user_id );
	...
	if ( ( defined( 'ebox_TEMPLATE_CONTENT_METHOD' ) ) && ( 'shortcode' === ebox_TEMPLATE_CONTENT_METHOD ) ) {
		$shown_content_key = 'ebox-shortcode-wrap-ld_navigation-' . absint( $course_id ) . '_' . (int) get_the_ID() . '_' . absint( $user_id );
		if ( false === strstr( $content, $shown_content_key ) ) {
		...
		if ( $all_quizzes_completed && $logged_in && ! empty( $course_id ) ) :
			...
			$can_complete = apply_filters( 'ebox-lesson-can-complete', true, get_the_ID(), $course_id, $user_id );
		endif;
	 ...
	 * @param int $user_id   User ID.
	 */
	do_action( 'ebox-topic-after', get_the_ID(), $course_id, $user_id );
