<?php
/**
 * ebox LD30 shortcodes functions
 *
 * @since 3.0.0
 *
...
/**
 * Builds the `ebox_login` shortcode output.
 *
 * @param array  $atts {
 ...
 */
function ebox_login_shortcode( $atts = array(), $content = '' ) {
	...
	/**
	 * Filters ebox_login shortcode attributes.
	 *
	 * @since 3.0.0
	 *
	 * @param array $attributes Shortcode attributes.
	 */
	$atts = apply_filters( 'ebox_login_shortcode_atts', $atts );

	$filter_args   = array();
	$filter_action = '';

	if ( 'logout' === $atts['action'] ) {
		$filter_action = 'ebox-login-shortcode-logout';
		...
		if ( false === (bool) $atts['label'] ) {
			if ( ! empty( $atts['logout_label'] ) ) {
				...
				$filter_args['label'] = __( 'Logout', 'ebox' );
			}
		} else {
		...
		$filter_action = 'ebox-login-shortcode-login';
		...
		if ( false === (bool) $atts['label'] ) {
			if ( ! empty( $atts['login_label'] ) ) {
				...
				$filter_args['label'] = __( 'Login', 'ebox' );
			}
		} else {
			$filter_args['label'] = $atts['label'];
		}
	 ...
	 */
	$filter_args['url'] = apply_filters( 'ebox_login_url', $filter_args['url'], $atts['action'], $atts );

	if ( ! empty( $filter_action ) ) {
		/**
		 * Filters the filter arguments depending on the action which is login and logout.
		 *
		 * The dynamic part depends on the action attribute it will be ebox-login-shortcode-login for the login action and ebox-login-shortcode-logout for the logout action.
		 *
	...
	if ( empty( $atts['return'] ) ) {
		...
		echo '<div class="ebox-wrapper"><a class="' . esc_attr( $filter_args['class'] ) . '" href="' . esc_attr( $filter_args['url'] ) . '">';
		...
		if ( ! in_array( get_post_type(), ebox_get_post_types( 'course' ), true ) && ! is_user_logged_in() && 'yes' === $atts['login_model'] ) {
...
add_shortcode( 'ebox_login', 'ebox_login_shortcode' );
