...
jQuery(function ($) {
	...
	eboxFocusModeSidebarAutoScroll();
	...
	$('body').on('click', function(e) {
		if ($('.ebox-wrapper').hasClass('ld-modal-open')) {
	...
	*/
	...
	if ('' == $('.registration-login-link').attr('href')) {
		$('.registration-login-link').on('click', function (e) {
			e.preventDefault();
			$('#ebox_registerform, .registration-login').hide();
			$(
		...
		$('.show-register-form').on('click', function (e) {
			...
			$('#ebox_registerform, .registration-login').show();
		});
	}
	...
	function ld_expand_element(elm, collapse) {
		...
		if (
			...
			var elmParentWrapper = elm.parents('.ebox-wrapper');
		}
		if (
			'undefined' === typeof elmParentWrapper ||
			!elmParentWrapper.length
		...
		if ($(elm)[0] && $(elm)[0].hasAttribute('data-ld-expands')) {
			...
			if (
				...
				console.log('ebox: No expandable content was found');
			}
			positionTooltips();
		}
	}
	...
	function initTooltips() {
		...
		if ($('#ebox-tooltips').length) {
			$('#ebox-tooltips').remove();
			$tooltips = $('*[data-ld-tooltip]');
		}

		if ($tooltips.length) {
			$('body').prepend('<div id="ebox-tooltips"></div>');
			var $ctr = 1;
	...
	function initLoginModal() {
		var modal_wrapper = $('.ebox-wrapper-login-modal');
		if ('undefined' !== typeof modal_wrapper && modal_wrapper.length) {
			// Move the model to be first element of the body. See ebox-3503
	...
	$('body').on('click', '.ld-pagination a', function (e) {
		...
		} else if ('course_modules' == linkVars.context) {
		...
		if ('team_courses' == linkVars.context) {
			linkVars.team_id = $(this).data('team_id');
			if ('undefined' !== typeof linkVars.team_id) {
				parent_container = $(parentVars.currentTarget).parents(
					'.ld-team-courses-' + linkVars.team_id
				);
				if (
					'undefined' !== typeof parent_container &&
					parent_container.length
				...
				if ('course_content_shortcode' == linkVars.context) {
					...
					if (
						...
						if (
							...
							if (
								'undefined' !== typeof response.data.nav_modules
							) {
								$(parentVars.parent_container)
									.html(response.data.nav_modules)
									.removeClass('ld-loading');
							}
						} else if (
							...
							'undefined' !== typeof response.data.modules
						) {
							$(parentVars.parent_container)
								.html(response.data.modules)
								.removeClass('ld-loading');
						}
					} else {
						if ($('#ld-item-list-' + linkVars.course_id).length) {
							if ('undefined' !== typeof response.data.modules) {
								$('#ld-item-list-' + linkVars.course_id)
									.html(response.data.modules)
									.removeClass('ld-loading');
							}
	...
	if ($('#ebox_timer').length) {
		var timer_el = jQuery('#ebox_timer');
		...
		if ('undefined' !== typeof cookie_key) {
			var cookie_name = 'ebox_timer_cookie_' + cookie_key;
		} else {
			var cookie_name = 'ebox_timer_cookie';
		}
		...
		$(timer_button_el).on('ebox-time-finished', function () {
			$(timer_el).hide();
		});
	}

	$(document).on('ebox_video_disable_assets', function (event, status) {
		if ('undefined' === typeof ebox_video_data) {
			return false;
		}

		if ('BEFORE' == ebox_video_data.videos_shown) {
			if (true == status) {
	...
	function ebox_ld30_show_user_statistic(e) {
		e.preventDefault();
		...
		jQuery.ajax({
			...
			success(reply_data) {
				if ('undefined' !== typeof reply_data.html) {
					...
					//console.log('trigger event change - ebox.js');
					jQuery('body').trigger(
						'ebox-statistics-contentchanged'
					);
	 ...
	 * to the active step.
	 */
	function eboxFocusModeSidebarAutoScroll() {
		if (jQuery('.ebox-wrapper .ld-focus').length) {
			var sidebar_wrapper = jQuery(
				'.ebox-wrapper .ld-focus .ld-focus-sidebar-wrapper'
			);

			var sidebar_current_topic = jQuery(
				'.ebox-wrapper .ld-focus .ld-focus-sidebar-wrapper .ld-is-current-item'
			);
			if (
				...
				var sidebar_current_lesson = jQuery(
					'.ebox-wrapper .ld-focus .ld-focus-sidebar-wrapper .ld-is-current-lesson'
				);
				if (
			...
			if (
				...
				var offset_top = 0;
				if (
					jQuery('.ebox-wrapper .ld-focus .ld-focus-header')
						.length
				) {
					var logo_height = jQuery(
						'.ebox-wrapper .ld-focus .ld-focus-header'
					).height();
				...
				if (
					jQuery(
						'.ebox-wrapper .ld-focus .ld-focus-sidebar .ld-focus-sidebar-wrapper'
					).length
				) {
	...
	function update_payment_forms(data) {
		...
		$('form.ebox-stripe-checkout input[name="stripe_price"]').val(
			data.total.stripe_value
		);

		// Remove Stripe Connect session to respect the new amount.
		const stripe_course_id = $(
			'.ebox-stripe-checkout input[name="stripe_course_id"]'
		).val();
	...
	$('.btn-join').on('click', function (e) {
		...
		if (0 === total) {
			$.ajax({
				...
				cache: false,
				data: {
					action: 'ebox_enroll_with_zero_price',
